...
class CrmLead(models.Model):
    ...
    def assign_partner(self, partner_id=False):
        ...
        for lead in self:
            ...
            if partner.user_id:
                lead._handle_salesmen_assignment(
                    user_ids=partner.user_id.ids, team_id=partner.team_id.id
                )
    ...
    def update_contact_details_from_portal(self, values):
        ...
        if any([key not in fields for key in values]):
            raise UserError(
                _("Not allowed to update the following field(s) : %s.")
    ...
    def _get_access_action(self, access_uid=None, force_website=False):
        ...
        if access_uid:
            try:
                ...
                return super(CrmLead, self)._get_access_action(
