...
class TestUiSession(HttpCase):
    def test_admin_survey_session(self):
        ...
        with patch(
            "odoo.addons.survey.models.survey_survey.Survey.action_open_session_manager",
            action_open_session_manager_mock,
        ):
        ...
        survey_session.write(
            {"session_start_time": test_start_time - relativedelta(minutes=10)}
        )
        ...
        self.assertTrue(
            all(attendee.is_session_answer for attendee in all_attendees),
            "Created answers should be within the session.",
        )
        self.assertTrue(
            all(attendee.state == "new" for attendee in all_attendees),
        ...
        attendee_1.save_lines(
            timed_scored_choice_question,
            [timed_scored_choice_answer_1.id, timed_scored_choice_answer_3.id],
        )
        attendee_2.save_lines(
            timed_scored_choice_question,
            [timed_scored_choice_answer_1.id, timed_scored_choice_answer_2.id],
        )
        attendee_3.save_lines(
            timed_scored_choice_question, [timed_scored_choice_answer_2.id]
        )
