...
class TestLivechatBasicFlowHttpCase(tests.HttpCase, TestLivechatCommon):
    ...
    def test_livechat_username(self):
        ...
        channel_1 = self.env["mail.channel"].search(
            [
                ("livechat_visitor_id", "=", self.visitor.id),
                ("livechat_active", "=", True),
            ],
        ...
        self.assertEqual(
            channel_1.name,
            "%s %s" % (self.visitor.display_name, self.operator.livechat_username),
        )
        channel_1.unlink()
        ...
        res = self.opener.post(url=self.open_chat_url, json=self.open_chat_params)
        self.assertEqual(res.status_code, 200)
        channel_2 = self.env["mail.channel"].search(
            [
                ("livechat_visitor_id", "=", self.visitor.id),
                ("livechat_active", "=", True),
            ],
        ...
        # Check Channel naming
        self.assertEqual(
            channel_2.name, "%s %s" % (self.visitor.display_name, self.operator.name)
        )
    ...
    def test_basic_flow_with_rating(self):
        ...
        self.assertEqual(
            ...
            "Odoobot must be the sender of the 'has left the conversation' message.",
        )
        self.assertEqual(
            channel.message_ids[0].body,
            "<p>%s has left the conversation.</p>" % self.visitor.display_name,
        )
        self.assertEqual(
            channel.livechat_active,
            False,
    ...
    def test_basic_flow_without_rating(self):
        ...
        self.assertEqual(
            channel.livechat_active,
            False,
            "The livechat session must be inactive since visitor has left the conversation.",
        )
    ...
    def test_visitor_info_access_rights(self):
        ...
        livechat_info = next(
            c for c in messages_info["channels"] if c["id"] == channel.id
        )
