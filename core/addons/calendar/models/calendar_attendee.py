...
class Attendee(models.Model):
    ...
    partner_id = fields.Many2one(
        "res.partner", "Attendee", required=True, readonly=True
    )
    ...
    state = fields.Selection(
        STATE_SELECTION, string="Status", readonly=True, default="needsAction"
    )
    ...
    def _send_mail_to_attendees(self, mail_template, force_send=False):
        ...
        if self.env["ir.config_parameter"].sudo().get_param(
            "calendar.block_mail"
        ) or self._context.get("no_mail_to_attendees"):
            return False
        ...
        for attendee in self:
            if attendee.email and attendee.partner_id != self.env.user.partner_id:
                ...
                body = mail_template._render_field(
                    "body_html", attendee.ids, compute_lang=True, post_process=True
                )[attendee.id]
                subject = mail_template._render_field(
                    "subject", attendee.ids, compute_lang=True
                )[attendee.id]
                attendee.event_id.with_context(no_document=True).message_notify(
                    email_from=attendee.event_id.user_id.email_formatted
