...
class GoogleEvent(abc.Set):
    ...
    def owner(self, env):
        ...
        try:
            ...
            real_owner_id = False
        real_owner = (
            real_owner_id and env["res.users"].browse(real_owner_id) or env["res.users"]
        )
    ...
    def full_recurring_event_id(self):
        ...
        if re.match(r"(\w+_)", self.recurringEventId).group(1) != re.match(
            r"(\w+_)", self.id
        ).group(1):
            return None
    ...
    def get_meeting_url(self):
        ...
        video_meeting = list(
            filter(
                lambda entryPoints: entryPoints["entryPointType"] == "video",
                self.conferenceData["entryPoints"],
            )
        )
