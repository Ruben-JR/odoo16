...
class Meeting(models.Model):
    ...
    def write(self, values):
        ...
        notify_context = self.env.context.get("dont_notify", False)
        res = super(Meeting, self.with_context(dont_notify=notify_context)).write(
            values
        )
        if (
            recurrence_update_setting in ("all_events",)
            and len(self) == 1
            and values.keys() & self._get_google_synced_fields()
        ):
    ...
    def _odoo_values(self, google_event, default_reminders=()):
        ...
        if not reminder_command:
            reminder_command = (
                google_event.reminders.get("useDefault") and default_reminders or ()
            )
        ...
        values = {
            ...
            or self.default_get(["privacy"])["privacy"],
    ...
    def _odoo_attendee_commands(self, google_event):
        ...
        if google_event.exists(self.env):
            existing_attendees = self.browse(
                google_event.odoo_id(self.env)
            ).attendee_ids
        attendees_by_emails = {
    ...
    def _google_values(self):
        ...
        values = {
            ...
            "end": end,
            "summary": self.name,
            "description": tools.html2plaintext(self.description)
            if not tools.is_html_empty(self.description)
            else "",
