...
class Http(models.AbstractModel):
    ...
    def _generate_routing_rules(cls, modules, converters):
        ...
        rewrites = dict(
            [
                (x.url_from, x)
                for x in request.env["website.rewrite"].sudo().search(domain)
            ]
        )
    ...
    def _get_public_users(cls):
        ...
        website = request.env(user=SUPERUSER_ID)[
            "website"
        ].get_current_website()  # sudo
        if website:
    ...
    def _get_default_lang(cls):
        if getattr(request, "is_frontend", True):
            ...
            return request.env["res.lang"].browse(
                [website._get_cached("default_lang_id")]
            )
    ...
    def _serve_page(cls):
        ...
        def _search_page(comparator="="):
            page_domain = [
                ("url", comparator, req_page)
            ] + request.website.website_domain()
            return (
                request.env["website.page"]
                .sudo()
                .search(page_domain, order="website_id asc", limit=1)
            )
    ...
    def _get_values_500_error(cls, env, values, exception):
        ...
        values["editable"] = request.uid and request.env.user.has_group(
        ...
        if result and cookie_type == "optional":
            ...
            if not isinstance(accepted_cookie_types, dict):
                request.future_response.set_cookie(
                    "website_cookies_bar", expires=0, max_age=0
                )
                return False
