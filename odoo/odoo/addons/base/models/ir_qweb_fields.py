...
class IntegerConverter(models.AbstractModel):
    ...
    def get_available_options(self):
        ...
        options.update(
            format_decimalized_number=dict(
                type="boolean", string=_("Decimalized number")
            ),
...
class FloatConverter(models.AbstractModel):
    ...
    def record_to_html(self, record, field_name, options):
        if "precision" not in options and "decimal_precision" not in options:
            _, precision = record._fields[field_name].get_digits(record.env) or (
                None,
                None,
            )
...
class DateConverter(models.AbstractModel):
    ...
    def get_available_options(self):
        ...
        options.update(format=dict(type="string", string=_("Date format")))
        return options
...
class SelectionConverter(models.AbstractModel):
    ...
    def record_to_html(self, record, field_name, options):
        ...
        return super(SelectionConverter, self).record_to_html(
...
class HTMLConverter(models.AbstractModel):
    ...
    def value_to_html(self, value, options):
        ...
        body = etree.fromstring(
            "<body>%s</body>" % value, etree.HTMLParser(encoding="utf-8")
        )[0]
...
class ImageConverter(models.AbstractModel):
    ...
    def value_to_html(self, value, options):
        ...
        return Markup(
            ...
            % (Image.MIME[image.format], value.decode("ascii"))
        )
...
class MonetaryConverter(models.AbstractModel):
    ...
    def get_available_options(self):
        ...
        options.update(
            from_currency=dict(
                type="model", params="res.currency", string=_("Original currency")
            ),
...
class RelativeDatetimeConverter(models.AbstractModel):
    ...
    def record_to_html(self, record, field_name, options):
        ...
        return super(RelativeDatetimeConverter, self).record_to_html(
            record, field_name, options
        )
...
class BarcodeConverter(models.AbstractModel):
    ...
    def get_available_options(self):
        ...
        options.update(
            ...
            humanreadable=dict(
                type="integer", string=_("Human Readable"), default_value=0
            ),
    ...
    def value_to_html(self, value, options=None):
        ...
        img_element.set(
            "src", "data:image/png;base64,%s" % base64.b64encode(barcode).decode()
        )
...
class QwebView(models.AbstractModel):
    ...
    def record_to_html(self, record, field_name, options):
        ...
        if view._name != "ir.ui.view":
            _logger.warning(
                "%s.%s must be a 'ir.ui.view', got %r.", record, field_name, view._name
            )
            return ""
