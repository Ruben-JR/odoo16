...
class TestPhonenumbersPatch(BaseCase):
    def test_region_CI_monkey_patch(self):
        ...
        if (
            ...
            parsed_phonenumber_1 = phonenumbers.parse(
                "20 25/35-51 ", region="CI", keep_raw_input=True
            )
            self.assertEqual(
                parsed_phonenumber_1.national_number,
                20253551,
            ...
            parsed_phonenumber_2 = phonenumbers.parse(
                "+225 22 52 25 86 ", region="CI", keep_raw_input=True
            )
            self.assertEqual(
                parsed_phonenumber_2.national_number,
                22522586,
                "The national part of the phonenumber should be 22522586",
            )
            self.assertEqual(
                parsed_phonenumber_2.country_code,
                225,
                "The country code of Ivory Coast is 225",
            )
        else:
