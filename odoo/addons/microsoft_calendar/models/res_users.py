...
class User(models.Model):
    ...
    microsoft_synchronization_stopped = fields.Boolean(
        "Outlook Synchronization stopped", copy=False
    )
    ...
    def _is_microsoft_calendar_valid(self):
        return (
            ...
            and self.microsoft_calendar_token_validity
    ...
    def _refresh_microsoft_calendar_token(self):
        ...
        if not client_id or not client_secret:
            raise UserError(
                _("The account for the Outlook Calendar service is not configured.")
            )
        ...
        with microsoft_calendar_token(self) as token:
            try:
                events, next_sync_token = calendar_service.get_events(
                    self.microsoft_calendar_sync_token, token=token
                )
