    ...
    def refresh_cache(self):
        for cache in self:
            ...
            products = Product.search(
                cache.get_product_domain(), order="sequence,default_code,name"
            )
            ...
            cache.write(
                {
                    "cache": base64.encodebytes(
                        json.dumps(res, default=date_utils.json_default).encode("utf-8")
                    ),
                }
            )
...
class pos_config(models.Model):
    ...
    def _get_oldest_cache_time(self):
        for cache in self:
            ...
            oldest_cache = pos_cache.search(
                [("config_id", "=", cache.id)], order="write_date", limit=1
            )
            cache.oldest_cache_time = oldest_cache.write_date

    cache_ids = fields.One2many("pos.cache", "config_id")
    oldest_cache_time = fields.Datetime(
        compute="_get_oldest_cache_time", string="Oldest cache time", readonly=True
    )
    limit_products_per_request = fields.Integer(
    ...
    def _compute_limit_products_per_request(self):
        limit = (
            ...
            .sudo()
            .get_param("pos_cache.limit_products_per_request", 0)
        )
