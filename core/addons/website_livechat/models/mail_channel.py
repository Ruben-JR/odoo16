...
class MailChannel(models.Model):
        ...
        for channel in self.filtered("livechat_visitor_id"):
            ...
            try:
                channel_infos_dict[channel.id]["visitor"] = {
                    ...
                    "country_code": visitor.country_id.code.lower()
    ...
    def _get_visitor_history(self, visitor):
        ...
        return " → ".join(
            visit.page_id.name + " (" + visit.visit_datetime.strftime("%H:%M") + ")"
    ...
    def _get_visitor_leave_message(self, operator=False, cancel=False):
        name = (
            _("The visitor")
            if not self.livechat_visitor_id
            else self.livechat_visitor_id.display_name
        )
        if cancel:
            message = (
                _(
