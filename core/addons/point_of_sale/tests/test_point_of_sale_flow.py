...
class TestPointOfSaleFlow(TestPointOfSaleCommon):
    ...
    def test_order_refund(self):
        ...
        self.assertEqual(
            order.amount_total,
            -1 * refund.amount_total,
            "The refund does not cancel the order (%s and %s)"
            % (order.amount_total, refund.amount_total),
        )
        ...
        self.assertTrue(
            refund.payment_ids.payment_method_id.is_cash_count,
    ...
    def test_order_to_picking(self):
        ...
        self.pos_make_payment_2 = self.PosMakePayment.with_context(
            context_make_payment
        ).create({"amount": untax1 + untax2 + atax1 + atax2})
        ...
        self.pos_make_payment_3 = self.PosMakePayment.with_context(
            context_make_payment
        ).create({"amount": untax1 + untax2 + atax1 + atax2})
        ...
        # I check that the order is marked as paid
        self.assertEqual(
            self.pos_order_pos2.state, "paid", "Order should be in paid state."
        )
        ...
        self.pos_make_payment_4 = self.PosMakePayment.with_context(
            context_make_payment
        ).create(
            {
        ...
        # I check that the order is marked as paid
        self.assertEqual(
            self.pos_order_pos3.state, "paid", "Order should be in paid state."
        )
    ...
    def test_order_to_picking02(self):
        ...
        self.env["stock.quant"]._update_available_quantity(
            tracked_product, shelf1_location, qty, lot_id=lot
        )
        self.env["stock.quant"]._update_available_quantity(
            untracked_product, shelf1_location, qty
        )
    ...
    def test_order_to_invoice(self):
        ...
        self.assertAlmostEqual(
            ...
            self.pos_order_pos1.amount_total,
            places=2,
            msg="Invoice not correct",
        )
    ...
    def test_create_from_ui(self):
        ...
        total_cash_payment = sum(
            rescue_session.mapped("order_ids.payment_ids")
            .filtered(lambda payment: payment.payment_method_id.type == "cash")
            .mapped("amount")
        )
        ...
        self.assertEqual(
            rescue_session.cash_register_balance_start,
            current_session.cash_register_balance_end_real,
            "Rescue session does not start with the same amount as the previous session",
        )
    ...
    def test_order_to_payment_currency(self):
        ...
        eur_pricelist = self.partner1.property_product_pricelist.copy(
            default={"currency_id": self.env.ref("base.EUR").id}
        )
        ...
        self.assertLess(
            ...
            0.01,
            "The order has a wrong total including tax and discounts",
        )
        ...
        self.assertEqual(
            self.pos_order_pos0.state, "draft", "Order should be in draft state."
        )
        ...
        self.assertLess(
            abs(defs["amount"] - ((450 * 2 + 300 * 3 * 1.05) - 100.0)),
            0.01,
        ...
        self.assertTrue(
            current_session.move_id,
            "Journal entry should have been attached to the session.",
        )
        ...
        for a, b in zip(sorted(credit_lines), [0.0, 22.5, 40.91, 409.09, 450]):
            self.assertAlmostEqual(a, b)
        for a, b in zip(
            sorted(amount_currency_lines), [-900, -818.18, -81.82, -45, 1845]
        ):
            self.assertAlmostEqual(a, b)
    ...
    def test_order_to_invoice_no_tax(self):
        ...
        self.assertFalse(
            ...
            "Invoice should not be attached to order yet.",
        )
    ...
    def test_order_with_deleted_tax(self):
        ...
        diff_line = pos_session.move_id.line_ids.filtered(
    ...
    def test_order_multi_step_route(self):
        ...
        picking_mls_no_stock = pickings.move_line_ids.filtered(
            lambda l: l.product_id.id == tracked_product.id
        )
        picking_mls_stock = pickings.move_line_ids.filtered(
            lambda l: l.product_id.id == tracked_product_2.id
        )
        self.assertEqual(pos_order.state, "paid")
    ...
    def test_sale_order_postponed_invoicing(self):
        ...
        with freeze_time("2020-01-03"):
            ...
            misc_reversal_entry = self.env["account.move"].search(
                [
                    (
                        "ref",
                        "=",
                        f"Reversal of POS closing entry {closing_entry.name} for order {pos_order.name} from session {pos_order.session_id.name}",
                    )
                ]
            )
            ...
            payment = self.env["account.move"].search(
                [
                    (
                        "ref",
                        "=like",
                        f"Invoice payment for {pos_order.name} ({pos_order.account_move.name}) using {self.cash_payment_method.name}",
                    )
                ]
            )
            # And thus only one bank statement for it
            statement = self.env["account.move"].search(
                [("journal_id", "=", self.company_data["default_journal_cash"].id)]
            )
            self.assertTrue(
                invoice.exists()
                and closing_entry.exists()
                and misc_reversal_entry.exists()
                and payment.exists()
            )
            # Check 1: Check that we have reversed every credit line on the closing entry.
            for closing_entry_line, misc_reversal_entry_line in zip(
                closing_entry.line_ids, misc_reversal_entry.line_ids
            ):
                if closing_entry_line.balance < 0:
                    self.assertEqual(
                        closing_entry_line.balance, -misc_reversal_entry_line.balance
                    )
                    self.assertEqual(
                        closing_entry_line.account_id,
                        misc_reversal_entry_line.account_id,
                    )
            ...
            self.assertEqual(
                invoice_receivable_line.matching_number,
                payment_receivable_line.matching_number,
            )
            # The payment receivable (POS) is reconciled with the closing entry receivable (POS)
            payment_receivable_pos_line = payment.line_ids.filtered(
                lambda line: line.account_id
                == self.company_data[
                    "company"
                ].account_default_pos_receivable_account_id
            )
            misc_receivable_pos_line = misc_reversal_entry.line_ids.filtered(
                lambda line: line.account_id
                == self.company_data[
                    "company"
                ].account_default_pos_receivable_account_id
            ...
            self.assertEqual(
                misc_receivable_pos_line.matching_number,
                payment_receivable_pos_line.matching_number,
            )
            # The closing entry receivable is reconciled with the bank statement
            closing_entry_receivable_line = closing_entry.line_ids.filtered(
                lambda line: line.account_id
                == self.company_data["default_account_receivable"]
            )  # Because the payment method use the default receivable
            statement_receivable_line = statement.line_ids.filtered(
                lambda line: line.account_id
                == self.company_data["default_account_receivable"]
                and line.name == pos_order.session_id.name
            )  # Because the payment method use the default receivable
            self.assertEqual(
                closing_entry_receivable_line.matching_number,
                statement_receivable_line.matching_number,
            )
    ...
    def test_order_pos_tax_same_as_company(self):
        ...
        self.pos_config.open_ui()
        current_session = self.pos_config.current_session_id
        current_session.company_id.account_default_pos_receivable_account_id = (
            self.partner1.property_account_receivable_id
        )
    ...
    def test_order_refund_with_owner(self):
        ...
        current_session.action_pos_session_closing_control()
        self.assertEqual(
            refund.picking_ids.move_line_ids_without_package.owner_id.id,
    ...
    def test_tracked_product_with_owner(self):
        ...
        self.env["stock.quant"]._update_available_quantity(
            self.product2, self.stock_location, 1, lot_id=lot1, owner_id=self.partner1
        )
        ...
        current_session.action_pos_session_closing_control()
        self.assertEqual(
            current_session.picking_ids.move_line_ids.owner_id.id, self.partner1.id
        )
