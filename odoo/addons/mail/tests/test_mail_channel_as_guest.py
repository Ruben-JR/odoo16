...
class TestMailPublicPage(HttpCase):
    ...
    def setUp(self):
        ...
        self.channel = self.env["mail.channel"].browse(
            self.env["mail.channel"].channel_create(group_id=None, name="Test channel")[
                "id"
            ]
        )
        ...
        self.group = self.env["mail.channel"].browse(
            self.env["mail.channel"].create_group(
                partners_to=(internal_user + portal_user).partner_id.ids,
                name="Test group",
            )["id"]
        )
