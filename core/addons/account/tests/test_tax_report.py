...
class TaxReportTest(AccountTestInvoicingCommon):
    ...
    def test_create_shared_tags(self):
        self.assertEqual(
            ...
            2,
            "tax_tags expressions created for reports within the same countries using the same formula should create a single pair of tags.",
        )
    ...
    def test_add_expression(self):
        ...
        self.assertEqual(
            len(tags_after),
            len(tags_before) + 2,
            "Two tags should have been created, +tournicoti and -tournicoti.",
        )
    ...
    def test_write_single_line_tagname_shared(self):
        ...
        self.assertEqual(
            len(self._get_tax_tags(self.test_country_1)),
            len(start_tags) + 2,
            "A + and - tag should have been created",
        )
        self.assertNotEqual(
            original_tags,
            new_tags,
    ...
    def test_edit_multi_line_tagname_all_different_new(self):
        ...
        self.assertEqual(
            len(tags_before) + 2,
            len(tags_after),
            "Only two distinct tags should have been created.",
        )
    ...
    def test_tax_report_change_country(self):
        ...
        self.assertEqual(
            ...
            len(country_2_tags_before_change) + 2 * len(copied_report_1.line_ids),
            "Modifying the country should have created a new + and - tag in the new country for each tax_tags expression of the report.",
        )
    ...
    def test_unlink_report_line_tags_used_by_amls(self):
        ...
        test_tag = tax_report_line.expression_ids._get_matching_tags().filtered(
        ...
        self.assertEqual(
            tags_after.mapped("tax_negate"),
            [False],
            "Unlinking a tax_tags expression should keep the tag if it was used on move lines, and unlink it otherwise.",
        )
        self.assertEqual(
    ...
    def test_tag_recreation_archived(self):
        ...
        tags_before[
            0
        ].unlink()  # we unlink one and archive the other, doesn't matter which one
        ...
        self.assertEqual(
            len(tags_after),
            2,
            "When creating a tax report line with an archived tag and it's complement doesn't exist, it should be re-created.",
        )
        self.assertEqual(
            tags_after.mapped("name"),
            ["+" + tag_name, "-" + tag_name],
            "After creating a tax report line with an archived tag and when its complement doesn't exist, both a negative and a positive tag should be created.",
        )
