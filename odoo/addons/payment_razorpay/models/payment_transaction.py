...
class PaymentTransaction(models.Model):
    ...
    def _get_specific_rendering_values(self, processing_values):
        ...
        order_data = self.provider_id._razorpay_make_request(
            endpoint="orders", payload=payload
        )
        _logger.info(
        ...
        if phone:
            ...
            phone_info = phone_sanitize_numbers(
                [phone], country_code, country_phone_code
            )
        ...
        rendering_values = {
            ...
            "return_url": url_join(
                base_url,
                f"{RazorpayController._return_url}?{url_encode(return_url_params)}",
            ),
        }
        ...
        if entity_type == "payment":
            ...
            if not reference:
                raise ValidationError(
                    "Razorpay: " + _("Received data with missing reference.")
                )
            tx = self.search(
    ...
    def _razorpay_create_refund_tx_from_notification_data(
    ...
    def _process_notification_data(self, notification_data):
        ...
        if not entity_id:
            raise ValidationError(
                "Razorpay: " + _("Received data with missing entity id.")
            )
        ...
        if not entity_status:
            raise ValidationError(
                "Razorpay: " + _("Received data with missing status.")
            )
