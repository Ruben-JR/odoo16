...
class RecruitmentSource(models.Model):
    ...
    medium_id = fields.Many2one(
        "utm.medium", default=lambda self: self.env.ref("utm.utm_medium_website")
    )

    def _compute_has_domain(self):
        self.has_domain = bool(
    ...
    def create_alias(self):
        ...
        for source in self:
            vals = {
                ...
                "alias_name": "%s+%s"
                % (source.job_id.alias_name or source.job_id.name, source.name),
                "alias_defaults": {
    ...
    def _get_view(self, view_id=None, view_type="form", **options):
        ...
        if view_type == "tree" and not bool(
            self.env["ir.config_parameter"].sudo().get_param("mail.catchall.domain")
        ):
...
class RecruitmentStage(models.Model):
    ...
    def default_get(self, fields):
        if (
            self._context
            and self._context.get("default_job_id")
    ...
    def _compute_is_warning_visible(self):
        ...
        applicants = dict(
            (data["stage_id"][0], data["stage_id_count"]) for data in applicant_data
        )
        for stage in self:
            if (
...
class Applicant(models.Model):
    ...
    employee_name = fields.Char(
        related="emp_id.name", string="Employee Name", readonly=False, tracking=False
    )
    ...
    def _compute_application_count(self):
        ...
        GROUP BY hr_applicant.id
            ...
            application_data_mapped = dict(
                (data["appl_id"], data["count"]) for data in self.env.cr.dictfetchall()
            )
        else:
            application_data_mapped = dict()
        for applicant in applicants:
            applicant.application_count = (
                application_data_mapped.get(applicant.id, 1) - 1
            )
    ...
    @api.depends(
        "partner_id", "partner_id.email", "partner_id.mobile", "partner_id.phone"
    )
    ...
    def _inverse_partner_email(self):
        for applicant in self.filtered(
            lambda a: a.partner_id and a.email_from and not a.partner_id.email
        ):
            applicant.partner_id.email = applicant.email_from

    def _inverse_partner_phone(self):
        for applicant in self.filtered(
            lambda a: a.partner_id and a.partner_phone and not a.partner_id.phone
        ):
            applicant.partner_id.phone = applicant.partner_phone

    def _inverse_partner_mobile(self):
        for applicant in self.filtered(
            lambda a: a.partner_id and a.partner_mobile and not a.partner_id.mobile
        ):
    ...
    def create(self, vals_list):
        ...
        if "default_activity_date_deadline" in self.env.context:
            deadline = fields.Datetime.to_datetime(
                self.env.context.get("default_activity_date_deadline")
            )
            category = self.env.ref("hr_recruitment.categ_meet_interview")
            for applicant in applicants:
                partners = (
                    applicant.partner_id
                    | applicant.user_id.partner_id
                    | applicant.department_id.manager_id.user_id.partner_id
                )
                self.env["calendar.event"].sudo().with_context(
    ...
    def get_view(self, view_id=None, view_type="form", **options):
        if view_type == "form" and self.user_has_groups(
    ...
    def _notify_get_recipients(self, message, msg_vals, **kwargs):
        ...
        return [
            ...
            if interviewer_group not in recipient["groups"]
        ]
    ...
    def action_makeMeeting(self):
        ...
        partners = (
            ...
            | self.department_id.manager_id.user_id.partner_id
        )
        ...
        for applicant in self:
            if applicant.partner_id:
                applicant._message_add_suggested_recipient(
                    recipients, partner=applicant.partner_id.sudo(), reason=_("Contact")
                )
