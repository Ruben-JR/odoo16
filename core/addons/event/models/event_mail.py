...
class EventTypeMail(models.Model):
    ...
    notification_type = fields.Selection(
        [("mail", "Mail")], string="Send", default="mail", required=True
    )
    ...
    template_ref = fields.Reference(
        string="Template", selection="_selection_template_model", required=True
    )
    ...
    def _compute_template_model_id(self):
        ...
        for mail in self:
            mail.template_model_id = (
                mail_model if mail.notification_type == "mail" else False
            )
...
class EventMailScheduler(models.Model):
    ...
    def set_template_ref_model(self):
        ...
        if self.notification_type == "mail":
            ...
            self.template_ref = (
                "{},{}".format("mail.template", record.id) if record else False
            )

    event_id = fields.Many2one(
        "event.event", string="Event", required=True, ondelete="cascade"
    )
...
class EventMailRegistration(models.Model):
    ...
    scheduler_id = fields.Many2one(
        "event.mail", "Mail Scheduler", required=True, ondelete="cascade"
    )
    ...
    def _compute_scheduled_date(self):
        for mail in self:
            if mail.registration_id:
                mail.scheduled_date = mail.registration_id.create_date + _INTERVALS[
