...
class TestSaleCouponProgramNumbers(TestSaleCouponNumbersCommon):
    def test_program_numbers_free_and_paid_product_qty(self):
        ...
        self.assertEqual(
            sum(
                order.order_line.filtered(lambda x: x.is_reward_line).mapped(
                    "product_uom_qty"
                )
            ),
            25,
    ...
    def test_program_numbers_one_discount_line_per_tax(self):
        ...
        self.assertAlmostEqual(
            ...
            1901.11,
            2,
            "The order total with programs should be 1901.11",
        )
        self.assertEqual(
            order.amount_untaxed,
            1594.95,
            "The order untaxed total without any programs should be 2576.77",
        )
        self.assertEqual(
            len(order.order_line.ids),
            5,
        ...
        self.assertEqual(
            order.amount_untaxed,
            1594.95,
            "The order untaxed total with programs should be 1594.95",
        )
        self.assertEqual(
            len(order.order_line.ids),
            8,
        ...
        self.assertEqual(
            order.amount_total, 1711, "The order total with programs should be 1711"
        )
        self.assertEqual(
            order.amount_untaxed,
            1435.46,
            "The order untaxed total with programs should be 1435.46",
        )
        self.assertEqual(
            len(order.order_line.ids),
            12,
        ...
        self.assertEqual(
            order.amount_total,
            1711,
            "Recomputing tax on sale order lines should not change total amount",
        )
        self.assertEqual(
            order.amount_untaxed,
            1435.46,
            "Recomputing tax on sale order lines should not change untaxed amount",
        )
        self.assertEqual(
            len(order.order_line.ids),
            12,
        ...
        self.assertEqual(
            order.amount_total, 1628.2, "The order total with programs should be 1628.2"
        )
        self.assertEqual(
            order.amount_untaxed,
            1363.46,
            "The order untaxed total with programs should be 1363.45",
        )
        self.assertEqual(
            len(order.order_line.ids),
            13,
        ...
        self._auto_rewards(order, self.all_programs)
        self.assertEqual(
            len(order.order_line.ids), 13, "The 10% discount line should be back"
        )
        ...
        self.assertAlmostEqual(
            ...
            1358.2,
            2,
            "The order total with programs should be 1358.20",
        )
        self.assertEqual(
            order.amount_untaxed,
            1118,
            "The order untaxed total with programs should be 1118.00",
        )
        self.assertEqual(
            len(order.order_line.ids),
            10,
    ...
    def test_program_next_order(self):
        ...
        self._auto_rewards(order, self.all_programs)
        self.assertEqual(
            len(order.order_line.ids), 1, "Nothing should be added to the cart"
        )
        self.assertEqual(
        ...
        self.assertEqual(
            len(order.order_line.ids), 1, "Nothing should be added to the cart (2)"
        )
        self.assertEqual(
        ...
        generated_coupon = order._get_reward_coupons()
        self.assertEqual(
            len(order.order_line.ids), 1, "Nothing should be added to the cart (3)"
        )
        self.assertEqual(
    ...
    def test_coupon_and_coupon_discount_fixed_amount_tax_excl(self):
        ...
        * Even applying the coupon in reverse order should yield same result
        ...
        self._auto_rewards(order, self.all_programs)
        self.assertEqual(
            len(order.order_line), 3, "The promotion program should be removed"
        )
        self._apply_promo_code(order, coupon.code)
        self.assertEqual(
            order.amount_total,
    ...
    def test_program_numbers_free_prod_with_min_amount_and_qty_on_same_prod(self):
        ...
        self._auto_rewards(order, self.all_programs)
        self.assertEqual(
            len(order.order_line.ids), 2, "The promotion lines should not be applied"
        )
        sol1.write({"product_uom_qty": 2.0})
        self._auto_rewards(order, self.all_programs)
        self.assertEqual(
            len(order.order_line.ids), 3, "The promotion lines should have been added"
        )
        self.assertEqual(
            order.amount_total,
    ...
    def test_program_step_percentages(self):
        ...
        self.assertEqual(
            len(order.order_line.ids), 2, "No discount applied while it should"
        )
    ...
    def test_program_modify_reward_line_qty(self):
        ...
        self.assertEqual(
            order.order_line.filtered(lambda x: x.is_reward_line).price_unit,
            -5,
