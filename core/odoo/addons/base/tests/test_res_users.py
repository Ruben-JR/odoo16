...
class TestUsers(TransactionCase):
    ...
    def test_deactivate_portal_users_archive_and_remove(self):
        ...
        self.assertEqual(
            portal_user.partner_id.name, "Portal", "Should have kept the partner name"
        )
        self.assertNotEqual(
            portal_user.login, "portal_user", "Should have removed the user login"
        )
        ...
        self.assertTrue(
            asked_deletion_1, "Should have added the user 1 in the deletion queue"
        )
        self.assertTrue(
            asked_deletion_2, "Should have added the user 2 in the deletion queue"
        )
        ...
        self.assertEqual(
            asked_deletion_1.state, "done", "Should have marked the deletion as done"
        )
        ...
        self.assertEqual(
            asked_deletion_2.state, "fail", "Should have marked the deletion as failed"
        )
...
class TestUsers2(TransactionCase):
    ...
    def test_read_group_with_reified_field(self):
        ...
        self.assertEqual(
            res_with_reified, res_without_reified, "Reified fields should be ignored"
        )
    ...
    def test_reified_groups_on_change(self):
        ...
        self.assertTrue(
            user_form.share, "The groups_id onchange should have been triggered"
        )
...
class TestUsersGroupWarning(TransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.sales_categ_field = name_selection_groups(
            (cls.group_sales_user | cls.group_sales_administrator).ids
        )
        ...
        cls.timesheets_categ_field = name_selection_groups(
            (
                cls.group_timesheets_user_own_timesheet
                | cls.group_timesheets_user_all_timesheet
                | cls.group_timesheets_administrator
            ).ids
        )
        cls.group_timesheets_administrator.implied_ids += (
            cls.group_timesheets_user_all_timesheet
        )
        cls.group_timesheets_user_all_timesheet.implied_ids += (
            cls.group_timesheets_user_own_timesheet
        )
        ...
        cls.project_categ_field = name_selection_groups(
            (cls.group_project_user | cls.group_project_admnistrator).ids
        )
        cls.group_project_admnistrator.implied_ids = (
            cls.group_project_user | cls.group_timesheets_user_all_timesheet
        )

        # Field Service
        (
            cls.group_field_service_user,
            cls.group_field_service_administrator,
        ) = ResGroups.create(
            [
        ...
        cls.group_field_service_administrator.implied_ids = (
            cls.group_sales_administrator
    ...
    def test_user_group_empty_group_warning(self):
        ...
        with self.assertQueryCount(__system__=549), Form(
            self.test_group_user.with_context(show_user_group_warning=True),
            view="base.view_users_form",
        ) as UserForm:
    ...
    def test_user_group_inheritance_warning_multi(self):
        ...
        with self.assertQueryCount(__system__=553), Form(
            self.test_group_user.with_context(show_user_group_warning=True),
            view="base.view_users_form",
        ) as UserForm:
            UserForm._values[self.sales_categ_field] = self.group_sales_user.id
    ...
    def test_user_group_least_possible_inheritance_warning(self):
        ...
        with self.assertQueryCount(__system__=550), Form(
            self.test_group_user.with_context(show_user_group_warning=True),
            view="base.view_users_form",
        ) as UserForm:
            UserForm._values[
                self.timesheets_categ_field
            ] = self.group_timesheets_user_own_timesheet.id
            ...
            UserForm._values[
                self.timesheets_categ_field
            ] = self.group_timesheets_user_all_timesheet.id
            UserForm._perform_onchange([self.timesheets_categ_field])
            self.assertFalse(UserForm.user_group_warning)
    ...
    def test_user_group_parent_inheritance_no_warning(self):
        ...
        with self.assertQueryCount(__system__=534), Form(
            self.test_group_user.with_context(show_user_group_warning=True),
            view="base.view_users_form",
        ) as UserForm:
            UserForm._values[
                self.field_service_categ_field
            ] = self.group_field_service_user.id
