...
class ResConfigSettings(models.TransientModel):
    ...
    l10n_it_edi_proxy_current_state = fields.Char(
        compute="_compute_l10n_it_edi_proxy_current_state"
    )
    l10n_it_edi_sdicoop_register = fields.Boolean(
    ...
    l10n_it_edi_sdicoop_demo_mode = fields.Selection(
        [("demo", "Demo"), ("test", "Test (experimental)"), ("prod", "Official")],
    ...
    def _create_proxy_user(self, company_id):
        ...
        self.env["account_edi_proxy_client.user"]._register_proxy_user(
            company_id, fattura_pa, edi_identification
        )
    ...
    def _compute_l10n_it_edi_sdicoop_demo_mode(self):
        for config in self:
            config.l10n_it_edi_sdicoop_demo_mode = self.env[
    ...
    def _set_l10n_it_edi_sdicoop_demo_mode(self):
        for config in self:
            self.env["ir.config_parameter"].set_param(
                "account_edi_proxy_client.demo", config.l10n_it_edi_sdicoop_demo_mode
            )
    ...
    def _compute_l10n_it_edi_proxy_current_state(self):
        ...
        for config in self:
            ...
            config.l10n_it_edi_proxy_current_state = (
                "inactive"
                if not proxy_user
                else "demo"
                if proxy_user.id_client[:4] == "demo"
                else "active"
            )
    ...
    def button_create_proxy_user(self):
        ...
        self.env["account_edi_proxy_client.user"]._register_proxy_user(
            self.company_id, fattura_pa, edi_identification
        )
    ...
    def _set_l10n_it_edi_sdicoop_register_demo_mode(self):
        ...
        for config in self:
            ...
            if (
                real_proxy_users
                and previous_demo_state != config.l10n_it_edi_sdicoop_demo_mode
            ):
                raise UserError(
                    _(
            ...
            if config.l10n_it_edi_sdicoop_register:
                ...
                # delete all demo users and then create the new user.
                    ...
                    proxy_user.id_client[:4] == "demo"
                    and config.l10n_it_edi_sdicoop_demo_mode != "demo"
                ):
