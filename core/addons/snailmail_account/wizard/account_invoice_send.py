...
class AccountInvoiceSend(models.TransientModel):
    ...
    partner_id = fields.Many2one(
        "res.partner", compute="_get_partner", string="Partner"
    )
    snailmail_is_letter = fields.Boolean(
        "Send by Post",
        help="Print and post the invoice by snailmail",
        default=lambda self: self.env.company.invoice_is_snailmail,
    )
    snailmail_cost = fields.Float(
        string="Stamp(s)", compute="_compute_snailmail_cost", readonly=True
    )
    invalid_addresses = fields.Integer(
        "Invalid Addresses Count", compute="_compute_invalid_addresses"
    )
    invalid_invoices = fields.Integer(
        "Invalid Invoices Count", compute="_compute_invalid_addresses"
    )
    invalid_partner_ids = fields.Many2many(
        "res.partner", string="Invalid Addresses", compute="_compute_invalid_addresses"
    )
    ...
    def _compute_invalid_addresses(self):
        for wizard in self:
            if any(not invoice.partner_id for invoice in wizard.invoice_ids):
                raise UserError(
                    _("You cannot send an invoice which has no partner assigned.")
                )
            invalid_invoices = wizard.invoice_ids.filtered(
                lambda i: not self.env["snailmail.letter"]._is_valid_address(
                    i.partner_id
                )
            )
    ...
    def snailmail_print_action(self):
        ...
        self.invoice_ids.filtered(lambda inv: not inv.is_move_sent).write(
            {"is_move_sent": True}
        )
    ...
    def send_and_print_action(self):
        if self.snailmail_is_letter:
            if self.env["snailmail.confirm.invoice"].show_warning():
                wizard = self.env["snailmail.confirm.invoice"].create(
