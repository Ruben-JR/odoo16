    ...
    def _prepare_invoice_vals(self):
        ...
        if sale_orders and sale_orders[0].payment_term_id:
            invoice_vals["invoice_payment_term_id"] = (
                sale_orders[0].payment_term_id.id,
            )
        return invoice_vals
...
class PosOrderLine(models.Model):
    ...
    def _export_for_ui(self, orderline):
        ...
        result["down_payment_details"] = (
            bool(orderline.down_payment_details) and orderline.down_payment_details
        )
        result["sale_order_origin_id"] = (
            bool(orderline.sale_order_origin_id)
            and orderline.sale_order_origin_id.read(fields=["name"])[0]
        )
        return result
