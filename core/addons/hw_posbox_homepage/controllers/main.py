...
class IoTboxHomepage(Home):
    ...
    def clean_partition(self):
        subprocess.check_call(
            [
                ...
                "bash",
                "-c",
                ". /home/pi/odoo/addons/point_of_sale/tools/posbox/configuration/upgrade.sh; cleanup",
            ]
        )
    ...
    def index(self):
        ...
        if (
            wifi.exists() == False or remote_server.exists() == False
        ) and helpers.access_point():
            return (
    ...
    def list_handlers(self):
        drivers_list = helpers.list_file_by_os(
            get_resource_path("hw_drivers", "iot_handlers", "drivers")
        )
        interfaces_list = helpers.list_file_by_os(
            get_resource_path("hw_drivers", "iot_handlers", "interfaces")
        )
        ...
        return list_credential_template.render(
            {
                ...
                "enterprise_code": helpers.read_file_first_line(
    ...
    def connect_to_wifi(self, essid, password, persistent=False):
        ...
        subprocess.check_call(
            [
                get_resource_path(
                    "point_of_sale", "tools/posbox/configuration/connect_to_wifi.sh"
                ),
                essid,
                password,
                persistent,
    ...
    def clear_handlers_list(self):
        for directory in ["drivers", "interfaces"]:
            for file in list(
                Path(get_resource_path("hw_drivers", "iot_handlers", directory)).glob(
                    "*"
                )
            ):
    ...
    def connect_to_server(self, token, iotname):
        ...
        if iotname and platform.system() == "Linux":
            subprocess.check_call(
                [
                    get_resource_path(
                        "point_of_sale", "tools/posbox/configuration/rename_iot.sh"
                    ),
                    iotname,
                ]
            )
    ...
    def step_by_step_configure(self, token, iotname, essid, password, persistent=False):
        ...
        subprocess.check_call(
            [
                get_resource_path(
                    "point_of_sale",
                    "tools/posbox/configuration/connect_to_server_wifi.sh",
                ),
                url,
                iotname,
                token,
    ...
    @http.route(
        "/six_payment_terminal_add", type="http", auth="none", cors="*", csrf=False
    )
    ...
    @http.route(
        "/six_payment_terminal_clear", type="http", auth="none", cors="*", csrf=False
    )
    ...
    def upgrade(self):
        ...
        if flashToVersion:
            flashToVersion = "%s.%s" % (
                flashToVersion.get("major", ""),
                flashToVersion.get("minor", ""),
            )
            ...
            response = (
                subprocess.check_output(
                    [
                        ...
                        "bash",
                        "-c",
                        ". /home/pi/odoo/addons/point_of_sale/tools/posbox/configuration/upgrade.sh; create_partition",
                    ]
                )
                .decode()
                .split("\n")[-2]
            ...
            response = (
                subprocess.check_output(
                    [
                        ...
                        "bash",
                        "-c",
                        ". /home/pi/odoo/addons/point_of_sale/tools/posbox/configuration/upgrade.sh; download_raspios",
                    ]
                )
                .decode()
                .split("\n")[-2]
            ...
            response = (
                subprocess.check_output(
                    [
                        ...
                        "bash",
                        "-c",
                        ". /home/pi/odoo/addons/point_of_sale/tools/posbox/configuration/upgrade.sh; copy_raspios",
                    ]
                )
                .decode()
                .split("\n")[-2]
    ...
    @http.route(
        "/iot_restart_odoo_or_reboot", type="json", auth="none", cors="*", csrf=False
    )
