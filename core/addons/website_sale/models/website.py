...
class Website(models.Model):
    ...
    pricelist_id = fields.Many2one(
        "product.pricelist", compute="_compute_pricelist_id", string="Default Pricelist"
    )
    ...
    all_pricelist_ids = fields.One2many(
        "product.pricelist", "website_id", string="All pricelists"
    )
    ...
    shop_ppg = fields.Integer(
        default=20, string="Number of products in the grid on the shop"
    )
    ...
    shop_extra_field_ids = fields.One2many(
        "website.sale.extra.field", "website_id", string="E-Commerce Extra Fields"
    )
    ...
    def _get_pl_partner_order(
        ...
        if show_visible:
            ...
            check_pricelist = lambda pl: pl.selectable or pl.id in (
                current_pl_id,
                order_pl_id,
            )
        else:
        ...
        if not self.env.user._is_public():
            ...
            partner_pricelist = pricelists.browse(partner_pl_id).filtered(
                lambda pl: pl._is_available_on_website(self)
    ...
    def get_current_pricelist(self):
        ...
        if request and request.session.get("website_sale_current_pl"):
            ...
            pricelist = (
                ProductPricelist.browse(request.session["website_sale_current_pl"])
                .exists()
                .sudo()
            )
            country_code = self._get_geoip_country_code()
            if (
                not pricelist
                or not pricelist._is_available_on_website(self)
                or not pricelist._is_available_in_country(country_code)
            ):
    ...
    def sale_product_domain(self):
        return expression.AND(
            [self._product_domain(), self.get_current_website().website_domain()]
        )
    ...
    def _prepare_sale_order_values(self, partner_sudo):
        ...
        if not request.website.is_public_user():
            ...
            if (
                last_sale_order and last_sale_order.partner_shipping_id.active
            ):  # first = me
                addr["delivery"] = last_sale_order.partner_shipping_id.id

        affiliate_id = request.session.get("affiliate_id")
        salesperson_user_sudo = (
            self.env["res.users"].sudo().browse(affiliate_id).exists()
        )
        if not salesperson_user_sudo:
            salesperson_user_sudo = (
                self.salesperson_id
                or partner_sudo.parent_id.user_id
                or partner_sudo.user_id
            )
        ...
        values = {
            ...
            or partner_sudo.parent_id.team_id.id
            or partner_sudo.team_id.id,
    ...
    def _get_product_page_grid_image_classes(self):
        ...
        return (
            spacing_map.get(self.product_page_image_spacing)
            + " "
    ...
    def _send_abandoned_cart_email(self):
        for website in self.search([]):
            ...
            for sale_order in abandoned_carts:
                ...
                template.send_mail(
                    sale_order.id,
                    email_values=dict(email_to=sale_order.partner_id.email),
                )
