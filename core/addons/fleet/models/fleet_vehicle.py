...
class FleetVehicle(models.Model):
    ...
    first_contract_date = fields.Date(
        string="First Contract Date", default=fields.Date.today
    )
    color = fields.Char(
    ...
    contract_renewal_name = fields.Text(
        compute="_compute_contract_reminder", string="Name of contract to renew soon"
    )
    ...
    plan_to_change_car = fields.Boolean(
        related="driver_id.plan_to_change_car", store=True, readonly=False
    )
    ...
    def _compute_service_activity(self):
        for vehicle in self:
            activities_state = set(
                state
                for state in vehicle.log_services.mapped("activity_state")
    ...
    def _compute_count_all(self):
        ...
        odometers_data = Odometer.read_group(
            [("vehicle_id", "in", self.ids)], ["vehicle_id"], ["vehicle_id"]
        )
        services_data = LogService.read_group(
            [("vehicle_id", "in", self.ids)],
            ["vehicle_id", "active"],
            ["vehicle_id", "active"],
            lazy=False,
        )
        logs_data = LogContract.read_group(
            [("vehicle_id", "in", self.ids), ("state", "!=", "closed")],
            ["vehicle_id", "active"],
            ["vehicle_id", "active"],
            lazy=False,
        )
        histories_data = History.read_group(
            [("vehicle_id", "in", self.ids)], ["vehicle_id"], ["vehicle_id"]
        )
        ...
        for odometer_data in odometers_data:
            mapped_odometer_data[odometer_data["vehicle_id"][0]] = odometer_data[
                "vehicle_id_count"
            ]
        for service_data in services_data:
            mapped_service_data[service_data["vehicle_id"][0]][
                service_data["active"]
            ] = service_data["__count"]
        for log_data in logs_data:
            mapped_log_data[log_data["vehicle_id"][0]][log_data["active"]] = log_data[
                "__count"
            ]
        for history_data in histories_data:
            mapped_history_data[history_data["vehicle_id"][0]] = history_data[
                "vehicle_id_count"
            ]
    ...
    def _search_contract_renewal_due_soon(self, operator, value):
        ...
        res_ids = (
            self.env["fleet.vehicle.log.contract"]
            .search(
                [
    ...
    def create(self, vals_list):
        ...
        ptc_values = [
            "plan_to_change_car" in vals.keys()
            and {"plan_to_change_car": vals.pop("plan_to_change_car")}
    ...
    def read_group(
        ...
        if "co2" in fields:
            fields.remove("co2")
        return super(FleetVehicle, self).read_group(
