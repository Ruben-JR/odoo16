...
class AccountAnalyticAccount(models.Model):
    ...
    production_ids = fields.One2many(
        "mrp.production", "analytic_account_id", string="Manufacturing Orders"
    )
    production_count = fields.Integer(
        "Manufacturing Orders Count", compute="_compute_production_count"
    )
    bom_ids = fields.One2many(
        "mrp.bom", "analytic_account_id", string="Bills of Materials"
    )
    bom_count = fields.Integer("BoM Count", compute="_compute_bom_count")
    workcenter_ids = fields.One2many(
        "mrp.workcenter", "costs_hour_account_id", string="Workcenters"
    )
