...
class TestProjectMilestone(TestProjectCommon):
    ...
    def test_milestones_settings_change(self):
        ...
        self.env["res.config.settings"].create(
            {"group_project_milestone": False}
        ).execute()
        self.assertFalse(
            self.env.user.has_group("project.group_project_milestone"),
            'The "Milestones" feature should not be globally enabled by default.',
        )
        self.assertFalse(
            self.project_pigs.allow_milestones,
            'The "Milestones" feature should not be enabled by default.',
        )
        self.env["res.config.settings"].create(
            {"group_project_milestone": True}
        ).execute()
        self.assertTrue(
            self.env.user.has_group("project.group_project_milestone"),
        ...
        with Form(self.env["project.project"]) as project_form:
            ...
            self.assertTrue(
                ...
                "New projects allow_milestones should be True by default.",
            )
