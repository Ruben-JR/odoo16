...
class HolidaysAllocation(models.Model):
    ...
    name_validity = fields.Char(
        "Description with validity", compute="_compute_description_validity"
    )
    active = fields.Boolean(default=True)
    private_name = fields.Char(
        "Allocation Description", groups="hr_holidays.group_hr_holidays_user"
    )
    ...
    linked_request_ids = fields.One2many(
        "hr.leave.allocation", "parent_id", string="Linked Requests"
    )
    ...
    nextcall = fields.Date(
        "Date of the next accrual allocation", default=False, readonly=True
    )
    ...
    taken_leave_ids = fields.One2many(
        "hr.leave",
    ...
    def _compute_description(self):
        ...
        for allocation in self:
            if (
                is_officer
                or allocation.employee_id.user_id == self.env.user
                or allocation.employee_id.leave_manager_id == self.env.user
            ):
        ...
        for allocation in self:
            if allocation.date_to:
                name_validity = _(
                    "%s (from %s to %s)",
                    allocation.name,
                    allocation.date_from.strftime("%b %d %Y"),
                    allocation.date_to.strftime("%b %d %Y"),
                )
            else:
                name_validity = _(
                    "%s (from %s to No Limit)",
                    allocation.name,
                    allocation.date_from.strftime("%b %d %Y"),
                )
    ...
    @api.depends(
        "employee_id",
        "holiday_status_id",
        "taken_leave_ids.number_of_days",
        "taken_leave_ids.state",
    )
    def _compute_leaves(self):
        employee_days_per_allocation = self.holiday_status_id.with_context(
            ignore_future=True
        )._get_employees_days_per_allocation(self.employee_id.ids)
        for allocation in self:
            ...
            allocation.leaves_taken = employee_days_per_allocation[
                allocation.employee_id.id
            ][allocation.holiday_status_id][allocation]["leaves_taken"]
    ...
    def _compute_from_holiday_type(self):
        default_employee_ids = (
            self.env["hr.employee"].browse(self.env.context.get("default_employee_id"))
    ...
    def _compute_from_holiday_status_id(self):
        ...
        if accrual_allocations:
            ...
            accruals_dict = {
                res["time_off_type_id"][0]: res["ids"] for res in accruals_read_group
            }
        for allocation in self:
    ...
    def _end_of_year_accrual(self):
        ...
        for allocation in self:
            ...
            lastcall = (
                current_level._get_previous_date(last_day_last_year)
                if allocation.lastcall
                < current_level._get_previous_date(last_day_last_year)
                else allocation.lastcall
            )
            ...
            if current_level.action_with_unused_accruals == "lost":
                ...
                current_level.action_with_unused_accruals == "postponed"
                and current_level.postpone_max_days
            ):
    ...
    def _process_accrual_plan_level(
        ...
        if level.is_based_on_worked_time:
            ...
            left = self.employee_id.sudo()._get_leave_days_data_batch(
                start_dt, end_dt, domain=[("time_type", "=", "leave")]
            )[self.employee_id.id]["hours"]
            work_entry_prorata = (
        ...
        ) and not level.is_based_on_worked_time:
    ...
    def _process_accrual_plans(self, date_to=False, force_period=False):
        ...
        for allocation in self:
            ...
            while allocation.nextcall <= date_to:
                ...
                if (
                    allocation.lastcall.year < allocation.nextcall.year
                    and current_level.action_with_unused_accruals == "postponed"
                    and current_level.postpone_max_days > 0
                ):
                    ...
                    allowed_to_keep = max(
                        0, current_level.postpone_max_days - allocation_days
                    )
                    number_of_days = min(
                        allocation_days, current_level.postpone_max_days
                    )
                    ...
                    days_added_per_level[current_level] = min(
                        total_gained_days, allowed_to_keep
                    )
            ...
            if days_added_per_level:
                ...
                allocation.number_of_days = (
                    min(
                        ...
                        current_level_maximum_leave + allocation.leaves_taken,
                    )
    ...
    def create(self, vals_list):
        ...
        holidays = super(
            HolidaysAllocation, self.with_context(mail_create_nosubscribe=True)
        ).create(vals_list)
        for holiday in holidays:
            ...
            if holiday.validation_type == "officer":
                ...
                partners_to_subscribe.add(
                    holiday.employee_id.leave_manager_id.partner_id.id
                )
    ...
    def write(self, values):
        if not self.env.context.get("toggle_active") and not bool(
            values.get("active", True)
        ):
            if any(
                allocation.state not in ["draft", "cancel", "refuse"]
                for allocation in self
            ):
                raise UserError(
                    _(
    ...
    def action_draft(self):
        if any(holiday.state not in ["confirm", "refuse"] for holiday in self):
            raise UserError(
                _(
                    'Allocation request state must be "Refused" or "To Approve" in order to be reset to Draft.'
                )
            )
        self.write(
            {
    ...
    def action_confirm(self):
        if self.filtered(
            lambda holiday: holiday.state != "draft" and holiday.validation_type != "no"
        ):
            raise UserError(
                _(
                    'Allocation request must be in Draft state ("To Submit") in order to confirm it.'
                )
            )
        validated_holidays = self.filtered(lambda holiday: holiday.state == "validate")
        res = (self - validated_holidays).write({"state": "confirm"})
        self.activity_update()
        no_employee_requests = [
            holiday.id
            for holiday in self.sudo()
            if holiday.holiday_status_id.employee_requests == "no"
        ]
        self.filtered(
            lambda holiday: (
                holiday.id in no_employee_requests or holiday.validation_type == "no"
            )
            and holiday.state != "validate"
        ).action_validate()
        return res
    ...
    def action_validate(self):
        ...
        if any(
            (
                ...
                and holiday.id not in no_employee_requests
                and holiday.validation_type != "no"
            )
            for holiday in self
        ):
            raise UserError(
        ...
        self.write({"state": "validate", "approver_id": current_employee.id})
    ...
    def _check_approval_update(self, state):
        ...
        for holiday in self:
            ...
            if (
                ...
                raise UserError(
                    _(
                        "Only a time off Officer/Responsible or Manager can approve or refuse time off requests."
                    )
                )
    ...
    def _notify_get_recipients_groups(self, msg_vals=None):
        ...
        directly from email."""
        groups = super(HolidaysAllocation, self)._notify_get_recipients_groups(
            msg_vals=msg_vals
        )
        if not self:
            return groups
        ...
        if self.state == "confirm":
            app_action = self._notify_get_action_link(
                "controller", controller="/allocation/validate", **local_msg_vals
            )
            hr_actions += [{"url": app_action, "title": _("Approve")}]
        if self.state in ["confirm", "validate", "validate1"]:
            ref_action = self._notify_get_action_link(
                "controller", controller="/allocation/refuse", **local_msg_vals
            )
    ...
    def message_subscribe(self, partner_ids=None, subtype_ids=None):
        ...
        return super(HolidaysAllocation, self).message_subscribe(
            partner_ids=partner_ids, subtype_ids=subtype_ids
        )
