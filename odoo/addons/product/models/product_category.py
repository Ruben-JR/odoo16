...
class ProductCategory(models.Model):
    ...
    parent_id = fields.Many2one(
        "product.category", "Parent Category", index=True, ondelete="cascade"
    )
    ...
    def _compute_product_count(self):
        ...
        group_data = dict(
            (data["categ_id"][0], data["categ_id_count"]) for data in read_group_res
        )
        for categ in self:
            product_count = 0
    ...
    def _unlink_except_default_category(self):
        ...
        if expense_category in self:
            raise UserError(
                _("You cannot delete the %s product category.", expense_category.name)
            )
