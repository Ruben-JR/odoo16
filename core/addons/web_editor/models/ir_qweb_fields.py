...
class IrQWeb(models.AbstractModel):
    ...
    def _compile_node(self, el, compile_context, indent):
        ...
        if (
            snippet_key == compile_context["template"]
            or compile_context.get("snippet-sub-call-key")
    ...
    def _compile_directive_snippet(self, el, compile_context, indent):
        ...
        div = (
            '<div name="%s" data-oe-type="snippet" data-oe-thumbnail="%s" data-oe-snippet-id="%s" data-oe-keywords="%s" %s>'
            % (
                ...
                f'data-oe-forbid-sanitize="{forbid_sanitize}"'
                if forbid_sanitize
                else "",
            )
        )
    ...
    def _compile_directive_install(self, el, compile_context, indent):
        ...
        if self.user_has_groups("base.group_system"):
            ...
            div = (
                '<div name="%s" data-oe-type="snippet" data-module-id="%s" data-oe-thumbnail="%s"><section/></div>'
                % (
                    escape(pycompat.to_text(name)),
                    module.id,
                    escape(pycompat.to_text(thumbnail)),
        ...
        if options["translate"] and field.type in ("char", "text"):
            ...
            if lang == "en_US":
                ...
                attrs["data-oe-translation-state"] = (
                    "translated" if value_en != value_lang else "to_translate"
                )
...
class Float(models.AbstractModel):
    ...
    def from_html(self, model, field, element):
        ...
        value = element.text_content().strip()
        return float(
            value.replace(lang.thousands_sep, "").replace(lang.decimal_point, ".")
        )
...
class Contact(models.AbstractModel):
    ...
    def get_record_to_html(self, ids, options=None):
        return self.value_to_html(
            self.env["res.partner"].search([("id", "=", ids[0])]), options=options
        )
        ...
        if options.get("inherit_branding"):
            ...
            if record[field_name]:
                ...
                value_format = pycompat.to_text(
                    babel.dates.format_date(date, format=babel_format, locale=locale)
                )
    ...
    def from_html(self, model, field, element):
        ...
        try:
            ...
            raise ValidationError(
                _(
                    "The datetime %s does not match the format %s",
                    value,
                    datetime_format,
                )
            )
...
class Selection(models.AbstractModel):
    ...
    def from_html(self, model, field, element):
        ...
        raise ValueError(
            "No value found for label %s in selection %s" % (value, selection)
        )
...
class HTML(models.AbstractModel):
    ...
    def attributes(self, record, field_name, options, values=None):
        ...
        if options.get("inherit_branding"):
            field = record._fields[field_name]
            if field.sanitize:
                if field.sanitize_overridable and not record.user_has_groups(
                ...
                if not (
                    field.sanitize_overridable
                    and record.user_has_groups("base.group_sanitize_override")
                ):
                    ...
                    attrs["data-oe-sanitize"] = (
                        1 if field.sanitize_form else "allow_form"
                    )
