...
class OssTemplateTestCase(AccountTestInvoicingCommon):
    ...
    def load_specific_chart_template(cls, chart_template_ref):
        try:
            ...
            if (
                e.args[0]
                == f"External ID not found in the system: {chart_template_ref}"
            ):
                cls.skipTest(
                    cls,
                    reason=f"The {chart_template_ref} CoA is required for this testSuite but the corresponding localization module isn't installed",
                )
            else:
                raise e
    ...
    def test_country_tag_from_belgium(self):
        ...
        for doc_type, report_expression_xml_id in (
            ...
            with self.subTest(
                ...
                expected_tag_id = (
                    self.env.ref(report_expression_xml_id)
                    ._get_matching_tags()
...
class TestOSSSpain(OssTemplateTestCase):
    ...
    def test_country_tag_from_spain(self):
        ...
        for doc_type, tag_xml_id in (("invoice", "l10n_es.mod_303_124"),):
    ...
    def test_oss_eu_tag_map(self):
        ...
        for chart_template in chart_templates:
            ...
            for tax_report_line_xml_id in filter(lambda d: d, oss_tags.values()):
                with self.subTest(
                    chart_template_xml_id=chart_template_xml_id,
