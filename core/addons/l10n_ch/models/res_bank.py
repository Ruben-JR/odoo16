...
class ResPartnerBank(models.Model):
    ...
    l10n_ch_show_subscription = fields.Boolean(
        compute="_compute_l10n_ch_show_subscription",
        default=lambda self: self.env.company.account_fiscal_country_id.code == "CH",
    )

    def _is_isr_issuer(self):
        return _is_l10n_ch_isr_issuer(
            self.l10n_ch_postal, "CHF"
    ...
    def _check_subscription_num(self):
        ...
        for rec in self:
            for currency in ["CHF", "EUR"]:
                subscrip = (
                    rec.l10n_ch_isr_subscription_chf
                    if currency == "CHF"
                    else rec.l10n_ch_isr_subscription_eur
                )
                if subscrip and not _is_l10n_ch_isr_issuer(subscrip, currency):
                    ...
                    raise ValidationError(
                        _(
                            ...
                            subscrip, currency, ISR_SUBSCRIPTION_CODE[currency], example
                        )
                    )
        return True
    ...
    def _compute_l10n_ch_show_subscription(self):
        for bank in self:
            if bank.partner_id:
                bank.l10n_ch_show_subscription = (
    ...
    def retrieve_acc_type(self, acc_number):
        ...
        if _is_l10n_ch_postal(acc_number) or (
            acc_number_split and _is_l10n_ch_postal(acc_number_split)
        ):
            return "postal"
        else:
    ...
    def _compute_l10n_ch_postal(self):
        for record in self:
            if record.acc_type == "iban":
                record.l10n_ch_postal = self._retrieve_l10n_ch_postal(
                    record.sanitized_acc_number
                )
                ...
                if record.acc_number and " " in record.acc_number:
                    ...
                    if record.partner_id and record.acc_number[:2] in ["01", "03"]:
                        record.acc_number = ("{} {}").format(
                            record.acc_number, record.partner_id.name
                        )
    ...
    def _eligible_for_qr_code(
        ...
        if (
            qr_method == "sct_qr"
            and debtor_partner.country_id.code == "CH"
            and self.journal_id.country_code == "CH"
        ):
            return False
        if qr_method == "ch_qr":
            error_messages = [
                _("The QR code could not be generated for the following reason(s):")
            ]
            ...
            if not debtor_partner or debtor_partner.country_id.code not in ("CH", "LI"):
                error_messages.append(
                    _("The debtor partner's address isn't located in Switzerland.")
                )
        ...
        return super()._eligible_for_qr_code(
            qr_method, debtor_partner, currency, raises_error
        )
