...
class Project(models.Model):
    ...
    def _compute_expenses_count(self):
        ...
        query.add_where(
            "hr_expense.analytic_distribution ?| array[%s]",
        ...
        data = {
            int(record.get("account_id")): record.get("expense_count")
        ...
        action.update(
            {
                ...
                "context": {
                    "default_analytic_distribution": {self.analytic_account_id.id: 100}
                },
    ...
    def _get_expenses_profitability_items(self, with_action=True):
        if not self.analytic_account_id:
            return {}
        can_see_expense = with_action and self.user_has_groups(
            "hr_expense.group_hr_expense_team_approver"
        )
        query = self.env["hr.expense"]._search(
            [("is_refused", "=", False), ("state", "in", ["approved", "done"])]
        )
        query.order = None
        query.add_where(
            "hr_expense.analytic_distribution ? %s", [str(self.analytic_account_id.id)]
        )
        query_string, query_param = query.select(
            "array_agg(id) as ids", "SUM(untaxed_amount) as untaxed_amount"
        )
        self._cr.execute(query_string, query_param)
        ...
        expense_profitability_items = {
            "costs": {
                ...
                "billed": -expense_data["untaxed_amount"],
                "to_bill": 0.0,
            },
        }
        if can_see_expense:
