...
class View(models.Model):
    ...
    def _get_pwd(self):
        for r in self:
            r.visibility_password_display = (
                r.sudo().visibility_password and "********" or ""
            )
        ...
        for r in self:
            if r.type == "qweb":
                r.sudo().visibility_password = (
                    r.visibility_password_display
                    and crypt_context.encrypt(r.visibility_password_display)
                    or ""
                )
    ...
    def name_get(self):
        if not (
            self._context.get("display_key") or self._context.get("display_website")
        ):
    ...
    def write(self, vals):
        ...
        # inheritance tree.
        for view in self.with_context(active_test=False).sorted(
            key="website_id", reverse=True
        ):
    ...
    def _create_all_specific_views(self, processed_modules):
        ...
        for record in self.browse(result.keys()):
            ...
            for specific_parent_view_id, website_id in zip(
                specific_parent_view_ids, website_ids
            ):
    ...
    def _get_inheriting_views(self):
        ...
        views = super(
            View, self.with_context(active_test=False)
    ...
    def _update_field_translations(self, fname, translations, digest=None):
        return super(View, self.with_context(no_cow=True))._update_field_translations(
            fname, translations, digest
        )
