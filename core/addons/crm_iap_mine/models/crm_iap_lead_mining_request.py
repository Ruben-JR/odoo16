...
class CRMLeadMiningRequest(models.Model):
    ...
    def _compute_display_lead_label(self):
        selection_description_values = {
            e[0]: e[1]
            for e in self._fields["lead_type"]._description_selection(self.env)
        }
        for request in self:
    ...
    def _compute_available_state_ids(self):
        ...
        for lead_mining_request in self:
            countries = lead_mining_request.country_ids.filtered(
                lambda country: country.code
                in iap_tools._STATES_FILTER_COUNTRIES_WHITELIST
            )
    ...
    def _onchange_available_state_ids(self):
        self.state_ids -= self.state_ids.filtered(
            lambda state: (state._origin.id or state.id)
            not in self.available_state_ids.ids
        )
    ...
    def action_submit(self):
        ...
        if results:
            ...
            return {
                ...
                "res_id": self.id,
                "context": dict(
                    self.env.context, edit=True, form_view_initial_mode="edit"
                ),
            }
        else:
