...
class AccountBankStatement(models.Model):
    ...
    def default_get(self, fields_list):
        ...
        if self._context.get("st_line_id"):
            st_line = self.env["account.bank.statement.line"].browse(
                self._context["st_line_id"]
            )
            defaults["balance_start"] = st_line.running_balance - (
                st_line.amount if st_line.state == "posted" else 0
            )
            return defaults
        ...
        if self._context.get("split_line_id"):
            current_st_line = self.env["account.bank.statement.line"].browse(
                self.env.context.get("split_line_id")
            )
        ...
        # completed statements, because it is probably due to a mistake from the user
            ...
            "active_model"
        ) == "account.bank.statement.line" and self._context.get("active_ids"):
            lines = (
                self.env["account.bank.statement.line"]
            ...
            if not lines:
                raise UserError(
                    _(
                        "One or more selected lines already belong to a complete statement."
                    )
                )
        if lines:
    ...
    def _compute_balance_end(self):
        for statement in self:
            statement.balance_end = statement.balance_start + sum(
                statement.line_ids.filtered(lambda l: l.state == "posted").mapped(
                    "amount"
                )
            )
    ...
    def create(self, vals_list):
        ...
        if self._context.get("split_line_id"):
            old_statement = (
                self.env["account.bank.statement.line"]
                .browse(self._context.get("split_line_id"))
                .statement_id
            )
