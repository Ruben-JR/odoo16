...
class AccountMove(models.Model):
    ...
    stock_valuation_layer_ids = fields.One2many(
        "stock.valuation.layer", "account_move_id", string="Stock Valuation Layer"
    )
    ...
    def _compute_show_reset_to_draft_button(self):
        ...
        for move in self:
            for line in move.line_ids:
                ...
                if (
                    line.sudo()
                    ._get_stock_valuation_layers(move)
                    .stock_valuation_layer_ids.filtered("account_move_line_id")
                ):
        ...
        for move in self:
            ...
            for prod in products:
                ...
                if product_interim_account.reconcile:
                    ...
                    product_account_moves |= (
                        product_stock_moves._get_all_related_aml().filtered(
                    ...
                    # Reconcile.
                    if any(
                        aml.amount_currency and not aml.balance
                        for aml in product_account_moves
                    ):
                        stock_aml = product_account_moves.filtered(
                            lambda aml: aml.move_id.stock_valuation_layer_ids.stock_move_id
                        )
                        invoice_aml = product_account_moves.filtered(
                            lambda aml: aml.move_id == move
                        )
                        correction_amls = (
                            product_account_moves - stock_aml - invoice_aml
                        )
                        if sum(correction_amls.mapped("balance")) > 0:
                            product_account_moves.with_context(
                                no_exchange_difference=True
                            ).reconcile()
                        else:
                            (invoice_aml | correction_amls).with_context(
                                no_exchange_difference=True
                            ).reconcile()
                            (invoice_aml | stock_aml).with_context(
                                no_exchange_difference=True
                            ).reconcile()
                    else:
...
class AccountMoveLine(models.Model):
    ...
    stock_valuation_layer_ids = fields.One2many(
        "stock.valuation.layer", "account_move_line_id", string="Stock Valuation Layer"
    )
    ...
    def _get_gross_unit_price(self):
        ...
        price_unit = self.tax_ids.with_context(round=False).compute_all(
            ...
            quantity=1.0,
            is_refund=self.move_id.move_type == "in_refund",
    ...
    def _get_stock_valuation_layers(self, move):
        ...
        if move.move_type == "in_refund":
            valued_moves = valued_moves.filtered(
    ...
    def _get_stock_layer_price_difference(self, layers, layers_price_unit, price_unit):
        ...
        for layer in layers:
            ...
            layers_to_correct[layer] = (
                qty_to_correct,
                unit_valuation_difference,
                price_difference_curr,
            )
    ...
    def _prepare_in_invoice_svl_vals(self, layers_correction):
        ...
        common_svl_vals = {
            ...
            and "%s - %s" % (self.move_id.name, self.product_id.name)
        ...
        for layer, (
            ...
            svl_vals.update(
                **common_svl_vals,
                ...
                price_diff_value=diff_value_curr,
            )
    ...
    def _stock_account_get_anglo_saxon_price_unit(self):
        ...
        return (
            ...
            if original_line
            else self.product_id.with_company(
                self.company_id
            )._stock_account_get_anglo_saxon_price_unit(uom=self.product_uom_id)
        )
