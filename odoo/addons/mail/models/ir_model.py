...
class IrModel(models.Model):
    ...
    def _reflect_model_params(self, model):
        ...
        vals["is_mail_activity"] = issubclass(
            type(model), self.pool["mail.activity.mixin"]
        )
        vals["is_mail_blacklist"] = issubclass(
            type(model), self.pool["mail.thread.blacklist"]
        )
        return vals
    ...
    def _instanciate(self, model_data):
        ...
        if (
            ...
            and model_class._name != "mail.activity.mixin"
        ):
            parents = model_class._inherit or []
            parents = [parents] if isinstance(parents, str) else parents
        ...
        if (
            ...
            and model_class._name != "mail.thread.blacklist"
        ):
            parents = model_class._inherit or []
            parents = [parents] if isinstance(parents, str) else parents
