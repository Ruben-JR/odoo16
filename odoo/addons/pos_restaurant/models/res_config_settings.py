...
class ResConfigSettings(models.TransientModel):
    ...
    def _get_floors_domain(self):
        return [
            "|",
            ("pos_config_id", "in", self.pos_config_id.ids),
            ("pos_config_id", "=", False),
        ]

    pos_floor_ids = fields.One2many(
        related="pos_config_id.floor_ids",
        readonly=False,
        domain=lambda self: self._get_floors_domain(),
    )
    pos_iface_orderline_notes = fields.Boolean(
    ...
    pos_is_table_management = fields.Boolean(
        compute="_compute_pos_module_pos_restaurant", store=True, readonly=False
    )
