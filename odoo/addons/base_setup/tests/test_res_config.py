...
class TestResConfig(TransactionCase):
    ...
    def test_multi_company_res_config_group(self):
        ...
        self.assertTrue(
            new_user in self.env.ref("base.group_multi_currency").sudo().users
        )
        ...
        ResConfig.create(default_values).execute()
        self.assertTrue(
            user not in self.env.ref("base.group_multi_currency").sudo().users
        )
        ...
        self.assertTrue(
            new_user not in self.env.ref("base.group_multi_currency").sudo().users
        )
