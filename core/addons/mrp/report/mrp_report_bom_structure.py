...
class ReportBomStructure(models.AbstractModel):
    ...
    def get_warehouses(self):
        return self.env["stock.warehouse"].search_read(
            [("company_id", "=", self.env.company.id)], fields=["id", "name"]
        )
    ...
    def _compute_current_production_capacity(self, bom_data):
        ...
        stockable_components = filter(
            ...
            and not float_is_zero(
                c["base_bom_line_qty"], precision_digits=c["uom"].rounding
            ),
            bom_data.get("components", []),
        )
        producibles = [
            float_round(
                comp["quantity_available"] / comp["base_bom_line_qty"],
                precision_digits=0,
                rounding_method="DOWN",
            )
            for comp in stockable_components
        ]
    ...
    def _compute_production_capacities(self, bom_qty, bom_data):
        ...
        if bom_data.get("producible_qty", 0):
            ...
            res["earliest_date"] = format_date(
                self.env, date_today + timedelta(days=bom_data["lead_time"])
            )
        ...
        if bom_data["availability_state"] != "unavailable":
            if same_delay:
                ...
                res["leftover_date"] = format_date(
                    self.env,
                    date_today + timedelta(days=bom_data["availability_delay"]),
                )
    ...
    def _get_pdf_doc(self, bom_id, data, quantity, product_variant_id=None):
        ...
        doc["show_availabilities"] = (
            False if data and data.get("availabilities") == "false" else True
        )
        doc["show_costs"] = False if data and data.get("costs") == "false" else True
        doc["show_operations"] = (
            False if data and data.get("operations") == "false" else True
        )
        doc["show_lead_times"] = (
            False if data and data.get("lead_times") == "false" else True
        )
        return doc
    ...
    def _get_bom_data(
        ...
        if not product_info[key].get(bom_key):
            ...
            )._get_resupply_route_info(warehouse, product, current_quantity, bom)
        ...
        if not ignore_stock:
            ...
            quantities_info = self._get_quantities_info(
                product, bom.product_uom_id, parent_bom, product_info
            )
        ...
        if not is_minimized:
            operations = self._get_operation_line(
                ...
                bom,
                float_round(
                    current_quantity, precision_rounding=1, rounding_method="UP"
                ),
                level + 1,
                index,
            )
    ...
    def _get_component_data(
        ...
        if not product_info[key].get(bom_key):
            product_info[key][bom_key] = self.with_context(
                product_info=product_info, parent_bom=parent_bom
            )._get_resupply_route_info(warehouse, bom_line.product_id, line_quantity)
        route_info = product_info[key].get(bom_key, {})
        ...
        if not ignore_stock:
            # Useless to compute quantities_info if it's not going to be used later on
            quantities_info = self._get_quantities_info(
                bom_line.product_id, bom_line.product_uom_id, parent_bom, product_info
            )
        availabilities = self._get_availabilities(
            bom_line.product_id,
            line_quantity,
    ...
    def _get_quantities_info(self, product, bom_uom, parent_bom, product_info):
        return {
            "free_qty": product.uom_id._compute_quantity(product.free_qty, bom_uom)
    ...
    def _get_operation_line(self, product, bom, qty, level, index):
        ...
        for operation in bom.operation_ids:
            ...
            operation_cycle = float_round(
                qty / capacity, precision_rounding=1, rounding_method="UP"
            )
            duration_expected = (
                operation_cycle
    ...
    def _get_resupply_route_info(self, warehouse, product, quantity, bom=False):
        found_rules = []
        if self._need_special_rules(
            self.env.context.get("product_info"),
            self.env.context.get("parent_bom"),
            self.env.context.get("parent_product_id"),
        ):
        ...
        return self._format_route_info(
            found_rules, rules_delay, warehouse, product, bom, quantity
        )

    @api.model
    def _need_special_rules(
        self, product_info, parent_bom=False, parent_product_id=False
    ):
        return False

    @api.model
    def _find_special_rules(
        self, product, product_info, parent_bom=False, parent_product_id=False
    ):
        return False
    ...
    def _format_route_info(self, rules, rules_delay, warehouse, product, bom, quantity):
        ...
        if manufacture_rules:
            ...
            manufacturing_lead = (
                bom.company_id.manufacturing_lead if bom and bom.company_id else 0
            )
            return {
    ...
    def _get_availabilities(
        ...
        if not ignore_stock:
            stock_state, stock_delay = self._get_stock_availability(
                product, quantity, product_info, quantities_info
            )
    ...
    def _get_stock_availability(self, product, quantity, product_info, quantities_info):
        ...
        if stock_loc == "in_stock":
            ...
            closest_forecasted = self.env["report.stock.quantity"]._read_group(
                domain, ["min_date:min(date)", "product_id"], ["product_id"]
            )
    ...
    def _has_attachments(self, data):
        return data["attachment_ids"] or any(
