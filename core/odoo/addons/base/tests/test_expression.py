...
class TestExpression(SavepointCaseWithUserDemo):
    ...
    def test_00_in_not_in_m2m(self):
        ...
        with_a_or_with_b = self._search(
            partners,
            ["|", ("category_id", "in", [cat_a.id]), ("category_id", "in", [cat_b.id])],
        )
        self.assertEqual(
            a + b + ab,
            with_a_or_with_b,
        ...
        self.assertEqual(
            ab, with_a_and_b, "Search for category_id contains cat_a and cat_b failed."
        )
        ...
        self.assertLessEqual(
            b + c, without_a, "Search for category_id doesn't contain cat_a failed (3)."
        )
        ...
        self.assertLessEqual(
            a + c, without_b, "Search for category_id doesn't contain cat_b failed (3)."
        )
    ...
    def test_10_hierarchy_access(self):
        ...
        self.assertEqual(
            Bank.search([("partner_id", "in", accessible.ids)]), bank_top + bank_bot
        )
        self.assertEqual(
            Bank.search([("partner_id", "child_of", top.ids)]), bank_top + bank_bot
        )
        self.assertEqual(
            Bank.search([("partner_id", "parent_of", bot.ids)]), bank_top + bank_bot
        )
    ...
    def test_15_m2o(self):
        ...
        res_partners = self._search(
            Partner, ["|", ("company_id", "not in", []), ("company_id", "=", False)]
        )
        ...
        self.assertTrue(
            len(partners) >= 4, "We should have at least 4 partners with no company"
        )
        ...
        partners = self._search(
            Partner, ["|", ("company_id", "not in", [1]), ("company_id", "=", False)]
        )
        self.assertTrue(
            len(partners) >= 8,
    ...
    def test_15_equivalent_one2many_2(self):
        ...
        currency_rate1 = self._search(
            CurrencyRate, [("currency_id", "not like", "probably_unexisting_name")]
        )
    ...
    def test_20_expression_parse(self):
        ...
        b2 = Users.create(
            {"name": "test_B2", "login": "test_B2", "parent_id": b1.partner_id.id}
        )
        ...
        users = self._search(
            Users, [("name", "like", "test"), ("parent_id", "=?", b1.partner_id.id)]
        )
    ...
    def test_30_normalize_domain(self):
        ...
        self.assertEqual(
            ...
            "Normalized domains should be left untouched",
        )
        domain = [
            ("x", "in", ["y", "z"]),
            ("a.v", "=", "e"),
        ...
        self.assertEqual(
            norm_domain,
            expression.normalize_domain(domain),
            "Non-normalized domains should be properly normalized",
        )

    def test_35_negating_thruty_leafs(self):
        self.assertEqual(
            expression.distribute_not(["!", "!", expression.TRUE_LEAF]),
            [expression.TRUE_LEAF],
            "distribute_not applied wrongly",
        )
        self.assertEqual(
            expression.distribute_not(["!", "!", expression.FALSE_LEAF]),
    ...
    def test_40_negating_long_expression(self):
        ...
        self.assertEqual(
            expression.distribute_not(source),
            expect,
...
class TestAutoJoin(TransactionCase):
    def test_auto_join(self):
        ...
        p_aa = partner_obj.create(
            {"name": "test__AA", "parent_id": p_a.id, "state_id": states[0].id}
        )
        p_ab = partner_obj.create(
            {"name": "test__AB", "parent_id": p_a.id, "state_id": states[1].id}
        )
        p_ba = partner_obj.create(
            {"name": "test__BA", "parent_id": p_b.id, "state_id": states[0].id}
        )
        ...
        partners = partner_obj.search(
            ["|", ("state_id.code", "=", states[0].code), ("name", "like", "C")]
        )
        ...
        patch_auto_join(partner_obj, "bank_ids", True)
        patch_domain(
            partner_obj, "child_ids", lambda self: ["!", ("name", "=", self._name)]
        )
        patch_domain(partner_obj, "bank_ids", [("sanitized_acc_number", "like", "2")])

        # Do: 2 cascaded one2many with _auto_join, test final leaf is an id
        partners = partner_obj.search(
            ["&", (1, "=", 1), ("child_ids.bank_ids.id", "in", [b_aa.id, b_ba.id])]
        )
        self.assertLessEqual(
            p_a, partners, "_auto_join on one2many with domains incorrect result"
        )
        self.assertFalse(
            (p_ab + p_ba) & partners,
        ...
        patch_domain(
            partner_obj, "child_ids", lambda self: [("name", "=", "__%s" % self._name)]
        )
        partners = partner_obj.search(
            ["&", (1, "=", 1), ("child_ids.bank_ids.id", "in", [b_aa.id, b_ba.id])]
        )
        self.assertFalse(
            partners, "_auto_join on one2many with domains incorrect result"
        )
...
class TestMany2many(TransactionCase):
    ...
    def test_regular(self):
        ...
        self.User.search(
            [("groups_id.rule_groups.name", "like", rule.name)], order="id"
        )
