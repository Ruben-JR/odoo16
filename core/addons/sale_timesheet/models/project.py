...
class Project(models.Model):
    ...
    warning_employee_rate = fields.Boolean(
        compute="_compute_warning_employee_rate", compute_sudo=True
    )
    partner_id = fields.Many2one(
        compute="_compute_partner_id", store=True, readonly=False
    )
    allocated_hours = fields.Float(
        compute="_compute_allocated_hours", store=True, readonly=False, copy=False
    )
    ...
    def _compute_billable_percentage(self):
        timesheets_read_group = self.env["account.analytic.line"]._read_group(
            ...
            ["project_id", "so_line", "unit_amount"],
            ["project_id", "so_line"],
            lazy=False,
        )
        timesheets_by_project = defaultdict(list)
        for res in timesheets_read_group:
            timesheets_by_project[res["project_id"][0]].append(
                (res["unit_amount"], bool(res["so_line"]))
            )
        for project in self:
            ...
            billable_percentage = (
                timesheet_billable / timesheet_total * 100 if timesheet_total > 0 else 0
            )
    ...
    def _compute_display_create_order(self):
        for project in self:
            project.display_create_order = (
                project.partner_id and project.pricing_type == "task_rate"
            )
    ...
    def _compute_warning_employee_rate(self):
        ...
        for line in employees:
            dict_project_employee[line["project_id"][0]] += (
                [line["employee_id"][0]] if line["employee_id"] else []
            )
        for project in projects:
            project.warning_employee_rate = any(
                x not in project.sale_line_employee_ids.employee_id.ids
                for x in dict_project_employee[project.id]
            )
    ...
    def _update_timesheets_sale_line_id(self):
        for project in self.filtered(lambda p: p.allow_billable and p.allow_timesheets):
            timesheet_ids = (
                ...
                .filtered(lambda t: not t.is_so_line_edited and t._is_not_billed())
            )
        ...
        if section_name in [
            ...
            if section_name == "billable_time":
                graph_view = self.env.ref(
                    "hr_timesheet.view_hr_timesheet_line_graph_all"
                ).id
            action["views"] = [
                (view_id, view_type)
                if view_type != "graph"
    ...
    def _get_sale_order_items_query(self, domain_per_model=None):
        ...
        (
            ...
            employee_mapping_params,
        ) = employee_mapping_query.select(
...
class ProjectTask(models.Model):
    ...
    def _get_default_partner_id(self, project, parent):
        ...
        if not res and project:
            ...
            if related_project.pricing_type == "employee_rate":
                return related_project.sale_line_employee_ids.sale_line_id.order_partner_id[
                    :1
                ]
        return res
    ...
    so_analytic_account_id = fields.Many2one(
        related="sale_order_id.analytic_account_id",
        string="Sale Order Analytic Account",
    )
    ...
    def _compute_remaining_hours_so(self):
        ...
        mapped_remaining_hours = {
            task._origin.id: task.sale_line_id
            and task.sale_line_id.remaining_hours
            or 0.0
            for task in self
        }
    ...
    def _compute_analytic_account_active(self):
        ...
        for task in self:
            task.analytic_account_active = (
                task.analytic_account_active or task.so_analytic_account_id.active
            )
    ...
    def _get_last_sol_of_customer(self):
        ...
        if (
            ...
            and self.commercial_partner_id
...
class ProjectTaskRecurrence(models.Model):
    ...
    def _get_recurring_fields(self):
        return ["so_analytic_account_id"] + super(
            ProjectTaskRecurrence, self
        )._get_recurring_fields()
