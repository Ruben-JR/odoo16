...
class MailTemplatePreview(models.TransientModel):
    ...
    def default_get(self, fields):
        ...
        if not result.get("mail_template_id") or "resource_ref" not in fields:
            return result
        mail_template = (
            self.env["mail.template"].browse(result["mail_template_id"]).sudo()
        )
    ...
    attachment_ids = fields.Many2many(
        "ir.attachment", "Attachments", compute="_compute_mail_template_fields"
    )
    ...
    def _compute_mail_template_fields(self):
        ...
        try:
            if not self.resource_ref:
                ...
                ).generate_email(self.resource_ref.id, self._MAIL_TEMPLATE_FIELDS)
