...
class ResourceCalendar(models.Model):
    ...
    def _get_global_attendances(self):
        return self.attendance_ids.filtered(
            lambda attendance: not attendance.date_from
    ...
    def switch_calendar_type(self):
        if self == self.env.company.resource_calendar_id:
            raise UserError(
                _(
                    "Impossible to switch calendar type for the default company schedule."
                )
            )
    ...
    def _get_max_number_of_hours(self, start, end):
        ...
        for attendance in self.attendance_ids.filtered(
            lambda a: (not a.date_from or not a.date_to)
...
class ResourceCalendarAttendance(models.Model):
    ...
    def _compute_display_name(self):
        ...
        for record in self.filtered(lambda l: l.display_type == "line_section"):
            section_name = "%s (%s)" % (
                section_names[record.week_type],
                section_info[this_week_type == record.week_type],
            )
...
class ResourceResource(models.Model):
    ...
    def _adjust_to_calendar(self, start, end, compute_leaves=True):
        ...
        for resource in self:
            ...
            calendar_start = resource.calendar_id._get_closest_work_time(
                start,
                resource=resource,
            ...
            calendar_end = resource.calendar_id._get_closest_work_time(
...
class ResourceCalendarLeaves(models.Model):
    ...
    def _compute_company_id(self):
        for leave in self:
            leave.company_id = (
                leave.calendar_id.company_id or leave.company_id or self.env.company
            )
