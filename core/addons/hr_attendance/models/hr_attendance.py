...
class HrAttendance(models.Model):
    ...
    def _get_day_start_and_day(self, employee, dt):
        ...
        return (
            start_day_employee_tz.astimezone(pytz.utc).replace(tzinfo=None),
    ...
    def _get_attendances_dates(self):
        ...
        for attendance in self.filtered(
            lambda a: a.employee_id.company_id.hr_attendance_overtime and a.check_in
        ):
    ...
    def _update_overtime(self, employee_attendance_dates=None):
        ...
        for emp, attendance_dates in employee_attendance_dates.items():
            ...
            expected_attendances -= (
                leave_intervals[False] | leave_intervals[emp.resource_id.id]
            )
                ...
                if not unfinished_shifts and attendances:
                    ...
                    # The employee usually work on that day
                        ...
                        for calendar_attendance in working_times[attendance_date]:
                            planned_start_dt = (
                                min(planned_start_dt, calendar_attendance[0])
                                if planned_start_dt
                                else calendar_attendance[0]
                            )
                            planned_end_dt = (
                                max(planned_end_dt, calendar_attendance[1])
                                if planned_end_dt
                                else calendar_attendance[1]
                            )
                        ...
                        for attendance in attendances:
                            ...
                            delta_in = (
                                planned_start_dt - local_check_in
                            ).total_seconds() / 3600.0
                            ...
                            if (
                                local_check_in <= planned_end_dt
                                and local_check_out >= planned_start_dt
                            ):
                                work_duration += (
                            ...
                            if local_check_out > planned_end_dt:
                                post_work_time += (
                                    local_check_out
                                    - max(planned_end_dt, local_check_in)
                                ).total_seconds() / 3600.0
                        ...
                        overtime_duration_real = (
                            sum(attendances.mapped("worked_hours"))
                            - planned_work_duration
                        )
