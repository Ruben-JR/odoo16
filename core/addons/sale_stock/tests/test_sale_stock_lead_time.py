...
class TestSaleStockLeadTime(TestSaleCommon, ValuationReconciliationTestCommon):
    ...
    def test_00_product_company_level_delays(self):
        ...
        self.assertEqual(
            order.order_line.customer_lead, self.test_product_order.sale_delay
        )
    ...
    def test_01_product_route_level_delays(self):
        ...
        self.company_data["default_warehouse"].write(
            {"delivery_steps": "pick_pack_ship"}
        )

        # Set delay on pull rule
        for pull_rule in self.company_data[
            "default_warehouse"
        ].delivery_route_id.rule_ids:
        ...
        out = order.picking_ids.filtered(
            ...
            == self.company_data["default_warehouse"].out_type_id
        )
        ...
        pack = order.picking_ids.filtered(
            lambda r: r.picking_type_id
            == self.company_data["default_warehouse"].pack_type_id
        )
        ...
        pick = order.picking_ids.filtered(
            lambda r: r.picking_type_id
            == self.company_data["default_warehouse"].pick_type_id
        )
    ...
    def test_02_delivery_date_propagation(self):
        ...
        # Set delay on pull rule
        self.company_data["default_warehouse"].delivery_route_id.rule_ids.write(
            {"delay": 5}
        )
        ...
        out_scheduled_date = deadline_date - timedelta(
        ...
        pack_scheduled_date = out_scheduled_date - timedelta(
        ...
        pick_scheduled_date = pack_scheduled_date - timedelta(
