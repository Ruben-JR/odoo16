...
class UtmCampaign(models.Model):
    ...
    ab_testing_total_pc = fields.Integer(
        "Total A/B test percentage", compute="_compute_ab_testing_total_pc", store=True
    )
        ...
        for campaign in self:
            campaign.ab_testing_total_pc = sum(
                [
                    ...
                    for mailing in campaign.mailing_mail_ids.filtered(
                        "ab_testing_enabled"
                    )
                ]
            )
    ...
    def _compute_mailing_mail_count(self):
        if self.ids:
            ...
            for data in mailing_data:
                if data["ab_testing_enabled"]:
                    ab_testing_mapped_data.setdefault(
                        data["campaign_id"][0], []
                    ).append(data["__count"])
                mapped_data.setdefault(data["campaign_id"][0], []).append(
                    data["__count"]
                )
        else:
            mapped_data = dict()
        ...
        for campaign in self:
            campaign.mailing_mail_count = sum(
                mapped_data.get(campaign._origin.id or campaign.id, [])
            )
            campaign.ab_testing_mailings_count = sum(
                ab_testing_mapped_data.get(campaign._origin.id or campaign.id, [])
            )
    ...
    def _check_ab_testing_total_pc(self):
        for campaign in self:
            if (
                not campaign.ab_testing_completed
                and campaign.ab_testing_total_pc >= 100
            ):
