...
class PaypalTest(PaypalCommon, PaymentHttpCommon):
    def _get_expected_values(self):
        ...
        if self.paypal.fees_active:
            fees = self.currency.round(
                self.paypal._compute_fees(
    ...
    def test_redirect_form_values(self):
        ...
        self.assertEqual(
            form_info["action"], "https://www.sandbox.paypal.com/cgi-bin/webscr"
        )
    ...
    def test_fees_computation(self):
        ...
        self.assertEqual(
            round(total_fee, 2), 3.3, "Wrong computation of the Paypal fees"
        )
    ...
    def test_parsing_pdt_validation_response_returns_notification_data(self):
        ...
        notification_data = PaypalController._parse_pdt_validation_response(
            response_content
        )
