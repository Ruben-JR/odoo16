...
class AutoCompleteController(http.Controller):
    def _translate_google_to_standard(self, google_fields):
        ...
        for google_field in google_fields:
            fields_standard = (
                FIELDS_MAPPING[google_field["type"]]
                if google_field["type"] in FIELDS_MAPPING
                else []
            )
            ...
            for field_standard in fields_standard:
                ...
                if field_standard == "country":
                    standard_data[field_standard] = (
                        request.env["res.country"]
                        .search([("code", "=", google_field["short_name"].upper())])[0]
                        .id
                    )
    ...
    def _perform_place_search(
        ...
        try:
            results = requests.get(
                f"{GOOGLE_PLACES_ENDPOINT}/autocomplete/json",
                params=params,
                timeout=TIMEOUT,
            ).json()
    ...
    def _perform_complete_place_search(
        ...
        try:
            results = requests.get(
                f"{GOOGLE_PLACES_ENDPOINT}/details/json", params=params, timeout=TIMEOUT
            ).json()
        except (TimeoutError, ValueError) as e:
            _logger.error(e)
            return {"address": None}
        ...
        results.sort(
            key=lambda r: FIELDS_PRIORITY.index(r["type"])
        ...
        if "number" not in standard_address:
            ...
            standard_address[
                "formatted_street_number"
            ] = f'{standard_address["number"]} {standard_address.get("street", "")}'
        else:
    ...
    def _autocomplete_address_full(
        self, address, session_id=None, google_place_id=None, **kwargs
    ):
        api_key = (
            request.env["website"].get_current_website().sudo().google_places_api_key
        )
