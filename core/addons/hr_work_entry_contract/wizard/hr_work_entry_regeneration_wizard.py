...
class HrWorkEntryRegenerationWizard(models.TransientModel):
    ...
    validated_work_entry_ids = fields.Many2many(
        "hr.work.entry",
    ...
    def _compute_validated_work_entry_ids(self):
        for wizard in self:
            ...
            if wizard.search_criteria_completed:
                ...
                validated_work_entry_ids = self.env["hr.work.entry"].search(
                    search_domain, order="date_start"
                )
    ...
    def _compute_valid(self):
        for wizard in self:
            wizard.valid = (
                ...
                and len(wizard.validated_work_entry_ids) == 0
            )
        ...
        for wizard in self:
            ...
            if wizard.search_criteria_completed:
                ...
                if (
                    ...
                    wizard.earliest_available_date_message = (
                        "The earliest available date is {date}".format(
                            date=self._date_to_string(wizard.earliest_available_date)
                        )
                    )
                if (
                    ...
                    wizard.latest_available_date_message = (
                        "The latest available date is {date}".format(
                            date=self._date_to_string(wizard.latest_available_date)
                        )
                    )
        ...
        if not self.env.context.get("work_entry_skip_validation"):
            if not self.valid:
                raise ValidationError(
                    _(
                        "In order to regenerate the work entries, you need to provide the wizard with an employee_id, a date_from and a date_to. In addition to that, the time interval defined by date_from and date_to must not contain any validated work entries."
                    )
                )
