...
class AccountMove(models.Model):
        ...
        for inv in self.filtered(lambda x: x.company_id.l10n_ar_company_requires_vat):
            ...
            for line in inv.mapped("invoice_line_ids").filtered(
                ...
                if len(vat_taxes) != 1:
                    raise UserError(
                        _(
                            'There should be a single tax from the "VAT" tax group per line, add it to "%s". If you already have it, please check the tax configuration, in advanced options, in the corresponding field "Tax Group".'
                        )
                        % line.name
                    )
                    ...
                    raise UserError(
                        _(
                            'On invoice id "%s" you must use VAT taxes different than VAT Not Applicable.'
                        )
                        % inv.id
                    )
    ...
    def _l10n_ar_get_amounts(self, company_currency=False):
        ...
        return {
            ...
            * sum(
                tax_lines.filtered(
                    lambda r: r.tax_line_id.tax_group_id.l10n_ar_tribute_afip_code
                    == "07"
                ).mapped(amount_field)
            ),
