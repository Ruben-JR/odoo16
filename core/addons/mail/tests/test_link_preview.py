...
class TestLinkPreview(MailCommon):
    ...
    def setUpClass(cls):
        ...
        cls.user_1 = mail_channel_new_test_user(
            cls.env, login="user_1", name="User 1", groups="base.group_user"
        )
    ...
    def test_01_link_preview_throttle(self):
        with patch.object(requests.Session, "get", _patched_get_html), patch.object(
            requests.Session, "head", _patch_head_html
        ):
            throttle = int(
