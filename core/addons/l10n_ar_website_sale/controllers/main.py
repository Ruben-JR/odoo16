            ...
            res.update(
                {
                    ...
                    "responsibility_types": request.env[
            ...
            res.update(
                {
                    "l10n_latam_identification_type_id": int(
                        data["l10n_latam_identification_type_id"]
                    )
        ...
        * Any other (Monotributista, RI, etc): should select always "CUIT" identification type
        ...
        if request.website.sudo().company_id.country_id.code == "AR":
            if mode[1] == "billing":
                ...
                id_type = (
                    request.env["l10n_latam.identification.type"].browse(id_type_id)
                    if id_type_id
                    else False
                )
                afip_resp = (
                    request.env["l10n_ar.afip.responsibility.type"].browse(afip_resp_id)
                    if afip_resp_id
                    else False
                )
