...
class TestMailNotifyAPI(TestMessagePostCommon):
        ...
        for email_xmlid in [
            ...
            with self.mock_mail_gateway():
                test_record._notify_thread_by_email(
                    test_message, recipients_data, force_send=False
                )
            self.assertEqual(
                len(self._new_mails),
                2,
    ...
    def test_notify_recipients_internals(self):
        ...
        classify_res = self.env[self.test_record._name]._notify_get_recipients_classify(
            pdata, "My Custom Model Name", msg_vals=notify_msg_vals
        )
        # find back information for each recipients
        partner_info = next(
            item for item in classify_res if item["recipients"] == self.partner_1.ids
        )
        emp_info = next(
            item
            for item in classify_res
            if item["recipients"] == self.partner_employee.ids
        )
        ...
        self.assertIn(
            "model=%s" % self.test_record._name, emp_info["button_access"]["url"]
        )
        self.assertIn(
            "res_id=%s" % self.test_record.id, emp_info["button_access"]["url"]
        )
        ...
        for model, res_id in (
            ...
            with self.subTest(model=model, res_id=res_id):
                ...
                partner_info = next(
                    item
                    for item in classify_res
                    if item["recipients"] == self.partner_1.ids
                )
                emp_info = next(
                    item
                    for item in classify_res
...
class TestMessageNotify(TestMessagePostCommon):
    ...
    def test_notify(self):
        ...
        self.assertMessageFields(
            new_notification,
            {
                ...
                "email_from": formataddr(
                    (self.partner_employee.name, self.partner_employee.email_normalized)
                ),
        ...
        self.assertIn(
            "model=%s" % self.test_record._name,
            admin_access_link,
            "The access link should contain a valid model argument",
        )
        self.assertIn(
            "res_id=%d" % self.test_record.id,
            admin_access_link,
    ...
    def test_notify_from_user_id(self):
        ...
        self.assertEqual(
            len(self._new_msgs), 2, "Should have 2 messages: tracking and assignment"
        )
        assign_notif = self._new_msgs.filtered(
            lambda msg: msg.message_type == "user_notification"
        )
...
class TestMessagePost(TestMessagePostCommon, CronMixinCase):
    ...
    def test_message_post(self):
        ...
        with self.assertSinglePostNotifications(
            [{"partner": self.partner_employee_2, "type": "inbox"}], {"content": "Body"}
        ):
        ...
        self.assertMessageFields(
            new_message,
            {
                ...
                "reply_to": formataddr(
                    (
                        "%s %s" % (self.company_admin.name, test_record.name),
                        "%s@%s" % (self.alias_catchall, self.alias_domain),
                    )
                ),
                "res_id": test_record.id,
        ...
        self.assertFalse(
            self.env["mail.mail"]
            .sudo()
            .search_count([("mail_message_id", "=", new_message.id)])
        )
    ...
    def test_message_post_defaults(self):
        ...
        self.assertMessageFields(
            new_message,
            {
                ...
                "reply_to": formataddr(
                    (
                        f"{self.company_admin.name} {test_record.name}",
    ...
    def test_message_post_schedule(self):
        ...
        with freeze_time(
            now
        ), self.assertMsgWithoutNotifications(), self.capture_triggers(cron_id) as capt:
            msg = test_record.message_post(
                body="<p>Test</p>",
                message_type="comment",
                subject="Subject",
        ...
        with freeze_time(now + timedelta(days=5)), self.mock_mail_gateway(
            mail_unlink_sent=True
        ):
            self.env["mail.message.schedule"].sudo()._send_notifications_cron()
    ...
    def test_message_post_schedule_update(self):
        ...
        with freeze_time(now), self.assertNoNotifications(), self.capture_triggers(
            cron.id
        ) as capt:
    ...
    def test_message_post_w_attachments(self):
        ...
        self.assertEqual(
            set(msg.attachment_ids.mapped("res_model")), set([self.test_record._name])
        )
        self.assertEqual(
            set(msg.attachment_ids.mapped("res_id")), set([self.test_record.id])
        )
        self.assertEqual(
        ...
        self.assertSentEmail(
            ...
            [self.partner_1],
            attachments=[
                ("List1", b"My first attachment", "application/octet-stream"),
    ...
    def test_message_post_w_attachments_filtering(self):
        ...
        """
        xml_attachment, pdf_attachment = ("List1", b"My xml attachment"), (
            "List2",
            b"My pdf attachment",
        )
    ...
    @mute_logger(
        "odoo.addons.mail.models.mail_mail", "odoo.addons.mail.models.mail_thread"
    )
    @users("employee")
...
class TestMessagePostHelpers(TestMessagePostCommon):
    ...
    def test_message_mail_with_template(self):
        ...
        with self.mock_mail_gateway():
            _new_mails, _new_messages = test_records.with_user(
                self.user_employee
    ...
    def test_message_post_with_template(self):
        ...
        with self.mock_mail_gateway():
            _new_mail, new_message = test_record.with_user(
                self.user_employee
            ).message_post_with_template(
                test_template.id,
...
class TestMessagePostLang(TestMailCommon, TestRecipients):
    ...
    def setUpClass(cls):
        ...
        cls.user_employee.write(
            {  # add group to create contacts, necessary for templates
                "groups_id": [(4, cls.env.ref("base.group_partner_manager").id)],
            }
        )
    ...
    def test_composer_lang_template_mass(self):
        ...
        for record, customer in zip(test_records, record0_customer + self.partner_2):
            ...
            #                  'Subject based on template should be translated')
            self.assertEqual(
                f"EnglishSubject for {record.name}",
                customer_email["subject"],
    ...
    def test_layout_email_lang_context(self):
        ...
        self.assertIn(
            "SpanishButtonTitle", body, "Groups-based action names should be translated"
        )
