...
class WebsiteTest(Home):
    ...
    def test_company_context(self):
        return request.make_response(
            json.dumps(request.context.get("allowed_company_ids"))
        )
    ...
    @http.route(
        "/test_get_dbname", type="json", auth="public", website=True, sitemap=False
    )
    ...
    @http.route(
        "/test_user_error_json", type="json", auth="public", website=True, sitemap=False
    )
