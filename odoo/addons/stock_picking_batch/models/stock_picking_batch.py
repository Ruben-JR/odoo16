...
class StockPickingBatch(models.Model):
    ...
    show_validate = fields.Boolean(
        compute="_compute_show_validate", string="Show Validate Button"
    )
    ...
    def _compute_show_validate(self):
        for batch in self:
            batch.show_validate = any(
                picking.show_validate for picking in batch.picking_ids
            )
    ...
    def _compute_scheduled_date(self):
        for rec in self:
            rec.scheduled_date = min(
                rec.picking_ids.filtered("scheduled_date").mapped("scheduled_date"),
                default=False,
            )
    ...
    def action_done(self):
        ...
        empty_waiting_pickings = self.mapped("picking_ids").filtered(
            lambda p: p.state == "waiting" and has_no_qty_done(p)
        )
        ...
        if len(empty_pickings) == len(pickings):
            ...
            context["pickings_to_detach"] = context["pickings_to_detach"] + list(
                empty_pickings
            )
            return pickings.with_context(
                skip_immediate=True, **context
            ).button_validate()
            ...
            if not batch.picking_ids <= batch.allowed_picking_ids:
                ...
                raise UserError(
                    _(
                        ...
                        "Incompatibilities: %s",
                        batch.name,
                        ", ".join(erroneous_pickings.mapped("name")),
                    )
                )
