...
class WebsiteForm(form.WebsiteForm):
    ...
    def _handle_website_form(self, model_name, **kwargs):
        ...
        if (
            model_record
            and hasattr(request.env[model_name], "_phone_format")
            or hasattr(request.env[model_name], "phone_get_sanitized_number")
        ):
    ...
    def insert_record(self, request, model, values, custom, meta=None):
        ...
        if is_lead_model:
            ...
            if (
                ...
                if values_phone and visitor_partner.phone:
                    if (
                        visitor_partner._phone_format(visitor_partner.phone)
                        == values_phone
                    ):
        ...
        result = super(WebsiteForm, self).insert_record(
