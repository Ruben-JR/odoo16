...
class TestFiscalPosition(common.TransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.alberto = cls.res_partner.create(
            dict(name="Alberto", vat="BE0477472701", country_id=mx.id)
        )
    ...
    def test_10_fp_country(self):
        ...
        self.eu_intra_b2b = self.fp.create(
            dict(
                name="EU-INTRA B2B",
                auto_apply=True,
        ...
        self.world = self.fp.create(
            dict(name="WORLD-EXTRA", auto_apply=True, vat_required=False, sequence=30)
        )
        ...
        assert_fp(
            george, self.fr_b2b, "FR-B2B should have precedence over EU-INTRA B2B"
        )
        ...
        # Zip range
        self.fr_b2b_zip100 = self.fr_b2b.copy(
            dict(zip_from=0, zip_to=5000, sequence=60)
        )
        george.zip = 6000
        assert_fp(george, self.fr_b2b, "FR-B2B with wrong zip range should not match")
        george.zip = 3000
        assert_fp(
            george, self.fr_b2b_zip100, "FR-B2B with zip range should have precedence"
        )

        # States
        self.fr_b2b_state = self.fr_b2b.copy(
            dict(state_ids=[(4, self.state_fr.id)], sequence=70)
        )
        george.state_id = self.state_fr
        assert_fp(
            george,
            self.fr_b2b_zip100,
            "FR-B2B with zip should have precedence over states",
        )
        george.zip = False
        assert_fp(
            george, self.fr_b2b_state, "FR-B2B with states should have precedence"
        )
