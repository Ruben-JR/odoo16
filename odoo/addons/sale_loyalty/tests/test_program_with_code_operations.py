...
class TestProgramWithCodeOperations(TestSaleCouponCommon):
    ...
    def test_on_next_order_reward_promotion_program(self):
        ...
        self.assertEqual(
            len(order_bis.order_line), 2, "You should get a free Product B"
        )
        # 10.
        self._apply_promo_code(order_bis, order._get_reward_coupons()[0].code)
        self.assertEqual(
            len(order_bis.order_line), 3, "You should get a 10% discount line"
        )
        self.assertAlmostEqual(
            order_bis.amount_total,
            order_bis.order_line[0].price_total * 0.9,
            2,
    ...
    def test_edit_and_reapply_promotion_program(self):
        ...
        self.assertEqual(
            len(order.order_line), 2, "You should get a discount line"
        )  # product + discount
        # 3.
        self.p1.write(
            {
