    ...
    def _compute_possible_product_template_attribute_value_ids(self):
        for bom in self:
            bom.possible_product_template_attribute_value_ids = (
                bom.product_tmpl_id.valid_product_template_attribute_line_ids._without_no_variant_attributes().product_template_value_ids._only_active()
            )
        ...
        for bom in self:
            for bom_line in bom.bom_line_ids:
                if bom.product_id:
                    ...
                    same_product = (
                        bom.product_tmpl_id == bom_line.product_id.product_tmpl_id
                    )
                if same_product:
...
class MrpBomLine(models.Model):
    ...
    product_id = fields.Many2one(
        "product.product", "Component", required=True, check_company=True
    )
    product_tmpl_id = fields.Many2one(
        "product.template",
        "Product Template",
    ...
    def _compute_manual_consumption_readonly(self):
        for line in self:
            line.manual_consumption_readonly = (
                line.tracking != "none" or line.operation_id
            )
    ...
    def _skip_bom_line(self, product):
        ...
        if product._name == "product.template":
            return False
        return not product._match_all_variant_values(
