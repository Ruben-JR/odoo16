...
class TestQWebTField(TransactionCase):
    ...
    def test_reject_crummy_tags(self):
        ...
        with self.assertRaisesRegex(
            QWebException, r"QWeb widgets do not work correctly"
        ):
    ...
    def test_reject_t_tag(self):
        ...
        with self.assertRaisesRegex(
            QWebException, r"t-field can not be used on a t element"
        ):
            self.engine._render(field, {"company": None})
    ...
    def test_xss_breakout(self):
        ...
        rendered = self.env["ir.qweb"]._render(
            view.id, {"malicious": '1</script><script>alert("pwned")</script><script>'}
        )
...
class TestQWebNS(TransactionCase):
    ...
    def test_render_static_xml_with_useless_distributed_namespace(self):
        ...
        self.assertEqual(
            etree.fromstring(self.env["ir.qweb"]._render(view1.id)), expected_result
        )
    ...
    def test_render_static_xml_with_namespace_dynamic(self):
        ...
        values = dict(
            h1="http://www.example.org/table", h2="http://www.w3.org/TD/html4/"
        )
        ...
        self.assertEqual(
            etree.fromstring(rendering), etree.fromstring(expected_result % values)
        )
    ...
    def test_render_static_xml_with_t_call(self):
        ...
        self.env.cr.execute(
            ...
            "VALUES ('dummy', 'ir.ui.view', %s, 'base')",
            [view1.id],
        )
...
class TestQWebBasic(TransactionCase):
    ...
    def test_render_comments(self):
        ...
        comment = "<!-- Hello, world! -->"
        view = self.env["ir.ui.view"].create(
            {"name": "dummy", "type": "qweb", "arch": f"<t><p>{comment}</p></t>"}
        )
    ...
    def test_render_processing_instructions(self):
        ...
        view = self.env["ir.ui.view"].create(
            {"name": "dummy", "type": "qweb", "arch": f"<t><p>{p_instruction}</p></t>"}
        )
        QWeb = self.env["ir.qweb"]
        self.assertEqual(
            QWeb.with_context(preserve_comments=False)._render(view.id),
...
class TestQwebCache(TransactionCase):
    ...
    def test_render_xml_nocache_use_the_root_values_and_cached_values_error(self):
        ...
        with self.assertRaisesRegex(
            QWebException, "The value type of 't-nocache-record' cannot be cached"
        ):
            self.env["ir.qweb"].with_context(is_t_cache_disabled=False)._render(
                template_page.id,
                {
                    "cache_id": 1,
    ...
    def test_render_xml_cache_with_t_set_in_cache(self):
        ...
        """
        self.assertEqual(
            etree.fromstring(render), etree.fromstring(result), "rendering 1"
        )
        ...
        """
        self.assertEqual(
            etree.fromstring(render), etree.fromstring(result), "rendering 2"
        )
        ...
        """
        self.assertEqual(
            etree.fromstring(render), etree.fromstring(result), "rendering 3"
        )
    ...
    def test_render_xml_cache_with_t_set_wrap_t_cache(self):
        ...
        """
        self.assertEqual(
            etree.fromstring(render), etree.fromstring(result), "rendering 4"
        )
    ...
    def test_render_xml_conditional_cache(self):
        ...
        result = etree.fromstring(
            IrQweb._render(
                view1.id, {"cache_id": 1, "condition": True, "value": [1, 2, 3]}
            )
        )
        self.assertEqual(result, expected_result, "First rendering (add in cache)")
        ...
        result = etree.fromstring(
            IrQweb._render(
                view1.id, {"cache_id": 1, "condition": True, "value": [10, 20, 30]}
            )
        )
        self.assertEqual(result, expected_result, "Next rendering use cache")
    ...
    def test_render_xml_cache_and_inherit_view(self):
        ...
        self.assertEqual(
            result,
            expected_result,
            "First rendering create cache from company and the value 1",
        )
        ...
        self.assertEqual(
            result,
            expected_result,
            "Next rendering create cache from company and the value 10",
        )
...
class TestQWebStaticXml(TransactionCase):
    ...
    def test_render_nodb(self):
        ...
        self.assertEqual(
            html.document_fromstring(rendering), html.document_fromstring(expected)
        )
    ...
    def qweb_test_file_path(cls):
        return os.path.dirname(
            get_module_resource("web", "static", "lib", "qweb", "qweb2.js")
        )
    ...
    def run_test_file(self, path):
        ...
        for template in loader:
            ...
            params = (
                {}
                if param is None
                else json.loads(param.text, object_pairs_hook=collections.OrderedDict)
            )
            ...
            result = remove_space(
                doc.find('result[@id="{}"]'.format(template)).text or ""
            ).replace("&quot;", "&#34;")

            try:
                rendering_static = remove_space(
                    render(template, values=params, load=loader)
                )
                self.assertEqual(
                    rendering_static, result, "%s (static rendering)" % template
                )
            except QWebException as e:
                if not isinstance(
                    e.__cause__, NotImplementedError
                ) and "Please use \"env['ir.qweb']._render\" method" in str(e):
                    raise
