...
mail_channel_new_test_user = partial(
    mail_new_test_user, context={"mail_channel_nosubscribe": False}
)
...
class TestMailChannelMembers(MailCommon):
    ...
    def setUpClass(cls):
        ...
        cls.user_public = mail_channel_new_test_user(
            cls.env, login="user_ublic", name="User Public", groups="base.group_public"
        )
    ...
    def test_group_01(self):
        ...
        res = self.env["mail.channel.member"].search(
        ...
        channel_member = (
            ...
            .search([("partner_id", "=", self.user_2.partner_id.id)])[0]
        )
        with self.assertRaises(AccessError):
        ...
        channel_member_1 = self.env["mail.channel.member"].search(
            [
                ("channel_id", "=", self.group.id),
    ...
    def test_group_members(self):
        ...
        channel_member_3 = self.env["mail.channel.member"].search(
            [
                ("channel_id", "=", self.group.id),
                ("partner_id", "=", self.user_portal.partner_id.id),
            ]
    ...
    def test_group_invite(self):
        ...
        with self.assertRaises(AccessError):
            self.group.with_user(self.user_2).add_members(
                self.user_portal.partner_id.ids
            )
        channel_members = self.env["mail.channel.member"].search(
            [("channel_id", "=", self.group.id)]
        )
    ...
    def test_group_leave(self):
        ...
        self.group.with_user(self.user_1).sudo().add_members(self.user_1.partner_id.ids)
        self.group.with_user(self.user_portal).sudo().add_members(
            self.user_portal.partner_id.ids
        )
        channel_members = self.env["mail.channel.member"].search(
            [("channel_id", "=", self.group.id)]
    ...
    def test_group_restricted_channel(self):
        ...
        self.group_restricted_channel.with_user(self.user_1).add_members(
            self.user_1.partner_id.ids
        )
        channel_members = self.env["mail.channel.member"].search(
            [("channel_id", "=", self.group_restricted_channel.id)]
        )
        ...
        with self.assertRaises(AccessError):
            self.group_restricted_channel.with_user(self.user_portal).add_members(
                self.user_portal.partner_id.ids
            )
    ...
    def test_public_channel(self):
        ...
        self.public_channel.with_user(self.user_1).add_members(
            self.user_1.partner_id.ids
        )
        channel_members = self.env["mail.channel.member"].search(
            [("channel_id", "=", self.public_channel.id)]
        ...
        self.public_channel.with_user(self.user_2).add_members(
            self.user_2.partner_id.ids
        )
        channel_members = self.env["mail.channel.member"].search(
            [("channel_id", "=", self.public_channel.id)]
        ...
        with self.assertRaises(AccessError):
            self.public_channel.with_user(self.user_portal).add_members(
                self.user_portal.partner_id.ids
            )
        with self.assertRaises(AccessError):
            self.public_channel.with_user(self.user_public).add_members(
                self.user_public.partner_id.ids
            )
    ...
    def test_channel_member_invite_with_guest(self):
        ...
        search = self.env["res.partner"].search_for_channel_invite(
    ...
    def test_unread_counter_with_message_post(self):
        channel_as_user_1 = self.env["mail.channel"].browse(
            self.env["mail.channel"]
            .with_user(self.user_1)
            .channel_create(group_id=None, name="Public channel")["id"]
        )
    ...
    def test_unread_counter_with_message_post_multi_channel(self):
        channel_1_as_user_1 = (
            ...
            .browse(
                self.env["mail.channel"]
                .with_user(self.user_1)
                .channel_create(group_id=None, name="wololo channel")["id"]
            )
        )
        channel_2_as_user_2 = (
            self.env["mail.channel"]
            ...
            .browse(
                self.env["mail.channel"]
                .with_user(self.user_2)
                .channel_create(group_id=None, name="walala channel")["id"]
            )
        )
