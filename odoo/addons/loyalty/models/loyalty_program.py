...
class LoyaltyProgram(models.Model):
    ...
    total_order_count = fields.Integer(
        "Total Order Count", compute="_compute_total_order_count"
    )
    ...
    trigger = fields.Selection(
        [("auto", "Automatic"), ("with_code", "Use a code")],
        compute="_compute_from_program_type",
        readonly=False,
        store=True,
        help="""
    ...
    def _compute_coupon_count(self):
        ...
        count_per_program = {
            r["program_id"][0]: r["program_id_count"] for r in read_group_data
        }
        for program in self:
    ...
    def _compute_is_payment_program(self):
        for program in self:
            program.is_payment_program = program.program_type in (
                "gift_card",
                "ewallet",
            )
    ...
    def action_open_loyalty_cards(self):
        ...
        action["context"] = {
            ...
            # For the wizard
            "default_mode": self.program_type == "ewallet"
            and "selected"
            or "anonymous",
        }
        return action
    ...
    def get_program_templates(self):
        ...
        if ctx_menu_type == "gift_ewallet":
            return {
                "gift_card": {
                    "title": _("Gift Card"),
                    "description": _(
                        "Sell Gift Cards, that can be used to purchase products"
                    ),
                    "icon": "gift_card",
                },
                "ewallet": {
                    "title": _("eWallet"),
                    "description": _(
                        "Fill in your eWallet, and use it to pay future orders"
                    ),
                    "icon": "ewallet",
                },
            }
        return {
            "promo_code": {
                "title": _("Promo Code"),
                "description": _(
                    "Get a code to receive 10% discount on specific products"
                ),
                "icon": "promo_code",
            },
            "loyalty": {
                ...
                "description": _(
                    "Win points with each purchases, and use points to get gifts"
                ),
                "icon": "loyalty_cards",
            },
            "fidelity": {
                ...
                "description": _(
                    "Buy 10 products, and get 10$ discount on the 11th one"
                ),
                "icon": "fidelity_cards",
            },
            "promotion": {
                ...
                "description": _(
                    "Automatic promotion: 10% discount on orders higher than $50"
                ),
            ...
            "next_order_coupons": {
                ...
                "description": _(
                    "Send unique, single-use coupon code for the next purchase"
                ),
                "icon": "coupons",
            },
        }
