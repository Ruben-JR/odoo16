...
def create_model_table(cr, tablename, comment=None, columns=()):
    ...
    cr.execute(
        'CREATE TABLE "{}" ({}, PRIMARY KEY(id))'.format(tablename, ", ".join(colspecs))
    )
...
def create_column(cr, tablename, columnname, columntype, comment=None):
    ...
    cr.execute(
        'ALTER TABLE "{}" ADD COLUMN "{}" {} {}'.format(
            tablename, columnname, columntype, coldefault
        )
    )
    if comment:
        cr.execute(
    ...
    _schema.debug(
        "Table %r: added column %r of type %s", tablename, columnname, columntype
    )
...
def _convert_column(cr, tablename, columnname, columntype, using):
    ...
    try:
        ...
        cr.execute(query)
    _schema.debug(
        "Table %r: column %r changed to type %s", tablename, columnname, columntype
    )
...
def drop_depending_views(cr, table, column):
    ...
    for v, k in get_depending_views(cr, table, column):
        cr.execute(
            "DROP {0} VIEW IF EXISTS {1} CASCADE".format(
                "MATERIALIZED" if k == "m" else "", v
            )
        )
...
def set_not_null(cr, tablename, columnname):
    ...
    query = 'ALTER TABLE "{}" ALTER COLUMN "{}" SET NOT NULL'.format(
    ...
    try:
        ...
        raise Exception(
            "Table %r: unable to set NOT NULL on column %r", tablename, columnname
        )
...
def drop_not_null(cr, tablename, columnname):
    ...
    cr.execute(
        'ALTER TABLE "{}" ALTER COLUMN "{}" DROP NOT NULL'.format(tablename, columnname)
    )
    _schema.debug(
        "Table %r: column %r: dropped constraint NOT NULL", tablename, columnname
    )
...
def add_constraint(cr, tablename, constraintname, definition):
    ...
    try:
        ...
        raise Exception(
            "Table %r: unable to add constraint %r as %s",
            tablename,
            constraintname,
            definition,
        )
...
def drop_constraint(cr, tablename, constraintname):
    ...
    try:
        ...
        _schema.warning(
            "Table %r: unable to drop constraint %r!", tablename, constraintname
        )
...
def fix_foreign_key(cr, tablename1, columnname1, tablename2, columnname2, ondelete):
    ...
    if not found:
        return add_foreign_key(
            cr, tablename1, columnname1, tablename2, columnname2, ondelete
        )
...
def create_index(cr, indexname, tablename, expressions, method="btree", where=""):
    ...
    if where:
        where = f" WHERE {where}"
    cr.execute(
        f'CREATE INDEX "{indexname}" ON "{tablename}" USING {method} ({args}){where}'
    )
...
def increment_fields_skiplock(records, *fields):
    ...
    """
        ...
        sets=SQL(", ").join(map(SQL("{0} = {0} + 1").format, map(Identifier, fields))),
    )
...
def pattern_to_translated_trigram_pattern(pattern):
    ...
    json_escaped = [
        json.dumps(t, ensure_ascii=False)[1:-1] for t in sub_texts if len(t) >= 3
    ]
