...
class TestTracking(TestMailCommon):
    ...
    def test_create_partner_from_tracking_multicompany(self):
        ...
        with patch(
            "odoo.addons.mail.models.mail_thread.MailThread._message_track_post_template",
    ...
    def test_track_template(self):
        ...
        self.registry("mail.test.container")._patch_method(
        ...
        self.registry("mail.test.container")._patch_method(
            "_track_template", _track_template
        )
...
class TestTrackingInternals(TestMailCommon):
    ...
    def test_track_groups(self):
        ...
        self.assertEqual(
            msg_emp[0].get("trackingValues"),
            [],
            "should not have protected tracking values",
        )
        self.assertEqual(
            msg_sudo[0].get("trackingValues"),
            formattedTrackingValues,
            "should have protected tracking values",
        )

        msg_emp = self.record._notify_by_email_prepare_rendering_context(
            self.record.message_ids, {}
        )
        msg_sudo = self.record.sudo()._notify_by_email_prepare_rendering_context(
            self.record.message_ids, {}
        )
        self.assertFalse(
            msg_emp.get("tracking_values"), "should not have protected tracking values"
        )
        self.assertTrue(
            msg_sudo.get("tracking_values"), "should have protected tracking values"
        )
