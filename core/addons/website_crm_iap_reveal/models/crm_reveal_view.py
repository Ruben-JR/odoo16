...
class CRMRevealView(models.Model):
    ...
    def init(self):
        self._cr.execute(
            ...
            ("crm_reveal_view_ip_rule_id",),
        )
        if not self._cr.fetchone():
            self._cr.execute(
                "CREATE UNIQUE INDEX crm_reveal_view_ip_rule_id ON crm_reveal_view (reveal_rule_id,reveal_ip)"
            )
        self._cr.execute(
            "SELECT indexname FROM pg_indexes WHERE indexname = %s",
            ("crm_reveal_view_state_create_date",),
        )
        if not self._cr.fetchone():
            self._cr.execute(
                "CREATE INDEX crm_reveal_view_state_create_date ON crm_reveal_view (reveal_state,create_date)"
            )
