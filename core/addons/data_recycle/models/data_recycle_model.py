...
class DataRecycleModel(models.Model):
    ...
    def _compute_records_to_recycle_count(self):
        ...
        counts = {
            cd["recycle_model_id"][0]: cd["recycle_model_id_count"] for cd in count_data
        }
        for model in self:
            model.records_to_recycle_count = (
                counts[model.id] if model.id in counts else 0
            )
    ...
    def _recycle_records(self, batch_commits=False):
        ...
        for records_to_clean_batch in split_every(
            DR_CREATE_STEP_MANUAL, records_to_clean
        ):
