...
class Contract(models.Model):
    ...
    wage = fields.Monetary(
        "Wage", required=True, tracking=True, help="Employee's monthly gross wage."
    )
    ...
    def _get_hr_responsible_domain(self):
        return (
            "[('share', '=', False), ('company_ids', 'in', company_id), ('groups_id', 'in', %s)]"
    ...
    def write(self, vals):
        ...
        for contract in self:
            if (
                contract == contract.employee_id.contract_id
        ...
        if (
            self.env.context.get("close_contract", True)
            and date_end
            and fields.Date.from_string(date_end) < fields.Date.context_today(self)
        ):
    ...
    def _track_subtype(self, init_values):
        ...
        if (
            ...
            and self.state == "open"
            and "kanban_state" in init_values
            and self.kanban_state == "blocked"
        ):
