...
class AccountAnalyticLine(models.Model):
    ...
    def default_get(self, field_list):
        ...
        if not self._context.get("default_project_id") and self._context.get(
            "is_timesheet"
        ):
    ...
    def name_get(self):
        ...
        timesheets_read = self.env[self._name].search_read(
            [("project_id", "!=", False), ("id", "in", self.ids)],
        ...
        timesheet_dict = {
            res["id"]: _get_display_name(res["project_id"], res["task_id"])
            for res in timesheets_read
        }
    ...
    def write(self, values):
        ...
        if not (
            ...
            self.env.user.id != analytic_line.user_id.id for analytic_line in self
        ):
        ...
        if values.get("employee_id"):
            ...
            if not employee.active:
                raise UserError(
                    _("You cannot set an archived employee to the existing timesheets.")
                )
    ...
    def _apply_timesheet_label(self, view_node, view_type="form"):
        ...
        for node in doc.xpath(
            ...
            node.set(
                "string",
                _("%s Spent") % (re.sub(r"[\(\)]", "", encoding_uom.name or "")),
            )
        return doc
    ...
    def _apply_time_label(self, view_node, related_model):
        ...
        encoding_uom_name = (
            self.env.company.timesheet_encode_uom_id.with_context(prefetch_fields=False)
            .sudo()
            .name
        )
        for node in doc.xpath(
            "//field[@widget='timesheet_uom'][not(@string)] | //field[@widget='timesheet_uom_no_toggle'][not(@string)]"
        ):
            name_with_uom = re.sub(
                _("Hours") + "|Hours",
                encoding_uom_name or "",
                Model._fields[node.get("name")]._description_string(self.env),
                flags=re.IGNORECASE,
            )
    ...
    def _timesheet_get_portal_domain(self):
        ...
        return [
            ...
            "|",
            (
                "task_id.project_id.message_partner_ids",
                "child_of",
                [self.env.user.partner_id.commercial_partner_id.id],
            ),
            (
                "task_id.message_partner_ids",
                "child_of",
                [self.env.user.partner_id.commercial_partner_id.id],
            ),
    ...
    def _timesheet_preprocess(self, vals):
        ...
        if task and not project:
            ...
            if not project:
                raise ValidationError(
                    _("You cannot create a timesheet on a private task.")
                )
        ...
        if task and not vals.get("account_id"):
            ...
            vals["company_id"] = (
                task_analytic_account_id.company_id.id or task.company_id.id
            )
            if not task_analytic_account_id.active:
                raise UserError(
                    _(
                        "You cannot add timesheets to a project or a task linked to an inactive analytic account."
                    )
                )
        # project implies analytic account
            ...
            vals["company_id"] = (
                project.analytic_account_id.company_id.id or project.company_id.id
            )
            if not project.analytic_account_id.active:
                raise UserError(
                    _(
                        "You cannot add timesheets to a project linked to an inactive analytic account."
                    )
                )
        ...
        if not vals.get("product_uom_id") and all(
            v in vals for v in ["account_id", "project_id"]
        ):  # project_id required to check this is timesheet flow
            analytic_account = (
                self.env["account.analytic.account"].sudo().browse(vals["account_id"])
            )
            ...
            if not uom_id:
                ...
                if not company_id:
                    ...
                    company_id = (
                        project.analytic_account_id.company_id.id
                        or project.company_id.id
                    )
                uom_id = (
                    self.env["res.company"].browse(company_id).project_time_mode_id.id
                )
    ...
    def _timesheet_postprocess(self, values):
        ...
        sudo_self = (
            self.sudo()
        )  # this creates only one env for all operation that required sudo() in `_timesheet_postprocess_values`override
