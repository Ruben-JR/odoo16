...
class TestEmailParsing(TestMailCommon):
    def test_message_parse_and_replace_binary_octetstream(self):
        ...
        with self.assertLogs(
            ...
            extracted_mail = self.env["mail.thread"]._message_parse_extract_payload(
                received_mail
            )
    ...
    def test_message_parse_body(self):
        ...
        res = self.env["mail.thread"].message_parse(
    ...
    def test_message_parse_bugs(self):
        ...
        self.env["mail.thread"].message_parse(
    ...
    def test_message_parse_extract_bounce_rfc822_headers_qp(self):
        ...
        msg_dict = {}
        msg = self.env["mail.thread"]._message_parse_extract_bounce(
            self.from_string(incoming_bounce), msg_dict
        )
        self.assertEqual(
            msg["bounced_email"],
            partner.email,
    ...
    def test_alias_name_unique(self):
        ...
        with self.assertRaises(exceptions.UserError), self.cr.savepoint():
            self.env["ir.config_parameter"].sudo().set_param(
                "mail.catchall.alias", new_mail_alias.alias_name
            )
        with self.assertRaises(exceptions.UserError), self.cr.savepoint():
            self.env["ir.config_parameter"].sudo().set_param(
                "mail.bounce.alias", new_mail_alias.alias_name
            )
...
class TestMailgateway(TestMailCommon):
    ...
    def test_message_process_alias_basic(self):
        ...
        record = self.format_and_process(
            MAIL_TEMPLATE, self.email_from, "groups@test.com", subject="Specific"
        )
    ...
    def test_message_process_cid(self):
        record = self.format_and_process(
            test_mail_data.MAIL_MULTIPART_IMAGE, self.email_from, "groups@test.com"
        )
    ...
    def test_message_process_followers(self):
        ...
        self.assertEqual(
            record.message_follower_ids.partner_id,
            self.partner_1,
        ...
        with self.mock_mail_gateway():
            record5 = self.format_and_process(
                MAIL_TEMPLATE,
                partner_private.email_formatted,
                "groups@test.com",
        ...
        self.assertEqual(
            record5.message_ids[0].email_from, partner_private.email_formatted
        )
        self.assertEqual(
            record5.message_follower_ids.partner_id,
            partner_private,
            "message_process: private partner is recognized",
        )
        self.assertEqual(
            record5.message_partner_ids,
            partner_private,
            "message_process: private partner is recognized",
        )
    ...
    def test_message_process_email_author(self):
        ...
        record = self.format_and_process(
            MAIL_TEMPLATE, self.partner_1.email, "groups@test.com", subject="Test2"
        )
    ...
    def test_message_process_email_partner_find(self):
        ...
        self.format_and_process(
            MAIL_TEMPLATE, from_1.email_formatted, "groups@test.com"
        )
    ...
    def test_message_process_alias_config_bounced_content(self):
        ...
        self.assertSentEmail(
            '"MAILER-DAEMON" <bounce.test@test.com>',
        ...
        for empty_content in [
            ...
            self.assertSentEmail(
                '"MAILER-DAEMON" <bounce.test@test.com>',
                ["whatever-2a840@postmaster.twitter.com"],
                body_content="<p>Dear Sender,<br /><br />\nThe message below could not be accepted by the address %s"
                % self.alias.display_name.lower(),
            )
    ...
    def test_message_process_alias_config_bounced_to(self):
        ...
        with self.mock_mail_gateway():
            self.format_and_process(
                MAIL_TEMPLATE,
                self.email_from,
                "other@gmail.com, groups@example.com",
                subject="Should Bounce",
            )
        self.assertIn(bounce_message_with_alias, self._mails[0].get("body"))
    ...
    def test_message_process_alias_defaults(self):
        ...
        record = self.format_and_process(
            MAIL_TEMPLATE, self.email_from, "groups@test.com", subject="Specific2"
        )
        self.assertEqual(len(record), 1)
        res = record.get_metadata()[0].get("create_uid") or [None]
        self.assertEqual(res[0], self.user_employee.id)
    ...
    def test_message_process_create_uid_email(self):
        ...
        record = self.format_and_process(
            MAIL_TEMPLATE,
            "Another name <%s>" % self.user_employee.email,
            "groups@test.com",
    ...
    def test_message_route_write_to_catchall(self):
        ...
        with self.mock_mail_gateway():
            record = self.format_and_process(
                MAIL_TEMPLATE,
                self.partner_1.email_formatted,
                '"My Super Catchall" <%s@%s>'
                % (self.alias_catchall, self.alias_domain),
                subject="Should Bounce",
            )
        self.assertFalse(record)
        self.assertSentEmail(
    ...
    def test_message_process_bounce_records_channel(self):
        ...
        for i in range(10):
            record = self.format_and_process(
                test_mail_data.MAIL_BOUNCE,
                "A third name <%s>" % self.partner_1.email,
                "groups@test.com",
                subject="Undelivered Mail Returned to Sender",
                extra=extra,
            )
    ...
    def test_message_process_references_multi_parent(self):
        ...
        self.assertEqual(
            new_msg.parent_id,
            self.fake_email,
            "Mail: flattening attach to original message",
        )
        self.assertEqual(
            new_msg.subtype_id,
        ...
        self.assertEqual(
            new_msg.subtype_id,
            self.env.ref("mail.mt_comment"),
            "Mail: parent should be a comment (before flattening)",
        )
    ...
    def test_message_process_references_multi_parent_notflat(self):
        ...
        self.assertEqual(
            new_msg.parent_id,
            reply1,
            "Mail: pseudo no flattening: getting up one level (reply1_1 parent)",
        )
        self.assertEqual(
            new_msg.subtype_id,
            self.env.ref("mail.mt_note"),
        ...
        self.assertEqual(
            new_msg.parent_id,
            reply2,
            "Mail: pseudo no flattening: getting up one level (reply2_1 parent",
        )
        self.assertEqual(
            new_msg.subtype_id,
            self.env.ref("mail.mt_comment"),
    ...
    def test_message_process_reply_to_new_thread(self):
        ...
        self.assertEqual(
            ...
            formataddr(
                (
                    "%s %s" % (self.user_employee.company_id.name, first_record.name),
                    "%s@%s" % ("catchall.test", "test.com"),
                )
            ),
        )
        ...
        res_test = self.format_and_process(
            ...
            extra="In-Reply-To: %s" % record_msg.message_id,
            msg_id=msgID,
    ...
    def test_message_process_extra_model_res_id(self):
        ...
        self.assertRaises(
            ...
            "noone@test.com",
            subject="spam",
            extra="In-Reply-To: <12321321-openerp-%d-mail.test.gateway@%s>"
    ...
    @mute_logger(
        "odoo.addons.mail.models.mail_thread", "odoo.addons.mail.models.mail_mail"
    )
    ...
    def test_routing_loop_alias(self):
        ...
        records = self.env["mail.test.gateway"].search(
        ...
        for email_from in (self.email_from, self.email_from.upper()):
            ...
            new_record = self.env["mail.test.gateway"].search(
    ...
    def test_message_process_file_encoding_ascii(self):
        ...
        self.assertEqual(
            record.message_main_attachment_id.name,
        ...
        self.assertIn(
            "Chauss������e de Bruxelles", record.message_main_attachment_id.raw.decode()
        )
