...
class UtmCampaign(models.Model):
        ...
        for campaign in self:
            campaign.ab_testing_total_pc += sum(
                [
                    ...
                    for mailing in campaign.mailing_sms_ids.filtered(
                        "ab_testing_enabled"
                    )
                ]
            )
    ...
    def _compute_mailing_sms_count(self):
        if self.ids:
            ...
            for data in mailing_sms_data:
                if data["ab_testing_enabled"]:
                    ab_testing_mapped_sms_data.setdefault(
                        data["campaign_id"][0], []
                    ).append(data["__count"])
                mapped_sms_data.setdefault(data["campaign_id"][0], []).append(
                    data["__count"]
                )
        else:
        ...
        for campaign in self:
            campaign.mailing_sms_count = sum(
                mapped_sms_data.get(campaign._origin.id or campaign.id, [])
            )
            campaign.ab_testing_mailings_sms_count = sum(
                ab_testing_mapped_sms_data.get(campaign._origin.id or campaign.id, [])
            )
