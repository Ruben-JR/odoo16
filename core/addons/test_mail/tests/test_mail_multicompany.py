...
class TestMultiCompanySetup(TestMailCommon, TestRecipients):
        ...
        cls.fake_email = cls.env["mail.message"].create(
            {
                ...
                "message_id": "<123456-openerp-%s-mail.test.gateway@%s>"
    ...
    def test_notify_reply_to_computation_mc(self):
        ...
        test_records = self.env["mail.test.multi.company"].create(
            [
                {"name": "Test", "company_id": self.user_employee.company_id.id},
                {"name": "Test", "company_id": self.user_employee_c2.company_id.id},
            ]
        )
        ...
        test_records = self.env["mail.test.multi.company"].create(
            [
                {"name": "Test1", "company_id": self.company_3.id},
                {"name": "Test2", "company_id": self.company_3.id},
            ]
        )
        res = test_records._notify_get_reply_to()
        for test_record in test_records:
    ...
    def test_post_wo_access(self):
        ...
        with patch.object(
            Message, "check_access_rule", autospec=True, side_effect=_original_car
        ) as mock_msg_car:
            with self.assertRaises(AccessError):
        ...
        attachments = self.env["ir.attachment"].create(
            self._generate_attachments_data(
                2, "mail.compose.message", 0, prefix="Other"
            )
        )
