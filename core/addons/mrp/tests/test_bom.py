...
class TestBoM(TestMrpCommon):
    def test_01_explode(self):
        ...
        self.assertEqual(
            ...
            set(
                ...
                .mapped("bom_line_ids")
                .filtered(
                    lambda line: not line.child_bom_id
                    or line.child_bom_id.type != "phantom"
                )
                .ids
            ),
        )
    ...
    def test_13_bom_kit_qty(self):
        ...
        self.env["stock.quant"]._update_available_quantity(
            self.product_2, location, 4.0
        )
        self.env["stock.quant"]._update_available_quantity(
            self.product_3, location, 8.0
        )
    ...
    def test_20_bom_report(self):
        ...
        # Operation cost is the sum of operation line.
        operation_cost = float_round(5 / 60 * 10, precision_digits=2) * 2 + float_round(
        ...
        self.assertEqual(
            float_compare(report_values["lines"]["bom_cost"], 6.17, precision_digits=2),
            0,
            "Product Unit Bom Price is not correct",
        )
        ...
        self.assertEqual(
            float_compare(
                report_values["lines"]["operations_cost"], 208.33, precision_digits=2
            ),
            0,
        )
        ...
        self.assertEqual(
            float_compare(
                report_values["lines"]["bom_cost"], 258.47, precision_digits=2
            ),
            0,
            "Product Bom Price is not correct",
        )
