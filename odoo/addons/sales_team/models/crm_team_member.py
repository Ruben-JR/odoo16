...
class CrmTeamMember(models.Model):
    ...
    crm_team_id = fields.Many2one(
        "crm.team",
        string="Sales Team",
        group_expand="_read_group_crm_team_id",
    ...
    def _constrains_membership(self):
        ...
        for membership in self:
            potential = existing.filtered(
                ...
                and m.crm_team_id == membership.crm_team_id
    ...
    def _compute_user_in_teams_ids(self):
        ...
        if all(m.is_membership_multi for m in self):
            ...
            member_user_ids = (
                self.env["crm.team.member"].search([("id", "not in", self.ids)]).user_id
            )
        else:
        ...
        for member in self:
            if member_user_ids:
                ...
                member.user_in_teams_ids = (
                    self.env["crm.team"]
                    .browse(self.env.context["default_crm_team_id"])
                    .member_ids
                )
            else:
            ...
            for member in active:
                ...
                if remaining:
                    member.member_warning = _(
                        "Adding %(user_name)s in this team would remove him/her from its current teams %(team_names)s.",
            ...
            self._synchronize_memberships(
                [
                    dict(
                        ...
                        crm_team_id=membership.crm_team_id.id,
                    )
                    for membership in self
                ]
            )
    ...
    def _synchronize_memberships(self, user_team_ids):
        ...
        user_memberships = dict.fromkeys(
            existing.user_id.ids, self.env["crm.team.member"]
        )
        for membership in existing:
        ...
        for values in user_team_ids:
            existing_to_archive += user_memberships.get(
                values["user_id"], self.env["crm.team.member"]
            ).filtered(lambda m: m.crm_team_id.id != values["crm_team_id"])
