...
class TestSaleOrder(SaleCommon):
    ...
    def test_onchange_packaging_00(self):
        ...
        so2_form.save()
        self.assertEqual(
            so2.order_line.product_packaging_id.id, packaging_pack_of_10.id
        )
        ...
        self.assertEqual(
            so2.order_line.product_packaging_id.id, packaging_pack_of_20.id
        )
    ...
    def test_validity_days(self):
        ...
        with freeze_time("2020-05-02"):
            ...
            self.assertEqual(
                sale_order.validity_date, fields.Date.today() + timedelta(days=5)
            )
    ...
    def test_state_changes(self):
        ...
        self.assertIn(
            self.sale_order.partner_id, self.sale_order.message_follower_ids.partner_id
        )
        ...
        self.assertEqual(
            ...
            "done",
            "The order wasn't automatically locked at confirmation.",
        )
        with self.assertRaises(UserError):
            self.sale_order.action_confirm()
