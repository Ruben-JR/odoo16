...
class MrpProduction(models.Model):
    ...
    def write(self, vals):
        origin_analytic_account = {
            production: production.analytic_account_id for production in self
        }
        res = super().write(vals)
        for production in self:
            if vals.get("name"):
                production.move_raw_ids.analytic_account_line_id.ref = (
                    production.display_name
                )
                for workorder in production.workorder_ids:
                    workorder.mo_analytic_account_line_id.ref = production.display_name
                    workorder.mo_analytic_account_line_id.name = _(
            ...
            if "analytic_account_id" in vals and production.state != "draft":
                if vals["analytic_account_id"] and origin_analytic_account[production]:
                    ...
                    production.move_raw_ids.analytic_account_line_id.write(
                        {"account_id": vals["analytic_account_id"]}
                    )
                elif (
                    vals["analytic_account_id"]
                    and not origin_analytic_account[production]
                ):
    ...
    def action_view_stock_valuation_layers(self):
        ...
        domain = [
            (
                ...
                "in",
                (
                    self.move_raw_ids + self.move_finished_ids + self.scrap_ids.move_id
