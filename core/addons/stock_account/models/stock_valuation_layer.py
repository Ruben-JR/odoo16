...
class StockValuationLayer(models.Model):
    ...
    stock_valuation_layer_id = fields.Many2one(
        "stock.valuation.layer", "Linked To", readonly=True, check_company=True
    )
    stock_valuation_layer_ids = fields.One2many(
        "stock.valuation.layer", "stock_valuation_layer_id"
    )
    stock_move_id = fields.Many2one(
        "stock.move", "Stock Move", readonly=True, check_company=True, index=True
    )
    ...
    def _consume_specific_qty(self, qty_valued, qty_to_value):
        ...
        for candidate in self:
            ...
            returned_qty = sum(
                [
                    sm.product_uom._compute_quantity(sm.quantity_done, self.uom_id)
            ...
            tmp_value += qty_taken_on_candidate * (
                (
                    candidate.value
                    + sum(candidate.stock_valuation_layer_ids.mapped("value"))
                )
                / candidate.quantity
            )
    ...
    def _consume_all(self, qty_valued, valued, qty_to_value):
        ...
        for svl in self:
            ...
            value_total += relevant_qty * (
                (svl.value + sum(svl.stock_valuation_layer_ids.mapped("value")))
                / svl.quantity
            )
