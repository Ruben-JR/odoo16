...
class Warehouse(models.Model):
    ...
    def _populate_factories(self):
        return super()._populate_factories() + [
            (
                "manufacture_steps",
                populate.iterate(["mrp_one_step", "pbm", "pbm_sam"], [0.6, 0.2, 0.2]),
            )
        ]
...
class MrpBom(models.Model):
    ...
    def _populate_factories(self):
        ...
        product_tmpl_ids = random.sample(
            product_tmpl_ids, int(len(product_tmpl_ids) * 0.8)
        )
    ...
    def _populate_factories(self):
        ...
        def get_product_id(values, counter, random):
            ...
            last_product_bom = (
                bom.product_id
                if bom.product_id
                else bom.product_tmpl_id.product_variant_ids[-1]
            )
            ...
            threshold = len_remaining_manu / (
                len_remaining_manu + sparsity * len_no_manu
            )
        ...
        for workcenter in res:
            ...
            if nb_alternative > 0:
                alternatives_workcenter_ids = random.sample(
                    (workcenters_by_company[workcenter.company_id.id] - workcenter).ids,
                    nb_alternative,
                )
...
class MrpProduction(models.Model):
    ...
    def _populate_factories(self):
        ...
        product_ids = products.filtered(
            lambda product: product.type in ("product", "consu")
        ).ids
        ...
        manu_picking_types_by_company_id = {
            ...
            for company_id, picking_ids in manu_picking_types_by_company_id.items()
        }
...
class StockMove(models.Model):
    ...
    def _populate_attach_record_generator(self):
        ...
        return {
            **super()._populate_attach_record_generator(),
            "raw_material_production_id": next_production_id(),
        }
