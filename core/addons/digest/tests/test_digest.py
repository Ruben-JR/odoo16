...
class TestDigest(mail_test.MailCommon):
    ...
    def test_digest_numbers(self):
        ...
        kpi_message_values = html.fromstring(mail.body_html).xpath(
            '//div[@data-field="kpi_mail_message_total"]//*[hasclass("kpi_value")]/text()'
        )
    ...
    def test_digest_tone_down_wlogs(self):
        ...
        logs = (
            ...
            .create({"create_uid": self.user_employee.id})
        )
        digest.flush_recordset()
        with self.mock_mail_gateway():
            digest.action_send()
...
class TestUnsubscribe(HttpCaseWithUserDemo):
    ...
    def test_unsubscribe_token(self):
        ...
        response = self._url_unsubscribe(
            token=self.user_demo_unsubscribe_token, user_id=self.user_demo.id
        )
        self.assertEqual(response.status_code, 200)
    ...
    def _url_unsubscribe(self, token=None, user_id=None, one_click=None, method="GET"):
        ...
        url = url_join(
            self.base_url,
            f"digest/{self.test_digest.id}/unsubscribe?{url_encode(url_params)}",
        )
        if method == "GET":
