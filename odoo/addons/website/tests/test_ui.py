...
class TestUiTranslate(odoo.tests.HttpCase):
    ...
    def test_snippet_translation(self):
        ...
        self.env.ref("website.s_cover").update_field_translations(
            "arch_db", {parseltongue.code: {"Contact us": "Contact us in Parseltongue"}}
        )
        self.env.ref("web_editor.snippets").update_field_translations(
            "arch_db",
            {
                fake_user_lang.code: {
                    "Save": "Save in fu_GB",
...
class TestUi(odoo.tests.HttpCase):
    ...
    def test_06_public_user_editor(self):
        ...
        self.env["website.page"].search(
            [("url", "=", "/"), ("website_id", "=", website_default.id)], limit=1
        ).arch = """
    ...
    def test_15_website_link_tools(self):
        self.start_tour(
            self.env["website"].get_client_action_url("/"), "link_tools", login="admin"
        )
    ...
    def test_18_website_snippets_menu_tabs(self):
        self.start_tour(
            "/?enable_editor=1", "website_snippets_menu_tabs", login="admin"
        )
    ...
    def test_20_snippet_editor_panel_options(self):
        self.start_tour(
            "/@/?enable_editor=1", "snippet_editor_panel_options", login="admin"
        )
