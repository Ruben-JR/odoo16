...
class MrpWorkorder(models.Model):
    ...
    def _create_or_update_analytic_entry(self):
        ...
        for wo in self.filtered(
            lambda wo: wo.production_id.analytic_account_id
            or wo.workcenter_id.costs_hour_account_id
        ):
            ...
            if mo_account:
                is_zero = float_is_zero(
                    value, precision_rounding=mo_account.currency_id.rounding
                )
                if wo.mo_analytic_account_line_id:
                    ...
                    mo_analytic_line_vals_list.append(
                        wo._prepare_analytic_line_values(mo_account, hours, value)
                    )
            if wc_account and wc_account != mo_account:
                is_zero = float_is_zero(
                    value, precision_rounding=wc_account.currency_id.rounding
                )
                if wo.wc_analytic_account_line_id:
                    ...
                    wc_analytic_line_vals_list.append(
                        wo._prepare_analytic_line_values(wc_account, hours, value)
                    )
        analytic_lines = (
            self.env["account.analytic.line"]
            .sudo()
            .create(mo_analytic_line_vals_list + wc_analytic_line_vals_list)
        )
        mo_analytic_lines, wc_analytic_lines = (
            analytic_lines[: len(wo_to_link_mo_analytic_line)],
            analytic_lines[len(wo_to_link_mo_analytic_line) :],
        )
