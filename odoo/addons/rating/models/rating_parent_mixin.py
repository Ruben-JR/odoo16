...
class RatingParentMixin(models.AbstractModel):
    ...
    def _compute_rating_percentage_satisfaction(self):
        ...
        data = self.env["rating.rating"].read_group(
            domain, ["parent_res_id", "rating"], ["parent_res_id", "rating"], lazy=False
        )

        # get repartition of grades per parent id
        default_grades = {"great": 0, "okay": 0, "bad": 0}
        grades_per_parent = dict(
            (parent_id, dict(default_grades)) for parent_id in self.ids
        )  # map: {parent_id: {'great': 0, 'bad': 0, 'ok': 0}}
    ...
    def _search_rating_avg(self, operator, value):
        ...
        if self._rating_satisfaction_days:
            ...
            domain = expression.AND(
                [domain, [("write_date", ">=", fields.Datetime.to_string(min_date))]]
            )
        rating_read_group = (
            self.env["rating.rating"]
            .sudo()
            .read_group(
                domain, ["parent_res_id", "rating_avg:avg(rating)"], ["parent_res_id"]
            )
        )
        parent_res_ids = [
            res["parent_res_id"]
