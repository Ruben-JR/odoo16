...
def remove_domain_leaf(domain, fields_to_remove):
    ...
    def _process_leaf(elements, index, operator, new_domain):
        ...
        if len(leaf) == 3:
            ...
            if (
                leaf == OR_OPERATOR
                and len(elements[index + 1]) == 3
                and len(elements[index + 2]) == 3
                and elements[index + 1][0] in fields_to_remove
                and elements[index + 2][0] in fields_to_remove
            ):
                new_domain.append(TRUE_LEAF)
                return 3
            new_domain.append(leaf)
            ...
            second_leaf_skip = _process_leaf(
                elements, index + 1 + first_leaf_skip, leaf, new_domain
            )
...
class ReportProjectTaskBurndownChart(models.AbstractModel):
    ...
    def _get_group_by_SQL(
        ...
        (
            project_task_from_clause,
            project_task_where_clause,
            project_task_where_clause_params,
        ) = project_task_query.get_sql()
        ...
        field_id = IrModelFieldsSudo.search(
            [("name", "=", "stage_id"), ("model", "=", "project.task")]
        ).id
        ...
        date_annotated_groupby = [
            annotated_groupby
            for annotated_groupby in annotated_groupbys
            if annotated_groupby["groupby"] == date_group_by_field
        ][0]
        date_begin, date_end = (
            date_annotated_groupby["qualified_field"].replace(
                '"%s"."%s"' % (self._table, date_annotated_groupby["field"]),
                '"%s_%s"' % (date_annotated_groupby["field"], field),
            )
        ...
        from_clause = from_clause.replace(
            '"project_task_burndown_chart_report"',
            '(%s) AS "project_task_burndown_chart_report"' % burndown_chart_query,
            1,
        )
    ...
    def _determine_domains(self, domain):
        ...
        * A domain that only contains fields that are specific to `project.task`
        ...
        task_specific_domain = remove_domain_leaf(
            domain, burndown_chart_specific_fields
        )
    ...
    def _read_group_raw(
        ...
        for fspec in fields:
            ...
            if func:
                ...
                if not field:
                    raise ValueError(
                        _("Invalid field %r on model %r", (fname, self._name))
                    )
                ...
                if not field:
                    raise ValueError(
                        _("Invalid field %r on model %r", (name, self._name))
                    )
                if not (
        ...
        self.env["project.task"]._flush_search(
            task_specific_domain, fields=self.task_specific_fields
        )
