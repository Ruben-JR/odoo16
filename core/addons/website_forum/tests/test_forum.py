...
class TestForum(TestForumCommon):
    ...
    def test_forum_mode_questions(self):
        ...
        self.assertEqual(
            not questions_post.uid_has_answered
            or questions_post.forum_id.mode == "discussions",
            False,
        )
        self.assertEqual(
            questions_post.uid_has_answered
            and questions_post.forum_id.mode == "questions",
            True,
        )
    ...
    def test_forum_mode_discussions(self):
        ...
        forum_discussions = Forum.create(
            {"name": "Discussions Forum", "mode": "discussions", "active": True}
        )
        Post = self.env["forum.post"]
        ...
        self.assertEqual(
            not discussions_post.uid_has_answered
            or discussions_post.forum_id.mode == "discussions",
            True,
        )
        self.assertEqual(
            discussions_post.uid_has_answered
            and discussions_post.forum_id.mode == "questions",
            False,
        )
