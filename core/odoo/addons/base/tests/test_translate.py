...
class TranslationToolsTestCase(BaseCase):
    ...
    def test_translate_xml_inline4(self):
        ...
        self.assertEqual(result, source)
        self.assertItemsEqual(
            terms, ["Form stuff", '<span class="fa fa-globe" title="Title stuff"/>']
        )
...
class TestLanguageInstall(TransactionCase):
    def test_language_install(self):
        ...
        with patch(
            "odoo.addons.base.models.ir_module.Module._load_module_terms",
            _load_module_terms,
        ):
            wizard.lang_install()
...
class TestTranslation(TransactionCase):
    ...
    def test_102_duplicate_record(self):
        ...
        self.assertEqual(
            category_no.name, "Customers", "Duplication should copy all translations"
        )
    ...
    def test_108_search_en(self):
        ...
        self.assertIn(
            self.customers, category_in, "Search with 'in' doesn't work for English"
        )
    ...
    def test_109_search_fr(self):
        ...
        self.assertIn(
            self.customers, category_in, "Search with 'in' doesn't work for non English"
        )
...
class TestTranslationWrite(TransactionCase):
    ...
    def test_field_selection(self):
        ...
        self.assertEqual(
            fg["state"]["selection"], [("manual", "Custo"), ("base", "Pas touche!")]
        )
...
class TestXMLTranslation(TransactionCase):
    ...
    def test_update_field_translations(self):
        ...
        view.update_field_translations(
            "arch_db",
            {"en_US": {"Fork": "Fork2"}, "fr_FR": {"Fourchette": "Fourchette2"}},
        )

        self.assertEqual(
            view.arch_db, '<form string="X">Bread and cheese<div>Fork2</div></form>'
        )
        self.assertEqual(
            view.with_context(lang="en_US").arch_db,
...
class TestTranslationTrigramIndexPatterns(BaseCase):
    def test_value_conversion(self):
        ...
        for value, expected, message in cases:
            self.assertEqual(
                sql.value_to_translated_trigram_pattern(value), expected, message
            )
