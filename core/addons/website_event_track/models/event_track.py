...
class Track(models.Model):
    ...
    partner_id = fields.Many2one(
        "res.partner",
        "Contact",
        help="Contact of the track, may be different from speaker.",
    )
    # speaker information
    ...
    date_end = fields.Datetime(
        "Track End Date", compute="_compute_end_date", store=True
    )
    ...
    def _compute_wishlist_visitor_ids(self):
        ...
        visitor_ids_map = {
            result["track_id"][0]: result["visitor_id"] for result in results
        }
        for track in self:
    ...
    def _search_wishlist_visitor_ids(self, operator, operand):
        ...
        track_visitors = (
            self.env["event.track.visitor"]
            .sudo()
            .search([("visitor_id", operator, operand), ("is_wishlisted", "=", True)])
        )
        ...
        for track in self:
            if not track.date:
                track.is_track_live = (
                    ...
                    track.is_track_today
                ) = track.is_track_upcoming = track.is_track_done = False
    ...
    def _compute_cta_time_data(self):
        ...
        for track in self:
            ...
            date_begin_utc = utc.localize(track.date, is_dst=False) + timedelta(
            ...
            if track.partner_id:
                if track.partner_id not in recipients:
                    track._message_add_suggested_recipient(
                        recipients, partner=track.partner_id, reason=_("Contact")
                    )
            else:
                #  Priority: contact information then speaker information
                if (
                    track.contact_email
                    and track.contact_email != track.partner_id.email
                ):
                    track._message_add_suggested_recipient(
                        recipients, email=track.contact_email, reason=_("Contact Email")
                    )
                if (
                    not track.contact_email
                    and track.partner_email
                    and track.partner_email != track.partner_id.email
                ):
                    track._message_add_suggested_recipient(
                        recipients, email=track.partner_email, reason=_("Speaker Email")
                    )
        return recipients
        ...
        if msg_vals.get("partner_ids") and not self.partner_id:
            ...
            if main_email:
                new_partner = message.partner_ids.filtered(
                    lambda partner: partner.email == main_email
                )
                if new_partner:
                    main_email_string = (
    ...
    def _get_event_track_visitors(self, force_create=False):
        ...
        track_visitors = (
            self.env["event.track.visitor"]
            .sudo()
            .search(expression.AND([domain, [("track_id", "in", self.ids)]]))
        )
    ...
    def _get_track_suggestions(self, restrict_domain=None, limit=None):
        ...
        if restrict_domain:
            base_domain = expression.AND([base_domain, restrict_domain])
