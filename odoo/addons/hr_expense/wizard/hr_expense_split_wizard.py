...
class HrExpenseSplitWizard(models.TransientModel):
    ...
    total_amount = fields.Monetary(
        "Total Amount", compute="_compute_total_amount", currency_field="currency_id"
    )
    total_amount_original = fields.Monetary(
    ...
    total_amount_taxes = fields.Monetary(
        "Taxes", currency_field="currency_id", compute="_compute_total_amount_taxes"
    )
    ...
    def _compute_total_amount(self):
        for wizard in self:
            wizard.total_amount = sum(
                wizard.expense_split_line_ids.mapped("total_amount")
            )

    @api.depends("expense_split_line_ids.amount_tax")
    def _compute_total_amount_taxes(self):
        for wizard in self:
            wizard.total_amount_taxes = sum(
                wizard.expense_split_line_ids.mapped("amount_tax")
            )

    @api.depends("total_amount_original", "total_amount")
    def _compute_split_possible(self):
        for wizard in self:
            wizard.split_possible = wizard.total_amount_original and (
                float_compare(
                    wizard.total_amount_original,
                    wizard.total_amount,
                    precision_digits=2,
                )
                == 0
            ...
            if self.expense_split_line_ids:
                ...
                for coplied_expense in copied_expenses:
                    for attachment in attachment_ids:
                        attachment.copy(
                            {"res_model": "hr.expense", "res_id": coplied_expense.id}
                        )
