...
class TestLeadConvertMass(crm_common.TestLeadConvertMassCommon):
    ...
    def test_assignment_salesmen(self):
        ...
        with self.assertQueryCount(
            user_sales_manager=543
        ):  # crm 537 / com 543 / ent 537
    ...
    def test_mass_convert_internals(self):
        ...
        for lead in self.lead_1 | self.lead_w_partner:
            ...
            if lead == self.lead_w_partner:
                self.assertEqual(
                    lead.user_id, self.env["res.users"]
                )  # user_id is bypassed
                self.assertEqual(lead.partner_id, self.contact_1)
            elif lead == self.lead_1:
                self.assertEqual(
                    lead.user_id, self.user_sales_leads
                )  # existing value not forced
        ...
        self.assertEqual(
            self.lead_1.user_id, self.user_sales_leads
        )  # existing value not forced
    ...
    def test_mass_convert_deduplicate(self):
        ...
        lead_w_partner_final = lead_w_partner_dups[
            0
        ]  # lead_w_partner has no stage -> lower in sort by confidence
        lead_w_partner_dups_partner = lead_w_partner_dups[
            1
        ]  # copy with a partner_id (with the same email)
