...
class GoogleGmailMixin(models.AbstractModel):
    ...
    google_gmail_authorization_code = fields.Char(
        string="Authorization Code", groups="base.group_system", copy=False
    )
    ...
    def _get_gmail_csrf_token(self):
        ...
        self.ensure_one()
        _logger.info(
            "Google Gmail: generate CSRF token for %s #%i", self._name, self.id
        )
