...
class BaseCommon(TransactionCase):
    ...
    def _enable_currency(cls, currency_code):
        currency = (
            ...
            .search([("name", "=", currency_code.upper())])
        )
...
class MockSmtplibCase:
    ...
    def mock_smtplib_connection(self):
        ...
        with patch(
            ...
            type(IrMailServer), "_is_test_mode", lambda self: False
        ), patch.object(
            type(IrMailServer),
            "connect",
            wraps=IrMailServer,
            side_effect=connect_origin,
        ) as connect_mocked, patch.object(
            type(IrMailServer), "_find_mail_server", side_effect=find_mail_server_origin
        ) as find_mail_server_mocked:
            self.connect_mocked = connect_mocked
            self.find_mail_server_mocked = find_mail_server_mocked
            yield

    def assert_email_sent_smtp(
        self,
        smtp_from=None,
        smtp_to_list=None,
        message_from=None,
        ...
        self.assertTrue(
            ...
            msg="Emails not sent, %i emails match the condition but %i are expected"
            % (matching_emails_count, emails_count),
        )
    ...
    def _init_mail_config(cls):
        ...
        cls.env["ir.config_parameter"].sudo().set_param(
            "mail.default.from", cls.default_from
        )
        cls.env["ir.config_parameter"].sudo().set_param(
            "mail.bounce.alias", cls.alias_bounce
        )
