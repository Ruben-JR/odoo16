...
class Blog(models.Model):
    ...
    def message_post(self, *, parent_id=False, subtype_id=False, **kwargs):
        ...
        return super(Blog, self).message_post(
...
class BlogPost(models.Model):
    ...
    def _set_post_date(self):
        for blog_post in self:
            ...
            if not blog_post.published_date:
                blog_post._write(
                    dict(post_date=blog_post.create_date)
        ...
        for post in self:
            ...
            if (
                ...
                copy_vals["published_date"] = (
                    vals[list(published_in_vals)[0]] and fields.Datetime.now() or False
                )
    ...
    def _get_access_action(self, access_uid=None, force_website=False):
        ...
        if not force_website and user.share and not self.sudo().website_published:
            return super(BlogPost, self)._get_access_action(
    ...
    def _notify_thread_by_inbox(
        ...
        if msg_vals.get("message_type", message.message_type) == "comment":
            return
        return super(BlogPost, self)._notify_thread_by_inbox(
