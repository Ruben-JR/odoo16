...
class Employee(models.Model):
    ...
    def _get_first_contract_date(self, no_gap=True):
        ...
        def remove_gap(contracts):
            ...
            for i, other_contract in enumerate(older_contracts):
                ...
                gap = (
                    current_date - (other_contract.date_end or date(2100, 1, 1))
                ).days
    ...
    def _compute_contract_warning(self):
        for employee in self:
            employee.contract_warning = (
                ...
                or employee.contract_id.state != "open"
            )

    def _compute_contracts_count(self):
        # read_group as sudo, since contract count is displayed on form view
        contract_histories = (
            self.env["hr.contract.history"]
            .sudo()
            .search([("employee_id", "in", self.ids)])
        )
        for employee in self:
            contract_history = contract_histories.filtered(
                lambda ch: ch.employee_id == employee
            )
    ...
    def _get_incoming_contracts(self, date_from, date_to):
        return self._get_contracts(
            date_from, date_to, states=["draft"], kanban_state=["done"]
        )
    ...
    def _get_all_contracts(self, date_from, date_to, states=["open"]):
        ...
        )._get_contracts(date_from, date_to, states=states)

    def write(self, vals):
        res = super(Employee, self).write(vals)
        if vals.get("contract_id"):
            for employee in self:
                employee.resource_calendar_id.transfer_leaves_to(
                    employee.contract_id.resource_calendar_id, employee.resource_id
                )
