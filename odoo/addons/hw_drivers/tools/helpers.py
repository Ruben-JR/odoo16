    ...
    if server:
        ...
        if path.exists():
            with path.open("r") as f:
                ...
                cert_end_date = datetime.datetime.strptime(
                ...
                if (
                    cn == "OdooTempIoTBoxCertificate"
                    or datetime.datetime.now() > cert_end_date
                ):
                    _logger.info(_("Your certificate %s must be updated") % (cn))
                    load_certificate()
                else:
                    _logger.info(
                        _("Your certificate %s is valid until %s") % (cn, cert_end_date)
                    )
        else:
            load_certificate()
...
def check_image():
    ...
    if valueActual == valueLastest:
        return False
    version = (
        checkFile.get(valueLastest, "Error")
        .replace("iotboxv", "")
        .replace(".zip", "")
        .split("_")
    )
...
def get_ssid():
    ...
    )  # if service is active return 0 else inactive
    if not ap:
        return (
            subprocess.check_output(
                ["grep", "-oP", "(?<=ssid=).*", "/etc/hostapd/hostapd.conf"]
            )
            .decode("utf-8")
            .rstrip()
        )
    process_iwconfig = subprocess.Popen(
        ["iwconfig"], stdout=subprocess.PIPE, stderr=subprocess.STDOUT
    )
    process_grep = subprocess.Popen(
        ["grep", 'ESSID:"'], stdin=process_iwconfig.stdout, stdout=subprocess.PIPE
    )
    return (
        subprocess.check_output(
            ["sed", 's/.*"\\(.*\\)"/\\1/'], stdin=process_grep.stdout
        )
        .decode("utf-8")
        .rstrip()
    )
...
def get_wifi_essid():
    wifi_options = []
    process_iwlist = subprocess.Popen(
        ["sudo", "iwlist", "wlan0", "scan"],
        stdout=subprocess.PIPE,
        stderr=subprocess.STDOUT,
    )
    process_grep = subprocess.Popen(
        ["grep", 'ESSID:"'], stdin=process_iwlist.stdout, stdout=subprocess.PIPE
    ).stdout.readlines()
...
def load_certificate():
    ...
    if db_uuid and enterprise_code:
        ...
        data = {"params": {"db_uuid": db_uuid, "enterprise_code": enterprise_code}}
        urllib3.disable_warnings()
        http = urllib3.PoolManager(cert_reqs="CERT_NONE")
        response = http.request(
            "POST",
        ...
        if result:
            ...
            if platform.system() == "Linux":
                with writable():
                    ...
                    ).write_text(result["x509_pem"])
                    Path("/etc/ssl/private/nginx-cert.key").write_text(
                        result["private_key_pem"]
                    )
                    Path(
                        "/root_bypass_ramdisks/etc/ssl/private/nginx-cert.key"
                    ).write_text(result["private_key_pem"])
            elif platform.system() == "Windows":
                Path(get_path_nginx()).joinpath("conf/nginx-cert.crt").write_text(
                    result["x509_pem"]
                )
                Path(get_path_nginx()).joinpath("conf/nginx-cert.key").write_text(
                    result["private_key_pem"]
                )
            time.sleep(3)
            if platform.system() == "Windows":
    ...
    if server:
        ...
        try:
            resp = pm.request(
                "POST",
                server,
                fields={"mac": get_mac_address(), "auto": auto},
                timeout=8,
            )
            if resp.data:
                with writable():
