...
class ReplenishmentReport(models.AbstractModel):
    ...
    def _serialize_docs(
        ...
        res["draft_purchase_orders"] = docs["draft_purchase_orders"].read(
            fields=["id", "name"]
        )
        return res
    ...
    def _compute_draft_quantity_count(
        ...
        res["draft_purchase_orders"] = po_lines.mapped("order_id").sorted(
            key=lambda po: po.name
        )
        res["draft_purchase_orders_matched"] = (
            self.env.context.get("purchase_line_to_match_id") in po_lines.ids
        )
        res["qty"]["in"] += in_sum
        return res
