...
class IrAttachment(models.Model):
    ...
    def _migrate(self):
        ...
        for index, attach in enumerate(self):
            _logger.debug(
                "Migrate attachment %s/%s to %s", index + 1, record_count, storage
            )
            ...
            if str2bool(max_resolution, True):
                try:
                    ...
                    if is_raw:
                        ...
                        img = ImageProcess(
                            base64.b64decode(values["datas"]), verify_resolution=False
                        )
    ...
    def _check_contents(self, values):
        ...
        xml_like = "ht" in mimetype or (  # hta, html, xhtml, etc.
            "xml" in mimetype
            and not "openxmlformats" in mimetype  # other xml (svg, text/xml, etc)
    ...
    res_id = fields.Many2oneReference(
        "Resource ID", model_field="res_model", readonly=True
    )
    ...
    raw = fields.Binary(
        string="File Content (raw)", compute="_compute_raw", inverse="_inverse_raw"
    )
    datas = fields.Binary(
    ...
    def _auto_init(self):
        ...
        tools.create_index(
            self._cr, "ir_attachment_res_idx", self._table, ["res_model", "res_id"]
        )
        return res
    ...
    def read_group(
        ...
        if any("(" in field for field in fields + groupby):
            raise AccessError(
                _(
                    "Sorry, the syntax 'name:agg(field)' is not available for attachments"
                )
            )
    ...
    def create(self, vals_list):
            ...
            if raw or datas:
                ...
                values.update(
                    self._get_datas_related_values(
                        raw or base64.b64decode(datas or b""), values["mimetype"]
                    )
                )
        ...
        for res_model, res_id in record_tuple_set:
            Attachments.check(
                "create", values={"res_model": res_model, "res_id": res_id}
            )
