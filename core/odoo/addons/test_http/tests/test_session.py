...
class TestHttpSession(TestHttpBase):
    ...
    def test_session2_geoip(self):
        ...
        with patch.object(
            odoo.http.root.geoip_resolver, "resolve"
    ...
    def test_session3_logout_15_0_geoip(self):
        ...
        self.assertEqual(
            urlparse(res.headers["Location"]).path, "/web/database/selector"
        )
    ...
    def test_session7_serializable(self):
        ...
        for value in [
            ...
            with self.assertLogs(level="WARNING"):
                ...
                self.assertFalse(
                    odoo.http.root.session_store.session_class({"foo": value}, 1234).foo
                )
