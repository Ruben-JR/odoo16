...
class CalendarController(http.Controller):
    ...
    def accept_meeting(self, token, id, **kwargs):
        attendee = (
            ...
            .sudo()
            .search([("access_token", "=", token), ("state", "!=", "accepted")])
        )
        attendee.do_accept()
    ...
    def decline_meeting(self, token, id, **kwargs):
        attendee = (
            request.env["calendar.attendee"]
            .sudo()
            .search([("access_token", "=", token), ("state", "!=", "declined")])
        )
        attendee.do_decline()
        return self.view_meeting(token, id)
    ...
    def view_meeting(self, token, id, **kwargs):
        ...
        company = (
            event.user_id and event.user_id.company_id or event.create_uid.company_id
        )
        ...
        if request.session.uid and request.env["res.users"].browse(
            request.session.uid
        ).user_has_groups("base.group_user"):
    ...
    def calendar_join_meeting(self, token, **kwargs):
        event = (
            request.env["calendar.event"].sudo().search([("access_token", "=", token)])
        )
        if not event:
            return request.not_found()
