...
class ReportSaleDetails(models.AbstractModel):
    ...
    def get_sale_details(
                ...
                if payment["session"] == session.id:
                    if not payment["cash"]:
                        for account_payment in account_payments:
                            if (
                                payment["id"]
                                == account_payment.pos_payment_method_id.id
                            ):
            ...
            category_dictionnary = {
                ...
                "products": sorted(
                    [
                        ...
                        for (product, price_unit, discount), qty in product_list.items()
                    ],
        ...
        refund_products, refund_info = self._get_total_and_qty_per_category(
            refund_products
        )
    ...
    class PosSession(models.Model):
        ...
        def update_closing_control_state_session(self, notes):
            ...
            self._post_cash_details_message(
                "Closing", self.cash_register_difference, notes
            )
        ...
        def get_total_sold_refund_per_category(self, group_by_user_id=None):
            ...
            if group_by_user_id or not total_sold_per_user_per_category:
                return list(total_sold_per_user_per_category.items()), list(
                    total_refund_per_user_per_category.items()
                )
            else:
                return list(total_sold_per_user_per_category[0].items()), list(
                    total_refund_per_user_per_category[0].items()
                )
