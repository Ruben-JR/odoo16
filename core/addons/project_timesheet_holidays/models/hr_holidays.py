...
class HolidaysType(models.Model):
    ...
    def _compute_timesheet_task_id(self):
        for leave_type in self:
            company = (
                leave_type.company_id if leave_type.company_id else self.env.company
            )
            default_task_id = company.leave_timesheet_task_id

            if (
                default_task_id
                and default_task_id.project_id == leave_type.timesheet_project_id
            ):
    ...
    def _check_timesheet_generate(self):
        for holiday_status in self:
            if holiday_status.timesheet_generate:
                if (
                    not holiday_status.timesheet_project_id
                    or not holiday_status.timesheet_task_id
                ):
                    raise ValidationError(
                        _(
                            "Both the internal project and task are required to "
...
class Holidays(models.Model):
    ...
    def _validate_leave_request(self):
        ...
        holidays = self.filtered(
            lambda l: l.holiday_type == "employee"
            and l.holiday_status_id.timesheet_project_id
            and l.holiday_status_id.timesheet_task_id
            and l.holiday_status_id.timesheet_project_id.sudo().company_id
            == (l.holiday_status_id.company_id or self.env.company)
        )
    ...
    def _timesheet_prepare_line_values(
        ...
        return {
            ...
            "company_id": self.holiday_status_id.timesheet_task_id.company_id.id
            or self.holiday_status_id.timesheet_project_id.company_id.id,
        }
