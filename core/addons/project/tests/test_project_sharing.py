...
class TestProjectSharing(TestProjectSharingCommon):
    def test_project_share_wizard(self):
        ...
        self.assertFalse(
            project_share_wizard.partner_ids, "No collaborator should be in the wizard."
        )
        self.assertFalse(
    ...
    def test_edit_task_in_project_sharing(self):
        ...
        self.assertFalse(
            task.child_ids.user_ids, "No user should be assigned to the new subtask."
        )
    ...
    def test_portal_user_cannot_see_all_assignees(self):
        ...
        self.assertEqual(
            len(self.task_cow.user_ids), 2, "2 users should be assigned in this task."
        )
    ...
    def test_portal_user_can_change_stage_with_rating(self):
        ...
        self.task_portal.with_user(self.user_portal).write(
            {"stage_id": self.project_portal.type_ids[-1].id}
        )
    ...
    def test_orm_method_with_true_false_domain(self):
        ...
        self.assertEqual(
            task,
            task.filtered_domain(expression.TRUE_DOMAIN),
            "The task found should be kept since the domain is truly",
        )
        self.assertFalse(
            task.filtered_domain(expression.FALSE_DOMAIN),
            "The task should not be found since the domain is falsy",
        )
        task = (
            self.env["project.task"]
            .with_user(self.user_portal)
        ...
        self.assertFalse(
            task, "No task should be found since the domain contained a falsy tuple."
        )
        ...
        self.assertEqual(
            ...
            1,
            "The task should be found with the read_group method containing a truly tuple.",
        )
        self.assertEqual(
        ...
        self.assertFalse(
            task_read_group[0]["__count"],
            "No result should found with the read_group since the domain is falsy.",
        )
