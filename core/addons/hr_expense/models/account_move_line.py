...
class AccountMoveLine(models.Model):
    ...
    def _check_payable_receivable(self):
        super(
            AccountMoveLine,
            self.filtered(
                lambda line: not line.expense_id
                or line.expense_id.payment_mode != "company_account"
            ),
        )._check_payable_receivable()

    def reconcile(self):
        # OVERRIDE
        not_paid_expenses = self.expense_id.filtered(
            lambda expense: expense.state != "done"
        )
        res = super().reconcile()
        ...
        not_paid_expense_sheets = not_paid_expenses.sheet_id.filtered(
            lambda sheet: sheet.account_move_id.payment_state != "reversed"
        )
        ...
        not_paid_expense_sheets.filtered(
            lambda sheet: all(
    ...
    def _compute_term_key(self):
        ...
        for line in self:
            if line.expense_id:
                line.term_key = line.term_key and frozendict(
                    **line.term_key, expense_id=line.expense_id.id
                )
