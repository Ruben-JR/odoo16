...
class MrpProduction(models.Model):
    ...
    def name_get(self):
        return [
            (record.id, "%s (%s)" % (record.incoming_picking.name, record.name))
            ...
            for product_id, lines in line_by_product.items():
                qty = sum(
                    line.product_uom_id._compute_quantity(
                        line.qty_done, product_id.uom_id
                    )
                    for line in lines
                )
    ...
    def _update_finished_move(self):
        ...
        self.ensure_one()
        subcontract_move_id = self._get_subcontract_move().filtered(
        ...
        if subcontract_move_id:
            ...
            if self.lot_producing_id:
                move_lines = subcontract_move_id.move_line_ids.filtered(
                    lambda ml: ml.lot_id == self.lot_producing_id or not ml.lot_id
                )
            else:
                move_lines = subcontract_move_id.move_line_ids.filtered(
            ...
            for ml in move_lines:
                ...
                if (
                    ...
                    ml.with_context(bypass_reservation_update=True).write(
                        {"reserved_uom_qty": new_qty_reserved, "qty_done": 0}
                    )

            if (
                float_compare(
                    quantity, 0, precision_rounding=self.product_uom_id.rounding
                )
                > 0
            ):
            ...
            if not self._get_quantity_to_backorder():
                ml_reserved = subcontract_move_id.move_line_ids.filtered(
