...
class TestExpenses(TestExpenseCommon):
    def test_expense_sheet_changing_employee(self):
        ...
        self.assertEqual(
            expense2.sheet_id.id, False, "Sheet should be unlinked from the expense"
        )
    ...
    def test_expense_sheet_payment_state(self):
        ...
        self.assertEqual(
            expense_sheet.payment_state, "partial", "payment_state should be partial"
        )
    ...
    def test_expense_values(self):
                ...
                "expense_line_ids": [
                    (
                        ...
                        0,
                        {
                            ...
                            "unit_amount": 1000.0,
                            "tax_ids": [
                                (6, 0, self.company_data["default_tax_purchase"].ids)
                            ],
        ...
        self.assertRecordValues(
            expense_sheet, [{"state": "draft", "total_amount": 1500.0}]
        )
        ...
        self.assertRecordValues(
            expense_sheet.account_move_id.line_ids.sorted("balance"),
            [
        ...
        self.assertRecordValues(
            expense_sheet.account_move_id.line_ids.analytic_line_ids.sorted("amount"),
            [
                {
                    "amount": -869.57,
    ...
    def test_account_entry_multi_currency(self):
        ...
        self.assertEqual(
            expense.state, "draft", "Expense should be created in Draft state"
        )
    ...
    def test_account_entry_multi_currency_company_account(self):
        ...
        expense_sheet.action_sheet_move_create()
        self.assertRecordValues(
            expense_sheet.account_move_id.payment_id,
            [
                {
    ...
    def test_reconcile_payment(self):
        ...
        self.assertEqual(
            sheet.state,
            "done",
            "Sheet state must not change when the payment linked to that sheet is canceled",
        )
    ...
    def test_expense_amount_total_signed_compute(self):
        ...
        self.assertEqual(
            ...
            10.0,
            "The total amount of the payment move is not correct",
        )
    ...
    def test_expense_account(self):
        ...
        account_expense_2 = self.env["account.account"].create(
            {"code": "610020", "name": "Expense Account 2"}
        )
        ...
        self.assertRecordValues(
            expense_sheet, [{"state": "draft", "total_amount": 345.0}]
        )
    ...
    def test_print_expense_check(self):
        ...
        payment_method_line = (
            self.env.company.bank_journal_ids.outbound_payment_method_line_ids.filtered(
                lambda m: m.code == "check_printing"
            )
        )
        with Form(
            self.env[action_data["res_model"]].with_context(action_data["context"])
        ) as wiz_form:
        ...
        for payment in payments:
            ...
            move = self.env[action_data["context"]["active_model"]].browse(
                action_data["context"]["active_ids"]
            )
            self.assertDictEqual(
                stub_line[0],
                {
                    ...
                    "number": " - ".join(
                        [move.name, move.ref] if move.ref else [move.name]
                    ),
    ...
    def test_hr_expense_split(self):
        ...
        self.assertRecordValues(
            expenses_after_split.sorted("total_amount"),
            [
                {
                    "name": expense.name,
                    "employee_id": expense.employee_id.id,
    ...
    def test_reset_move_to_draft(self):
        ...
        self.assertEqual(
            expense.state,
            "draft",
            "Expense state must be draft before sheet submission",
        )
        self.assertEqual(
            expense_sheet.state, "draft", "Sheet state must be draft before submission"
        )
        ...
        self.assertEqual(
            expense.state,
            "reported",
            "Expense state must be reported after sheet submission",
        )
        self.assertEqual(
            expense_sheet.state, "submit", "Sheet state must be submit after submission"
        )
        ...
        self.assertEqual(
            expense.state,
            "approved",
            "Expense state must be draft after sheet approval",
        )
        self.assertEqual(
            expense_sheet.state, "approve", "Sheet state must be draft after approval"
        )
        ...
        self.assertEqual(
            expense.state, "approved", "Expense state must be draft after posting move"
        )
        self.assertEqual(
            expense_sheet.state, "post", "Sheet state must be draft after posting move"
        )
        ...
        self.assertEqual(
            expense.state, "done", "Expense state must be done after payment"
        )
        self.assertEqual(
            expense_sheet.state, "done", "Sheet state must be done after payment"
        )
        ...
        self.assertEqual(
            expense.state,
            "approved",
            "Expense state must be approved after resetting move to draft",
        )
        self.assertEqual(
            expense_sheet.state,
            "post",
            "Sheet state must be done after resetting move to draft",
        )
