...
class TestRecruitmentSurvey(common.SingleTransactionCase):
    ...
    def test_send_survey(self):
        ...
        self.survey_sysadmin.write(
            {"access_mode": "public", "users_login_required": False}
        )
        ...
        self.assertEqual(
            set(answers.mapped("email")), set([self.job_sysadmin.email_from])
        )
    ...
    def test_print_survey(self):
        ...
        self.job_sysadmin.response_id = self.env["survey.user_input"].create(
            {"survey_id": self.survey_sysadmin.id}
        )
        action_print_with_response = self.job_sysadmin.action_print_survey()
        self.assertIn(
            self.job_sysadmin.response_id.access_token,
