...
class MailActivityMixin(models.AbstractModel):
    ...
    def _read_progress_bar(self, domain, group_by, progress_bar):
        ...
        if not (
            ...
            and self._fields[group_by_fname].store
        ):
        ...
        groupby_terms, _orderby_terms = self._read_group_prepare(
            "activity_state", [], annotated_groupbys, query
        )
        select_terms = [
        ...
        where_params[num_from_params:num_from_params] = [
            tz
        ] * 3  # timezone after from parameters
    ...
    def activity_send_mail(self, template_id):
        ...
        if not template:
            return False
        for record in self:
            record.message_post_with_template(template_id, composition_mode="comment")
        return True
    ...
    def activity_schedule(
        ...
        for record in self:
            ...
            if not create_vals.get("user_id"):
                create_vals["user_id"] = (
                    activity_type.default_user_id.id or self.env.uid
                )
    ...
    def activity_reschedule(
        self, act_type_xmlids, user_id=None, date_deadline=None, new_user_id=None
    ):
    ...
    def activity_feedback(
        self, act_type_xmlids, user_id=None, feedback=None, attachment_ids=None
    ):
