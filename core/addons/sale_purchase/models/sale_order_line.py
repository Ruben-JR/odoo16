...
class SaleOrderLine(models.Model):
    ...
    def _compute_purchase_count(self):
        database_data = (
            ...
            .sudo()
            .read_group(
                [("sale_line_id", "in", self.ids)], ["sale_line_id"], ["sale_line_id"]
            )
        )
        mapped_data = dict(
            [(db["sale_line_id"][0], db["sale_line_id_count"]) for db in database_data]
        )
        for line in self:
    ...
    def _onchange_service_product_uom_qty(self):
        if (
            ...
            and self.product_id.service_to_purchase
        ):
    ...
    def write(self, values):
        ...
        if "product_uom_qty" in values:
            ...
            increased_lines = self.sudo().filtered(
                lambda r: r.product_id.service_to_purchase
    ...
    def _purchase_get_date_order(self, supplierinfo):
        ...
        commitment_date = fields.Datetime.from_string(
            self.order_id.commitment_date or fields.Datetime.now()
        )
    ...
    def _purchase_service_prepare_line_values(self, purchase_order, quantity=False):
        ...
        return {
            ...
            + relativedelta(days=int(supplierinfo.delay)),
