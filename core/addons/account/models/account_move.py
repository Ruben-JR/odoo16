...
class AccountMove(models.Model):
                ...
                if payment_state_matters:
                    if currency.is_zero(invoice.amount_residual):
                        if any(x["has_payment"] for x in reconciliation_vals):
                            ...
                            if all(
                                x["all_payments_matched"] for x in reconciliation_vals
                            ):
                                new_pmt_state = "paid"
                            else:
            ...
            if move.is_invoice(include_receipts=True):
                ...
                if rounding_line:
                    amount_total_rounded = (
                        ...
                        + sign * rounding_line.amount_currency
                    )
    ...
    def onchange(self, values, field_name, field_onchange):
        ...
        with self.env.protecting(
            [self._fields[fname] for fname in field_name or []], self
        ):
    ...
    def _check_total_amount(self, amount_total):
        ...
        if not float_is_zero(
            ...
            if _("Untaxed Amount") in totals["groups_by_subtotal"]:
                totals["groups_by_subtotal"][_("Untaxed Amount")][0][
                    "tax_group_amount"
