...
class ServerActions(models.Model):
    ...
    activity_user_field_name = fields.Char(
        "User field name", compute="_compute_activity_info", readonly=False, store=True
    )

    @api.depends("model_id", "state")
    def _compute_template_id(self):
        to_reset = self.filtered(
            lambda act: act.state != "mail_post"
            or (act.model_id != act.template_id.model_id)
        )
        if to_reset:
    ...
    def _compute_activity_type_id(self):
        to_reset = self.filtered(
            lambda act: act.state != "next_activity"
            or (act.model_id.model != act.activity_type_id.res_model)
        )
        if to_reset:
    ...
    def _check_model_coherency(self):
        for action in self:
            if (
                action.state in ("followers", "next_activity")
                and action.model_id.transient
            ):
                raise ValidationError(
                    _("This action cannot be done on transient models.")
                )
            if action.state == "followers" and not action.model_id.is_mail_thread:
                raise ValidationError(
                    _("Add Followers can only be done on a mail thread models")
                )
    ...
    def _run_action_mail_post_multi(self, eval_context=None):
        ...
        if (
            ...
            or (
                ...
                and not self._context.get("active_id")
            )
