                    ...
                    urls.url_encode(
                        {
                            ...
                            or self.env.ref("utm.utm_medium_website").name,
...
class Applicant(models.Model):
    ...
    def website_form_input_filter(self, request, values):
        if "partner_name" in values:
            applicant_job = (
                self.env["hr.job"].sudo().search([("id", "=", values["job_id"])]).name
                if "job_id" in values
                else False
            )
            name = (
                "%s - %s" % (values["partner_name"], applicant_job)
                if applicant_job
                else _("%s's Application", values["partner_name"])
            )
