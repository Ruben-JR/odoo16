...
class LinkPreview(models.Model):
    ...
    message_id = fields.Many2one(
        "mail.message", string="Message", index=True, ondelete="cascade", required=True
    )
    ...
    def _create_link_previews(self, message):
        ...
        for url in set(urls):
            if len(link_previews) >= 5:
                break
            link_previews |= self.env["mail.link.preview"]._create_link_preview(
                url, message.id, requests_session
            )
        ...
        self.env["bus.bus"]._sendmany(
            [(target, "mail.link.preview/insert", link_previews._link_preview_format())]
        )
    ...
    def _delete_and_notify(self):
        ...
        for link_preview in self:
            if (
                link_preview.message_id.model == "mail.channel"
                and link_preview.message_id.res_id
            ):
            ...
            notifications.append(
                (target, "mail.link.preview/delete", {"id": link_preview.id})
            )
