...
class StockMoveLine(models.Model):
    ...
    def _onchange_lot_id(self):
        if (
            not self.picking_type_use_existing_lots
            or not self.product_id.use_expiration_date
        ):
            return
        if self.lot_id:
    ...
    def _onchange_product_id(self):
        ...
        if self.picking_type_use_create_lots:
            if self.product_id.use_expiration_date:
                self.expiration_date = fields.Datetime.today() + datetime.timedelta(
            ...
            res.update(
                {
                    ...
                    "alert_date": self.expiration_date
