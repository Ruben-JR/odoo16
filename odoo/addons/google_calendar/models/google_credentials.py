...
class GoogleCredentials(models.Model):
    ...
    synchronization_stopped = fields.Boolean(
        "Google Synchronization stopped", copy=False
    )
        ...
        self.write(
            {
                ...
                "calendar_token_validity": fields.Datetime.now()
    ...
    def _is_google_calendar_valid(self):
        self.ensure_one()
        return self.calendar_token_validity and self.calendar_token_validity >= (
    ...
    def _refresh_google_calendar_token(self):
        ...
        if not client_id or not client_secret:
            raise UserError(
                _("The account for the Google Calendar service is not configured.")
            )
