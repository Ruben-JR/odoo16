...
class PurchaseOrder(models.Model):
    ...
    def _compute_dest_address_id(self):
        ...
        for order in dropship_subcontract_pos:
            ...
            order.dest_address_id = (
                subcontractor_ids if len(subcontractor_ids) == 1 else False
            )
    ...
    def _get_destination_location(self):
        ...
        if in_bom_products and not_in_bom_products:
            raise UserError(
                _(
                    "It appears some components in this RFQ are not meant for subcontracting. Please create a separate order for these."
                )
            )
