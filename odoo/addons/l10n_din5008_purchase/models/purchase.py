...
class PurchaseOrder(models.Model):
    ...
    l10n_din5008_document_title = fields.Char(
    ...
    def _compute_l10n_din5008_template_data(self):
        for record in self:
            ...
            if record.date_approve:
                data.append(
                    (_("Order Date"), format_date(self.env, record.date_approve))
                )
            elif record.date_order:
                data.append(
                    (_("Order Deadline"), format_date(self.env, record.date_order))
                )
    ...
    def _compute_l10n_din5008_addresses(self):
        for record in self:
            ...
            if record.dest_address_id:
                ...
                "picking_type_id" in record._fields
                and record.picking_type_id.warehouse_id
            ):
                data.append(
                    (
                        _("Shipping Address:"),
                        record.picking_type_id.warehouse_id.partner_id,
                    )
                )
