    ...
    def write(self, vals):
        ...
        for order in self:
            if order.company_id._is_accounting_unalterable():
                ...
                if order.state in ["paid", "done", "invoiced"] and set(
                    vals
                ).intersection(ORDER_FIELDS):
                    raise UserError(
                        _(
        ...
        if has_been_posted:
            for order in self.filtered(
                lambda o: o.company_id._is_accounting_unalterable()
    ...
    def write(self, vals):
        ...
        if set(vals).intersection(LINE_FIELDS):
            if any(
                ...
                and l.order_id.state in ["done", "invoiced"]
                for l in self
            ):
                raise UserError(
                    _(
