...
class TestTourManualConsumption(HttpCase):
    def test_mrp_manual_consumption(self):
        ...
        url = "/web#model=mrp.production&view_type=form&action=%s&id=%s" % (
            str(action_id.id),
            str(mo.id),
        )
        ...
        move_lot = backorder.move_raw_ids.filtered(
            lambda m: m.product_id == product_lot
        )
        self.assertEqual(move_nt.manual_consumption, False)
        self.assertEqual(move_sn.manual_consumption, True)
        self.assertEqual(move_lot.manual_consumption, True)
...
class TestManualConsumption(TestMrpCommon):
    ...
    def test_manual_consumption_backorder_00(self):
        ...
        self.assertTrue(
            mo.move_raw_ids.filtered(lambda m: m.product_id == c1).manual_consumption
        )
        self.assertFalse(
            mo.move_raw_ids.filtered(lambda m: m.product_id == c2).manual_consumption
        )
        ...
        self.env["stock.quant"]._update_available_quantity(
            c1, self.stock_location, 8, lot_id=lot
        )
        ...
        self.assertEqual(
            sum(
                mo.move_raw_ids.filtered(lambda m: m.product_id.id == c1.id).mapped(
                    "quantity_done"
                )
            ),
            0,
        ...
        self.assertEqual(
            sum(
                mo.move_raw_ids.filtered(lambda m: m.product_id.id == c2.id).mapped(
                    "quantity_done"
                )
            ),
            1,
        ...
        details_operation_form = Form(
            mo.move_raw_ids.filtered(lambda m: m.product_id == c1),
        ...
        self.assertTrue(
            backorder_mo.move_raw_ids.filtered(
                lambda m: m.product_id == c1
            ).manual_consumption
        )
        self.assertFalse(
            backorder_mo.move_raw_ids.filtered(
                lambda m: m.product_id == c2
            ).manual_consumption
        )
    ...
    def test_manual_consumption_split_merge_00(self):
        ...
        self.assertTrue(
            mo.move_raw_ids.filtered(lambda m: m.product_id == p1).manual_consumption
        )
        self.assertFalse(
            mo.move_raw_ids.filtered(lambda m: m.product_id == p2).manual_consumption
        )
