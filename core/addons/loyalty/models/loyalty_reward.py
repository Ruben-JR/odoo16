...
class LoyaltyReward(models.Model):
    ...
    def default_get(self, fields_list):
        ...
        if "program_type" in self.env.context:
            ...
            program_default_values = self.env[
                "loyalty.program"
            ]._program_type_default_values()
            if (
                program_type in program_default_values
                and len(program_default_values[program_type]["reward_ids"]) == 2
                and isinstance(
                    program_default_values[program_type]["reward_ids"][1][2], dict
                )
            ):
                result.update(
                    {
                        k: v
                        for k, v in program_default_values[program_type]["reward_ids"][
                            1
                        ][2].items()
                        if k in fields_list
                    }
    ...
    def name_get(self):
        return [
            (reward.id, "%s - %s" % (reward.program_id.name, reward.description))
            for reward in self
        ]
    ...
    reward_type = fields.Selection(
        [("product", "Free Product"), ("discount", "Discount")],
        default="discount",
        required=True,
    )
    ...
    discount_product_ids = fields.Many2many(
        "product.product", string="Discounted Products"
    )
    discount_product_category_id = fields.Many2one(
        "product.category", string="Discounted Prod. Categories"
    )
    discount_product_tag_id = fields.Many2one(
        "product.tag", string="Discounted Prod. Tag"
    )
    all_discount_product_ids = fields.Many2many(
        "product.product", compute="_compute_all_discount_product_ids"
    )
    ...
    reward_product_uom_id = fields.Many2one(
        "uom.uom", compute="_compute_reward_product_uom_id"
    )
    ...
    def _compute_reward_product_domain(self):
        compute_all_discount_product = (
            self.env["ir.config_parameter"]
            .sudo()
            .get_param("loyalty.compute_all_discount_product_ids", "enabled")
        )
        for reward in self:
            if compute_all_discount_product == "enabled":
                reward.reward_product_domain = "null"
            else:
                reward.reward_product_domain = json.dumps(
    ...
    def _compute_multi_product(self):
        for reward in self:
            products = (
                reward.reward_product_id + reward.reward_product_tag_id.product_ids
            )
        ...
        for reward in self:
            ...
            if reward.program_type == "gift_card":
                ...
                if len(products) == 0:
                    ...
                    reward_string = _(
                        "Free Product - %s", reward.reward_product_id.name
                    )
                else:
                    reward_string = _(
                        "Free Product - [%s]", ", ".join(products.mapped("name"))
                    )
