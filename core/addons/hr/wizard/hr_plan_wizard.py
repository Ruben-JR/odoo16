...
class HrPlanWizard(models.TransientModel):
    ...
    employee_ids = fields.Many2many(
        "hr.employee",
        "hr_employee_hr_plan_wizard_rel",
        "employee_id",
        "plan_wizard_id",
        string="Employee",
        required=True,
    ...
    company_id = fields.Many2one(
        "res.company", "Company", compute="_compute_company_id", required=True
    )
        ...
        for wizard in self:
            if len(wizard.employee_ids.mapped("company_id")) > 1:
                raise ValidationError(
                    _("The employees should belong to the same company.")
                )
    ...
    def _compute_warning(self):
        for wizard in self:
            ...
            if warnings:
                warning_display = _(
                    "The plan %s cannot be launched: <br><ul>", wizard.plan_id.name
                )
