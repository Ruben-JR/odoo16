...
class Message(models.Model):
    ...
    * snailmail.letter (snailmail addon)
    ...
    def default_get(self, fields):
        ...
        if missing_author or missing_email_from:
            author_id, email_from = self.env["mail.thread"]._message_compute_author(
                res.get("author_id"), res.get("email_from"), raise_on_email=False
            )
    ...
    record_name = fields.Char(
        "Message Record Name"
    )  # name_get() of the related document
    ...
    canned_response_ids = fields.One2many(
        "mail.shortcode", "message_ids", string="Canned Responses", store=False
    )
    ...
    def _compute_is_current_user_or_guest_author(self):
        ...
        for message in self:
            if not user._is_public() and (
                message.author_id and message.author_id == user.partner_id
            ):
                message.is_current_user_or_guest_author = True
            elif user._is_public() and (
                message.author_guest_id
                and message.author_guest_id == self.env.context.get("guest")
            ):
                message.is_current_user_or_guest_author = True
            else:
    ...
    def _search_has_error(self, operator, operand):
        ...
        return [
            "!",
            ("notification_ids.notification_status", "in", ("bounce", "exception")),
        ]  # this wont work and will be equivalent to "not in" beacause of orm restrictions. Dont use "has_error = False"
    ...
    def init(self):
        ...
        self._cr.execute(
            """CREATE INDEX IF NOT EXISTS mail_message_model_res_id_id_idx ON mail_message (model, res_id, id)"""
        )
    ...
    def _search(
        ...
        super(
            Message, self.with_user(access_rights_uid or self._uid)
    ...
    def check_access_rule(self, operation):
        ...
        def _generate_model_record_ids(msg_val, msg_ids):
            ...
            for id in msg_ids:
                ...
                if vals.get("model") and vals.get("res_id"):
                    model_record_ids.setdefault(vals["model"], set()).add(
                        vals["res_id"]
                    )
        ...
        if not self.env["res.users"].has_group("base.group_user"):
            ...
            if self._cr.fetchall():
                raise AccessError(
                    ...
                    + " - ({} {}, {} {})".format(
                        _("Records:"), self.ids[:6], _("User:"), self._uid
                    )
                )
        ...
        self.flush_recordset(
            ["model", "res_id", "author_id", "parent_id", "message_type", "partner_ids"]
        )
        self.env["mail.notification"].flush_model(["mail_message_id", "res_partner_id"])
        ...
        if operation == "read":
            ...
            self._cr.execute(
                ...
                % self._table,
                dict(
                    pid=self.env.user.partner_id.id, uid=self.env.user.id, ids=self.ids
                ),
            )
            for (
                mid,
            ...
            self._cr.execute(
                """SELECT DISTINCT id, model, res_id, author_id, parent_id, message_type FROM "%s" WHERE id = ANY (%%s)"""
                % self._table,
                (self.ids,),
            )
            for (
        ...
        if operation == "read":
            author_ids = [
                mid
                for mid, message in message_values.items()
        ...
        for model, doc_ids in model_record_ids.items():
            ...
            if hasattr(DocumentModel, "_get_mail_message_access"):
                check_operation = DocumentModel._get_mail_message_access(
                    doc_ids, operation
                )  ## why not giving model here?
            else:
                check_operation = self.env["mail.thread"]._get_mail_message_access(
        ...
        if operation == "create":
            ...
            self._cr.execute(
                """SELECT DISTINCT m.id, partner_rel.res_partner_id FROM "%s" m
                LEFT JOIN "mail_message_res_partner_rel" partner_rel
        ...
        raise AccessError(
            ...
            + " - ({} {}, {} {})".format(
                _("Records:"), list(messages_to_check)[:6], _("User:"), self._uid
            )
        )
        ...
        for values in values_list:
            if "email_from" not in values:  # needed to compute reply_to
                _author_id, email_from = self.env[
                    ...
                    values.get("author_id"), email_from=None, raise_on_email=False
                )
    ...
    def toggle_message_starred(self):
        ...
        if starred:
            self.sudo().write(
                {"starred_partner_ids": [Command.link(self.env.user.partner_id.id)]}
            )
        else:
            self.sudo().write(
                {"starred_partner_ids": [Command.unlink(self.env.user.partner_id.id)]}
            )
    ...
    def _message_add_reaction(self, content):
        ...
        self.env[self.model].browse(self.res_id)._message_add_reaction_after_hook(
            message=self, content=reaction.content
        )
    ...
    def _message_remove_reaction(self, content):
        ...
        reaction.unlink()
        self.env[self.model].browse(self.res_id)._message_remove_reaction_after_hook(
    ...
    def _message_format(self, fnames, format_reply=True, legacy=False):
        ...
        for vals in vals_list:
            ...
            if message_sudo.model and message_sudo.res_id:
                ...
                record_name = False
            reactions_per_content = defaultdict(
                self.env["mail.message.reaction"].sudo().browse
            )
            ...
            if (
                format_reply
                and message_sudo.model == "mail.channel"
                and message_sudo.parent_id
            ):
                vals["parentMessage"] = message_sudo.parent_id.message_format(
                    format_reply=False
                )[0]
            allowed_tracking_ids = message_sudo.tracking_value_ids.filtered(
                lambda tracking: not tracking.field_groups
                or self.env.is_superuser()
                or self.user_has_groups(tracking.field_groups)
            )
            vals.update(
                {
                    "author": author,
                    ...
                    if not legacy
                    else message_sudo.attachment_ids._attachment_format(legacy=True),
    ...
    def message_format(self, format_reply=True):
        ...
        """
        vals_list = self._message_format(
            self._get_message_format_fields(), format_reply=format_reply
        )
        ...
        for vals in vals_list:
            ...
            vals.update(
                {
                    "needaction_partner_ids": notifs.filtered(
                        lambda n: not n.is_read
                    ).res_partner_id.ids,
                    "history_partner_ids": notifs.filtered(
                        lambda n: n.is_read
                    ).res_partner_id.ids,
                    ...
                    "recipients": [
                        {"id": p.id, "name": p.name} for p in message_sudo.partner_ids
                    ],
                }
            )
            if vals["model"] and self.env[vals["model"]]._original_module:
                vals["module_icon"] = modules.module.get_module_icon(
                    self.env[vals["model"]]._original_module
                )
        return vals_list
        ...
        return [
            {
                ...
                "model": message.model,
                "res_model_name": message.env["ir.model"]
                ._get(message.model)
                .display_name,
                "date": message.date,
    ...
    def _notify_message_notification_update(self):
        ...
        updates = [
            (
                ...
                {"elements": messages._message_notification_format()},
            )
    ...
    def _filter_empty(self):
        ...
        return self.filtered(
            lambda msg: (not msg.body or tools.is_html_empty(msg.body))
            and (not msg.subtype_id or not msg.subtype_id.description)
            and not msg.attachment_ids
            and not msg.tracking_value_ids
        )
    ...
    def _get_reply_to(self, values):
        ...
        if self.is_thread_message(
            {"model": model, "res_id": res_id, "message_type": message_type}
        ):
