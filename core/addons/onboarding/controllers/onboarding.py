...
class OnboardingController(http.Controller):
    ...
    def get_onboarding_data(self, route_name=None):
        ...
        onboarding = request.env["onboarding.onboarding"].search(
            [("route_name", "=", route_name)]
        )
        if (
            onboarding
            and not onboarding._search_or_create_progress().is_onboarding_closed
        ):
            # JS implementation of the onboarding panel expects this data structure
            return {
                "html": request.env["ir.qweb"]._render(
                    "onboarding.onboarding_panel",
                    onboarding._prepare_rendering_values(),
                )
            }
