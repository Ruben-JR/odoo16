...
class PaymentTransaction(models.Model):
    ...
    def _stripe_create_checkout_session(self):
        ...
        if self.operation == "online_redirect":
            return_url = (
                f"{urls.url_join(base_url, StripeController._checkout_return_url)}"
    ...
    def _stripe_create_payment_intent(self):
        ...
        if "error" not in response:
            ...
            payment_intent = response["error"].get(
                "payment_intent"
            )  # Get the PI from the error
    ...
    def _process_notification_data(self, notification_data):
        ...
        if self.operation == "validation":
            ...
            if (
                "charge" in notification_data
            ):  # The online_redirect operation may include a charge.
    ...
    def _stripe_tokenize_from_notification_data(self, notification_data):
        ...
        """
        if self.operation == "online_redirect":
            payment_method_id = notification_data.get("charge", {}).get(
                "payment_method"
            )
