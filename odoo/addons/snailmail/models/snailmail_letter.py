...
class SnailmailLetter(models.Model):
    ...
    def _compute_display_name(self):
        for letter in self:
            if letter.attachment_id:
                letter.display_name = "%s - %s" % (
                    letter.attachment_id.name,
    ...
    def _snailmail_create(self, route):
        ...
        for letter in self:
            recipient_name = (
                letter.partner_id.name
                or letter.partner_id.parent_id
                and letter.partner_id.parent_id.name
            )
        ...
        letters_send = self.search(
            [
                ...
                (
                    "error_code",
                    "in",
                    ["TRIAL_ERROR", "CREDIT_ERROR", "MISSING_REQUIRED_FIELDS"],
                ),
            ]
        )
    ...
    def _append_cover_page(self, invoice_bin: bytes):
        address_split = (
            self.partner_id.with_context(show_address=True, lang="en_US")
            ._get_name()
            .split("\n")
        )
        address_split[0] = (
