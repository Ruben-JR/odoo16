...
class TestExpensesMailImport(TestExpenseCommon):
    ...
    def test_import_expense_from_email(self):
        message_parsed = {
            ...
            "subject": "%s %s"
            % (self.product_a.default_code, self.product_a.standard_price),
    ...
    def test_import_expense_from_mail_parsing_subjects(self):
        def assertParsedValues(
            subject, currencies, exp_description, exp_amount, exp_product
        ):
        ...
        self.expense_user_employee.groups_id |= self.env.ref(
