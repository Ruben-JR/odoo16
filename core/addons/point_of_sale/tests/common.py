...
class TestPointOfSaleCommon(ValuationReconciliationTestCommon):
    ...
    def setUpClass(cls, chart_template_ref=None):
        ...
        cls.product4.write(
            {
                "taxes_id": [
                    (6, 0, [account_tax_05_incl.id, account_tax_05_incl_chicago.id])
                ]
            }
        )
...
class TestPoSCommon(ValuationReconciliationTestCommon):
    ...
    def create_ui_order_data(
        ...
        order_lines = [
            ...
            for product, quantity, *discount in pos_order_lines_ui_args
        ]
    ...
    def _create_orders(self, order_data_params):
        ...
        for params in order_data_params:
            ...
            result[params["uid"]] = self.env["pos.order"].browse(
                [
                    order["id"]
                    for order in self.env["pos.order"].create_from_ui([order_data])
                ]
            )
        return result
