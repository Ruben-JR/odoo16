...
class AccountMoveLine(models.Model):
        ...
        if term_lines:
            ...
            for line in term_lines:
                ...
                account_id = (
                    ...
                    or accounts.get(
                        ("res.partner", move.commercial_partner_id.id, account_type)
                    )
                    or accounts.get(
                        ("res.partner", move.company_id.partner_id.id, account_type)
                    )
        ...
        for record in self:
            if not record.tax_repartition_line_id and not record.tax_ids:
                ...
                record.tax_tag_invert = (
                    record.tax_tag_ids and record.move_id.is_inbound()
                )
            ...
            if record.move_id.tax_cash_basis_origin_move_id and (
                record.move_id.reversed_entry_id or record.tax_base_amount < 0
            ):
    ...
    def _compute_tax_audit(self):
        ...
        for record in self:
            ...
            for tag in record.tax_tag_ids:
                tag_amount = (
                    (record.tax_tag_invert and -1 or 1)
                    * (tag.tax_negate and -1 or 1)
                    * record.balance
                )
                ...
                audit_str += (
                    tag_name
                    + ": "
                    + formatLang(self.env, tag_amount, currency_obj=currency)
                )
    ...
    def _get_computed_taxes(self):
        ...
        if self.move_id.is_sale_document(include_receipts=True):
            ...
            if self.product_id.taxes_id:
                ...
                tax_ids = self.account_id.tax_ids.filtered(
                ...
                line.tax_key = frozendict(
                    {
                        ...
                        "display_type": "epd"
                        if line.name and _("(Discount)") in line.name
                        else line.display_type,
                    }
                )
            else:
    ...
    def _compute_epd_key(self):
        for line in self:
            if (
                ...
                and line.company_id.early_pay_discount_computation == "mixed"
            ):
    ...
    def _compute_epd_needed(self):
        for line in self:
            ...
            if (
                ...
                or line.company_id.early_pay_discount_computation != "mixed"
            ):
                continue
                    ...
                    percentages_to_apply.append(
                        {
                            ...
                            "term_percentage": abs(
                                term["amount_currency"] / amount_total
                            )
                            if amount_total
                            else 0,
                        }
            ...
            for percentages in percentages_to_apply:
                    ...
                    frozendict(
                        {
                            ...
                            "tax_tag_ids": line.compute_all_tax[
                                frozendict({"id": line.id})
                            ]["tax_tag_ids"],
                            "display_type": "epd",
                        }
                    ),
                ...
                epd_needed_vals["amount_currency"] -= (
                    line.amount_currency * percentage * line_percentage
                )
                epd_needed_vals["balance"] -= (
                    line.balance * percentage * line_percentage
                )
                epd_needed_vals["price_subtotal"] -= (
                    line.price_subtotal * percentage * line_percentage
                )
                epd_needed_vals = epd_needed.setdefault(
                    frozendict(
                        {
    ...
    def _inverse_partner_id(self):
        self._conditional_add_to_compute(
            ...
            lambda line: (
                ...
                or (
                    line.move_id.is_invoice(True)
                    and line.display_type == "product"
                    and not line.product_id
                )  # recompute based on most used account
            ),
        )
    ...
    def _check_constrains_account_id_journal_id(self):
        for line in self.filtered(
            ...
            if (
                ...
                raise UserError(
                    _(
                        "You cannot use this account (%s) in this journal, check the field 'Allowed Journals' on the related account.",
            ...
            is_account_control_ok = (
                not journal.account_control_ids
            ...
            if not is_account_control_ok:
                raise UserError(
                    _(
                        "You cannot use this account (%s) in this journal, check the section 'Control-Access' under "
                        "tab 'Advanced Settings' on the related journal.",
                        account.display_name,
                    )
                )
    ...
    def search_read(self, domain=None, fields=None, offset=0, limit=None, order=None):
        ...
        return super(AccountMoveLine, contextualized).search_read(
    ...
    def create(self, vals_list):
        ...
        with moves._check_balanced(move_container), moves._sync_dynamic_lines(
            move_container
    ...
    def write(self, vals):
        ...
        with self.move_id._check_balanced(
            move_container
        ), self.move_id._sync_dynamic_lines(move_container), self._sync_invoice(
            {"records": self}
        ):
            self = line_to_write
            if not self:
    ...
    def _search_panel_domain_image(
        ...
        return {
            ...
            for root in self.env["account.root"].browse(
            ...
            for i, changed_field in enumerate(changes):
                if tracked_fields[changed_field]["type"] in ["one2many", "many2many"]:
                    ...
                    vals = {
                        ...
                        "old_value_char": ", ".join(
                            initial[changed_field].mapped("name")
                        ),
    ...
    def _prepare_reconciliation_partials(self, vals_list):
        ...
        debit_vals_list = iter(
            [x for x in vals_list if x["balance"] > 0.0 or x["amount_currency"] > 0.0]
        )
    ...
    def _add_exchange_difference_cash_basis_vals(self, exchange_diff_vals):
        ...
        )  # in the form {(move, account, repartition_line): move_lines}
        move_vals = exchange_diff_vals["move_vals"]
        for move in self.move_id:
            ...
            for caba_treatment, line in move_values["to_process_lines"]:
                ...
                if caba_treatment == "tax":
                        ...
                        account_vals_to_fix[grouping_key].update(
                            {
                                ...
                                "amount_currency": account_vals_to_fix[grouping_key][
                                    "amount_currency"
                                ]
                    ...
                    if line.account_id.reconcile:
                        caba_lines_to_reconcile[
                            (move, line.account_id, line.tax_repartition_line_id)
                        ] |= line
                    ...
                    if grouping_key not in account_vals_to_fix:
                        ...
                        account_vals_to_fix[grouping_key]["debit"] += vals["debit"]
                        account_vals_to_fix[grouping_key]["credit"] += vals["credit"]
                        account_vals_to_fix[grouping_key]["amount_currency"] += vals[
                            "amount_currency"
                        ]
            ...
            for line in cash_basis_moves.line_ids:
                ...
                if line.tax_repartition_line_id:
                    # Tax line.
                    transition_account = (
                        line.tax_line_id.cash_basis_transition_account_id
                    )
                    grouping_key = self.env[
                        "account.partial.reconcile"
            ...
            for transition_line in filter(
                lambda x: x.account_id in caba_transition_accounts,
                cash_basis_moves.line_ids,
            ):
                caba_reconcile_key = (
                    transition_line.move_id,
                    transition_line.account_id,
                    transition_line.tax_repartition_line_id,
                )
            ...
            last_caba_move = (
                max(cash_basis_moves, key=lambda m: m.date)
                if cash_basis_moves
            ...
            currency_rate = (
                currency_line.balance / currency_line.amount_currency
                if currency_line.amount_currency
                else 1.0
            )
    ...
    def reconcile(self):
        ...
        involved_partials = (
            involved_lines.matched_credit_ids | involved_lines.matched_debit_ids
        )
        ...
        if all(
            is_line_reconciled(line, has_multiple_currencies) for line in involved_lines
        ):
            ...
            if not self._context.get("no_exchange_difference"):
                ...
                if is_cash_basis_needed:
                    caba_lines_to_reconcile = (
                        involved_lines._add_exchange_difference_cash_basis_vals(
                            exchange_diff_vals
                        )
                    )
        ...
        for line in self.filtered(lambda line: line.display_type == "product"):
            line._validate_distribution(
                **{
                    ...
                    "business_domain": line.move_id.move_type
                    in ["out_invoice", "out_refund", "out_receipt"]
                    and "invoice"
                    or line.move_id.move_type
    ...
    def _all_reconciled_lines(self):
        reconciliation_lines = self.filtered(
            lambda x: x.account_id.reconcile
            or x.account_id.account_type in ("asset_cash", "liability_credit_card")
        )
    ...
    def _is_eligible_for_early_payment_discount(self, currency, reference_date):
        self.ensure_one()
        return (
            self.display_type == "payment_term"
            and self.currency_id == currency
            and self.move_id.move_type
