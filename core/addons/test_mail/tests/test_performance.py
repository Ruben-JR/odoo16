...
class TestMailAPIPerformance(BaseMailPerformance):
    ...
    def test_mail_composer_form_attachments(self):
        ...
        self.assertEqual(
            message.notified_partner_ids, customer + self.user_test.partner_id
        )
    ...
    def test_mail_composer_mass_w_template(self):
        ...
        with self.assertQueryCount(admin=156, employee=159), self.mock_mail_gateway():
            composer._action_send_mail()
    ...
    def test_mail_composer_w_template_form(self):
        ...
        self.assertEqual(
            ...
            customer + self.user_test.partner_id + new_partner,
        )
    ...
    def test_message_log(self):
        ...
        with self.assertQueryCount(admin=1, employee=1):
            record._message_log(body="<p>Test _message_log</p>", message_type="comment")
    ...
    def test_message_log_batch(self):
        ...
        with self.assertQueryCount(admin=1, employee=1):
            records._message_log_batch(
                bodies=dict(
                    (record.id, "<p>Test _message_log</p>") for record in records
                ),
    ...
    def test_message_subscribe_subtypes(self):
        ...
        with self.assertQueryCount(admin=5, employee=5):
            record.message_subscribe(
                partner_ids=self.user_test.partner_id.ids, subtype_ids=subtype_ids
            )
...
class TestMailComplexPerformance(BaseMailPerformance):
    def setUp(self):
        ...
        for x in range(0, 10):
            self.partners |= Partners.create(
                {"name": "Test %s" % x, "email": "test%s@example.com" % x}
            )
        self.container.message_subscribe(
            self.partners.ids,
            subtype_ids=[
    ...
    def test_complex_mail_mail_send_batch_complete(self):
        ...
        with self.assertQueryCount(admin=43, employee=43), patch.object(
            type(self.env["mail.mail"]), "unlink", _patched_unlink
        ):
    ...
    def test_complex_message_post_template(self):
        ...
        with self.assertQueryCount(admin=63, employee=62):
            record.message_post_with_template(
                template_id, message_type="comment", composition_mode="comment"
            )
    ...
    def test_message_format(self):
        ...
        customer_id_field = self.env["ir.model.fields"]._get(
...
class TestMailHeavyPerformancePost(BaseMailPerformance):
    ...
    def test_complete_message_post(self):
        ...
        recipients = (
            self.user_inbox.partner_id + self.user_email.partner_id + self.partner
        )
        ...
        self.assertEqual(
            attachments.mapped("res_model"), [record_container._name for i in range(3)]
        )
        self.assertEqual(
            attachments.mapped("res_id"), [record_container.id for i in range(3)]
        )
        self.assertTrue(
