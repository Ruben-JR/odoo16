...
class AccountEdiFormat(models.Model):
    ...
    def _l10n_it_edi_search_tax_for_import(
        self, company, percentage, extra_domain=None, vat_only=True
    ):
        ...
        return super()._l10n_it_edi_search_tax_for_import(
            company, percentage, extra_domain
        )
    ...
    def _l10n_it_edi_check_taxes_configuration(self, invoice):
        ...
        for invoice_line in invoice.invoice_line_ids.filtered(
            ...
            vat_taxes, withholding_taxes, pension_fund_taxes = [
                invoice_line.tax_ids._l10n_it_filter_kind(kind)
                for kind in ("vat", "withholding", "pension_fund")
            ]
            if not invoice_line.display_type:
                if len(vat_taxes) != 1:
                    errors.append(
                        _(
                            "Bad tax configuration for line %s, there must be one and only one VAT tax per line",
                            invoice_line.name,
                        )
                    )
                if len(pension_fund_taxes) > 1 or len(withholding_taxes) > 1:
                    errors.append(
                        _(
                            "Bad tax configuration for line %s, there must be one Withholding tax and one Pension Fund tax at max.",
                            invoice_line.name,
                        )
                    )
        return errors
    ...
    def _import_fattura_pa_line(self, element, invoice_line_form, extra_info):
        messages_to_log = super()._import_fattura_pa_line(
        ...
        for other_data_element in element.xpath(".//AltriDatiGestionali"):
            ...
            if not data_kind_element or not text_element or not number_element:
                continue
            data_kind, data_text, number_text = (
                data_kind_element[0].text.lower(),
                text_element[0].text.lower(),
                number_element[0].text,
            )
            if data_kind != "cassa-prev" or (
                "enasarco" not in data_text and not "tc07" in data_text
            ):
                continue
