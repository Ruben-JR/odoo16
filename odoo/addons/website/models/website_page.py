...
class Page(models.Model):
    ...
    view_id = fields.Many2one(
        "ir.ui.view", string="View", required=True, ondelete="cascade"
    )
    ...
    is_in_menu = fields.Boolean(
        compute="_compute_website_menu", inverse="_inverse_website_menu"
    )
    is_homepage = fields.Boolean(
        compute="_compute_is_homepage", inverse="_set_is_homepage", string="Homepage"
    )
        ...
        for page in self:
            ...
            if "name" in vals and page.name != vals["name"]:
                vals["key"] = (
                    self.env["website"]
                    .with_context(website_id=website_id)
                    .get_unique_key(slugify(vals["name"]))
                )
