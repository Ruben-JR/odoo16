...
class SaleOrderLine(models.Model):
    ...
    def _compute_qty_to_deliver(self):
        ...
        for line in self:
            ...
            if line.state == "sale":
                ...
                boms = boms._bom_find(
                    line.product_id, company_id=line.company_id.id, bom_type="phantom"
                )[line.product_id]
            relevant_bom = boms.filtered(
                ...
                and (
                    ...
                    or (
                        b.product_tmpl_id == line.product_id.product_tmpl_id
                ...
                if relevant_bom:
                    ...
                    order_line.qty_delivered += (
                        relevant_bom.product_uom_id._compute_quantity(
                            qty_delivered, order_line.product_uom
                        )
                    )
    ...
    def _get_qty_procurement(self, previous_product_uom_qty=False):
        ...
        bom = self.env["mrp.bom"]._bom_find(self.product_id, bom_type="phantom")[
            self.product_id
        ]
        if bom:
            ...
            order_qty = (
                previous_product_uom_qty.get(self.id, 0)
                if previous_product_uom_qty
                else self.product_uom_qty
            )
            order_qty = self.product_uom._compute_quantity(
                order_qty, bom.product_uom_id
            )
            qty = moves._compute_kit_quantities(
                self.product_id, order_qty, bom, filters
            )
