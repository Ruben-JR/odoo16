...
class DeliveryCarrier(models.Model):
    ...
    delivery_type = fields.Selection(
        [("fixed", "Fixed Price")], string="Provider", default="fixed", required=True
    )
    ...
    def onchange_countries(self):
        self.state_ids = [
            (
                6,
                0,
                self.state_ids.filtered(
                    lambda state: state.id in self.country_ids.mapped("state_ids").ids
                ).ids,
            )
        ]
    ...
    def rate_shipment(self, order):
        ...
        if hasattr(self, "%s_rate_shipment" % self.delivery_type):
            ...
            if (
                res["success"]
                and self.free_over
                and order._compute_amount_total_without_delivery() >= self.amount
            ):
    ...
    def _get_commodities_from_order(self, order):
        ...
        for line in order.order_line.filtered(
            ...
            unit_quantity = line.product_uom._compute_quantity(
    ...
    def _get_commodities_from_stock_move_lines(self, move_lines):
        ...
        for line in move_lines.filtered(
            ...
            country_of_origin = (
                line.product_id.country_of_origin.code
                or line.picking_id.picking_type_id.warehouse_id.partner_id.country_id.code
            )
            commodities.append(
                DeliveryCommodity(
                    line.product_id,
