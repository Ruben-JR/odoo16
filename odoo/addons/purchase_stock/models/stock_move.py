...
class StockMove(models.Model):
    ...
    def _get_price_unit(self):
        ...
        if self.state == "done":
            received_qty -= self.product_uom._compute_quantity(
                self.quantity_done, line.product_uom
            )
        if (
            float_compare(
                line.qty_invoiced,
            ...
            receipt_value = sum(move_layer.mapped("value")) + sum(
                invoiced_layer.mapped("value")
            )
            invoiced_value = 0
            invoiced_qty = 0
            for invoice_line in line.invoice_lines:
                if invoice_line.tax_ids:
                    invoiced_value += invoice_line.tax_ids.with_context(
                        round=False
                    ).compute_all(
                        invoice_line.price_unit,
                        currency=invoice_line.account_id.currency_id,
    ...
    def _generate_valuation_lines_data(
        ...
        if (
            ...
            or purchase_currency == self.company_id.currency_id
        ):
            return rslt
        ...
        if not svl.account_move_line_id:
            if self.purchase_line_id.product_id.cost_method == "standard":
                purchase_price_unit = (
                    self.purchase_line_id.product_id.cost_currency_id._convert(
            ...
            currency_move_valuation = purchase_currency.round(
                purchase_price_unit * abs(qty)
            )
            rslt["credit_line_vals"]["amount_currency"] = (
                rslt["credit_line_vals"]["balance"] < 0
                and -currency_move_valuation
                or currency_move_valuation
            )
            rslt["debit_line_vals"]["amount_currency"] = (
                rslt["debit_line_vals"]["balance"] < 0
                and -currency_move_valuation
                or currency_move_valuation
            )
    ...
    def _get_upstream_documents_and_responsibles(self, visited):
        if (
            ...
            and self.created_purchase_line_id.state not in ("done", "cancel")
            and (
                self.created_purchase_line_id.state != "draft"
                or self._context.get("include_draft_documents")
            )
        ):
            return [
                (
                    self.created_purchase_line_id.order_id,
                    self.created_purchase_line_id.order_id.user_id,
                    visited,
                )
            ]
        elif self.purchase_line_id and self.purchase_line_id.state not in (
            "done",
            "cancel",
        ):
            return [
    ...
    def _get_related_invoices(self):
        ...
        rslt += self.mapped("picking_id.purchase_id.invoice_ids").filtered(
    ...
    def _get_all_related_aml(self):
        ...
        return (
            ...
            | self.purchase_line_id.invoice_lines.move_id.line_ids.filtered(
