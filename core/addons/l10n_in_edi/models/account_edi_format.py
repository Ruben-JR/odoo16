...
class AccountEdiFormat(models.Model):
    ...
    def _check_move_configuration(self, move):
        ...
        error_message += self._l10n_in_validate_partner(
            move.company_id.partner_id, is_company=True
        )
        if not re.match("^.{1,16}$", move.name):
            error_message.append(
                _("Invoice number should not be more than 16 characters")
            )
        for line in move.invoice_line_ids.filtered(
            lambda line: line.display_type
            not in ("line_note", "line_section", "rounding")
        ):
            ...
            if line.product_id:
                hsn_code = self._l10n_in_edi_extract_digits(
    ...
    def _l10n_in_edi_get_iap_buy_credits_message(self, company):
        base_url = (
            "https://iap-sandbox.odoo.com/iap/1/credit"
            if not company.sudo().l10n_in_edi_production_env
            else ""
        )
        url = self.env["iap.account"].get_credits_url(
            service_name="l10n_in_edi", base_url=base_url
        )
    ...
    def _l10n_in_edi_post_invoice(self, invoice):
        ...
        if response.get("error"):
            ...
            if "1005" in error_codes:
                ...
                authenticate_response = self._l10n_in_edi_authenticate(
                    invoice.company_id
                )
                if not authenticate_response.get("error"):
                    error = []
                    response = self._l10n_in_edi_generate(
                        invoice.company_id, generate_json
                    )
                    if response.get("error"):
                        error = response["error"]
                        error_codes = [e.get("code") for e in error]
            ...
            if "no-credit" in error_codes:
                ...
                error_message = "<br/>".join(
                    [
                        "[%s] %s" % (e.get("code"), html_escape(e.get("message")))
                        for e in error
                    ]
                )
                return {
                    invoice: {
                        "success": False,
                        "error": error_message,
                        "blocking_level": ("404" in error_codes)
                        and "warning"
                        or "error",
                    }
                }
    ...
    def _l10n_in_validate_partner(self, partner, is_company=False):
        ...
        if partner.phone and not re.match(
            "^[0-9]{10,12}$", self._l10n_in_edi_extract_digits(partner.phone)
        ):
            message.append(
    ...
    def _get_l10n_in_edi_saler_buyer_party(self, move):
        return {
            ...
            "dispatch_details": move._l10n_in_get_warehouse_address()
    ...
    def _get_l10n_in_edi_partner_details(
        ...
        if set_phone_and_email:
            ...
            if partner.phone:
                partner_details.update(
                    {"Ph": self._l10n_in_edi_extract_digits(partner.phone)}
                )
        if pos_state_id:
        ...
        if set_vat:
            ...
            partner_details.update(
                {"Nm": partner.name or partner.commercial_partner_id.name}
            )
    ...
    def _get_l10n_in_edi_line_details(self, index, line, line_tax_details):
        ...
        full_discount_or_zero_quantity = line.discount == 100.00 or float_is_zero(
            quantity, 3
        )
        ...
        return {
            ...
            "Unit": line.product_uom_id.l10n_in_code
            ...
            "IgstAmt": self._l10n_in_round_value(
                tax_details_by_code.get("igst_amount", 0.00)
            ),
            "CgstAmt": self._l10n_in_round_value(
                tax_details_by_code.get("cgst_amount", 0.00)
            ),
            "SgstAmt": self._l10n_in_round_value(
                tax_details_by_code.get("sgst_amount", 0.00)
            ),
            "CesRt": self._l10n_in_round_value(
                tax_details_by_code.get("cess_rate", 0.00), 3
            ),
            "CesAmt": self._l10n_in_round_value(
            ...
            "StateCesRt": self._l10n_in_round_value(
                tax_details_by_code.get("state_cess_rate_amount", 0.00), 3
            ),
            "StateCesAmt": self._l10n_in_round_value(
            ...
            "OthChrg": self._l10n_in_round_value(
                tax_details_by_code.get("other_amount", 0.00)
            ),
    ...
    def _l10n_in_edi_generate_invoice_json_managing_negative_lines(
        self, invoice, json_payload
    ):
        ...
        def put_discount_on(discount_line_vals, other_line_vals):
            ...
            if (
                ...
                discount_line_vals.update(
                    {
                        "AssAmt": self._l10n_in_round_value(
                            discount_line_vals["AssAmt"] + other_line_vals["AssAmt"]
                        ),
                        "IgstAmt": self._l10n_in_round_value(
                            discount_line_vals["IgstAmt"] + other_line_vals["IgstAmt"]
                        ),
                        "CgstAmt": self._l10n_in_round_value(
                            discount_line_vals["CgstAmt"] + other_line_vals["CgstAmt"]
                        ),
                        "SgstAmt": self._l10n_in_round_value(
                            discount_line_vals["SgstAmt"] + other_line_vals["SgstAmt"]
                        ),
                        "CesAmt": self._l10n_in_round_value(
                            discount_line_vals["CesAmt"] + other_line_vals["CesAmt"]
                        ),
                        "CesNonAdvlAmt": self._l10n_in_round_value(
                            discount_line_vals["CesNonAdvlAmt"]
                            + other_line_vals["CesNonAdvlAmt"]
                        ),
                        "StateCesAmt": self._l10n_in_round_value(
                            discount_line_vals["StateCesAmt"]
                            + other_line_vals["StateCesAmt"]
                        ),
                        "StateCesNonAdvlAmt": self._l10n_in_round_value(
                            discount_line_vals["StateCesNonAdvlAmt"]
                            + other_line_vals["StateCesNonAdvlAmt"]
                        ),
                        "OthChrg": self._l10n_in_round_value(
                            discount_line_vals["OthChrg"] + other_line_vals["OthChrg"]
                        ),
                        "TotItemVal": self._l10n_in_round_value(
                            discount_line_vals["TotItemVal"]
                            + other_line_vals["TotItemVal"]
                        ),
                    }
                )
            ...
            other_line_vals.update(
                {
                    ...
                    "AssAmt": self._l10n_in_round_value(
                        other_line_vals["AssAmt"] + discount_line_vals["AssAmt"]
                    ),
                    "IgstAmt": self._l10n_in_round_value(
                        other_line_vals["IgstAmt"] + discount_line_vals["IgstAmt"]
                    ),
                    "CgstAmt": self._l10n_in_round_value(
                        other_line_vals["CgstAmt"] + discount_line_vals["CgstAmt"]
                    ),
                    "SgstAmt": self._l10n_in_round_value(
                        other_line_vals["SgstAmt"] + discount_line_vals["SgstAmt"]
                    ),
                    "CesAmt": self._l10n_in_round_value(
                        other_line_vals["CesAmt"] + discount_line_vals["CesAmt"]
                    ),
                    "CesNonAdvlAmt": self._l10n_in_round_value(
                        other_line_vals["CesNonAdvlAmt"]
                        + discount_line_vals["CesNonAdvlAmt"]
                    ...
                    "OthChrg": self._l10n_in_round_value(
                        other_line_vals["OthChrg"] + discount_line_vals["OthChrg"]
                    ),
                    "TotItemVal": self._l10n_in_round_value(
                        other_line_vals["TotItemVal"] + discount_line_vals["TotItemVal"]
                    ),
                }
            )
            return True
        ...
        for discount_line in discount_lines:
            apply_discount_on_lines = lines_grouped_and_sorted.get(
    ...
    def _l10n_in_edi_generate_invoice_json(self, invoice):
        ...
        json_payload = {
            ...
            "TranDtls": {
                ...
                "IgstOnIntra": is_intra_state
                and tax_details_by_code.get("igst")
                and "Y"
                or "N",
            },
            "DocDtls": {
            ...
            "SellerDtls": self._get_l10n_in_edi_partner_details(
                saler_buyer.get("seller_details")
            ),
            "BuyerDtls": self._get_l10n_in_edi_partner_details(
                saler_buyer.get("buyer_details"),
                pos_state_id=invoice.l10n_in_state_id,
            ...
            "ValDtls": {
                ...
                "CgstVal": self._l10n_in_round_value(
                    tax_details_by_code.get("cgst_amount", 0.00)
                ),
                "SgstVal": self._l10n_in_round_value(
                    tax_details_by_code.get("sgst_amount", 0.00)
                ),
                "IgstVal": self._l10n_in_round_value(
                    tax_details_by_code.get("igst_amount", 0.00)
                ),
                ...
                "RndOffAmt": self._l10n_in_round_value(rounding_amount),
            ...
            json_payload.update(
                {
                    "DispDtls": self._get_l10n_in_edi_partner_details(
                        saler_buyer.get("dispatch_details"),
                        set_vat=False,
            ...
            json_payload.update(
                {
                    "ShipDtls": self._get_l10n_in_edi_partner_details(
                        saler_buyer.get("ship_to_details"), is_overseas=is_overseas
                    )
                }
            )
        if is_overseas:
            json_payload.update(
                {
                    "ExpDtls": {
                        ...
                        "CntCode": saler_buyer.get("buyer_details").country_id.code
                        or "",
                    }
                }
            )
            ...
            if invoice.l10n_in_shipping_bill_date:
                json_payload["ExpDtls"].update(
                    {
                        "ShipBDt": invoice.l10n_in_shipping_bill_date.strftime(
                            "%d/%m/%Y"
                        ),
                    }
                )
        ...
        return self._l10n_in_edi_generate_invoice_json_managing_negative_lines(
            invoice, json_payload
        )

    @api.model
    def _l10n_in_prepare_edi_tax_details(
        self, move, in_foreign=False, filter_invl_to_apply=None
    ):
        def l10n_in_grouping_key_generator(base_line, tax_values):
            ...
            if not invl.currency_id.is_zero(tax_values["tax_amount_currency"]):
                if any(tag in tags for tag in self.env.ref("l10n_in.tax_tag_cess")):
                    ...
                    tag in tags for tag in self.env.ref("l10n_in.tax_tag_state_cess")
                ):
                    if tax.amount_type != "percent":
                    ...
                    for gst in ["cgst", "sgst", "igst"]:
                        if any(
                            tag in tags
                            for tag in self.env.ref("l10n_in.tax_tag_%s" % (gst))
                        ):
                            line_code = gst
            return {
                "tax": tax,
    ...
    def _get_l10n_in_tax_details_by_line_code(self, tax_details):
        ...
        for tax_detail in tax_details.values():
            ...
            l10n_in_tax_details.setdefault(
                "%s_rate" % (tax_detail["line_code"]), tax_detail["tax"].amount
            )
            l10n_in_tax_details.setdefault(
                "%s_amount" % (tax_detail["line_code"]), 0.00
            )
            l10n_in_tax_details.setdefault(
                "%s_amount_currency" % (tax_detail["line_code"]), 0.00
            )
            l10n_in_tax_details["%s_amount" % (tax_detail["line_code"])] += tax_detail[
                "tax_amount"
            ]
            l10n_in_tax_details[
                "%s_amount_currency" % (tax_detail["line_code"])
            ] += tax_detail["tax_amount_currency"]
        return l10n_in_tax_details

    def _l10n_in_get_supply_type(self, move, tax_details_by_code):
        supply_type = "B2B"
        if move.l10n_in_gst_treatment in (
            "overseas",
            "special_economic_zone",
        ) and tax_details_by_code.get("igst_amount"):
            supply_type = (
                move.l10n_in_gst_treatment == "overseas" and "EXPWP" or "SEZWP"
            )
        elif move.l10n_in_gst_treatment in ("overseas", "special_economic_zone"):
            supply_type = (
                move.l10n_in_gst_treatment == "overseas" and "EXPWOP" or "SEZWOP"
            )
    ...
    def _l10n_in_edi_get_token(self, company):
        ...
        if (
            sudo_company.l10n_in_edi_username
            and sudo_company._l10n_in_edi_token_is_valid()
        ):
    ...
    def _l10n_in_edi_connect_to_server(self, company, url_path, params):
        ...
        endpoint = (
            ...
            .sudo()
            .get_param("l10n_in_edi.endpoint", default_endpoint)
        )
    ...
    def _l10n_in_edi_authenticate(self, company):
        ...
        response = self._l10n_in_edi_connect_to_server(
            company, url_path="/iap/l10n_in_edi/1/authenticate", params=params
        )
        ...
        if "data" in response:
            ...
            company.sudo().l10n_in_edi_token_validity = fields.Datetime.to_string(
                utc_time
            )
    ...
    def _l10n_in_edi_generate(self, company, json_payload):
        ...
        return self._l10n_in_edi_connect_to_server(
            company, url_path="/iap/l10n_in_edi/1/generate", params=params
        )
    ...
    def _l10n_in_edi_cancel(self, company, json_payload):
        ...
        return self._l10n_in_edi_connect_to_server(
            company, url_path="/iap/l10n_in_edi/1/cancel", params=params
        )
