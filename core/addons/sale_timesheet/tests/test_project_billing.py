...
class TestProjectBilling(TestCommonSaleTimesheet):
    ...
    def test_make_billable_at_task_rate(self):
        ...
        task = Task.with_context(
            default_project_id=self.project_non_billable.id
        ).create(
            {
                "name": "first task",
        ...
        # create wizard
        wizard = (
            self.env["project.create.sale.order"]
            .with_context(
                active_id=self.project_non_billable.id, active_model="project.project"
            )
            .create(
                {
                    "line_ids": [
                        Command.create(
                            {
                                ...
                                "price_unit": self.product_delivery_timesheet3.lst_price,
                            }
                        ),
                    ],
                }
        ...
        self.assertEqual(
            wizard.partner_id,
            self.project_non_billable.partner_id,
        ...
        self.assertEqual(
            sale_order.order_line.project_id,
        ...
        self.assertEqual(
            sale_order.order_line.task_id.project_id,
            self.project_non_billable,
    ...
    def test_make_billable_at_employee_rate(self):
        ...
        Task = self.env["project.task"]
        # set a customer on the project
        self.project_non_billable.write({"partner_id": self.partner_2.id})
        # create a task and 2 timesheets
        task = Task.with_context(
            default_project_id=self.project_non_billable.id
        ).create(
            ...
            .create(
                {
                    ...
                    "line_ids": [
                        ...
                        ),  # product creates no T (same product than previous one)
                        (
                            0,
                            0,
                            {
                        ...
                        ),  # product creates new T in new P
                    ],
                }
            )
        )
        ...
        self.assertEqual(
            wizard.project_id,
            self.project_non_billable,
            "The wizard'project should be the non billable project",
        )
        ...
        self.assertEqual(
            ...
            2,
            "The SO should have 2 lines, as in wizard map there were 2 time the same product with the same price (for 2 different employees)",
        )
        self.assertEqual(
            len(self.project_non_billable.sale_line_employee_ids),
            3,
            "The project have 3 lines in its map",
        )
        self.assertEqual(
            self.project_non_billable.pricing_type,
            "employee_rate",
            "The pricing type of the project should be employee rate since we have some mappings in this project.",
        )
        self.assertEqual(
            self.project_non_billable.sale_line_id,
            sale_order.order_line[0],
            "The wizard sets sale line fallbakc on project as the first of the list",
        )
        self.assertEqual(
            task.sale_line_id,
            sale_order.order_line[0],
            "The wizard sets sale line fallback on tasks",
        )
        self.assertEqual(
            task.partner_id,
            wizard.partner_id,
            "The wizard sets the customer on tasks to make SOL line field visible",
        )

        line1 = sale_order.order_line.filtered(
            lambda sol: sol.product_id == self.product_delivery_timesheet1
        )
        line2 = sale_order.order_line.filtered(
            lambda sol: sol.product_id == self.product_delivery_timesheet3
        )
        ...
        self.assertFalse(
            line1.project_id,
            "Sale line 1 should be linked to the 'non billable' project",
        )
        self.assertEqual(
            line2.project_id,
            self.project_non_billable,
            "Sale line 3 should be linked to the 'non billable' project",
        )
        self.assertEqual(
            line1.price_unit, 15.0, "The unit price of SOL 1 should be 15.0"
        )
        self.assertEqual(
            line1.product_uom_qty, 3, "The ordered qty of SOL 1 should be 3"
        )
        self.assertEqual(
            line2.product_uom_qty, 2, "The ordered qty of SOL 2 should be 2"
        )

        self.assertEqual(
            self.project_non_billable.sale_line_employee_ids.mapped("sale_line_id"),
            sale_order.order_line,
            "The SO lines of the map should be the same of the sales order",
        )
        self.assertEqual(
            timesheet1.so_line,
            line1,
            "Timesheet1 should be linked to sale line 1, as employee manager create the timesheet",
        )
        self.assertEqual(
            timesheet2.so_line,
            line2,
            "Timesheet2 should be linked to sale line 2, as employee tde create the timesheet",
        )
        self.assertEqual(
            timesheet1.unit_amount,
            line1.qty_delivered,
            "Sale line 1 should have a delivered qty equals to the sum of its linked timesheets",
        )
        self.assertEqual(
            timesheet2.unit_amount,
            line2.qty_delivered,
            "Sale line 2 should have a delivered qty equals to the sum of its linked timesheets",
        )
    ...
    def test_billing_employee_rate(self):
        ...
        self.assertEqual(
            ...
            "Subtask should have the same customer as the one from their mother",
        )
    ...
    def test_project_form_view(self):
        ...
        """
        with Form(
            self.env["project.project"].with_context({"tracking_disable": True})
        ) as project_form:
            ...
            project_form._view["modifiers"]["sale_line_employee_ids"][
                "invisible"
            ] = False
