...
class SlideChannelPartner(models.Model):
    ...
    def _recompute_completion(self):
        ...
        partner_has_completed = {
            channel_partner.partner_id.id: channel_partner.channel_id
        ...
        employees = (
            ...
            .sudo()
            .search([("user_id.partner_id", "in", list(partner_has_completed.keys()))])
        )

        if employees:
            HrResumeLine = self.env["hr.resume.line"].sudo()
            line_type = self.env.ref(
                "hr_skills_slides.resume_type_training", raise_if_not_found=False
            )
    ...
    def _send_completed_mail(self):
        ...
        for scp in self:
            if self.env.user.employee_ids:
                msg = _(
                    'The employee has completed the course <a href="%(link)s">%(course)s</a>',
...
class Channel(models.Model):
    ...
    def _action_add_members(self, target_partners, **member_values):
        ...
        for channel in self:
            channel._message_employee_chatter(
                _(
                    'The employee subscribed to the course <a href="%(link)s">%(course)s</a>',
    ...
    def _remove_membership(self, partner_ids):
        ...
        for channel in self:
            channel._message_employee_chatter(
                _(
                    'The employee left the course <a href="%(link)s">%(course)s</a>',
                    link=channel.website_url,
                    course=channel.name,
                ),
                partners,
