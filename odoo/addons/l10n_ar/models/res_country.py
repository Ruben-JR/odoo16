...
class ResCountry(models.Model):
    ...
    l10n_ar_afip_code = fields.Char(
        "AFIP Code", size=3, help="This code will be used on electronic invoice"
    )
    l10n_ar_natural_vat = fields.Char(
        "Natural Person VAT",
        size=11,
        help="Generic VAT number defined by AFIP in order to recognize partners from"
        " this country that are natural persons",
    )
    l10n_ar_legal_entity_vat = fields.Char(
        "Legal Entity VAT",
        size=11,
        help="Generic VAT number defined by AFIP in order to recognize partners from this"
