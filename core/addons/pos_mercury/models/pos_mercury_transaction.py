...
class MercuryTransaction(models.Model):
    ...
    def _get_pos_session(self):
        ...
        if not pos_session:
            raise UserError(
                _(
                    "No opened point of sale session for user %s found.",
                    self.env.user.name,
                )
            )
    ...
    def _setup_request(self, data):
        ...
        pos_mercury_config = self._get_pos_mercury_config_id(
            config, data["payment_method_id"]
        )
  ...
  <soapenv:Body>
    <mer:CreditTransaction>
      ...
      <mer:pw>{password}</mer:pw>
            ...
            transaction=str(self.env["ir.qweb"]._render(template, data)),
            password=data["merchant_pwd"],
        )
        ...
        if (
            ...
            .sudo()
            .get_param("pos_mercury.enable_test_env")
        ):
    ...
    def _gc_old_tokens(self):
        expired_creation_date = (date.today() - timedelta(days=6 * 30)).strftime(
