...
class ProductTemplate(models.Model):
    ...
    def _populate(self, size):
        ...
        def set_barcode_variant(sample_ratio):
            ...
            product_variants_ids = random.sample(
                product_variants_ids, int(len(product_variants_ids) * sample_ratio)
            )
    ...
    def _populate_factories(self):
        ...
        def get_attributes(values, counter, random):
            ...
            if (
                random.random() < 0.20
            ):  # 20 % chance of using only always attributes (to test when product has lot of variant)
            ...
            def will_exceed(i):
                return (
                    not no_variant
                    and reduce(
                        (lambda x, y: (x or 1) * (y or 1)),
                        values_count[i:] + [values_count[i] + 1] + values_count[:i],
                    )
                    > 1000
                )
        ...
        return [
            ...
            (
                "description",
                populate.constant("product_template_description_{counter}"),
            ),
    ...
    def _populate_factories(self):
        ...
        def get_value_ids(values, counter, random):
            attr_val = (
                ...
                .browse(values["product_template_attribute_value_id"])
                .attribute_line_id
            )
            remaining_lines = (
                multi_values_attribute_lines_by_tmpl[values["product_tmpl_id"]]
                - attr_val
            )
            return [
                (
                    ...
                    6,
                    0,
                    [random.choice(remaining_lines.product_template_value_ids).id],
                )
            ]
