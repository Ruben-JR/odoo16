...
class IrModuleModule(models.Model):
    ...
    def button_choose_theme(self):
        ...
        if result.get("tag") == "website_preview" and result.get("context", {}).get(
            "params", {}
        ).get("enable_editor"):
    ...
    def _load_module_terms(self, modules, langs, overwrite=False):
        ...
        for generic_arch_db, specific_arch_db, specific_id in self.env.cr.fetchall():
            ...
            specific_arch_db_update = {
                k: specific_arch_db.get(k, specific_arch_db_en) for k in langs_update
            }
            ...
            cache.update_raw(
                View.browse(specific_id), field, [specific_arch_db], dirty=True
            )
        ...
        o_menu_name = [
            f"'{lang}', o_menu.name->>'{lang}'" for lang in langs if lang != "en_US"
        ]
