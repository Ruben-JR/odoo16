...
class TestUBLCommon(AccountEdiTestCommon):
    ...
    def _assert_imported_invoice_from_etree(self, invoice, attachment):
        ...
        new_invoice = self.edi_format.with_context(
    ...
    def _assert_imported_invoice_from_file(
        ...
        amount_tax,
        list_line_subtotals,
        list_line_price_unit=None,
        list_line_discount=None,
        list_line_taxes=None,
        ...
        self.update_invoice_from_file(
            "l10n_account_edi_ubl_cii_tests", subfolder, filename, invoice
        )
        ...
        if list_line_price_unit:
            self.assertEqual(
                invoice.invoice_line_ids.mapped("price_unit"), list_line_price_unit
            )
        if list_line_discount:
            ...
            self.assertEqual(
                [round(d, dp) for d in invoice.invoice_line_ids.mapped("discount")],
                list_line_discount,
            )
        if list_line_taxes:
    ...
    def _assert_invoice_attachment(self, invoice, xpaths, expected_file):
        ...
        expected_file_path = get_resource_path(
            "l10n_account_edi_ubl_cii_tests", "tests/test_files", expected_file
        )
    ...
    def _import_invoice_attachment(self, invoice, edi_code, journal):
        ...
        edi_attachment = invoice.edi_document_ids.filtered(
            lambda doc: doc.edi_format_id.code == edi_code
        ).attachment_id
