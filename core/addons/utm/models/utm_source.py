...
class UtmSource(models.Model):
    ...
    def create(self, vals_list):
        new_names = self.env["utm.mixin"]._get_unique_names(
    ...
    def _generate_name(self, record, content):
        ...
        create_date = fields.date.strftime(
            create_date, tools.DEFAULT_SERVER_DATE_FORMAT
        )
...
class UtmSourceMixin(models.AbstractModel):
    ...
    def write(self, values):
        if values.get(self._rec_name) and not values.get("name"):
            values["name"] = self.env["utm.source"]._generate_name(
                self, values[self._rec_name]
            )
        if values.get("name"):
            values["name"] = self.env["utm.mixin"]._get_unique_names(
    ...
    def copy(self, default=None):
        ...
        default = default or {}
        default["name"] = self.env["utm.mixin"]._get_unique_names(
