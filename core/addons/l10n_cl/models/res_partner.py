...
class ResPartner(models.Model):
    ...
    def _format_vat_cl(self, values):
        ...
        partner_country_is_chile = (
            country.code == "CL" or identification_type.country_id.code == "CL"
        )
        if (
            partner_country_is_chile
            and values.get("l10n_latam_identification_type_id") in identification_types
            and values.get("vat")
        ):
            return (
    ...
    def write(self, values):
        if any(
            field in values
            for field in ["vat", "l10n_latam_identification_type_id", "country_id"]
        ):
            for record in self:
                vat_values = {
