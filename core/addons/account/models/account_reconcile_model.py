...
class AccountReconcileModelLine(models.Model):
    ...
    model_id = fields.Many2one(
        "account.reconcile.model", readonly=True, ondelete="cascade"
    )
    ...
    * From Label: There is no need for regex delimiter, only the regex is needed. For instance if you want to extract the amount from\nR:9672938 10/07 AX 9415126318 T:5L:NA BRT: 3358,07 C:\nYou could enter\nBRT: ([\d,]+)""",
    ...
    tax_ids = fields.Many2many(
        "account.tax", string="Taxes", ondelete="restrict", check_company=True
    )
    ...
    def _apply_in_manual_widget(self, residual_amount_currency, partner, currency):
        ...
        if self.amount_type == "percentage":
            ...
            raise UserError(
                _(
                    "This reconciliation model can't be used in the manual reconciliation widget because its "
    ...
    def _apply_in_bank_widget(self, residual_amount_currency, partner, st_line):
        ...
        if self.amount_type == "percentage_st_line":
            ...
            if match:
                ...
                try:
                    extracted_match_group = re.sub(
                        r"[^\d" + decimal_separator + "]", "", match.group(1)
                    )
                    extracted_balance = float(
                        extracted_match_group.replace(decimal_separator, ".")
                    )
                    amount_currency = copysign(
        ...
        if amount_currency is None:
            aml_vals = self._apply_in_manual_widget(
                residual_amount_currency, partner, currency
            )
        else:
            aml_vals = {
                **self._prepare_aml_vals(partner),
...
class AccountReconcileModel(models.Model):
    ...
    match_transaction_type_param = fields.Char(
    ...
    def _compute_number_entries(self):
        ...
        mapped_data = dict(
            [(d["reconcile_model_id"][0], d["reconcile_model_id_count"]) for d in data]
        )
        for model in self:
    ...
    def _compute_show_decimal_separator(self):
        for record in self:
            record.show_decimal_separator = any(
                l.amount_type == "regex" for l in record.line_ids
            )
    ...
    def _compute_payment_tolerance_param(self):
        for record in self:
            if record.payment_tolerance_type == "percentage":
                record.payment_tolerance_param = min(
                    100.0, max(0.0, record.payment_tolerance_param)
                )
            else:
                record.payment_tolerance_param = max(
                    0.0, record.payment_tolerance_param
                )

    @api.constrains(
        "allow_payment_tolerance", "payment_tolerance_param", "payment_tolerance_type"
    )
    def _check_payment_tolerance_param(self):
        for record in self:
            if record.allow_payment_tolerance:
                if (
                    record.payment_tolerance_type == "percentage"
                    and not 0 <= record.payment_tolerance_param <= 100
                ):
                    raise ValidationError(
                        _(
                            "A payment tolerance defined as a percentage should always be between 0 and 100"
                        )
                    )
                elif (
                    record.payment_tolerance_type == "fixed_amount"
                    and record.payment_tolerance_param < 0
                ):
                    raise ValidationError(
                        _(
    ...
    def copy(self, default=None):
        ...
        while self.env["account.reconcile.model"].search(
            [("name", "=", name)], limit=1
        ):
            name = _("%s (copy)", name)
        default["name"] = name
