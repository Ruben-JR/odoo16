...
class DeliveryCarrier(models.Model):
    ...
    delivery_type = fields.Selection(
        selection_add=[("onsite", "Pickup in store")],
    ...
    def _check_warehouse_company(self):
        for carrier in self:
            if (
                carrier.warehouse_id.company_id
                and carrier.company_id
                and carrier.company_id != carrier.warehouse_id.company_id
            ):
                raise ValidationError(
                    _("The picking site and warehouse must share the same company")
                )
