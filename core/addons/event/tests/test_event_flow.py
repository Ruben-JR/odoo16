...
class TestEventFlow(EventCase):
    ...
    def test_event_auto_confirm(self):
        ...
        self.assertEqual(
            test_reg1.state, "open", "Event: auto_confirmation of registration failed"
        )
        self.assertEqual(
            test_event.seats_reserved,
            1,
            "Event: wrong number of reserved seats after confirmed registration",
        )
        test_reg2 = (
            self.env["event.registration"]
            .with_user(self.user_eventuser)
        ...
        self.assertEqual(
            test_reg2.state, "open", "Event: auto_confirmation of registration failed"
        )
        self.assertEqual(
            test_event.seats_reserved,
            2,
        ...
        self.assertEqual(
            test_reg1.state, "done", "Event: wrong state of attended registration"
        )
        self.assertEqual(
            test_event.seats_used,
            1,
            "Event: incorrect number of attendees after closing registration",
        )
    ...
    def test_event_default_datetime(self):
        ...
        self.assertEqual(
            default_event.date_begin,
            datetime.datetime.strptime("2022-09-05 15:30:00", "%Y-%m-%d %H:%M:%S"),
        )
        self.assertEqual(
            default_event.date_end,
            datetime.datetime.strptime("2022-09-06 15:30:00", "%Y-%m-%d %H:%M:%S"),
        )
