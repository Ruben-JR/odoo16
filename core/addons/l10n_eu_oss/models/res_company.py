...
class Company(models.Model):
    ...
    def _map_eu_taxes(self):
        ...
        for company in self:
            ...
            for destination_country in oss_countries:
                ...
                foreign_taxes = {
                    tax.amount: tax
                    for tax in fpos.tax_ids.tax_dest_id
                ...
                for domestic_tax in taxes:
                    ...
                    if tax_amount and domestic_tax not in fpos.tax_ids.tax_src_id:
                        if not foreign_taxes.get(tax_amount, False):
                            oss_tax_group_local_xml_id = (
                                f"oss_tax_group_{str(tax_amount).replace('.', '_')}"
                            )
                            if not self.env.ref(
                                f"l10n_eu_oss.{oss_tax_group_local_xml_id}",
                        ...
                        mapping.append(
                            (
                                ...
                                0,
                                {
                                    ...
                                    "tax_dest_id": foreign_taxes[tax_amount].id,
                                },
                            )
                        )
                if mapping:
                    fpos.write({"tax_ids": mapping})
    ...
    def _get_repartition_lines_oss(self):
        ...
        for doc_type in "invoice", "refund":
            if oss_account:
                defaults[f"{doc_type}_repartition_line_ids"][tax_line][vals][
                    "account_id"
                ] = oss_account.id
            if oss_tags:
                defaults[f"{doc_type}_repartition_line_ids"][base_line][vals][
                    "tag_ids"
                ] += [Command.link(tag.id) for tag in oss_tags[f"{doc_type}_base_tag"]]
                defaults[f"{doc_type}_repartition_line_ids"][tax_line][vals][
                    "tag_ids"
                ] += [Command.link(tag.id) for tag in oss_tags[f"{doc_type}_tax_tag"]]
        return (
    ...
    def _get_oss_account(self):
        self.ensure_one()
        if not self.env.ref(
            f"l10n_eu_oss.oss_tax_account_company_{self.id}", raise_if_not_found=False
        ):
            sales_tax_accounts = (
                self.env["account.tax"]
                .search([("type_tax_use", "=", "sale"), ("company_id", "=", self.id)])
            ...
            new_code = self.env["account.account"]._search_new_account_code(
                self, len(sales_tax_accounts[0].code), sales_tax_accounts[0].code[:-2]
            )
    ...
    def _get_oss_tags(self):
        ...
        [chart_template_xml_id] = (
            self.chart_template_id.parent_id.get_external_id().values()
            or self.chart_template_id.get_external_id().values()
        )
