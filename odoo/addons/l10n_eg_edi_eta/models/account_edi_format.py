...
class AccountEdiFormat(models.Model):
    ...
    def _l10n_eg_get_eta_qr_domain(self, production_enviroment=False):
        return (
            ...
            and ETA_DOMAINS["invoice.production"]
    ...
    def _l10n_eg_get_eta_token_domain(self, production_enviroment=False):
        return (
            production_enviroment
            and ETA_DOMAINS["token.production"]
    ...
    def _l10n_eg_eta_connect_to_server(
        ...
        if not request_response.ok:
            ...
            return {"error": request_response.reason, "blocking_level": "error"}
    ...
    def _l10n_eg_eta_get_access_token(self, invoice):
        ...
        if response_data.get("error"):
            return response_data
        return {
            "access_token": response_data.get("response").json().get("access_token")
        }
            ...
            lines.append(
                {
                    ...
                    "taxableItems": [
                        {
                            "taxType": tax["tax_repartition_line"]
                            .tax_id.l10n_eg_eta_code.split("_")[0]
                            .upper()
                            .upper(),
                            "amount": self._l10n_eg_edi_round(abs(tax["tax_amount"])),
                            "subType": tax["tax_repartition_line"]
                            .tax_id.l10n_eg_eta_code.split("_")[1]
                            .upper(),
                            "rate": abs(tax["tax_repartition_line"].tax_id.amount),
                        }
                        for tax_details in line_tax_details.get(
                            "tax_details", {}
                        ).values()
                        for tax in tax_details.get("group_tax_details")
                    ],
                    "salesTotal": price_subtotal_before_discount,
                    "netTotal": self._l10n_eg_edi_round(abs(line.balance)),
                    "total": self._l10n_eg_edi_round(
                        abs(line.balance) + line_tax_details.get("tax_amount", 0.0)
                    ),
                }
            )
            totals["discount_total"] += discount_amount
            totals[
                "total_price_subtotal_before_discount"
            ] += price_subtotal_before_discount
