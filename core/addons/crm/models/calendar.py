...
class CalendarEvent(models.Model):
    ...
    def create(self, vals):
        ...
        for event in events:
            if event.opportunity_id and not event.activity_ids:
                event.opportunity_id.log_meeting(
    ...
    def _is_crm_lead(self, defaults, ctx=None):
        ...
        """
        res_model = (
            defaults.get("res_model", False) or ctx and ctx.get("default_res_model")
        )
        res_model_id = (
            defaults.get("res_model_id", False)
            or ctx
            and ctx.get("default_res_model_id")
        )

        return (
            res_model
            and res_model == "crm.lead"
            or res_model_id
