...
class PayuLatamController(http.Controller):
    ...
    def payulatam_return_from_checkout(self, **data):
        ...
        """
        _logger.info(
            "handling redirection from PayU Latam with data:\n%s", pprint.pformat(data)
        )
    ...
    def payulatam_webhook(self, **raw_data):
        ...
        try:
            ...
            tx_sudo = (
                request.env["payment.transaction"]
                .sudo()
                .with_context(payulatam_is_confirmation_page=True)
                ._get_tx_from_notification_data("payulatam", data)
            )
    ...
    def _verify_notification_signature(notification_data, tx_sudo):
        ...
        expected_signature = tx_sudo.provider_id._payulatam_generate_sign(
            notification_data
        )
