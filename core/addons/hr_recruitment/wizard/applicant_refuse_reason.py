...
class ApplicantGetRefuseReason(models.TransientModel):
    ...
    refuse_reason_id = fields.Many2one(
        "hr.applicant.refuse.reason", "Refuse Reason", required=True
    )
    applicant_ids = fields.Many2many("hr.applicant")
    send_mail = fields.Boolean(
        "Send Email", compute="_compute_send_mail", store=True, readonly=False
    )
        ...
        for wizard in self:
            ...
            if applicants and wizard.send_mail:
                wizard.applicant_without_email = "%s\n%s" % (
                    _(
                        "The email will not be sent to the following applicant(s) as they don't have email address."
                    ),
    ...
    def action_refuse_reason_apply(self):
        if self.send_mail:
            ...
            if not self.applicant_ids.filtered(
                lambda x: x.email_from or x.partner_id.email
            ):
                raise UserError(
                    _("Email of the applicant is not set, email won't be sent.")
                )
        self.applicant_ids.write(
            {"refuse_reason_id": self.refuse_reason_id.id, "active": False}
        )
        if self.send_mail:
