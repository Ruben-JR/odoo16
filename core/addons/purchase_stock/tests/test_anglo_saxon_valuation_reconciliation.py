...
class TestValuationReconciliation(ValuationReconciliationTestCommon):
    ...
    def _create_purchase(
        self, product, date, quantity=1.0, set_tax=False, price_unit=66.0
    ):
    ...
    def test_invoice_shipment(self):
        ...
        self.assertEqual(
            refund_invoice.payment_state, "paid", "Refund should be in 'paid' state"
        )
    ...
    def test_multiple_shipments_invoices(self):
        ...
        purchase_order = self._create_purchase(
            test_product, date_po_and_delivery0, quantity=5.0
        )
        self._process_pickings(
            purchase_order.picking_ids, quantity=2.0, date=date_po_and_delivery0
        )
        picking = self.env["stock.picking"].search(
        ...
        picking = self.env["stock.picking"].search(
            [("purchase_id", "=", purchase_order.id)], order="id desc", limit=1
        )
    ...
    def test_rounding_discount(self):
        ...
        purchase_order = self._create_purchase(
            test_product, date_po_and_delivery, quantity=10000, set_tax=True
        )
        self._process_pickings(purchase_order.picking_ids, date=date_po_and_delivery)
        ...
        invoice_layer = self.env["stock.valuation.layer"].search(
            [("account_move_line_id", "in", invoice.line_ids.ids)]
        )
        self.assertTrue(
            len(invoice_layer) == 1, "A price difference line should be created"
        )
        ...
        self.assertAlmostEqual(
            invoice_layer.value + picking.move_ids.stock_valuation_layer_ids.value,
            invoice.line_ids[0].debit,
        )
        self.assertAlmostEqual(
            invoice_layer.value + picking.move_ids.stock_valuation_layer_ids.value,
            invoice.invoice_line_ids.price_subtotal / 2,
            2,
        )
        self.check_reconciliation(invoice, picking)
    ...
    def test_rounding_price_unit(self):
        ...
        purchase_order = self._create_purchase(
            test_product, date_po_and_delivery, quantity=1000000, price_unit=0.0005
        )
        self._process_pickings(purchase_order.picking_ids, date=date_po_and_delivery)
    ...
    def test_price_difference_exchange_difference_accounting_date(self):
        ...
        purchase_order = self._create_purchase(
            test_product, date_po_receipt, quantity=10, price_unit=3000
        )
        ...
        self.assertTrue(
            len(price_diff_line) == 1, "A price difference line should be created"
        )
        ...
        self.assertTrue(
            ...
            "The reconciliation should be total at that point.",
        )
    ...
    def test_reconcile_cash_basis_bill(self):
        ...
        self.company_data[
            "company"
        ].account_cash_basis_base_account_id = cash_basis_base_account
        ...
        date_po_and_delivery = "2018-01-01"
        purchase_order = self._create_purchase(
            product_A, date_po_and_delivery, set_tax=True, price_unit=300.0
        )
        self._process_pickings(purchase_order.picking_ids, date=date_po_and_delivery)
    ...
    def test_reconciliation_differed_billing(self):
        ...
        self.assertRecordValues(
            pol, [{"qty_invoiced": line.qty_received} for line in pol]
        )
