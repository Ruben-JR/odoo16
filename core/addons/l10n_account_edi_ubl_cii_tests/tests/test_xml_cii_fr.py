...
class TestCIIFR(TestUBLCommon):
    ...
    def test_import_partner_facturx(self):
        ...
        * Uploading the attachment as a vendor bill should create a bill with the vendor = partner_1.
        ...
        new_invoice = self._import_invoice_attachment(
            invoice, "facturx_1_0_05", self.company_data["default_journal_sale"]
        )
        self.assertEqual(self.partner_2, new_invoice.partner_id)

        new_invoice = self._import_invoice_attachment(
            invoice, "facturx_1_0_05", self.company_data["default_journal_purchase"]
        )
    ...
    def test_import_journal_facturx(self):
        ...
        edi_attachment = invoice._get_edi_attachment(
            self.env.ref("account_edi_ubl_cii.edi_facturx_1_0_05")
        ).id
    ...
    def test_import_fnfe_examples(self):
        ...
        self._assert_imported_invoice_from_file(
            ...
            amount_total=108,
            amount_tax=8,
            list_line_subtotals=[-5, 10, 60, 28, 7],
        )
