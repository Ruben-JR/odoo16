...
class Twitter(http.Controller):
    ...
    @http.route(
        ["/website_twitter/get_favorites"], type="json", auth="public", website=True
    )
    def get_tweets(self, limit=20):
        ...
        debug = request.env["res.users"].has_group(
        ...
        if not key or not secret:
            if debug:
                return {
                    "error": _(
                        "Please set the Twitter API Key and Secret in the Website Settings."
                    )
                }
            return []
        if not screen_name:
            if debug:
                return {
                    "error": _(
                        "Please set a Twitter screen name to load favorites from, "
        ...
        if len(tweets) < 12:
            if debug:
                return {
                    "error": _(
                        "Twitter user @%(username)s has less than 12 favorite tweets. "
