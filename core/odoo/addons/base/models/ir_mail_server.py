...
class IrMailServer(models.Model):
    ...
    smtp_authentication = fields.Selection(
        [("login", "Username"), ("certificate", "SSL Certificate")],
    ...
    def _check_smtp_ssl_files(self):
        ...
        for mail_server in self:
            if (
                ...
                and not mail_server.smtp_ssl_private_key
            ):
                raise UserError(
                ...
                and not mail_server.smtp_ssl_certificate
            ):
                raise UserError(
        ...
        for server in self:
            ...
            if is_multiple_server_usage:
                usage_details.append(
                    _("%s (Dedicated Outgoing Mail Server):", server.display_name)
                )
    ...
    def _get_test_email_addresses(self):
        ...
        if self.from_filter:
            ...
            default_from = (
                ...
                .sudo()
                .get_param("mail.default.from", "odoo")
            )
        ...
        for server in self:
            ...
            try:
                ...
                if code != 250:
                    raise UserError(
                        _(
                            "The server refused the sender address (%(email_from)s) "
                            "with error %(repl)s"
                        )
                        % locals()
                    )
                ...
                if code not in (250, 251):
                    raise UserError(
                        _(
                            "The server refused the test recipient (%(email_to)s) "
                            "with error %(repl)s"
                        )
                        % locals()
                    )
                ...
                if code != 354:
                    raise UserError(
                        _(
                            "The server refused the test connection "
                            "with error %(repl)s"
                        )
                        % locals()
                    )
    ...
    def build_email(
        ...
        if subtype == "html" and not body_alternative:
            ...
            msg.add_alternative(
                tools.html2plaintext(email_body), subtype="plain", charset="utf-8"
            )
    ...
    def _prepare_email_message(self, message, smtp_session):
        ...
        if (
            notifications_email
            and smtp_from == notifications_email
            and message["From"] != notifications_email
        ):
    ...
    def _find_mail_server(self, email_from, mail_servers=None):
        ...
        mail_server = mail_servers.filtered(
            lambda m: email_normalize(m.from_filter) == email_from_normalized
        )
        if mail_server:
            return mail_server[0], email_from

        mail_server = mail_servers.filtered(
            lambda m: email_domain_normalize(m.from_filter) == email_from_domain
        )
        if mail_server:
            return mail_server[0], email_from
        ...
        if notifications_email:
            mail_server = mail_servers.filtered(
                lambda m: email_normalize(m.from_filter) == notifications_email
            )
            if mail_server:
                return mail_server[0], notifications_email

            mail_server = mail_servers.filtered(
                lambda m: email_domain_normalize(m.from_filter) == notifications_domain
            )
            if mail_server:
                return mail_server[0], notifications_email
    ...
    def _match_from_filter(self, email_from, from_filter):
        ...
        return email_domain_extract(normalized_mail_from) == email_domain_normalize(
            from_filter
        )
