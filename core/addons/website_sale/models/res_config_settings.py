...
class ResConfigSettings(models.TransientModel):
    ...
    cart_abandoned_delay = fields.Float(
        string="Send After", related="website_id.cart_abandoned_delay", readonly=False
    )
    ...
    terms_url = fields.Char(
        compute="_compute_terms_url",
        string="URL",
        help="A preview will be available at this URL.",
    )
    ...
    website_sale_contact_us_button_url = fields.Char(
        string="Button URL", related="website_id.contact_us_button_url", readonly=False
    )
    website_sale_enabled_portal_reorder_button = fields.Boolean(
        string="Re-order From Portal",
    ...
    def _compute_module_delivery(self):
        for wizard in self:
            wizard.module_delivery = wizard.sale_delivery_settings in [
                "internal",
                "website",
            ]
            wizard.module_website_sale_delivery = (
                wizard.sale_delivery_settings == "website"
            )
    ...
    def action_open_extra_info(self):
        ...
        return self.env["website"].get_client_action(
            "/shop/extra_info?open_editor=true", True, self.website_id.id
        )
