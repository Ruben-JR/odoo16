...
class TestProgramWithoutCodeOperations(TestSaleCouponCommon):
    ...
    def test_immediate_program_basic_operation(self):
        ...
        order.write(
            {"order_line": [(1, order.order_line[0].id, {"product_uom_qty": 2.0})]}
        )
        order._update_programs_and_rewards()
        self._claim_reward(order, self.immediate_promotion_program)
        self.assertEqual(
        ...
        order.write(
            {"order_line": [(1, order.order_line[0].id, {"product_uom_qty": 1.0})]}
        )
        order._update_programs_and_rewards()
        self._claim_reward(order, self.immediate_promotion_program)
        self.assertEqual(
        ...
        self.assertEqual(
            order.order_line[0].product_id.id,
            self.product_A.id,
            "The wrong line has been removed",
        )
        self.assertEqual(
            order.order_line[1].product_id.id,
            self.product_B.id,
            "The wrong line has been removed",
        )
