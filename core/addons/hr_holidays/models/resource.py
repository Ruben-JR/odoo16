...
class CalendarLeaves(models.Model):
            ...
            if not record.resource_id:
                ...
                if record.calendar_id:
                    existing_leaves = existing_leaves.filtered(
                        lambda l: not l.calendar_id
                        or l.calendar_id == record.calendar_id
                    )
                if existing_leaves:
    ...
    def _get_domain(self, time_domain_dict):
        ...
        return expression.AND(
            [domain, [("state", "!=", "refuse"), ("active", "=", True)]]
        )
    ...
    def _reevaluate_leaves(self, time_domain_dict):
        ...
        for previous_duration, leave, state in zip(
            ...
            if (
                leave.number_of_days > previous_duration
        ...
        leaves_to_cancel._force_cancel(
            _("a new public holiday completely overrides this leave."),
            "mail.mt_comment",
        )
...
class ResourceCalendar(models.Model):
    ...
    associated_leaves_count = fields.Integer(
        "Leave Count", compute="_compute_associated_leaves_count"
    )

    def _compute_associated_leaves_count(self):
        leaves_read_group = self.env["resource.calendar.leaves"].read_group(
            [("resource_id", "=", False)], ["calendar_id"], ["calendar_id"]
        )
        result = dict(
            (
                data["calendar_id"][0] if data["calendar_id"] else "global",
                data["calendar_id_count"],
            )
            for data in leaves_read_group
        )
