...
class TestItEdiReverseCharge(TestItEdi):
    ...
    def setUpClass(cls):
        ...
        def repartition_lines(*lines):
            ...
            return [(5, 0, 0)] + [
                (0, 0, {**line._asdict(), "tag_ids": get_tag_ids(line[2])})
                for line in lines
            ]
        ...
        def product_lines(*lines):
            ...
            return [
                (0, 0, {**line[0], "name": line[1], "product_id": line[2]})
                for line in lines
            ]
        ...
        cls.purchase_tax_4p_already_in_italy = (
            ...
            .create(tax_data_4p_already_in_italy)
        )
        cls.line_tax_4p_already_in_italy = cls.standard_line.copy()
        cls.line_tax_4p_already_in_italy["tax_ids"] = [
            (6, 0, cls.purchase_tax_4p_already_in_italy.ids)
        ]
        ...
        cls.purchase_tax_22p = (
            cls.env["account.tax"].with_company(cls.company).create(tax_data_22p)
        )
        ...
        cls.sale_tax_0v = (
            cls.env["account.tax"].with_company(cls.company).create(tax_data_0v)
        )
        ...
        cls.reverse_charge_bill = (
            cls.env["account.move"].with_company(cls.company).create(bill_data)
        )
        ...
        bill_data_2 = {
            **bill_data,
            "invoice_line_ids": product_lines(
                ProductLine(cls.line_tax_22p, "Product A", product_a.id),
                ProductLine(
                    cls.line_tax_4p_already_in_italy,
                    "Product B, taxed 4% Already in Italy",
                    product_b.id,
                ),
            ),
        }
        cls.reverse_charge_bill_2 = (
            cls.env["account.move"].with_company(cls.company).create(bill_data_2)
        )
        cls.reverse_charge_refund = cls.reverse_charge_bill.with_company(
            cls.company
        )._reverse_moves(
            [
                {
        ...
        cls.reverse_charge_bill_san_marino = (
            cls.env["account.move"]
            .with_company(cls.company)
            .create(bill_data_san_marino)
        )
    ...
    def test_reverse_charge_invoice(self):
        self._test_invoice_with_sample_file(
            self.reverse_charge_invoice, "reverse_charge_invoice.xml"
        )

    def test_reverse_charge_bill(self):
        self._test_invoice_with_sample_file(
            self.reverse_charge_bill, "reverse_charge_bill.xml"
        )
