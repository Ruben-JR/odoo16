    ...
    * add tests for new mail.message and mail.thread fields;
    ...
    def test_message_sms_internals_body(self):
        ...
        self.assertSMSNotification(
            [{"partner": self.partner_1}], "Mega SMS\nTop moumoutte", messages
        )

    def test_message_sms_internals_resend_existingd(self):
        with self.with_user("employee"), self.mockSMSGateway(
            sim_error="wrong_number_format"
        ):
            test_record = self.env["mail.test.sms"].browse(self.test_record.id)
            messages = test_record._message_sms(
                self._test_body, partner_ids=self.partner_1.ids
            )
        ...
        with self.with_user("employee"), self.mockSMSGateway():
            test_record = self.env["mail.test.sms"].browse(self.test_record.id)
            test_record._notify_thread_by_sms(
                messages,
                [{"id": self.partner_1.id, "notif": "sms"}],
...
class TestSMSPostException(TestSMSCommon, TestSMSRecipients):
    ...
    def test_message_sms_crash_credit_single(self):
        with self.with_user("employee"), self.mockSMSGateway(
            nbr_t_error={self.partner_2.phone_get_sanitized_number(): "credit"}
        ):
            test_record = self.env["mail.test.sms"].browse(self.test_record.id)
            messages = test_record._message_sms(
                self._test_body,
    ...
    def test_message_sms_crash_unregistered_single(self):
        with self.with_user("employee"), self.mockSMSGateway(
            nbr_t_error={self.partner_2.phone_get_sanitized_number(): "unregistered"}
        ):
            test_record = self.env["mail.test.sms"].browse(self.test_record.id)
            messages = test_record._message_sms(
                self._test_body,
    ...
    def test_message_sms_crash_wrong_number_single(self):
        with self.with_user("employee"), self.mockSMSGateway(
            nbr_t_error={
                self.partner_2.phone_get_sanitized_number(): "wrong_number_format"
            }
        ):
            test_record = self.env["mail.test.sms"].browse(self.test_record.id)
            messages = test_record._message_sms(
