...
class MrpWorkcenter(models.Model):
    ...
    costs_hour = fields.Float(
        string="Cost per hour", help="Hourly processing cost.", default=0.0
    )
    ...
    workorder_ready_count = fields.Integer(
        "# Read Work Orders", compute="_compute_workorder_count"
    )
    ...
    def _compute_workorder_count(self):
        ...
        for res_group in res:
            result[res_group["workcenter_id"][0]][res_group["state"]] = res_group[
                "__count"
            ]
            if res_group["state"] in ("pending", "waiting", "ready", "progress"):
                result_duration_expected[res_group["workcenter_id"][0]] += res_group[
                    "duration_expected"
                ]
        for workcenter in self:
    ...
    def _compute_oee(self):
        for order in self:
            if order.productive_time:
                order.oee = round(
                    order.productive_time
                    * 100.0
                    / (order.productive_time + order.blocked_time),
                    2,
                )
            else:
                order.oee = 0.0
    ...
    def _get_expected_duration(self, product_id):
        ...
        return (
            self.time_start
            + self.time_stop
            + (capacity.time_start + capacity.time_stop if capacity else 0.0)
        )
...
class MrpWorkcenterProductivityLoss(models.Model):
    ...
    def _convert_to_duration(self, date_start, date_stop, workcenter=False):
        ...
        """
        if (
            (self.loss_type not in ("productive", "performance"))
            and workcenter
...
class MrpWorkcenterProductivity(models.Model):
        ...
        for blocktime in self:
            if blocktime.date_start and blocktime.date_end:
                blocktime.duration = blocktime.loss_id._convert_to_duration(
                    blocktime.date_start, blocktime.date_end, blocktime.workcenter_id
                )
            else:
    ...
    def _check_open_time_ids(self):
        for workorder in self.workorder_id:
            open_time_ids_by_user = self.env["mrp.workcenter.productivity"].read_group(
                [("id", "in", workorder.time_ids.ids), ("date_end", "=", False)],
                ["user_id", "open_time_ids_count:count(id)"],
                ["user_id"],
            )
...
class MrpWorkCenterCapacity(models.Model):
    ...
    time_start = fields.Float(
        "Setup Time (minutes)", help="Time in minutes for the setup."
    )
    time_stop = fields.Float(
        "Cleanup Time (minutes)", help="Time in minutes for the cleaning."
    )
