...
class AccountMove(models.Model):
    ...
    def _compute_l10n_in_edi_ewaybill_show_send_button(self):
        ...
        posted_moves = self.filtered(
            lambda x: x.is_invoice() and x.state == "posted" and x.country_code == "IN"
        )
        for move in posted_moves:
            already_sent = move.edi_document_ids.filtered(
                lambda x: x.edi_format_id == edi_format
                and x.state in ("sent", "to_cancel", "to_send")
            )
            if already_sent:
    ...
    def _compute_l10n_in_edi_ewaybill_direct(self):
        for move in self:
            base = self.env[
                "account.edi.format"
            ]._l10n_in_edi_ewaybill_base_irn_or_direct(move)
        ...
        for move in self:
            ...
            if existing_edi_document:
                if existing_edi_document.state in ("sent", "to_cancel"):
                    raise UserError(
                        _("E-waybill is already created") % "\n".join(errors)
                    )
