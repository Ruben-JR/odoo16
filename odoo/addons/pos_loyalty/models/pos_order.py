...
class PosOrder(models.Model):
    ...
    def confirm_coupon_programs(self, coupon_data):
        ...
        coupons_to_create = {
            k: v for k, v in coupon_data.items() if k < 0 and not v.get("giftCardId")
        }
        coupon_create_vals = [
            {
    ...
    def _add_activated_coupon_to_draft_orders(self, table_orders):
        ...
        for order in table_orders:
            ...
            rewards_list = [
                ...
                if orderline[2]["is_reward_line"] and orderline[2]["reward_id"]
            ]

            order_reward_ids = self.env["loyalty.reward"].browse(
                set([reward_id["reward_id"] for reward_id in rewards_list])
            )

            for reward in rewards_list:
                order_reward_id = order_reward_ids.filtered(
                    lambda order_reward: order_reward.id == reward["reward_id"]
                )
