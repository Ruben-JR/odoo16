...
class TestEquipment(TransactionCase):
    ...
    def test_20_cron(self):
        ...
        maintenance_request_cron = self.maintenance_request.create(
            {
                ...
                "user_id": self.user.id,
                "request_date": (
                    datetime.datetime.now() + relativedelta.relativedelta(days=7)
                ).strftime("%Y-%m-%d"),
        ...
        tot_requests = self.maintenance_request.search(
            [("equipment_id", "=", equipment_cron.id)]
        )
        self.assertEqual(
            len(tot_requests),
            1,
            "The cron should have generated just 1 request for the High Maintenance Monitor.",
        )
    ...
    def test_21_cron(self):
        ...
        self.assertEqual(
            tot_requests.maintenance_team_id.id,
            team_test.id,
            "The maintenance team should be the same as equipment one",
        )
        self.assertEqual(
            tot_requests.duration,
            3.0,
            "Equipement maintenance duration is not the same as the request one",
        )
