...
class AdvancedFollowersTest(TestMailCommon):
    ...
    def test_auto_subscribe_post(self):
        ...
        self.assertEqual(
            self.test_track.message_partner_ids,
            self.user_employee.partner_id | self.user_admin.partner_id,
        )
    ...
    def test_auto_subscribe_post_email(self):
        ...
        self.test_track.with_user(self.user_admin).message_post(
            body="Coucou hibou", message_type="email"
        )
        self.assertEqual(
            self.test_track.message_partner_ids,
            self.user_employee.partner_id | self.user_admin.partner_id,
    ...
    def test_auto_subscribe_not_on_notification(self):
        ...
        self.test_track.with_user(self.user_admin).message_post(
            body="Coucou hibou", message_type="notification"
        )
        self.assertEqual(
            self.test_track.message_partner_ids, self.user_employee.partner_id
        )
...
class RecipientsNotificationTest(TestMailCommon):
    ...
    def test_recipients_fetch(self):
        ...
        recipients_data = self.env["mail.followers"]._get_recipient_data(
            self.env["mail.thread"], False, False, pids=test_partners.ids
        )
