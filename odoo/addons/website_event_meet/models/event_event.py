...
class Event(models.Model):
    ...
    meeting_room_ids = fields.One2many(
        "event.meeting.room", "event_id", string="Meeting rooms"
    )
    ...
    meeting_room_allow_creation = fields.Boolean(
        "Allow Room Creation",
        compute="_compute_meeting_room_allow_creation",
        readonly=False,
        store=True,
    ...
    def _compute_meeting_room_allow_creation(self):
        for event in self:
            if (
                ...
                event.meeting_room_allow_creation = (
                    event.event_type_id.meeting_room_allow_creation
                )
            elif (
                event.community_menu
                and event.community_menu != event._origin.community_menu
            ):
