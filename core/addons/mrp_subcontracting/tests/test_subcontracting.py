...
class TestSubcontractingFlows(TestMrpSubcontractingCommon):
    def test_flow_1(self):
        ...
        self.assertTrue(
            all(
                ...
                for m in picking_receipt.move_ids
            )
        )
    ...
    def test_flow_2(self):
        ...
        self.assertEqual(
            picking_receipt.scheduled_date,
            picking.scheduled_date + relativedelta(days=self.finished.produce_delay),
        )
    ...
    def test_flow_8(self):
        ...
        backorder_wiz = Form(
            self.env[backorder_wiz["res_model"]].with_context(backorder_wiz["context"])
        ).save()
        ...
        mo_done = backorder.move_ids.move_orig_ids.production_id.filtered(
    ...
    def test_flow_flexible_bom_1(self):
        ...
        picking_receipt.action_confirm()
        self.assertEqual(
            picking_receipt.display_action_record_components, "facultative"
        )
    ...
    def test_subcontracting_rules_replication(self):
        ...
        warehouse_related_rules = reference_location_rules.filtered(
            lambda r: r.warehouse_id
        )
        ...
        warehouse = self.env["stock.warehouse"].create(
            {"name": "Additional Warehouse", "code": "ADD"}
        )
        company_subcontracting_locations_rules_count = self.env[
            "stock.rule"
        ].search_count(
            [
                "&",
        ...
        self.assertEqual(
            len(warehouse_related_rules) * 4 + len(company_rules) * 2,
...
class TestSubcontractingTracking(TransactionCase):
    ...
    def test_flow_tracked_only_finished(self):
        ...
        move_details = Form(
            picking_receipt.move_ids, view="stock.view_stock_move_nosuggest_operations"
        )
        ...
        self.assertEqual(
            set(mos.lot_producing_id.mapped("name")), set(lot_names_finished)
        )
    ...
    def test_flow_tracked_backorder02(self):
        ...
        for qty in [3, 1]:
            ...
            if qty == 3:
                wizard = Form(
                    self.env[wizard_data["res_model"]].with_context(
                        wizard_data["context"]
                    )
                ).save()
                wizard.process()
    ...
    def test_flow_backorder_production(self):
        ...
        self.env["stock.quant"]._update_available_quantity(
            component, self.env.ref("stock.stock_location_stock"), todo_nb
        )
...
class TestSubcontractingSerialMassReceipt(TransactionCase):
    def setUp(self):
        ...
        self.finished = self.env["product.product"].create(
            {"name": "Finished", "type": "product", "tracking": "serial"}
        )
    ...
    def test_receive_after_resupply(self):
        ...
        for quantity in quantities:
            ...
            Form(
                self.env["stock.assign.serial"].with_context(
                    ...
                    default_next_serial_number=self.env["stock.lot"]._get_next_serial(
                        ...
                        picking_receipt.move_ids[0].product_id,
                    )
                    or "sn#1",
