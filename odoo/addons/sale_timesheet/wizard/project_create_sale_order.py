...
class ProjectCreateSalesOrder(models.TransientModel):
    ...
    def default_get(self, fields):
        ...
        if "project_id" in fields and active_id:
            ...
            if project.pricing_type != "task_rate" and not result.get(
                "line_ids", False
            ):
                    ...
                    result["line_ids"] = [
                        (
                            ...
                            0,
                            {
                                ...
                                "product_id": e.timesheet_product_id.id
                                or default_product.id,
                                "price_unit": e.price_unit
                                if e.timesheet_product_id
                                else default_product.lst_price,
                            },
                        )
                        for e in project.sale_line_employee_ids
                    ]
                    employee_from_timesheet = (
                        project.task_ids.timesheet_ids.employee_id
                        - project.sale_line_employee_ids.employee_id
                    )
                    result["line_ids"] += [
                        (
                            0,
    ...
    line_ids = fields.One2many(
        "project.create.sale.order.line", "wizard_id", string="Lines"
    )
    ...
    def _compute_info_invoice(self):
        for line in self:
            ...
            timesheet = self.env["account.analytic.line"].read_group(
                domain
                + [
                    ("task_id", "in", line.project_id.tasks.ids),
                    ("so_line", "=", False),
    ...
    def action_create_sale_order(self):
        ...
        if self.line_ids.employee_id:
            ...
            timesheet_employees = (
                ...
                .search([("task_id", "in", self.project_id.tasks.ids)])
                .mapped("employee_id")
            )
    ...
    def _make_billable(self, sale_order):
        if (
            not self.line_ids.employee_id
        ):  # Then we configure the project with pricing type is equal to project rate
    ...
    def _make_billable_at_project_rate(self, sale_order):
        ...
        for wizard_line in self.line_ids:
            task_ids = self.project_id.tasks.filtered(
                ...
                and task.timesheet_product_id == wizard_line.product_id
            )
            task_left -= task_ids
            ...
            self.env.remove_to_compute(
                self.env["sale.order.line"]._fields["price_unit"], sale_order_line
            )
    ...
    def _make_billable_at_employee_rate(self, sale_order):
        ...
        lines_already_present = dict(
            [(l.employee_id.id, l) for l in self.project_id.sale_line_employee_ids]
        )
        ...
        for map_entry in map_entries:
            ...
            # Avoid recomputing price_unit
            self.env.remove_to_compute(
                self.env["sale.order.line"]._fields["price_unit"],
...
class ProjectCreateSalesOrderLine(models.TransientModel):
    ...
    _sql_constraints = [
        (
            "unique_employee_per_wizard",
            "UNIQUE(wizard_id, employee_id)",
