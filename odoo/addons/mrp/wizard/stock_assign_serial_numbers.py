...
class StockAssignSerialNumbers(models.TransientModel):
    ...
    show_backorders = (
        fields.Boolean()
    )  # Technical field to show the Create Backorder and No Backorder buttons
    multiple_lot_components_names = (
        fields.Text()
    )  # Names of components with multiple lots, used to show warning
        ...
        if self.next_serial_number and self.next_serial_count:
            generated_serial_numbers = "\n".join(
                self.env["stock.lot"].generate_lot_names(
                    self.next_serial_number, self.next_serial_count
                )
            )
            self.serial_numbers = (
    ...
    def _onchange_serial_numbers(self):
        ...
        duplicate_serial_numbers = [
            serial_number
            for serial_number, counter in Counter(serial_numbers).items()
            if counter > 1
        ]
        if duplicate_serial_numbers:
            ...
            self.produced_qty = 0
            raise UserError(
                _("Duplicate Serial Numbers (%s)") % ",".join(duplicate_serial_numbers)
            )
        ...
        if len(serial_numbers) > self.expected_qty:
            ...
            self.produced_qty = 0
            raise UserError(
                _("There are more Serial Numbers than the Quantity to Produce")
            )
        self.produced_qty = len(serial_numbers)
        self.show_apply = self.produced_qty == self.expected_qty
        self.show_backorders = (
            self.produced_qty > 0 and self.produced_qty < self.expected_qty
        )

    def _assign_serial_numbers(self, cancel_remaining_quantity=False):
        serial_numbers = self._get_serial_numbers()
        productions = self.production_id._split_productions(
            {self.production_id: [1] * len(serial_numbers)},
            cancel_remaining_quantity,
