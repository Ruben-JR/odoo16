...
class test_challenge(TestGamificationCommon):
    ...
    def test_20_update_all_goals_filter(self):
        ...
        self.env["res.users.log"].search(
            [("create_uid", "in", challenge.user_ids.ids)]
        ).unlink()
        ...
        self.env["res.users.log"].create(
            [
                {
                    "create_uid": internal_login_before_update.id,
                    "create_date": now - datetime.timedelta(minutes=3),
                },
                {
                    "create_uid": portal_login_before_update.id,
                    "create_date": now - datetime.timedelta(minutes=3),
                },
            ]
        )
        ...
        self.env["res.users.log"].create(
            [
                {
                    "create_uid": internal_login_after_update.id,
                    "create_date": now + datetime.timedelta(minutes=3),
                },
                {
                    "create_uid": portal_login_after_update.id,
                    "create_date": now + datetime.timedelta(minutes=3),
                },
            ]
        )
