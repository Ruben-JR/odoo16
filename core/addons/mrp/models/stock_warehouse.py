...
class StockWarehouse(models.Model):
    ...
    manufacture_mto_pull_id = fields.Many2one("stock.rule", "Manufacture MTO Rule")
    pbm_mto_pull_id = fields.Many2one(
        "stock.rule", "Picking Before Manufacturing MTO Rule"
    )
    sam_rule_id = fields.Many2one("stock.rule", "Stock After Manufacturing Rule")
    ...
    pbm_route_id = fields.Many2one(
        "stock.route", "Picking Before Manufacturing Route", ondelete="restrict"
    )
    ...
    def _get_global_route_rules_values(self):
        ...
        location_src = (
            ...
            and self.lot_stock_id
            or self.pbm_loc_id
        )
        production_location = self._get_production_location()
        location_dest_id = (
            self.manufacture_steps == "pbm_sam" and self.sam_loc_id or self.lot_stock_id
        )
        rules.update(
            {
                "manufacture_pull_id": {
                    ...
                    "update_values": {
                        ...
                        "name": self._format_rulename(
                            location_dest_id, False, "Production"
                        ),
                ...
                "manufacture_mto_pull_id": {
                    ...
                    "update_values": {
                        "name": self._format_rulename(
                            location_src, production_location, "MTO"
                        ),
                        "active": self.manufacture_to_resupply,
                    },
                },
        ...
        values.update(
            {
                ...
                "sam_loc_id": {
                    ...
                    "usage": "internal",
                    "barcode": self._valid_barcode(
                        code + "-POSTPRODUCTION", company_id
                    ),
                },
            }
        )
        ...
        values.update(
            {
                "pbm_type_id": {
                    "name": self.name
                    + " "
                    + _("Sequence picking before manufacturing"),
                    "prefix": self.code + "/PC/",
                    "padding": 5,
                    "company_id": self.company_id.id,
                },
                "sam_type_id": {
                    "name": self.name + " " + _("Sequence stock after manufacturing"),
    ...
    def _get_all_routes(self):
        ...
        routes |= (
            self.filtered(
                lambda self: self.manufacture_to_resupply
...
class Orderpoint(models.Model):
    ...
    def _get_orderpoint_products(self):
        non_kit_ids = []
        for products in split_every(
            2000,
            super()._get_orderpoint_products().ids,
