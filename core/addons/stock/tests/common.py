...
class TestStockCommon(common.TransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.supplier_location = cls.ModelDataObj._xmlid_to_res_id(
            "stock.stock_location_suppliers"
        )
        cls.stock_location = cls.ModelDataObj._xmlid_to_res_id(
        ...
        cls.customer_location = cls.ModelDataObj._xmlid_to_res_id(
        ...
        cls.uom_kg = cls.env["uom.uom"].search(
            [("category_id", "=", cls.categ_kgm), ("uom_type", "=", "reference")],
            limit=1,
        )
        ...
        cls.uom_unit = cls.env["uom.uom"].search(
            [("category_id", "=", cls.categ_unit), ("uom_type", "=", "reference")],
            limit=1,
        )
        cls.uom_unit.write({"name": "Test-Unit", "rounding": 1.0})
