...
class IrActions(models.Model):
    ...
    binding_type = fields.Selection(
        [("action", "Action"), ("report", "Report")], required=True, default="action"
    )
...
class IrActionsActWindow(models.Model):
    ...
    def _check_model(self):
        for action in self:
            ...
            if (
                ...
                raise ValidationError(
                    _(
                        "Invalid model name %r in action definition.",
    ...
    def _check_view_mode(self):
        for rec in self:
            ...
            if len(modes) != len(set(modes)):
                raise ValidationError(
                    _("The modes in view_mode must not be duplicated: %s", modes)
                )
            if " " in modes:
    ...
    groups_id = fields.Many2many(
        "res.groups", "ir_act_window_group_rel", "act_id", "gid", string="Groups"
    )
...
# To return an action, assign: action = {...}\n\n\n\n"""
    ...
    usage = fields.Selection(
        [("ir_actions_server", "Server Action"), ("ir_cron", "Scheduled Action")],
        string="Usage",
    ...
    crud_model_id = fields.Many2one(
        "ir.model",
        string="Target Model",
        compute="_compute_crud_model_id",
        readonly=False,
        store=True,
    ...
    link_field_id = fields.Many2one(
        "ir.model.fields",
        string="Link Field",
        compute="_compute_link_field_id",
        readonly=False,
        store=True,
    ...
    groups_id = fields.Many2many(
        "res.groups", "ir_act_server_group_rel", "act_id", "gid", string="Groups"
    )
    ...
    def _get_runner(self):
        multi = True
        t = type(self)
        fn = getattr(t, f"_run_action_{self.state}_multi", None) or getattr(
            t, f"run_action_{self.state}_multi", None
        )
        if not fn:
            multi = False
            fn = getattr(t, f"_run_action_{self.state}", None) or getattr(
                t, f"run_action_{self.state}", None
            )
    ...
    def create_action(self):
        ...
        for action in self:
            action.write(
                {"binding_model_id": action.model_id.id, "binding_type": "action"}
            )
        return True
    ...
    def _run_action_object_write(self, eval_context=None):
        ...
        if self._context.get("onchange_self"):
            ...
            self.env[self.model_id.model].browse(self._context.get("active_id")).write(
                res
            )
...
class IrServerObjectLines(models.Model):
    ...
    def _compute_resource_ref(self):
        for line in self:
            if (
                line.evaluation_type in ["reference", "value"]
                and line.col1
    ...
    def _raise_many2many_error(self):
        if self.filtered(
            lambda line: line.col1.ttype == "many2many"
            and line.evaluation_type == "reference"
        ):
            raise ValidationError(
...
class IrActionsActClient(models.Model):
    ...
    res_model = fields.Char(
        string="Destination Model", help="Optional model, mostly used for needactions."
    )
    context = fields.Char(
        string="Context Value",
        default="{}",
