...
class Task(models.Model):
    ...
    def _compute_leave_types_count(self):
        ...
        time_off_type_count_per_task = {
            res["timesheet_task_id"][0]: res["timesheet_task_id_count"]
            for res in time_off_type_read_group
        }
        for task in self:
    ...
    def _search_is_timeoff_task(self, operator, value):
        ...
        timeoff_task_ids = (
            leave_type_read_group[0]["timesheet_task_ids"]
            if leave_type_read_group[0]["timesheet_task_ids"]
            else []
        )
