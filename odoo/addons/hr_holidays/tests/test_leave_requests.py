...
class TestLeaveRequests(TestHrHolidaysCommon):
    def _check_holidays_status(self, holiday_status, ml, lt, rl, vrl):
        self.assertEqual(
            holiday_status.max_leaves, ml, "hr_holidays: wrong type days computation"
        )
        self.assertEqual(
            holiday_status.leaves_taken, lt, "hr_holidays: wrong type days computation"
        )
        self.assertEqual(
            holiday_status.remaining_leaves,
            rl,
            "hr_holidays: wrong type days computation",
        )
        self.assertEqual(
            holiday_status.virtual_remaining_leaves,
            vrl,
            "hr_holidays: wrong type days computation",
        )
    ...
    def _check_holidays_count(
        self, holidays_count_result, ml, lt, rl, vrl, vlt, closest_allocation
    ):
        self.assertEqual(
    ...
    def test_timezone_company_validated(self):
        ...
        leave_form = Form(
            self.env["hr.leave"], view="hr_holidays.hr_leave_view_form_manager"
        )
    ...
    def _test_leave_with_tz(self, tz, local_date_from, local_date_to, number_of_days):
        ...
        values = {
            "date_from": tz.localize(local_date_from)
            .astimezone(UTC)
            .replace(tzinfo=None),
            "date_to": tz.localize(local_date_to)
            .astimezone(UTC)
            .replace(tzinfo=None),  # note that this can be the next day in UTC
        }
        values.update(
            self.env["hr.leave"]
            .with_user(self.user_employee_id)
            ._default_get_request_parameters(values)
        )
    ...
    def test_leave_defaults_with_timezones(self):
        ...
        for tz in timezones_to_test:
            self._test_leave_with_tz(
                tz, local_date_from, local_date_to, 6
            )  # TODO JUD check why this fails
    ...
    def test_several_allocations_split(self):
        ...
        for unit in ["hour", "day"]:
            ...
            allocation_days = self.holidays_type_2._get_employees_days_per_allocation(
                [self.employee_emp_id]
            )

            self.assertEqual(
                allocation_days[self.employee_emp_id][self.holidays_type_2][
                    allocation_4days
                ]["leaves_taken"],
    ...
    def test_holiday_type_requires_no_allocation(self):
        ...
        with freeze_time("2020-09-15"):
            ...
            self._check_holidays_count(
                self.holidays_type_2.get_employees_days([self.employee_emp_id])[
                    self.employee_emp_id
                ][self.holidays_type_2.id],
                ml=5,
                lt=0,
                rl=5,
                vrl=2,
                vlt=3,
                closest_allocation=allocation,
            )
    ...
    def test_archived_allocation(self):
        with freeze_time("2022-09-15"):
            ...
            self._check_holidays_count(
                self.holidays_type_2.get_employees_days(
                    [self.employee_emp_id], date=date(2021, 12, 1)
                )[self.employee_emp_id][self.holidays_type_2.id],
                ml=10,
                lt=5,
                rl=5,
                vrl=5,
                vlt=5,
                closest_allocation=allocation_2021,
            )

            # Virtual remaining leave is equal to 1 because there is only one day remaining in the allocation based on its validity
            self._check_holidays_count(
                self.holidays_type_2.get_employees_days(
                    [self.employee_emp_id], date=date(2021, 12, 31)
                )[self.employee_emp_id][self.holidays_type_2.id],
                ml=10,
                lt=5,
                rl=5,
                vrl=1,
                vlt=5,
                closest_allocation=allocation_2022,
            )
