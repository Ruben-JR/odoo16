...
class TestCRMLead(TestCrmCommon):
    ...
    def test_crm_lead_creation_partner_no_address(self):
        ...
        self.assertEqual(
            lead.mobile,
            empty_partner.mobile,
            "Mobile from partner should be set on the lead",
        )
        self.assertEqual(
            lead.title,
            empty_partner.title,
            "Title from partner should be set on the lead",
        )
        self.assertEqual(
            lead.function,
    ...
    def test_crm_lead_currency_sync(self):
        lead = self.env["crm.lead"].create(
            {"name": "Lead 1", "company_id": self.company_main.id}
        )
    ...
    def test_crm_lead_partner_sync_email_phone(self):
        ...
        with self.debug_mode():
            ...
            partner_phone, partner_mobile = (
                self.test_phone_data[2],
                self.test_phone_data[1],
            )
            ...
            partner_mobile_sanitized = phone_format(
                partner_mobile, "US", "1", force_format="E164"
            )
            ...
            self.assertEqual(
                partner_mobile_sanitized, self.test_phone_data_sanitized[1]
            )
            # ensure initial data
    ...
    def test_crm_lead_stages(self):
        ...
        self.assertEqual(
            lead.stage_id, self.stage_gen_won
        )  # generic won stage has lower sequence than team won stage
    ...
    def test_phone_mobile_update(self):
        ...
        lead.write(
            {"phone": self.test_phone_data[1], "mobile": self.test_phone_data[2]}
        )
