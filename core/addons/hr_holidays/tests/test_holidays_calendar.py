...
class TestHolidaysCalendar(HttpCase, TestHrHolidaysCommon):
    ...
    def test_hours_time_off_request_calendar_view(self):
        ...
        last_leave = (
            self.env["hr.leave"]
            .search([("employee_id.id", "=", self.env.user.employee_id.id)])
            .sorted(lambda leave: leave.create_date)[-1]
        )
        self.assertEqual(last_leave.date_from.weekday(), 3, "It should be Thursday")
        self.assertEqual(
            last_leave.date_from.hour, expected_start_thursday, "Wrong start of the day"
        )
        self.assertEqual(
            last_leave.date_to.hour, expected_end_thursday, "Wrong end of the day"
        )
