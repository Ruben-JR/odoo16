...
class Challenge(models.Model):
    ...
    def default_get(self, fields_list):
        ...
        if "user_domain" in fields_list and "user_domain" not in res:
            ...
            res[
                "user_domain"
            ] = f'["&", ("groups_id", "=", "{user_group_id.name}"), ("active", "=", True)]'
        return res
    ...
    reward_id = fields.Many2one(
        "gamification.badge", string="For Every Succeeding User"
    )
    ...
    def _get_serialized_challenge_lines(
        self, user=(), restrict_goals=(), restrict_top=0
    ):
        ...
        for challenge in self:
            ...
            if challenge_ended:
                ...
                if rewarded_users:
                    ...
                    message_body += _(
                        "<br/>Nobody has succeeded to reach every goal, no badge is rewarded for this challenge."
                    )
