...
class TestWebsiteSaleDeliveryExpressCheckoutFlows(HttpCaseWithUserDemo):
    ...
    def test_express_checkout_public_user_shipping_address_change(self):
        ...
        with patch(
            ...
            self._make_json_rpc_request(
                urls.url_join(
                    self.base_url(),
                    WebsiteSaleDeliveryController._express_checkout_shipping_route,
                ),
                data={
                    "partial_shipping_address": dict(
            ...
            for k in self.express_checkout_anonymized_shipping_values:
                if k in ["state", "country"]:
                    ...
                    continue
                self.assertEqual(
                    new_partner[k], self.express_checkout_anonymized_shipping_values[k]
                )
    ...
    def test_express_checkout_partial_delivery_address_context_key(self):
        ...
        self.assertTrue(
            sale_order._context.get("express_checkout_partial_delivery_address")
        )
    ...
    def test_express_checkout_registered_user_with_shipping_option(self):
        ...
        with patch(
            ...
            self._make_json_rpc_request(
                urls.url_join(self.base_url(), "/shop/express"),
                data={
