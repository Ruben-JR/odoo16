...
class SurveyQuestion(models.Model):
    ...
    description = fields.Html(
        "Description",
        translate=True,
        sanitize=False,  # TDE TODO: sanitize but find a way to keep youtube iframe media stuff
    ...
    scoring_type = fields.Selection(
        related="survey_id.scoring_type", string="Scoring Type", readonly=True
    )
    sequence = fields.Integer("Sequence", default=10)
    # page specific
    is_page = fields.Boolean("Is a page?")
    question_ids = fields.One2many(
        "survey.question", string="Questions", compute="_compute_question_ids"
    )
    ...
    page_id = fields.Many2one(
        "survey.question", string="Page", compute="_compute_page_id", store=True
    )
    ...
    triggering_answer_id = fields.Many2one(
        ...
        copy=False,
        compute="_compute_triggering_answer_id",
        store=True,
        readonly=False,
        help="Answer that will trigger the display of the current question.",
        ...
        for question in self:
            if question.is_page:
                ...
                question.question_ids = question.survey_id.question_ids.filtered(
                    lambda q: q._index() > question._index()
                    and (not next_page_index or q._index() < next_page_index)
                )
            else:
    ...
    def _compute_triggering_answer_id(self):
        ...
        for question in self:
            if (
                not question.triggering_question_id
                or question.triggering_question_id
                != question.triggering_answer_id.question_id
    ...
    def validate_question(self, answer, comment=None):
        ...
        if (
            ...
            and not answer
            and self.question_type not in ["simple_choice", "multiple_choice"]
        ):
            return {
...
class SurveyQuestionAnswer(models.Model):
    ...
    question_id = fields.Many2one(
        "survey.question", string="Question", ondelete="cascade"
    )
    matrix_question_id = fields.Many2one(
        "survey.question", string="Question (as matrix row)", ondelete="cascade"
    )
