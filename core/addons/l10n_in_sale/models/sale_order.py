...
class SaleOrder(models.Model):
    ...
    l10n_in_journal_id = fields.Many2one(
        "account.journal",
        string="Journal",
        compute="_compute_l10n_in_journal_id",
        store=True,
        readonly=True,
        states={"draft": [("readonly", False)], "sent": [("readonly", False)]},
    )
    ...
    def _compute_l10n_in_gst_treatment(self):
        for order in self:
            ...
            if order.country_code == "IN":
                ...
                if (
                    not l10n_in_gst_treatment
                    and order.partner_id.country_id
                    and order.partner_id.country_id.code != "IN"
                ):
                    l10n_in_gst_treatment = "overseas"
                if not l10n_in_gst_treatment:
                    l10n_in_gst_treatment = (
                        order.partner_id.vat and "regular" or "consumer"
                    )
