...
class TestRepair(AccountTestInvoicingCommon):
    ...
    def test_02_repair_noneinv(self):
        ...
        self.assertEqual(
            repair.move_id.location_id.id,
            self.stock_warehouse.lot_stock_id.id,
            "Repaired product was taken in the wrong location",
        )
        self.assertEqual(
            repair.move_id.location_dest_id.id,
            self.stock_warehouse.lot_stock_id.id,
            "Repaired product went to the wrong location",
        )
        self.assertEqual(
            repair.operations.move_id.location_id.id,
            self.stock_warehouse.lot_stock_id.id,
            "Consumed product was taken in the wrong location",
        )
        self.assertEqual(
            repair.operations.move_id.location_dest_id.id,
            self.product_product_5.property_stock_production.id,
        ...
        self.assertNotEqual(
            len(repair.invoice_id), 1, "Invoice should not exist for this repair order"
        )
    ...
    def test_repair_state(self):
        ...
        self.assertEqual(
            repair.state,
            "2binvoiced",
            "Repair order should be in 2binvoiced state, if invoice is deleted.",
        )
        repair.action_repair_invoice_create()
        repair.action_repair_cancel()
        # Repair order and linked invoice both should be cancelled.
        self.assertEqual(
            repair.state, "cancel", "Repair order should be in cancel state."
        )
        self.assertEqual(
            repair.invoice_id.state, "cancel", "Invoice should be in cancel state."
        )
        repair.action_repair_cancel_draft()
        # Linked invoice should be unlinked
        self.assertEqual(
            len(repair.invoice_id),
            0,
            "No invoice should be exists for this repair order",
        )
    ...
    def test_repair_return(self):
        ...
        self.assertEqual(
            len(return_picking.repair_ids),
            1,
            "A repair order should have been created and linked to original return.",
        )
    ...
    def test_repair_compute_product_uom(self):
        ...
        self.assertEqual(
            repair.operations[0].product_uom, self.product_product_11.uom_id
        )
    ...
    def test_repair_compute_location(self):
        ...
        self.assertEqual(
            repair.operations[0].location_id, self.stock_warehouse.lot_stock_id
        )
