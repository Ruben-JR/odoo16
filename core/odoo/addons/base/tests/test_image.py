...
class TestImage(TransactionCase):
    ...
    def test_00_base64_to_image(self):
        ...
        self.assertEqual(
            type(image), PngImagePlugin.PngImageFile, "base64 as bytes, correct format"
        )
    ...
    def _assertAlmostEqualSequence(self, rgb1, rgb2, delta=10):
        ...
        for index, t in enumerate(zip(rgb1, rgb2)):
            self.assertAlmostEqual(
                t[0], t[1], delta=delta, msg="%s vs %s at %d" % (rgb1, rgb2, index)
            )
    ...
    def test_ptype_image_to_jpeg(self):
        ...
        self.assertEqual(
            tools.image.image_apply_opt(image1, "JPEG"),
            tools.image.image_apply_opt(image2, "JPEG"),
        )
