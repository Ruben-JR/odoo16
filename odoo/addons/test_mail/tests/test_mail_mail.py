...
class TestMailMail(TestMailCommon):
    ...
    def setUpClass(cls):
        ...
        cls.test_message = cls.test_record.message_post(
            body="<p>Message</p>", subject="Subject"
        )
    ...
    def test_mail_mail_schedule(self):
        ...
        scheduled_datetimes = [
            ...
            now,
            pytz.timezone("Australia/Brisbane").localize(now),
            # string
        ...
        expected_datetimes = [
            ...
            False,
            now,
            now - pytz.timezone("Australia/Brisbane").utcoffset(now),
    ...
    def test_mail_mail_send_exceptions_origin(self):
        ...
        self.assertFalse(
            mail.failure_type, "Mail: void from: no failure type, should be updated"
        )
        ...
        self.assertEqual(
            ...
            "unknown",
            "Mail: void from: unknown failure type, should be updated",
        )
        ...
        self.assertEqual(
            mail.failure_reason,
            "Malformed 'Return-Path' or 'From' address: strange@example¢¡.com - It should contain one valid plain ASCII email",
        )
        self.assertFalse(
            mail.failure_type,
            "Mail: bugged from (ascii): no failure type, should be updated",
        )
        self.assertEqual(mail.state, "exception")
        self.assertEqual(
            notification.failure_reason,
        ...
        self.assertIn(
            "Malformed 'Return-Path' or 'From' address: bounce.test@domain¢¡.com",
            mail.failure_reason,
        )
        self.assertFalse(
            mail.failure_type,
            "Mail: bugged catchall domain (ascii): no failure type, should be updated",
        )
        self.assertEqual(mail.state, "exception")
        self.assertEqual(
            notification.failure_reason,
            "Malformed 'Return-Path' or 'From' address: bounce.test@domain¢¡.com - It should contain one valid plain ASCII email",
        )
        self.assertEqual(
            notification.failure_type,
            "unknown",
            "Mail: bugged catchall domain (ascii): unknown failure type, should be updated",
        )
        self.assertEqual(notification.notification_status, "exception")
        ...
        self.assertEqual(
            mail.failure_reason,
            "Malformed 'Return-Path' or 'From' address: robert - It should contain one valid plain ASCII email",
        )
        self.assertFalse(
            mail.failure_type,
            "Mail: bugged from (ascii): no failure type, should be updated",
    ...
    def test_mail_mail_send_exceptions_recipients_emails(self):
        ...
        for email_to in self.emails_falsy:
            ...
            self.assertFalse(
                mail.failure_type,
                "Mail: missing email_to: no failure type, should be updated",
            )
            self.assertEqual(mail.state, "exception")
            if email_to == " ":
                self.assertFalse(
                    notification.failure_reason, "Mail: failure reason not propagated"
                )
        ...
        for email_to, failure_type in zip(
            ...
            self.assertFalse(
                mail.failure_type,
                "Mail: invalid email_to: no failure type, should be updated",
            )
            self.assertEqual(mail.state, "exception")
            self.assertFalse(
                notification.failure_reason, "Mail: failure reason not propagated"
    ...
    def test_mail_mail_send_exceptions_recipients_partners(self):
        ...
        for partner in partners_falsy:
            ...
            self.assertFalse(
                mail.failure_type,
                "Mail: void recipient partner: no failure type, should be updated",
            )
            self.assertEqual(mail.state, "exception")
            self.assertFalse(
                notification.failure_reason, "Mail: failure reason not propagated"
        ...
        for partner in partners_invalid:
            ...
            self.assertFalse(
                mail.failure_type,
                "Mail: invalid recipient partner: no failure type, should be updated",
            )
            self.assertEqual(mail.state, "exception")
            self.assertFalse(
                notification.failure_reason, "Mail: failure reason not propagated"
    ...
    def test_mail_mail_send_exceptions_recipients_partners_mixed(self):
        ...
        for partner in partners_falsy + partners_invalid:
            ...
            self.assertFalse(
                notification.failure_reason, "Mail: void email considered as invalid"
            )
            self.assertEqual(
                notification.failure_type,
                "mail_email_invalid",
                "Mail: void email considered as invalid",
            )
            self.assertEqual(notification.notification_status, "exception")
    ...
    def test_mail_mail_send_exceptions_raise_management(self):
        ...
        with self.mock_mail_gateway():
            ...
            for error, msg in [
                ...
                self.assertEqual(mail.failure_reason, msg)
                self.assertFalse(
                    mail.failure_type, "Mail: unlogged failure type to fix"
                )
                self.assertEqual(mail.state, "exception")
                self.assertEqual(notification.failure_reason, msg)
                self.assertEqual(
                    notification.failure_type, "unknown", "Mail: generic failure type"
                )
                self.assertEqual(notification.notification_status, "exception")
    ...
    def test_mail_mail_send_server(self):
        ...
        self.connect_mocked.assert_has_calls(
            calls=[
                ...
                call(
                    ...
                    mail_server_id=self.server_domain_2.id,
                ),
                call(
                ...
                call(
                    smtp_from="user_1@test_2.com", mail_server_id=self.server_domain.id
                ),
            ],
            any_order=True,
        )
        ...
        self.assert_email_sent_smtp(
            message_from='"test_2" <notifications@test.com>',
            emails_count=5,
            from_filter=self.server_notification.from_filter,
        )
        self.assert_email_sent_smtp(
            ...
            emails_count=5,
            from_filter=self.server_domain_2.from_filter,
        )
        self.assert_email_sent_smtp(
        ...
        def send_email(self, message, *args, **kwargs):
            with this.registry.cursor() as cr, mute_logger("odoo.sql_db"):
                try:
                    ...
                    cr.execute(
                        "SELECT notification_status FROM mail_notification WHERE id = %s FOR UPDATE NOWAIT",
                        [notif.id],
                    )
