...
class QrInvoiceWizard(models.TransientModel):
    ...
    def default_get(self, fields):
        ...
        def determine_invoices_text(nb_inv, inv_format):
            ...
            return _(
                "%s invoices could be printed in the %s format.", nb_inv, inv_format
            )
        ...
        results.update(
            {
                ...
                "isr_inv_text": determine_invoices_text(
                    nb_inv=len(dispatched_invoices["isr"]), inv_format="ISR"
                ),
    ...
    def action_view_faulty_invoices(self):
        ...
        for inv in faulty_invoices:
            try:
                ...
                inv.partner_bank_id._eligible_for_qr_code(
                    "ch_qr", inv.partner_id, inv.currency_id, raises_error=True
                )
