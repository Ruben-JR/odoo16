...
class ResConfigSettings(models.TransientModel):
    ...
    allow_out_of_stock_order = fields.Boolean(
        string="Continue selling when out-of-stock", default=True
    )
    ...
    def get_values(self):
        ...
        IrDefault = self.env["ir.default"].sudo()
        allow_out_of_stock_order = IrDefault.get(
            "product.template", "allow_out_of_stock_order"
        )

        res.update(
            allow_out_of_stock_order=allow_out_of_stock_order
            if allow_out_of_stock_order is not None
            else True,
            available_threshold=IrDefault.get("product.template", "available_threshold")
            or 5.0,
            show_availability=IrDefault.get("product.template", "show_availability")
            or False,
        )
        return res
