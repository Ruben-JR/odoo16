...
class MicrosoftCalendarService:
    ...
    def _get_events_from_paginated_url(
        ...
        next_sync_token = (
            urls.url_parse(token_url).decode_query().get("$deltatoken", False)
            if token_url
            else None
        )
    ...
    def _get_events_delta(self, sync_token=None, token=None, timeout=TIMEOUT):
        ...
        try:
            ...
            if (
                ...
                and "fullSyncRequired" in str(e.response.content)
                and sync_token
            ):
    ...
    def _microsoft_authentication_url(self, from_url="http://www.odoo.com"):
        return self.microsoft_service._get_authorize_uri(
            from_url, service="calendar", scope=self._get_calendar_scope()
        )
