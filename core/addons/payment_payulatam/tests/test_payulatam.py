...
class PayULatamTest(PayULatamCommon, PaymentHttpCommon):
    def test_compatibility_with_supported_currencies(self):
        ...
        for supported_currency_code in SUPPORTED_CURRENCIES:
            ...
            compatible_providers = self.env[
                "payment.provider"
            ]._get_compatible_providers(
                self.company.id,
                self.partner.id,
                self.amount,
                currency_id=supported_currency.id,
            )
    ...
    def test_feedback_processing(self):
        ...
        self.assertEqual(
            tx.state,
            "pending",
            "Payulatam: wrong state after receiving a valid pending notification",
        )
        self.assertEqual(
            tx.state_message,
            payulatam_post_data["message"],
            "Payulatam: wrong state message after receiving a valid pending notification",
        )
        self.assertEqual(
            tx.provider_reference,
            "b232989a-4aa8-42d1-bace-153236eee791",
            "Payulatam: wrong txn_id after receiving a valid pending notification",
        )
