...
class PortalAccount(CustomerPortal):
    def _prepare_home_portal_values(self, counters):
        ...
        if "invoice_count" in counters:
            invoice_count = (
                request.env["account.move"].search_count(self._get_invoices_domain())
    ...
    def details_form_validate(self, data, partner_creation=False):
        ...
        if not partner_creation and not partner.can_edit_vat():
            ...
            if "company_name" in data and (data["company_name"] or False) != (
                ...
                error_message.append(
                    _(
                        "Changing your company name is not allowed once invoices have been issued for your account. Please contact us directly for this operation."
                    )
                )
        return error, error_message

    def extra_details_form_validate(
        self, data, additional_required_fields, error, error_message
    ):
