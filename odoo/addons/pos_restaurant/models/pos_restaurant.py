...
class RestaurantFloor(models.Model):
    ...
    def _unlink_except_active_pos_session(self):
        confs = self.mapped("pos_config_id").filtered(
            lambda c: c.is_table_management == True
        )
    ...
    def write(self, vals):
        for floor in self:
            if floor.pos_config_id.has_active_session and (
                vals.get("pos_config_id") or vals.get("active")
            ):
                raise UserError(
            ...
            if (
                ...
                and floor.pos_config_id.id
                and vals.get("pos_config_id") != floor.pos_config_id.id
            ):
                raise UserError(
                    _("The %s is already used in another Pos Config.", floor.name)
                )
...
class RestaurantTable(models.Model):
    ...
    name = fields.Char(
        "Table Name", required=True, help="An internal identification of a table"
    )
    ...
    def _unlink_except_active_pos_session(self):
        confs = (
            self.mapped("floor_id")
            .mapped("pos_config_id")
            .filtered(lambda c: c.is_table_management == True)
        )
        opened_session = self.env["pos.session"].search(
            [("config_id", "in", confs.ids), ("state", "!=", "closed")]
        )
