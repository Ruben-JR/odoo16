...
class TestMailResend(TestMailCommon):
    ...
    def setUpClass(cls):
        ...
        cls.partner1 = (
            cls.env["res.partner"]
            .with_context(cls._test_context)
            .create({"name": "Partner 1", "email": "p1"})  # invalid email
        )
        cls.partner2 = (
            cls.env["res.partner"]
            .with_context(cls._test_context)
            .create({"name": "Partner 2", "email": "p2@example.com"})
        )
        cls.partners = cls.env["res.partner"].concat(
            cls.user1.partner_id, cls.user2.partner_id, cls.partner1, cls.partner2
        )
    ...
    def test_mail_resend_workflow(self):
        with self.assertSinglePostNotifications(
            [
                {"partner": partner, "type": "email", "status": "exception"}
        ...
        self.assertEqual(
            ...
            self.partners,
            "wizard should manage notifications for each failed partner",
        )
                    ...
                    "notif": [
                        {
                            ...
                            "type": "email",
                            "status": "exception"
                            if partner in self.user1.partner_id | self.partner1
                            else "sent",
                        }
                        for partner in self.partners
                    ],
        ...
        with self.mock_mail_gateway(), self.assertBus(expected_bus_notifications * 2):
            self.env["mail.resend.message"].with_context(
                {"mail_message_to_resend": message.id}
            ).create({}).resend_mail_action()
        done_msgs, done_notifs = self.assertMailNotifications(
            message,
            [
                {
                    ...
                    "notif": [
                        {
                            ...
                            "type": "email",
                            "status": "exception"
                            if partner == self.partner1
                            else "sent",
                            "check_send": partner == self.partner1,
                        }
                        for partner in self.partners
                    ],
                }
    ...
    def test_remove_mail_become_canceled(self):
        ...
        with self.mock_mail_gateway(), self.assertBus(
            [(self.cr.dbname, "res.partner", self.partner_admin.id)] * 2
        ):
            message = self.test_record.with_user(self.user_admin).message_post(
                partner_ids=self.partners.ids,
                subtype_xmlid="mail.mt_comment",
        ...
        wizard.partner_ids.filtered(lambda p: p.partner_id == self.partner1).write(
            {"resend": False}
        )
        wizard.resend_mail_action()
                    ...
                    "notif": [
                        {
                            "partner": partner,
                            "type": "email",
                            "status": (partner == self.user1.partner_id and "exception")
                            or (partner == self.partner1 and "canceled")
                            or "sent",
                        }
                        for partner in self.partners
                    ],
                    ...
                    "notif": [
                        {
                            ...
                            "type": "email",
                            "check_send": partner
                            in self.user1.partner_id | self.partner1,
                            "status": "canceled"
                            if partner in self.user1.partner_id | self.partner1
                            else "sent",
                        }
