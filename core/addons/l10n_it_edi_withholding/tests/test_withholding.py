...
class TestWithholdingAndPensionFundTaxes(TestItEdi):
    ...
    def setUpClass(cls):
        ...
        cls.edi_withholding_tax_xml = cls._get_withholding_test_file_content(
            "IT00470550013_withh.xml"
        )
        cls.edi_pension_fund_tax_xml = cls._get_withholding_test_file_content(
            "IT00470550013_pfund.xml"
        )
        cls.edi_enasarco_tax_xml = cls._get_withholding_test_file_content(
    ...
    def test_withholding_taxes_export(self):
        ...
        expected_etree = self.with_applied_xpath(
            ...
            + "".join(
             ...
             """
                for n, (name, price) in enumerate(invoice_data.lines, 1)
            )
            + f"""
                  <DatiRiepilogo>
                    <AliquotaIVA>22.00</AliquotaIVA>
    ...
    def test_withholding_taxes_import(self):
        ...
        invoice = self.edi_format._create_invoice_from_xml_tree(
            "IT00470550013_withh.xml", imported_etree, self.purchase_journal
        )
        invoice_data = self.get_real_client_invoice_data()
        for line in invoice.line_ids.filtered(
            lambda x: x.name in [data[0] for data in invoice_data.lines]
        ):
    ...
    def test_pension_fund_taxes_import(self):
        ...
        invoice = self.edi_format._create_invoice_from_xml_tree(
            "IT00470550013_pfund.xml", imported_etree, self.purchase_journal
        )
        invoice_data = self.get_real_client_invoice_data()
        for line in invoice.line_ids.filtered(
            lambda x: x.name in [data[0] for data in invoice_data.lines]
    ...
    def test_enasarco_tax_import(self):
        ...
        invoice = self.edi_format._create_invoice_from_xml_tree(
            "IT00470550013_enasa.xml", imported_etree, self.purchase_journal
        )
        ...
        for line in invoice.line_ids.filtered(
            ...
            self.assertEqual(
                self.withholding_purchase_tax_23,
