...
class TestSaleMargin(SaleCommon):
    ...
    def test_negative_margin(self):
        ...
        self.assertEqual(
            order.order_line[0].margin, -20.00, "Sales order profit should be -20.00"
        )
        self.assertEqual(
    ...
    def test_margin_no_cost(self):
        ...
        # Verify that margin field of Sale Order Lines gets bind with the value.
        self.assertEqual(
            order.order_line[0].margin, 70.00, "Sales order profit should be 70.00"
        )
        self.assertEqual(
            order.order_line[0].margin_percent,
            1.0,
    ...
    def test_margin_considering_product_qty(self):
        ...
        # Verify that margin field of Sale Order Lines gets bind with the value.
        self.assertEqual(
            order.order_line[0].margin, 150.00, "Sales order profit should be 150.00"
        )
        self.assertEqual(
            order.order_line[0].margin_percent, 0.5, "Sales order margin should be 100%"
        )
        self.assertEqual(
            order.order_line[1].margin, -50.00, "Sales order profit should be -50.00"
        )
        self.assertEqual(
