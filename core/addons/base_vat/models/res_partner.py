...
class ResPartner(models.Model):
    ...
    def _run_vat_test(self, vat_number, default_country, partner_is_company=True):
        ...
        vat_has_legit_country_code = self.env["res.country"].search(
            [("code", "=", vat_country_code.upper())], limit=1
        )
        if not vat_has_legit_country_code:
            vat_has_legit_country_code = (
                vat_country_code.lower() in _region_specific_vat_codes
            )
        ...
        if default_country:
            check_result = self.simple_vat_check(
                default_country.code.lower(), vat_number
            )
            if check_result:
    ...
    __check_vat_mx_re = re.compile(
        rb"(?P<primeras>[A-Za-z\xd1\xf1&]{3,4})"
        rb"[ \-_]?"
        rb"(?P<ano>[0-9]{2})(?P<mes>[01][0-9])(?P<dia>[0-3][0-9])"
        rb"[ \-_]?"
        rb"(?P<code>[A-Za-z0-9&\xd1\xf1]{3})$"
    )
    ...
    def check_vat_pe(self, vat):
        if len(vat) != 11 or not vat.isdigit():
            return False
        dig_check = 11 - (
            sum([int("5432765432"[f]) * int(vat[f]) for f in range(0, 10)]) % 11
        )
        if dig_check == 10:
            dig_check = 0
        elif dig_check == 11:
    ...
    def format_vat_ch(self, vat):
        stdnum_vat_format = getattr(
            stdnum.util.get_cc_module("ch", "vat"), "format", None
        )
    ...
    def _fix_vat_number(self, vat, country_id):
        ...
        stdnum_vat_fix_func = getattr(
            stdnum.util.get_cc_module(vat_country, "vat"), "compact", None
        )
