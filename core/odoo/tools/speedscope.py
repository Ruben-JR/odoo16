...
class Speedscope:
    ...
    def add_output(
        self, names, complete=True, display_name=None, use_context=True, **params
    ):
        entries = []
        ...
        if complete:
            ...
            init_stack_trace_ids = self.stack_to_ids(
                self.init_stack_trace, use_context and entries[0].get("exec_context")
            )
            for frame_id in init_stack_trace_ids:
                start_stack.append({"type": "O", "frame": frame_id, "at": start})
            for frame_id in reversed(init_stack_trace_ids):
                end_stack.append({"type": "C", "frame": frame_id, "at": end})
    ...
    def add_default(self):
        if len(self.profiles_raw) > 1:
            ...
            self.add_output(
                self.profiles_raw, display_name="Combined no context", use_context=False
            )
    ...
    def process(
        ...
        if last_entry["stack"]:
            entries.append(
                {"stack": [], "start": last_entry["start"] + last_entry.get("time", 0)}
            )
        ...
        for index, entry in enumerate(entries):
            ...
            if continuous:
                level = -1
                for level, at_level in enumerate(
                    zip(current_stack_ids, entry_stack_ids)
                ):
            ...
            for frame in reversed(current_stack_ids[level:]):
                events.append({"type": "C", "frame": frame, "at": close_time})
            for frame in entry_stack_ids[level:]:
                events.append({"type": "O", "frame": frame, "at": entry_start})
