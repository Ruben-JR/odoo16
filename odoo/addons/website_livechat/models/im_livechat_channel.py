...
class ImLivechatChannel(models.Model):
    ...
    def _get_livechat_mail_channel_vals(
        self, anonymous_name, operator=None, chatbot=None, user_id=None, country_id=None
    ):
        mail_channel_vals = super(
            ImLivechatChannel, self
        )._get_livechat_mail_channel_vals(
            anonymous_name, operator, chatbot, user_id=user_id, country_id=country_id
        )
        ...
        if visitor_sudo:
            ...
            for mail_channel in chat_request_channel:
                operator_name = (
                    operator.name if operator else chatbot.operator_partner_id.name
                )
