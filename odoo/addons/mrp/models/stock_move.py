        ...
        if operator in ["not in", "!=", "not ilike"]:
            if value is False:
                return expression.OR(
                    [[("production_id.picking_type_id", operator, value)], res]
                )
            else:
    ...
    def _reservation_is_updatable(self, quantity, reserved_quant):
        ...
        if self.produce_line_ids.lot_id:
            ...
            ml_remaining_qty = self.product_uom_id._compute_quantity(
                ml_remaining_qty, self.product_id.uom_id, rounding_method="HALF-UP"
            )
            if (
                float_compare(
                    ml_remaining_qty,
                ...
                return False
        return super(StockMoveLine, self)._reservation_is_updatable(
    ...
    def write(self, vals):
        for move_line in self:
            ...
            if (
                ...
                and any(
                    field in vals for field in ("lot_id", "location_id", "qty_done")
                )
            ):
...
class StockMove(models.Model):
    ...
    order_finished_lot_id = fields.Many2one(
        "stock.lot",
        string="Finished Lot/Serial Number",
        related="raw_material_production_id.lot_producing_id",
        store=True,
    )
    ...
    def _compute_manual_consumption(self):
        for move in self:
            ...
            move.manual_consumption = (
                ...
                and not move.raw_material_production_id.use_auto_consume_components_lots
            ) or (move.has_tracking == "none" and move.bom_line_id.manual_consumption)
    ...
    def create(self, vals_list):
        ...
        for values in vals_list:
            ...
            if (
                mo_id
                and not values.get("scrapped")
                and not location_dest.scrap_location
            ):
    ...
    def action_explode(self):
        ...
        for move in self:
            ...
            if not bom:
                moves_ids_to_return.add(move.id)
                continue
            if move.picking_id.immediate_transfer or float_is_zero(
                move.product_uom_qty, precision_rounding=move.product_uom.rounding
            ):
                factor = (
                    move.product_uom._compute_quantity(
                        move.quantity_done, bom.product_uom_id
                    )
                    / bom.product_qty
                )
            else:
            ...
            boms, lines = bom.sudo().explode(
                move.product_id, factor, picking_type=bom.picking_type_id
            )
            for bom_line, line_data in lines:
                if move.picking_id.immediate_transfer or float_is_zero(
                    ...
                    phantom_moves_vals_list += move._generate_move_phantom(
                        bom_line, 0, line_data["qty"]
                    )
                else:
                    phantom_moves_vals_list += move._generate_move_phantom(
                        bom_line, line_data["qty"], 0
                    )
    ...
    def _get_backorder_move_vals(self):
        ...
        return {
            ...
            "manual_consumption": self.bom_line_id.manual_consumption
    ...
    def _should_bypass_set_qty_producing(self):
        ...
        if float_is_zero(
            self.product_uom_qty, precision_rounding=self.product_uom.rounding
        ):
            return True
        if (
            (
                not self.raw_material_production_id.use_auto_consume_components_lots
                and self.has_tracking != "none"
            )
            or self.manual_consumption
            or self._origin.manual_consumption
        ):
            return True
        return False
