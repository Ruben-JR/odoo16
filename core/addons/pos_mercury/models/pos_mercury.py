...
class BarcodeRule(models.Model):
    ...
    type = fields.Selection(
        selection_add=[("credit", "Credit Card")], ondelete={"credit": "set default"}
    )
...
class PoSPayment(models.Model):
    ...
    mercury_card_number = fields.Char(
        string="Card Number", help="The last 4 numbers of the card used to pay"
    )
    ...
    mercury_card_brand = fields.Char(
        string="Card Brand", help="The brand of the payment card (e.g. Visa, AMEX, ...)"
    )
    ...
    mercury_record_no = fields.Char(
        string="Vantiv record number", help="Payment record number from Vantiv Pay"
    )
    mercury_invoice_no = fields.Char(
        string="Vantiv invoice number", help="Invoice number from Vantiv Pay"
    )
...
class PoSPaymentMethod(models.Model):
    ...
    pos_mercury_config_id = fields.Many2one(
        ...
        fields.update(
            {
                ...
                "mercury_card_owner_name": ui_paymentline.get(
