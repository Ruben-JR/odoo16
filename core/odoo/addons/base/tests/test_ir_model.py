...
class TestXMLID(TransactionCase):
    ...
    def test_create_inherits(self):
        ...
        user = model._load_records(
            [dict(xml_id=xml_id, values={"name": "Foo", "login": "foo"})]
        )
...
class TestIrModel(TransactionCase):
    ...
    def test_model_order_constraint(self):
        ...
        fields_value = [
            ...
            Command.create(
                {"name": "x_length", "ttype": "float", "field_description": "Length"}
            ),
            Command.create(
                {"name": "x_color", "ttype": "integer", "field_description": "Color"}
            ),
        ]
    ...
    def test_group_expansion(self):
        ...
        groups = self.env["x_bananas"].read_group(
            domain=[], fields=["x_ripeness_id"], groupby=["x_ripeness_id"]
        )
        expected = [
            {
    ...
    def test_rec_name_deletion(self):
        ...
        self.assertEqual(
            self.registry.field_depends[type(record).display_name], ("x_name",)
        )
