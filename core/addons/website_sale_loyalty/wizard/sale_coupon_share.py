...
class SaleCouponShare(models.TransientModel):
    ...
    website_id = fields.Many2one(
        "website", required=True, default=_get_default_website_id
    )
    ...
    def _check_program(self):
        if self.filtered(
            lambda record: not record.coupon_id
            and record.program_id.program_type == "coupons"
        ):
    ...
    def create_share_action(self, coupon=None, program=None):
        ...
        return {
            "name": _("Share")
            + f' {self.env["loyalty.program"]._program_items_name().get((program or coupon).program_type, "")}',
            "type": "ir.actions.act_window",
            "view_mode": "form",
            "res_model": "coupon.share",
            "target": "new",
