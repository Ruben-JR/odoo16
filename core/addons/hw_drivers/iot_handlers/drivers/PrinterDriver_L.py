...
from odoo.addons.hw_drivers.iot_handlers.interfaces.PrinterInterface_L import (
    PPDs,
    conn,
    cups_lock,
)
...
def cups_notification_handler(
    message, uri, device_identifier, state, reason, accepting_jobs
):
    if device_identifier in iot_devices:
        ...
        iot_devices[device_identifier].update_status(
            state_value[state], message, reason
        )
...
try:
    ...
    conn.createSubscription(
        uri="/printers/", recipient_uri="dbus://", events=["printer-state-changed"]
    )

# Listen for notifications from Cups
bus = dbus.SystemBus()
bus.add_signal_receiver(
    cups_notification_handler,
    signal_name="PrinterStateChanged",
...
class PrinterDriver(Driver):
    ...
    def supported(cls, device):
        ...
        if (
            ...
            with cups_lock:
                if ppdFile:
                    conn.addPrinter(
                        name=device["identifier"], ppdname=ppdFile, device=device["url"]
                    )
                else:
    ...
    def get_device_model(cls, device):
        ...
        if device.get("device-id"):
            for device_id in [
                device_lo for device_lo in device["device-id"].split(";")
            ]:
    ...
    def print_raw(self, data):
        process = subprocess.Popen(
            ["lp", "-d", self.device_identifier], stdin=subprocess.PIPE
        )
