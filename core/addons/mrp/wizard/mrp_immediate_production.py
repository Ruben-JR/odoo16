...
class MrpImmediateProductionLine(models.TransientModel):
    ...
    immediate_production_id = fields.Many2one(
        "mrp.immediate.production", "Immediate Production", required=True
    )
...
class MrpImmediateProduction(models.TransientModel):
    ...
    def default_get(self, fields):
        ...
        if "immediate_production_line_ids" in fields:
            if self.env.context.get("default_mo_ids"):
                ...
                res["immediate_production_line_ids"] = [
                    (0, 0, {"to_immediate": True, "production_id": mo_id[1]})
    ...
    def _compute_show_production(self):
        for wizard in self:
            wizard.show_productions = (
                len(wizard.immediate_production_line_ids.production_id) > 1
            )
    ...
    def process(self):
        ...
        for production in productions_to_do:
            ...
            for move in production.move_raw_ids.filtered(
                ...
                if move.has_tracking in ("serial", "lot") and float_is_zero(
            ...
            if error_msg:
                error_msg = (
                    _("You need to supply Lot/Serial Number for products:") + error_msg
                )
        ...
        if productions_to_validate:
            ...
            if consumption_issues:
                return productions_to_validate._action_generate_consumption_wizard(
                    consumption_issues
                )
