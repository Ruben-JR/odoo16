...
class ReplenishmentReport(models.AbstractModel):
    ...
    def _serialize_docs(
        ...
        for i in range(len(docs["lines"])):
            if (
                not docs["lines"][i]["move_out"]
                or not docs["lines"][i]["move_out"]["picking_id"]
                or not docs["lines"][i]["move_out"]["picking_id"]["sale_id"]
            ):
                continue
    ...
    def _compute_draft_quantity_count(
        ...
        res["draft_sale_orders"] = so_lines.mapped("order_id").sorted(
            key=lambda so: so.name
        )
        res["draft_sale_orders_matched"] = (
            self.env.context.get("sale_line_to_match_id") in so_lines.ids
        )
        res["qty"]["out"] += out_sum
        return res
