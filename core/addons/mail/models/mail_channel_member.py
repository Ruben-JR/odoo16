...
class ChannelMember(models.Model):
    ...
    partner_id = fields.Many2one(
        "res.partner", string="Recipient", ondelete="cascade", index=True
    )
    ...
    fold_state = fields.Selection(
        [("open", "Open"), ("folded", "Folded"), ("closed", "Closed")],
    ...
    last_interest_dt = fields.Datetime(
        ...
        help="Contains the date and time of the last interesting event that happened in this channel for this partner. This includes: creating, joining, pinning, and new message posted.",
    )
    last_seen_dt = fields.Datetime("Last seen date")
    # RTC
    rtc_session_ids = fields.One2many(
        string="RTC Sessions",
        comodel_name="mail.channel.rtc.session",
        inverse_name="channel_member_id",
    )
    rtc_inviting_session_id = fields.Many2one(
        "mail.channel.rtc.session", string="Ringing session"
    )
    ...
    def name_get(self):
        return [
            (record.id, record.partner_id.name or record.guest_id.name)
            for record in self
        ]

    def init(self):
        self.env.cr.execute(
            "CREATE UNIQUE INDEX IF NOT EXISTS mail_channel_member_partner_unique ON %s (channel_id, partner_id) WHERE partner_id IS NOT NULL"
            % self._table
        )
        self.env.cr.execute(
            "CREATE UNIQUE INDEX IF NOT EXISTS mail_channel_member_guest_unique ON %s (channel_id, guest_id) WHERE guest_id IS NOT NULL"
            % self._table
        )
    ...
    def create(self, vals_list):
        ...
        if (
            ...
            and not self.env.context.get("mail_create_bypass_create_check")
            ...
            for vals in vals_list:
                if "channel_id" in vals:
                    ...
                    if not channel_id._can_invite(vals.get("partner_id")):
                        raise AccessError(
                            _("This user can not be added in this channel")
                        )
    ...
    def _get_as_sudo_from_request_or_raise(self, request, channel_id):
        channel_member = self._get_as_sudo_from_request(
    ...
    def _get_as_sudo_from_request(self, request, channel_id):
        ...
        if guest:
            return (
                guest.env["mail.channel.member"]
                .sudo()
                .search(
                    [("channel_id", "=", channel_id), ("guest_id", "=", guest.id)],
                    limit=1,
                )
            )
    ...
    def _notify_typing(self, is_typing):
        ...
        for member in self:
            ...
            notifications.append(
                [
                    member.channel_id,
                    "mail.channel.member/typing_status",
                    formatted_member,
                ]
            )
            notifications.append(
                [
                    member.channel_id.uuid,
                    "mail.channel.member/typing_status",
                    formatted_member,
                ]
            )  # notify livechat users
    ...
    def _mail_channel_member_format(self, fields=None):
        ...
        for member in self:
            ...
            if "channel" in fields:
                data["channel"] = member.channel_id._channel_format(
                    fields=fields.get("channel")
                ).get(member.channel_id)
            if "persona" in fields:
                if member.partner_id:
                    persona = {
                        "partner": member._get_partner_data(
                            fields=fields.get("persona", {}).get("partner")
                        )
                    }
                if member.guest_id:
                    persona = {
                        "guest": member.guest_id.sudo()
    ...
    def _rtc_join_call(self, check_rtc_session_ids=None):
        ...
        rtc_session = self.env["mail.channel.rtc.session"].create(
            {"channel_member_id": self.id}
        )
        ...
        ) == 1 and self.channel_id.channel_type in {"chat", "group"}:
            self.channel_id.message_post(
                body=_(
                    "%s started a live conference",
                    self.partner_id.name or self.guest_id.name,
                ),
            ...
            if invited_members:
                res["invitedMembers"] = [
                    (
                        "insert",
                        list(
                            invited_members._mail_channel_member_format(
                                fields={
                                    "id": True,
                                    "channel": {},
                                    "persona": {
    ...
    def _rtc_sync_sessions(self, check_rtc_session_ids=None):
        ...
        check_rtc_sessions = self.env["mail.channel.rtc.session"].browse(
            [
    ...
    def _rtc_invite_members(self, member_ids=None):
        ...
        if members:
            ...
            self.env["bus.bus"]._sendone(
                self.channel_id, "mail.thread/insert", channel_data
            )
        return members
