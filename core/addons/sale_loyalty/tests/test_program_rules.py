...
class TestProgramRules(TestSaleCouponCommon):
    ...
    def test_program_rules_minimum_purchased_amount(self):
        ...
        self.immediate_promotion_program.rule_ids.write(
            {"minimum_amount": 1006, "minimum_amount_tax_mode": "excl"}
        )
    ...
    def test_program_rules_validity_dates_and_uses(self):
        ...
        self.assertEqual(
            len(order.order_line.ids),
            2,
            "The promo offer shouldn't have been applied we're not between the validity dates",
        )
        ...
        self.assertEqual(
            len(order.order_line.ids),
            2,
            "The promo offer shouldn't have been applied as the number of uses is exceeded",
        )
