...
class HrExpense(models.Model):
    ...
    def _get_employee_id_domain(self):
        ...
        ) or self.user_has_groups("account.group_account_user"):
            res = "['|', ('company_id', '=', False), ('company_id', '=', company_id)]"  # Then, domain accepts everything
        elif (
            self.user_has_groups("hr_expense.group_hr_expense_team_approver")
    ...
    product_uom_category_id = fields.Many2one(
        related="product_id.uom_id.category_id", readonly=True, string="UoM Category"
    )
    ...
    unit_amount_display = fields.Float(
        "Unit Price Display", compute="_compute_unit_amount_display"
    )
    ...
    payment_mode = fields.Selection(
        [("own_account", "Employee (to reimburse)"), ("company_account", "Company")],
        default="own_account",
        tracking=True,
        states={
            "done": [("readonly", True)],
    ...
    approved_by = fields.Many2one(
        "res.users", string="Approved By", related="sheet_id.user_id", tracking=False
    )
    approved_on = fields.Datetime(
        string="Approved On", related="sheet_id.approval_date"
    )
    reference = fields.Char("Bill Reference")
    is_refused = fields.Boolean(
        "Explicitly Refused by manager or accountant", readonly=True, copy=False
    )

    is_editable = fields.Boolean(
        "Is Editable By Current User", compute="_compute_is_editable"
    )
    is_ref_editable = fields.Boolean(
        "Reference Is Editable By Current User", compute="_compute_is_ref_editable"
    )
    product_has_cost = fields.Boolean(
        "Is product with non zero cost selected", compute="_compute_product_has_cost"
    )
    ...
    duplicate_expense_ids = fields.Many2many(
        "hr.expense", compute="_compute_duplicate_expense_ids"
    )
    ...
    def _compute_currency_rate(self):
        ...
        for expense in self:
            ...
            expense.currency_rate = expense.company_id and self.env[
                "res.currency"
    ...
    def _compute_same_currency(self):
        for expense in self:
            expense.same_currency = bool(
                not expense.company_id
                or (
                    expense.currency_id
                    and expense.currency_id == expense.company_currency_id
                )
            )
    ...
    def _compute_product_has_cost(self):
        for expense in self:
            expense.product_has_cost = expense.product_id and (
                float_compare(
                    expense.product_id.standard_price, 0.0, precision_digits=2
                )
                != 0
            )
            tax_ids = expense.product_id.supplier_taxes_id.filtered(
    ...
    def _compute_state(self):
        for expense in self:
            if not expense.sheet_id or expense.sheet_id.state == "draft":
                ...
                expense.sheet_id.state == "approve" or expense.sheet_id.state == "post"
            ):
    ...
    def _compute_amount(self):
        for expense in self:
            if expense.product_id and not expense.unit_amount:
                continue
            taxes = expense._get_taxes(
                price=expense.unit_amount, quantity=expense.quantity
            )
    ...
    def _compute_amount_residual(self):
        for expense in self:
            if not expense.sheet_id:
                ...
                continue
            if (
                not expense.currency_id
                or expense.currency_id == expense.company_id.currency_id
            ):
                residual_field = "amount_residual"
            else:
                residual_field = "amount_residual_currency"
            payment_term_lines = (
                expense.sheet_id.account_move_id.sudo().line_ids.filtered(
    ...
    def _compute_label_convert_rate(self):
        records_with_diff_currency = self.filtered(
            lambda x: not x.same_currency and x.currency_id
        )
    ...
    def _compute_is_editable(self):
        ...
        for expense in self:
            if expense.state == "draft" or expense.sheet_id.state in [
                "draft",
                "submit",
            ]:
    ...
    def _compute_sheet_is_editable(self):
        for expense in self:
            expense.sheet_is_editable = (
                not expense.sheet_id or expense.sheet_id.is_editable
            )
    ...
    def _compute_unit_amount_display(self):
        for expense in self:
            expense.unit_amount_display = (
                expense.unit_amount
                if expense.product_has_cost
                else expense.total_amount_company
            )
    ...
    def _compute_from_product_id_company_id(self):
        for expense in self:
            if not expense.product_id:
                expense.account_id = self.env["ir.property"]._get(
            ...
            if not expense.attachment_number or (
                ...
                expense.unit_amount = expense.product_id.price_compute(
                    "standard_price", currency=expense.currency_id
    ...
    def _compute_duplicate_expense_ids(self):
        ...
        expenses = self.filtered(
            lambda e: e.employee_id and e.product_id and e.total_amount
        )
        if expenses.ids:
            duplicates_query = """
    ...
    def _check_product_uom_category(self):
        for expense in self:
            if (
                expense.product_id
                and expense.product_uom_id.category_id
                != expense.product_id.uom_id.category_id
            ):
                raise UserError(
                    _(
    ...
    def write(self, vals):
        if "sheet_id" in vals:
            self.env["hr.expense.sheet"].browse(vals["sheet_id"]).check_access_rule(
                "write"
            )
        if "tax_ids" in vals or "analytic_distribution" in vals or "account_id" in vals:
            if any(not expense.is_editable for expense in self):
                raise UserError(
                    _("You are not authorized to edit this expense report.")
                )
        if "reference" in vals:
            if any(not expense.is_ref_editable for expense in self):
                raise UserError(
                    _(
                        "You are not authorized to edit the reference of this expense report."
                    )
                )
    ...
    def get_empty_list_help(self, help_message):
        return super(HrExpense, self).get_empty_list_help(
    ...
    def _get_empty_list_mail_alias(self):
        ...
        alias_record = (
            use_mailgateway and self.env.ref("hr_expense.mail_alias_expense") or False
        )
...
Or send your receipts at <a href="mailto:%(email)s?subject=Lunch%%20with%%20customer%%3A%%20%%2412.32">%(email)s</a>.
    ...
    def _get_default_expense_sheet_values(self):
        ...
        sheets = (
            [own_expenses, company_expenses]
            if create_two_reports
            else [expenses_with_amount]
        )
        values = []
        for todo in sheets:
            if len(todo) == 1:
    ...
    def _get_split_values(self):
        ...
        price_round_up = float_round(
            half_price, precision_digits=2, rounding_method="UP"
        )
        price_round_down = float_round(
            half_price, precision_digits=2, rounding_method="DOWN"
        )
    ...
    def _prepare_move_line_vals(self):
        ...
        return {
            ...
            else self.total_amount_company,
            "product_id": self.product_id.id,
            ...
            else self.employee_id.sudo().address_home_id.commercial_partner_id.id,
    ...
    def get_expense_dashboard(self):
        ...
        for expense in expenses:
            state = expense["state"]
            currency = (
                self.env["res.currency"].browse(expense["currency_id"][0])
    ...
    def message_new(self, msg_dict, custom_values=None):
        ...
        vals = {
            ...
            "tax_ids": [
                (4, tax.id, False)
                for tax in product.supplier_taxes_id.filtered(
        ...
        expense = super(HrExpense, self).message_new(
            msg_dict, dict(custom_values or {}, **vals)
        )
    ...
    def _parse_price(self, expense_description, currencies):
        ...
        if matches:
            ...
            )  # get the longuest match. e.g. "2 chairs 120$" -> the price is 120$, not 2
            full_str = match[0]
    ...
    def _send_expense_success_mail(self, msg_dict, expense):
        mail_template_id = (
            "hr_expense.hr_expense_template_register"
            if expense.employee_id.user_id
            else "hr_expense.hr_expense_template_register_no_user"
        )
...
class HrExpenseSheet(models.Model):
    ...
    expense_line_ids = fields.One2many(
        "hr.expense", "sheet_id", string="Expense Lines", copy=False
    )
    ...
    is_editable = fields.Boolean(
        "Expense Lines Are Editable By Current User", compute="_compute_is_editable"
    )
    ...
    total_amount_taxes = fields.Monetary(
        "Taxes", currency_field="currency_id", compute="_compute_amount", store=True
    )
    ...
    journal_displayed_id = fields.Many2one(
        "account.journal", string="Journal", compute="_compute_journal_displayed_id"
    )  # fix in stable
    ...
    def _compute_journal_displayed_id(self):
        for sheet in self:
            ...
            sheet.journal_displayed_id = (
                sheet.journal_id if paid_by_employee else sheet.bank_journal_id
            )

    @api.depends(
        "expense_line_ids.total_amount_company", "expense_line_ids.amount_tax_company"
    )
    def _compute_amount(self):
        for sheet in self:
            ...
            sheet.total_amount_taxes = sum(
                sheet.expense_line_ids.mapped("amount_tax_company")
            )
    ...
    def _compute_is_multiple_currency(self):
        for sheet in self:
            sheet.is_multiple_currency = (
                len(sheet.expense_line_ids.mapped("currency_id")) > 1
            )
    ...
    def _compute_can_reset(self):
        ...
        for sheet in self:
            sheet.can_reset = (
                is_expense_user
                if is_expense_user
                else sheet.employee_id.user_id == self.env.user
            )
    ...
    def _compute_expense_number(self):
        read_group_result = self.env["hr.expense"]._read_group(
    ...
    def _compute_from_employee_id(self):
        for sheet in self:
            ...
            sheet.user_id = (
                sheet.employee_id.expense_manager_id
                or sheet.employee_id.parent_id.user_id
            )
    ...
    def _compute_is_editable(self):
        ...
        for report in self:
            ...
            is_editable = (
                report.employee_id.user_id == self.env.user and report.state == "draft"
            ) or (is_manager and report.state in ["draft", "submit", "approve"])
            if not is_editable and report.state in ["draft", "submit", "approve"]:
                # expense manager can edit, unless it's own expense
                current_managers = (
                    report.employee_id.expense_manager_id
                    | report.employee_id.parent_id.user_id
                    | report.employee_id.department_id.manager_id.user_id
                    | report.user_id
                )
                is_editable = (
                    is_approver or self.env.user in current_managers
                ) and report.employee_id.user_id != self.env.user
    ...
    def _check_payment_mode(self):
        for sheet in self:
            ...
            if expense_lines and any(
                ...
                raise ValidationError(
                    _("Expenses must have the same To Reimburse status.")
                )
    ...
    def _check_expense_lines_company(self):
        for sheet in self:
            if any(
                expense.company_id != sheet.company_id
                for expense in sheet.expense_line_ids
            ):
                raise ValidationError(
                    _(
                        "An expense report must contain only lines from the same company."
                    )
                )
    ...
    def _get_mail_thread_data_attachments(self):
        ...
        expense_attachments = self.env["ir.attachment"].search(
            [("res_id", "in", expense_ids.ids), ("res_model", "=", "hr.expense")],
            order="id desc",
        )
    ...
    def action_sheet_move_create(self):
        ...
        if any(not sheet.journal_id for sheet in self):
            raise UserError(
                _("Specify expense journal to generate accounting entries.")
            )
        ...
        paid_expenses_company = self.filtered(
            lambda m: m.payment_mode == "company_account"
        )
        paid_expenses_company.write(
    ...
    def _prepare_move_vals(self):
        ...
        return {
            ...
            or fields.Date.context_today(self),  # expense payment behave as bills
            "ref": self.name,
    ...
    def action_unpost(self):
        draft_moves = self.account_move_id.filtered(
            lambda _move: _move.state == "draft"
        )
        draft_moves.unlink()
        moves = self.account_move_id - draft_moves
        moves._reverse_moves(
            default_values_list=[
                {"invoice_date": fields.Date.context_today(move), "ref": False}
                for move in moves
            ],
            cancel=True,
        )
    ...
    def action_open_account_move(self):
        ...
        return {
            ...
            if self.payment_mode == "own_account"
            else self.account_move_id.payment_id.id,
        }
    ...
    def _check_can_approve(self):
        if not self.user_has_groups("hr_expense.group_hr_expense_team_approver"):
            ...
            if (
                not self.env.user in current_managers
    ...
    def approve_expense_sheets(self):
        ...
        duplicates = self.expense_line_ids.duplicate_expense_ids.filtered(
