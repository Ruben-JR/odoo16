...
class Project(models.Model):
    ...
    has_any_so_to_invoice = fields.Boolean(
        "Has SO to Invoice", compute="_compute_has_any_so_to_invoice"
    )
    ...
    invoice_count = fields.Integer(
        related="analytic_account_id.invoice_count",
        groups="account.group_account_readonly",
    )
    vendor_bill_count = fields.Integer(
        related="analytic_account_id.vendor_bill_count",
        groups="account.group_account_readonly",
    )
               ...
               AND (   EXISTS(SELECT 1
                    ...
                    OR EXISTS(SELECT 1
                               ...
                               WHERE sol.id = pp.sale_line_id
                                 AND so.invoice_status = %(invoice_status)s))
               AND id in %(ids)s""",
            {"ids": tuple(self.ids), "invoice_status": invoice_status},
        )
    ...
    def _compute_sale_order_count(self):
        ...
        for project in self:
            project.sale_order_count = len(
                sale_order_items_per_project_id.get(
    ...
    def action_get_list_view(self):
        ...
        if self.allow_billable:
            action["views"] = [
                (self.env.ref("sale_project.project_milestone_view_tree").id, "tree"),
                (False, "form"),
            ]
        return action
    ...
    def action_open_project_invoices(self):
        ...
        query.add_where(
            "analytic_distribution ? %s", [str(self.analytic_account_id.id)]
        )
        query.order = None
    ...
    def _fetch_sale_order_items_per_project_id(self, domain_per_model=None):
        ...
        query_str, params = self._get_sale_order_items_query(domain_per_model).select(
        ...
        return {
            row["id"]: self.env["sale.order.line"].browse(row["sale_line_ids"])
    ...
    def _get_sale_order_items_query(self, domain_per_model=None):
        ...
        query = Query(
            ...
            " UNION ".join([project_query_str, task_query_str, milestone_query_str]),
        )
    ...
    def get_sale_items_data(self, domain=None, offset=0, limit=None, with_action=True):
        ...
        action_per_sol = (
            sols.sudo(False)._filter_access_rules_python("read")._get_action_per_item()
            if with_action
            else {}
        )
            ...
            for res in product_read_group:
                ...
                if res["type"] == "service":
                    service_policy = general_to_service_map.get(
                        (res["invoice_policy"], res["service_type"]), "ordered_prepaid"
                    )
                for product_id, (
                    amount_to_invoice,
                    amount_invoiced,
    ...
    def _get_revenues_items_from_invoices(self, excluded_move_line_ids=None):
        ...
        query = (
            ...
            .sudo()
            ._search(
                self._get_revenues_items_from_invoices_domain(
        ...
        if invoices_move_line_read:
            ...
            currency_ids = {
                iml["currency_id"]
                for iml in invoices_move_line_read
                + [{"currency_id": self.analytic_account_id.currency_id.id}]
            }
            rates = (
                self.env["res.currency"]
                .browse(list(currency_ids))
                ._get_rates(self.company_id, date.today())
            )
            conversion_rates = {
                cid: rates[self.analytic_account_id.currency_id.id] / rate_from
                for cid, rate_from in rates.items()
            }

            amount_invoiced = amount_to_invoice = 0.0
            for moves_read in invoices_move_line_read:
                price_subtotal = self.analytic_account_id.currency_id.round(
                    moves_read["price_subtotal"]
                    * conversion_rates[moves_read["currency_id"]]
                )
    ...
    def _get_profitability_items(self, with_action=True):
        ...
        revenue_items_from_invoices = self._get_revenues_items_from_invoices(
            excluded_move_line_ids=self.env["sale.order.line"]
            .browse(
                [
                    sol_id
                    for sol_read in sale_line_read_group
                    for sol_id in sol_read["ids"]
                ]
            )
            .invoice_lines.ids
        )
        revenues["data"] += revenue_items_from_invoices["data"]
        revenues["total"]["to_invoice"] += revenue_items_from_invoices["total"][
            "to_invoice"
        ]
        revenues["total"]["invoiced"] += revenue_items_from_invoices["total"][
            "invoiced"
        ]
