...
class TestWorkEntryHolidaysMultiContract(TestWorkEntryHolidaysBase):
    ...
    def test_multi_contract_holiday(self):
        ...
        leave = self.create_leave(
            datetime(2015, 11, 17, 7, 0), datetime(2015, 11, 20, 18, 0)
        )
        leave.action_approve()
        start = datetime.strptime("2015-11-01", "%Y-%m-%d")
        end_generate = datetime(2015, 11, 30, 23, 59, 59)
        work_entries = self.jules_emp.contract_ids._generate_work_entries(
            start, end_generate
        )
        work_entries.action_validate()
        work_entries = work_entries.filtered(
            lambda we: we.contract_id == self.contract_cdi
        )
    ...
    def test_move_contract_in_leave(self):
        ...
        self.contract_cdi.write(
            {"date_start": datetime.strptime("2015-12-30", "%Y-%m-%d").date()}
        )
        ...
        with self.assertRaises(ValidationError):
            self.contract_cdi.date_start = datetime.strptime(
                "2015-11-17", "%Y-%m-%d"
            ).date()
    ...
    def test_leave_multi_contracts_same_schedule(self):
        ...
        # resource calendar
        leave = self.create_leave(
            datetime(2022, 6, 1, 7, 0, 0), datetime(2022, 6, 30, 18, 0, 0)
        )
        leave.action_approve()
    ...
    def test_leave_multi_contracts_split(self):
        ...
        # working schedule
        leave = self.create_leave(
            datetime(2022, 6, 1, 5, 0, 0), datetime(2022, 6, 30, 20, 0, 0)
        )
        leave.action_approve()
