...
class MassMailController(http.Controller):
        ...
        if mailing.exists():
            ...
            if mailing.mailing_model_real == "mailing.contact":
                ...
                opt_out_list_ids = subscription_list_ids.filtered(
                    lambda rel: rel.opt_out
                ).mapped("list_id")
                opt_in_list_ids = subscription_list_ids.filtered(
                    lambda rel: not rel.opt_out
                ).mapped("list_id")
                opt_out_list_ids = set(
                    [
                        list.id
                        for list in opt_out_list_ids
                ...
                unsubscribed_list = ", ".join(
                    str(list.name)
                    for list in mailing.contact_list_ids
                    if list.is_public
                )
        ...
        if mailing.exists() and email:
            ...
            for record in records:
                record.sudo().message_post(
                    body=_(
                        "Feedback from %(email)s: %(feedback)s",
                        email=email,
                        feedback=feedback,
                    )
                )
    ...
    def track_mail_open(self, mail_id, token, **post):
        ...
        if not consteq(
            token,
            tools.hmac(request.env(su=True), "mass_mailing-mail_mail-open", mail_id),
        ):
            raise BadRequest()
    ...
    def full_url_redirect(self, code, mailing_trace_id, **post):
        ...
        return request.redirect(
            request.env["link.tracker"].get_url_from_code(code), code=301, local=False
        )
    ...
    def turn_off_mailing_reports(self, token, user_id):
        if not token or not user_id:
            raise werkzeug.exceptions.NotFound()
        user_id = int(user_id)
        correct_token = consteq(
            token, request.env["mailing.mailing"]._get_unsubscribe_token(user_id)
        )
        ...
        if correct_token and user.has_group("mass_mailing.group_mass_mailing_user"):
            ...
            if user.has_group("base.group_system"):
                ...
                return request.render(
                    "mass_mailing.mailing_report_deactivated", {"menu_id": menu_id}
                )
    ...
    def view(self, mailing_id, email=None, res_id=None, token=""):
        ...
        if mailing.exists():
            ...
            if not self._valid_unsubscribe_token(
                mailing_id, res_id, email, str(token)
            ...
            html_markupsafe = html_markupsafe.replace(
                "/unsubscribe_from_list", mailing._get_unsubscribe_url(email, res_id)
            )
