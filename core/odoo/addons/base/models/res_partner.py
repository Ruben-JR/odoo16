...
class FormatAddressMixin(models.AbstractModel):
    ...
    def _view_get_address(self, arch):
        ...
        if (
            address_view_id
            and not self._context.get("no_address_format")
            ...
            city_line = [
                line.split(" ") for line in address_format.split("\n") if "city" in line
            ]
            if city_line:
                field_order = [
                    field.replace("%(", "").replace(")s", "") for field in city_line[0]
                ]
                for address_node in arch.xpath("//div[hasclass('o_address_format')]"):
                    concerned_fields = {"zip", "city", "state_id"} - {field_order[0]}
                    current_field = address_node.find(
                        f".//field[@name='{field_order[0]}']"
                    )
...
class Partner(models.Model):
    ...
    def _compute_avatar(self, avatar_field, image_field):
        ...
        for partner in self - partners_with_internal_user:
            partner[avatar_field] = partner[image_field] or base64.b64encode(
                partner._avatar_get_placeholder()
            )
    ...
    def _onchange_parent_id_for_lang(self):
        ...
        if self.parent_id:
            self.lang = (
                self.parent_id.lang
                or self.env.context.get("default_lang")
                or self.env.lang
            )
    ...
    def _check_barcode_unicity(self):
        if (
            self.barcode
            and self.env["res.partner"].search_count([("barcode", "=", self.barcode)])
            > 1
        ):
    ...
    def _load_records_create(self, vals_list):
        partners = super(
            Partner, self.with_context(_partners_skip_fields_sync=True)
    ...
    def _parse_partner_name(self, text):
        ...
        if email and not name:
            ...
            if fallback_emails:
                ...
                name = (
                    text[: text.index(email)].replace('"', "").replace("<", "").strip()
                )
