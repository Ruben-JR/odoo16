...
class MrpProduction(models.Model):
    ...
    qty_producing = fields.Float(
        string="Quantity Producing", digits="Product Unit of Measure", copy=False
    )
    ...
    use_auto_consume_components_lots = fields.Boolean(
        related="picking_type_id.use_auto_consume_components_lots"
    )
    ...
    user_id = fields.Many2one(
        ...
        states={"done": [("readonly", True)], "cancel": [("readonly", True)]},
        domain=lambda self: [
            ("groups_id", "in", self.env.ref("mrp.group_mrp_user").id)
        ],
    )
    ...
    show_serial_mass_produce = fields.Boolean(
        "Display the serial mass produce wizard action",
        compute="_compute_show_serial_mass_produce",
    )
    ...
    def _compute_mrp_production_backorder(self):
        for production in self:
            production.mrp_production_backorder_count = len(
                production.procurement_group_id.mrp_production_ids
            )
    ...
    def _compute_components_availability(self):
        ...
        for production in productions:
            if any(
                ...
                forecast_date = max(
                    production.move_raw_ids.filtered("forecast_expected_date").mapped(
                ...
                if forecast_date:
                    ...
                    if production.date_planned_start:
                        production.components_availability_state = (
                            "late"
                            if forecast_date > production.date_planned_start
                            else "expected"
                        )
    ...
    def _compute_product_id(self):
        for production in self:
            ...
            if bom and (
                ...
                or bom.product_tmpl_id != production.product_tmpl_id
                or bom.product_id
    ...
    def _compute_bom_id(self):
        mo_by_picking_type_and_company_id = defaultdict(
            lambda: self.env["mrp.production"]
        )
        for mo in self:
            if not mo.product_id and not mo.bom_id:
                mo.bom_id = False
                continue
            mo_by_picking_type_and_company_id[
                (mo.picking_type_id, mo.company_id.id)
            ] |= mo

        for (
            picking_type,
            company_id,
        ), productions in mo_by_picking_type_and_company_id.items():
            boms_by_product = (
                self.env["mrp.bom"]
        ...
        for production in self:
            ...
            if moves:
                production_capacity = min(
                    moves.mapped(
                        lambda move: move.product_id.uom_id._compute_quantity(
    ...
    def _compute_production_real_duration(self):
        for production in self:
            production.production_real_duration = sum(
                production.workorder_ids.mapped("duration")
            )
        ...
        for production in self:
            if production.workorder_ids:
                production.is_planned = any(
                    wo.date_planned_start and wo.date_planned_finished
    ...
    def _compute_delay_alert_date(self):
        delay_alert_date_data = self.env["stock.move"]._read_group(
            [("id", "in", self.move_raw_ids.ids), ("delay_alert_date", "!=", False)],
            ["delay_alert_date:max"],
            "raw_material_production_id",
        )
        delay_alert_date_data = {
            data["raw_material_production_id"][0]: data["delay_alert_date"]
            for data in delay_alert_date_data
        }
        for production in self:
            production.delay_alert_date = delay_alert_date_data.get(
                production.id, False
            )

    def _compute_json_popover(self):
        production_no_alert = self.filtered(
            lambda m: m.state in ("done", "cancel") or not m.delay_alert_date
        )
            ...
            production.json_popover = json.dumps(
                {
                    ...
                    "delay_alert_date": format_datetime(
                        self.env, production.delay_alert_date, dt_format=False
                    ),
                    "late_elements": [
                        {
                            "id": late_document.id,
    ...
    def _compute_confirm_cancel(self):
        ...
        res = self.env["stock.move"].read_group(
            domain,
            ["state", "production_id", "raw_material_production_id"],
        ...
        for rec in res:
            production_record = (
                rec["production_id"] or rec["raw_material_production_id"]
            )
            if production_record:
                productions_with_done_move[production_record[0]] = True
        for production in self:
            production.confirm_cancel = productions_with_done_move.get(
                production.id, False
            )
    ...
    def _compute_production_location(self):
        ...
        location_by_company = {
            lbc["company_id"][0]: lbc["ids"] for lbc in location_by_company
        }
        for production in self:
            if production.product_id:
                production.production_location_id = production.product_id.with_company(
                    production.company_id
                ).property_stock_production
            else:
                production.production_location_id = location_by_company.get(
                    production.company_id.id
                )[0]
    ...
    def _compute_state(self):
        ...
        for production in self:
            ...
            elif production.product_uom_id and not float_is_zero(
                production.qty_producing,
                precision_rounding=production.product_uom_id.rounding,
            ):
                production.state = "progress"
    ...
    def _compute_reservation_state(self):
        ...
        for production in self:
            if production.state in ("draft", "done", "cancel"):
                continue
            relevant_move_state = (
                production.move_raw_ids._get_relevant_state_among_moves()
            )
    ...
    def _compute_show_serial_mass_produce(self):
        ...
        for order in self:
            if (
                ...
                and order.product_id.tracking == "serial"
                and float_compare(
                    order.product_qty,
                    1,
        ...
        for order in self:
            ...
            if order.product_id:
                virtual_available = order.product_id.with_context(
                    warehouse=warehouse.id, to_date=order.date_planned_start
                ).virtual_available
    ...
    def _compute_date_planned_finished(self):
        for production in self:
            ...
            date_planned_finished = production.date_planned_start + relativedelta(
                days=days_delay
            )
    ...
    def _onchange_product_id(self):
        for move in self.move_raw_ids:
            if self.product_id == move.product_id:
                message = _(
                    "The component %s should not be the same as the product to produce.",
    ...
    def write(self, vals):
        ...
        if "move_raw_ids" in vals and self.state not in ["draft", "cancel", "done"]:
            ...
            for move_vals in vals["move_raw_ids"]:
                ...
                if command == Command.CREATE and not field_values.get(
                    "warehouse_id", False
                ):
        ...
        for production in self:
            if "date_planned_start" in vals and not self.env.context.get(
                "force_date", False
            ):
            ...
            if (
                ...
                and not vals.get("date_planned_finished")
            ):
                new_date_planned_start = fields.Datetime.to_datetime(
                    vals.get("date_planned_start")
                )
                if (
                    not production.date_planned_finished
                    or new_date_planned_start >= production.date_planned_finished
                ):
                    production.date_planned_finished = (
                        new_date_planned_start + datetime.timedelta(hours=1)
                    )
        return res
    ...
    def copy_data(self, default=None):
        ...
        if not default or "move_finished_ids" not in default:
            ...
            default["move_finished_ids"] = [
                (0, 0, move.copy_data()[0]) for move in move_finished_ids
            ]
        if not default or "move_raw_ids" not in default:
            default["move_raw_ids"] = [
                (0, 0, move.copy_data()[0])
                for move in self.move_raw_ids.filtered(lambda m: m.product_qty != 0.0)
            ]
            ...
            for workorder in self.workorder_ids:
                if workorder.blocked_by_workorder_ids:
                    ...
                    for dependency in workorder.blocked_by_workorder_ids:
                        dependencies.append(
                            Command.link(workorders_mapping[dependency].id)
                        )
    ...
    def _get_move_finished_values(
        ...
        return {
            ...
            "move_dest_ids": [
                (4, x.id) for x in self.move_dest_ids if not byproduct_id
            ],
        ...
        for production in self:
            ...
            for bom_line, line_data in lines:
                if (
                    ...
                    or bom_line.product_id.type not in ["product", "consu"]
                ):
                    continue
                operation = (
    ...
    def _set_qty_producing(self):
        ...
        for move in self.move_raw_ids | self.move_finished_ids.filtered(
            ...
            if self.use_auto_consume_components_lots and move.has_tracking in (
                "lot",
                "serial",
            ):
                if (
    ...
    def _get_ready_to_produce_state(self):
        ...
        moves_in_first_operation = moves_in_first_operation.filtered(
            ...
            and not move.bom_line_id._skip_bom_line(self.product_id)
        )
    ...
    def _get_sources(self):
        ...
        return (dest_moves | parent_moves).group_id.mrp_production_ids.filtered(
            lambda p: p.origin != self.origin
        ) - self
    ...
    def _action_generate_immediate_wizard(self):
        ...
        return {
            ...
            "target": "new",
            "context": dict(
                self.env.context, default_mo_ids=[(4, mo.id) for mo in self]
            ),
        }
    ...
    def _link_workorders_and_moves(self):
        ...
        if self.allow_workorder_dependencies:
            ...
            for workorder in self.workorder_ids:
                if (
                    previous_workorder
                    and previous_workorder.operation_id.bom_id
                    == workorder.operation_id.bom_id
                ):
                    workorder.blocked_by_workorder_ids = [
        ...
        for move in self.move_raw_ids | self.move_finished_ids:
            if move.operation_id:
                ...
                bom = (
                    move.bom_line_id.bom_id
                    if (move.bom_line_id and move.bom_line_id.bom_id in workorder_boms)
                    else self.bom_id
                )
                move.write({"workorder_id": last_workorder_per_bom[bom].id})
    ...
    def _plan_workorders(self, replan=False):
        ...
        self.with_context(force_date=True).write(
            {
                "date_planned_start": min(
                    [workorder.leave_id.date_from for workorder in workorders]
                ),
                "date_planned_finished": max(
                    [workorder.leave_id.date_to for workorder in workorders]
                ),
            }
        )
    ...
    def button_unplan(self):
        if any(wo.state == "done" for wo in self.workorder_ids):
            raise UserError(
                _(
                    "Some work orders are already done, you cannot unplan this manufacturing order."
                )
            )
    ...
    def _get_consumption_issues(self):
        ...
        """
        issues = []
        if self.env.context.get("skip_consumption", False) or self.env.context.get(
        ...
        for order in self:
            ...
            for move in order.move_raw_ids:
                ...
                if not (
                    ...
                    or float_is_zero(qty_done, precision_rounding=rounding)
                ):
    ...
    def _get_quantity_produced_issues(self):
        ...
        for order in self:
            if not float_is_zero(
                order._get_quantity_to_backorder(),
                precision_rounding=order.product_uom_id.rounding,
            ):
    ...
    def _action_cancel(self):
        ...
        self.filtered(
            ...
            and p.bom_id.consumption == "flexible"
        ).write({"state": "done"})
    ...
    def _post_inventory(self, cancel_backorder=False):
        ...
        self.env["stock.move"].browse(moves_to_do)._action_done(
        ...
        for order in self:
            ...
            if finish_moves and not finish_moves.quantity_done:
                finish_moves._set_quantity_done(
                    float_round(
    ...
    def _split_productions(
        ...
        for production in self:
            ...
            if total_amount < production.product_qty and not cancel_remaining_qty:
                ...
                production.product_qty,
                precision_rounding=production.product_uom_id.rounding,
            ) > 0 or production.state in ["done", "cancel"]:
                raise UserError(
        ...
        for production in self:
            for move in production.move_raw_ids | production.move_finished_ids:
                ...
                move.with_context(do_not_unreserve=True).product_uom_qty = (
                    production.product_qty * unit_factor
                )
        ...
        for initial_move, backorder_moves in move_to_backorder_moves.items():
            ...
            if (
                ...
                and not initial_move.move_line_ids
                and set_consumed_qty
            ):
                ...
                for move in list(
                    ...
                    new_ml_vals = dict(
                        ml_vals, qty_done=move.product_uom_qty, move_id=move.id
                    )
        ...
        for initial_move, backorder_moves in move_to_backorder_moves.items():
            ...
            for move_line in initial_move.move_line_ids:
                available_qty = move_line.product_uom_id._compute_quantity(
                    move_line.reserved_uom_qty, product_uom
                )
                if (
                    float_compare(
                        available_qty,
            ...
            initial_move.move_line_ids.with_context(
                bypass_reservation_update=True
            ).write({"reserved_uom_qty": 0})
            ...
            for quantity, move_line, ml_vals in ml_by_move:
                while (
                    ...
                    taken_qty_uom = product_uom._compute_quantity(
                        taken_qty, move_line.product_uom_id
                    )
                    if move == initial_move:
                        ...
                        new_ml_vals = dict(
                            ml_vals, reserved_uom_qty=taken_qty_uom, move_id=move.id
                        )
                        if set_consumed_qty:
            ...
            move_lines_to_unlink.update(
                initial_move.move_line_ids.filtered(
                    lambda ml: not ml.reserved_uom_qty and not ml.qty_done
                ).ids
            )
    ...
    def button_mark_done(self):
        ...
        if not backorders:
            ...
            if (
                self.user_has_groups("mrp.group_mrp_reception_report")
        ...
        action = {
            ...
            "type": "ir.actions.act_window",
            "context": dict(
                context, mo_ids_to_backorder=None, button_mark_done_production_ids=None
            ),
        }
        if len(backorders) == 1:
            action.update(
    ...
    def _pre_button_mark_done(self):
        ...
        for production in self:
            ...
            if production.move_raw_ids and not any(
                production.move_raw_ids.mapped("quantity_done")
            ):
                raise UserError(
                    _(
                        "You must indicate a non-zero amount consumed for at least one of your components"
                    )
                )
    ...
    def action_serial_mass_produce_wizard(self):
        ...
        if self.reservation_state != "assigned":
            ...
            message = _(
                "Make sure enough quantities of these components are reserved to do the production:\n"
            )
        ...
        multiple_lot_components = set(
            [p for p, l in lot_components.items() if len(l) != 1]
        )
        action = self.env["ir.actions.actions"]._for_xml_id(
        ...
        action["context"] = {
            ...
            "default_multiple_lot_components_names": ",".join(
                c.display_name for c in multiple_lot_components
            )
    ...
    def action_merge(self):
        ...
        self.procurement_group_id.stock_move_ids.group_id = (
    ...
    def _check_sn_uniqueness(self):
        ...
        for move in self.move_finished_ids:
            ...
            for move_line in move.move_line_ids:
                if self._is_finished_sn_already_produced(
        ...
        for move in self.move_raw_ids:
            ...
            for move_line in move.move_line_ids:
                ...
                if duplicates:
                    ...
                    duplicates_returned = move.product_id._count_returned_sn_products(
                        move_line.lot_id
                    )
                    ...
                    if not (
                        (duplicates_returned or removed)
                        and duplicates - duplicates_returned - removed + unremoved == 0
                    ):
                        raise UserError(message)
                # Check presence of same sn in current production
                duplicates = (
                    co_prod_move_lines.filtered(
                        lambda ml: ml.qty_done and ml.lot_id == move_line.lot_id
                    )
                    - move_line
                )
                if duplicates:
    ...
    def _is_finished_sn_already_produced(self, lot, excluded_sml=None):
        ...
        domain = [("lot_id", "=", lot.id), ("qty_done", "=", 1), ("state", "=", "done")]
        ...
        if duplicates:
            ...
            if not (
                ...
                and duplicates - duplicates_unbuild - removed == 0
            ):
                return True
        # Check presence of same sn in current production
    ...
    def _check_immediate(self):
        ...
        for production in self:
            if all(
                float_is_zero(ml.qty_done, precision_digits=pd)
                for ml in production.move_raw_ids.filtered(
                    lambda m: not m.manual_consumption
                    and m.state not in ("done", "cancel")
                ).move_line_ids
