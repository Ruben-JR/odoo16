...
class Populate(Command):
    ...
    @classmethod
    def populate(
        cls, env, size, model_patterns=False, profiling_enabled=False, commit=True
    ):
        ...
        try:
            registry.populated_models = (
                {}
            )  # todo master, initialize with already populated models
            ...
            for model in ordered_models:
                if profiling_enabled:
                    profiling_context = odoo.tools.profiler.Profiler(
                        description=f"{model} {size}", db=env.cr.dbname
                    )
                else:
                ...
                if model_time > 1:
                    _logger.info(
                        "Populated database for model %s (total: %fs) (average: %fms per record)",
                        model._name,
                        model_time,
                        model_time / len(registry.populated_models[model._name]) * 1000,
                    )
        except:
    ...
    def _get_ordered_models(cls, env, model_patterns=False):
        ...
        for model in env.values():
            if model_patterns and not any(
                fnmatch.fnmatch(model._name, match) for match in model_patterns
            ):
                continue
            if model._transient or model._abstract:
                continue
            ir_model = env["ir.model"].search([("model", "=", model._name)])
            if not model_patterns and all(
