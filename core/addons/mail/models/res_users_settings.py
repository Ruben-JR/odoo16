...
class ResUsersSettings(models.Model):
    ...
    user_id = fields.Many2one(
        "res.users", string="User", required=True, readonly=True, ondelete="cascade"
    )
    ...
    _sql_constraints = [
        (
            "unique_user_id",
            "UNIQUE(user_id)",
            "One user should only have one mail user settings.",
        )
    ]
    ...
    def _res_users_settings_format(self, fields_to_format=None):
        ...
        if not fields_to_format:
            fields_to_format = [
                ...
                if name == "id" or not field.automatic
            ]
    ...
    def set_res_users_settings(self, new_settings):
        ...
        self.env["bus.bus"]._sendone(
            ...
            "res.users.settings/insert",
            self._res_users_settings_format([*changed_settings.keys(), "id"]),
        )
    ...
    def set_volume_setting(self, partner_id, volume, guest_id=None):
        ...
        self.env["bus.bus"]._sendone(
            self.user_id.partner_id,
            "res.users.settings.volumes/insert",
