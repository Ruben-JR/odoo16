...
class TestUBLBE(TestUBLCommon):
    ...
    def test_export_import_invoice(self):
        ...
        self.assertEqual(
            attachment.name[-12:], "ubl_bis3.xml"
        )  # ensure we test the right format !
    ...
    def test_rounding_price_unit(self):
        ...
        * Invoice line net amount shall be rounded to two decimals
        ...
        self.assertTrue(
            ...
            "The decimal precision for Product Price is required for this test",
        )
    ...
    def test_import_partner_ubl(self):
        ...
        * Uploading the attachment as a vendor bill should create a bill with the vendor = partner_1.
        ...
        new_invoice = self._import_invoice_attachment(
            invoice, "ubl_bis3", self.company_data["default_journal_sale"]
        )
        self.assertEqual(self.partner_2, new_invoice.partner_id)

        new_invoice = self._import_invoice_attachment(
            invoice, "ubl_bis3", self.company_data["default_journal_purchase"]
        )
    ...
    def test_import_journal_ubl(self):
        ...
        edi_attachment = invoice._get_edi_attachment(
            self.env.ref("account_edi_ubl_cii.ubl_bis3")
        ).id

        new_invoice = (
            self.env["account.journal"]
            .with_context(default_move_type="out_invoice")
            ._create_document_from_attachment(edi_attachment)
        )
        self.assertEqual(
            new_invoice.journal_id, self.company_data["default_journal_sale"]
        )

        new_invoice = (
            self.env["account.journal"]
            .with_context(default_journal_id=journal2.id)
            ._create_document_from_attachment(edi_attachment)
        )
    ...
    def test_import_invoice_xml_open_peppol_examples(self):
        ...
        self._assert_imported_invoice_from_file(
            ...
            amount_total=7125,
            amount_tax=1225,
            list_line_subtotals=[200, -200, 4000, 1000, 900],
        )
        ...
        self._assert_imported_invoice_from_file(
            ...
            amount_tax=331.25,
            list_line_subtotals=[25, 2800, -1500],
            move_type="in_refund",
        )
