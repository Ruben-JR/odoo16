...
class ProjectCustomerPortal(CustomerPortal):
    ...
    @http.route(
        [
            ...
            "/my/project/<int:project_id>/project_sharing",
        ],
        type="http",
        auth="public",
    )
    def portal_project_routes_outdated(self, **kwargs):
        ...
        return request.redirect(
            request.httprequest.full_path.replace("/my/project/", "/my/projects/")
        )

    @http.route(
        ["/my/task", "/my/task/page/<int:page>", "/my/task/<int:task_id>"],
        type="http",
        auth="public",
    )
    def portal_my_task_routes_outdated(self, **kwargs):
        """Redirect the outdated routes to the new routes."""
        return request.redirect(
            request.httprequest.full_path.replace("/my/task", "/my/tasks")
        )
        ...
        **kw,
        ...
        try:
            ...
            searchbar_filters = self._get_my_tasks_searchbar_filters(
                [("id", "=", task_sudo.project_id.id)], task_domain
            )
    ...
    def _task_get_search_domain(self, search_in, search):
        ...
        if search_in in ("priority", "all"):
            search_domain.append(
                [("priority", "ilike", search == "normal" and "0" or "1")]
            )
    ...
    def _prepare_tasks_values(
        ...
        searchbar_sortings = dict(
            sorted(
                self._task_get_searchbar_sortings(milestones_allowed).items(),
        ...
        if not su and Task.check_access_rights("read"):
            domain = AND(
                [domain, request.env["ir.rule"]._compute_domain(Task._name, "read")]
            )
        Task_sudo = Task.sudo()
        ...
        def get_grouped_tasks(pager_offset):
                ...
                if group == "milestone_id":
                    grouped_tasks = [
                        ...
                        for k, g in groupbyelem(
                            tasks_project_allow_milestone, itemgetter(group)
                        )
                    ]
            ...
            task_states = dict(
                Task_sudo._fields["kanban_state"]._description_selection(request.env)
            )
            if sortby == "status":
