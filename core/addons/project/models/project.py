...
class ProjectTaskType(models.Model):
    ...
    def unlink_wizard(self, stage_view=False):
        ...
        readgroup = (
            ...
            .env["project.task"]
            ._read_group([("stage_id", "in", self.ids)], ["project_id"], ["project_id"])
        )
        project_ids = list(
            set(
                [project["project_id"][0] for project in readgroup]
                + self.project_ids.ids
            )
        )

        wizard = (
            self.with_context(project_ids=project_ids)
            .env["project.task.type.delete.wizard"]
            .create({"project_ids": project_ids, "stage_ids": self.ids})
        )
        ...
        return {
            ...
            "views": [
                (
                    self.env.ref("project.view_project_task_type_delete_wizard").id,
                    "form",
                )
            ],
    ...
    def write(self, vals):
        if "active" in vals and not vals["active"]:
            self.env["project.task"].search([("stage_id", "in", self.ids)]).write(
                {"active": False}
            )
        ...
        if stage_active and inactive_tasks:
            ...
            return {
                ...
                "views": [
                    (
                        self.env.ref(
                            "project.view_project_task_type_unarchive_wizard"
                        ).id,
                        "form",
                    )
                ],
    ...
    def _compute_disabled_rating_warning(self):
        for stage in self:
            disabled_projects = stage.project_ids.filtered(
                lambda p: not p.rating_active
            )
            if disabled_projects:
                stage.disabled_rating_warning = "\n".join(
                    "- %s" % p.name for p in disabled_projects
                )
            else:
    ...
    def _check_personal_stage_not_linked_to_projects(self):
        if any(stage.user_id and stage.project_ids for stage in self):
            raise UserError(
                _(
                    "A personal stage cannot be linked to a project because it is only visible to its corresponding user."
                )
            )
    ...
    def remove_personal_stage(self):
        ...
        users_personal_stages = self.env["project.task.type"].search(
            [("user_id", "=", self.user_id.id)], order="sequence DESC"
        )
        if len(users_personal_stages) == 1:
            raise ValidationError(
                _(
                    "You should at least have one personal stage. Create a new stage to which the tasks can be transferred after this one is deleted."
                )
            )
        ...
        self.env["project.task.stage.personal"].search(
            [("stage_id", "=", self.id)]
        ).write(
            {
...
class Project(models.Model):
    ...
    def _compute_task_count(self):
        ...
        task_all_data = (
            self.env["project.task"]
            .with_context(active_test=False)
            ._read_group(domain, fields, groupby)
        )
    ...
    task_ids = fields.One2many(
        "project.task", "project_id", string="Tasks", domain=[("is_closed", "=", False)]
    )
    ...
    alias_enabled = fields.Boolean(
        string="Use Email Alias", compute="_compute_alias_enabled", readonly=False
    )
    ...
    privacy_visibility_warning = fields.Char(
        "Privacy Visibility Warning", compute="_compute_privacy_visibility_warning"
    )
    access_instruction_message = fields.Char(
        "Access Instruction Message", compute="_compute_access_instruction_message"
    )
    ...
    allow_subtasks = fields.Boolean(
        "Sub-tasks",
        default=lambda self: self.env.user.has_group("project.group_subtask_project"),
    )
    ...
    allow_milestones = fields.Boolean(
        "Milestones",
        default=lambda self: self.env.user.has_group("project.group_project_milestone"),
    )
    tag_ids = fields.Many2many(
        "project.tags", relation="project_project_project_tags_rel", string="Tags"
    )
    task_properties_definition = fields.PropertiesDefinition("Task Properties")

    # Project Sharing fields
    collaborator_ids = fields.One2many(
        "project.collaborator", "project_id", string="Collaborators", copy=False
    )
    collaborator_count = fields.Integer(
        "# Collaborators", compute="_compute_collaborator_count", compute_sudo=True
    )
    ...
    allow_rating = fields.Boolean(
        "Allow Customer Ratings",
        compute="_compute_allow_rating",
        default=lambda self: self.env.user.has_group("project.group_project_rating"),
    )
    rating_status = fields.Selection(
        [("stage", "Rating when changing stage"), ("periodic", "Periodic rating")],
    ...
    milestone_count = fields.Integer(
        compute="_compute_milestone_count", groups="project.group_project_milestone"
    )
    ...
    is_milestone_exceeded = fields.Boolean(
        compute="_compute_is_milestone_exceeded", search="_search_is_milestone_exceeded"
    )
    ...
    def _compute_milestone_count(self):
        ...
        mapped_count = {
            group["project_id"][0]: group["project_id_count"] for group in read_group
        }
        for project in self:
    ...
    def _compute_collaborator_count(self):
        ...
        collaborator_count_by_project = {
            res["project_id"][0]: res["project_id_count"]
            for res in collaborator_read_group
        }
        for project in self:
    ...
    def _compute_privacy_visibility_warning(self):
        for project in self:
            if not project.ids:
                ...
                and project._origin.privacy_visibility != "portal"
            ):
        ...
        for project in self:
            if project.privacy_visibility == "portal":
                project.access_instruction_message = _(
                    "Grant portal users access to your project or tasks by adding them as followers."
                )
    ...
    def map_tasks(self, new_project_id):
        ...
        ).write({"display_project_id": project.id})
        return True
    ...
    def copy(self, default=None):
        ...
        if self.allow_milestones:
            ...
            project.milestone_ids = [
                milestone.copy().id for milestone in self.milestone_ids
            ]
        ...
        if "name" in vals and self.analytic_account_id:
            ...
            analytic_account_to_update = self.env["account.analytic.account"].browse(
                [
                    ...
                    and res["analytic_account_id_count"] == 1
                ]
            )
    ...
    def message_subscribe(self, partner_ids=None, subtype_ids=None):
        ...
        currently following.
        """
        res = super(Project, self).message_subscribe(
                ...
                for task in self.task_ids:
                    ...
                    if partners:
                        task.message_subscribe(
                            partner_ids=list(partners), subtype_ids=task_subtypes
                        )
    ...
    def _track_template(self, changes):
        ...
        project = self[0]
        if (
            self.user_has_groups("project.group_project_stages")
    ...
    def _mail_get_message_subtypes(self):
        ...
        if len(self) == 1:
            dependency_subtype = self.env.ref(
    ...
    def action_get_list_view(self):
        ...
        return {
            ...
            "context": {"default_project_id": self.id, **self.env.context},
        }
    ...
    def _get_stat_buttons(self):
        ...
        if self.rating_count != 0 and self.user_has_groups(
    ...
    def _create_analytic_account_from_values(self, values):
        company = (
            self.env["res.company"].browse(values.get("company_id"))
...
class Task(models.Model):
    ...
    task_properties = fields.Properties(
        "Properties", definition="project_id.task_properties_definition", copy=True
    )
    ...
    personal_stage_type_id = fields.Many2one(
        ...
        compute="_compute_personal_stage_type_id",
        inverse="_inverse_personal_stage_type_id",
        store=False,
        search="_search_personal_stage_type_id",
        default=_default_personal_stage_type_id,
    ...
    rating_active = fields.Boolean(
        string="Project Rating Status", related="project_id.rating_active"
    )
    ...
    is_closed = fields.Boolean(
        ...
        store=True,
        index=True,
        help="Folded in Kanban stages are closing stages.",
    )
    ...
    allow_subtasks = fields.Boolean(
        string="Allow Sub-tasks", related="project_id.allow_subtasks", readonly=True
    )
    ...
    project_privacy_visibility = fields.Selection(
        related="project_id.privacy_visibility", string="Project Visibility"
    )
    ...
    is_private = fields.Boolean(
        compute="_compute_is_private", search="_search_is_private"
    )
    ...
    is_blocked = fields.Boolean(
        compute="_compute_is_blocked", store=True, recursive=True
    )

    # Project sharing fields
    display_parent_task_button = fields.Boolean(
        compute="_compute_display_parent_task_button", compute_sudo=True
    )
    ...
    recurring_count = fields.Integer(
        string="Tasks in Recurrence", compute="_compute_recurring_count"
    )
    ...
    recurrence_message = fields.Char(
        string="Next Recurrencies", compute="_compute_recurrence_message"
    )
    ...
    is_analytic_account_id_changed = fields.Boolean(
        "Is Analytic Account Manually Changed",
        compute="_compute_is_analytic_account_id_changed",
        store=True,
    )
    ...
    def _compute_is_analytic_account_id_changed(self):
        for task in self:
            task.is_analytic_account_id_changed = (
                task.project_id
                and task.analytic_account_id != task.project_id.analytic_account_id
            )
            ...
            for user_id in user_ids:
                ...
                if not stage:
                    stages = (
                        ...
                        .sudo()
                        .with_context(
                            lang=user_id.partner_id.lang, default_project_id=False
                        )
                        .create(
                            self.with_context(
    ...
    def _compute_repeat_visibility(self):
        for task in self:
            task.repeat_show_day = (
                task.recurring_task
                and (task.repeat_unit == "month" and task.repeat_on_month == "date")
                or (task.repeat_unit == "year" and task.repeat_on_year == "date")
            )
            task.repeat_show_week = (
                task.recurring_task
                and (task.repeat_unit == "month" and task.repeat_on_month == "day")
                or (task.repeat_unit == "year" and task.repeat_on_year == "day")
            )
    ...
    def _compute_recurrence_message(self):
        ...
        for task in self.filtered(
            ...
            recurrence_left = (
                task.recurrence_id.recurrence_left
                if task.recurrence_id
                else task.repeat_number
            )
            number_occurrences = min(
                5, recurrence_left if task.repeat_type == "after" else 5
            )
            ...
            if recurrence_left == 0:
                ...
                recurrence_title = _(
                    "A new task will be created on the following dates:"
                )
            task.recurrence_message = (
                '<p><span class="fa fa-check-circle"></span> %s</p><ul>'
                % recurrence_title
            )
            task.recurrence_message += "".join(
                [
                    "<li>%s</li>" % date.strftime(date_format)
                    for date in recurring_dates[:5]
                ]
            )
            if (
                task.repeat_type == "after"
                and recurrence_left > 5
                or task.repeat_type == "forever"
                or len(recurring_dates) > 5
            ):
    ...
    def _is_recurrence_valid(self):
        ...
        return (
            ...
            and (not self.repeat_show_dow or self._get_weekdays())
            and (self.repeat_type != "after" or self.repeat_number)
            and (
                self.repeat_type != "until"
                or self.repeat_until
                and self.repeat_until > fields.Date.today()
            )
        )
    ...
    def _compute_recurring_count(self):
        ...
        tasks_count = {
            c.get("recurrence_id")[0]: c.get("recurrence_id_count") for c in count
        }
    ...
    def _compute_is_blocked(self):
        for task in self:
            task.is_blocked = any(
                not blocking_task.is_closed or blocking_task.is_blocked
                for blocking_task in task.depend_on_ids
            )
    ...
    def _compute_access_warning(self):
        ...
        for task in self.filtered(
            lambda x: x.project_id.privacy_visibility != "portal"
        ):
    ...
    def _compute_subtask_planned_hours(self):
        for task in self:
            task.subtask_planned_hours = sum(
                child_task.planned_hours + child_task.subtask_planned_hours
    ...
    def _compute_child_text(self):
        for task in self:
            if not task.subtask_count:
                ...
                task.child_text = _(
                    "(+ %(child_count)s tasks)", child_count=task.subtask_count
                )
    ...
    def copy(self, default=None):
        ...
        if self.allow_subtasks:
            default["child_ids"] = [
                child.copy({"name": child.name} if has_default_name else None).id
        ...
        if self.allow_task_dependencies:
            ...
            self.write(
                {
                    "depend_on_ids": [
                        Command.unlink(t.id)
                        for t in self.depend_on_ids
                        if t.id in new_tasks
                    ]
                }
            )
            self.write(
            ...
            task_copy.write(
                {
                    "depend_on_ids": [
                        Command.link(task_mapping.get(t.id, t.id))
                        for t in self.depend_on_ids
                    ]
                }
            )
        ...
        if self.allow_milestones:
            ...
            task_copy.milestone_id = milestone_mapping.get(
                task_copy.milestone_id.id, task_copy.milestone_id.id
            )
        return task_copy
    ...
    def _valid_field_parameter(self, field, name):
        ...
        return name == "task_dependency_tracking" or super()._valid_field_parameter(
            field, name
        )
    ...
    def _get_depends_tracked_fields(self):
        ...
        fields = {
            ...
            if getattr(field, "task_dependency_tracking", None)
        }
    ...
    def fields_get(self, allfields=None, attributes=None):
        ...
        for field_name, description in public_fields.items():
            if field_name not in writable_fields and not description.get(
                "readonly", False
            ):
    ...
    def _ensure_fields_are_accessible(
        self, fields, operation="read", check_group_user=True
    ):
        ...
        if (
            fields
            and (not check_group_user or self.env.user.has_group("base.group_portal"))
            and not self.env.su
        ):
            unauthorized_fields = set(fields) - (
                self.SELF_READABLE_FIELDS
                if operation == "read"
                else self.SELF_WRITABLE_FIELDS
            )
            if unauthorized_fields:
                if operation == "read":
                    error_message = _(
                        "You cannot read %s fields in task.",
                        ", ".join(unauthorized_fields),
                    )
                else:
                    error_message = _(
                        "You cannot write on %s fields in task.",
                        ", ".join(unauthorized_fields),
                    )
    ...
    def read_group(
        ...
        if domain:
            fields_list += [
                ...
                and term not in [expression.TRUE_LEAF, expression.FALSE_LEAF]
            ]
        self._ensure_fields_are_accessible(fields_list)
        return super(Task, self).read_group(
            domain,
            fields,
            groupby,
            offset=offset,
    ...
    def _search(
        ...
        self._ensure_fields_are_accessible(fields_list)
        return super(Task, self)._search(
            args,
            offset=offset,
            limit=limit,
            order=order,
    ...
    def copy_data(self, default=None):
        ...
        if self.env.user.has_group("project.group_project_user"):
            return defaults
        return [
            {k: v for k, v in default.items() if k in self.SELF_READABLE_FIELDS}
    ...
    def _load_records_create(self, vals_list):
        projects_with_recurrence = self.env["project.project"].search(
        ...
        for vals in vals_list:
            if vals.get("recurring_task"):
                if vals.get(
                    "project_id"
                ) in projects_with_recurrence.ids and not vals.get("recurrence_id"):
                    default_val = self.default_get(self._get_recurrence_fields())
                    vals.update(**default_val)
                else:
                    for field_name in self._get_recurrence_fields() + [
                        "recurring_task"
                    ]:
    ...
    def create(self, vals_list):
        ...
        for vals in vals_list:
            if is_portal_user:
                self._ensure_fields_are_accessible(
                    vals.keys(), operation="write", check_group_user=False
                )
            ...
            if not project_id and (
                "stage_id" in vals or self.env.context.get("default_stage_id")
            ):
                vals["stage_id"] = False
    ...
    def write(self, vals):
        ...
        if (
            "active" in vals
            and not vals.get("active")
            and any(self.mapped("recurrence_id"))
        ):
            vals["recurring_task"] = False
        if (
            "recurrence_id" in vals
            and vals.get("recurrence_id")
            and any(not task.active for task in self)
        ):
            raise UserError(
                _(
                    "Archived tasks cannot be recurring. Please unarchive the task first."
                )
            )
        ...
        if "stage_id" in vals and vals.get("stage_id"):
            tasks.filtered(
                lambda x: x.project_id.rating_active
                and x.project_id.rating_status == "stage"
    ...
    def _compute_email_from(self):
        for task in self:
            task.email_from = (
                task.partner_id.email
                or ((task.partner_id or task.parent_id) and task.email_from)
                or task.parent_id.email_from
            )
    ...
    def _search_has_late_and_unreached_milestone(self, operator, value):
        if operator not in ("=", "!=") or not isinstance(value, bool):
            raise NotImplementedError(
                _(
                    "The search does not support the %s operator or %s value.",
                    operator,
                    value,
                )
            )
        domain = [
            ...
            ("milestone_id.deadline", "!=", False),
            ("milestone_id.deadline", "<", fields.Date.today()),
        ]
        if (operator == "!=" and value) or (operator == "=" and not value):
    ...
    def _notify_by_email_prepare_rendering_context(
        ...
        if self.date_deadline:
            ...
            render_context["subtitles"].append(
                _(
                    "Assigned On: %s",
                    self.date_assign.strftime(get_lang(self.env).date_format),
                )
            )
    ...
    def _message_auto_subscribe_followers(self, updated_values, default_subtype_ids):
        ...
        value = self._fields["user_ids"].convert_to_cache(
            updated_values.get("user_ids", []), self.env["project.task"], validate=False
        )
    ...
    def _mail_track(self, tracked_fields, initial_values):
        ...
        if (
            depends_changes
            and self.allow_task_dependencies
            and self.user_has_groups("project.group_project_task_dependencies")
        ):
            ...
            if parent_ids:
                ...
                depends_tracking_value_ids = [
                    tracking_values
                    for tracking_values in tracking_value_ids
                    if tracking_values[2]["field"] in field_ids
                ]
                subtype = self.env["ir.model.data"]._xmlid_to_res_id(
                ...
                child_subtype = self._track_subtype(
                    dict((col_name, initial_values[col_name]) for col_name in changes)
                )
                child_subtype_info = (
                    child_subtype.description or child_subtype.name
                    if child_subtype
                    else False
                )
                ...
                body = self.env["ir.qweb"]._render(
                    "project.task_track_depending_tasks",
                    {
                        "child": self,
                ...
                for p in parent_ids:
                    p.message_post(
                        body=body,
                        subtype_id=subtype,
                        tracking_value_ids=depends_tracking_value_ids,
                    )
    ...
    def _track_subtype(self, init_values):
        ...
        if "stage_id" in init_values:
            ...
            "kanban_state_label" in init_values
            and self.kanban_state in mail_message_subtype_per_kanban_state
        ):
            return self.env.ref(
                mail_message_subtype_per_kanban_state[self.kanban_state]
            )
    ...
    def _notify_get_recipients_groups(self, msg_vals=None):
        ...
        if self.project_privacy_visibility == "portal":
            groups.insert(
                0, ("allowed_portal_users", lambda pdata: pdata["type"] == "portal", {})
            )
        portal_privacy = self.project_id.privacy_visibility == "portal"
        for group_name, _group_method, group_data in groups:
            if (
                group_name in ("customer", "user")
                or group_name == "portal_customer"
                and not portal_privacy
            ):
    ...
    def message_new(self, msg, custom_values=None):
        ...
        partner_ids = [
            ...
            for p in self.env["mail.thread"]._mail_find_partner_from_emails(
                email_list, records=task, force_create=False
            )
            if p
        ]
    ...
    def action_project_sharing_view_parent_task(self):
        if self.parent_id.project_id != self.project_id and self.user_has_groups(
            "base.group_portal"
        ):
        ...
        action["views"] = [
            (self.env.ref("project.project_sharing_project_task_view_form").id, "form")
        ]
        return action
    ...
    def action_project_sharing_open_subtasks(self):
        ...
        return {
            ...
            "type": "ir.actions.act_url",
            "url": f"/my/projects/{self.project_id.id}/task/{self.id}/subtasks"
            if len(subtasks) > 1
    ...
    def action_open_ratings(self):
        ...
        if self.rating_count == 1:
            ...
            action["views"] = [
                [self.env.ref("project.rating_rating_view_form_project").id, "form"]
            ]
            return action
        else:
            return action
    ...
    def action_stop_recurrence(self):
        tasks = (
            self.env["project.task"]
            .with_context(active_test=False)
            .search([("recurrence_id", "in", self.recurrence_id.ids)])
        )
    ...
    def rating_apply(
        ...
        if self.stage_id and self.stage_id.auto_validation_kanban_state:
            kanban_state = (
                "done" if rating.rating >= rating_data.RATING_LIMIT_OK else "blocked"
            )
...
class ProjectTags(models.Model):
    ...
    project_ids = fields.Many2many(
        "project.project", "project_project_project_tags_rel", string="Projects"
    )
    ...
    def _get_project_tags_domain(self, domain, project_id):
        tag_ids = list(
            self.with_user(SUPERUSER_ID)._search(
                [
                    "|",
    ...
    def read_group(
        ...
        if "project_id" in self.env.context:
            domain = self._get_project_tags_domain(
                domain, self.env.context.get("project_id")
            )
