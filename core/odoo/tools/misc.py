...
def _exec_pipe(prog, args, env=None):
    warnings.warn(
        "Since 16.0, just use `subprocess`.", DeprecationWarning, stacklevel=3
    )
    cmd = (prog,) + args
...
def exec_command_pipe(name, *args):
    warnings.warn(
        "Since 16.0, use `subprocess` directly.", DeprecationWarning, stacklevel=2
    )
...
def logged(f):
    warnings.warn(
        "Since 16.0, it's never been super useful.", DeprecationWarning, stacklevel=2
    )
...
def resolve_attr(obj, attr, default=raise_error):
    warnings.warn("Since 16.0, component of `attrgetter`.", stacklevel=2)
...
def attrgetter(*items):
    warnings.warn(
        "Since 16.0, super old backport of Python 2.6's `operator.attrgetter`.",
        stacklevel=2,
    )
    if len(items) == 1:
        attr = items[0]
...
def format_time(env, value, tz=False, time_format="medium", lang_code=False):
    ...
    return babel.dates.format_time(
        localized_datetime, format=time_format, locale=locale
    )
