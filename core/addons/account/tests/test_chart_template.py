...
class TestChartTemplate(TransactionCase):
    ...
    def setUpClass(cls):
        ...
        if not cls.chart_template:
            cls.skipTest(
                cls,
                "Accounting Tests skipped because the generic chart of accounts was not found",
            )

        cls.fiscal_position_template = cls._create_fiscal_position_template(
            "account.test_fiscal_position_template",
            "US fiscal position test",
            us_country_id,
        )
        cls.tax_template_1 = cls._create_tax_template(
            "account.test_tax_template_1", "Tax name 1", 1, tag_name="tag_name_1"
        )
        cls.tax_template_2 = cls._create_tax_template(
            "account.test_tax_template_2", "Tax name 2", 2, tag_name="tag_name_2"
        )
        cls.fiscal_position_tax_template_1 = cls._create_fiscal_position_tax_template(
            cls.fiscal_position_template,
            "account.test_fp_tax_template_1",
            cls.tax_template_1,
            cls.tax_template_2,
        )

        cls.chart_template.try_loading(company=cls.company, install_demo=False)
        cls.chart_template_xmlid = cls.chart_template.get_external_id()[
            cls.chart_template.id
        ]
    ...
    @classmethod
    def _create_tax_template(
        cls, tax_template_xmlid, name, amount, tag_name=None, chart_template_id=None
    ):
        if tag_name:
    ...
    @classmethod
    def _create_fiscal_position_template(
        cls, fp_template_xmlid, fp_template_name, country_id
    ):
    ...
    @classmethod
    def _create_fiscal_position_tax_template(
        cls,
        fiscal_position_template,
        fiscal_position_tax_template_xmlid,
        tax_template_src,
        tax_template_dest,
    ):
    ...
    def test_update_taxes_new_template(self):
        ...
        """
        tax_template_3 = self._create_tax_template(
            "account.test_tax_3_template", "Tax name 3", 3, tag_name="tag_name_3"
        )
        tax_template_4 = self._create_tax_template(
            "account.test_tax_4_template", "Tax name 4", 4
        )
        self._create_fiscal_position_tax_template(
            self.fiscal_position_template,
            "account.test_fiscal_position_tax_template",
            tax_template_3,
            tax_template_4,
        )
        ...
        self.assertRecordValues(
            self.fiscal_position.tax_ids.tax_src_id,
            [
                {"name": "Tax name 1"},
                {"name": "Tax name 3"},
            ],
        ...
        self.assertRecordValues(
            self.fiscal_position.tax_ids.tax_dest_id,
            [
                {"name": "Tax name 2"},
                {"name": "Tax name 4"},
            ],
    ...
    def test_update_taxes_existing_template_update(self):
        ...
        self.assertEqual(
            tax.invoice_repartition_line_ids.tag_ids.name,
            self.tax_template_1.invoice_repartition_line_ids.tag_ids.name,
        )
    ...
    def test_update_taxes_conflict_name(self):
        ...
        self.assertRecordValues(
            taxes_from_template_1,
            [
                {"name": f"[old] {self.tax_template_1.name}", "amount": old_amount},
                {
                    "name": f"{self.tax_template_1.name}",
                    "amount": self.tax_template_1.amount,
                },
            ],
        )
            ...
            self.env["mail.message"].search_count(
                [
                    ...
                    (
                        "body",
                        "like",
                        f"%{self.tax_template_1.name}%",
                    ),  # we look for taxes' name that have been sent in the message's body
                ]
            ),
            1,
        )
    ...
    def test_update_taxes_foreign_taxes(self):
        ...
        self._create_tax_template(
            "account.test_tax_test_template",
            "Tax name 1 TEST",
            10,
            chart_template_id=chart_template_test.id,
        )
        self.env["account.tax.template"]._try_instantiating_foreign_taxes(
            country_test, self.company
        )
        self._create_tax_template(
            "account.test_tax_test_template2",
            "Tax name 2 TEST",
            15,
            chart_template_id=chart_template_test.id,
        )
        ...
        self.assertEqual(
            len(tax_test_model_data),
            1,
            "Taxes should have been created even if the chart_template is installed through fiscal position system.",
        )
    ...
    def test_update_taxes_chart_template_country_check(self):
        ...
        self.tax_template_1.amount += 1
        self.tax_template_2.invoice_repartition_line_ids.tag_ids.name = (
            "tag_name_2_modified"
        )
