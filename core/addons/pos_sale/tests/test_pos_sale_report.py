...
class TestPoSSaleReport(TestPoSCommon):
    ...
    def test_different_shipping_address(self):
        ...
        data = self.create_ui_order_data(
            [(product_0, 1)], self.env.ref("base.res_partner_1"), True
        )
        data["data"]["lines"][0][2]["sale_order_origin_id"] = sale_order.read()[0]
        data["data"]["lines"][0][2]["sale_order_line_id"] = sale_order.order_line[
            0
        ].read()[0]
        ...
        self.assertEqual(
            move_id.partner_shipping_id.id, self.env.ref("base.res_partner_2").id
        )
