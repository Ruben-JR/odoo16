...
class MailRtcSession(models.Model):
    ...
    channel_member_id = fields.Many2one(
        "mail.channel.member", required=True, ondelete="cascade"
    )
    ...
    def _notify_peers(self, notifications):
        ...
        return self.env["bus.bus"]._sendmany(
            [
                (target, "mail.channel.rtc.session/peer_notification", payload)
    ...
    def _mail_rtc_session_format(self, fields=None):
        ...
        if "channelMember" in fields:
            vals["channelMember"] = self.channel_member_id._mail_channel_member_format(
                fields=fields.get("channelMember")
    ...
    def _mail_rtc_session_format_by_channel(self):
        data = {}
        for rtc_session in self:
            data.setdefault(rtc_session.channel_id, []).append(
                rtc_session._mail_rtc_session_format()
            )
        return data
