...
class StockRule(models.Model):
    ...
    action = fields.Selection(
        selection_add=[("buy", "Buy")], ondelete={"buy": "cascade"}
    )
    ...
    def _get_lead_days(self, product, **values):
        ...
        if not bypass_delay_description:
            delay_description.append(
                (_("Purchase Security Lead Time"), _("+ %d day(s)", security_delay))
            )
        ...
        return (
            delay + supplier_delay + security_delay + days_to_order,
            delay_description,
        )
    ...
    def _make_po_get_domain(self, company_id, values, partner):
        ...
        group = (
            ...
            or (gpo == "propagate" and "group_id" in values and values["group_id"])
            or False
        )
        ...
        if values.get("orderpoint_id") and delta_days is not False:
            procurement_date = fields.Date.to_date(
                values["date_planned"]
            ) - relativedelta(days=int(values["supplier"].delay))
            ...
            domain += (
                (
                    ...
                    "<=",
                    datetime.combine(
                        procurement_date + relativedelta(days=delta_days),
                        datetime.max.time(),
                    ),
                ),
                (
