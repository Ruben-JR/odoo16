...
class SlidePartnerRelation(models.Model):
    ...
    _sql_constraints = [
        ...
        ("check_vote", "CHECK(vote IN (-1, 0, 1))", "The vote must be 1, 0 or -1."),
    ]
...
class Slide(models.Model):
    ...
    category_id = fields.Many2one(
        "slide.slide", string="Section", compute="_compute_category_id", store=True
    )
    ...
    question_ids = fields.One2many(
        "slide.question", "slide_id", string="Questions", copy=True
    )
    ...
    def _compute_embed_counts(self):
        ...
        if self.ids:
            read_group_res = self.env["slide.embed"]._read_group(
                [("slide_id", "in", self.ids)], ["count_views"], ["slide_id"]
            )
            mapped_data = {
                res["slide_id"][0]: res.get("count_views", 0) for res in read_group_res
            }
    ...
    def _compute_slides_statistics_category(self, read_group_res):
        ...
        slide_categories = (
            self.env["slide.slide"]._fields["slide_category"].get_values(self.env)
        )
        ...
        for slide in self:
            ...
            if slide.slide_category == "video":
                if slide.video_source_type == "youtube":
                    ...
                    embed_code = Markup(
                        '<iframe src="//www.youtube-nocookie.com/embed/%s?%s" allowFullScreen="true" frameborder="0"></iframe>'
                    ) % (slide.youtube_id, query_params)
                elif slide.video_source_type == "google_drive":
                    embed_code = Markup(
                        '<iframe src="//drive.google.com/file/d/%s/preview" allowFullScreen="true" frameborder="0"></iframe>'
                    ) % (slide.google_drive_id)
                ...
                slide_url_external = base_url + url_for(
                    "/slides/embed_external/%s?page=1" % slide.id
                )
                base_embed_code = Markup(
                    '<iframe src="%s" class="o_wslides_iframe_viewer" allowFullScreen="true" height="%s" width="%s" frameborder="0"></iframe>'
                )
                ...
                self.update(
                    {
                        ...
                        and not any(slide[key] for slide in self)
                    }
                )
    ...
    def toggle_active(self):
        ...
        if to_archive:
            to_archive.filtered(
                lambda slide: not slide.is_category
            ).is_published = False
        return res
    ...
    def _get_access_action(self, access_uid=None, force_website=False):
        ...
        return super(Slide, self)._get_access_action(
    ...
    def _post_publication(self):
        for slide in self.filtered(
            lambda slide: slide.website_published
            and slide.channel_id.publish_template_id
        ):
    ...
    def _action_set_quiz_done(self, completed=True):
        ...
        if any(
            ...
            raise UserError(
                ...
                if completed
                    ...
                    "You cannot mark a slide quiz as not completed if you are not among its members."
                )
            )
        ...
        for slide in self:
            ...
            points += (
                gains[user_membership_sudo.quiz_attempts_count - 1]
                if user_membership_sudo.quiz_attempts_count <= len(gains)
                else gains[-1]
            )
        ...
        if data_bytes.startswith(b"%PDF-"):
            try:
                pdf = PyPDF2.PdfFileReader(
                    io.BytesIO(data_bytes), overwriteWarnings=False
                )
