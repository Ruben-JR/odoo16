...
class PaymentProvider(models.Model):
    ...
    code = fields.Selection(
        selection_add=[("payumoney", "PayUmoney")],
        ondelete={"payumoney": "set default"},
    )
    payumoney_merchant_key = fields.Char(
        string="Merchant Key",
        help="The key solely used to identify the account with PayU money",
    ...
    def _payumoney_generate_sign(self, values, incoming=True):
        ...
        if incoming:
            keys = (
                "salt|status||||||udf5|udf4|udf3|udf2|udf1|email|firstname|productinfo|amount|"
                "txnid|key"
            )
