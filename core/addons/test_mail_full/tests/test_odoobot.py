...
class TestOdoobot(TestMailCommon, TestRecipients):
    ...
    def setUpClass(cls):
        ...
        cls.odoobot_ping_body = (
            '<a href="http://odoo.com/web#model=res.partner&amp;id=%s" class="o_mail_redirect" data-oe-id="%s" data-oe-model="res.partner" target="_blank">@OdooBot</a>'
            % (cls.odoobot.id, cls.odoobot.id)
        )
    ...
    def test_fetch_listener(self):
        ...
        odoobot_in_fetch_listeners = self.env["mail.channel.member"].search(
            [("channel_id", "=", channel.id), ("partner_id", "=", odoobot.id)]
        )
        self.assertEqual(
    ...
    def test_odoobot_ping(self):
        ...
        with patch("random.choice", lambda x: x[0]):
            self.assertNextMessage(
                self.test_record_employe.with_context(
    ...
    def test_onboarding_flow(self):
        ...
        last_message = self.assertNextMessage(
            channel.message_post(**kwargs), sender=self.odoobot, answer=("help",)
        )
        ...
        self.assertNextMessage(
            channel.message_post(**kwargs), sender=self.odoobot, answer=("attachment",)
        )
        kwargs["body"] = ""
        attachment = (
    ...
    def test_odoobot_no_default_answer(self):
        ...
        self.assertNextMessage(
            self.test_record_employe.message_post(**kwargs), answer=False
        )
