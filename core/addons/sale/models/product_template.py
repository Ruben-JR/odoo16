...
class ProductTemplate(models.Model):
    ...
    visible_expense_policy = fields.Boolean(
        "Re-Invoice Policy visible", compute="_compute_visible_expense_policy"
    )
    sales_count = fields.Float(
        compute="_compute_sales_count", string="Sold", digits="Product Unit of Measure"
    )
    visible_qty_configurator = fields.Boolean(
        "Quantity visible in configurator", compute="_compute_visible_qty_configurator"
    )
    ...
    def _check_sale_product_company(self):
        ...
        if target_company:  # don't prevent writing `False`, should always work
            product_data = (
                ...
                .search_read([("product_tmpl_id", "in", self.ids)], fields=["id"])
            )
            ...
            if so_lines:
                raise ValidationError(
                    ...
                    % (target_company.name, ", ".join(used_products))
                )
    ...
    def _get_combination_info(
        ...
        has_discounted_price = (
            pricelist or product_template
        ).currency_id.compare_amounts(price_without_discount, price) == 1
