...
class TestChartTemplate(TransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.tax_1_template = cls.create_tax_template(
            "Tax 1", "account.test_tax_1_template", 15
        )
        cls.tax_2_template = cls.create_tax_template(
            "Tax 2", "account.test_tax_2_template", 0
        )

        cls.fiscal_position_template = cls.env[
    ...
    def test_update_taxes_from_templates(self):
        ...
        tax_3_template = self.create_tax_template(
            "Tax 3", "account.test_tax_3_template", 16
        )
        tax_4_template = self.create_tax_template(
            "Tax 4", "account.test_tax_4_template", 17
        )
        ...
        chart_template_xml_id = self.chart_template.get_external_id()[
            self.chart_template.id
        ]
        ...
        self.assertRecordValues(
            fiscal_position.tax_ids.tax_src_id,
            [
                {"name": "Tax 1"},
                {"name": "Tax 3"},
                {"name": "Tax 2"},
        ...
        self.assertRecordValues(
            fiscal_position.tax_ids.tax_dest_id,
            [
                {"name": "Tax 2"},
                {"name": "Tax 1"},
                {"name": "Tax 4"},
    ...
    def test_update_taxes_removed_from_templates(self):
        ...
        self.assertEqual(
            len(
                self.env["account.tax"].search([("company_id", "=", self.company_1.id)])
            ),
            2,
        )
        self.assertEqual(len(fiscal_position.tax_ids), 1)
    ...
    def test_update_taxes_conflict_name(self):
        ...
        self.chart_template.create_record_with_xmlid(
            self.company_1, self.tax_1_template, "account.tax", template_vals
        )
        update_taxes_from_templates(self.env.cr, chart_template_xml_id)
        tax_1_old = self.env["account.tax"].search(
            [
                ("company_id", "=", self.company_1.id),
                ("name", "=", "[old] " + self.tax_1_template.name),
            ]
        )
        tax_1_new = self.env["account.tax"].search(
            [
                ("company_id", "=", self.company_1.id),
                ("name", "=", self.tax_1_template.name),
            ]
        )
        self.assertEqual(
            len(tax_1_old), 1, "Old tax still exists but with a different name."
        )
        self.assertEqual(
