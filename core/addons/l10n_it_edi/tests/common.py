...
class TestItEdi(AccountEdiTestCommon):
    ...
    def setUpClass(
        ...
        cls.edi_simplified_basis_xml = cls._get_test_file_content(
            "IT00470550013_simpl.xml"
        )
    ...
    def _cleanup_etree(self, content, xpaths=None):
        ...
        return self.with_applied_xpath(
            ...
            "".join(
                [
                    f"<xpath expr='{x}' position='replace'>{y}</xpath>"
                    for x, y in xpaths.items()
                ]
            ),
        )

    def _test_invoice_with_sample_file(
        self, invoice, filename, xpaths_file=None, xpaths_result=None
    ):
