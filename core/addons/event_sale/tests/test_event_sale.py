...
class TestEventSale(TestEventSaleCommon):
    ...
    def test_event_crm_sale(self):
        ...
        ticket1_line = customer_so.order_line.filtered(
            lambda line: line.event_ticket_id == ticket1
        )
        ticket2_line = customer_so.order_line.filtered(
        ...
        self.assertEqual(
            set(ticket1_new_reg.mapped("name")), set(["ManualEntry1", "ManualEntry2"])
        )
        self.assertEqual(
    ...
    def test_event_sale_free_confirm(self):
        ...
        # Limiting seats
        self.event_0.write(
            {"auto_confirm": False, "seats_limited": True, "seats_max": 5}
        )
    ...
    def test_event_sale_free_full_event_no_confirm(self):
        ...
        self.event_0.write(
            {"auto_confirm": False, "seats_limited": True, "seats_max": 2}
        )
        ...
        self.assertEqual(len(self.event_0.registration_ids), TICKET_COUNT)
        self.assertTrue(
            all(reg.state == "draft" for reg in self.event_0.registration_ids)
        )
