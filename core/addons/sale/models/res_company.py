...
class ResCompany(models.Model):
    ...
    def action_open_sale_onboarding_payment_provider(self):
        ...
        action = self.env["ir.actions.actions"]._for_xml_id(
            "sale.action_open_sale_payment_provider_onboarding_wizard"
        )
        return action
    ...
    def _mark_payment_onboarding_step_as_done(self):
        ...
        if (
            self.sale_onboarding_payment_method
        ):  # The onboarding step was started from Sales
        ...
        if len(sample_sales_order) == 0:
            ...
            if len(product) == 0:
                default_image_path = get_module_resource(
                    "product", "static/img", "product_product_13-image.jpg"
                )
    ...
    def get_and_update_sale_quotation_onboarding_state(self):
        ...
        return self._get_and_update_onboarding_state(
            "sale_quotation_onboarding_state", steps
        )
