...
class GoogleSync(models.AbstractModel):
    ...
    def write(self, vals):
        ...
        for record in self.filtered("need_sync"):
            if record.google_id:
                record._google_patch(
                    google_service, record.google_id, record._google_values(), timeout=3
                )
    ...
    def _sync_google2odoo(self, google_events: GoogleEvent, default_reminders=()):
        ...
        new_odoo = self.with_context(dont_notify=True)._create_from_google(
            new, odoo_values
        )
    ...
    def _google_error_handling(self, http_error):
        ...
        if http_error.response.status_code in (403, 400):
            ...
            if self._name == "calendar.event":
                start = (
                    self.start
                    and self.start.strftime("%Y-%m-%d at %H:%M")
                    or _("undefined time")
                )
                event_ids = self.id
                name = self.name
                ...
                start = (
                    event.start
                    and event.start.strftime("%Y-%m-%d at %H:%M")
                    or _("undefined time")
                )
                event_ids = _(
                ...
                error_log = (
                    "Error while syncing recurrence [{id} - {name} - {rrule}]: ".format(
                        id=self.id, name=self.name, rrule=self.rrule
                    )
                )
        ...
        with google_calendar_token(self.env.user.sudo()) as token:
            if token:
                try:
                    ...
                    google_id = google_service.insert(
    ...
    def _get_sync_partner(self, emails):
        ...
        partners = [
            user_partner
            for user_partner in user_partners
            if user_partner.type != "private"
        ]
        remaining = [
            email
