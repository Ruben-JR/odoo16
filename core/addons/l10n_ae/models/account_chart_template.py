    ...
    def _load_template(
        ...
        if self == self.env.ref("l10n_ae.uae_chart_template_standard"):
            ifrs_journal = (
                ...
                .search([("company_id", "=", company.id), ("code", "=", "IFRS")])
                .id
            )
            if ifrs_journal:
                ifrs_account_ids = [
                    self.env.ref("l10n_ae.uae_account_100101").id,
            ...
            self.env.ref("l10n_ae.ae_tax_group_5").write(
                {
                    "property_tax_payable_account_id": account_ref.get(
                        self.env.ref("l10n_ae.uae_account_202003").id
                    ),
                    "property_tax_receivable_account_id": account_ref.get(
                        self.env.ref("l10n_ae.uae_account_100103").id
                    ),
                }
            )
