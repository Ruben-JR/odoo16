...
class TestImportModule(odoo.tests.TransactionCase):
    ...
    def test_import_zip(self):
        ...
        files = [
            (
                "foo/__manifest__.py",
                b"{'data': ['data.xml', 'res.partner.csv', 'data.sql']}",
            ),
            (
                "foo/data.xml",
                b"""
            ...
            (
                "foo/data.sql",
                b"INSERT INTO res_partner (active, name) VALUES (true, 'baz');",
            ),
        ...
        self.assertEqual(
            self.env["res.partner"].search_count([("name", "=", "baz")]), 1
        )
    ...
    def test_import_zip_ignore_unexpected_data_extension(self):
        ...
        with self.assertLogs(
            ...
            self.assertIn(
                "module foo: skip unsupported file res.partner.xls",
    ...
    def test_import_module_addons_path(self):
        ...
        with tempfile.TemporaryDirectory() as module_dir:
            for path, data in files:
                os.makedirs(
                    os.path.join(module_dir, os.path.dirname(path)), exist_ok=True
                )
                with open(os.path.join(module_dir, path), "wb") as fp:
                    fp.write(data)
            try:
                __addons_path__.append(module_dir)
                self.env["ir.module.module"]._import_module(
                    "foo", os.path.join(module_dir, "foo")
                )
            finally:
