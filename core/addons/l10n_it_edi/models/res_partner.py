...
class ResPartner(models.Model):
    ...
    def _l10n_it_onchange_vat(self):
        if (
            not self.l10n_it_codice_fiscale
            and self.vat
            and (self.country_id.code == "IT" or self.vat.startswith("IT"))
        ):
            self.l10n_it_codice_fiscale = self._l10n_it_normalize_codice_fiscale(
                self.vat
            )
    ...
    def validate_codice_fiscale(self):
        for record in self:
            if record.l10n_it_codice_fiscale and (
                ...
                raise UserError(
                    _(
                        "Invalid Codice Fiscale '%s': should be like 'MRTMTT91D08F205J' for physical person and '12345670546' or 'IT12345670546' for businesses.",
