...
class MassMailCase(MailCase, MockLinkTracker):
    ...
    def assertMailTraces(
        ...
        for recipient_info, link_info, record in zip(
            recipients_info, mail_links_info, records
        ):
            ...
            recipient_trace = traces.filtered(
                lambda t: (t.email == email or (not email and not t.email))
            ...
            self.assertTrue(
                ...
                % (email, partner, status, record, len(recipient_trace)),
            )
            ...
            if "failure_type" in recipient_info or status in (
                ...
                self.assertEqual(
                    recipient_trace.failure_type, recipient_info["failure_type"]
                )
    ...
    def gateway_mail_click(self, mailing, record, click_label):
        ...
        for _url_href, link_url, _dummy, label in re.findall(
            tools.HTML_TAG_URL_REGEX, email["body"]
        ):
            if (
            ...
            .create(
                {
                    ...
                    "contact_ids": [
                        (
                            0,
                            0,
                            {"name": "Déboulonneur", "email": "fleurus@example.com"},
                        ),
