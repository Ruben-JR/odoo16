...
class Project(models.Model):
    ...
    def _compute_purchase_orders_count(self):
        ...
        query_string = (
            f"{query_string} GROUP BY jsonb_object_keys(analytic_distribution)"
        )

        self._cr.execute(query_string, query_param)
        data = {
            int(record.get("account_id")): record.get("purchase_order_count")
        ...
        return expression.AND(
            [
                ...
                [
                    "|",
                    ("move_line_id", "=", False),
                    ("move_line_id.purchase_line_id", "=", False),
                ],
            ]
        )
    ...
    def _get_profitability_items(self, with_action=True):
        ...
        if self.analytic_account_id:
            ...
            purchase_order_line_read = [
                {
                    **pol,
                    "invoice_lines": self.env["purchase.order.line"]
                    .browse(pol["id"])
                    .invoice_lines,  # One2Many cannot be queried, they are not columns
                }
            ...
            if purchase_order_line_read:
                ...
                currency_ids = {
                    pol["currency_id"]
                    for pol in purchase_order_line_read
                ...
                for pol_read in purchase_order_line_read:
                    purchase_order_line_invoice_line_ids.extend(
                        pol_read["invoice_lines"].ids
                    )
                    price_unit = self.analytic_account_id.currency_id.round(
                        pol_read["price_unit"]
                        * conversion_rates[pol_read["currency_id"]]
                    )
                    analytic_contribution = (
                        pol_read["analytic_distribution"][
                            str(self.analytic_account_id.id)
                        ]
                        / 100.0
                    )
                    amount_invoiced -= (
                        price_unit * pol_read["qty_invoiced"] * analytic_contribution
                    ...
                    if pol_read["qty_to_invoice"] > 0:
                        amount_to_invoice -= (
                            price_unit
                            * pol_read["qty_to_invoice"]
                            * analytic_contribution
                        )
                    else:
                        amount_to_invoice -= (
                            price_unit
                            * (pol_read["product_uom_qty"] - pol_read["qty_invoiced"])
                            * analytic_contribution
                        )
            ...
            if bills_move_line_read:
                ...
                currency_ids = {
                    bml["currency_id"]
                    for bml in bills_move_line_read
                    + [{"currency_id": self.analytic_account_id.currency_id.id}]
                }
                rates = (
                    self.env["res.currency"]
