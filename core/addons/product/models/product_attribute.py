...
class ProductAttribute(models.Model):
    ...
    attribute_line_ids = fields.One2many(
        "product.template.attribute.line", "attribute_id", "Lines"
    )
    ...
    def _compute_products(self):
        for pa in self:
            pa.with_context(
                active_test=False
            ).product_tmpl_ids = pa.attribute_line_ids.product_tmpl_id
            ...
            for pa in self:
                if (
                    ...
                    raise UserError(
                        ...
                        % (
                            pa.display_name,
                            ", ".join(pa.product_tmpl_ids.mapped("display_name")),
                        )
                    )
...
class ProductAttributeValue(models.Model):
    ...
    sequence = fields.Integer(
        string="Sequence", help="Determine the display order", index=True
    )
    ...
    def write(self, values):
        if "attribute_id" in values:
            for pav in self:
                if (
                    pav.attribute_id.id != values["attribute_id"]
                    and pav.is_used_on_products
                ):
                    raise UserError(
                        _(
                            "You cannot change the attribute of the value %s because it is used on the following products:%s"
                        )
                        % (
                            pav.display_name,
                            ", ".join(
        ...
        invalidate = "sequence" in values and any(
            record.sequence != values["sequence"] for record in self
        )
...
class ProductTemplateAttributeLine(models.Model):
    ...
    value_count = fields.Integer(
        compute="_compute_value_count", store=True, readonly=True
    )
        ...
        for ptal in self:
            if ptal.active and not ptal.value_ids:
                raise ValidationError(
                    _(
                        "The attribute %s must have at least one value for the product %s."
                    )
                    % (
    ...
    def create(self, vals_list):
        ...
        res = activated_lines + super(ProductTemplateAttributeLine, self).create(
            create_values
        )
        ...
        if "attribute_id" in values:
            for ptal in self:
                if ptal.attribute_id.id != values["attribute_id"]:
                    raise UserError(
                        _(
                            "On the product %s you cannot transform the attribute %s into the attribute %s."
                        )
                        % (
                            ptal.product_tmpl_id.display_name,
                            ptal.attribute_id.display_name,
...
class ProductTemplateAttributeValue(models.Model):
    ...
    attribute_line_id = fields.Many2one(
        "product.template.attribute.line", required=True, ondelete="cascade", index=True
    )
    ...
    html_color = fields.Char(
        "HTML Color Index", related="product_attribute_value_id.html_color"
    )
    is_custom = fields.Boolean(
        "Is custom value", related="product_attribute_value_id.is_custom"
    )
    display_type = fields.Selection(
        related="product_attribute_value_id.display_type", readonly=True
    )
    ...
    def _check_valid_values(self):
        for ptav in self:
            if ptav.product_attribute_value_id not in ptav.attribute_line_id.value_ids:
                raise ValidationError(
                    _(
                        "The value %s is not defined for the attribute %s on the product %s."
                    )
                    % (
                        ptav.product_attribute_value_id.display_name,
                        ptav.attribute_id.display_name,
        ...
        if pav_in_values or product_in_values:
            for ptav in self:
                if (
                    ...
                    raise UserError(
                        _(
                            "You cannot change the value of the value %s set on product %s."
                        )
                ...
                if (
                    ...
                    raise UserError(
                        _(
                            "You cannot change the product of the value %s set on product %s."
                        )
                        % (ptav.display_name, ptav.product_tmpl_id.display_name)
                    )
    ...
    def _compute_name(self):
        for record in self:
            ...
            if record.custom_product_template_attribute_value_id.display_name:
                name = "%s: %s" % (
                    record.custom_product_template_attribute_value_id.display_name,
                    name,
                )
            record.name = name
