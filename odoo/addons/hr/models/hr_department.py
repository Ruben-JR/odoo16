...
class Department(models.Model):
    ...
    def _compute_plan_count(self):
        ...
        plans_count = {
            x["department_id"][0]: x["department_id_count"] for x in plans_data
        }
        for department in self:
    ...
    def create(self, vals_list):
        ...
        departments = super(
            Department, self.with_context(mail_create_nosubscribe=True)
        ).create(vals_list)
    ...
    def get_formview_action(self, access_uid=None):
        ...
        if not self.user_has_groups("hr.group_hr_user") and self.env.context.get(
