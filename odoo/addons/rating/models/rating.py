...
class Rating(models.Model):
    ...
    def _compute_res_name(self):
        for rating in self:
            ...
            rating.res_name = (
                name and name[0][1] or ("%s/%s") % (rating.res_model, rating.res_id)
            )
        ...
        for rating in self:
            ...
            if rating.parent_res_model and rating.parent_res_id:
                ...
                name = (
                    name
                    and name[0][1]
                    or ("%s/%s") % (rating.parent_res_model, rating.parent_res_id)
                )
        ...
        for rating in self:
            try:
                image_path = get_resource_path(
                    "rating", "static/src/img", rating._get_rating_image_filename()
                )
                rating.rating_image_url = (
                    "/rating/static/src/img/%s" % rating._get_rating_image_filename()
                )
                rating.rating_image = (
