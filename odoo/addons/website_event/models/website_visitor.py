...
class WebsiteVisitor(models.Model):
    ...
    @api.depends(
        "event_registration_ids.email",
        "event_registration_ids.mobile",
        "event_registration_ids.phone",
    )
    def _compute_email_phone(self):
        ...
        for visitor in self.filtered(
            ...
            if not visitor.mobile:
                visitor.mobile = next(
                    (
                        reg.mobile or reg.phone
                        for reg in linked_registrations
                        if reg.mobile or reg.phone
                    ),
                    False,
                )
    ...
    def _search_event_registered_ids(self, operator, operand):
        ...
        if operator == "not in":
            raise NotImplementedError(
                "Unsupported 'Not In' operation on visitors registrations"
            )
