...
class ChatbotScriptStep(models.Model):
    ...
    def _compute_is_forward_operator_child(self):
        ...
        for chatbot in self.chatbot_script_id:
            parent_steps_by_chatbot[chatbot.id] = chatbot.script_step_ids.filtered(
                lambda step: step.step_type
                in ["forward_operator", "question_selection"]
        ...
        for step in self:
            ...
            step.is_forward_operator_child = (
                parent and parent.step_type == "forward_operator"
            )
    ...
    def _chatbot_prepare_customer_values(
        self, mail_channel, create_partner=True, update_partner=True
    ):
    ...
    def _process_step(self, mail_channel):
        ...
        return mail_channel._chatbot_post_message(
            self.chatbot_script_id, plaintext2html(self.message)
        )
    ...
    def _process_step_forward_operator(self, mail_channel):
        ...
        if human_operator and human_operator != self.env.user:
            ...
            mail_channel._chatbot_post_message(
                ...
                % _("%s has joined", human_operator.partner_id.name),
            )
    ...
    def _format_for_frontend(self):
        ...
        return {
            ...
            "chatbot_step_message": plaintext2html(self.message)
            if not is_html_empty(self.message)
            else False,
