...
class TestControllers(tests.HttpCase):
    ...
    def test_last_created_pages_autocompletion(self):
        ...
        res = self.opener.post(
            url=suggested_links_url, json={"params": {"needle": "/"}}
        )
        ...
        last_modified_values = map(
            lambda o: o["value"], last_modified_history["values"]
        )

        matching_pages = set(
            map(lambda o: o["value"], suggested_links["matching_pages"])
        )
        self.assertEqual(
    ...
    def test_02_client_action_iframe_url(self):
        ...
        for url in urls:
            ...
            self.assertEqual(
                resp.url,
                base_url + url,
                "Public user should have landed in the frontend",
            )
        self.authenticate("admin", "admin")
        for url in urls:
            resp = self.url_open(f"/@{url}")
            backend_params = url_encode(
                dict(action="website.website_preview", path=url)
            )
            self.assertEqual(
                resp.url,
                f"{base_url}/web#{backend_params}",
