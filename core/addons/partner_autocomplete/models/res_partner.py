...
class ResPartner(models.Model):
    ...
    def _iap_replace_location_codes(self, iap_data):
        country_code, country_name = iap_data.pop("country_code", False), iap_data.pop(
            "country_name", False
        )
        state_code, state_name = iap_data.pop("state_code", False), iap_data.pop(
            "state_name", False
        )
    ...
    def autocomplete(self, query, timeout=15):
        suggestions, _ = self.env["iap.autocomplete.api"]._request_partner_autocomplete(
            "search",
            {
                "query": query,
            },
    ...
    def _is_vat_syncable(self, vat):
        ...
        return self._is_company_in_europe(vat_country_code) and (
            partner_country_code == vat_country_code or not partner_country_code
        )

    def _is_synchable(self):
        already_synched = self.env["res.partner.autocomplete.sync"].search(
