...
class AccountPayment(models.Model):
    ...
    def action_cancel(self):
        ...
        for payment in self:
            if payment.expense_sheet_id:
                ...
                paid_by_employee = (
                    payment.expense_sheet_id.payment_mode == "own_account"
                )
                payment.expense_sheet_id.state = (
                    "post" if paid_by_employee else "approve"
                )
    ...
    def _creation_message(self):
        ...
        if self.move_id.expense_sheet_id:
            return _(
                "Payment created for: %s",
                self.move_id.expense_sheet_id._get_html_link(),
            )
