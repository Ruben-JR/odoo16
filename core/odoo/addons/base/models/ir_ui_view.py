...
def transfer_node_to_modifiers(node, modifiers):
    ...
    for attr in ("invisible", "readonly", "required"):
        ...
        if value_str:
            ...
            if value or (
                attr not in modifiers or not isinstance(modifiers[attr], list)
            ):
...
* if False, the view currently does not extend its parent but can be enabled
                ...
                if fullpath:
                    ...
                    if arch_fs:
                        ...
                        translation_dictionary = (
                            field_arch_db.get_translation_dictionary(
                                view.with_env(env_en).arch_db,
                                {lang: view.with_env(env_lang).arch_db},
                            )
                        )
                        arch_fs = field_arch_db.translate(
                            lambda term: translation_dictionary[term][lang], arch_fs
                        )
                else:
                    _logger.warning(
                        "View %s: Full path [%s] cannot be found.", xml_id, view.arch_fs
                    )
                    arch_fs = False
    ...
    def _check_groups(self):
        for view in self:
            if view.groups_id and view.inherit_id and view.mode != "primary":
                raise ValidationError(
                    _(
    ...
    def create(self, vals_list):
            ...
            if "arch" in values:
                ...
                if "install_filename" in self._context:
                    ...
                    path_info = get_resource_from_path(
                        self._context["install_filename"]
                    )
                    if path_info:
        ...
        self.clear_caches()
        result = super(
            View, self.with_context(ir_ui_view_partial_validation=True)
        ).create(vals_list)
    ...
    def write(self, vals):
        ...
        if (
            ...
            and "install_filename" not in self._context
        ):
    ...
    def _get_inheriting_views(self):
        ...
        self._flush_search(
            domain, fields=["inherit_id", "priority", "model", "mode"], order="id"
        )
        query = f"""
    ...
    def _filter_loaded_views(self):
        ...
        loaded_modules = tuple(self.pool._init_modules) + (
            self._context.get("install_module"),
        )
        query = self._get_filter_xmlid_query()
        self.env.cr.execute(
            query, {"res_ids": tuple(ids_to_check), "modules": loaded_modules}
        )
    ...
    def _log_view_warning(self, message, node):
        ...
        _logger.warning(
            "%s\nView error context:\n%s", message, pprint.pformat(error_context)
        )
    ...
    def read_combined(self, fields=None):
        ...
        * Returns the view with all requested fields
        ...
        """
        warnings.warn(
            "use get_combined_arch() instead", DeprecationWarning, stacklevel=2
        )
        if fields:
        ...
        for node in tree.xpath("//*[@model_access_rights]"):
            ...
            if node.tag == "field":
                ...
                for action, operation in (
                    ...
                    if (
                        not node.get(action)
                        and not model.check_access_rights(
    ...
    def _postprocess_context_dependent(self, tree):
        ...
        for node in tree.xpath("//*[@context-dependent-modifiers]"):
            ...
            for attr, value in json.loads(
                node.attrib.pop("context-dependent-modifiers")
            ).items():
    ...
    def _get_x2many_missing_view_archs(self, field, field_node, node_info):
        ...
        """
        current_view_types = [
            el.tag for el in field_node.xpath("./*[descendant::field]")
        ]
        missing_view_types = []
        if not any(
            view_type in current_view_types
            for view_type in field_node.get("mode", "kanban,tree").split(",")
        ):
            missing_view_types.append(
                field_node.get(
                    "mode", "kanban" if node_info.get("mobile") else "tree"
                ).split(",")[0]
            )
        ...
        for additional_field in (
            ...
            if node.get(additional_field):
                name_manager.has_field(
                    node, node.get(additional_field).split(".", 1)[0]
                )
        for f in node:
            if f.tag == "filter":
    ...
    def _postprocess_tag_groupby(self, node, name_manager, node_info):
        ...
        self._postprocess_view(
            node, field.comodel_name, editable=False, parent_name_manager=name_manager
        )
    ...
    def _validate_tag_graph(self, node, name_manager, node_info):
        ...
        for child in node.iterchildren(tag=etree.Element):
            if child.tag != "field" and not isinstance(child, etree._Comment):
                msg = _(
                    "A <graph> can only contains <field> nodes, found a <%s>", child.tag
                )
                self._raise_view_error(msg, child)
    ...
    def _validate_tag_filter(self, node, name_manager, node_info):
        ...
        if domain:
            ...
            fnames, vnames = self._get_domain_identifiers(node, domain, desc)
            self._check_field_paths(
                node, fnames, name_manager.model._name, f"{desc} ({domain})"
            )
            if vnames:
    ...
    def _validate_tag_button(self, node, name_manager, node_info):
        ...
        if special:
            if special not in ("cancel", "save", "add"):
                self._raise_view_error(
                    _("Invalid special '%(value)s' in button", value=special), node
                )
        elif type_:
            if type_ == "edit":  # list_renderer, used in kanban view
                ...
                try:
                    ...
                    model, action_id = self.env[
                        "ir.model.data"
                    ]._xmlid_to_res_model_res_id(name, raise_if_not_found=False)
                    if not action_id:
                        msg = _(
                        ...
                        self._raise_view_error(msg, node)
                    if not issubclass(
                        self.pool[model], self.pool["ir.actions.actions"]
                    ):
                        msg = _(
            ...
            if attr in ("class", "t-att-class", "t-attf-class"):
                ...
                for key, val_ast in get_dict_asts(expr).items():
                    if isinstance(val_ast, ast.List):
                        ...
                        fnames, vnames = self._get_domain_identifiers(
                            node, val_ast, attr, expr
                        )
                        name_manager.must_have_fields(
                            node, fnames | vnames, f"attrs ({expr})"
                        )
                    else:
                        vnames = get_variable_names(val_ast)
                        if vnames:
                            name_manager.must_have_fields(
                                node, vnames, f"attrs ({expr})"
                            )
                ...
                for group in expr.replace("!", "").split(","):
                    ...
                    if not self.env["ir.model.data"]._xmlid_to_res_id(
                        group.strip(), raise_if_not_found=False
                    ):
    ...
    def _validate_classes(self, node, expr):
        ...
        if any(klass.startswith("btn") for klass in classes):
            if node.tag in ("a", "button", "select"):
                pass
            elif node.tag == "input" and node.get("type") in (
                "button",
                "submit",
                "reset",
            ):
                ...
                klass in classes for klass in ("btn-group", "btn-toolbar", "btn-ship")
            ):
                pass
            else:
                msg = (
    ...
    def _get_domain_identifiers(self, node, domain, use, expr=None):
        try:
            ...
            msg = _(
                "Invalid domain format %(expr)s in %(use)s",
                expr=expr or domain,
                use=use,
            )
            self._raise_view_error(msg, node)
    ...
    def _get_view_id(self, template):
        ...
        res_model, res_id = self.env["ir.model.data"]._xmlid_to_res_model_res_id(
            template, raise_if_not_found=True
        )
        assert res_model == self._name, "Call _get_view_id, expected %r, got %r" % (
            self._name,
            res_model,
        )
        return res_id
    ...
    def _get_specific_views(self):
        ...
        return (
            ...
            .search([("key", "=", self.key)])
            .filtered(lambda r: not r.xml_id == r.key)
        )
...
class ResetViewArchWizard(models.TransientModel):
    ...
    def default_get(self, fields):
        view_ids = (
            self._context.get("active_model") == "ir.ui.view"
...
class Model(models.AbstractModel):
    ...
    def get_views(self, views, options=None):
        ...
        result["views"] = {
            v_type: self.get_view(
                v_id, v_type if v_type != "list" else "tree", **options
            )
    ...
    def _get_view_fields(self, view_type, models):
        ...
        if view_type in ("kanban", "tree", "form"):
            ...
            models[self._name].union(
                fname for fname, field in self._fields.items() if field.groupable
            )
        return models
...
class NameManager:
    ...
    def __init__(self, model, parent=None):
        ...
        self.mandatory_fields = collections.defaultdict(
            dict
        )  # {field_name: {'groups': 'use}}
        self.mandatory_parent_fields = collections.defaultdict(
            dict
        )  # {field_name: {'groups': use}}
    ...
    def field_info(self):
        field_info = self.model.fields_get(
            attributes=["invisible", "states", "readonly", "required"]
        )
        has_access = functools.partial(
            self.model.check_access_rights, raise_exception=False
        )
    ...
    def _get_node_groups(self, node):
        return tuple(
            tuple(n.get("groups").split(","))
            for n in node.xpath("ancestor-or-self::*[@groups]")
        )
    ...
    def check(self, view):
        ...
        for name, groups_uses in self.mandatory_fields.items():
            ...
            def combinate(groups):
                ...
                negative = tuple(
                    group for group in groups[0] if group and group.startswith("!")
                )
                combinations = (
                    [negative + p for p in positive] if positive else [negative]
                )
                if len(groups) > 1:
                    combinations = [
                        group + tuple(c)
                        for c in combinate(groups[1:])
                        for group in combinations
                    ]
                return [
                    set(combination)
                    for combination in set(
                        tuple(combination) for combination in combinations
                    )
                ]
            ...
            for mandatory_for_groups, use in groups_uses.items():
                ...
                if (
                    ...
                    and not any(
                        {
                            group[1:] if group.startswith("!") else "!" + group
                    ...
                    and not all(
                        any(
                            ...
                            for combination in (
                                ...
                                for group in mandatory_combination
                                if group and not group.startswith("!")
                                for trans_group in view.env.ref(group)
                                .trans_implied_ids.get_external_id()
                                .values()
                            )
                        )
                        for mandatory_combination in mandatory_combinations
                    ...
                    and not all(
                        any(
                            ...
                            for combination in (
                                ...
                                for group in available_combination
                                if group and group.startswith("!")
                                for trans_group in view.env.ref(group[1:])
                                .trans_implied_ids.get_external_id()
                                .values()
                            )
                        )
