...
class HrEmployeeBase(models.AbstractModel):
    ...
    job_title = fields.Char(
        "Job Title", compute="_compute_job_title", store=True, readonly=False
    )
    ...
    work_phone = fields.Char(
        "Work Phone", compute="_compute_phones", store=True, readonly=False
    )
    ...
    related_contact_ids = fields.Many2many(
        "res.partner", "Related Contacts", compute="_compute_related_contacts"
    )
    related_contacts_count = fields.Integer(
        "Number of related contacts", compute="_compute_related_contacts_count"
    )
    ...
    def _compute_part_of_department(self):
        ...
        if not active_department:
            ...
            for employee in self:
                employee.member_of_department = (
                    employee.department_id in child_departments
                )
    ...
    def _search_part_of_department(self, operator, value):
        ...
        return (["!"] if operator == "!=" else []) + [
            ("department_id", "child_of", self.env.user.employee_id.department_id.id)
        ]
    ...
    def _compute_presence_state(self):
        ...
        check_login = literal_eval(
            ...
            .sudo()
            .get_param("hr.hr_presence_control_login", "False")
        )
        employee_to_check_working = self.filtered(
            lambda e: e.user_id.im_status == "offline"
        )
        ...
        for employee in self:
            ...
            if check_login:
                if employee.user_id.im_status in ["online", "leave_online"]:
                    ...
                    employee.user_id.im_status in ["offline", "leave_offline"]
    ...
    def _compute_presence_icon(self):
        ...
        """
        working_now_list = self.filtered(
            lambda e: e.hr_presence_state == "present"
    ...
    def _compute_work_location_id(self):
        to_reset = self.filtered(
            lambda e: e.address_id != e.work_location_id.address_id
        )
