...
class ProductFetchImageWizard(models.TransientModel):
    ...
    _description = (
        "Fetch product images from Google Images based on the product's barcode number."
    )
    ...
    def default_get(self, fields_list):
        ...
        google_custom_search_key_is_set = bool(
        ...
        products_to_process = product_ids.filtered(
            lambda p: not p.image_1920 and p.barcode
        )
    ...
    nb_products_selected = fields.Integer(
        string="Number of selected products", readonly=True
    )
    ...
    nb_products_to_process = fields.Integer(
        string="Number of products to process", readonly=True
    )
    ...
    def action_fetch_image(self):
        ...
        :rtype: dict
        """
        self.products_to_process.image_fetch_pending = (
            True  # Flag products to process for the cron
        )

        # Process the first 10 products immediately
        matching_images_count = self._process_products(
    ...
    def _get_products_to_process(self, limit=10000):
        ...
        """
        products_to_process = self.products_to_process or self.env[
            "product.product"
        ].search([("image_fetch_pending", "=", True)], limit=limit)
        return products_to_process.filtered(
            ...
            and p.barcode
            and p.image_fetch_pending
        )[
            :limit
        ]  # Apply the limit after the filter with self.products_to_process for more results
        ...
        for product in products_to_process:
            ...
            try:
                ...
                if response.status_code == requests.codes.forbidden:
                    ...
                    raise UserError(
                        _("Your API Key or your Search Engine ID is incorrect.")
                    )
            ...
            if int(response_content["searchInformation"]["totalResults"]) > 0:
                for item in response_content[
                    "items"
                ]:  # Only populated if totalResults > 0
                    try:
