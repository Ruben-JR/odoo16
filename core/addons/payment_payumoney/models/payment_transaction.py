...
class PaymentTransaction(models.Model):
    ...
    def _get_specific_rendering_values(self, processing_values):
        ...
        payumoney_values = {
            ...
            "return_url": urls.url_join(
                self.get_base_url(), PayUMoneyController._return_url
            ),
            "api_url": api_url,
        }
    ...
    def _get_tx_from_notification_data(self, provider_code, notification_data):
        ...
        if not reference:
            raise ValidationError(
                "PayUmoney: "
                + _("Received data with missing reference (%s)", reference)
            )

        tx = self.search(
            [("reference", "=", reference), ("provider_code", "=", "payumoney")]
        )
        if not tx:
            raise ValidationError(
                "PayUmoney: "
    ...
    def _process_notification_data(self, notification_data):
        ...
        if status == "success":
            ...
            self._set_error(
                "PayUmoney: "
                + _("The payment encountered an error with code %s", error_code)
            )
