...
class TestEventPerformance(EventPerformanceCase):
    ...
    def test_event_create_batch_notype(self):
        ...
        with freeze_time(self.reference_now), self.assertQueryCount(
            event_user=335
        ):  # tef 335
            self.env.cr._now = (
                self.reference_now
...
class TestRegistrationPerformance(EventPerformanceCase):
    ...
    def test_registration_create_batch(self):
        ...
        with freeze_time(self.reference_now), self.assertQueryCount(
            event_user=720
        ):  # tef only: 674? - com runbot 716 - ent runbot 719
            self.env.cr._now = (
                self.reference_now
            )  # force create_date to check schedulers
            registration_values = [
