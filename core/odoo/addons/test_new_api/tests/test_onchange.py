...
class TestOnChange(SavepointCaseWithUserDemo):
    ...
    def test_onchange_one2many_multi(self):
        ...
        values = {
            ...
            "lines": [
                ...
                Command.create(
                    {"name": False, "partner": False, "tags": [Command.clear()]}
                ),
            ],
        }
        self.env.invalidate_all()
        ...
        result = multi.onchange(
            values, "partner", dict(field_onchange, **{"lines.multi": None})
        )
        self.assertEqual(result["value"], expected_value)
    ...
    def test_onchange_specific(self):
        ...
        self.assertItemsEqual(
            ...
            [Command.clear()]
            + [Command.link(user.id) for user in discussion.participants + demo],
        )
    ...
    def test_onchange_one2many_with_domain_on_related_field(self):
        ...
        view_info = self.Discussion.get_view(
            self.env.ref("test_new_api.discussion_form").id, "form"
        )
        ...
        values = {
            ...
            "participants": [Command.link(usr.id) for usr in discussion.participants],
            "important_messages": [
                Command.link(msg.id) for msg in discussion.important_messages
            ],
            "important_emails": [
                Command.link(eml.id) for eml in discussion.important_emails
            ],
        }
        self.env.invalidate_all()
    ...
    def test_onchange_one2many_many2one_in_form(self):
        ...
        result = self.env["test_new_api.monetary_order_line"].onchange(
            values, [], field_onchange
        )
    ...
    def test_onchange_inherited(self):
        ...
        foo, bar = self.env["test_new_api.multi.tag"].create(
            [{"name": "Foo"}, {"name": "Bar"}]
        )
...
class TestComputeOnchange(common.TransactionCase):
    ...
    def test_onchange_once(self):
        ...
        form = Form(
            self.env["test_new_api.compute.onchange"].with_context(default_foo="oof")
        )
        record = form.save()
