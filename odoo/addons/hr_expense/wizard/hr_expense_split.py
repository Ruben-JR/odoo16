...
class HrExpenseSplit(models.TransientModel):
    ...
    amount_tax = fields.Monetary(
        string="Tax amount in Currency", compute="_compute_amount_tax"
    )
    ...
    def _compute_from_product_id(self):
        for split in self:
            ...
            if split.product_has_cost:
                split.total_amount = split.product_id.price_compute(
    ...
    def _onchange_product_id(self):
        ...
        self.tax_ids = (
            ...
            else self.product_id.supplier_taxes_id.filtered(
    ...
    def _compute_product_has_tax(self):
        for split in self:
            split.product_has_tax = (
                split.product_id
                and split.product_id.supplier_taxes_id.filtered(
                    lambda tax: tax.company_id == split.company_id
                )
            )
    ...
    def _get_values(self):
        ...
        vals = {
            ...
            "unit_amount": self.product_id.price_compute(
                "standard_price", currency=self.currency_id
            )[self.product_id.id],
        }
