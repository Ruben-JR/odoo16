...
class AssetsBundle(object):
    ...
    def __init__(self, name, files, env=None, css=True, js=True):
        ...
        self.user_direction = (
            self.env["res.lang"]
            ._lang_get(self.env.context.get("lang") or self.env.user.lang)
            .direction
        )
    ...
    def to_node(
        ...
        if css and self.stylesheets:
            ...
            if self.css_errors:
                ...
                response.append(
                    JavascriptAsset(self, inline=self.dialog_message(msg)).to_node()
                )
                response.append(
                    StylesheetAsset(
                        self, url="/web/static/lib/bootstrap/dist/css/bootstrap.css"
                    ).to_node()
                )
        ...
        if js and self.javascripts:
            ...
            src = (
                self.get_debug_asset_url(name=js_attachment.name, extension="")
                if is_debug_assets
                else js_attachment[0].url
            )
            attr = dict(
                [
    ...
    def last_modified_combined(self):
        ...
        assets = [
            WebAsset(self, url=f["url"], filename=f["filename"], inline=f["content"])
            for f in self.files
            if f["atype"]
            in [
                "text/sass",
    ...
    def checksum(self):
        ...
        """
        check = "%s%s" % (
            json.dumps(self.files, sort_keys=True),
            self.last_modified_combined,
        )
    ...
    def _get_asset_url_values(
        self, id, unique, extra, name, sep, extension
    ):  # extra can contain direction or/and website
        return {
            "id": id,
            "unique": unique,
            "extra": extra,
    ...
    def _unlink_attachments(self, attachments):
        ...
        """
        to_delete = set(
            attach.store_fname for attach in attachments if attach.store_fname
        )
        self.env.cr.execute(
            f"""DELETE FROM {attachments._table} WHERE id IN (
    ...
    def clean_attachments(self, extension):
        ...
        url = self.get_asset_url(
            ...
            % (
                "rtl/"
                if extension in ["css", "min.css"] and self.user_direction == "rtl"
                else ""
            ),
            name=self.name,
            sep="",
    ...
    def save_attachment(self, extension, content):
        ...
        if self.env and "bus.bus" in self.env and self.name in self.TRACKED_BUNDLES:
            ...
            _logger.debug(
                "Asset Changed: bundle: %s -- version: %s", self.name, self.version
            )
    ...
    def js(self, is_minified=True):
        ...
        if not js_attachment:
            ...
            if self.templates:
                ...
                template_bundle = textwrap.dedent(
                    f"""
        ...
        generator = SourceMapGenerator(
            source_root="/".join(
                [
                    ".."
                    for i in range(
                        0, len(self.get_debug_asset_url(name=self.name).split("/")) - 2
                    )
                ]
            )
            + "/",
            ...
            for template_tree in list(content_templates_tree):
                ...
                if "t-inherit" in template_tree.attrib:
                    ...
                    if inherit_mode not in ["primary", "extension"]:
                        raise ValueError(
                            _(
                                "Invalid inherit mode. Module %r and template name %r",
                                addon,
                                template_name,
                            )
                        )
                    ...
                    parent_addon, parent_name = (
                        split_name_attempt
                        if len(split_name_attempt) == 2
                        else (addon, parent_dotted_name)
                    )
                    if parent_addon not in template_dict:
                        if parent_dotted_name in template_dict[addon]:
                            ...
                            raise ValueError(
                                _(
                                    "Module %r not loaded or inexistent (try to inherit %r), or templates of addon being loaded %r are misordered (template %r)",
                                    parent_addon,
                                    parent_name,
                                    addon,
                                    template_name,
                    ...
                    if parent_name not in template_dict[parent_addon]:
                        raise ValueError(
                            _(
                                "Cannot create %r because the template to inherit %r is not found."
                            )
                            % (
                                f"{addon}.{template_name}",
                                f"{parent_addon}.{parent_name}",
                            )
                        )
                    ...
                    if show_inherit_info:
                        ...
                        for item in template_tree:
                            ...
                            comment = etree.Comment(
                                f""" Filepath: {asset.url} ; position="{position}" ; {attrib} """
                            )
                            if position == "attributes":
                                if item.get("expr"):
                                    comment_node = etree.Element(
                                        "xpath",
                                        {
                                            "expr": item.get("expr"),
                                            "position": "before",
                    ...
                    if inherit_mode == "primary":  # New template_tree: A' = B(A)
                        ...
                        if not template_name:
                            raise ValueError(
                                _("Template name is missing in file %r.", asset.url)
                            )
                        template_dict[addon][template_name] = (
                            inherited_template,
                            parent_urls + [asset.url],
                        )
                    else:  # Modifies original: A = B(A)
                        template_dict[parent_addon][parent_name] = (
                            inherited_template,
                            parent_urls + [asset.url],
                        )
                elif template_name:
                    if template_name in template_dict[addon]:
                        raise ValueError(
                            _(
                                "Template %r already exists in module %r",
                                template_name,
                                addon,
                            )
                        )
                    ...
                    template_name = "%s__extend_%s" % (
                        template_tree.attrib.get("t-extend"),
                        len(template_dict[addon]),
                    )
                    template_dict[addon][template_name] = (template_tree, [asset.url])
                else:
                    raise ValueError(
    ...
    def css_with_sourcemap(self, content_import_rules):
        ...
        debug_asset_url = self.get_debug_asset_url(
            name=self.name, extra="rtl/" if self.user_direction == "rtl" else ""
        )
        generator = SourceMapGenerator(
            source_root="/".join(
    ...
    def is_css_preprocessed(self):
        ...
        for atype in asset_types:
            outdated = False
            assets = dict(
                (asset.html_url, asset)
                for asset in self.stylesheets
                if isinstance(asset, atype)
            )
            if assets:
                assets_domain = self._get_assets_domain_for_already_processed_css(
                    assets
                )
        ...
        if self.stylesheets:
            ...
            if self.user_direction == "rtl":
                stylesheet_assets = [
                    ...
                    if not isinstance(
                        asset,
                        (SassStylesheetAsset, ScssStylesheetAsset, LessStylesheetAsset),
                    )
                ]
                compiled += "\n".join(
            ...
            while fragments:
                ...
                asset = next(
                    asset for asset in self.stylesheets if asset.id == asset_id
                )
    ...
    def compile_css(self, compiler, source):
        ...
        compiled = re.sub(
            r"(appearance: (\w+);)",
            r"-webkit-appearance: \2; -moz-appearance: \2; \1",
            compiled,
        )

        # Most of those are only useful for wkhtmltopdf (some for old PhantomJS)
        compiled = re.sub(
            r"(display: ((?:inline-)?)flex((?: ?!important)?);)",
            r"display: -webkit-\2box\3; display: -webkit-\2flex\3; \1",
            compiled,
        )
        compiled = re.sub(
            r"(justify-content: flex-(\w+)((?: ?!important)?);)",
            r"-webkit-box-pack: \2\3; \1",
            compiled,
        )
        compiled = re.sub(
            r"(flex-flow: (\w+ \w+);)", r"-webkit-flex-flow: \2; \1", compiled
        )
        compiled = re.sub(
        ...
        compiled = re.sub(
            r"(flex-wrap: (\w+);)", r"-webkit-flex-wrap: \2; \1", compiled
        )
        compiled = re.sub(
            r"(flex: ((\d)+ \d+ (?:\d+|auto));)",
            r"-webkit-box-flex: \3; -webkit-flex: \2; \1",
            compiled,
        )
    ...
    def run_rtlcss(self, source):
        ...
        try:
            ...
            try:
                process = Popen(["rtlcss", "--version"], stdout=PIPE, stderr=PIPE)
    ...
    def get_preprocessor_error(self, stderr, source=None):
        ...
        error += (
            "This error occurred while compiling the bundle '%s' containing:"
            % self.name
        )
        for asset in self.stylesheets:
...
class PreprocessedCSS(StylesheetAsset):
    ...
    def __init__(self, *args, **kw):
        ...
        self.html_url_format = "%%s/%s%s/%%s.css" % (
            "rtl/" if self.direction == "rtl" else "",
            self.bundle.name,
        )
