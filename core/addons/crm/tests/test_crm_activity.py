...
class TestCrmMailActivity(TestCrmCommon):
    ...
    def test_crm_activity_ordering(self):
        ...
        )  # force updating this test is order changes
        ...
        # 6+              /             0/1/2/0    /
        today = date.today()
        deadline_in2d, deadline_in1d = today + timedelta(days=2), today + timedelta(
            days=1
        )
        deadline_was2d, deadline_was1d = today + timedelta(days=-2), today + timedelta(
            days=-1
        )
        deadlines_my = [
            deadline_in2d,
        ...
        test_leads[0:3].activity_schedule(
            act_type_xmlid="crm.initial_contact", date_deadline=deadline_in2d
        )
        test_leads[5].activity_schedule(
            act_type_xmlid="crm.initial_contact", date_deadline=deadline_in2d
        )
        (test_leads[1] | test_leads[3]).activity_schedule(
            act_type_xmlid="crm.initial_contact", date_deadline=deadline_was1d
        )
        (test_leads[2] | test_leads[4]).activity_schedule(
            act_type_xmlid="crm.call_for_demo", date_deadline=deadline_was2d
        )
    ...
    def test_crm_activity_next_action(self):
        ...
        self.assertEqual(
            self.lead_1.activity_summary, activity.activity_type_id.summary
        )
        self.assertEqual(self.lead_1.activity_type_id, activity.activity_type_id)
        # self.assertEqual(fields.Datetime.from_string(self.lead.activity_date_deadline), datetime.now() + timedelta(days=activity.activity_type_id.days))
