...
class Forum(models.Model):
    ...
    count_flagged_posts = fields.Integer(
        string="Number of flagged posts", compute="_compute_count_flagged_posts"
    )
    # karma generation
    ...
    def _compute_forum_statistics(self):
        ...
        for res_group in read_group_res:
            ...
            result[cid]["total_favorites"] += (
                1 if res_group.get("favourite_count", 0) else 0
            )
    ...
    def _compute_count_posts_waiting_validation(self):
        for forum in self:
            ...
            forum.count_posts_waiting_validation = self.env["forum.post"].search_count(
                domain
            )
        ...
        for tag in (tag for tag in tags.split(",") if tag):
            if tag.startswith("_"):  # it's a new tag
                ...
                if tag_ids:
                    ...
                    if (
                        user.exists()
                        and user.karma >= self.karma_tag_create
                        and len(tag)
                        and len(tag[1:].strip())
                    ):
    ...
    def _update_website_count(self):
        for website in self.env["website"].sudo().search([]):
            website.forums_count = (
                self.env["forum.forum"].sudo().search_count(website.website_domain())
            )
...
class Post(models.Model):
    ...
    plain_content = fields.Text(
        "Plain Content", compute="_get_plain_content", store=True
    )
    tag_ids = fields.Many2many(
        "forum.tag", "forum_tag_rel", "forum_id", "forum_tag_id", string="Tags"
    )
    ...
    favourite_count = fields.Integer(
        "Favorite", compute="_get_favorite_count", store=True
    )
    ...
    self_reply = fields.Boolean(
        "Reply to own question", compute="_is_self_reply", store=True
    )
    ...
    has_validated_answer = fields.Boolean(
        "Is answered", compute="_get_has_validated_answer", store=True
    )
    ...
    karma_edit = fields.Integer(
        "Karma to edit", compute="_get_post_karma_rights", compute_sudo=False
    )
    karma_close = fields.Integer(
        "Karma to close", compute="_get_post_karma_rights", compute_sudo=False
    )
    karma_unlink = fields.Integer(
        "Karma to unlink", compute="_get_post_karma_rights", compute_sudo=False
    )
    karma_comment = fields.Integer(
        "Karma to comment", compute="_get_post_karma_rights", compute_sudo=False
    )
    ...
    karma_flag = fields.Integer(
        "Flag a post as offensive", compute="_get_post_karma_rights", compute_sudo=False
    )
    can_ask = fields.Boolean(
        "Can Ask", compute="_get_post_karma_rights", compute_sudo=False
    )
    can_answer = fields.Boolean(
        "Can Answer", compute="_get_post_karma_rights", compute_sudo=False
    )
    can_accept = fields.Boolean(
        "Can Accept", compute="_get_post_karma_rights", compute_sudo=False
    )
    can_edit = fields.Boolean(
        "Can Edit", compute="_get_post_karma_rights", compute_sudo=False
    )
    can_close = fields.Boolean(
        "Can Close", compute="_get_post_karma_rights", compute_sudo=False
    )
    can_unlink = fields.Boolean(
        "Can Unlink", compute="_get_post_karma_rights", compute_sudo=False
    )
    can_upvote = fields.Boolean(
        "Can Upvote", compute="_get_post_karma_rights", compute_sudo=False
    )
    can_downvote = fields.Boolean(
        "Can Downvote", compute="_get_post_karma_rights", compute_sudo=False
    )
    can_comment = fields.Boolean(
        "Can Comment", compute="_get_post_karma_rights", compute_sudo=False
    )
    can_comment_convert = fields.Boolean(
        "Can Convert to Comment", compute="_get_post_karma_rights", compute_sudo=False
    )
    ...
    can_flag = fields.Boolean(
        "Can Flag", compute="_get_post_karma_rights", compute_sudo=False
    )
    can_moderate = fields.Boolean(
        "Can Moderate", compute="_get_post_karma_rights", compute_sudo=False
    )
    ...
    def _get_user_vote(self):
        votes = self.env["forum.post.vote"].search_read(
            [("post_id", "in", self._ids), ("user_id", "=", self._uid)],
            ["vote", "post_id"],
        )
    ...
    def _get_post_karma_rights(self):
        ...
        for post, post_sudo in zip(self, self.sudo()):
            ...
            post.can_view = (
                ...
                or (
                    post_sudo.create_uid.karma > 0
                    and (post_sudo.active or post_sudo.create_uid == user)
                )
            )
            post.can_display_biography = (
                is_admin or post_sudo.create_uid.karma >= post.forum_id.karma_user_bio
            )
    ...
    def _update_content(self, content, forum_id):
        ...
        if content and self.env.user.karma < forum.karma_dofollow:
            for match in re.findall(r'<a\s.*href=".*?">', content):
                ...
                content = re.sub(
                    match, match[:3] + 'rel="nofollow" ' + match[3:], content
                )
    ...
    def create(self, vals_list):
        for vals in vals_list:
            if "content" in vals and vals.get("forum_id"):
                vals["content"] = self._update_content(
                    vals["content"], vals["forum_id"]
                )
    ...
    def _get_mail_message_access(self, res_ids, operation, model_name=None):
        ...
        if operation in ("write", "unlink") and (
            not model_name or model_name == "forum.post"
        ):
        ...
        return super(Post, self)._get_mail_message_access(
        ...
        for post in self:
            ...
            if "is_correct" in vals:
                ...
                if (
                    ...
                    and post.create_uid.id != self._uid
                ):
    ...
    def bump(self):
        ...
        if (
            ...
            and (
                datetime.today()
                - datetime.strptime(
                    self.write_date, tools.DEFAULT_SERVER_DATETIME_FORMAT
                )
            ).days
            > 9
        ):
    ...
    def convert_comment_to_answer(self, message_id, default=None):
        ...
        if (
            not comment.author_id or not comment.author_id.user_ids
    ...
    def unlink_comment(self, message_id):
        ...
        for post in self:
            ...
            if not can_unlink:
                raise AccessError(
                    _("%d karma required to unlink a comment.", karma_unlink)
                )
    ...
    def _get_access_action(self, access_uid=None, force_website=False):
        ...
        if not force_website and not self.state == "active":
            return super(Post, self)._get_access_action(
    ...
    def _notify_thread_by_inbox(
        ...
        if msg_vals.get("message_type", message.message_type) == "comment":
            return
        return super(Post, self)._notify_thread_by_inbox(
...
class Vote(models.Model):
    ...
    post_id = fields.Many2one(
        "forum.post", string="Post", ondelete="cascade", required=True
    )
        ...
        for vote in self:
            ...
            if "vote" in values:
                if values["vote"] == "1" or vote.vote == "-1" and values["vote"] == "0":
                    ...
                    values["vote"] == "-1" or vote.vote == "1" and values["vote"] == "0"
                ):
                    upvote = False
...
class Tags(models.Model):
    ...
    posts_count = fields.Integer(
        "Number of Posts", compute="_get_posts_count", store=True
    )
