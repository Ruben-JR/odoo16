...
class WebsiteAccount(CustomerPortal):
    ...
    def _prepare_home_portal_values(self, counters):
        ...
        if "lead_count" in counters:
            values["lead_count"] = request.env["crm.lead"].search_count(
                self.get_domain_my_lead(request.env.user)
            )
        if "opp_count" in counters:
            values["opp_count"] = request.env["crm.lead"].search_count(
                self.get_domain_my_opp(request.env.user)
            )
        return values
    ...
    def portal_my_opportunities(
        ...
        this_week_end_date = fields.Date.to_string(
            fields.Date.from_string(today) + datetime.timedelta(days=7)
        )
...
class WebsiteCrmPartnerAssign(WebsitePartnerPage):
    ...
    def partners(self, country=None, grade=None, page=0, **post):
        ...
        return request.render(
            "website_crm_partner_assign.index", values, status=partners and 200 or 404
        )
