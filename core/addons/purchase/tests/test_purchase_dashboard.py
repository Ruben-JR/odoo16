...
class TestPurchaseDashboard(AccountTestInvoicingCommon, MailCase):
    ...
    def setUpClass(cls, chart_template_ref=None):
        ...
        cls.user_a = new_test_user(
            cls.env, login="purchaseusera", groups="purchase.group_purchase_user"
        )
    ...
    def test_purchase_dashboard(self):
        ...
        self.assertListEqual(
            [dashboard_result[key] for key in zero_value_keys],
            [0] * len(zero_value_keys),
        )
        ...
        self.assertEqual(
            dashboard_result["all_avg_order_value"],
            format_amount(
                self.env,
                self.tax_purchase_a.compute_all(700.0)["total_included"],
                currency_id,
            ),
        )
        self.assertEqual(dashboard_result["all_avg_days_to_purchase"], 0)
        self.assertEqual(
            dashboard_result["all_total_last_7_days"],
            format_amount(
                self.env,
                self.tax_purchase_a.compute_all(2100.0)["total_included"],
                currency_id,
            ),
        )
