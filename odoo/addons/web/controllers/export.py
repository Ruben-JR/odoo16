...
class GroupsTreeNode:
    ...
    def _get_aggregate(self, field_name, data, group_operator):
        ...
        if not aggregate_func:
            _logger.warning(
                "Unsupported export of group_operator '%s' for field %s on model %s",
                group_operator,
                field_name,
                self._model._name,
            )
        ...
        return aggregate_func(
            (
                child.aggregated_values.get(field_name)
        ...
        for field_name in self._export_field_names:
            ...
            if field_name in self._get_aggregated_field_names():
                ...
                aggregated_values[field_name] = self._get_aggregate(
                    field_name, field_data, field.group_operator
                )
...
class Export(http.Controller):
        ...
        return [
            {
                "tag": "xlsx",
                "label": "XLSX",
                "error": None if xlsxwriter else "XlsxWriter 0.9.3 required",
            },
            {"tag": "csv", "label": "CSV"},
        ]
    ...
    def namelist(self, model, export_id):
        ...
        export_fields_list = (
            request.env["ir.exports.line"].browse(export["export_fields"]).read()
        )

        fields_data = self.fields_info(model, [f["name"] for f in export_fields_list])
