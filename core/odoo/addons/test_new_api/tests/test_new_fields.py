...
class TestFields(TransactionCaseWithUserDemo):
    def setUp(self):
        ...
        self.env.ref("test_new_api.discussion_0").write(
    ...
    def test_10_computed_custom_invalid_transitive_depends(self):
        self.patch(
            type(self.env["ir.model.fields"]), "_check_depends", lambda self: True
        )
        self.env["ir.model.fields"].create(
            {
    ...
    def test_15_constraint_inverse(self):
        ...
        log = []
        model = self.env["test_new_api.compute.inverse"].with_context(
    ...
    def test_20_monetary_opw_2223134(self):
        ...
        def check(value):
            ...
            self.env.flush_all()
            self.cr.execute(
                "SELECT total FROM test_new_api_monetary_order WHERE id=%s", [record.id]
            )
    ...
    def test_21_date_datetime_helpers(self):
        ...
        self.assertEqual(
            start_of(_datetime, "day"), _datetime.replace(hour=0, minute=0, second=0)
        )
        ...
        with self.assertRaises(ValueError):
            start_of(_date, "hour")
        self.assertEqual(
            start_of(_datetime, "hour"), _datetime.replace(minute=0, second=0)
        )
        ...
        self.assertEqual(
            end_of(_datetime, "month"), datetime.combine(date(2077, 10, 31), time.max)
        )

        # week
        self.assertEqual(end_of(_date, "week"), date(2077, 10, 24))
        self.assertEqual(
            end_of(_datetime, "week"),
        ...
        self.assertEqual(
            end_of(_datetime, "day"), datetime.combine(_datetime, time.max)
        )
    ...
    def test_25_related_multi(self):
        ...
        oof = self.env["test_new_api.foo"].create(
    ...
    def test_28_company_dependent_search(self):
        ...
        # many2one fields
        tag1, tag2 = self.env["test_new_api.multi.tag"].create(
            [{"name": "one"}, {"name": "two"}]
        )
        test_field(
            "tag_id",
    ...
    def test_41_new_one2many(self):
        ...
        move = self.env["test_new_api.move"].create({})
        line = self.env["test_new_api.move_line"].create(
    ...
    def test_91_binary_svg_attachment(self):
        ...
        record = (
            self.env["test_new_api.binary_svg"]
            .with_user(
                self.user_demo,
            )
            .create(
    ...
    def test_94_image(self):
        ...
        self.assertEqual(
            Image.open(io.BytesIO(base64.b64decode(record.image_128))).size, (128, 64)
        )
        # test create related store (resize, width limited)
        self.assertEqual(
            Image.open(io.BytesIO(base64.b64decode(record.image_512))).size, (512, 256)
        )
        # test create related no store (resize, width limited)
        self.assertEqual(
            Image.open(io.BytesIO(base64.b64decode(record.image_256))).size, (256, 128)
        )
        ...
        self.assertEqual(
            Image.open(io.BytesIO(base64.b64decode(record.image_128))).size, (64, 128)
        )
        # test write related store (resize, height limited)
        self.assertEqual(
            Image.open(io.BytesIO(base64.b64decode(record.image_512))).size, (256, 512)
        )
        # test write related no store (resize, height limited)
        self.assertEqual(
            Image.open(io.BytesIO(base64.b64decode(record.image_256))).size, (128, 256)
        )
        ...
        self.assertEqual(
            Image.open(io.BytesIO(base64.b64decode(record.image))).size, (4000, 2000)
        )
        self.assertEqual(
            Image.open(io.BytesIO(base64.b64decode(record.image_256))).size, (256, 128)
        )
        ...
        self.assertEqual(
            Image.open(io.BytesIO(base64.b64decode(record.image))).size, (2000, 4000)
        )
        self.assertEqual(
            Image.open(io.BytesIO(base64.b64decode(record.image_256))).size, (128, 256)
        )
        ...
        record = (
            self.env["test_new_api.model_image"]
        ...
        self.assertEqual(
            image_data_uri(record.image_256)[:30], "data:image/png;base64,iVBORw0K"
        )
    ...
    def test_95_binary_bin_size(self):
        ...
        record_no_bin_size = (
            self.env["test_new_api.model_binary"]
...
class TestX2many(common.TransactionCase):
    def test_definition_many2many(self):
        ...
        self.assertEqual(
            field.relation, "test_new_api_multi_line_test_new_api_multi_tag_rel"
        )
        ...
        self.assertEqual(
            field.relation, "test_new_api_multi_line2_test_new_api_multi_tag_rel"
        )
    ...
    def test_12_active_test_one2many_with_context(self):
        ...
        self.assertEqual(
            parent.with_context(active_test=False).all_children_ids, all_children
        )

        self.assertEqual(parent.active_children_ids, act_children)
        self.assertEqual(
            parent.with_context(active_test=True).active_children_ids, act_children
        )
        self.assertEqual(
            parent.with_context(active_test=False).active_children_ids, act_children
        )
    ...
    def test_search_one2many(self):
        ...
        result = recs.search(
            [("id", "in", recs.ids), ("lines", "in", (line1 + line3 + line4).ids)]
        )
        self.assertEqual(result, recX + recY)

        result = recs.search(
            [("id", "in", recs.ids), ("lines", "in", (line1 + line4).ids)]
        )
        ...
        # test 'not in'
        result = recs.search(
            [("id", "in", recs.ids), ("lines", "not in", (line1 + line2 + line3).ids)]
        )
...
%endif"""
        ...
        self.assertIn(
            '<tr style="', record.comment3, "Style attr should not have been stripped"
        )
        # sanitize does not keep classes if asked to
        self.assertNotIn('<tr class="', record.comment3)

        self.assertNotIn(
            '<tr style="', record.comment4, "Style attr should have been stripped"
        )
    ...
    def test_01_sanitize_groups(self):
        ...
        self.assertEqual(
            record.comment5, normalized_val, "should be normalized (not in groups)"
        )
...
class TestMagicFields(common.TransactionCase):
    ...
    def test_mro_mixin(self):
        ...
        self.patch(
            registry, "models", OrderedDict(sorted(models.items(), reverse=True))
        )
        registry.setup_models(self.cr)
        field = registry["test_new_api.display"].display_name
        self.assertFalse(field.automatic)
...
class TestMany2oneReference(common.TransactionCase):
    ...
    def test_search_inverse_one2many_autojoin(self):
        ...
        reference = self.env["test_new_api.model_many2one_reference"].create(
            {"res_id": record.id}
        )
        ...
        self.assertTrue(
            self.env.cache.has_dirty_fields(reference, [type(reference).res_model])
        )
    ...
    def setUp(self):
        ...
        for model_name in (self.MODEL_BASE, self.MODEL_REQUIRED):
            ...
            self.addCleanup(
                ...
                Model._BaseModel__base_classes,
            )
...
def select(model, *fnames):
    ...
    table = model._table
    terms = ", ".join(
        f'"{table}"."{fname}" AS "{fname}"' for fname in ["id"] + list(fnames)
    )
...
class TestPrecompute(common.TransactionCase):
    def test_precompute(self):
        ...
        with self.assertQueries(
            [
                insert(
                    model, "name", "lower", "upper", "lowup", "commercial_id", "size"
                ),
    ...
    def test_precompute_combo(self):
        ...
        # default value
        with self.assertQueries(QUERIES), self.assertLogs(
            "precompute_setter", level="WARNING"
        ):
    ...
    def test_precompute_batch(self):
        ...
        partners = self.env["res.partner"].create(
            [{"name": name} for name in "Foo Bar Baz".split()]
        )
