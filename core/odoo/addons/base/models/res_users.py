...
class Groups(models.Model):
    ...
    full_name = fields.Char(
        compute="_compute_full_name", string="Group Name", search="_search_full_name"
    )
    ...
    def _search(
        ...
        return super(Groups, self)._search(
            args,
            offset=offset,
            limit=limit,
            order=order,
...
class Users(models.Model):
    ...
    def _default_groups(self):
        default_user_id = self.env["ir.model.data"]._xmlid_to_res_id(
            "base.default_user", raise_if_not_found=False
        )
        return (
            self.env["res.users"].browse(default_user_id).sudo().groups_id
            if default_user_id
            else []
        )
    ...
    login_date = fields.Datetime(
        related="log_ids.create_date", string="Latest authentication", readonly=False
    )
    share = fields.Boolean(
    ...
    def _check_action_id(self):
        ...
        for user in self.sudo():
            if user.action_id.type == "ir.actions.client":
                action = self.env["ir.actions.client"].browse(
                    user.action_id.id
                )  # magic
    ...
    def _search(
        ...
        if not self.env.su and args:
            domain_fields = {
                term[0] for term in args if isinstance(term, (tuple, list))
            }
    ...
    def write(self, values):
        ...
        if "groups_id" in values and default_user and default_user in self:
            ...
            if added_groups:
                ...
                internal_users.write(
                    {"groups_id": [Command.link(gid) for gid in added_groups.ids]}
                )
        ...
        if "company_id" in values:
            for user in self:
                ...
                if (
                    ...
                    and user.partner_id.company_id.id != values["company_id"]
                ):
        ...
        if (invalidation_fields & values.keys()) or any(
            key.startswith("context_") for key in values
        ):
            self.clear_caches()
    ...
    def context_get(self):
        ...
        context = {key: values[name] for name, key in name_to_key.items()}
    ...
    def _has_group(self, group_ext_id):
        ...
        assert group_ext_id and "." in group_ext_id, (
            "External ID '%s' must be fully qualified" % group_ext_id
        )
    ...
    def _on_login_cooldown(self, failures, previous):
        ...
        return failures >= min_failures and (
            datetime.datetime.now() - previous
    ...
    def _remove_group(self, implied_group):
        ...
        if groups:
            ...
            users_to_unlink = [
                ...
                if implied_group
                not in (user.groups_id - implied_group).trans_implied_ids
            ]
            if users_to_unlink:
    ...
    def _update_user_groups_view(self):
        ...
        if self._context.get("install_filename") or self._context.get(
            ...
            for (
                ...
                gs,
                category_name,
            ) in sorted_tuples:  # we process the user type first
                attrs = {}
                    ...
                    for g in gs:
                        ...
                        xml0.append(
                            E.field(
                                name=field_name,
                                **dict(
                                    attrs, invisible="1", groups="!base.group_no_one"
                                ),
                            )
                        )
                        group_count += 1
            ...
            xml = E.field(
                *(xml0),
                ...
                E.group(
                    *(xml4), attrs=str(user_type_attrs), groups="base.group_no_one"
                ),
                name="groups_id",
                position="replace",
            )
...
class UsersView(models.Model):
    ...
    user_group_warning = fields.Text(
        string="User Group Warning", compute="_compute_user_group_warning"
    )
    ...
    def _compute_user_group_warning(self):
        ...
        if self._context.get("show_user_group_warning"):
            for user in self.filtered_domain([("share", "=", False)]):
                group_inheritance_warnings = (
                    self._prepare_warning_for_group_inheritance(user)
                )
    ...
    def create(self, vals_list):
        ...
        if group_multi_company_id:
            for user in users:
                if (
                    len(user.company_ids) <= 1
                    and group_multi_company_id in user.groups_id.ids
                ):
                    user.write({"groups_id": [Command.unlink(group_multi_company_id)]})
                elif (
                    len(user.company_ids) > 1
                    and group_multi_company_id not in user.groups_id.ids
                ):
    ...
    def write(self, values):
        ...
        if group_multi_company:
            for user in self:
                if (
                    len(user.company_ids) <= 1
                    and user.id in group_multi_company.users.ids
                ):
                    user.write({"groups_id": [Command.unlink(group_multi_company.id)]})
                elif (
                    len(user.company_ids) > 1
                    and user.id not in group_multi_company.users.ids
                ):
    ...
    def _prepare_warning_for_group_inheritance(self, user):
        ...
        for group in current_groups:
            current_groups_by_category[
                group.category_id
            ] |= group.trans_implied_ids.filtered(
                lambda grp: grp.category_id == group.category_id
            )

        missing_groups = {}
        # We don't want to show warning for "Technical" and "Extra Rights" groups
        categories_to_ignore = self.env.ref(
            "base.module_category_hidden"
        ) + self.env.ref("base.module_category_usability")
        for group in current_groups:
            ...
            missing_implied_groups = missing_implied_groups.filtered(
                ...
                not in (group.category_id | categories_to_ignore)
                and g not in current_groups_by_category[g.category_id]
                and (self.user_has_groups("base.group_no_one") or g.category_id)
            )
            if missing_implied_groups:
                # prepare missing group message, by categories
                missing_groups[group] = ", ".join(
                    f'"{missing_group.category_id.name or _("Other")}: {missing_group.name}"'
    ...
    def _remove_reified_groups(self, values):
        ...
        if "groups_id" not in values and (add or rem):
            ...
            values1["groups_id"] = list(
                itertools.chain(zip(repeat(3), rem), zip(repeat(4), add))
            )
    ...
    def onchange(self, values, field_name, field_onchange):
        ...
        if not field_name:  # merged default_get
            self._add_reified_groups(
                filter(is_reified_group, field_onchange), result.setdefault("value", {})
            )
        return result
    ...
    def _add_reified_groups(self, fields, values):
        ...
        for f in fields:
            if is_boolean_group(f):
                ...
                sel_groups = (
                    self.env["res.groups"].sudo().browse(get_selection_groups(f))
                )
                sel_order = {
                    g: len(g.trans_implied_ids & sel_groups) for g in sel_groups
                }
    ...
    def fields_get(self, allfields=None, attributes=None):
        ...
        for app, kind, gs, category_name in (
            self.env["res.groups"].sudo().get_groups_by_application()
        ):
            if kind == "selection":
        ...
        missing = (
            ...
            .union(self.SELF_READABLE_FIELDS)
            .difference(res.keys())
        )
        if allfields:
            ...
            res.update(
                {
                    key: dict(
                        values,
                        readonly=key not in self.SELF_WRITEABLE_FIELDS,
                        searchable=False,
                    )
                    for key, values in super(UsersView, self.sudo())
                    .fields_get(missing, attributes)
                    .items()
                }
            )
        return res
...
class ChangePasswordUser(models.TransientModel):
    ...
    wizard_id = fields.Many2one(
        "change.password.wizard", string="Wizard", required=True, ondelete="cascade"
    )
...
class APIKeysUser(models.Model):
    ...
    def _check_credentials(self, password, user_agent_env):
        ...
        if (
            self.env["res.users.apikeys"]._check_credentials(scope="rpc", key=password)
            == self.env.uid
        ):
            return
...
class APIKeys(models.Model):
    ...
    user_id = fields.Many2one(
        "res.users", index=True, required=True, readonly=True, ondelete="cascade"
    )
    ...
    def init(self):
        ...
        """
            ).format(table=table, index_name=sql.Identifier(index_name))
        )
