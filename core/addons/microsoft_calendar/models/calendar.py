...
class Meeting(models.Model):
    ...
    def write(self, values):
        ...
        if "recurrency" in values and values["recurrency"]:
            for e in self.filtered(lambda e: not e.recurrency and not e.recurrence_id):
                e._microsoft_delete(
                    e._get_organizer(), e.ms_organizer_event_id, timeout=3
                )
    ...
    def _microsoft_to_odoo_values(
        ...
        values.update(
            {
                ...
                "location": microsoft_event.location
                and microsoft_event.location.get("displayName")
                or False,
                "user_id": microsoft_event.owner_id(self.env),
                "privacy": sensitivity_o2m.get(
                    microsoft_event.sensitivity,
        ...
        if with_ids:
            values["microsoft_id"] = combine_ids(
                microsoft_event.id, microsoft_event.iCalUId
            )
    ...
    def _odoo_attendee_commands_m(self, microsoft_event):
        ...
        partners = self.env["mail.thread"]._mail_find_partner_from_emails(
            emails, records=self, force_create=True
        )
        ...
        for email, partner, attendee_info in zip(emails, partners, microsoft_attendees):
            state = ATTENDEE_CONVERTER_M2O.get(
                attendee_info.get("status").get("response"), "needsAction"
            )
