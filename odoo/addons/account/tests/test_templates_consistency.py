...
class AccountingTestTemplConsistency(TransactionCase):
    ...
    def check_fields_consistency(self, model_from, model_to, exceptions=None):
        ...
        from_fields = self.get_model_fields(
            model_from, extra_domain=extra_domain
        ).filtered_domain([("modules", "=", "account")])

        to_fields_set = set([f.name for f in self.get_model_fields(model_to)])
        for field in from_fields:
            assert (
                field.name in to_fields_set
            ), 'Missing field "%s" from "%s" in model "%s".' % (
                field.name,
                model_from,
                model_to,
            )
    ...
    def test_account_account_fields(self):
        ...
        self.check_fields_consistency(
            ...
            "account.account",
            exceptions=["chart_template_id", "nocreate"],
        )
        self.check_fields_consistency(
            "account.account",
            "account.account.template",
    ...
    def test_account_tax_fields(self):
        ...
        self.check_fields_consistency(
            "account.tax.template", "account.tax", exceptions=["chart_template_id"]
        )
        self.check_fields_consistency(
            "account.tax",
            "account.tax.template",
            exceptions=["company_id", "country_id", "real_amount"],
        )
        self.check_fields_consistency(
            "account.tax.repartition.line.template",
            "account.tax.repartition.line",
            exceptions=["plus_report_expression_ids", "minus_report_expression_ids"],
        )
        self.check_fields_consistency(
            "account.tax.repartition.line",
            "account.tax.repartition.line.template",
            exceptions=["tag_ids", "country_id", "company_id", "sequence"],
        )
    ...
    def test_fiscal_position_fields(self):
        ...
        self.check_fields_consistency(
            ...
            exceptions=["chart_template_id"],
        )
        self.check_fields_consistency(
            "account.fiscal.position",
            "account.fiscal.position.template",
            exceptions=["active", "company_id", "states_count", "foreign_vat"],
        )
        # taxes
        self.check_fields_consistency(
        ...
        self.check_fields_consistency(
            "account.fiscal.position.tax", "account.fiscal.position.tax.template"
        )
        # accounts
        self.check_fields_consistency(
    ...
    def test_reconcile_model_fields(self):
        ...
        mail_thread_fields = [
            field.name for field in self.get_model_fields("mail.thread")
        ]
        self.check_fields_consistency(
            "account.reconcile.model",
            "account.reconcile.model.template",
            exceptions=mail_thread_fields
            + ["active", "company_id", "past_months_limit", "partner_mapping_line_ids"],
        )
        # lines
        self.check_fields_consistency(
        ...
        self.check_fields_consistency(
            "account.reconcile.model.line",
            "account.reconcile.model.line.template",
            exceptions=["company_id", "journal_id", "analytic_distribution", "amount"],
        )
    ...
    def test_account_group_fields(self):
        ...
        self.check_fields_consistency(
            ...
            exceptions=["company_id", "parent_path"],
        )
        self.check_fields_consistency(
            "account.group.template", "account.group", exceptions=["chart_template_id"]
        )
