...
class TestMassMailing(models.TransientModel):
    ...
    def send_mail_test(self):
        ...
        if record:
            ...
            body = mailing._render_field("body_html", record.ids, post_process=True)[
                record.id
            ]
            preview = mailing._render_field("preview", record.ids, post_process=True)[
                record.id
            ]
        ...
        for mail_sudo in mails_sudo:
            if mail_sudo.state == "sent":
                ...
                notification_messages.append(
                    _(
                        "Test mailing could not be sent to %s:<br>%s",
                        mail_sudo.email_to,
