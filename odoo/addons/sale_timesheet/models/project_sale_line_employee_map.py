...
class ProjectProductEmployeeMap(models.Model):
    ...
    company_id = fields.Many2one(
        "res.company", string="Company", related="project_id.company_id"
    )
    ...
    is_cost_changed = fields.Boolean(
        "Is Cost Manually Changed", compute="_compute_is_cost_changed", store=True
    )
    ...
    def _compute_sale_line_id(self):
        self.filtered(
            lambda map_entry: map_entry.sale_line_id
