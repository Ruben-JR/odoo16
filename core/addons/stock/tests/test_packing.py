...
class TestPacking(TestPackingCommon):
    def test_put_in_pack(self):
        ...
        self.assertEqual(
            len(pick_picking.package_level_ids),
            1,
            "Put some products in pack should create a package_level",
        )
        self.assertEqual(
            pick_picking.package_level_ids[0].state,
            "new",
            'A new pack should be in state "new"',
        )
        pick_picking.move_line_ids.filtered(
            lambda ml: ml.product_id == self.productA and ml.qty_done == 0.0
        ).qty_done = 4.0
        pick_picking.move_line_ids.filtered(
            lambda ml: ml.product_id == self.productB and ml.qty_done == 0.0
        ).qty_done = 3.0
        ...
        self.assertEqual(
            len(packing_picking.package_level_ids),
            2,
            "Two package levels must be created after assigning picking",
        )
    ...
    def test_pick_a_pack_confirm(self):
        ...
        self.assertEqual(
            len(package_level.move_ids), 1, "The move should be in the package level"
        )
        self.assertEqual(
            package_level.state,
            "confirmed",
        ...
        self.assertEqual(
            picking.move_line_ids.package_level_id.id,
            package_level.id,
        ...
        self.assertEqual(
            package_level.state,
            "assigned",
            "The package level must be in state assigned",
        )
        ...
        self.assertEqual(
            package_level.move_line_ids[0].qty_done,
            20.0,
        ...
        self.assertEqual(
            package_level.state, "done", "The package level must be in state done"
        )
        self.assertEqual(
            pack.location_id.id,
    ...
    def test_multi_pack_reservation(self):
        ...
        picking.action_confirm()
        self.assertEqual(
            picking.package_level_ids.mapped("location_id.id"),
            [shelf1_location.id],
            "The package levels should still in the same location after confirmation.",
        )
        picking.action_assign()
        package_level_reserved = picking.package_level_ids.filtered(
            lambda pl: pl.state == "assigned"
        )
        package_level_confirmed = picking.package_level_ids.filtered(
            lambda pl: pl.state == "confirmed"
        )
        self.assertEqual(
            package_level_reserved.location_id.id,
            shelf1_location.id,
            "The reserved package level must be reserved in shelf1",
        )
        self.assertEqual(
            package_level_confirmed.location_id.id,
            shelf1_location.id,
            "The not reserved package should keep its location",
        )
        ...
        self.assertEqual(
            picking.package_level_ids.mapped("is_done"),
            [True, True],
            "Both package should still done",
        )
    ...
    def test_pack_in_receipt_two_step_single_putway(self):
        ...
        * In this internal transfer, adds the package then valid it.
        ...
        self.assertEqual(
            receipt_package.location_dest_id.id, receipt.location_dest_id.id
        )
        self.assertEqual(
    ...
    def test_entire_pack_overship(self):
        ...
        self.assertEqual(
            sum(ml.qty_done for ml in picking.move_line_ids),
            100.0,
            "total move_line.qty_done should = 100",
        )
    ...
    def test_2_steps_and_backorder(self):
        ...
        picking, _, _ = create_picking(
            delivery_type, delivery_type.default_location_src_id, self.customer_location
        )
    ...
    def test_pack_in_receipt_two_step_multi_putaway_02(self):
        ...
        self.env.user.write(
            {
                "groups_id": [
                    (4, self.env.ref("stock.group_stock_storage_categories").id)
                ]
            }
        )
        self.env.user.write(
            {"groups_id": [(4, self.env.ref("stock.group_stock_multi_locations").id)]}
        )
        ...
        move_form = Form(
            receipt_picking.move_ids, view="stock.view_stock_move_operations"
        )
        ...
        move_form = Form(
            internal_picking.move_ids, view="stock.view_stock_move_operations"
        )
    ...
    def test_pack_in_receipt_two_step_multi_putaway_03(self):
        ...
        move_form = Form(
            receipt_picking.move_ids[0], view="stock.view_stock_move_operations"
        )
        with move_form.move_line_ids.edit(0) as line:
            line.qty_done = 50
            line.result_package_id = package_02
        move_form.save()
        move_form = Form(
            receipt_picking.move_ids[1], view="stock.view_stock_move_operations"
        )
        with move_form.move_line_ids.edit(0) as line:
            line.qty_done = 50
            line.result_package_id = package_02
    ...
    def test_pack_in_receipt_two_step_multi_putaway_04(self):
        ...
        moves.move_line_ids.result_package_id = self.env["stock.quant.package"].create(
            {"package_type_id": package_type.id}
        )
        receipt.button_validate()
        dest_moves = moves.move_dest_ids
        self.assertEqual(
            dest_moves.move_line_ids.location_dest_id,
            self.stock_location,
            ...
            [
                {
                    ...
                    .create({"package_type_id": package_type.id})
                    .id,
        ...
        self.assertEqual(
            receipt.move_ids.move_dest_ids.move_line_ids.location_dest_id,
            loc01 | loc02,
    ...
    def test_put_out_of_pack_transfer(self):
        ...
        self.assertEqual(
            moveA.move_line_ids.package_level_id.id,
            False,
            "Package level should have been unlinked from this move line.",
        )
        self.assertEqual(
        ...
        self.assertEqual(
            moveB.move_line_ids.package_level_id.id,
            pack_level.id,
            "Package level should have stayed the same.",
        )
