...
class IrActionsReport(models.Model):
    ...
    def _prepare_html(self, html, report_model=False):
        ...
        if not bodies:
            body = "".join(
                lxml.html.tostring(c, encoding="unicode")
                for c in body_parent.getchildren()
            )
            bodies.append(body)
    ...
    def _get_report(self, report_ref):
        ...
        if report:
            if report._name != "ir.actions.report":
                raise ValueError(
                    "Fetching report %r: type %s, expected ir.actions.report"
                    % (report_ref, report._name)
                )
            return report.sudo()
    ...
    def _render_qweb_pdf_prepare_streams(self, report_ref, data, res_ids=None):
        ...
        if is_whtmltopdf_needed:
            ...
            if len(res_ids_wo_stream) > 1 and set(res_ids_wo_stream) == set(
                ...
                has_valid_outlines = (
                    "/Outlines" in root and "/First" in root["/Outlines"]
                )
