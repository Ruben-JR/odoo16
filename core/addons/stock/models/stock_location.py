...
class Location(models.Model):
    ...
    storage_category_id = fields.Many2one(
        "stock.storage.category", string="Storage Category", check_company=True
    )
    outgoing_move_line_ids = fields.One2many(
        "stock.move.line", "location_id"
    ...
    @api.depends(
        "outgoing_move_line_ids.reserved_qty",
        "incoming_move_line_ids.reserved_qty",
    ...
    def _compute_weight(self):
        for location in self:
            ...
            quants = location.quant_ids.filtered(
                lambda q: q.product_id.type != "service"
            )
            excluded_sml_ids = self._context.get("exclude_sml_ids", [])
            incoming_move_lines = location.incoming_move_line_ids.filtered(
                lambda ml: ml.product_id.type != "service"
                and ml.state not in ["draft", "done", "cancel"]
                and ml.id not in excluded_sml_ids
            )
            outgoing_move_lines = location.outgoing_move_line_ids.filtered(
                lambda ml: ml.product_id.type != "service"
                and ml.state not in ["draft", "done", "cancel"]
                and ml.id not in excluded_sml_ids
            )
    ...
    def _check_scrap_location(self):
        for record in self:
            if record.scrap_location and self.env["stock.picking.type"].search(
                [
                    ("code", "=", "mrp_operation"),
                ...
                raise ValidationError(
                    _(
                        "You cannot set a location as a scrap location when it assigned as a destination location for a manufacturing type operation."
                    )
                )
    ...
    def write(self, values):
        ...
        if "usage" in values or "scrap_location" in values:
            modified_locations = self.filtered(
                lambda l: any(
                    l[f] != values[f] if f in values else False
            ...
            if values["active"] == False:
                for location in self:
                    ...
                    if warehouses:
                        raise UserError(
                            _(
                                "You cannot archive the location %s as it is"
            ...
            if locations.storage_category_id:
                if package and package.package_type_id:
                    ...
                    for values in move_line_data:
                        qty_by_location[values["location_dest_id"][0]] = values[
                            "result_package_id"
                        ]
                    for values in quant_data:
                        qty_by_location[values["location_id"][0]] += values[
                            "package_id"
                        ]
                else:
                    move_line_data = self.env["stock.move.line"]._read_group(
                        ...
                        [
                            "location_dest_id",
                            "product_id",
                            "reserved_qty:array_agg",
                            "qty_done:array_agg",
                            "product_uom_id:array_agg",
                        ],
                        ["location_dest_id"],
                    )
                    quant_data = self.env["stock.quant"]._read_group(
                        ...
                        ["location_id", "product_id", "quantity:sum"],
                        ["location_id"],
                    )
                    ...
                    for values in move_line_data:
                        ...
                        qty_done = sum(
                            ...
                            for qty_reserved, qty, ml_uom in zip(
                                values["reserved_qty"], values["qty_done"], list(uoms)
                            )
                        )
    ...
    def _check_can_be_used(self, product, quantity=0, package=None, location_qty=0):
        ...
        if self.storage_category_id:
            ...
            if package and package.package_type_id:
                ...
                if self.storage_category_id.max_weight < self.forecast_weight + sum(
                    package_smls.mapped(
                        lambda sml: sml.reserved_qty * sml.product_id.weight
                    )
                ):
                    return False
                # check if enough space
                ...
                if (
                    ...
                    < self.forecast_weight + product.weight * quantity
                ):
                    return False
                product_capacity = (
            ...
            if (
                self.storage_category_id.allow_new_product == "same"
                and positive_quant
                and positive_quant.product_id != product
            ):
                return False
        return True
...
class StockRoute(models.Model):
    ...
    warehouse_domain_ids = fields.One2many(
        "stock.warehouse", compute="_compute_warehouses"
    )
