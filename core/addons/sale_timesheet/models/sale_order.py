...
class SaleOrder(models.Model):
    ...
    project_id = fields.Many2one(
        domain="[('pricing_type', '!=', 'employee_rate'), ('analytic_account_id', '!=', False), ('company_id', '=', company_id)]"
    )
    ...
    def _compute_timesheet_ids(self):
        timesheet_groups = (
            ...
            .read_group(
                [
                    ("so_line", "in", self.mapped("order_line").ids),
                    ("project_id", "!=", False),
                ],
        ...
        timesheets_per_sol = {
            group["so_line"][0]: (group["ids"], group["so_line_count"])
        ...
        for order in self:
            ...
            for sale_line_id in order.order_line.filtered("is_service").ids:
                list_timesheet_ids, count = timesheets_per_sol.get(
                    sale_line_id, ([], 0)
                )
            ...
            order.update(
                {
                    "timesheet_ids": self.env["account.analytic.line"].browse(
                        timesheet_ids
                    ),
    ...
    def _compute_field_value(self, field):
        ...
        upsellable_orders = self.filtered(
            ...
            and (
                so.user_id or so.partner_id.user_id
    ...
    def action_view_timesheet(self):
        ...
        if self.order_line:
            ...
            if tasks:
                ...
                projects = self.order_line.project_id._filter_access_rules_python(
                    "write"
                )
                if projects:
...
class SaleOrderLine(models.Model):
            ...
            for line in self:
                ...
                if line.remaining_hours_available:
                    ...
                    if encoding_uom == uom_hour:
                        ...
                        minutes = (
                            math.ceil(round_minutes)
                            if line.remaining_hours >= 0
                            else math.floor(round_minutes)
                        )
                        if minutes > 1:
                            minutes = 0
                            hours += 1
                            ...
                            minutes = minutes * 30
                        remaining_time = (
                            " ({sign}{hours:02.0f}:{minutes:02.0f})".format(
    ...
    def _timesheet_create_project(self):
        ...
        for line in self.order_id.order_line:
            ...
            if line.is_service and (
                product_type == "task_in_project" or product_type == "project_only"
            ):
                if (
                    uom_per_id.get(line.product_uom.id)
                    or line.product_uom.id == uom_unit.id
                ):
                    allocated_hours += (
                        line.product_uom_qty
                        * uom_per_id.get(line.product_uom.id, project_uom).factor_inv
                        * uom_hour.factor
                    )
    ...
    def _get_action_per_item(self):
        ...
        if self.user_has_groups("hr_timesheet.group_hr_timesheet_user"):
            ...
            timesheet_ids_per_sol = {
                res["so_line"][0]: res["ids"] for res in timesheet_read_group
            }
        for sol in self:
