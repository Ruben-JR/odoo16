...
class Repair(models.Model):
    ...
    pricelist_id = fields.Many2one(
        "product.pricelist",
        "Pricelist",
        default=lambda self: self.env["product.pricelist"]
    ...
    fees_lines = fields.One2many(
        "repair.fee", "repair_id", "Operations", copy=True, readonly=False
    )
    ...
    picking_id = fields.Many2one(
        ...
        check_company=True,
        copy=False,
        help="Return Order from which the product to be repaired comes from.",
    )
    allowed_picking_type_ids = fields.Many2many(
        "stock.picking.type", compute="_compute_allowed_picking_type_ids"
    )
    ...
    def _compute_allowed_picking_type_ids(self):
        ...
        self.allowed_picking_type_ids = [
            pt["return_picking_type_id"]
            for pt in out_picking_types
            if pt["return_picking_type_id"]
        ]
    ...
    def _compute_default_address_id(self):
        for order in self:
            if order.partner_id:
                order.default_address_id = order.partner_id.address_get(["contact"])[
                    "contact"
                ]
    ...
    def _compute_is_returned(self):
        ...
        returned = self.filtered(
            lambda r: r.picking_id and r.picking_id.state == "done"
        )
    ...
    def _unlink_except_confirmed(self):
        for order in self:
            if order.invoice_id and order.invoice_id.posted_before:
                raise UserError(
                    _(
                        "You can not delete a repair order which is linked to an invoice which has been posted once."
                    )
                )
            if order.state not in ("draft", "cancel"):
                raise UserError(
                    _(
                        "You can not delete a repair order once it has been confirmed. You must first cancel it."
                    )
                )
    ...
    def action_repair_cancel(self):
        ...
        invoice_to_cancel = self.filtered(
            lambda repair: repair.invoice_id.state == "draft"
        ).invoice_id
    ...
    def _create_invoices(self, group=False):
        ...
        for repair in repairs:
            ...
            for operation in repair.operations.filtered(lambda op: op.type == "add"):
                ...
                account = operation.product_id.product_tmpl_id.get_product_accounts(
                    fiscal_pos=fpos
                )["income"]
                if not account:
                    raise UserError(
            ...
            for fee in repair.fees_lines:
                ...
                account = fee.product_id.product_tmpl_id.get_product_accounts(
                    fiscal_pos=fpos
                )["income"]
                if not account:
                    raise UserError(
    ...
    def _check_product_tracking(self):
        invalid_lines = self.operations.filtered(
            lambda x: x.tracking != "none" and not x.lot_id
        )
        if invalid_lines:
...
class RepairLine(models.Model):
    ...
    company_id = fields.Many2one(related="repair_id.company_id", store=True, index=True)
    currency_id = fields.Many2one(related="repair_id.currency_id")
    type = fields.Selection(
        [("add", "Add"), ("remove", "Remove")], "Type", default="add", required=True
    )
    product_id = fields.Many2one(
        "product.product",
        "Product",
    ...
    tax_id = fields.Many2many(
        "account.tax",
        "repair_operation_line_tax",
        "repair_operation_line_id",
        "tax_id",
        "Taxes",
    ...
    tracking = fields.Selection(
        string="Product Tracking", related="product_id.tracking"
    )
        ...
        for line in self:
            if not line.type:
                ...
                args = (
                    ...
                    and [("company_id", "=", line.repair_id.company_id.id)]
                    or []
                )
                ...
                line.location_dest_id = (
                    ...
                    .search(
                        [
                            ("scrap_location", "=", True),
                            ("company_id", "in", [line.repair_id.company_id.id, False]),
                        ],
                        limit=1,
                    )
                    .id
    ...
    def onchange_product_id(self):
        ...
        if self.type != "remove":
            if partner:
                fpos = self.env["account.fiscal.position"]._get_fiscal_position(
                    partner_invoice, delivery=self.repair_id.address_id
                )
                taxes = self.product_id.taxes_id.filtered(
                    lambda x: x.company_id == self.repair_id.company_id
                )
...
class RepairFee(models.Model):
    ...
    company_id = fields.Many2one(related="repair_id.company_id", index=True, store=True)
    currency_id = fields.Many2one(related="repair_id.currency_id")
    ...
    def create(self, vals_list):
        for vals in vals_list:
            if not vals.get("product_uom"):
                vals["product_uom"] = (
                    self.env["product.product"].browse(vals.get("product_id")).uom_id.id
                )
        return super().create(vals_list)
