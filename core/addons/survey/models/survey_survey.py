...
class Survey(models.Model):
    ...
    def _get_default_session_code(self):
        ...
        for digits_count in range(4, 10):
            ...
            colliding_codes = self.sudo().search_read(
                [("session_code", "in", list(code_candidates))], ["session_code"]
            )
            code_candidates -= set(
                [colliding_code["session_code"] for colliding_code in colliding_codes]
            )
            if code_candidates:
    ...
    certification_badge_id = fields.Many2one(
        "gamification.badge", "Certification Badge", copy=False
    )
    ...
    session_code = fields.Char(
        "Session Code",
        default=lambda self: self._get_default_session_code(),
        copy=False,
    ...
    def _compute_background_image_url(self):
        ...
        for survey in self.filtered(
            lambda survey: survey.background_image and survey.access_token
        ):
            survey.background_image_url = (
                "/survey/%s/get_background_image" % survey.access_token
            )
    ...
    @api.depends(
        "question_and_page_ids.is_conditional", "users_login_required", "access_mode"
    )
        ...
        for survey in self:
            if survey.session_code:
                survey.session_link = werkzeug.urls.url_join(
                    survey.get_base_url(), "/s/%s" % survey.session_code
                )
            else:
                survey.session_link = werkzeug.urls.url_join(
    ...
    def _compute_scoring_type(self):
        for survey in self:
            if survey.certification and survey.scoring_type not in [
    ...
    def copy(self, default=None):
        ...
        questions_map = {
            src.id: dst.id for src, dst in zip(self.question_ids, clone.question_ids)
        }
        answers_map = {
            source_answer.id: copy_answer.id
            for source_answer, copy_answer in zip(
                self.question_ids.suggested_answer_ids,
                clone.question_ids.suggested_answer_ids,
            )
        }
        for src, dst in zip(self.question_ids, clone.question_ids):
            if src.is_conditional:
                dst.triggering_question_id = questions_map.get(
                    src.triggering_question_id.id
                )
        ...
        if test_entry:
            ...
            if self.access_mode == "authentication":
                ...
                if self.users_can_signup and not user and not partner:
                    raise exceptions.UserError(
                        _(
                            "Creating token for external people is not allowed for surveys requesting authentication."
                        )
                    )
            ...
            if self.access_mode == "internal" and (not user or not user._is_internal()):
                raise exceptions.UserError(
                    _(
                        "Creating token for anybody else than employees is not allowed for internal surveys."
                    )
                )
    ...
    def _prepare_user_input_predefined_questions(self):
        ...
        for page in self.page_ids:
            if self.questions_selection == "all":
                ...
                if (
                    page.random_questions_count > 0
                    and len(page.question_ids) > page.random_questions_count
                ):
    ...
    def _can_go_back(self, answer, page_or_question):
        ...
        if self.users_can_go_back and answer.state == "in_progress":
            if (
                ...
                and page_or_question != self.page_ids[0]
            ):
                return True
            elif (
    ...
    def _get_next_page_or_question(
        self, user_input, page_or_question_id, go_back=False
    ):
        ...
        (
            ...
            selected_answers,
        ) = user_input._get_conditional_values()
    ...
    def _is_last_page_or_question(self, user_input, page_or_question):
        ...
        if next_page_or_question_candidates:
            ...
            if self.questions_layout == "page_per_question":
                ...
                is_triggering_question = any(
                    triggering_answer in triggered_questions_by_answer.keys()
                    for triggering_answer in page_or_question.suggested_answer_ids
                )
    ...
    def _get_survey_questions(self, answer=None, page_id=None, question_id=None):
        ...
        if self.questions_layout == "page_per_section":
            ...
            questions = (
                ...
                .sudo()
                .search([("survey_id", "=", self.id), ("page_id", "=", page_id)])
            )
    ...
    def _get_conditional_maps(self):
        ...
        triggered_questions_by_answer = {}
        for question in self.question_ids:
            triggering_answer_by_question[question] = (
                question.is_conditional and question.triggering_answer_id
            )
    ...
    def _get_session_most_voted_answers(self):
        ...
        current_user_inputs = self.user_input_ids.filtered(
            lambda input: input.create_date > self.session_start_time
        )
        current_user_input_lines = current_user_inputs.mapped(
            "user_input_line_ids"
        ).filtered(lambda answer: answer.suggested_answer_id)
        ...
        fake_user_input = self.env["survey.user_input"].new(
            {
                ...
                "predefined_question_ids": [
                    (6, 0, self._prepare_user_input_predefined_questions().ids)
                ],
            }
        )
    ...
    def _handle_certification_badges(self, vals):
        if vals.get("certification_give_badge"):
            ...
            surveys_with_badge = self.filtered(
                lambda survey: survey.certification_badge_id
