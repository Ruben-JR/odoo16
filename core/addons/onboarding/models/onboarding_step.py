...
class OnboardingStep(models.Model):
    ...
    done_text = fields.Char(
        ...
        default=_("Step Completed! - Click to review"),
        translate=True,
    )
        ...
        for step in self:
            if step in existing_progress_steps.step_id:
                ...
                if len(current_progress_step_id) > 1:
                    current_progress_step_id = current_progress_step_id.sorted(
    ...
    def _create_progress_steps(self):
        ...
        for onboarding_progress_record in onboarding_progress_records:
            progress_step_values += [
                ...
                for step_id in self.filtered(
                    lambda step: self.onboarding_id
                    == onboarding_progress_record.onboarding_id
                )
