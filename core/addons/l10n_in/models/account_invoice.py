...
class AccountMove(models.Model):
    ...
    l10n_in_shipping_bill_number = fields.Char(
        "Shipping bill number", readonly=True, states={"draft": [("readonly", False)]}
    )
    l10n_in_shipping_bill_date = fields.Date(
        "Shipping bill date", readonly=True, states={"draft": [("readonly", False)]}
    )
        ...
        for record in indian_invoice:
            ...
            if not gst_treatment:
                ...
                if record.partner_id.country_id.code == "IN" and record.partner_id.vat:
                    ...
                    and record.partner_id.country_id.code != "IN"
                ):
        ...
        for move in self:
            if move.country_code == "IN" and move.journal_id.type == "sale":
                ...
                if country_code == "IN":
                    ...
                    move.l10n_in_state_id = self.env.ref(
                        "l10n_in.state_in_oc", raise_if_not_found=False
                    )
                else:
    ...
    def _unlink_l10n_in_except_once_post(self):
        ...
        if any(
            m.country_code == "IN" and m.posted_before for m in self
        ) and not self._context.get("force_delete"):
            raise UserError(
                _(
    ...
    def unlink(self):
        ...
        if any(m.country_code == "IN" and m.posted_before for m in self):
            if self._context.get("force_delete"):
                moves_details = ", ".join(
                    "{entry_number} ({move_id}) amount {amount_total} {currency} and partner {partner_name}".format(
                        entry_number=m.name,
                        move_id=m.id,
                ...
                logger_msg = "Force deleted Journal Entries %s by %s (%s)" % (
                    moves_details,
                    self.env.user.name,
                    self.env.user.id,
                )
