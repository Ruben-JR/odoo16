...
class MailTracking(models.Model):
    ...
    field = fields.Many2one(
        "ir.model.fields", required=True, readonly=1, index=True, ondelete="cascade"
    )
    ...
    tracking_sequence = fields.Integer(
        "Tracking field sequence", readonly=1, default=100
    )
            ...
            values.update(
                {
                    ...
                    and fields.Datetime.to_string(
                        datetime.combine(
                            fields.Date.from_string(initial_value), datetime.min.time()
                        )
                    )
                    or False,
            ...
            values.update(
                {
                    ...
                    and dict(col_info["selection"]).get(initial_value, initial_value)
                    or "",
                    "new_value_char": new_value
                    and dict(col_info["selection"])[new_value]
                    or "",
                }
            )
            ...
            values.update(
                {
                    ...
                    "old_value_char": initial_value
                    and initial_value.sudo().name_get()[0][1]
                    or "",
                    "new_value_char": new_value
                    and new_value.sudo().name_get()[0][1]
                    or "",
                }
            )
        else:
