...
class TestSaleStockMargin(TestStockValuationCommon):
    ...
    def test_so_and_multicurrency(self):
        ...
        other_currency = (
            ...
            if company_currency == self.env.ref("base.USD")
            else self.env.ref("base.USD")
        )

        date = fields.Date.today()
        ResCurrencyRate.create(
            {"currency_id": company_currency.id, "rate": 1, "name": date}
        )
        other_currency_rate = ResCurrencyRate.search(
            [("name", "=", date), ("currency_id", "=", other_currency.id)]
        )
        if other_currency_rate:
            other_currency_rate.rate = 2
        else:
            ResCurrencyRate.create(
                {"currency_id": other_currency.id, "rate": 2, "name": date}
            )
    ...
    def test_purchase_price_changes(self):
        ...
        so.with_context(**email_ctx).message_post_with_template(
