...
class StockPackageLevel(models.Model):
    ...
    location_id = fields.Many2one(
        "stock.location", "From", compute="_compute_location_id", check_company=True
    )
    ...
    def _compute_is_done(self):
        for package_level in self:
            ...
            if package_level.is_fresh_package:
                ...
                package_level.is_done = (
                    package_level._check_move_lines_map_quant_package(
                        package_level.package_id, "qty_done"
                    )
                )
                    ...
                    for quant in package_level.package_id.quant_ids:
                        ...
                        if corresponding_mls:
                            for ml in corresponding_mls:
                                qty = (
                                    min(to_dispatch, ml.move_id.product_qty)
                                    if len(corresponding_mls) > 1
                                    else to_dispatch
                                )
    ...
    @api.depends(
        "move_line_ids", "move_line_ids.package_id", "move_line_ids.result_package_id"
    )
    ...
    def _compute_state(self):
        for package_level in self:
            if not package_level.move_ids and not package_level.move_line_ids:
                ...
                and not package_level.move_line_ids.filtered(
                ...
                if package_level.is_fresh_package:
                    ...
                    package_level.package_id, "reserved_uom_qty"
                ):
            ...
            ) or package_level.move_ids.filtered(lambda m: m.state == "cancel"):
    ...
    def _compute_show_lot(self):
        for package_level in self:
            if any(
                ml.product_id.tracking != "none" for ml in package_level.move_line_ids
            ):
                if (
                    package_level.picking_id.picking_type_id.use_existing_lots
                    or package_level.state == "done"
                ):
    ...
    def create(self, vals_list):
        ...
        for package_level, vals in zip(package_levels, vals_list):
            if vals.get("location_dest_id"):
                package_level.move_line_ids.write(
                    {"location_dest_id": vals["location_dest_id"]}
                )
                package_level.move_ids.write(
                    {"location_dest_id": vals["location_dest_id"]}
                )
