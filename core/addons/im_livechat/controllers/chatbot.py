...
class LivechatChatbotScriptController(http.Controller):
    ...
    def chatbot_trigger_step(self, channel_uuid, chatbot_script_id=None):
        ...
        if mail_channel.chatbot_current_step_id:
            ...
            chatbot_script_id
        ):  # when restarting, we don't have a "current step" -> set "next" as first step of the script
            chatbot = request.env["chatbot.script"].sudo().browse(chatbot_script_id)
            if chatbot.exists():
        ...
        return {
            "chatbot_posted_message": posted_message.message_format()[0]
            if posted_message
            else None,
            "chatbot_step": {
                "chatbot_operator_found": next_step.step_type == "forward_operator"
                and len(mail_channel.channel_member_ids) > 2,
                ...
                "chatbot_step_message": plaintext2html(next_step.message)
                if not is_html_empty(next_step.message)
                else False,
