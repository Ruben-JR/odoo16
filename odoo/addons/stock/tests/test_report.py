...
class TestReports(TestReportsCommon):
    def test_reports(self):
        ...
        rendering, qweb_type = self.env["ir.actions.report"]._render_qweb_text(
            "stock.label_lot_template", lot1.id
        )
        self.assertEqual(
            target, rendering.replace(b" ", b""), "The rendering is not good"
        )
    ...
    def test_report_quantity_1(self):
        ...
        self.assertEqual(
            sum(
                [
                    r["product_qty"]
                    for r in report_records_tomorrow
                    if r["state"] == "forecast"
                ]
            ),
            30.0,
        ...
        self.assertEqual(
            sum(
                [
                    r["product_qty"]
                    for r in report_records_yesterday
                    if r["state"] == "forecast"
                ]
            ),
            -30.0,
        ...
        self.assertEqual(
            sum(
                [r["product_qty"] for r in report_records_today if r["state"] == "out"]
            ),
            0.0,
        )
        self.assertEqual(
            sum([r["product_qty"] for r in report_records_today if r["state"] == "in"]),
            0.0,
        )
    ...
    def test_report_quantity_2(self):
        ...
        self.assertEqual(
            sum([r["product_qty"] for r in report_records if r["state"] == "forecast"]),
            40.0,
        )
        report_records = self.env["report.stock.quantity"].read_group(
            [("product_id", "=", product.id), ("date", "=", date.today())],
    ...
    def test_report_quantity_3(self):
        ...
        self.assertEqual(
            sum([r["product_qty"] for r in report_records if r["product_qty"]]), 0.0
        )
    ...
    def test_report_forecast_7_multiple_variants(self):
        ...
        self.assertEqual(
            docs["product_variants"].ids, product_template.product_variant_ids.ids
        )
    ...
    def test_report_forecast_8_delivery_to_receipt_link(self):
        ...
        self.assertEqual(
            delivery2.move_ids.forecast_expected_date, receipt.move_ids.date
        )
    ...
    def test_report_forecast_9_delivery_to_receipt_link_over_received(self):
        ...
        self.assertEqual(delivery2.move_ids.forecast_availability, 200)
        self.assertEqual(
            delivery.move_ids.forecast_expected_date, receipt.move_ids.date
        )
        self.assertEqual(
            delivery2.move_ids.forecast_expected_date, receipt.move_ids.date
        )
    ...
    def test_report_reception_2_two_receipts(self):
        ...
        self.assertTrue(
            all_lines[0]["is_qty_assignable"],
            "1 receipt is done => should have 1 reservable move.",
        )
        self.assertEqual(
            all_lines[1]["quantity"],
            3,
    ...
    def test_report_reception_5_move_splitting(self):
        ...
        report.action_assign(
            delivery.move_ids_without_package.ids,
            [incoming_qty],
            receipt.move_ids_without_package.ids,
        )
        mto_move = delivery.move_ids_without_package.filtered(
        ...
        self.assertEqual(
            len(delivery.move_ids_without_package),
            2,
            "Delivery moves should have split into assigned + not assigned",
        )
        self.assertEqual(
            len(delivery.move_ids_without_package.mapped("move_orig_ids")),
            1,
            "Only 1 delivery + 1 receipt move should be assigned",
        )
        self.assertEqual(
            len(receipt.move_ids_without_package.mapped("move_dest_ids")),
            1,
        ...
        self.assertEqual(
            mto_move.reserved_availability,
            0,
        ...
        self.assertEqual(
            non_mto_move.reserved_availability,
            qty_in_stock,
        ...
        report.action_unassign(
            [mto_move.id], incoming_qty, receipt.move_ids_without_package.ids
        )
        self.assertEqual(
            mto_move.product_uom_qty,
            incoming_qty,
    ...
    def test_report_reception_6_backorders(self):
        ...
        self.assertEqual(
            receipt.move_ids_without_package.move_dest_ids.ids,
            delivery.move_ids_without_package.ids,
        ...
        self.assertEqual(
            receipt.move_ids_without_package.move_dest_ids,
            backorder.move_ids_without_package.move_dest_ids,
        ...
        self.assertEqual(
            all_lines[0]["quantity"],
            incoming_qty - orig_incoming_qty_done,
        ...
        report.action_unassign(
            delivery.move_ids_without_package.ids,
            outgoing_qty,
            backorder.move_ids_without_package.ids,
        )
        self.assertEqual(
            len(delivery.move_ids_without_package),
            2,
        ...
        self.assertEqual(
            all_lines[0]["quantity"],
            outgoing_qty - orig_incoming_qty_done,
        ...
        report.action_assign(
            (delivery.move_ids_without_package - reserved_move).ids,
            [outgoing_qty - orig_incoming_qty_done],
            backorder.move_ids_without_package.ids,
        )
