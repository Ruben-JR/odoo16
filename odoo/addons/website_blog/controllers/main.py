...
class WebsiteBlog(http.Controller):
    ...
    def nav_list(self, blog=None):
        ...
        groups = request.env["blog.post"]._read_group_raw(
            dom, ["name", "post_date"], groupby=["post_date"], orderby="post_date desc"
        )
        for group in groups:
            ...
            group["month"] = babel.dates.format_datetime(
                start, format="MMMM", tzinfo=tzinfo, locale=locale
            )
            group["year"] = babel.dates.format_datetime(
                start, format="yyyy", tzinfo=tzinfo, locale=locale
            )

        return OrderedDict(
            (year, [m for m in months])
            for year, months in itertools.groupby(groups, lambda g: g["year"])
        )
    ...
    def _prepare_blog_values(
        ...
        fullwidth_cover = request.website.is_view_active(
        ...
        if not blogs:
            ...
            all_tags = tools.lazy(
                lambda: blogs.all_tags(join=True)
                if not blog
    ...
    def blog_post(
        ...
        if not blog_post.blog_id.id == blog.id:
            return request.redirect(
                "/blog/%s/%s" % (slug(blog_post.blog_id), slug(blog_post)), code=301
            )
