...
class ResCompany(models.Model):
    ...
    def _autorise_lock_date_changes(self, vals):
        ...
        * You cannot delete a tax lock date, lock a period that is not finished yet or the tax lock date must be set after
        ...
        """
        period_lock_date = vals.get("period_lock_date") and fields.Date.from_string(
            vals["period_lock_date"]
        )
        fiscalyear_lock_date = vals.get(
            "fiscalyear_lock_date"
        ) and fields.Date.from_string(vals["fiscalyear_lock_date"])
        tax_lock_date = vals.get("tax_lock_date") and fields.Date.from_string(
            vals["tax_lock_date"]
        )

        previous_month = fields.Date.today() + relativedelta(months=-1)
        days_previous_month = calendar.monthrange(
        ...
        for company in self:
            ...
            if old_tax_lock_date and not tax_lock_date and "tax_lock_date" in vals:
                raise UserError(
                    _("The tax lock date is irreversible and can't be removed.")
                )
            ...
            if (
                old_tax_lock_date
                and tax_lock_date
                and tax_lock_date < old_tax_lock_date
            ):
                raise UserError(
                    _("The new tax lock date must be set after the previous lock date.")
                )
            ...
            if tax_lock_date and tax_lock_date > previous_month:
                raise UserError(
                    _(
                        "You cannot lock a period that has not yet ended. Therefore, the tax lock date must be anterior (or equal) to the last day of the previous month."
                    )
                )
            ...
            if (
                ...
                raise UserError(
                    _(
                        "The lock date for accountants is irreversible and can't be removed."
                    )
                )
    ...
    def write(self, vals):
        ...
        if (
            "fiscalyear_lock_date" in vals
            or "period_lock_date" in vals
            or "tax_lock_date" in vals
        ):
