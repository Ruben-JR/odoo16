...
def iap_jsonrpc(url, method="call", params=None, timeout=15):
    ...
    try:
        ...
        raise exceptions.AccessError(
            _(
                "The url that this service requested returned an error. Please contact the author of the app. The url it tried to contact was %s",
                url,
            )
        )
...
def iap_charge(
    ...
    transaction_token = iap_authorize(
        env, key, account_token, credit, dbuuid, description, credit_template, ttl
    )
    try:
