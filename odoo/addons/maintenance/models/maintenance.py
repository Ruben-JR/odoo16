...
class MaintenanceEquipmentCategory(models.Model):
    ...
    technician_user_id = fields.Many2one(
        "res.users", "Responsible", tracking=True, default=lambda self: self.env.uid
    )
    color = fields.Integer("Color Index")
    note = fields.Html("Comments", translate=True)
    equipment_ids = fields.One2many(
        "maintenance.equipment", "category_id", string="Equipments", copy=False
    )
...
class MaintenanceEquipment(models.Model):
    ...
    def _name_search(
        ...
        if (
            ...
            equipment_ids = self._search(
                [("name", "=", name)] + args,
                limit=limit,
                access_rights_uid=name_get_uid,
            )
        return equipment_ids or super()._name_search(
    ...
    maintenance_count = fields.Integer(
        compute="_compute_maintenance_count", string="Maintenance Count", store=True
    )
    ...
    maintenance_team_id = fields.Many2one(
        "maintenance.team", string="Maintenance Team", check_company=True
    )
    ...
    def _compute_next_maintenance(self):
        ...
        for equipment in equipments:
            ...
            if next_maintenance_todo and last_maintenance_done:
                ...
                if (
                    date_gap > timedelta(0)
                    and date_gap > timedelta(days=equipment.period) * 2
                    and next_maintenance_todo.request_date > date_now
                ):
    ...
    def _onchange_company_id(self):
        if self.company_id and self.maintenance_team_id:
            if (
                self.maintenance_team_id.company_id
                and not self.maintenance_team_id.company_id.id == self.company_id.id
            ):
...
class MaintenanceRequest(models.Model):
    ...
    def onchange_equipment_id(self):
        if self.equipment_id:
            self.user_id = (
                ...
                else self.equipment_id.category_id.technician_user_id
            )
    ...
    def onchange_category_id(self):
        if (
            not self.user_id
            or not self.equipment_id
            or (self.user_id and not self.equipment_id.technician_user_id)
        ):
        ...
        for request in self.filtered(lambda request: request.schedule_date):
            ...
            if not updated:
                if request.equipment_id:
                    note = _(
                        "Request planned for %s", request.equipment_id._get_html_link()
                    )
                else:
                    note = False
...
class MaintenanceTeam(models.Model):
    ...
    def _compute_todo_requests(self):
        for team in self:
            ...
            team.todo_request_count_date = self.env["maintenance.request"].search_count(
                [("maintenance_team_id", "=", team.id), ("schedule_date", "!=", False)]
            )
            team.todo_request_count_high_priority = self.env[
                "maintenance.request"
            ].search_count(
                [("maintenance_team_id", "=", team.id), ("priority", "=", "3")]
            )
            team.todo_request_count_block = self.env[
                "maintenance.request"
            ].search_count(
                [
            ...
            team.todo_request_count_unscheduled = self.env[
                "maintenance.request"
            ].search_count(
                [("maintenance_team_id", "=", team.id), ("schedule_date", "=", False)]
            )
