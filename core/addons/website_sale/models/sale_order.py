...
class SaleOrder(models.Model):
            ...
            if vals.get("website_id"):
                ...
                if "company_id" in vals:
                    ...
                    if website.company_id.id != company.id:
                        raise ValueError(
                            _(
                                "The company of the website you are trying to sale from (%s) is different than the one you want to use (%s)"
                            )
                            % (website.company_id.name, company.name)
                        )
                else:
    ...
    def _compute_cart_info(self):
        for order in self:
            ...
            order.only_services = all(
                l.product_id.type == "service" for l in order.website_order_line
            )
    ...
    def _search_abandoned_cart(self, operator, value):
        ...
        abandoned_domain = [("state", "=", "draft"), ("order_line", "!=", False)]
    ...
    def _prepare_order_line_values(
        ...
        **kwargs,
        ...
        received_combination = (
            product.product_template_attribute_value_ids | received_no_variant_values
        )
        ...
        for ptav in combination.filtered(
            ...
            and ptav not in received_no_variant_values
        ):
        ...
        for line in self.website_order_line.filtered("product_id"):
            ...
            if accessory_products:
                ...
                all_accessory_products |= accessory_products.filtered(
                    lambda product: product not in products
                    and (
                        not product.company_id or product.company_id == line.company_id
                    )
    ...
    def _notify_get_recipients_groups(self, msg_vals=None):
        ...
        if customer_portal_group:
            ...
            if self._context.get("website_sale_send_recovery_email"):
                ...
                access_opt["url"] = "%s/shop/cart?access_token=%s" % (
                    self.get_base_url(),
                    self.access_token,
                )
        return groups
