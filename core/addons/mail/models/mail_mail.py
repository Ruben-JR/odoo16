...
class MailMail(models.Model):
    ...
    def default_get(self, fields):
        ...
        if (
            self._context.get("default_state")
            not in type(self).state.base_field.selection
        ):
    ...
    to_delete = fields.Boolean(
        "To Delete",
        help="If set, the mail will be deleted during the next Email Queue CRON run.",
    )
    ...
    fetchmail_server_id = fields.Many2one(
        "fetchmail.server", "Inbound Mail Server", readonly=True
    )
    ...
    def _compute_restricted_attachments(self):
        ...
        for mail_sudo, mail in zip(self.sudo(), self):
            mail.unrestricted_attachment_ids = IrAttachment._filter_attachment_access(
    ...
    def _inverse_unrestricted_attachment_ids(self):
        ...
        for mail_sudo, mail in zip(self.sudo(), self):
            restricted_attaments = (
                ...
                - IrAttachment._filter_attachment_access(mail_sudo.attachment_ids.ids)
            )
            mail_sudo.attachment_ids = (
    ...
    def _split_by_mail_configuration(self):
        ...
        for values in mail_values:
            ...
            group_per_email_from[(mail_server_id, values["email_from"])].append(
                values["id"]
            )
        ...
        for (mail_server_id, email_from), mail_ids in group_per_email_from.items():
            if not mail_server_id:
                mail_server, smtp_from = self.env["ir.mail_server"]._find_mail_server(
