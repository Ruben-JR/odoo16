...
class ProjectUpdate(models.Model):
    ...
    user_id = fields.Many2one(
        "res.users", string="Author", required=True, default=lambda self: self.env.user
    )
    ...
    def _get_milestone_values(self, project):
        ...
        if project.last_update_id.create_date:
            domain = expression.AND(
                [domain, [("create_date", ">", project.last_update_id.create_date)]]
            )
        created_milestones = Milestone.search(domain)._get_data_list()
        return {
            "show_section": (
                list_milestones or updated_milestones or created_milestones
            )
            and True
            or False,
