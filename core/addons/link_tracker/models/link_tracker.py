...
class LinkTracker(models.Model):
    ...
    def _compute_code(self):
        for tracker in self:
            record = self.env["link.tracker.code"].search(
                [("link_id", "=", tracker.id)], limit=1, order="id DESC"
            )
    ...
    def _get_title_from_url(self, url):
        try:
            ...
            ) > URL_MAX_SIZE or "text/html" not in head.headers.get(
            ...
            p = html.fromstring(
                page.text.encode("utf-8"), parser=html.HTMLParser(encoding="utf-8")
            )
    ...
    def create(self, vals_list):
        ...
        link_tracker_codes = self.env["link.tracker.code"]._get_random_code_strings(
            len(vals_list)
        )
    ...
    def recent_links(self, filter, limit):
        if filter == "newest":
            ...
            return self.search_read(
                [("count", "!=", 0)], order="count DESC", limit=limit
            )
        elif filter == "recently-used":
            return self.search_read(
                [("count", "!=", 0)], order="write_date DESC, id DESC", limit=limit
            )
        else:
...
class LinkTrackerCode(models.Model):
    ...
    def _get_random_code_strings(self, n=1):
        ...
        while True:
            ...
            if len(set(code_propositions)) != n or self.search(
                [("code", "in", code_propositions)]
            ):
                size += 1
            else:
...
class LinkTrackerClick(models.Model):
    ...
    def add_click(self, code, **route_values):
        ...
        existing = self.search_count(
            ["&", ("link_id", "=", tracker_code.link_id.id), ("ip", "=", ip)]
        )
        if existing:
            return None
