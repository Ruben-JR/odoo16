...
class Bank(models.Model):
    ...
    state = fields.Many2one(
        "res.country.state", "Fed. State", domain="[('country_id', '=?', country)]"
    )
    ...
    bic = fields.Char(
        "Bank Identifier Code", index=True, help="Sometimes called BIC or Swift."
    )
...
class ResPartnerBank(models.Model):
    ...
    def name_get(self):
        return [
            (
                acc.id,
                "{} - {}".format(acc.acc_number, acc.bank_id.name)
                if acc.bank_id
                else acc.acc_number,
            )
            for acc in self
    ...
    def _search(
        ...
        while pos < len(args):
            ...
            pos += 1
        return super(ResPartnerBank, self)._search(
