...
class WebsiteEventBoothController(WebsiteEventController):
    ...
    def _prepare_booth_registration_partner_values(self, event, kwargs):
        if request.env.user._is_public():
            contact_name_email = tools.formataddr(
                (kwargs["contact_name"], kwargs["contact_email"])
            )
            partner = (
                request.env["res.partner"].sudo().find_or_create(contact_name_email)
            )
    ...
    @http.route(
        "/event/booth/check_availability", type="json", auth="public", methods=["POST"]
    )
    ...
    @http.route(
        ["/event/booth_category/get_available_booths"], type="json", auth="public"
    )
