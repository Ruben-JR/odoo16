...
class HrContract(models.Model):
        ...
        for contract in self:
            ...
            for interval in real_leaves:
                ...
                if interval[0] == interval[1]:  # if start == stop
                    continue
                leave_entry_type = contract._get_interval_leave_work_entry_type(
                    interval, leaves, bypassing_work_entry_type_codes
                )
                interval_leaves = [
                    leave
                    for leave in leaves
                ...
                contract_vals += [
                    dict(
                        ...
                        + contract._get_more_vals_leave_interval(
    ...
    def write(self, vals):
        ...
        if any(key in dependendant_fields for key in vals.keys()):
            for contract in self:
                date_from = max(
                    contract.date_start, contract.date_generated_from.date()
                )
                date_to = min(
                    contract.date_end or date.max, contract.date_generated_to.date()
                )
    ...
    def _cron_generate_missing_work_entries(self):
        ...
        contracts_todo = contracts.filtered(
            lambda c: (c.date_generated_from > start or c.date_generated_to < stop)
        ...
        if countract_todo_count > BATCH_SIZE:
            self.env.ref(
                "hr_work_entry_contract.ir_cron_generate_missing_work_entries"
            )._trigger()
