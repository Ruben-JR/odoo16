...
class AccountPaymentTerm(models.Model):
    ...
    def _default_line_ids(self):
        return [
            Command.create(
                {"value": "balance", "value_amount": 0.0, "days": 0, "end_month": False}
            )
        ]

    def _default_example_amount(self):
        return (
            self._context.get("example_amount") or 100
        )  # Force default value if the context is set to False
    ...
    def _compute_example_invalid(self):
        for payment_term in self:
            payment_term.example_invalid = (
                len(payment_term.line_ids.filtered(lambda l: l.value == "balance")) != 1
            )
        ...
        for record in self:
            ...
            if not record.example_invalid:
                ...
                for i, info_by_dates in enumerate(
                    record._get_amount_by_date(terms, currency).values()
                ):
                    ...
                    example_preview += _(
                        ...
                        count=i + 1,
                        amount=formatLang(
                            self.env, amount, monetary=True, currency_obj=currency
                        ),
                        date=date,
                    )
                    if discount_date:
    ...
    def _check_lines(self):
        for terms in self:
            ...
            if terms.line_ids.filtered(
                lambda r: r.value == "fixed" and r.discount_percentage
            ):
                raise ValidationError(
                    _("You can't mix fixed amount with early payment percentage")
                )
    ...
    def _unlink_except_referenced_terms(self):
        if self.env["account.move"].search(
            ...
            raise UserError(
                _(
                    "You can not delete payment terms as other records still reference it. However, you can archive it."
                )
            )
...
class AccountPaymentTermLine(models.Model):
    ...
    value = fields.Selection(
        [("balance", "Balance"), ("percent", "Percent"), ("fixed", "Fixed Amount")],
        string="Type",
        required=True,
        default="percent",
    ...
    end_month = fields.Boolean(
        string="End of month",
        help="Switch to end of the month after having added months or days",
    )
    days_after = fields.Integer(
        string="Days after End of month", help="Days to add after the end of the month"
    )
    discount_percentage = fields.Float(
        string="Discount %", help="Early Payment Discount granted for this line"
    )
    ...
    def _check_percent(self):
        for term_line in self:
            ...
            if term_line.discount_percentage and (
                term_line.discount_percentage < 0.0
                or term_line.discount_percentage > 100.0
            ):
                raise ValidationError(
                    _(
    ...
    def _check_positive(self):
        for term_line in self:
            if term_line.months < 0 or term_line.days < 0:
                raise ValidationError(
                    _(
                        "The Months and Days of the Payment Terms lines must be positive."
                    )
                )
            if term_line.discount_days < 0:
                raise ValidationError(
                    _("The discount days of the Payment Terms lines must be positive.")
                )
