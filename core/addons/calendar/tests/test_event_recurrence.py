...
class TestCreateRecurrentEvents(TestRecurrentEvents):
    ...
    def test_ambiguous_dst_time_winter(self):
        ...
        dt = (
            eastern.localize(datetime(2002, 10, 20, 1, 30, 00))
            .astimezone(pytz.utc)
            .replace(tzinfo=None)
        )
    ...
    def test_ambiguous_dst_time_spring(self):
        ...
        eastern = pytz.timezone("US/Eastern")
        dt = (
            eastern.localize(datetime(2002, 3, 31, 2, 30, 00))
            .astimezone(pytz.utc)
            .replace(tzinfo=None)
        )
    ...
    def test_backward_pass_dst(self):
        ...
        dt = (
            tz.localize(datetime(2023, 3, 27, 9, 0, 00))
            .astimezone(pytz.utc)
            .replace(tzinfo=None)
        )
        self.event.start = dt
...
class TestUpdateRecurrentEvents(TestRecurrentEvents):
    ...
    def test_update_recurrence_future(self):
        ...
        self.assertEqual(
            events[2], self.events[2], "Events on Tuesdays should not have changed"
        )
        self.assertNotEqual(
