...
class Project(models.Model):
    ...
    production_count = fields.Integer(
        related="analytic_account_id.production_count", groups="mrp.group_mrp_user"
    )
    workorder_count = fields.Integer(
        related="analytic_account_id.workorder_count", groups="mrp.group_mrp_user"
    )
    bom_count = fields.Integer(
        related="analytic_account_id.bom_count", groups="mrp.group_mrp_user"
    )
    ...
    def _get_profitability_items(self, with_action=True):
        ...
        if mrp_aal_read_group:
            can_see_manufactoring_order = (
                with_action
                and len(self) == 1
                and self.user_has_groups("mrp.group_mrp_user")
            )
            mrp_costs = {
                "id": mrp_category,
