...
class SaleOrderLine(models.Model):
    ...
    is_reward_line = fields.Boolean(
        "Is a program reward line", compute="_compute_is_reward_line"
    )
    ...
    points_cost = fields.Float(
        help="How much point this reward cost on the loyalty card."
    )
    ...
    def write(self, vals):
        ...
        if cost_in_vals:
            # Update our coupon points if the order is in a confirmed state
            for line in self:
                if previous_cost[line] != line.points_cost and line.order_id.state in (
                    "sale",
                    "done",
                ):
    ...
    def unlink(self):
        ...
        related_lines |= self.order_id.order_line.filtered(
            lambda l: (l.reward_id, l.coupon_id, l.reward_identifier_code)
