...
class SaleOrderOption(models.Model):
    ...
    order_id = fields.Many2one(
        "sale.order", "Sales Order Reference", ondelete="cascade", index=True
    )

    product_id = fields.Many2one(
        comodel_name="product.product", required=True, domain=[("sale_ok", "=", True)]
    )
    ...
    sequence = fields.Integer(
        string="Sequence",
        help="Gives the sequence order when displaying a list of optional products.",
    )
    ...
    quantity = fields.Float(
        string="Quantity", required=True, digits="Product Unit of Measure", default=1
    )
    ...
    def _compute_is_present(self):
        ...
        for option in self:
            option.is_present = bool(
                option.order_id.order_line.filtered(
                    lambda l: l.product_id == option.product_id
                )
            )
