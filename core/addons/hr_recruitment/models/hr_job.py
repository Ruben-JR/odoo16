...
class Job(models.Model):
    ...
    application_count = fields.Integer(
        compute="_compute_application_count", string="Application Count"
    )
    all_application_count = fields.Integer(
        compute="_compute_all_application_count", string="All Application Count"
    )
    ...
    applicant_hired = fields.Integer(
        compute="_compute_applicant_hired", string="Applicants Hired"
    )
    ...
    document_ids = fields.One2many(
        "ir.attachment", compute="_compute_document_ids", string="Documents"
    )
    ...
    def _compute_activities(self):
        ...
        for activity in self.env.cr.dictfetchall():
            job_activities[activity["job_id"]][activity["act_state"]] = activity[
                "act_count"
            ]
        for job in self:
    ...
    def _compute_document_ids(self):
        applicants = self.mapped("application_ids").filtered(
            lambda self: not self.emp_id
        )
        app_to_job = dict(
            (applicant.id, applicant.job_id.id) for applicant in applicants
        )
