...
class TestSaleProject(TransactionCase):
    ...
    def test_sale_order_with_project_task(self):
        ...
        # service_tracking 'no'
        self.assertFalse(
            so_line_order_no_task.project_id,
            "The project should not be linked to no task product",
        )
        self.assertFalse(
            so_line_order_no_task.task_id,
            "The task should not be linked to no task product",
        )
        # service_tracking 'task_global_project'
        self.assertFalse(
            so_line_order_task_in_global.project_id,
            "Only task should be created, project should not be linked",
        )
        self.assertEqual(
            self.project_global.tasks.sale_line_id,
            so_line_order_task_in_global,
            "Global project's task should be linked to so line",
        )
        #  service_tracking 'task_in_project'
        self.assertTrue(
            so_line_order_new_task_new_project.project_id,
            "Sales order line should be linked to newly created project",
        )
        self.assertTrue(
            so_line_order_new_task_new_project.task_id,
            "Sales order line should be linked to newly created task",
        )
        # service_tracking 'project_only'
        self.assertFalse(
            so_line_order_only_project.task_id, "Task should not be created"
        )
        self.assertTrue(
            so_line_order_only_project.project_id,
            "Sales order line should be linked to newly created project",
        )

        self.assertEqual(
            self.project_global._get_sale_order_items(),
            self.project_global.sale_line_id | self.project_global.tasks.sale_line_id,
            "The _get_sale_order_items should returns all the SOLs linked to the project and its active tasks.",
        )
        ...
        self.assertEqual(
            self.project_global._get_sale_orders(), sale_order | sale_order_2
        )

        sale_order_lines = (
            sale_order.order_line + sale_line_1_order_2
        )  # exclude the Section and Note Sales Order Items
        ...
        expected_sale_line_dict = {
            ...
            for sol_read in sale_order_lines.read(
                [
                    "display_name",
                    "product_uom_qty",
                    "qty_delivered",
        ...
        self.assertNotIn(
            section_sale_line_order_2.id,
            actual_sol_ids,
            "The section Sales Order Item should not be takken into account in the Sales section of project.",
        )
        self.assertNotIn(
            note_sale_line_order_2.id,
            actual_sol_ids,
            "The note Sales Order Item should not be takken into account in the Sales section of project.",
        )
    ...
    def test_sol_product_type_update(self):
        ...
        self.assertFalse(
            sale_order_line.is_service,
            "As the product is consumable, the SOL should not be a service",
        )

        self.product_order_service3.type = "service"
        self.assertTrue(
            sale_order_line.is_service,
            "As the product is a service, the SOL should be a service",
        )
    ...
    def test_cancel_so_linked_to_project(self):
        ...
        self.env.user.write(
            {"groups_id": [(6, 0, [group_sale_manager.id, group_project_user.id])]}
        )
        ...
        self.assertFalse(
            self.project_global.tasks.sale_line_id,
            "The project tasks should not be linked to the SOL",
        )

        sale_order.action_confirm()
        self.assertEqual(
            self.project_global.tasks.sale_line_id.id,
            sale_order_line.id,
            "The project tasks should be linked to the SOL from the SO",
        )

        self.project_global.sale_line_id = sale_order_line
        sale_order.with_context({"disable_cancel_warning": True}).action_cancel()
        self.assertFalse(
            self.project_global.sale_line_id,
            "The project should not be linked to the SOL anymore",
        )
