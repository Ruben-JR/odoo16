...
class WebsiteTwitter(models.Model):
    ...
    twitter_api_secret = fields.Char(
        string="Twitter API secret", help="Twitter API Secret"
    )
    ...
    def fetch_favorite_tweets(self):
        ...
        for website in self:
            if not all(
                (
                    website.twitter_api_key,
                    website.twitter_api_secret,
                    website.twitter_screen_name,
                )
            ):
                _logger.debug(
                    "Skip fetching favorite tweets for unconfigured website %s", website
                )
                continue
