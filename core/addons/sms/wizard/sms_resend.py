...
class SMSRecipient(models.TransientModel):
    ...
    notification_id = fields.Many2one(
        "mail.notification", required=True, ondelete="cascade"
    )
...
class SMSResend(models.TransientModel):
    ...
    mail_message_id = fields.Many2one(
        "mail.message", "Message", readonly=True, required=True
    )
    recipient_ids = fields.One2many(
        "sms.resend.recipient", "sms_resend_id", string="Recipients"
    )
    ...
    has_unregistered_account = fields.Boolean(
    ...
    def _compute_has_unregistered_account(self):
        self.has_unregistered_account = self.recipient_ids.filtered(
            lambda p: p.failure_type == "sms_acc"
        )
    ...
    def _check_access(self):
        if (
            not self.mail_message_id
            or not self.mail_message_id.model
            or not self.mail_message_id.res_id
        ):
    ...
    def action_resend(self):
        ...
        sudo_self = self.sudo()
        to_cancel_ids = [
            r.notification_id.id for r in sudo_self.recipient_ids if not r.resend
        ]
        to_resend_ids = [
        ...
        if to_resend_ids:
            ...
            recipients_data = []
            all_recipients_data = self.env["mail.followers"]._get_recipient_data(
                record, "sms", False, pids=pids
            )[record.id]
