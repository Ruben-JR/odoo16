...
class ResConfigSettings(models.TransientModel):
    ...
    group_warning_account = fields.Boolean(
        string="Warnings in Invoices", implied_group="account.group_warning_account"
    )
    ...
    group_show_purchase_receipts = fields.Boolean(
        string="Purchase Receipt", implied_group="account.group_purchase_receipts"
    )
    ...
    invoice_is_print = fields.Boolean(
        string="Print", related="company_id.invoice_is_print", readonly=False
    )
    invoice_is_email = fields.Boolean(
        string="Send Email", related="company_id.invoice_is_email", readonly=False
    )
    ...
    def _compute_account_default_credit_limit(self):
        for setting in self:
            setting.account_default_credit_limit = self.env["ir.property"]._get(
    ...
    def _compute_group_show_line_subtotals(self):
        for wizard in self:
            wizard.group_show_line_subtotals_tax_included = (
                wizard.show_line_subtotals_tax_selection == "tax_included"
            )
            wizard.group_show_line_subtotals_tax_excluded = (
                wizard.show_line_subtotals_tax_selection == "tax_excluded"
            )
