...
class Survey(http.Controller):
    ...
    def _check_validity(
        ...
        if (
            ...
            and answer_sudo.deadline < datetime.now()
        ):
    ...
    def _redirect_with_error(self, access_data, error_key):
        ...
        if error_key == "survey_void" and access_data["can_answer"]:
            ...
            if not answer_sudo:  # survey is not even started
                redirect_url = (
                    "/web/login?redirect=/survey/start/%s" % survey_sudo.access_token
                )
            elif (
                ...
                if answer_sudo.partner_id and (
                    ...
                    if answer_sudo.partner_id.user_ids:
                        ...
                        answer_sudo.partner_id.signup_prepare(
                            expiration=fields.Datetime.now() + relativedelta(days=1)
                        )
    ...
    def survey_start(self, survey_token, answer_token=None, email=False, **post):
        ...
        * a token linked to an answer or generate a new token if access is allowed;
        ...
        return request.redirect(
            "/survey/%s/%s" % (survey_sudo.access_token, answer_sudo.access_token)
        )
    ...
    def _prepare_survey_data(self, survey_sudo, answer_sudo, **post):
        ...
        if "previous_page_id" in post:
            ...
            page_or_question = (
                ...
                .sudo()
                .browse(previous_page_or_question_id)
            )
            data.update(
                {
    ...
    def _prepare_question_html(self, survey_sudo, answer_sudo, **post):
        ...
        if answer_sudo.state == "done":
            survey_content = request.env["ir.qweb"]._render(
                "survey.survey_fill_form_done", survey_data
            )
        else:
            survey_content = request.env["ir.qweb"]._render(
        ...
        return {
            ...
            "survey_navigation": request.env["ir.qweb"]._render(
    ...
    def survey_get_question_image(
        self, survey_token, answer_token, question_id, suggested_answer_id
    ):
        access_data = self._get_access_data(
            survey_token, answer_token, ensure_token=True
        )
    ...
    def survey_submit(self, survey_token, answer_token, **post):
        ...
        for question in questions:
            ...
            if (
                question in inactive_questions
            ):  # if question is inactive, skip validation and save
                continue
    ...
    def _get_user_input_domain(self, survey, line_filter_domain, **post):
        ...
        if post.get("failed"):
            user_input_domain = expression.AND(
                [[("scoring_success", "=", False)], user_input_domain]
            )
        elif post.get("passed"):
            user_input_domain = expression.AND(
                [[("scoring_success", "=", True)], user_input_domain]
            )
