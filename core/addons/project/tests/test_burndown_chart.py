...
class TestBurndownChartCommon(TestProjectCommon):
    ...
    def set_create_date(cls, table, res_id, create_date):
        cls.env.cr.execute(
            "UPDATE {} SET create_date=%s WHERE id=%s".format(table),
            (create_date, res_id),
        )
    ...
    def setUpClass(cls):
        ...
        cls.set_create_date(
            "project_task", task_f.id, datetime(cls.current_year - 1, 12, 20)
        )
...
class TestBurndownChart(TestBurndownChartCommon):
    def map_read_group_result(self, read_group_result):
        return {
            (res["date:month"], res["stage_id"][0]): res["__count"]
    ...
    def check_read_group_results(self, domain, expected_results_dict):
        ...
        for (month, stage_id), __count in read_group_result_dict.items():
            ...
            self.assertEqual(
                __count,
                expected_count,
                "In %s, the number of tasks should be equal to %s in %s stage."
                % (month, expected_count, stages_dict.get(stage_id, "Unknown")),
            )
    ...
    def test_burndown_chart(self):
        ...
        self.check_read_group_results(
            AND([burndown_chart_domain, all_projects_domain_with_ilike]),
            project_expected_dict,
        )

        date_from, date_to = (
            "%s-01-01" % (self.current_year - 1),
            "%s-02-01" % (self.current_year - 1),
        )
