...
class Category(models.Model):
    ...
    display_name = fields.Char(
        compute="_compute_display_name", recursive=True, inverse="_inverse_display_name"
    )
    dummy = fields.Char(store=False)
    discussions = fields.Many2many(
        "test_new_api.discussion",
...
class Discussion(models.Model):
    ...
    important_messages = fields.One2many(
        "test_new_api.message", "discussion", domain=[("important", "=", True)]
    )
    ...
    important_emails = fields.One2many(
        "test_new_api.emailmessage", "discussion", domain=[("important", "=", True)]
    )
...
class Message(models.Model):
    ...
    def _check_author(self):
        for message in self.with_context(active_test=False):
            if (
                message.discussion
                and message.author not in message.discussion.participants
            ):
    ...
    def _compute_name(self):
        for message in self:
            message.name = self._context.get(
                "compute_name",
...
class EmailMessage(models.Model):
    ...
    message = fields.Many2one(
        "test_new_api.message", "Message", required=True, ondelete="cascade"
    )
    email_to = fields.Char("To")
    active = fields.Boolean(
        "Active Message", related="message.active", store=True, related_sudo=False
    )
...
class Book(models.Model):
    ...
    editions = fields.One2many(
        "test_new_api.creativework.edition",
        "res_id",
...
class Bar(models.Model):
    ...
    name = fields.Char()
    foo = fields.Many2one(
        "test_new_api.foo", compute="_compute_foo", search="_search_foo"
    )
...
class Related(models.Model):
    ...
    related_related_name = fields.Char(
        related="related_name", string="A related on a related on Name", readonly=False
    )
...
class Move(models.Model):
    ...
    line_ids = fields.One2many(
        "test_new_api.move_line", "move_id", domain=[("visible", "=", True)]
    )
...
class ComputeOnchange(models.Model):
    ...
    line_ids = fields.One2many(
        "test_new_api.compute.onchange.line",
        "record_id",
...
class ComputeDynamicDepends(models.Model):
    ...
    def _get_full_name_fields(self):
        ...
        depends = self.env["ir.config_parameter"].get_param(
...
class MonetaryInherits(models.Model):
    ...
    monetary_id = fields.Many2one(
        "test_new_api.monetary_base", required=True, ondelete="cascade"
    )
...
class MonetaryOrderLine(models.Model):
    ...
    order_id = fields.Many2one(
        "test_new_api.monetary_order", required=True, ondelete="cascade"
    )
...
class ModelA(models.Model):
    ...
    a_restricted_b_ids = fields.Many2many(
        "test_new_api.model_b", relation="rel_model_a_model_b_1"
    )
    b_restricted_b_ids = fields.Many2many(
        "test_new_api.model_b", relation="rel_model_a_model_b_2", ondelete="restrict"
    )
...
class ModelB(models.Model):
    ...
    a_restricted_a_ids = fields.Many2many(
        "test_new_api.model_a", relation="rel_model_a_model_b_1", ondelete="restrict"
    )
    b_restricted_a_ids = fields.Many2many(
        "test_new_api.model_a", relation="rel_model_a_model_b_2"
    )
...
class ModelActiveField(models.Model):
    ...
    all_children_ids = fields.One2many(
        "test_new_api.model_active_field", "parent_id", context={"active_test": False}
    )
    active_children_ids = fields.One2many(
        "test_new_api.model_active_field", "parent_id", context={"active_test": True}
    )
...
class InverseM2oRef(models.Model):
    ...
    model_ids = fields.One2many(
        "test_new_api.model_many2one_reference", "res_id", string="Models"
    )
...
class ModelParentM2o(models.Model):
    ...
    child_ids = fields.One2many(
        "test_new_api.model_child_m2o", "parent_id", string="Children"
    )
...
class ComputeMember(models.Model):
    ...
    container_id = fields.Many2one(
        "test_new_api.compute.container", compute="_compute_container", store=True
    )
...
class TriggerMiddle(models.Model):
    ...
    _description = (
        "model linking test_new_api.trigger.left and test_new_api.trigger.right"
    )
...
class Ship(models.Model):
    ...
    pirate_ids = fields.Many2many(
        "test_new_api.pirate", "test_new_api_crew", "ship_id", "pirate_id"
    )
    prisoner_ids = fields.Many2many(
        "test_new_api.prisoner", "test_new_api_crew", "ship_id", "prisoner_id"
    )
...
class Pirate(models.Model):
    ...
    ship_ids = fields.Many2many(
        "test_new_api.ship", "test_new_api_crew", "pirate_id", "ship_id"
    )
...
class Prisoner(models.Model):
    ...
    name = fields.Char("Name")
    ship_ids = fields.Many2many(
        "test_new_api.ship", "test_new_api_crew", "prisoner_id", "ship_id"
    )
...
class Precompute(models.Model):
    ...
    commercial_id = fields.Many2one(
        "res.partner", compute="_compute_commercial_id", store=True, precompute=True
    )
...
class RelatedTranslation2(models.Model):
    ...
    parent_id = fields.Many2one(
        "test_new_api.related_translation_1", string="Parent Model"
    )
...
class RelatedTranslation3(models.Model):
    ...
    parent_id = fields.Many2one(
        "test_new_api.related_translation_2", string="Parent Model"
    )
    name = fields.Char("Name Related", related="parent_id.name", readonly=False)
    html = fields.Html("HTML Related", related="parent_id.html", readonly=False)
...
class UnlinkLine(models.Model):
    ...
    container_name = fields.Char(
        "Container Name", related="container_id.name", store=True
    )
