    ...
    def test_mailing_w_blacklist_opt_out(self):
        ...
        for recipient in recipients_all:
            ...
            # falsy: error (failed mail)
                ...
                self.assertEqual(
                    mailing._unsubscribe_token(
                        unsubscribe_params["res_id"], (unsubscribe_params["email"])
                    ),
                ...
                self.assertEqual(
                    mailing._unsubscribe_token(
                        view_params["res_id"], (view_params["email"])
                    ),
                    view_params["token"],
                )
        ...
        self.assertMailingStatistics(
            mailing, expected=13, delivered=6, sent=6, canceled=7, failed=0
        )
