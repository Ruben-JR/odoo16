...
class User(models.Model):
    ...
    google_calendar_rtoken = fields.Char(
        related="google_calendar_account_id.calendar_rtoken", groups="base.group_system"
    )
    ...
    google_synchronization_stopped = fields.Boolean(
        related="google_calendar_account_id.synchronization_stopped", readonly=False
    )

    _sql_constraints = [
        (
            "google_token_uniq",
            "unique (google_calendar_account_id)",
            "The user has already a google account",
        ),
    ]
    ...
    def _get_google_calendar_token(self):
        self.ensure_one()
        if (
            self.google_calendar_account_id.calendar_rtoken
            and not self.google_calendar_account_id._is_google_calendar_valid()
        ):
        ...
        with google_calendar_token(self) as token:
            try:
                (
                    ...
                    self.google_calendar_account_id.calendar_sync_token, token=token
                )
    ...
    def restart_google_synchronization(self):
        ...
        if not self.google_calendar_account_id:
            self.google_calendar_account_id = (
                self.env["google.calendar.credentials"]
                .sudo()
                .create([{"user_ids": [Command.set(self.ids)]}])
            )
