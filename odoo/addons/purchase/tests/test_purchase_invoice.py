...
class TestPurchaseToInvoice(TestPurchaseToInvoiceCommon):
    ...
    def test_purchase_order_to_invoice_analytic_rule_with_account_prefix(self):
        ...
        self.assertFalse(
            po.order_line.analytic_distribution, "There should be no analytic set."
        )
        po.button_confirm()
...
class TestInvoicePurchaseMatch(TestPurchaseToInvoiceCommon):
    ...
    def test_subset_total_match_prefer_purchase(self):
        ...
        invoice._find_and_set_purchase_orders(
            ...
            prefer_purchase_line=True,
        )
        additional_unmatch_po_line = po.order_line.filtered(
            lambda l: l.product_id == self.service_order
        )

        self.assertTrue(invoice.id in po.invoice_ids.ids)
        self.assertTrue(
            additional_unmatch_po_line.id in invoice.line_ids.purchase_line_id.ids
        )
        self.assertTrue(
            invoice.line_ids.filtered(
                lambda l: l.purchase_line_id == additional_unmatch_po_line
            ).quantity
            == 0
        )
    ...
    def test_subset_total_match_reject_purchase(self):
        ...
        self.assertTrue(invoice.id in po.invoice_ids.ids)
        self.assertTrue(
            additional_unmatch_po_line.id not in invoice.line_ids.purchase_line_id.ids
        )
