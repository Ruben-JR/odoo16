...
class TestSelfAccessProfile(TestHrCommon):
    ...
    def test_readonly_fields(self):
        ...
        employee_related_fields = {
            ...
            if fields[el.get("name")].related
            and fields[el.get("name")].related.split(".")[0] == "employee_id"
        }
        ...
        for field in employee_related_fields:
            with self.assertRaises(
                AssertionError,
                msg="Field '%s' should be readonly in the employee profile when self editing is not allowed."
                % field,
            ):
        ...
        cls.richard_emp = cls.env["hr.employee"].create(
            {
                ...
                .create({"name": "Richard", "phone": "21454", "type": "private"})
                .id,
            }
        )
        ...
        cls.hubert_emp = cls.env["hr.employee"].create(
            {
                ...
                "address_home_id": cls.env["res.partner"]
                .create({"name": "Hubert", "type": "private"})
                .id,
            }
        )
