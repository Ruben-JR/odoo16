...
class PurchaseOrder(models.Model):
    ...
    name = fields.Char(
        "Order Reference", required=True, index="trigram", copy=False, default="New"
    )
    ...
    product_id = fields.Many2one(
        "product.product", related="order_line.product_id", string="Product"
    )
    ...
    def _notify_get_recipients_groups(self, msg_vals=None):
        ...
        groups = super(PurchaseOrder, self)._notify_get_recipients_groups(
            msg_vals=msg_vals
        )
        if not self:
            return groups
        ...
        try:
            ...
            if self.env.context.get("is_reminder"):
                ...
                actions.extend(
                    [
                        {
                            "url": self.get_confirm_url(confirm_type="reminder"),
                            "title": _("Accept"),
                        },
        ...
        for order in self:
            for inv in order.invoice_ids:
                if inv and inv.state not in ("cancel", "draft"):
                    raise UserError(
                        _(
                            "Unable to cancel this purchase order. You must first cancel the related vendor bills."
                        )
                    )
    ...
    def _add_supplier_to_product(self):
        ...
        for line in self.order_line:
            ...
            if (
                line.product_id
                and partner not in line.product_id.seller_ids.partner_id
    ...
    def confirm_reminder_mail(self, confirmed_date=False):
        for order in self:
            if (
                ...
                and not order.mail_reminder_confirmed
            ):
                order.mail_reminder_confirmed = True
                date = confirmed_date or self.date_planned.date()
                order.message_post(
                    body=_(
                        "%s confirmed the receipt will take place on %s.",
    ...
    def _confirm_reception_mail(self):
        for order in self:
            if (
                ...
                order.message_post(
                    body=_(
                        "The order receipt has been acknowledged by %s.",
                        order.partner_id.name,
                    )
                )
...
class PurchaseOrderLine(models.Model):
    ...
    date_approve = fields.Datetime(
        related="order_id.date_approve", string="Confirmation Date", readonly=True
    )
    ...
    def _product_id_change(self):
        ...
        if not (
            ...
            and self.product_id.uom_id.category_id == self.product_uom_category_id
        ):
    ...
    def action_purchase_history(self):
        ...
        action["display_name"] = _(
            "Purchase History for %s", self.product_id.display_name
        )
