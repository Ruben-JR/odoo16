            ...
            or (
                ...
                and (
                    any(  # attribute to translate
                        val
                        and key in TRANSLATED_ATTRS
                        and TRANSLATED_ATTRS[key](node[pos])
    ...
    def _get_translation(self, source, module=None):
        try:
            ...
            if lang and lang != "en_US":
                ...
                return code_translations.get_python_translations(module, lang).get(
                    source, source
                )
            else:
                _logger.debug(
    ...
    def __iter__(self):
        for entry in self.pofile:
            ...
            for occurrence, line_number in entry.occurrences:
                match = re.match(
                    r"(model|model_terms):([\w.]+),([\w]+):(\w+)\.([^ ]+)", occurrence
                )
                if match:
    ...
    def add_entry(self, modules, tnrs, source, trad, comments=None):
        ...
        for typy, name, res_id in tnrs:
            ...
            if isinstance(res_id, int) or res_id.isdigit():
                ...
                entry.occurrences.append(("%s:%s" % (typy, name), str(res_id)))
            else:
                entry.occurrences.append(("%s:%s:%s" % (typy, name, res_id), ""))
        if code:
            ...
            for record in records:
                ...
                for field_name, field in record._fields.items():
                    ...
                    if (
                        field.translate
                        and field.store
                        and str(field) != "ir.actions.actions.name"
                    ):
                        name = model + "," + field_name
                        try:
                            value_en = record[field_name] or ""
                            value_lang = (
                                record.with_context(lang=self._lang)[field_name] or ""
                            )
                        except Exception:
                            continue
                        trans_type = (
                            "model_terms" if callable(field.translate) else "model"
                        )
                        for term_en, term_langs in field.get_translation_dictionary(
    ...
    def _babel_extract_terms(
        ...
        try:
            for extracted in extract.extract(
                extract_method, src_file, keywords=extract_keywords, options=options
            ):
...
class TranslationImporter:
    ...
    def load(self, fileobj, fileformat, lang, xmlids=None):
        ...
        if not self.env["res.lang"]._lang_get(lang):
            _logger.error(
                "Couldn't read translation for lang '%s', language not found", lang
            )
            return None
        try:
            fileobj.seek(0)
    ...
    def save(self, overwrite=False, force_overwrite=False):
        ...
        * model terms translation: the term value in the language is different from the term value in ``en_US``.
                ...
                for sub_xmlids in cr.split_for_in_conditions(field_dictionary.keys()):
                    ...
                    for id_, xmlid, values, noupdate in cr.fetchall():
                        ...
                        translation_dictionary = field.get_translation_dictionary(
                            value_en, {k: v for k, v in values.items() if k in langs}
                        )
        ...
        for model_name, model_dictionary in self.model_translations.items():
            ...
            model_table = Model._table
            for field_name, field_dictionary in model_dictionary.items():
                for sub_field_dictionary in cr.split_for_in_conditions(
...
def trans_load(cr, filepath, lang, verbose=True, overwrite=False):
    warnings.warn(
        "The function trans_load is deprecated in favor of TranslationImporter",
        DeprecationWarning,
    )
...
def trans_load_data(cr, fileobj, fileformat, lang, verbose=True, overwrite=False):
    warnings.warn(
        "The function trans_load_data is deprecated in favor of TranslationImporter",
        DeprecationWarning,
    )
    translation_importer = TranslationImporter(cr, verbose=verbose)
...
class CodeTranslations:
    ...
    def _get_po_paths(mod, lang):
        ...
        po_paths = [
            get_resource_path(mod, "i18n", lang_base + ".po"),
    ...
    def _load_web_translations(self, module_name, lang):
        ...
        self.web_translations[(module_name, lang)] = {
            "messages": [
                {"id": src, "string": value} for src, value in translations.items()
            ]
        }
