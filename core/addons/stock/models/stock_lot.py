...
class StockLot(models.Model):
    ...
    company_id = fields.Many2one(
        "res.company", "Company", required=True, store=True, index=True
    )
    delivery_ids = fields.Many2many(
        "stock.picking", compute="_compute_delivery_ids", string="Transfers"
    )
    delivery_count = fields.Integer(
        "Delivery order count", compute="_compute_delivery_ids"
    )
    last_delivery_partner_id = fields.Many2one(
        "res.partner", compute="_compute_delivery_ids"
    )
    ...
    def generate_lot_names(self, first_lot, count):
        ...
        for i in range(0, count):
            lot_names.append(
                "%s%s%s" % (prefix, str(initial_number + i).zfill(padding), suffix)
            )
        return lot_names
    ...
    def _compute_display_complete(self):
        ...
        for prod_lot in self:
            prod_lot.display_complete = prod_lot.id or self._context.get(
                "display_complete"
            )
    ...
    def _compute_delivery_ids(self):
        ...
        for lot in self:
            ...
            if lot.product_id.tracking == "serial" and lot.delivery_count > 0:
                lot.last_delivery_partner_id = lot.delivery_ids.sorted(
                    key=attrgetter("date_done"), reverse=True
                )[0].partner_id
