...
class SurveyInvite(models.TransientModel):
    ...
    emails = fields.Text(
        ...
        help="This list of emails of recipients will not be converted in contacts.\
    ...
    existing_text = fields.Text(
        "Resend Comment", compute="_compute_existing_text", readonly=True, store=False
    )
    # technical info
    ...
    def _compute_existing_partner_ids(self):
        self.existing_partner_ids = list(
            set(self.survey_id.user_input_ids.partner_id.ids)
            & set(self.partner_ids.ids)
        )
    ...
    def _send_mail(self, answer):
        ...
        if email_layout_xmlid:
            template_ctx = {
                ...
                .new(
                    dict(
                        body=mail_values["body_html"], record_name=self.survey_id.title
                    )
                ),
