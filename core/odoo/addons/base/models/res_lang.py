...
class Lang(models.Model):
    ...
    def _lang_get_direction(self, code):
        return (
            self.with_context(active_test=True).search([("code", "=", code)]).direction
        )
    ...
    def get_sorted(self):
        return sorted(
            [
                (lang.code, lang.url_code, lang.name, lang.active, lang.flag_image_url)
                for lang in self
            ],
            key=itemgetter(2),
        )
