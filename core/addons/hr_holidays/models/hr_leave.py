...
class HolidaysRequest(models.Model):
    ...
    def default_get(self, fields_list):
        ...
        if (
            self.env.context.get("holiday_status_name_get", True)
            and "holiday_status_id" in fields_list
        ...
        if "date_from" and "date_to" in fields_list:
            ...
            if (
                not self._context.get("leave_compute_date_from_to")
                and defaults["date_from"].time() == time(0, 0, 0)
                and defaults["date_to"].time() == time(23, 59, 59)
            ):
                employee = (
                    self.env["hr.employee"].browse(defaults["employee_id"])
                ...
                attendance_from, attendance_to = self._get_attendances(
                    employee, date_from, date_to
                )

                defaults["date_from"] = self._get_start_or_end_from_attendance(
                    attendance_from.hour_from, date_from, employee
                )
                defaults["date_to"] = self._get_start_or_end_from_attendance(
                    attendance_to.hour_to, date_to, employee
                )
    ...
    def _default_get_request_parameters(self, values):
        ...
        if values.get("date_from") and values.get("date_to"):
            date_from = self._adjust_date_based_on_tz(
                values["date_from"].date(), values["date_from"].time()
            )
            date_to = self._adjust_date_based_on_tz(
                values["date_to"].date(), values["date_to"].time()
            )
            new_values.update(
            ...
            employee = (
                self.env["hr.employee"].browse(values["employee_id"])
                if values.get("employee_id")
                else self.env.user.employee_id
            )
            default_start_time = self._get_start_or_end_from_attendance(
                7, datetime.now().date(), employee
            ).time()
            default_end_time = self._get_start_or_end_from_attendance(
                19, datetime.now().date(), employee
            ).time()
            if (
                values["date_from"].time() == default_start_time
                and values["date_to"].time() == default_end_time
            ):
                attendance_from, attendance_to = self._get_attendances(
                    employee, date_from, date_to
                )
                new_values["date_from"] = self._get_start_or_end_from_attendance(
                    attendance_from.hour_from, date_from, employee
                )
                new_values["date_to"] = self._get_start_or_end_from_attendance(
                    attendance_to.hour_to, date_to, employee
                )
    ...
    private_name = fields.Char(
        "Time Off Description", groups="hr_holidays.group_hr_holidays_user"
    )
    ...
    report_note = fields.Text(
        "HR Comments", copy=False, groups="hr_holidays.group_hr_holidays_manager"
    )
    ...
    manager_id = fields.Many2one(
        "hr.employee", compute="_compute_from_employee_id", store=True, readonly=False
    )
    ...
    employee_id = fields.Many2one(
        "hr.employee",
        compute="_compute_from_employee_ids",
        store=True,
        string="Employee",
        index=True,
        readonly=False,
    ...
    active_employee = fields.Boolean(
        related="employee_id.active", string="Employee Active", readonly=True
    )
    ...
    duration_display = fields.Char(
        ...
        store=True,
        help="Field allowing to see the leave request duration in days or hours depending on the leave_type_request_unit",
    )  # details
    # details
    ...
    all_employee_ids = fields.Many2many(
        "hr.employee", compute="_compute_all_employees", compute_sudo=True
    )
    ...
    request_unit_half = fields.Boolean(
        "Half Day", compute="_compute_request_unit_half", store=True, readonly=False
    )
    ...
    def _compute_state(self):
        for leave in self:
            leave.state = (
                "confirm" if leave.validation_type != "no_validation" else "draft"
            )
    ...
    def _compute_date_from_to(self):
        for holiday in self:
            ...
            if not holiday.request_date_from:
                ...
                and not holiday.request_unit_hours
                and not holiday.request_date_to
            ):
                ...
                attendance_from, attendance_to = holiday._get_attendances(
                    holiday.employee_id,
                    holiday.request_date_from,
                ...
                holiday.date_from = self._get_start_or_end_from_attendance(
                    hour_from,
                    compensated_request_date_from,
                    holiday.employee_id or holiday,
                )
                holiday.date_to = self._get_start_or_end_from_attendance(
                    hour_to, compensated_request_date_to, holiday.employee_id or holiday
                )
    ...
    def _compute_from_holiday_type(self):
        ...
        if self._context.get(
            "active_model"
        ) == "hr.leave.allocation" and self._context.get("active_id"):
            allocation_from_domain = allocation_from_domain.browse(
                ...
                if not holiday.employee_ids:
                    if allocation_from_domain:
                        ...
                        holiday.holiday_status_id = (
                            allocation_from_domain.holiday_status_id
                        )
                    else:
                        ...
                        holiday.employee_ids = (
                            holiday.employee_id or self.env.user.employee_id
                        )
    ...
    def _compute_has_stress_day(self):
        ...
        if date_from and date_to:
            ...
            for leave in self:
                ...
                leave.has_stress_day = (
                    leave.date_from
                    and leave.date_to
                    and stress_days.filtered_domain(domain)
                )
        else:
            ...
            if holiday.date_from and holiday.date_to:
                ...
                if holiday.state == "validate":
                    ...
                    intervals = (
                        ...
                        - calendar._leave_intervals_batch(start_dt, end_dt, None)[False]
    ...
    def _compute_can_cancel(self):
        ...
        for leave in self:
            leave.can_cancel = (
                leave.id
                and leave.employee_id.user_id == self.env.user
                and leave.state == "validate"
                and leave.date_from
                and leave.date_from > now
            )
                ...
                if holidays_only_have_uid:
                    for conflicting_holiday_data in conflicting_holidays_list:
                        conflicting_holidays_string = _(
                            "From %(date_from)s To %(date_to)s - %(state)s",
    ...
    def _check_holidays(self):
        for holiday in self:
            ...
            if (
                ...
                and not holiday.sudo().employee_ids
            ...
            if holiday.employee_id:
                ...
                if (
                    ...
                    == -1
                    or float_compare(
                        leave_days["virtual_remaining_leaves"], 0, precision_digits=2
                    )
                    == -1
                ):
                    raise ValidationError(
    ...
    def _get_number_of_days(self, date_from, date_to, employee_id):
        """Returns a float equals to the timedelta between two dates given as string."""
        if employee_id:
            return self._get_number_of_days_batch(date_from, date_to, employee_id)[
                employee_id
            ]
        ...
        hours = self.env.company.resource_calendar_id.get_work_hours_count(
            date_from, date_to
        )
        days = (
            hours / (today_hours or HOURS_PER_DAY)
    ...
    def _adjust_date_based_on_tz(self, leave_date, hour):
        ...
        request_date_to_utc = (
            UTC.localize(datetime.combine(leave_date, hour))
            .astimezone(user_tz)
            .replace(tzinfo=None)
        )
    ...
    def onchange(self, values, field_name, field_onchange):
        ...
        if (
            field_onchange.get("employee_id")
            and "employee_id" not in self._context
            and values
        ):
            employee_id = get_employee_from_context(
                values, self._context, self.env.user.employee_id.id
            )
            ...
            if self.env.context.get("short_name"):
                ...
                if leave.leave_type_request_unit == "hour":
                    ...
                    if leave.number_of_days > 1 and date_from_utc and date_to_utc:
                        display_date += (
                            " / %s" % format_date(self.env, date_to_utc) or ""
                        )
                    if (
                        not target
                        or self.env.context.get("hide_employee_name")
                        and "employee_id" in self.env.context.get("group_by", [])
                    ):
                        res.append(
                            (
                                leave.id,
                                _(
                                    "%(leave_type)s: %(duration).2f days (%(start)s)",
                                    leave_type=leave.holiday_status_id.name,
                        ...
                        res.append(
                            (
                                leave.id,
                                _(
                                    "%(person)s on %(leave_type)s: %(duration).2f days (%(start)s)",
                                    person=target,
                                    leave_type=leave.holiday_status_id.name,
                                    duration=leave.number_of_days,
                                    start=display_date,
    ...
    def _check_stress_day(self):
        ...
        if not is_leave_user and any(leave.has_stress_day for leave in self):
            raise ValidationError(
                _("You are not allowed to request a time off on a Stress Day.")
            )
    ...
    def create(self, vals_list):
        ...
        for values in vals_list:
            if values.get("employee_id"):
                ...
                if values.get("date_from") and values.get("date_to"):
                    ...
                    if (
                        values["employee_id"]
                        not in leave_date_employees[(date_from, date_to)]
                    ):
                        leave_date_employees[(date_from, date_to)].append(
                            values["employee_id"]
                        )
        ...
        if self._context.get("leave_compute_date_from_to") and employees:
            ...
            for (date_from, date_to), employee_ids in leave_date_employees.items():
                employee_leave_date_duration[
                    (date_from, date_to)
                ] = self._get_number_of_days_batch(date_from, date_to, employee_ids)
            for values in vals_list:
                ...
                if employee_id and values.get("date_from") and values.get("date_to"):
                    ...
                    attendance_from, attendance_to = self._get_attendances(
                        employee, date_from.date(), date_to.date()
                    )
                    ...
                    values["date_from"] = self._get_start_or_end_from_attendance(
                        hour_from, date_from.date(), employee
                    )
                    values["date_to"] = self._get_start_or_end_from_attendance(
                        hour_to, date_to.date(), employee
                    )
                    values["request_date_from"], values["request_date_to"] = (
                        values["date_from"].date(),
                        values["date_to"].date(),
                    )
                    values["number_of_days"] = employee_leave_date_duration[
                        (date_from, date_to)
        ...
        if not self._context.get("leave_fast_create"):
            ...
            for values in vals_list:
                ...
                if not values.get("department_id"):
                    values.update(
                        {
                            "department_id": employees.filtered(
                                lambda emp: emp.id == employee_id
                            ).department_id.id
                        }
                    )
        ...
        holidays = super(
            HolidaysRequest, self.with_context(mail_create_nosubscribe=True)
        ).create(vals_list)
        ...
        for holiday in holidays:
            if not self._context.get("leave_fast_create"):
                ...
                if holiday.validation_type == "no_validation":
                    ...
                    holiday_sudo.message_subscribe(
                        partner_ids=[
                            holiday._get_responsible_for_approval().partner_id.id
                        ]
                    )
        ...
        if not self.user_has_groups("hr_holidays.group_hr_holidays_user"):
            for hol in self:
                if hol.state not in ["draft", "confirm"]:
                    raise UserError(
                        error_message % state_description_values.get(self[:1].state)
                    )
                if hol.date_from < now:
                    raise UserError(
                        _("You cannot delete a time off which is in the past")
                    )
                if hol.sudo().employee_ids and not hol.employee_id:
                    raise UserError(
                        _("You cannot delete a time off assigned to several employees")
                    )
        else:
            for holiday in self.filtered(
                lambda holiday: holiday.state not in ["draft", "cancel", "confirm"]
            ):
                raise UserError(
                    error_message % (state_description_values.get(holiday.state),)
                )

    def unlink(self):
        return super(
            HolidaysRequest, self.with_context(leave_skip_date_check=True)
        ).unlink()
    ...
    def read_group(
        ...
        if (
            not self.user_has_groups("hr_holidays.group_hr_holidays_user")
            and "private_name" in groupby
        ):
            raise UserError(_("Such grouping is not allowed."))
        return super(HolidaysRequest, self).read_group(
            domain,
            fields,
            groupby,
            offset=offset,
    ...
    def _validate_leave_request(self):
        ...
        holidays = self.filtered(
            lambda request: request.holiday_type == "employee" and request.employee_id
        )
    ...
    def _prepare_employees_holiday_values(self, employees):
        self.ensure_one()
        work_days_data = employees._get_work_days_data_batch(
    ...
    def action_draft(self):
        if any(holiday.state not in ["confirm", "refuse"] for holiday in self):
            raise UserError(
                _(
                    'Time off request state must be "Refused" or "To Approve" in order to be reset to draft.'
                )
            )
        self.write(
            {
    ...
    def action_confirm(self):
        if self.filtered(lambda holiday: holiday.state != "draft"):
            raise UserError(
                _(
                    'Time off request must be in Draft state ("To Submit") in order to confirm it.'
                )
            )
    ...
    def action_validate(self):
        ...
        if any(
            ...
            raise UserError(
                _("Time off request must be confirmed in order to approve it.")
            )
            ...
            if leave.holiday_type != "employee" or (
                ...
                if conflicting_leaves:
                    ...
                    if leave.leave_type_request_unit != "day" or any(
                        ...
                        raise ValidationError(
                            _(
                                "You can not have 2 time off that overlaps on the same day."
                            )
                        )
                    ...
                    split_leaves.filtered(
                        lambda l: l.state in "validate"
        ...
        for holiday in self:
            ...
            if not is_manager and state != "confirm":
                if state == "draft":
                    ...
                    if (
                        holiday.date_from
                        and holiday.date_from.date() <= fields.Date.today()
                    ):
                        raise UserError(
                    ...
                    if (
                        ...
                        holiday.employee_id | holiday.sudo().employee_ids
                    ).leave_manager_id:
                        if holiday.employee_id:
                            employees = holiday.employee_id
                        else:
                            employees = ", ".join(
                                holiday.employee_ids.filtered(
                                    lambda e: e.leave_manager_id != self.env.user
                                ).mapped("name")
                            )
                        raise UserError(
                            _(
    ...
    def _split_leave_on_gto(self, gto):  # gto = global time off
        ...
        if gto_start <= leave_start and gto_end > leave_start and gto_end < leave_end:
            self.write(
                {
        ...
        if gto_start > leave_start and gto_end < leave_end:
            copys = {
                "date_from": self.date_from,
        ...
        if gto_start > leave_start and gto_start < leave_end and gto_end >= leave_end:
            self.write(
                {
                    "date_to": leave_tz.localize(gto_start - timedelta(seconds=1))
                    .astimezone(UTC)
    ...
    def split_leave(self, time_domain_dict):
        ...
        for global_time_off in sorted(
            filter(
                lambda r: r["date_to"] > self.date_from
                and r["date_from"] < self.date_to,
                time_domain_dict,
            ),
        ...
        for leave in self:
            ...
            if recipient:
                self.env["mail.thread"].sudo().message_notify(
                    body=message, partner_ids=[recipient]
                )
    ...
    def _notify_get_recipients_groups(self, msg_vals=None):
        ...
        directly from email."""
        groups = super(HolidaysRequest, self)._notify_get_recipients_groups(
            msg_vals=msg_vals
        )
        if not self:
            return groups
        ...
        if self.state == "confirm":
            app_action = self._notify_get_action_link(
                "controller", controller="/leave/validate", **local_msg_vals
            )
            hr_actions += [{"url": app_action, "title": _("Approve")}]
        if self.state in ["confirm", "validate", "validate1"]:
            ref_action = self._notify_get_action_link(
                "controller", controller="/leave/refuse", **local_msg_vals
            )
        ...
        new_group = (
            ...
            and holiday_user_group_id in pdata["groups"],
            {"actions": hr_actions},
        )
    ...
    def message_subscribe(self, partner_ids=None, subtype_ids=None):
        ...
        if self.state in ["validate", "validate1"]:
            self.check_access_rights("read")
            self.check_access_rule("read")
            return super(HolidaysRequest, self.sudo()).message_subscribe(
                partner_ids=partner_ids, subtype_ids=subtype_ids
            )
        return super(HolidaysRequest, self).message_subscribe(
            partner_ids=partner_ids, subtype_ids=subtype_ids
        )

    @api.model
    def get_unusual_days(self, date_from, date_to=None):
        return self.env.user.employee_id.sudo(False)._get_unusual_days(
            date_from, date_to
        )
    ...
    def _get_start_or_end_from_attendance(self, hour, date, employee):
        ...
        return (
            holiday_tz.localize(datetime.combine(date, hour))
            .astimezone(UTC)
            .replace(tzinfo=None)
        )
    ...
    def _get_attendances(self, employee, request_date_from, request_date_to):
        ...
        if resource_calendar_id.two_weeks_calendar:
            ...
            start_week_type = self.env["resource.calendar.attendance"].get_week_type(
                request_date_from
            )
            ...
            end_week_type = self.env["resource.calendar.attendance"].get_week_type(
                request_date_to
            )
            attendance_actual_week = [
                att
