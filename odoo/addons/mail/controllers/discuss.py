...
class DiscussController(http.Controller):
    ...
    def discuss_channel_chat_from_token(
        self, create_token, channel_name=None, **kwargs
    ):
    ...
    def discuss_channel_invitation(self, channel_id, invitation_token, **kwargs):
        ...
        if (
            not channel_sudo
            or not channel_sudo.uuid
            or not consteq(channel_sudo.uuid, invitation_token)
        ):
            raise NotFound()
    ...
    def discuss_channel(self, channel_id, **kwargs):
        channel_member_sudo = request.env[
            "mail.channel.member"
        ]._get_as_sudo_from_request_or_raise(
            request=request, channel_id=int(channel_id)
        )
        return self._response_discuss_public_channel_template(
    ...
    def _response_discuss_channel_from_token(
        ...
        if (
            ...
            raise NotFound()
        channel_sudo = (
            request.env["mail.channel"].sudo().search([("uuid", "=", create_token)])
        )
        if not channel_sudo:
            try:
        ...
        return self._response_discuss_channel_invitation(
            channel_sudo=channel_sudo, is_channel_token_secret=False
        )
    ...
    def _response_discuss_channel_invitation(
        ...
        channel_member_sudo = channel_sudo.env[
            "mail.channel.member"
        ]._get_as_sudo_from_request(request=request, channel_id=channel_sudo.id)
        if channel_member_sudo:
            ...
            if not channel_sudo.env.user._is_public():
                    ...
                    guest = channel_sudo.env["mail.guest"].create(
                        {
                            ...
                            .search(
                                [("code", "=", request.geoip.get("country_code"))],
                                limit=1,
                            )
                            .id,
                            "lang": get_lang(channel_sudo.env).code,
                            "name": _("Guest"),
                            "timezone": channel_sudo.env[
                                "mail.guest"
                            ]._get_timezone_from_request(request),
                        }
                    )
                ...
                channel_sudo = channel_sudo.with_context(guest=guest)
        response = self._response_discuss_public_channel_template(
                    ...
                    "discussPublicViewData": dict(
                        {
                            "channel": [
                                (
                                    "insert",
                                    {"id": channel_sudo.id, "model": "mail.channel"},
                                )
                            ],
    ...
    @http.route(
        "/mail/channel/<int:channel_id>/partner/<int:partner_id>/avatar_128",
        methods=["GET"],
        type="http",
        auth="public",
    )
    def mail_channel_partner_avatar_128(self, channel_id, partner_id, **kwargs):
        channel_member_sudo = request.env[
            "mail.channel.member"
        ]._get_as_sudo_from_request(request=request, channel_id=channel_id)
        partner_sudo = (
            channel_member_sudo.env["res.partner"].browse(partner_id).exists()
        )
        placeholder = partner_sudo._avatar_get_placeholder_path()
        if channel_member_sudo and channel_member_sudo.env[
            "mail.channel.member"
        ].search(
            [("channel_id", "=", channel_id), ("partner_id", "=", partner_id)], limit=1
        ):
            return (
                request.env["ir.binary"]
                ._get_image_stream_from(
                    partner_sudo, field_name="avatar_128", placeholder=placeholder
                )
                .get_response()
            )
        if request.env.user.share:
            return (
                request.env["ir.binary"]
                ._get_placeholder_stream(placeholder)
                .get_response()
            )
        return (
            request.env["ir.binary"]
            ._get_image_stream_from(
                partner_sudo.sudo(False),
    ...
    @http.route(
        "/mail/channel/<int:channel_id>/guest/<int:guest_id>/avatar_128",
        methods=["GET"],
        type="http",
        auth="public",
    )
    def mail_channel_guest_avatar_128(self, channel_id, guest_id, **kwargs):
        ...
        if channel_member_sudo and channel_member_sudo.env[
            ...
            [("channel_id", "=", channel_id), ("guest_id", "=", guest_id)], limit=1
        ):
            return (
                request.env["ir.binary"]
                ._get_image_stream_from(
                    guest_sudo, field_name="avatar_128", placeholder=placeholder
                )
                .get_response()
            )
        if request.env.user.share:
        ...
        return (
            request.env["ir.binary"]
            ._get_image_stream_from(
                guest_sudo.sudo(False), field_name="avatar_128", placeholder=placeholder
            )
            .get_response()
        )

    @http.route(
        "/mail/channel/<int:channel_id>/attachment/<int:attachment_id>",
        methods=["GET"],
        type="http",
        auth="public",
    )
    def mail_channel_attachment(
        self, channel_id, attachment_id, download=None, **kwargs
    ):
        channel_member_sudo = request.env[
            "mail.channel.member"
        ]._get_as_sudo_from_request_or_raise(
        ...
        return (
            request.env["ir.binary"]
            ._get_stream_from(attachment_sudo)
            .get_response(as_attachment=download)
        )
    ...
    @http.route(
        "/mail/load_message_failures", methods=["POST"], type="json", auth="user"
    )
    ...
    def discuss_starred_messages(self, max_id=None, min_id=None, limit=30, **kwargs):
        return (
            ...
            ._message_fetch(
                domain=[
                    ("starred_partner_ids", "in", [request.env.user.partner_id.id])
                ],
                max_id=max_id,
                min_id=min_id,
                limit=limit,
    ...
    @http.route(
        "/mail/message/update_content", methods=["POST"], type="json", auth="public"
    )
    def mail_message_update_content(self, message_id, body, attachment_ids):
        ...
        if (
            not message_sudo.is_current_user_or_guest_author
            and not guest.env.user._is_admin()
        ):
            raise NotFound()
        if not message_sudo.model or not message_sudo.res_id:
            raise NotFound()
        request.env[message_sudo.model].browse(
            [message_sudo.res_id]
        )._message_update_content(message_sudo, body, attachment_ids=attachment_ids)
        return {
            "id": message_sudo.id,
    ...
    def mail_attachment_delete(self, attachment_id, access_token=None, **kwargs):
        attachment_sudo = (
            request.env["ir.attachment"].browse(int(attachment_id)).sudo().exists()
        )
        if not attachment_sudo:
        ...
        guest = request.env["mail.guest"]._get_guest_from_request(request)
        message_sudo = (
            guest.env["mail.message"]
            .sudo()
            .search([("attachment_ids", "in", attachment_sudo.ids)], limit=1)
        )
        if message_sudo:
    ...
    @http.route(
        "/mail/message/add_reaction", methods=["POST"], type="json", auth="public"
    )
    def mail_message_add_reaction(self, message_id, content):
        ...
        if request.env.user.sudo()._is_public():
            if (
                not guest_sudo
                or not message_sudo.model == "mail.channel"
    ...
    @http.route(
        "/mail/message/remove_reaction", methods=["POST"], type="json", auth="public"
    )
    ...
    def mail_channel_add_guest_as_member(self, channel_id, channel_uuid, **kwargs):
        channel_sudo = (
            request.env["mail.channel"].browse(int(channel_id)).sudo().exists()
        )
        if (
            not channel_sudo
            or not channel_sudo.uuid
            or not consteq(channel_sudo.uuid, channel_uuid)
        ):
            raise NotFound()
        if channel_sudo.channel_type == "chat":
            raise NotFound()
        ...
        if not guest:
            raise NotFound()
        channel_member = channel_sudo.env[
            "mail.channel.member"
        ]._get_as_sudo_from_request(request=request, channel_id=channel_id)
    ...
    def mail_channel_messages(
        self, channel_id, max_id=None, min_id=None, limit=30, **kwargs
    ):
        channel_member_sudo = request.env[
            "mail.channel.member"
        ]._get_as_sudo_from_request_or_raise(
        ...
        messages = channel_member_sudo.env["mail.message"]._message_fetch(
            domain=[
                ("res_id", "=", channel_id),
                ("model", "=", "mail.channel"),
    ...
    @http.route(
        "/mail/channel/notify_typing", methods=["POST"], type="json", auth="public"
    )
    ...
    def channel_ping(self, channel_id, rtc_session_id=None, check_rtc_session_ids=None):
        ...
        (
            ...
            outdated_rtc_sessions,
        ) = channel_member_sudo._rtc_sync_sessions(
        ...
        return {
            "rtcSessions": [
                (
                    "insert",
                    [
                        rtc_session_sudo._mail_rtc_session_format()
                        for rtc_session_sudo in current_rtc_sessions
                    ],
                ),
                (
                    "insert-and-unlink",
                    [
                        {"id": missing_rtc_session_sudo.id}
                        for missing_rtc_session_sudo in outdated_rtc_sessions
                    ],
                ),
            ]
        }
    ...
    def mail_thread_messages(
        ...
        messages = request.env["mail.message"]._message_fetch(
            domain=[
                ("res_id", "=", int(thread_id)),
                ("model", "=", thread_model),
                ("message_type", "!=", "user_notification"),
    ...
    @http.route(
        "/mail/read_subscription_data", methods=["POST"], type="json", auth="user"
    )
    ...
    def channel_call_join(self, channel_id, check_rtc_session_ids=None):
        ...
        return channel_member_sudo._rtc_join_call(
            check_rtc_session_ids=check_rtc_session_ids
        )
    ...
    def channel_call_cancel_invitation(self, channel_id, member_ids=None):
        ...
        return channel_member_sudo.channel_id._rtc_cancel_invitations(
            member_ids=member_ids
        )

    @http.route(
        "/mail/rtc/audio_worklet_processor", methods=["GET"], type="http", auth="public"
    )
    ...
    @http.route(
        "/mail/link_preview/delete", methods=["POST"], type="json", auth="public"
    )
    def mail_link_preview_delete(self, link_preview_id):
        guest = request.env["mail.guest"]._get_guest_from_request(request)
        link_preview_sudo = (
            guest.env["mail.link.preview"]
            .sudo()
            .search([("id", "=", int(link_preview_id))])
        )
        if not link_preview_sudo:
            return
        if (
            not link_preview_sudo.message_id.is_current_user_or_guest_author
            and not guest.env.user._is_admin()
        ):
            return
