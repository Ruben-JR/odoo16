...
class PosConfig(models.Model):
    ...
    iface_orderline_notes = fields.Boolean(
        string="Internal Notes", help="Allow custom internal notes on Orderlines."
    )
    ...
    def _force_http(self):
        ...
        if not enforce_https and self.printer_ids.filtered(
            lambda pt: pt.printer_type == "epson_epos"
        ):
            return True
    ...
    def add_cash_payment_method(self):
        ...
        for company in companies.filtered("chart_template_id"):
            pos_configs = self.search(
                [("company_id", "=", company.id), ("module_pos_restaurant", "=", True)]
            )
            journal_counter = 2
