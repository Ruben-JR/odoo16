...
class TestSurveyInternals(common.TestSurveyCommon):
    ...
    def test_copy_conditional_question_settings(self):
        ...
        self.assertEqual(
            q_food_vegetarian_cloned.triggering_question_id.id,
            q_is_vegetarian_cloned.id,
        )
        self.assertEqual(
            q_food_vegetarian_cloned.triggering_answer_id.id,
            q_is_vegetarian_cloned.suggested_answer_ids[0].id,
        )
        # Doesn't reference the original survey
        self.assertNotEqual(
            q_food_vegetarian_cloned.triggering_question_id.id, q_is_vegetarian.id
        )
        self.assertNotEqual(
            q_food_vegetarian_cloned.triggering_answer_id.id,
        ...
        # Correct conditional layout
        self.assertEqual(
            q_food_not_vegetarian_cloned.triggering_question_id.id,
            q_is_vegetarian_cloned.id,
        )
        self.assertEqual(
            q_food_not_vegetarian_cloned.triggering_answer_id.id,
            q_is_vegetarian_cloned.suggested_answer_ids[1].id,
        )
        # Doesn't reference the original survey
        self.assertNotEqual(
            q_food_not_vegetarian_cloned.triggering_question_id.id, q_is_vegetarian.id
        )
        self.assertNotEqual(
            q_food_not_vegetarian_cloned.triggering_answer_id.id,
    ...
    def test_get_pages_and_questions_to_show(self):
        ...
        text_box_1.write(
            {"is_conditional": True, "triggering_question_id": multiple_choice_1.id}
        )
        numerical_box.write(
            {"is_conditional": True, "triggering_question_id": text_box_1.id}
        )
        text_box_2.write(
            {"is_conditional": True, "triggering_question_id": multiple_choice_1.id}
        )
        matrix.write(
            {"is_conditional": True, "triggering_question_id": page_with_description.id}
        )
        simple_choice_2.write(
            {"is_conditional": True, "triggering_question_id": multiple_choice_2.id}
        )
        simple_choice_3.write(
            {"is_conditional": True, "triggering_question_id": text_box_2.id}
        )
        text_box_3.write(
            {"is_conditional": True, "triggering_question_id": simple_choice_3.id}
        )
