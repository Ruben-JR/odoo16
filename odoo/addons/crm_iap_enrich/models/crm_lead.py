...
class Lead(models.Model):
    ...
    def iap_enrich(self, from_cron=False):
        ...
        for leads in batches:
            ...
            with self._cr.savepoint():
                try:
                    self._cr.execute(
                        "SELECT 1 FROM {} WHERE id in %(lead_ids)s FOR UPDATE NOWAIT".format(
                            self._table
                        ),
                    ...
                    if lead_emails:
                        try:
                            ...
                            _logger.info(
                                "Sent batch %s enrich requests: success",
                                len(lead_emails),
                            )
    ...
    def _iap_enrich_from_response(self, iap_response):
        ...
        for lead in self.search(
            [("id", "in", list(iap_response.keys()))]
        ):  # handle unlinked data by performing a search
            ...
            if (
                ...
                and len(iap_data["phone_numbers"]) > 1
            ):
