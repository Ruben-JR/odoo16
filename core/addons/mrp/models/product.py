    ...
    def write(self, values):
        if "active" in values:
            self.filtered(lambda p: p.active != values["active"]).with_context(
                active_test=False
            ).bom_ids.write({"active": values["active"]})
...
class ProductProduct(models.Model):
    ...
    def write(self, values):
        if "active" in values:
            self.filtered(lambda p: p.active != values["active"]).with_context(
                active_test=False
            ).variant_bom_ids.write({"active": values["active"]})
        return super().write(values)
    ...
    def _compute_quantities_dict(
        ...
        for product in bom_kits:
            ...
            for bom_line, bom_line_data in bom_sub_lines:
                component = bom_line.product_id.with_context(
                ...
                component_res = (
                    ...
                    if component.id in qties
                        ...
                        "virtual_available": float_round(
                            component.virtual_available, precision_rounding=rounding
                        ),
                        "qty_available": float_round(
                            component.qty_available, precision_rounding=rounding
                        ),
                        "incoming_qty": float_round(
                            component.incoming_qty, precision_rounding=rounding
                        ),
                        "outgoing_qty": float_round(
                            component.outgoing_qty, precision_rounding=rounding
                        ),
            ...
            if (
                ...
                res[product.id] = {
                    "virtual_available": min(ratios_virtual_available)
                    * bom_kits[product].product_qty
                    // 1,
                    "qty_available": min(ratios_qty_available)
                    * bom_kits[product].product_qty
                    // 1,
                    "incoming_qty": min(ratios_incoming_qty)
                    * bom_kits[product].product_qty
                    // 1,
                    "outgoing_qty": min(ratios_outgoing_qty)
                    * bom_kits[product].product_qty
                    // 1,
                    "free_qty": min(ratios_free_qty)
                    * bom_kits[product].product_qty
                    // 1,
                }
            else:
    ...
    def action_compute_bom_days(self):
        ...
        for product in self:
            bom_data = (
                ...
                ._get_bom_data(
                    bom_by_products[product], warehouse, product, ignore_stock=True
                )
            )
            availability_delay = bom_data.get("resupply_avail_delay")
            product.days_to_prepare_mo = (
                availability_delay - bom_data.get("lead_time", 0)
                if availability_delay
                else 0
            )
    ...
    def _search_qty_available_new(
        ...
        product_ids = super(ProductProduct, self)._search_qty_available_new(
