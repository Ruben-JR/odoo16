...
class WebsiteVisitor(models.Model):
    ...
    mail_channel_ids = fields.One2many(
        "mail.channel",
        "livechat_visitor_id",
    ...
    @api.depends(
        "mail_channel_ids.livechat_active", "mail_channel_ids.livechat_operator_id"
    )
    ...
    def _compute_session_count(self):
        sessions = self.env["mail.channel"].search(
    ...
    def action_send_chat_request(self):
        ...
        unavailable_visitors_count = self.env["mail.channel"].search_count(
            [("livechat_visitor_id", "in", self.ids), ("livechat_active", "=", True)]
        )
        ...
        for visitor in self:
            ...
            visitor_name = (
                "%s (%s)" % (visitor.display_name, country.name)
                if country
                else visitor.display_name
            )
            ...
            if visitor.partner_id:
                ...
                members_to_add.append(
                    Command.link(self.env.ref("base.public_partner").id)
                )
    ...
    def _upsert_visitor(self, access_token, force_track_values=None):
        ...
        if upsert == "inserted":
            ...
            mail_channel_uuid = json.loads(
                request.httprequest.cookies.get("im_livechat_session", "{}")
            ).get("uuid")
            if mail_channel_uuid:
                mail_channel = (
                    request.env["mail.channel"]
                    .sudo()
                    .search([("uuid", "=", mail_channel_uuid)])
                )
                mail_channel.write(
                    {
