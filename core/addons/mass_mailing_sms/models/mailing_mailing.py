...
class Mailing(models.Model):
    ...
    def _compute_medium_id(self):
        ...
        for mailing in self:
            if mailing.mailing_type == "sms" and (
                ...
                or mailing.medium_id == self.env.ref("utm.utm_medium_email")
            ):
                mailing.medium_id = self.env.ref("mass_mailing_sms.utm_medium_sms").id
            elif mailing.mailing_type == "mail" and (
                not mailing.medium_id
                or mailing.medium_id == self.env.ref("mass_mailing_sms.utm_medium_sms")
            ):
    ...
    def _compute_sms_has_iap_failure(self):
        ...
        if not self.ids:
            ...
            for t in traces:
                trace_dict[t["mass_mailing_id"][0]][t["failure_type"]] = bool(
                    t["__count"]
                )
    ...
    def _get_seen_list_sms(self):
        ...
        if issubclass(type(target), self.pool["mail.thread.phone"]):
            ...
            phone_fields = [
                fname
                for fname in target._sms_get_number_fields()
        ...
        if phone_fields:
            ...
            select_query = "target.id, " + ", ".join(
                "target.%s" % fname for fname in phone_fields
            )
            where_query = " OR ".join(
                "target.%s IS NOT NULL" % fname for fname in phone_fields
            )
            join_add_query = ""
        else:
            ...
            select_query = "target.id, " + ", ".join(
                "partner.%s" % fname for fname in partner_phone_fields
            )
            where_query = " OR ".join(
                "partner.%s IS NOT NULL" % fname for fname in partner_phone_fields
            )
            join_add_query = (
                "JOIN res_partner partner ON (target.%s = partner.id)" % partner_field
            )
    ...
    def _get_default_mailing_domain(self):
        ...
        if (
            ...
            and "phone_sanitized_blacklisted"
            in self.env[self.mailing_model_name]._fields
        ):
            mailing_domain = expression.AND(
                [mailing_domain, [("phone_sanitized_blacklisted", "=", False)]]
            )
