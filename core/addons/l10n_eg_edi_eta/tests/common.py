...
class TestEGEdiCommon(AccountEdiTestCommon):
    ...
    def setUpClass(
        cls,
        chart_template_ref="l10n_eg.egypt_chart_template_standard",
        edi_format_ref="l10n_eg_edi_eta.edi_eg_eta",
    ):
        super().setUpClass(
        ...
        cls.env["res.currency.rate"].create(
            {
                "currency_id": cls.currency_aed_id.id,
                "rate": 0.198117095128,
                "name": "2022-03-15",
            }
        )
    ...
    def _get_tax_by_xml_id(cls, trailing_xml_id):
        return cls.env.ref(
            f"l10n_es.{cls.env.company.id}_account_tax_template_{trailing_xml_id}"
        )
