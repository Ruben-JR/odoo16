...
class PaymentTransaction(models.Model):
    ...
    def _get_specific_rendering_values(self, processing_values):
        ...
        payulatam_values = {
            ...
            "amount": float_repr(
                processing_values["amount"], self.currency_id.decimal_places or 2
            ),
            "tax": 0,
            "taxReturnBase": 0,
            ...
            "confirmationUrl": urls.url_join(
                base_url, PayuLatamController._webhook_url
            ),
            "api_url": api_url,
        }
        if self.provider_id.state != "enabled":
    ...
    def _get_tx_from_notification_data(self, provider_code, notification_data):
        ...
        tx = self.search(
            [("reference", "=", reference), ("provider_code", "=", "payulatam")]
        )
        if not tx:
