...
class SkillLevel(models.Model):
    ...
    def name_get(self):
        ...
        return [
            (record.id, f"{record.name} ({record.level_progress}%)") for record in self
        ]
        ...
        for skill_type in set(self.mapped("skill_type_id")):
            if len(skill_type.skill_level_ids.filtered("default_level")) > 1:
                raise ValidationError(
                    _("Only one default level is allowed per skill type.")
                )

    def action_set_default(self):
        self.ensure_one()
        self.skill_type_id.skill_level_ids.with_context(
            no_skill_level_check=True
        ).default_level = False
