...
class IrAsset(models.Model):
    ...
    def _fill_asset_paths(self, bundle, addons, installed, css, js, asset_paths, seen):
        ...
        def process_path(directive, target, path_def):
            ...
            if directive == INCLUDE_DIRECTIVE:
                ...
                self._fill_asset_paths(
                    path_def, addons, installed, css, js, asset_paths, seen + [bundle]
                )
                return
        ...
        for addon in self._topological_sort(tuple(addons)):
            for command in odoo.modules.module.get_manifest(addon)["assets"].get(
                bundle, ()
            ):
    ...
    def _get_installed_addons_list(self):
        ...
        return self.env.registry._init_modules.union(
            odoo.conf.server_wide_modules or []
        ).union(self.env.context.get("install_module", []))
