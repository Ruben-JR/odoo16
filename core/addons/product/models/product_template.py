...
class ProductTemplate(models.Model):
    ...
    pricelist_item_count = fields.Integer(
        "Number of price rules", compute="_compute_item_count"
    )
    ...
    def _compute_can_image_1024_be_zoomed(self):
        for template in self:
            template.can_image_1024_be_zoomed = (
                template.image_1920
                and tools.is_image_size_above(template.image_1920, template.image_1024)
            )
    ...
    def _compute_has_configurable_attributes(self):
        ...
        for product in self:
            product.has_configurable_attributes = (
                ...
                or any(len(ptal.value_ids) >= 2 for ptal in product.attribute_line_ids)
            )
    ...
    def _constrains_detailed_type(self):
        type_mapping = self._detailed_type_mapping()
        for record in self:
            if record.type != type_mapping.get(
                record.detailed_type, record.detailed_type
            ):
                raise ValidationError(
                    _("The Type of this product doesn't match the Detailed Type")
                )
    ...
    def _check_uom(self):
        if any(
            template.uom_id
            and template.uom_po_id
            and template.uom_id.category_id != template.uom_po_id.category_id
            for template in self
        ):
            raise ValidationError(
                _(
    ...
    def _onchange_uom(self):
        if (
            self.uom_id
            and self.uom_po_id
            and self.uom_id.category_id != self.uom_po_id.category_id
        ):
    ...
    def write(self, vals):
        ...
        if "uom_id" in vals or "uom_po_id" in vals:
            ...
            uom_po_id = (
                self.env["uom.uom"].browse(vals.get("uom_po_id")) or self.uom_po_id
            )
    ...
    def _name_search(
        ...
        while True:
            ...
            products_ids = Product._name_search(
                name,
                args + domain,
                operator=operator,
                name_get_uid=name_get_uid,
                ...
                [
                    (
                        ...
                        self.env["product.template"]._search(
                            [("product_variant_ids.active", "=", True)]
                        ),
                    )
                ]
            )
        if tmpl_without_variant_ids:
            domain = expression.AND(
                [args or [], [("id", "in", tmpl_without_variant_ids.ids)]]
            )
            searched_ids |= set(
    ...
    def price_compute(
        ...
        for template in self:
            ...
            if price_type == "list_price" and self._context.get(
            ...
            if not tmpl_id.has_dynamic_attributes():
                ...
                for combination_tuple in all_combinations:
                    ...
                    is_combination_possible = (
                        tmpl_id._is_combination_possible_by_config(
    ...
    def _is_combination_possible_by_config(self, combination, ignore_no_variant=False):
        ...
        if not (
            attribute_lines.product_template_value_ids._only_active() >= combination
        ):
    ...
    def _get_variant_id_for_combination(self, filtered_combination):
        ...
        if combination_indices_ids:
            domain = expression.AND(
                [domain, [("combination_indices", "=", combination_indices_ids)]]
            )
        else:
            domain = expression.AND(
                [domain, [("combination_indices", "in", ["", False])]]
            )
    ...
    def _get_possible_combinations(
        ...
        product_template_attribute_values_per_line = [
            ptal.product_template_value_ids._only_active() for ptal in attribute_lines
        ]
