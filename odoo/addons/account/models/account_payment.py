...
class AccountPayment(models.Model):
    ...
    qr_code = fields.Char(string="QR Code URL", compute="_compute_qr_code")
    ...
    def _get_aml_default_display_name_list(self):
        ...
        values = [
            (
                ...
                else display_map[(self.payment_type, self.partner_type)],
            ),
            ("sep", " "),
            (
                "amount",
        ...
        for pay in self:
            if pay.payment_type == "inbound":
                pay.available_journal_ids = journals.filtered(
                    lambda j: j.company_id == pay.company_id
                    and j.inbound_payment_method_line_ids.ids != []
                )
            else:
                pay.available_journal_ids = journals.filtered(
                    lambda j: j.company_id == pay.company_id
                    and j.outbound_payment_method_line_ids.ids != []
                )
        ...
        for pay in self:
            if pay.is_internal_transfer:
                pay.destination_account_id = (
                    pay.destination_journal_id.company_id.transfer_account_id
                )
    ...
    def new(self, values=None, origin=None, ref=None):
        payment = super(AccountPayment, self.with_context(is_payment=True)).new(
            values, origin, ref
        )
        if (
            not payment.journal_id
    ...
    def create(self, vals_list):
        ...
        payments = (
            super(AccountPayment, self.with_context(is_payment=True))
            .create(vals_list)
        ...
        for i, pay in enumerate(payments):
            ...
            for k, v in vals_list[i].items():
                if (
                    k in self._fields
                    and self._fields[k].store
                    and k in pay.move_id._fields
