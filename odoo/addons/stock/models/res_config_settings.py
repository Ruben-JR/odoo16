...
class ResConfigSettings(models.TransientModel):
    ...
    group_stock_tracking_lot = fields.Boolean(
        "Packages", implied_group="stock.group_tracking_lot"
    )
    group_stock_tracking_owner = fields.Boolean(
        "Consignment", implied_group="stock.group_tracking_owner"
    )
    ...
    group_warning_stock = fields.Boolean(
        "Warnings for Stock", implied_group="stock.group_warning_stock"
    )
    group_stock_sign_delivery = fields.Boolean(
        "Signature", implied_group="stock.group_stock_sign_delivery"
    )
    module_stock_picking_batch = fields.Boolean("Batch Transfers")
    group_stock_picking_wave = fields.Boolean(
        "Wave Transfers",
        implied_group="stock.group_stock_picking_wave",
    ...
    stock_move_email_validation = fields.Boolean(
        related="company_id.stock_move_email_validation", readonly=False
    )
    ...
    annual_inventory_month = fields.Selection(
        related="company_id.annual_inventory_month", readonly=False
    )
    ...
    group_stock_reception_report = fields.Boolean(
        "Reception Report", implied_group="stock.group_reception_report"
    )
    ...
    def set_values(self):
        ...
        if (
            self.group_stock_storage_categories
            and storage_cate_grp not in base_user_implied_ids
        ):
            putaway_rules = PutawayRule.search(
                [("active", "=", False), ("storage_category_id", "!=", False)]
            )
            if putaway_rules:
                putaway_rules.active = True
        elif (
            not self.group_stock_storage_categories
            and storage_cate_grp in base_user_implied_ids
        ):
        ...
        if (
            not was_operations_showed
            and self.env["stock.picking.type"]
            .with_user(SUPERUSER_ID)
            ._default_show_operations()
        ):
            self.env["stock.picking.type"].with_context(
                ...
                [("code", "!=", "incoming"), ("show_operations", "=", False)]
