...
class TestValuationReconciliation(TestValuationReconciliationCommon):
    ...
    def test_multiple_shipments_invoices(self):
        ...
        picking = self.env["stock.picking"].search(
            [("sale_id", "=", sale_order.id)], order="id asc", limit=1
        )

        invoice = self._create_invoice_for_so(
            sale_order, test_product, "2018-02-03", quantity=3
        )
        invoice.action_post()
        self.check_reconciliation(
            invoice, picking, full_reconcile=False, operation="sale"
        )

        invoice2 = self._create_invoice_for_so(
            sale_order, test_product, "2018-03-12", quantity=2
        )
        invoice2.action_post()
        self.check_reconciliation(
            invoice2, picking, full_reconcile=False, operation="sale"
        )
        ...
        picking = self.env["stock.picking"].search(
            [("sale_id", "=", sale_order.id)], order="id desc", limit=1
        )
