...
class AccountAnalyticLine(models.Model):
    ...
    def _compute_commercial_partner(self):
        for timesheet in self:
            timesheet.commercial_partner_id = (
                timesheet.task_id.commercial_partner_id
                or timesheet.project_id.commercial_partner_id
            )
        ...
        for timesheet in self:
            if timesheet.project_id:  # AAL will be set to False
                ...
                if timesheet.amount >= 0:
                    if (
                        timesheet.so_line
                        and timesheet.so_line.product_id.type == "service"
                    ):
    ...
    def _is_not_billed(self):
        self.ensure_one()
        return (
            not self.timesheet_invoice_id or self.timesheet_invoice_id.state == "cancel"
        )
    ...
    def _check_can_write(self, values):
        if self.sudo().filtered(
            ...
            lambda t: t.timesheet_invoice_id
            and t.timesheet_invoice_id.state != "cancel"
        ):
            if any(
        ...
        if self.task_id.allow_billable and self.task_id.sale_line_id:
            if self.task_id.pricing_type in ("task_rate", "fixed_rate"):
                ...
                map_entry = self.project_id.sale_line_employee_ids.filtered(
                    lambda map_entry: map_entry.employee_id == self.employee_id
                    and map_entry.sale_line_id.order_partner_id.commercial_partner_id
                    == self.task_id.commercial_partner_id
                )
                if map_entry:
                    return map_entry.sale_line_id
                return self.task_id.sale_line_id
    ...
    def _get_timesheets_to_merge(self):
        ...
        return res.filtered(
            lambda l: not l.timesheet_invoice_id
            or l.timesheet_invoice_id.state != "posted"
        )
