...
class HolidaysRequest(models.Model):
    ...
    def _populate_factories(self):
        ...
        allocationless_leave_type_ids = hr_leave_type_records.filtered(
            lambda lt: lt.requires_allocation == "no"
        ).ids

        employee_records = self.env["hr.employee"].browse(employee_ids)
        employee_by_company = {
            k: list(v)
            for k, v in groupby(employee_records, key=lambda rec: rec["company_id"].id)
        }
        company_by_type = {
            rec.id: rec.company_id.id
            for rec in self.env["hr.leave.type"].browse(hr_leave_type_ids)
        }
