...
class TestHttpStatic(TestHttpStaticCommon):
    def test_static00_static(self):
        with self.subTest(x_sendfile=False):
            ...
            self.assertEqual(
                res.headers.get("Cache-Control", ""), "public, max-age=604800"
            )

        with self.subTest(x_sendfile=True), patch.object(
            config, "options", {**config.options, "x_sendfile": True}
        ):
    ...
    def test_static07_attachment_external_url(self):
        ...
        self.assertEqual(
            res.headers.get("Location"), "https://www.youtube.com/watch?v=dQw4w9WgXcQ"
        )
    ...
    def test_static16_public_user_image(self):
        ...
        res = self.url_open(
            f"/web/image/res.users/{public_user.id}/image_128?download=1"
        )
        self.assertEqual(res.status_code, 404)
...
class TestHttpStaticCache(TestHttpStaticCommon):
    ...
    def test_static_cache0_standard(self, domain=""):
        ...
        one_week_away = (datetime.utcnow() + timedelta(weeks=1)).strftime(
            http_date_format
        )
        ...
        self.assertEqual(
            res1.headers.get("Cache-Control"), "public, max-age=604800"
        )  # one week
        self.assertEqual(res1.headers.get("Expires"), one_week_away)
        self.assertIn("ETag", res1.headers)

        res2 = self.nodb_url_open(
            f"{domain}/test_http/static/src/img/gizeh.png",
            headers={"If-None-Match": res1.headers["ETag"]},
        )
        res2.raise_for_status()
        self.assertEqual(
            res2.status_code, 304, "We should not download the file again."
        )
    ...
    def test_static_cache1_unique(self, domain=""):
        ...
        # somehow strftime doesn't set it.
        http_date_format = "%a, %d %b %Y %H:%M:%S GMT"
        one_year_away = (datetime.utcnow() + timedelta(days=365)).strftime(
            http_date_format
        )

        res1 = self.assertDownloadGizeh(
            f"{domain}/web/content/test_http.gizeh_png?unique=1"
        )
        self.assertEqual(
            res1.headers.get("Cache-Control"), "public, max-age=31536000, immutable"
        )  # one year
        self.assertEqual(res1.headers.get("Expires"), one_year_away)
        self.assertIn("ETag", res1.headers)

        res2 = self.db_url_open(
            f"{domain}/web/content/test_http.gizeh_png?unique=1",
            headers={"If-None-Match": res1.headers["ETag"]},
        )
        res2.raise_for_status()
    ...
    def test_static_cache2_nocache(self, domain=""):
        res1 = self.assertDownloadGizeh(
            f"{domain}/web/content/test_http.gizeh_png?nocache=1"
        )
