...
class AccrualPlan(models.Model):
    ...
    def _compute_level_count(self):
        ...
        mapped_count = {
            group["accrual_plan_id"][0]: group["accrual_plan_id_count"]
            for group in level_read_group
        }
        for plan in self:
    ...
    def _compute_employee_count(self):
        ...
        allocations_dict = {
            res["accrual_plan_id"][0]: res["employee_count"]
            for res in allocations_read_group
        }
        for plan in self:
