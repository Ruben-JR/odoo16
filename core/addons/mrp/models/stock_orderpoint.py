...
class StockWarehouseOrderpoint(models.Model):
    ...
    manufacturing_visibility_days = fields.Float(
        default=0.0, help="Visibility Days applied on the manufacturing routes."
    )
    ...
    def _get_qty_multiple_to_order(self):
        ...
        if "manufacture" in self.rule_ids.mapped("action"):
            bom = self.env["mrp.bom"]._bom_find(self.product_id, bom_type="normal")[
                self.product_id
            ]
            return bom.product_uom_id._compute_quantity(
                bom.product_qty, self.product_uom
            )
