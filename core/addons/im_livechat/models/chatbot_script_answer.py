...
class ChatbotScriptAnswer(models.Model):
    ...
    def name_get(self):
        ...
        for answer in self:
            ...
            result.append((answer.id, "%s: %s" % (shortened_message, answer.name)))
    ...
    def _name_search(
        ...
        """
        force_domain_chatbot_script_id = self.env.context.get(
            "force_domain_chatbot_script_id"
        )
        ...
        if force_domain_chatbot_script_id:
            domain = expression.AND(
                [domain, [("chatbot_script_id", "=", force_domain_chatbot_script_id)]]
            )
