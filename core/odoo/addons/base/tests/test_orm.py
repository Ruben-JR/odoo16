...
class TestORM(TransactionCase):
    ...
    def test_access_partial_deletion(self):
        ...
        self.assertTrue(
            type(Model).display_name.automatic, "test assumption not satisfied"
        )
    ...
    def test_search_read(self):
        ...
        self.assertEqual(len(found), 1)
        self.assertTrue(
            field in list(found[0]) for field in ["id", "name", "display_name", "email"]
        )
    ...
    def test_unlink_with_property(self):
        ...
        self.env["ir.property"].with_user(user)._set_multi(
            "ref", "res.partner", {p1.id: "Nain poilu"}
        )
        p1_prop = (
            self.env["ir.property"]
            .with_user(user)
            ._get("ref", "res.partner", res_id=p1.id)
        )
        self.assertEqual(p1_prop, "Nain poilu", "p1_prop should have been created")
...
class TestInherits(TransactionCase):
    ...
    def test_copy_with_ancestor(self):
        ...
        self.assertFalse(
            user_bar.password, "password should not be copied from original record"
        )
