...
class TestSaleStock(TestSaleCommon, ValuationReconciliationTestCommon):
    ...
    def test_00_sale_stock_invoice(self):
        ...
        self.assertEqual(
            ...
            "no",
            'Sale Stock: so invoice_status should be "nothing to invoice" after invoicing',
        )
        ...
        self.assertEqual(
            self.so.invoice_status,
            "to invoice",
            'Sale Stock: so invoice_status should be "to invoice" after partial delivery',
        )
        del_qties = [sol.qty_delivered for sol in self.so.order_line]
        del_qties_truth = [
            1.0 if sol.product_id.type in ["product", "consu"] else 0.0
        ...
        self.assertTrue(
            all(
                [
                    il.product_id.invoice_policy == "delivery"
        ...
        self.assertEqual(
            self.so.invoice_status,
            "to invoice",
            'Sale Stock: so invoice_status should be "to invoice" after complete delivery',
        )
        del_qties = [sol.qty_delivered for sol in self.so.order_line]
        del_qties_truth = [
            2.0 if sol.product_id.type in ["product", "consu"] else 0.0
            for sol in self.so.order_line
        ]
        self.assertEqual(
            del_qties,
    ...
    def test_01_sale_stock_order(self):
        ...
        self.assertEqual(
            self.so.invoice_status,
            "to invoice",
            "Sale Stock: so should be to invoice after invoicing deposit",
        )
    ...
    def test_02_sale_stock_return(self):
        ...
        self.assertEqual(
            self.so.invoice_status,
            "to invoice",
            'Sale Stock: so invoice_status should be "to invoice" instead of "%s" before invoicing'
            % self.so.invoice_status,
        )
        ...
        self.assertEqual(
            self.so.invoice_status,
            "invoiced",
            'Sale Stock: so invoice_status should be "invoiced" instead of "%s" after invoicing'
            % self.so.invoice_status,
        )
        self.assertEqual(
            len(self.inv_1),
        ...
        self.assertEqual(
            self.so.invoice_status,
            "to invoice",
            'Sale Stock: so invoice_status should be "to invoice" instead of "%s" after picking return'
            % self.so.invoice_status,
        )
        self.assertAlmostEqual(
            self.so.order_line.sorted()[0].qty_delivered,
            3.0,
        ...
        self.assertEqual(
            self.so.invoice_status,
            "no",
            'Sale Stock: so invoice_status should be "no" instead of "%s" after invoicing the return'
            % self.so.invoice_status,
        )
    ...
    def test_03_sale_stock_delivery_partial(self):
        ...
        self.assertEqual(
            self.so.invoice_status,
            "to invoice",
            'Sale Stock: so invoice_status should be "to invoice" before invoicing',
        )
        self.inv_1 = self.so._create_invoices()
        self.assertEqual(
            self.so.invoice_status,
            "no",
            'Sale Stock: so invoice_status should be "no" after invoicing',
        )
        self.assertEqual(
    ...
    def test_06_uom(self):
        ...
        move2 = so1.picking_ids.move_ids.filtered(
            lambda m: m.product_uom.id == uom_dozen.id
        )
    ...
    def test_08_sale_return_qty_and_cancel(self):
        ...
        wizard = (
            ...
            .with_context({"order_id": so.id})
            .create({"order_id": so.id})
        )
    ...
    def test_packaging_propagation(self):
        ...
        packOf10 = self.env["product.packaging"].create(
            {"name": "PackOf10", "product_id": product.id, "qty": 10}
        )

        packOf20 = self.env["product.packaging"].create(
            {"name": "PackOf20", "product_id": product.id, "qty": 20}
        )
        ...
        so.order_line[0].write(
            {"product_packaging_id": packOf20.id, "product_uom_qty": 20}
        )
    ...
    def test_16_multi_uom(self):
        ...
        picking.button_validate()
        self.assertEqual(
            so.order_line.mapped("qty_delivered"),
            [4.0],
    ...
    def test_17_qty_update_propagation(self):
        ...
        self.assertEqual(
            ...
            2,
            "A picking and a delivery picking should have been created.",
        )
        ...
        self.assertEqual(
            len(move_out), 1, "Only one move should be created for a single product."
        )
        self.assertEqual(
        ...
        self.assertEqual(
            move_out.product_uom_qty,
            30,
            "The move quantity should have been decreased as the sale order line and the pick line were.",
        )
        self.assertEqual(
            len(sale_order.picking_ids),
            2,
        ...
        sale_order.write(
            {"order_line": [(1, sale_order.order_line.id, {"product_uom_qty": 40})]}
        )
        self.assertEqual(
            move_pick.product_uom_qty,
            40,
    ...
    def test_return_with_mto_and_multisteps(self):
        ...
        (
            ...
            out_location,
            custo_location,
        ) = warehouse.delivery_route_id.rule_ids.location_dest_id
    ...
    def test_mtso_and_qty_decreasing(self):
        ...
        self.env["stock.quant"]._update_available_quantity(
            self.product_a, warehouse.lot_stock_id, 10
        )
