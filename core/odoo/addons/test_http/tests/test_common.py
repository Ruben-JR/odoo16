...
class TestHttpBase(HttpCase):
    ...
    def setUpClass(cls):
        ...
        cls.classPatch(
            odoo.http.root, "session_store", MemorySessionStore(session_class=Session)
        )
    ...
    def nodb_url_open(self, url, *args, allow_redirects=False, **kwargs):
        with patch("odoo.http.db_list") as db_list, patch(
            "odoo.http.db_filter"
        ) as db_filter:
    ...
    def multidb_url_open(self, url, *args, allow_redirects=False, dblist=(), **kwargs):
        ...
        with patch("odoo.http.db_list") as db_list, patch(
            "odoo.http.db_filter"
        ) as db_filter, patch("odoo.http.Registry") as Registry:
