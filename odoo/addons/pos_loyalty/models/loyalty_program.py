...
class LoyaltyProgram(models.Model):
        ...
        for program in self:
            ...
            if program.pos_report_print_id:
                if not program.mail_template_id:
                    ...
                    raise UserError(
                        _(
                            "You must set '%s' before setting '%s'.",
                            mail_template_label,
                            pos_report_print_label,
                        )
                    )
                else:
                    if not program.communication_plan_ids:
                        program.communication_plan_ids = self.env[
                            "loyalty.mail"
                        ].create(
                            {
    ...
    def _compute_pos_order_count(self):
        ...
        for program in self:
            ...
            program.pos_order_count = sum(
                1 if any(id in group["reward_id"] for id in program_reward_ids) else 0
    ...
    def action_view_pos_orders(self):
        ...
        pos_order_ids = list(
            unique(
                r["order_id"]
                for r in self.env["pos.order.line"].search_read(
                    [("reward_id", "in", self.reward_ids.ids)], fields=["order_id"]
                )
            )
        )
        return {
