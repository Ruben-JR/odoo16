...
class TestThirdChecks(L10nLatamCheckTest):
    def create_third_party_check(self, journal=False):
        ...
        vals = {
            ...
            "payment_method_line_id": self.third_party_check_journal._get_available_payment_method_lines(
                "inbound"
            )
    ...
    def test_02_third_party_check_delivery(self):
        ...
        self.assertFalse(
            check.l10n_latam_check_current_journal_id,
        ...
        vals = {
            ...
            "payment_method_line_id": self.rejected_check_journal._get_available_payment_method_lines(
                "inbound"
            )
        ...
        with self.assertRaisesRegex(
            ValidationError, "it can't be received it again"
        ), self.cr.savepoint():
            self.env["account.payment"].create(vals).action_post()
        ...
        operations = self.env["account.payment"].search(
            [("l10n_latam_check_id", "=", check.id), ("state", "=", "posted")],
    ...
    def test_03_check_deposit(self):
        ...
        deposit = (
            ...
            .with_context(active_model="account.payment", active_ids=[check.id])
            .create({"destination_journal_id": bank_journal.id})
            ._create_payments()
        )
        self.assertEqual(
            check.l10n_latam_check_current_journal_id,
        ...
        with self.assertRaisesRegex(
            UserError, "All selected checks must be on the same journal and on hand"
        ), self.cr.savepoint():
            self.env["l10n_latam.payment.mass.transfer"].with_context(
                active_model="account.payment", active_ids=[check.id]
            ).create({"destination_journal_id": bank_journal.id})._create_payments()
        ...
        self.assertEqual(
            len(operations), 5, "There should be 5 operations on the check"
        )
        self.assertEqual(
            operations[0], customer_return, "Last operation should be customer return"
        )
        self.assertEqual(
            operations[2], bank_rejection, "Previous operation should be bank rejection"
        )
        self.assertEqual(
            operations[4], deposit, "First operation should be the deposit"
        )
    ...
    def test_04_check_transfer(self):
        ...
        self.env["l10n_latam.payment.mass.transfer"].with_context(
            active_model="account.payment", active_ids=[check.id, check2.id]
        ).create(
