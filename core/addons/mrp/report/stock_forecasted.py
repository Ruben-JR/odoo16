...
class ReplenishmentReport(models.AbstractModel):
    ...
    def _serialize_docs(
        ...
        for i, line in enumerate(docs["lines"]):
            if (
                not line["move_out"]
                or not line["move_out"]["raw_material_production_id"]
            ):
                continue
            raw_material_production = line["move_out"]["raw_material_production_id"]
            res["lines"][i]["move_out"][
                "raw_material_production_id"
            ] = raw_material_production.read(
    ...
    def _compute_draft_quantity_count(
        ...
        grouped_mo = self.env["mrp.production"].read_group(
        ...
        res["draft_production_qty"]["out"] = sum(
            move["product_qty"] for move in grouped_moves
        )
