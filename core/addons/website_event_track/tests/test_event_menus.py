...
class TestEventWebsiteTrack(OnlineEventCase):
    def _get_menus(self):
        return super(TestEventWebsiteTrack, self)._get_menus() | set(
            ["Talks", "Agenda", "Talk Proposals"]
        )
    ...
    def test_create_menu(self):
        ...
        self._assert_website_menus(
            event,
            ["Introduction", "Location", "Register", "Community"],
            menus_out=["Talks", "Agenda", "Talk Proposals"],
        )
    ...
    def test_menu_management_frontend(self):
        ...
        introduction_menu = event.menu_id.child_id.filtered(
            lambda menu: menu.name == "Introduction"
        )
        introduction_menu.unlink()
        self._assert_website_menus(
            event,
            ["Location", "Register", "Community", "Talks", "Agenda", "Talk Proposals"],
            menus_out=["Introduction"],
        )

        menus = event.menu_id.child_id.filtered(
            lambda menu: menu.name in ["Agenda", "Talk Proposals"]
        )
        menus.unlink()
        self.assertTrue(event.website_track)
        ...
        self._assert_website_menus(
            event,
            ["Location", "Register", "Community"],
            menus_out=["Introduction", "Talks", "Agenda", "Talk Proposals"],
        )
        ...
        self._assert_website_menus(
            event,
            ["Location", "Register", "Community", "Talk Proposals"],
            menus_out=["Introduction", "Talks", "Agenda"],
        )
