...
class PosPaymentMethod(models.Model):
            ...
            if existing_payment_method:
                raise ValidationError(
                    _(
                        ...
                        payment_method.stripe_serial_number,
    ...
    def stripe_connection_token(self):
        ...
        try:
            resp = requests.post(
                endpoint,
                auth=(self.sudo()._get_stripe_secret_key(), ""),
                timeout=TIMEOUT,
            )
        except requests.exceptions.RequestException:
            _logger.exception("Failed to call stripe_connection_token endpoint")
            raise UserError(
                _("There are some issues between us and Stripe, try again later.")
            )
    ...
    def stripe_capture_payment(self, paymentIntentId, amount=None):
        ...
        endpoint = ("payment_intents/%s/capture") % (
        ...
        return (
            ...
            ._stripe_make_request(endpoint, data)
        )
