...
class AccountPartialReconcile(models.Model):
    ...
    full_reconcile_id = fields.Many2one(
        comodel_name="account.full.reconcile", string="Full Reconcile", copy=False
    )
    ...
    max_date = fields.Date(
        string="Max Date of Matched Lines", store=True, compute="_compute_max_date"
    )
    ...
    def _prepare_cash_basis_base_line_vals(self, base_line, balance, amount_currency):
        ...
        tax_ids = base_line.tax_ids.filtered(
    ...
    def _prepare_cash_basis_tax_line_vals(self, tax_line, balance, amount_currency):
        ...
        base_tags = tax_ids.get_tax_tags(
            tax_line.tax_repartition_line_id.refund_tax_id, "base"
        )
    ...
    def _create_tax_cash_basis_moves(self):
        ...
        for move_values in tax_cash_basis_values_per_move.values():
            ...
            for partial_values in move_values["partials"]:
                ...
                move_date = (
                    partial.max_date
                    if partial.max_date > (move.company_id.period_lock_date or date.min)
                    else today
                )
                move_vals = {
                    "move_type": "entry",
