...
class GoogleCalendarService:
    ...
    def get_events(self, sync_token=None, token=None, timeout=TIMEOUT):
        ...
        try:
            status, data, time = self.google_service._do_request(
    ...
    def insert(self, values, token=None, timeout=TIMEOUT):
        ...
        url = (
            "/calendar/v3/calendars/primary/events?conferenceDataVersion=1&sendUpdates=%s"
            % ("all" if send_updates else "none")
        )
        headers = {
    ...
    def _google_authentication_url(self, from_url="http://www.odoo.com"):
        state = {"d": self.google_service.env.cr.dbname, "s": "calendar", "f": from_url}
        base_url = (
            self.google_service._context.get("base_url")
