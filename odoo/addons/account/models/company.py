...
class ResCompany(models.Model):
    ...
    chart_template_id = fields.Many2one(
        "account.chart.template", help="The chart template for the company (if any)"
    )
    ...
    default_cash_difference_income_account_id = fields.Many2one(
        "account.account", string="Cash Difference Income Account"
    )
    default_cash_difference_expense_account_id = fields.Many2one(
        "account.account", string="Cash Difference Expense Account"
    )
    account_journal_suspense_account_id = fields.Many2one(
        "account.account", string="Journal Suspense Account"
    )
    account_journal_payment_debit_account_id = fields.Many2one(
        "account.account", string="Journal Outstanding Receipts Account"
    )
    account_journal_payment_credit_account_id = fields.Many2one(
        "account.account", string="Journal Outstanding Payments Account"
    )
    ...
    account_setup_bank_data_state = fields.Selection(
        ONBOARDING_STEP_STATES,
        string="State of the onboarding bank data step",
        default="not_done",
    )
    account_setup_fy_data_state = fields.Selection(
        ONBOARDING_STEP_STATES,
        string="State of the onboarding fiscal year step",
        default="not_done",
    )
    account_setup_coa_state = fields.Selection(
        ONBOARDING_STEP_STATES,
        string="State of the onboarding charts of account step",
        default="not_done",
    )
    account_setup_taxes_state = fields.Selection(
        ONBOARDING_STEP_STATES,
        string="State of the onboarding Taxes step",
        default="not_done",
    )
    account_onboarding_invoice_layout_state = fields.Selection(
        ONBOARDING_STEP_STATES,
        string="State of the onboarding invoice layout step",
        default="not_done",
    )
    account_onboarding_create_invoice_state = fields.Selection(
        ONBOARDING_STEP_STATES,
    ...
    account_onboarding_create_invoice_state_flag = fields.Boolean(
    ...
    account_setup_bill_state = fields.Selection(
        ONBOARDING_STEP_STATES,
    ...
    account_default_pos_receivable_account_id = fields.Many2one(
        "account.account", string="Default PoS Receivable Account"
    )
    ...
    def _compute_account_onboarding_create_invoice_state(self):
        for record in self:
            if record.account_onboarding_create_invoice_state_flag:
                ...
                [("company_id", "=", record.id), ("move_type", "=", "out_invoice")],
                limit=1,
            ):
    ...
    def _compute_invoice_terms_html(self):
        for company in self.filtered(
            lambda company: is_html_empty(company.invoice_terms_html)
    ...
    def reflect_code_prefix_change(self, old_code, new_code):
        ...
        for account in accounts:
            if account.code.startswith(old_code):
                account.write(
                    {
                        "code": self.get_new_account_code(
                            account.code, old_code, new_code
                        )
                    }
                )
    ...
    def _validate_fiscalyear_lock(self, values):
        if values.get("fiscalyear_lock_date"):
            ...
            if unreconciled_statement_lines:
                ...
                action_error = (
                    self._get_fiscalyear_lock_statement_lines_redirect_action(
    ...
    def _get_user_fiscal_lock_date(self):
        ...
        self.ensure_one()
        lock_date = max(
            self.period_lock_date or date.min, self.fiscalyear_lock_date or date.min
        )
    ...
    def _auto_balance_opening_move(self):
        ...
        """
        if (
            self.account_opening_move_id
            and self.account_opening_move_id.state == "draft"
        ):
            ...
            if float_is_zero(
                debit_diff + credit_diff, precision_rounding=currency.rounding
            ):
                ...
                if balancing_move_line:
                    ...
                    balancing_move_line.write(
                        {"debit": credit_diff, "credit": debit_diff}
                    )
                else:
    ...
    def _check_hash_integrity(self):
        ...
        results_by_journal = {
            "results": [],
            "printing_date": format_date(
                self.env, fields.Date.to_string(fields.Date.context_today(self))
            ),
        }
        ...
        for journal in journals:
            rslt = {
                ...
                "restricted_by_hash_table": journal.restrict_mode_hash_table
                and "V"
                or "X",
                "msg_cover": "",
                "first_hash": "None",
            ...
            if not moves:
                rslt.update(
                    {
                        "msg_cover": _(
                            "There isn't any journal entry flagged for data inalterability yet for this journal."
                        ),
                    }
                )
                results_by_journal["results"].append(rslt)
            ...
            for move in moves:
                ...
                )._compute_hash(previous_hash=previous_hash)
                while (
                ...
                if move.inalterable_hash != computed_hash:
                    rslt.update(
                        {
                            "msg_cover": _(
                                "Corrupted data on journal entry with id %s.", move.id
                            )
                        }
                    )
                    results_by_journal["results"].append(rslt)
            ...
            if len(moves) == all_moves_count:
                ...
                rslt.update(
                    {
                        "msg_cover": _("Entries are hashed from %s (%s)")
                        % (
                            start_move_info[0],
