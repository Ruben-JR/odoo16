...
class MrpWorkorder(models.Model):
    ...
    operation_note = fields.Html(
        "Description", related="operation_id.note", readonly=True
    )
    ...
    production_date = fields.Datetime(
        "Production Date", related="production_id.date_planned_start", store=True
    )
    ...
    def _compute_state(self):
        ...
        for workorder in self:
            ...
            if not all(
                ...
                continue
            if workorder.production_id.reservation_state not in (
                "waiting",
                "confirmed",
                "assigned",
            ):
                continue
            if (
                workorder.production_id.reservation_state == "assigned"
                and workorder.state == "waiting"
            ):
                workorder.state = "ready"
            elif (
                workorder.production_id.reservation_state != "assigned"
                and workorder.state == "ready"
            ):
    ...
    def _compute_json_popover(self):
        ...
        for wo in self:
            ...
            if wo.state in ("pending", "waiting", "ready"):
                ...
                prev_start = (
                    min([workorder.date_planned_start for workorder in previous_wos])
                    if previous_wos
                    else False
                )
                prev_finished = (
                    max([workorder.date_planned_finished for workorder in previous_wos])
                    if previous_wos
                    else False
                )
                if (
                    wo.state == "pending"
                    and prev_start
                    and not (prev_start > wo.date_planned_start)
                ):
                    infos.append(
                        {
                            "color": "text-primary",
    ...
    def _compute_working_users(self):
        ...
        for order in self:
            order.working_user_ids = [
                (4, order.id)
                for order in order.time_ids.filtered(lambda time: not time.date_end)
                .sorted("date_start")
                .mapped("user_id")
            ]
            if order.working_user_ids:
                ...
                order.last_working_user_id = (
                    order.time_ids.filtered("date_end").sorted("date_end")[-1].user_id
                    if order.time_ids.filtered("date_end")
    ...
    def _calculate_date_planned_finished(self, date_planned_start=False):
        return self.workcenter_id.resource_calendar_id.plan_hours(
            ...
            date_planned_start or self.date_planned_start,
            compute_leaves=True,
    ...
    def _calculate_duration_expected(
        self, date_planned_start=False, date_planned_finished=False
    ):
        interval = self.workcenter_id.resource_calendar_id.get_work_duration_data(
            date_planned_start or self.date_planned_start,
            date_planned_finished or self.date_planned_finished,
            domain=[("time_type", "in", ["leave", "other"])],
        )
            ...
            for workorder in self:
                if workorder.workcenter_id.id != values["workcenter_id"]:
                    if workorder.state in ("progress", "done", "cancel"):
                        raise UserError(
                            _(
                                "You cannot change the workcenter of a work order that is in progress or done."
                            )
                        )
                    workorder.leave_id.resource_id = (
                        self.env["mrp.workcenter"]
                        .browse(values["workcenter_id"])
                        .resource_id
                    )
        if "date_planned_start" in values or "date_planned_finished" in values:
            for workorder in self:
                ...
                if "duration_expected" not in values and not self.env.context.get(
                    ...
                    if values.get("date_planned_start") and values.get(
                        "date_planned_finished"
                    ):
                        computed_finished_time = (
                            workorder._calculate_date_planned_finished(start_date)
                        )
                ...
                if (
                    ...
                    and "date_planned_start" in values
                ):
                ...
                if (
                    ...
                    and "date_planned_finished" in values
                ):
    ...
    def gantt_unavailability(
        ...
        def search_workcenter_ids(row):
            if (
                row.get("groupedBy")
                and row.get("groupedBy")[0] == "workcenter_id"
                and row.get("resId")
            ):
        ...
        cell_dt = (
            (scale in ["day", "week"] and timedelta(hours=1))
            or (scale == "month" and timedelta(days=1))
    ...
    def button_start(self):
        ...
        if any(
            not time.date_end
            for time in self.time_ids.filtered(
    ...
    def button_finish(self):
        ...
        for workorder in self:
            ...
            if (
                not workorder.date_planned_start
                or end_date < workorder.date_planned_start
            ):
    ...
    def _get_conflicted_workorder_ids(self):
        ...
        """
        self.flush_model(
            ["state", "date_planned_start", "date_planned_finished", "workcenter_id"]
        )
        sql = """
    ...
    def _update_finished_move(self):
        ...
        if production_move.product_id.tracking != "none":
            ...
            if move_line:
                ...
                putaway_location = (
                    production_move.location_dest_id._get_putaway_strategy(
