...
class PosOrder(models.Model):
    ...
    multiprint_resume = fields.Char(
        string="Multiprint Resume", help="Last printed state of the order"
    )
    ...
    def _get_pack_lot_lines(self, order_lines):
        ...
        pack_lots = self.env["pos.pack.operation.lot"].search_read(
            domain=[
                (
                    "pos_order_line_id",
                    "in",
                    [order_line["id"] for order_line in order_lines],
                )
            ],
            fields=["id", "lot_name", "pos_order_line_id"],
        )
        ...
        for order_line_id, pack_lot_ids in groupby(
            pack_lots, key=lambda x: x["order_line"]
        ):
            next(
                order_line
                for order_line in order_lines
                if order_line["id"] == order_line_id
            )["pack_lot_ids"] = list(pack_lot_ids)
    ...
    def _prepare_order_line(self, order_line):
        ...
        if not "pack_lot_ids" in order_line:
            ...
            order_line["pack_lot_ids"] = [
                [0, 0, lot] for lot in order_line["pack_lot_ids"]
            ]
        return order_line
    ...
    def _get_order_lines(self, orders):
        ...
        for order_id, order_lines in groupby(
            extended_order_lines, key=lambda x: x[2]["order_id"]
        ):
            next(order for order in orders if order["id"] == order_id[0])[
                "lines"
            ] = list(order_lines)
    ...
    def _get_payment_lines(self, orders):
        ...
        for order_id, payment_lines in groupby(
            extended_payment_lines, key=lambda x: x[2]["pos_order_id"]
        ):
            next(order for order in orders if order["id"] == order_id[0])[
                "statement_ids"
            ] = list(payment_lines)
    ...
    def get_table_draft_orders(self, table_ids):
        ...
        for order in table_orders:
            ...
            order["uid"] = search(
                r"\d{5,}-\d{3,}-\d{4,}", order["pos_reference"]
            ).group(0)
    ...
    def set_tip(self, tip_line_vals):
        ...
        process_line = partial(
            PosOrderLine._order_line_fields, session_id=self.session_id.id
        )
        ...
        tip_line = self.lines.filtered(
            lambda line: line.product_id == self.session_id.config_id.tip_product_id
        )
        if not tip_line:
