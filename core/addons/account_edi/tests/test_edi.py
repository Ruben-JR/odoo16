...
class TestAccountEdi(AccountEdiTestCommon, CronMixinCase):
    ...
    def test_export_edi(self):
        ...
        with self.with_custom_method(
            ...
            "_test_edi_post_invoice", lambda edi_format, inv: {inv: {"success": True}}
        ):
            invoice.action_post()
    ...
    def test_prepare_jobs_no_batching(self):
        ...
        with self.with_custom_method(
            ...
            "_needs_web_services", lambda edi_format: True
        ):
    ...
    def test_edi_flow(self):
        ...
        with self.with_custom_method(
            ...
            "_test_edi_cancel_invoice", lambda edi_format, inv: {inv: {"success": True}}
        ):
            invoice.action_post()
            self.assertRecordValues(invoice.edi_document_ids, [{"state": "to_send"}])
    ...
    def test_edi_flow_two_steps(self):
        ...
        with self.with_custom_method(
            "_get_move_applicability", get_move_applicability
        ), self.with_custom_method(
            "_needs_web_services", lambda edi_format: True
        ), self.with_custom_method(
            "_test_edi_post_invoice_step1", step1
        ), self.with_custom_method(
            "_test_edi_post_invoice_step2", step2
        ):
            invoice.action_post()
            self.assertRecordValues(invoice.edi_document_ids, [{"state": "to_send"}])
    ...
    def test_cron_triggers(self):
        ...
        with self.with_custom_method(
            ...
            "account_edi.ir_cron_edi_network"
        ) as capt:
            invoice.action_post()
