...
class StockPicking(models.Model):
    ...
    def _compute_display_action_record_components(self):
        ...
        for picking in self:
            ...
            if (
                picking.state in ("draft", "cancel", "done")
                or not picking._is_subcontract()
            ):
                continue
    ...
    def _action_done(self):
        ...
        for move in self.move_ids.filtered(lambda move: move.is_subcontract):
            ...
            recorded_productions = productions.filtered(
                lambda p: p._has_been_recorded()
            )
            recorded_qty = sum(recorded_productions.mapped("qty_producing"))
            sm_done_qty = sum(
                productions._get_subcontract_move().mapped("quantity_done")
            )
            ...
            if len(production) > 1:
                raise UserError(
                    _(
                        "There shouldn't be multiple productions to record for the same subcontracted move."
                    )
                )
            ...
            productions = production._split_productions(
                {production: amounts}, set_consumed_qty=True
            )
        ...
        for picking in self:
            ...
            productions_to_done.with_context(
                mo_ids_to_backorder=production_ids_backorder
            ).button_mark_done()
    ...
    def _prepare_subcontract_mo_vals(self, subcontract_move, bom):
        ...
        vals = {
            ...
            "bom_id": bom.id,
            "location_src_id": subcontract_move.picking_id.partner_id.with_company(
