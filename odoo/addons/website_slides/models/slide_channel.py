...
class ChannelUsersRelation(models.Model):
    ...
    channel_id = fields.Many2one(
        "slide.channel", index=True, required=True, ondelete="cascade"
    )
        ...
        for template, records in template_to_records.items():
            record_email_values.update(
                template.generate_email(
                    records.ids, ["subject", "body_html", "email_from", "partner_to"]
                )
            )
                ...
                add_context={
                    ...
                    .new(
                        dict(
                            ...
                            record_name=record.channel_id.name,
                        )
                    ),
                    "model_description": _(
                        "Completed Course"
...
class Channel(models.Model):
    ...
    website_background_image_url = fields.Char(
        "Background image URL", compute="_compute_website_background_image_url"
    )
    ...
    def _compute_members_count(self):
        read_group_res = (
            ...
            .sudo()
            ._read_group([("channel_id", "in", self.ids)], ["channel_id"], "channel_id")
        )
        data = dict(
    ...
    def _compute_members_done_count(self):
        read_group_res = (
            ...
            .sudo()
            ._read_group(
                ["&", ("channel_id", "in", self.ids), ("completed", "=", True)],
                ["channel_id"],
                "channel_id",
            )
        )
    ...
    def _get_placeholder_filename(self, field):
        ...
        if field in image_fields:
            return "website_slides/static/src/img/channel-%s-default.jpg" % (
                "training" if self.channel_type == "training" else "documentation"
            )
    ...
    def _compute_website_background_image_url(self):
        for channel in self:
            channel.website_background_image_url = (
                channel.website_id.image_url(channel, "image_256")
                if channel.image_1920
    ...
    def action_view_ratings(self):
        ...
        action["domain"] = expression.AND(
            [ast.literal_eval(action.get("domain", "[]")), [("res_id", "in", self.ids)]]
        )
        return action
    ...
    def _search_get_detail(self, website, order, options):
        ...
        if search_tags:
            ...
            try:
                tag_ids = list(
                    filter(None, [unslug(tag)[1] for tag in search_tags.split(",")])
                )
                tags = (
