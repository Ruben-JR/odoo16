...
class SaleOrderLine(models.Model):
    ...
    project_id = fields.Many2one(
        "project.project", "Generated Project", index=True, copy=False
    )
    ...
    is_service = fields.Boolean(
        "Is a Service", compute="_compute_is_service", store=True, compute_sudo=True
    )
                    ...
                    result += [
                        (
                            ...
                            % (
                                names.get(line.id),
                                format_amount(
                                    self.env, line.price_unit, line.currency_id
                                ),
                            ),
                        )
                        for line in sols
    ...
    @api.depends(
        ...
        "product_uom_qty",
        "reached_milestones_ids.quantity_percentage",
    )
    def _compute_qty_delivered(self):
        lines_by_milestones = self.filtered(
            lambda sol: sol.qty_delivered_method == "milestones"
        )
        ...
        reached_milestones_per_sol = {
            res["sale_line_id"][0]: res["quantity_percentage"]
            for res in project_milestone_read_group
        }
        for line in lines_by_milestones:
            sol_id = line.id or line._origin.id
            line.qty_delivered = (
                reached_milestones_per_sol.get(sol_id, 0.0) * line.product_uom_qty
            )
    ...
    def _timesheet_create_project_prepare_values(self):
        ...
        if not account:
            ...
            default_code = (
                service_products.default_code if len(service_products) == 1 else None
            )
    ...
    def _prepare_invoice_line(self, **optional_values):
        ...
        if not values.get("analytic_distribution"):
            task_analytic_account = (
                self.task_id._get_task_analytic_account_id() if self.task_id else False
            )
            if task_analytic_account:
                ...
                analytic_account_ids = {
                    rec["analytic_account_id"][0]
                    for rec in (task_analytic_account_id + project_analytic_account_id)
                }
