...
class TestSMSActionsCommon(TestSMSCommon, TestSMSRecipients):
    ...
    def test_search_phone_mobile_search_equal(self):
        ...
        # test "=" search
        results = self.env["mail.test.sms.bl"].search(
            [("phone_mobile_search", "=", "0475")]
        )
        self.assertFalse(
            results,
            "Search on phone_mobile_search: = should return only matching results",
        )
        results = self.env["mail.test.sms.bl"].search(
            [("phone_mobile_search", "=", "0475000000")]
        )
        self.assertEqual(results, test_phone_records[0])
        results = self.env["mail.test.sms.bl"].search(
            [("phone_mobile_search", "=", "0032475110606")]
        )
        self.assertEqual(results, test_phone_records[6])
        results = self.env["mail.test.sms.bl"].search(
            [("phone_mobile_search", "=", "+32475110606")]
        )
        self.assertEqual(results, test_phone_records[6])
    ...
    def test_search_phone_mobile_search_ilike(self):
        ...
        results = self.env["mail.test.sms.bl"].search(
            [("phone_mobile_search", "not ilike", "0475")]
        )
        self.assertEqual(results, test_phone_records - test_phone_records[:5])
        results = self.env["mail.test.sms.bl"].search(
            [("phone_mobile_search", "not ilike", "101")]
        )
