...
class ProductTemplate(models.Model):
    ...
    cost_method = fields.Selection(
        related="categ_id.property_cost_method", readonly=True
    )
    ...
    def write(self, vals):
        ...
        if "categ_id" in vals:
            ...
            # the stock.
            new_product_category = self.env["product.category"].browse(
                vals.get("categ_id")
            )

            for product_template in self:
                product_template = product_template.with_company(
                ...
                if (
                    ...
                    != new_product_category.property_valuation
                ):
                    valuation_impacted = True
                ...
                (
                    ...
                    products,
                ) = Product._svl_empty_stock(
                    description, product_template=product_template
                )
                out_stock_valuation_layers = SVL.create(out_svl_vals_list)
                if product_template.valuation == "real_time":
                    move_vals_list += Product._svl_empty_stock_am(
                        out_stock_valuation_layers
                    )
                impacted_templates[product_template] = (
                    products,
                    description,
                    products_orig_quantity_svl,
                )
        ...
        for product_template, (
            products,
            description,
            products_orig_quantity_svl,
        ) in impacted_templates.items():
            # Replenish the stock with the new cost method.
            in_svl_vals_list = products._svl_replenish_stock(
                description, products_orig_quantity_svl
            )
            in_stock_valuation_layers = SVL.create(in_svl_vals_list)
            if product_template.valuation == "real_time":
                move_vals_list += Product._svl_replenish_stock_am(
                    in_stock_valuation_layers
                )

        # Check access right
        if move_vals_list and not self.env["stock.valuation.layer"].check_access_rights(
            "read", raise_exception=False
        ):
            raise UserError(
                _(
                    "The action leads to the creation of a journal entry, for which you don't have the access rights."
                )
            )
    ...
    def _get_product_accounts(self):
        ...
        accounts.update(
            {
                "stock_input": res["stock_input"]
                or self.categ_id.property_stock_account_input_categ_id,
                "stock_output": res["stock_output"]
...
class ProductProduct(models.Model):
    ...
    def write(self, vals):
        if "standard_price" in vals and not self.env.context.get("disable_auto_svl"):
            self.filtered(lambda p: p.cost_method != "fifo")._change_standard_price(
                vals["standard_price"]
            )
    ...
    def _run_fifo_vacuum(self, company=None):
        ...
        if product.product_tmpl_id.cost_method == "average" and not float_is_zero(
    ...
    def _create_fifo_vacuum_anglo_saxon_expense_entry(self, vacuum_svl, svl_to_vacuum):
        ...
        """
        if (
            not vacuum_svl.company_id.anglo_saxon_accounting
            or not svl_to_vacuum.stock_move_id._is_out()
        ):
            return False
        ...
        reconciled_line_ids = list(
            set(account_move_lines._reconciled_lines()) - set(account_move_lines.ids)
        )
        account_move = AccountMove.search(
            [("line_ids", "in", reconciled_line_ids)], limit=1
        )
        # If delivered quantity is not invoiced then no need to create this entry
        if not account_move:
            return False
        accounts = svl_to_vacuum.product_id.product_tmpl_id.get_product_accounts(
        ...
        to_reconcile_account_move_lines = vacuum_svl.account_move_id.line_ids.filtered(
            lambda l: not l.reconciled
            and l.account_id == accounts["stock_output"]
            and l.account_id.reconcile
        )
        to_reconcile_account_move_lines += new_account_move.line_ids.filtered(
            lambda l: not l.reconciled
            and l.account_id == accounts["stock_output"]
            and l.account_id.reconcile
        )
    ...
    def _svl_empty_stock(
        ...
        for product in impacted_products:
            ...
            if (
                ...
                svsl_vals = product._prepare_in_svl_vals(
                    abs(product.quantity_svl), product.value_svl / product.quantity_svl
                )
            svsl_vals["description"] = description + svsl_vals.pop(
                "rounding_adjustment", ""
            )
        ...
        for product in self:
            ...
            if quantity_svl:
                if (
                    ...
                    svl_vals = product._prepare_in_svl_vals(
    ...
    def _svl_empty_stock_am(self, stock_valuation_layers):
        ...
        for out_stock_valuation_layer in stock_valuation_layers:
            ...
            if not stock_input_account:
                raise UserError(
                    _(
                        "You don't have any stock input account defined on your product category. You must define one before processing this operation."
                    )
                )
            if not product_accounts[product.id].get("stock_valuation"):
                raise UserError(
    ...
    def _svl_replenish_stock_am(self, stock_valuation_layers):
        ...
        for out_stock_valuation_layer in stock_valuation_layers:
            ...
            if not product_accounts[product.id].get("stock_input"):
                raise UserError(
                    _(
                        "You don't have any input valuation account defined on your product category. You must define one before processing this operation."
                    )
                )
            if not product_accounts[product.id].get("stock_valuation"):
                raise UserError(
    ...
    def _compute_average_price(
        ...
        if "value_invoiced" in self.env.context:
            qty_valued, valuation = candidates._consume_all(
                qty_invoiced, value_invoiced, qty_to_invoice
            )
        else:
            qty_valued, valuation = candidates._consume_specific_qty(
        ...
        for sml in stock_moves.move_line_ids:
            if not sml.owner_id or sml.owner_id == sml.company_id.partner_id:
                continue
            missing -= sml.product_uom_id._compute_quantity(
                sml.qty_done, self.uom_id, rounding_method="HALF-UP"
            )
...
class ProductCategory(models.Model):
    ...
    property_valuation = fields.Selection(
        [("manual_periodic", "Manual"), ("real_time", "Automated")],
        ...
        for category in self:
            ...
            if valuation_account and valuation_account in input_and_output_accounts:
                raise ValidationError(
                    _(
                        "The Stock Input and/or Output accounts cannot be the same as the Stock Valuation account."
                    )
                )
    ...
    def onchange_property_cost(self):
        ...
        return {
            "warning": {
                "title": _("Warning"),
                "message": _(
                    "Changing your cost method is an important change that will impact your inventory valuation. Are you sure you want to make that change?"
                ),
            }
        }
    ...
    def write(self, vals):
        ...
        if "property_cost_method" in vals or "property_valuation" in vals:
            ...
            for product_category in self:
                property_stock_fields = [
                    "property_stock_account_input_categ_id",
                    "property_stock_account_output_categ_id",
                    "property_stock_valuation_account_id",
                ]
                if (
                    "property_valuation" in vals
                    and vals["property_valuation"] == "manual_periodic"
                    and product_category.property_valuation != "manual_periodic"
                ):
                    for stock_property in property_stock_fields:
                        vals[stock_property] = False
                elif (
                    "property_valuation" in vals
                    and vals["property_valuation"] == "real_time"
                    and product_category.property_valuation != "real_time"
                ):
                    company_id = self.env.company
                    for stock_property in property_stock_fields:
                        vals[stock_property] = (
                            vals.get(stock_property, False)
                            or company_id[stock_property]
                        )
                ...
                if (
                    new_cost_method
                    and new_cost_method != product_category.property_cost_method
                ):
                    valuation_impacted = True
                if (
                    new_valuation
                    and new_valuation != product_category.property_valuation
                ):
                    valuation_impacted = True
                if valuation_impacted is False:
                    continue
                ...
                if new_cost_method:
                    description = _(
                        ...
                        product_category.property_cost_method,
                        new_cost_method,
                    )
                else:
                    description = _(
                ...
                impacted_categories[product_category] = (
                    products,
                    description,
                    products_orig_quantity_svl,
                )
    ...
    def create(self, vals_list):
        for vals in vals_list:
            if (
                "property_valuation" not in vals
                or vals["property_valuation"] == "manual_periodic"
            ):
            ...
            if (
                ...
                company_id = self.env.company
                vals["property_stock_account_input_categ_id"] = (
                    vals.get("property_stock_account_input_categ_id", False)
                    or company_id.property_stock_account_input_categ_id
                )
                vals["property_stock_account_output_categ_id"] = (
                    vals.get("property_stock_account_output_categ_id", False)
                    or company_id.property_stock_account_output_categ_id
                )
                vals["property_stock_valuation_account_id"] = (
                    vals.get("property_stock_valuation_account_id", False)
                    or company_id.property_stock_valuation_account_id
                )
    ...
    def onchange_property_valuation(self):
        ...
        if self.property_valuation == "real_time":
            ...
            self.property_stock_account_input_categ_id = (
                company_id.property_stock_account_input_categ_id
            )
            ...
            self.property_stock_valuation_account_id = (
                company_id.property_stock_valuation_account_id
            )
