...
class TestLeadAssign(TestLeadAssignCommon):
    ...
    def test_assign_count(self):
        ...
        self.members.invalidate_model(["lead_month_count"])
        self.assertEqual(
            self.sales_team_1_m1.lead_month_count, 0
        )  # archived do not get leads
        self.assertEqual(
            self.sales_team_1_m2.lead_month_count, 0
        )  # opt-out through assignment_max = 0
        self.assertEqual(
            self.sales_team_1_m3.lead_month_count, 14
        )  # 15 max on 4 days (2) + existing 12
        ...
        self.assertEqual(
            self.sales_team_1_m3.lead_month_count, 16
        )  # 15 max on 4 days (2) + existing 14 and not capped anymore
    ...
    def test_assign_duplicates(self):
        ...
        self.assertEqual(
            len(leads_st1) + len(leads_stc), len(leads)
        )  # Make sure all lead are assigned

        # salespersons assign
        self.members.invalidate_model(["lead_month_count"])
        self.assertMemberAssign(
            self.sales_team_1_m1, 11
        )  # 45 max on 2 days (3) + compensation (8.4)
        self.assertMemberAssign(
            self.sales_team_1_m2, 4
        )  # 15 max on 2 days (1) + compensation (2.8)
        self.assertMemberAssign(
            self.sales_team_1_m3, 4
        )  # 15 max on 2 days (1) + compensation (2.8)
        self.assertMemberAssign(
            self.sales_team_convert_m1, 8
        )  # 30 max on 15 (2) + compensation (5.6)
        self.assertMemberAssign(
            self.sales_team_convert_m2, 15
        )  # 60 max on 15 (4) + compsantion (11.2)
    ...
    def test_assign_no_duplicates(self):
        ...
        self.assertMemberAssign(
            self.sales_team_convert_m2, 15
        )  # 60 max on 15 (4) + compensation (11.2)
    ...
    def test_assign_populated(self):
        ...
        self.assertLessEqual(
            len(leads_st1), 225
        )  # 75 * 600 / 300 * 1.5 (because random)
        self.assertLessEqual(
            len(leads_st2), 270
        )  # 90 * 600 / 300 * 1.5 (because random)
        self.assertLessEqual(
            len(leads_st3), 405
        )  # 135 * 600 / 300 * 1.5 (because random)
        self.assertGreaterEqual(
            len(leads_st1), 75
        )  # 75 * 600 / 300 * 0.5 (because random)
        self.assertGreaterEqual(
            len(leads_st2), 90
        )  # 90 * 600 / 300 * 0.5 (because random)
        self.assertGreaterEqual(
            len(leads_st3), 135
        )  # 135 * 600 / 300 * 0.5 (because random)
    ...
    def test_assign_specific_won_lost(self):
        ...
        leads[
            0
        ].stage_id = self.stage_gen_won.id  # is won -> should not be taken into account
        ...
        )  # assigned lead should not be re-assigned
