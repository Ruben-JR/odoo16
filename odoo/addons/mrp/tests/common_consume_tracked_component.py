...
class TestConsumeTrackedComponentCommon(common.TransactionCase):
    ...
    def executeConsumptionTriggers(self, mrp_productions):
        ...
        if isAvailable:
            ...
            error = False
            has_zero_tracked_component = not mrp_productions[
                i
            ].picking_type_id.use_auto_consume_components_lots and any(
                m.state not in ["done", "cancel"]
                and m.has_tracking != "none"
                and float_is_zero(m.quantity_done, m.product_uom.rounding)
                for m in mrp_productions[i].move_raw_ids
            )
            try:
            ...
            if has_zero_tracked_component:
                self.assertTrue(
                    error, "Immediate Production Wizard shall raise an error."
                )
            else:
                self.assertFalse(
                    error, "Immediate Production Wizard shall not raise an error."
                )
