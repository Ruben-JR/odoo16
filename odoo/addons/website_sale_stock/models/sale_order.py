...
class SaleOrder(models.Model):
    ...
    def _get_warehouse_available(self):
        ...
        if not warehouse and self.user_id and self.company_id:
            warehouse = self.user_id.with_company(
                self.company_id.id
        ...
        if product.type == "product" and not product.allow_out_of_stock_order:
            ...
            if available_qty < total_cart_qty:
                ...
                if allowed_line_qty > 0:
                    if order_line:
                        order_line._set_shop_warning_stock(
                            total_cart_qty, available_qty
                        )
                    else:
    ...
    def _get_cart_and_free_qty(self, line=None, product=None, **kwargs):
        ...
        if not line and not product:
            return 0, 0
        cart_qty = sum(
            self._get_common_product_lines(line, product, **kwargs).mapped(
                "product_uom_qty"
            )
        )
        free_qty = (
            (product or line.product_id)
            ...
            for line in sale_order.order_line:
                ...
                if not combination_info:
                    combination_info = product.with_context(
                        **self._get_context_for_line(line)
                    )._get_combination_info_variant(add_qty=line.product_uom_qty)
