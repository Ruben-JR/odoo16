...
class HrEmployee(models.Model):
    ...
    newly_hired_employee = fields.Boolean(
    ...
    def _compute_newly_hired_employee(self):
        ...
        for employee in self:
            employee.newly_hired_employee = bool(
                employee.create_date > (now - timedelta(days=90))
            )
    ...
    def default_get(self, fields):
        ...
        if "work_contact_id" in fields:
            current_applicant = self.env["hr.applicant"].browse(
                self.env.context.get("default_applicant_id")
            )
