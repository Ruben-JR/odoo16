...
class SaleOrder(models.Model):
    ...
    def _compute_team_id(self):
        ...
        for order in self:
            default_team_id = (
                ...
                or order.team_id.id
                or order.partner_id.team_id.id
            )
        ...
        for order in self:
            ...
            if show_warning:
                ...
                order.partner_credit_warning = self.env[
                    "account.move"
                ]._build_credit_warning_message(order, updated_credit)
    ...
    def _action_confirm(self):
        ...
        for order in self:
            if any(
                ...
                for expense_policy in order.order_line.product_id.mapped(
                    "expense_policy"
                )
            ):
    ...
    def action_update_taxes(self):
        ...
        lines_to_recompute = self.order_line.filtered(
    ...
    def _notify_by_email_prepare_rendering_context(
        ...
        if self.validity_date and self.state in ["draft", "sent"]:
            formatted_date = format_date(
                self.env, self.validity_date, lang_code=lang_code
            )
        ...
        return {
            ...
            - sum(
                self.invoice_ids.filtered(
                    ...
                    and x.invoice_line_ids.sale_line_ids.order_id == self
                ).mapped("amount_total")
            ),
            "currency_id": self.currency_id.id,
            "partner_id": self.partner_invoice_id.id,
