...
class PurchaseRequisitionType(models.Model):
    ...
    exclusive = fields.Selection(
        [
            ...
            ("multiple", "Select multiple RFQ (non-exclusive)"),
        ],
...
class PurchaseRequisition(models.Model):
    ...
    def _onchange_vendor(self):
        ...
        if not self.vendor_id:
            ...
            self.currency_id = (
                self.vendor_id.property_purchase_currency_id.id
    ...
    def action_done(self):
        ...
        if any(
            ...
            raise UserError(
                _(
                    "You have to cancel or validate every RfQ before closing the purchase requisition."
                )
            )
        for requisition in self:
            for requisition_line in requisition.line_ids:
...
class PurchaseRequisitionLine(models.Model):
    ...
    def create(self, vals_list):
        ...
        for line, vals in zip(lines, vals_list):
            if (
                line.requisition_id.state not in ["draft", "cancel", "done"]
                and line.requisition_id.is_quantity_copy == "none"
            ):
    ...
    def unlink(self):
        to_unlink = self.filtered(
            lambda r: r.requisition_id.state not in ["draft", "cancel", "done"]
        )
    ...
    def create_supplier_info(self):
        ...
        if (
            purchase_requisition.type_id.quantity_copy == "none"
