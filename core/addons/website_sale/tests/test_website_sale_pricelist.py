...
class TestWebsitePriceList(TransactionCase):
    def setUp(self):
        ...
        self.env.user.partner_id.country_id = (
        ...
        (
            self.env["product.pricelist"].search([]) - self.env.ref("product.list0")
        ).write({"website_id": False, "active": False})
    ...
    def test_get_pricelist_available_show_with_auto_property(self):
        show = True
        self.env.user.partner_id.country_id = self.env.ref(
            "base.be"
        )  # Add EUR pricelist auto
        current_pl = False
...
class TestWebsitePriceListAvailableGeoIP(TestWebsitePriceListAvailable):
    def setUp(self):
        ...
        c_BE = self.env["res.country.group"].create(
            {"name": "Belgium", "country_ids": [(6, 0, [self.BE.id])]}
        )
        c_NL = self.env["res.country.group"].create(
        ...
        (
            ...
            + self.generic_pl_code
            + self.w1_pl_select
        ).write({"country_group_ids": [(6, 0, [c_BE.id])]})
        (self.generic_pl_code_select + self.w1_pl + self.w2_pl).write(
            {"country_group_ids": [(6, 0, [c_BENELUX.id])]}
        )
    ...
    def test_get_pricelist_available_geoip4(self):
        ...
        with patch(
            ...
            "odoo.addons.website_sale.models.website.Website._get_cached_pricelist_id",
...
class TestWebsitePriceListMultiCompany(TransactionCaseWithUserDemo):
    def setUp(self):
        ...
        self.assertEqual(
            (irp1, irp2),
            (self.c1_pl, self.c2_pl),
            "Ensure there is an `ir.property` for demo partner for every company, and that the pricelist is the company specific one.",
        )
        # ---------------------------------- IR.PROPERTY -------------------------------------
