...
class SurveyCase(common.TransactionCase):
    ...
    def _access_begin(self, survey, token):
        url = survey.get_base_url() + "/survey/begin/%s/%s" % (
            survey.access_token,
            token,
        )
    ...
    def _access_submit(self, survey, token, post_data):
        url = survey.get_base_url() + "/survey/submit/%s/%s" % (
            survey.access_token,
            token,
        )
    ...
    def _find_csrf_token(self, text):
        csrf_token_re = re.compile(
            '(input.+csrf_token.+value=")([a-f0-9]{40}o[0-9]*)', re.MULTILINE
        )
    ...
    def _answer_question(
        self, question, answer, answer_token, csrf_token, button_submit="next"
    ):
