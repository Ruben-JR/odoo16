...
class DockerDeb(Docker):
    ...
    def start_test(self):
        ...
        cmds = [
            ...
            "/usr/bin/dpkg -i /data/src/odoo_%s.%s_all.deb ; /usr/bin/apt-get install -f -y"
            % (VERSION, TSTAMP),
            'su odoo -s /bin/bash -c "odoo -d mycompany -i base --stop-after-init"',
            'su odoo -s /bin/bash -c "odoo -d mycompany --pidfile=/data/src/odoo.pid"',
        ]
...
class DockerRpm(Docker):
    ...
    def build(self):
        ...
        os.rename(
            glob("%s/dist/odoo-*.noarch.rpm" % self.args.build_dir)[0],
...
class KVM(object):
    ...
    def start(self):
        kvm_cmd = [
            "kvm",
            "-cpu",
            "Skylake-Client,hypervisor=on,hle=off,rtm=off",
            "-smp",
            "2,sockets=2,cores=1,threads=1",
            "-net",
            "nic,model=e1000e,macaddr=52:54:00:d3:38:5e",
            "-net",
            "user,hostfwd=tcp:127.0.0.1:10022-:22,hostfwd=tcp:127.0.0.1:18069-:8069,hostfwd=tcp:127.0.0.1:15432-:5432",
            "-m",
            "2048",
            "-drive",
            f"if=virtio,file={self.image},snapshot=on",
            "-nographic",
            "-serial",
            "none",
        ]
        ...
        try:
            self.wait_ssh(
                30
            )  # give some time to the VM to start, otherwise the SSH server may not be ready
            signal.alarm(2400)
    ...
    def rsync(
        ...
        cmd = [
            ...
            "-a",
            "-e",
            "ssh -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no -p 10022 -i %s"
            % self.ssh_key,
        ]
        cmd.extend(options)
...
class KVMWinBuildExe(KVM):
    def run(self):
        ...
        with open(
            os.path.join(self.args.build_dir, "setup/win32/Makefile.version"),
            "w",
            encoding="utf-8",
        ) as f:
        ...
        with open(
            os.path.join(self.args.build_dir, "setup/win32/Makefile.python"),
            "w",
            encoding="utf-8",
        ) as f:
            f.write("PYTHON_VERSION=%s\n" % self.args.vm_winxp_python_version)
        with open(
            os.path.join(self.args.build_dir, "setup/win32/Makefile.servicename"),
            "w",
            encoding="utf-8",
        ) as f:
        ...
        self.rsync(
            [
                "%s/" % self.args.build_dir,
                "%s@127.0.0.1:%s" % (self.login, remote_build_dir),
            ]
        )
        self.ssh("cd {}setup/win32;time make allinone;".format(remote_build_dir))
        self.rsync(
            [
                "%s@127.0.0.1:%ssetup/win32/release/" % (self.login, remote_build_dir),
                "%s/" % self.args.build_dir,
            ]
        )
...
class KVMWinTestExe(KVM):
    def run(self):
        ...
        self.ssh(
            'PGPASSWORD=openpgpwd /cygdrive/c/"Program Files"/"Odoo %s"/PostgreSQL/bin/createdb.exe -e -U openpg mycompany'
            % setupversion
        )
        self.ssh("netsh advfirewall set publicprofile state off")
        self.ssh(
            '/cygdrive/c/"Program Files"/"Odoo {sv}"/python/python.exe \'c:\\Program Files\\Odoo {sv}\\server\\odoo-bin\' -d mycompany -i base --stop-after-init'.format(
                sv=setupversion
            )
        )
...
def parse_args():
    ...
    ap.add_argument(
        "--vm-winxp-image",
        default="/home/odoo/vm/win1036/win10_winpy36.qcow2",
        help="%(default)s",
    )
    ap.add_argument(
        "--vm-winxp-ssh-key", default="/home/odoo/vm/win1036/id_rsa", help="%(default)s"
    )
    ap.add_argument(
        "--vm-winxp-login", default="Naresh", help="Windows login %(default)s"
    )
    ap.add_argument(
        "--vm-winxp-python-version",
        default="3.7.7",
        help="Windows Python version installed in the VM (default: %(default)s)",
    )
