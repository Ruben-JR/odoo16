...
class MailComposer(models.TransientModel):
    ...
    def default_get(self, fields):
        ...
        if (
            ...
            and "default_email_layout_xmlid" not in self._context
        ):
            self = self.with_context(
                default_email_layout_xmlid=self._context["custom_layout"]
            )
        ...
        if missing_author or missing_email_from:
            author_id, email_from = self.env["mail.thread"]._message_compute_author(
                result.get("author_id"), result.get("email_from"), raise_on_email=False
            )
        ...
        if (
            "reply_to_mode" in fields
            and "reply_to_mode" not in result
            and result.get("model")
        ):
            # doesn't support threading
            if result["model"] not in self.env or not hasattr(
                self.env[result["model"]], "message_post"
            ):
                result["reply_to_mode"] = "new"

        if (
            "active_domain" in self._context
        ):  # not context.get() because we want to keep global [] domains
            result["active_domain"] = "%s" % self._context.get("active_domain")
        if result.get("composition_mode") == "comment" and (
            set(fields)
            & set(["model", "res_id", "partner_ids", "record_name", "subject"])
        ):
    ...
    body = fields.Html(
        "Contents", render_engine="qweb", compute=False, default="", sanitize_style=True
    )
    parent_id = fields.Many2one("mail.message", "Parent Message", ondelete="set null")
    template_id = fields.Many2one(
        "mail.template", "Use template", domain="[('model', '=', model)]"
    )
    ...
    notify = fields.Boolean(
        "Notify followers", help="Notify followers of the document (mass post only)"
    )
                ...
                for res_id, mail_values in all_mail_values.items():
                    if wizard.composition_mode == "mass_mail":
                        ...
                        post_params = dict(
                            ...
                            and wizard.email_add_signature,
                            mail_auto_delete=wizard.template_id.auto_delete
                            if wizard.template_id
                            else self._context.get("mail_auto_delete", True),
                        ...
                        if ActiveModel._name == "mail.thread":
                            ...
                            result_messages += ActiveModel.browse(res_id).message_post(
                                **post_params
                            )
        ...
        for record in self:
            ...
            if record.attachment_ids:
                attachments = (
                    ...
                    .sudo()
                    .browse(record.attachment_ids.ids)
                    .filtered(
                ...
                if attachments:
                    attachments.write(
                        {"res_model": template._name, "res_id": template.id}
                    )
    ...
    def get_mail_values(self, res_ids):
        ...
        if mass_mail_mode and not self.reply_to_force_new:
            ...
            for res_id, reply_to in reply_to_value.items():
                if not reply_to:
                    reply_to_value[res_id] = rendered_values.get(res_id, {}).get(
                        "email_from", False
                    )
        ...
        for res_id in res_ids:
            ...
            if mass_mail_mode and self.model:
                ...
                mail_values["recipient_ids"] = [
                    Command.link(id) for id in mail_values.pop("partner_ids", [])
                ]
                ...
                mail_values["attachment_ids"] = (
                    ...
                    .with_context(attached_to=record)
    ...
    def _process_state(self, mail_values_dict):
        ...
        for record_id, mail_values in mail_values_dict.items():
            ...
            mail_to_normalized = (
                recipients["mail_to_normalized"][0]
                if recipients["mail_to_normalized"]
                else ""
            )
    ...
    def _get_blacklist_record_ids(self, mail_values_dict):
        ...
        if self.composition_mode == "mass_mail" and issubclass(
            ...
            if blacklist:
                targets = (
                    self.env[self.model]
                    .browse(mail_values_dict.keys())
                    .read(["email_normalized"])
                )
    ...
    def render_message(self, res_ids):
        ...
        bodies = self._render_field(
            ...
            res_ids,
            post_process=True,
            options={"preserve_comments": self.composition_mode == "mass_mail"},
        )
