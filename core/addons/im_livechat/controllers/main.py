...
class LivechatController(http.Controller):
    ...
    @http.route(
        "/im_livechat/external_lib.<any(css,js):ext>", type="http", auth="public"
    )
    ...
    def livechat_init(self, channel_id):
        ...
        matching_rule = (
            request.env["im_livechat.channel.rule"]
            .sudo()
            .match_rule(channel_id, url, country_id)
        )
        if matching_rule and (
            not matching_rule.chatbot_script_id
            or matching_rule.chatbot_script_id.script_step_ids
        ):
            rule = {
            ...
            if (
                ...
                and (
                    not matching_rule.chatbot_only_if_no_operator
                    or (
                        not operator_available
                        and matching_rule.chatbot_only_if_no_operator
                    )
                )
                and matching_rule.chatbot_script_id.script_step_ids
            ):
        ...
        return {
            ...
            or operator_available
            and (not rule or rule["action"] != "hide_button"),
            "rule": rule,
        }

    @http.route(
        "/im_livechat/operator/<int:operator_id>/avatar",
        type="http",
        auth="public",
        cors="*",
    )
    def livechat_operator_get_avatar(self, operator_id):
        ...
        if operator.exists():
            is_livechat_member = bool(
                request.env["im_livechat.channel"]
                .sudo()
                .search_count([("user_ids", "in", operator.user_ids.ids)])
            )
        ...
        if not is_livechat_member:
            ...
            is_livechat_member = bool(
                ...
                .sudo()
                .search_count([("operator_partner_id", "in", operator.ids)])
            )
    ...
    def get_session(
        ...
        **kwargs
        ...
        if chatbot_script_id:
            chatbot_script = (
                request.env["chatbot.script"].sudo().browse(chatbot_script_id)
            )
    ...
    def feedback(self, uuid, rate, reason=None, **kwargs):
        channel = (
            request.env["mail.channel"].sudo().search([("uuid", "=", uuid)], limit=1)
        )
        if channel:
    ...
    def history_pages(self, pid, channel_uuid, page_history=None):
        ...
        channel = (
            ...
            .search(
                [
                    ("uuid", "=", channel_uuid),
                    ("channel_partner_ids", "in", partner_ids),
                ]
            )
        )
        if channel:
    ...
    def notify_typing(self, uuid, is_typing):
        ...
        if not channel:
            raise NotFound()
        channel_member = channel.env["mail.channel.member"].search(
            [
    ...
    @http.route(
        "/im_livechat/email_livechat_transcript", type="json", auth="public", cors="*"
    )
    def email_livechat_transcript(self, uuid, email):
        channel = (
            request.env["mail.channel"]
            .sudo()
            .search([("channel_type", "=", "livechat"), ("uuid", "=", uuid)], limit=1)
        )
        if channel:
