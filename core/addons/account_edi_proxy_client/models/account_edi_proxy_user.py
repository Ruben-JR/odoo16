...
class AccountEdiProxyClientUser(models.Model):
    ...
    _sql_constraints = [
        ...
        (
            "unique_edi_identification_per_format",
            "unique(edi_identification, edi_format_id)",
            "This edi identification is already assigned to a user",
        ),
    ]

    def _compute_private_key_filename(self):
        for record in self:
            record.private_key_filename = (
                f"{record.id_client}_{record.edi_identification}.key"
            )
    ...
    def _get_demo_state(self):
        demo_state = (
            ...
            .sudo()
            .get_param("account_edi_proxy_client.demo", False)
        )
        return (
            "prod"
    ...
    def _get_server_url(self):
        return (
            ...
            if self._get_demo_state() == "test"
            else self.env["ir.config_parameter"]
            .sudo()
            .get_param("account_edi_proxy_client.edi_server_url", DEFAULT_SERVER_URL)
        )
    ...
    def _make_request(self, url, params=False):
        ...
        if "error" in response:
            message = _(
                "The url that this service requested returned an error. The url it tried to contact was %s. %s",
                url,
        ...
        if self._get_demo_state() == "demo":
            ...
            try:
                ...
                response = self._make_request(
                    self._get_server_url() + "/iap/account_edi/1/create_user",
                    params={
    ...
    def _renew_token(self):
        ...
        try:
            with self.env.cr.savepoint(flush=False):
                self.env.cr.execute(
                    "SELECT * FROM account_edi_proxy_client_user WHERE id IN %s FOR UPDATE NOWAIT",
                    [tuple(self.ids)],
                )
