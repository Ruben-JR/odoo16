...
class Lead2OpportunityPartner(models.TransientModel):
        ...
        for convert in self:
            ...
            convert.duplicated_lead_ids = (
                ...
                ._get_lead_duplicates(
                    ...
                    if convert.lead_id.partner_id.email
                    else convert.lead_id.email_from,
                    include_lost=True,
                )
                .ids
            )
    ...
    def _compute_user_id(self):
        for convert in self:
            convert.user_id = (
                convert.lead_id.user_id if convert.lead_id.user_id else False
            )
    ...
    def _convert_handle_partner(self, lead, action, partner_id):
        ...
        lead.with_context(default_user_id=self.user_id.id)._handle_partner_assignment(
            force_partner_id=partner_id, create_missing=(action == "create")
        )
