...
class PurchaseOrder(models.Model):
    ...
    incoterm_location = fields.Char(
        string="Incoterm Location", states={"done": [("readonly", True)]}
    )
    incoming_picking_count = fields.Integer(
        "Incoming Shipment count", compute="_compute_incoming_picking_count"
    )
    ...
    dest_address_id = fields.Many2one(
        "res.partner", compute="_compute_dest_address_id", store=True, readonly=False
    )
...
class PurchaseOrderLine(models.Model):
    ...
    move_dest_ids = fields.One2many(
        "stock.move", "created_purchase_line_id", "Downstream Moves"
    )
    ...
    def _compute_qty_received(self):
        from_stock_lines = self.filtered(
            lambda order_line: order_line.qty_received_method == "stock_moves"
        )
        ...
        for line in self:
            ...
            if line.product_id:
                virtual_available = line.product_id.with_context(
                    warehouse=warehouse.id, to_date=line.date_planned
                ).virtual_available
                if line.state == "draft":
    ...
    def _check_orderpoint_picking_type(self):
        ...
        if (
            ...
            raise UserError(
                _(
                    "For the product %s, the warehouse of the operation type (%s) is inconsistent with the location (%s) of the reordering rule (%s). Change the operation type or cancel the request for quotation.",
                    self.product_id.display_name,
                    self.order_id.picking_type_id.display_name,
                    self.orderpoint_id.location_id.display_name,
                    self.orderpoint_id.display_name,
                )
            )
    ...
    def _prepare_stock_move_vals(
        ...
        return {
            ...
            "location_dest_id": (
                self.orderpoint_id and not (self.move_ids | self.move_dest_ids)
            )
