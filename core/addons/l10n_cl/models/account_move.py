    ...
    def _l10n_cl_get_invoice_totals_for_report(self):
        ...
        if include_sii:
            for vals in base_line_vals_list:
                vals["taxes"] = (
                    ...
                    .filtered(lambda tax: tax.l10n_cl_sii_code != 14)
                )
        ...
        if include_sii:
            tax_line_vals_list = [
                ...
                if x["tax_repartition_line"].tax_id.l10n_cl_sii_code != 14
            ]
        ...
        if include_sii:
            ...
            tax_totals["amount_untaxed"] = self.currency_id.round(
                ...
                - sum(
                    [x["tax_amount"] for x in tax_line_vals_list if "tax_amount" in x]
                )
            )
            ...
            if tax_totals["subtotals"]:
                tax_totals["subtotals"][0]["formatted_amount"] = tax_totals[
