...
class BaseModel(models.AbstractModel):
            ...
            if model and res_ids:
                ...
                for alias in mail_aliases:
                    result_email.setdefault(
                        ...
                        "%s@%s" % (alias.alias_name, alias_domain),
                    )
    ...
    def _notify_get_reply_to_formatted_email(self, record_email, record_name):
        ...
        if len(formatted_email) > 78:
            formatted_email = tools.formataddr(
                (record_name or company_name, record_email)
            )
        if len(formatted_email) > 78:
