...
class TestSnippets(HttpCase):
    ...
    def test_03_snippets_all_drag_and_drop(self):
        ...
        data_snippet_els = html_template.xpath(
            "//*[@class='o_panel' and not(contains(@class, 'd-none'))]//*[@data-snippet]"
        )
        blacklist = [
        ...
        path = url_encode({"path": "/?" + snippets_names_encoded})
        self.start_tour(
            "/web#action=website.website_preview&%s" % path,
