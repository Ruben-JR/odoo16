...
class StockMoveLine(models.Model):
    ...
    move_id = fields.Many2one(
        "stock.move", "Stock Operation", check_company=True, index=True
    )
    company_id = fields.Many2one(
        "res.company", string="Company", readonly=True, required=True, index=True
    )
    ...
    qty_done = fields.Float(
        "Done", default=0.0, digits="Product Unit of Measure", copy=False
    )
    ...
    package_level_id = fields.Many2one(
        "stock.package_level", "Package Level", check_company=True
    )
    ...
    picking_type_entire_packs = fields.Boolean(
        related="picking_id.picking_type_id.show_entire_packs", readonly=True
    )
    ...
    reference = fields.Char(
        related="move_id.reference", store=True, related_sudo=False, readonly=False
    )
    ...
    def _compute_reserved_qty(self):
        for line in self:
            line.reserved_qty = line.product_uom_id._compute_quantity(
                line.reserved_uom_qty, line.product_id.uom_id, rounding_method="HALF-UP"
            )
    ...
    def _set_reserved_qty(self):
        ...
        raise UserError(
            _(
                "The requested operation cannot be processed because of a programming error setting the `reserved_qty` field instead of the `reserved_uom_qty`."
            )
        )
    ...
    def _onchange_product_id(self):
        if self.product_id:
            if self.picking_id:
                ...
                self.description_picking = product._get_description(
                    ...
                    if counter.get(self.lot_name) and counter[self.lot_name] > 1:
                        ...
                        if quants:
                            message = _(
                                ...
                                self.lot_name,
                                ", ".join(quants.location_id.mapped("display_name")),
                            )
                elif self.lot_id:
    ...
    def _onchange_qty_done(self):
        ...
        if self.qty_done and self.product_id.tracking == "serial":
            qty_done = self.product_uom_id._compute_quantity(
                self.qty_done, self.product_id.uom_id
            )
            if (
                float_compare(
                    qty_done, 1.0, precision_rounding=self.product_id.uom_id.rounding
                )
                != 0
            ):
                message = _(
    ...
    def _onchange_putaway_location(self):
        if (
            not self.id
            and self.user_has_groups("stock.group_stock_multi_locations")
            and self.product_id
            and self.qty_done
        ):
            qty_done = self.product_uom_id._compute_quantity(
        ...
        for package, smls in groupby(self, lambda sml: sml.result_package_id):
            ...
            if package.package_type_id:
                best_loc = smls.move_id.location_dest_id.with_context(
                    exclude_sml_ids=excluded_smls.ids, products=smls.product_id
                )._get_putaway_strategy(self.env["product.product"], package=package)
                smls.location_dest_id = (
                    smls.package_level_id.location_dest_id
                ) = best_loc
            elif package:
                used_locations = set()
                for sml in smls:
                    ...
                    sml.location_dest_id = sml.move_id.location_dest_id.with_context(
                        exclude_sml_ids=excluded_smls.ids
                    )._get_putaway_strategy(
                        sml.product_id, quantity=sml.reserved_uom_qty
                    )
                    excluded_smls -= sml
                ...
                for sml in smls:
                    ...
                    putaway_loc_id = sml.move_id.location_dest_id.with_context(
                        ...
                        sml.product_id,
                        quantity=qty,
                        packaging=sml.move_id.product_packaging_id,
                    )
    ...
    def create(self, vals_list):
        ...
        for move_line in mls:
            ...
            if move_line.picking_id.state != "done":
                moves = move_line.picking_id.move_ids.filtered(
                    lambda x: x.product_id == move_line.product_id
                )
                moves = sorted(
                    moves, key=lambda m: m.quantity_done < m.product_qty, reverse=True
                )
                if moves:
        ...
        for ml, vals in zip(mls, vals_list):
            if (
                ml.move_id
                and ml.move_id.picking_id
                and ml.move_id.picking_id.immediate_transfer
                and ml.move_id.state != "done"
                and "qty_done" in vals
            ):
                ml.move_id.with_context(
                    avoid_putaway_rules=True
                ).product_uom_qty = ml.move_id.quantity_done
            if ml.state == "done":
                if "qty_done" in vals:
    ...
    def write(self, vals):
            ...
            for ml in self.filtered(
                ...
                if not ml.move_id._should_bypass_reservation(
                    ...
                    if reserved_qty != new_reserved_uom_qty:
                        new_reserved_uom_qty = ml.product_id.uom_id._compute_quantity(
                            reserved_qty, ml.product_uom_id, rounding_method="HALF-UP"
                        )
                        moves_to_recompute_state |= ml.move_id
                        ml.with_context(
                            bypass_reservation_update=True
                        ).reserved_uom_qty = new_reserved_uom_qty
        ...
        if updates or "qty_done" in vals:
            ...
            if (
                not updates
            ):  # we can skip those where qty_done is already good up to UoM rounding
                mls = mls.filtered(
            ...
            for ml in mls:
                ...
                qty_done_orig = ml.product_uom_id._compute_quantity(
                ...
                quantity = product_uom_id._compute_quantity(
        ...
        if updates or "qty_done" in vals:
            moves = self.filtered(lambda ml: ml.move_id.state == "done").mapped(
                "move_id"
            )
            moves |= self.filtered(
                lambda ml: ml.move_id.state not in ("done", "cancel")
                and ml.move_id.picking_id.immediate_transfer
            ).mapped("move_id")
            for move in moves:
    ...
    def unlink(self):
        ...
        for ml in self:
            ...
            if (
                not float_is_zero(ml.reserved_qty, precision_digits=precision)
                and ml.move_id
    ...
    def _action_done(self):
        ...
        for ml in self:
            ...
            if float_compare(uom_qty, qty_done, precision_digits=precision_digits) != 0:
                raise UserError(
                    _(
                        'The quantity done for the product "%s" doesn\'t respect the rounding precision '
        ...
        if ml_ids_tracked_without_lot:
            ...
            raise UserError(
                ...
                + "\n - ".join(
                    mls_tracked_without_lot.mapped("product_id.display_name")
                )
            )
        ml_to_create_lot = self.env["stock.move.line"].browse(ml_ids_to_create_lot)
        ml_to_create_lot.with_context(
            bypass_reservation_update=True
        ...
        for ml in mls_todo:
            if ml.product_id.type == "product":
                ...
                if (
                    not ml.move_id._should_bypass_reservation(ml.location_id)
                    and float_compare(
                        ml.qty_done, ml.reserved_uom_qty, precision_rounding=rounding
                    )
                    > 0
                ):
    ...
    def _log_message(self, record, move, template, vals):
        ...
        if "package_id" in vals and vals["package_id"] != move.package_id.id:
            data["package_name"] = (
                self.env["stock.quant.package"].browse(vals.get("package_id")).name
            )
        if (
            "package_result_id" in vals
            and vals["package_result_id"] != move.package_result_id.id
        ):
            data["result_package_name"] = (
                self.env["stock.quant.package"]
                .browse(vals.get("result_package_id"))
                .name
            )
    ...
    def _free_reservation(
        ...
        if quantity > available_quantity:
            ...
            for candidate in outdated_candidates:
                if (
                    float_compare(
                        candidate.reserved_qty, quantity, precision_rounding=rounding
                    )
                    <= 0
                ):
    ...
    def action_revert_inventory(self):
        ...
        for move_line in self:
            if move_line.is_inventory and not float_is_zero(
                move_line.qty_done, precision_digits=move_line.product_uom_id.rounding
            ):
