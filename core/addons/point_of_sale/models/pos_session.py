...
class PosSession(models.Model):
    ...
    cash_control = fields.Boolean(
        compute="_compute_cash_all", string="Has Cash Control", compute_sudo=True
    )
    ...
    cash_register_total_entry_encoding = fields.Monetary(
        compute="_compute_cash_balance", string="Total Cash Transaction", readonly=True
    )
        ...
        for session in self:
            ...
            if cash_payment_method:
                ...
                session.cash_register_total_entry_encoding = sum(
                    session.statement_line_ids.mapped("amount")
    ...
    def _check_start_date(self):
        for record in self:
            ...
            if (
                company.period_lock_date and start_date <= company.period_lock_date
            ) or (
                company.fiscalyear_lock_date
                and start_date <= company.fiscalyear_lock_date
            ):
                raise ValidationError(
                    _("You cannot create a session before the accounting lock date.")
                )
    ...
    def create(self, vals_list):
        for vals in vals_list:
            ...
            pos_name = (
                self.env["ir.sequence"].with_context(ctx).next_by_code("pos.session")
            )
            if vals.get("name"):
                pos_name += " " + vals["name"]
        ...
        if self.user_has_groups("point_of_sale.group_pos_user"):
            sessions = super(PosSession, self.with_context(ctx).sudo()).create(
                vals_list
            )
        else:
    ...
    def _cannot_close_session(self, bank_payment_method_diffs=None):
        ...
        if any(order.state == "draft" for order in self.order_ids):
            return {
                "successful": False,
                "message": _("You cannot close the POS when orders are still in draft"),
                "redirect": False,
            }
        if self.state == "closed":
            return {
    ...
    def _create_picking_at_end_of_session(self):
        ...
        for order in self.order_ids:
            if (
                order.company_id.anglo_saxon_accounting
                and order.is_invoiced
                or order.to_ship
            ):
                continue
            destination_id = (
            ...
            if not order_is_invoiced:
                ...
                if self.company_id.anglo_saxon_accounting and order.picking_ids.ids:
                    ...
                    for move in stock_moves:
                        ...
                        amount = (
                            signed_product_qty
                            * move.product_id._compute_average_price(
                                0, move.quantity_done, move
                            )
                        )
    ...
    def _create_non_reconciliable_move_lines(self, data):
        ...
        tax_names_no_account = [
            line["name"] for line in tax_vals if line["account_id"] == False
        ]
        ...
        account_payment.move_id.write(
            {
                "line_ids": [
                    ...
                    Command.update(
                        outstanding_line.id,
                        {
                            "debit": new_balance_compare_to_zero > 0
                            and new_balance
                            or 0.0,
                            "credit": new_balance_compare_to_zero < 0
                            and -new_balance
                            or 0.0,
                        },
                    ),
            ...
            if not float_is_zero(
                ...
                combine_cash_statement_line_vals.append(
                    self._get_combine_statement_line_vals(
                        payment_method.journal_id.id, amounts["amount"], payment_method
                    )
                )
    ...
    def _create_stock_output_lines(self, data):
        ...
        for stock_moves in [stock_output, stock_return]:
            for account, amounts in stock_moves.items():
                stock_output_vals[account].append(
    ...
    def _reconcile_account_move_lines(self, data):
        ...
        pickings |= self.order_ids.filtered(lambda o: not o.is_invoiced).mapped(
            "picking_ids"
        )
        stock_moves = self.env["stock.move"].search(
            [("picking_id", "in", pickings.ids)]
        )
        stock_account_move_lines = (
    ...
    def _get_split_receivable_vals(self, payment, amount, amount_converted):
        ...
        if not accounting_partner:
            raise UserError(
                _(
                    'You have enabled the "Identify Customer" option for %s payment method,'
                    "but the order %s does not contain a customer."
                )
                % (payment.payment_method_id.name, payment.pos_order_id.name)
            )
        partial_vals = {
    ...
    def _get_sale_vals(self, key, amount, amount_converted):
        ...
        if applied_taxes:
            name = "%s with %s" % (
                title,
                ", ".join([tax.name for tax in applied_taxes]),
            )
        partial_vals = {
            "name": name,
    ...
    def _get_attributes_by_ptal_id(self):
        ...
        product_attributes_by_id = {
            product_attribute.id: product_attribute
    ...
    def _pos_data_process(self, loaded_data):
        ...
        loaded_data["units_by_id"] = {
            unit["id"]: unit for unit in loaded_data["uom.uom"]
        }

        loaded_data["taxes_by_id"] = {
            tax["id"]: tax for tax in loaded_data["account.tax"]
        }
        ...
        fiscal_position_by_id = {
            fpt["id"]: fpt
            for fpt in self._get_pos_ui_account_fiscal_position_tax(
        ...
        for fiscal_position in loaded_data["account.fiscal.position"]:
            fiscal_position["fiscal_position_taxes_by_id"] = {
                tax_id: fiscal_position_by_id[tax_id]
    ...
    def _get_pos_ui_res_company(self, params):
        ...
        if company["country_id"]:
            ...
            params_country["search_params"]["domain"] = [
                ("id", "=", company["country_id"][0])
            ]
            company["country"] = self.env["res.country"].search_read(
                **params_country["search_params"]
            )[0]
        else:
    ...
    def _get_pos_ui_decimal_precision(self, params):
        decimal_precisions = self.env["decimal.precision"].search_read(
    ...
    def _loader_params_res_country(self):
        return {
            "search_params": {"domain": [], "fields": ["name", "vat_label", "code"]}
        }
    ...
    def _loader_params_pos_config(self):
        return {
            "search_params": {"domain": [("id", "=", self.config_id.id)], "fields": []}
        }

    def _get_pos_ui_pos_config(self, params):
        config = self.env["pos.config"].search_read(**params["search_params"])[0]
        config["use_proxy"] = config["is_posbox"] and (
            config["iface_electronic_scale"]
            or config["iface_print_via_proxy"]
            or config["iface_scan_via_proxy"]
            or config["iface_customer_facing_display_via_proxy"]
        )
        return config
    ...
    def _get_pos_ui_res_users(self, params):
        ...
        user["role"] = (
            "manager"
            if any(
                id == self.config_id.group_pos_manager_id.id for id in user["groups_id"]
            )
            else "cashier"
        )
        del user["groups_id"]
    ...
    def _get_pos_ui_account_fiscal_position(self, params):
        return self.env["account.fiscal.position"].search_read(
            **params["search_params"]
        )
    ...
    def _get_pos_ui_account_fiscal_position_tax(self, params):
        return self.env["account.fiscal.position.tax"].search_read(
            **params["search_params"]
        )
