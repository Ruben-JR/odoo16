        ...
        with patch.object(
            ...
            with self.assertRaises(exceptions.AccessError):
                searched_activity = (
                    ...
                    ._search([("id", "=", test_activity.id)], count=False)
                )
...
class TestActivityMixin(TestActivityCommon):
    ...
    def test_activity_mixin(self):
        ...
        with self.with_user("employee"):
            ...
            act2 = self.test_record.activity_schedule(
                "test_mail.mail_act_test_meeting", today_user + relativedelta(days=-1)
            )
            ...
            self.test_record.activity_feedback(
                ["test_mail.mail_act_test_todo"], feedback="Test feedback"
            )
            ...
            self.assertEqual(
                self.test_record.message_ids.mapped("subtype_id"),
                self.env.ref("mail.mt_activities"),
            )
    ...
    def test_mail_activity_mixin_search_state_basic(self):
        ...
        with patch(
            "odoo.addons.mail.models.mail_activity.datetime", MockedDatetime
        ), patch(
            "odoo.addons.mail.models.mail_activity_mixin.datetime", MockedDatetime
        ):
            ...
            self.assertEqual(
                result,
                all_activity_mixin_record.filtered(
                    lambda p: p.activity_state not in ("today",)
                ),
            )
