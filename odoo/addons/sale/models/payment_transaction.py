        ...
        for tx in self:
            ...
            if len(tx.sale_order_ids) == 1:
                ...
                if (
                    quotation
                    and len(
                        quotation.transaction_ids.filtered(
                            lambda tx: tx.state
                            in ("authorized", "done")  # Only consider confirmed tx
                        )
                    )
                    == 1
                ):
                    ...
                    if (
                        quotation.currency_id.compare_amounts(
                            tx.amount, quotation.amount_total
                        )
                        == 0
                    ):
                        ...
                        _logger.warning(
                            ...
                            "(ID %(so_id)s): expected %(so_amount)s, got %(tx_amount)s",
                            {
                                ...
                                "tx_amount": format_amount(
                                    tx.env, tx.amount, tx.currency_id
                                ),
                            },
                        )
