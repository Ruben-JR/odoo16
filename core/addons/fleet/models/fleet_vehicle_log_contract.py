...
class FleetVehicleLogContract(models.Model):
    ...
    vehicle_id = fields.Many2one(
        "fleet.vehicle", "Vehicle", required=True, check_company=True
    )
    ...
    name = fields.Char(
        string="Name", compute="_compute_contract_name", store=True, readonly=False
    )
    active = fields.Boolean(default=True)
    user_id = fields.Many2one(
        "res.users", "Responsible", default=lambda self: self.env.user, index=True
    )
    ...
    def scheduler_manage_contract_expiration(self):
        ...
        reminder_activity_type = (
            self.env.ref(
                "fleet.mail_act_fleet_contract_to_renew", raise_if_not_found=False
            )
