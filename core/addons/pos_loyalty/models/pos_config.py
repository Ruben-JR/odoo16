...
class PosConfig(models.Model):
    ...
    def use_coupon_code(self, code, creation_date, partner_id):
        ...
        if (
            ...
            or (
                coupon.program_id.date_to
                and coupon.program_id.date_to < fields.Date.context_today(self)
            )
            or (
                coupon.program_id.limit_usage
                and coupon.program_id.total_order_count >= coupon.program_id.max_usage
            )
        ):
            return {
                "successful": False,
        ...
        if not coupon.program_id.reward_ids or not any(
            reward.required_points <= coupon.points
