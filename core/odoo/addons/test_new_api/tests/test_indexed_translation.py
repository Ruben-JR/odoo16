...
class TestIndexedTranslation(odoo.tests.TransactionCase):
    ...
    def test_search_ilike(self):
        ...
        self.assertEqual(
            record_en.search([("name", "ilike", 'class="my_class')]), record_en
        )

        # escaped and unescaped PG wildcards
        self.assertEqual(
            record_en.search([("name", "ilike", "class%class")]), record_en
        )
        self.assertEqual(
            record_en.search([("name", "ilike", r'class="m_\_class')]), record_en
        )
        self.assertEqual(
            record_en.search([("name", "ilike", "bonjour")]), record_en.browse()
        )
        self.assertEqual(
            record_en.search([("name", "ilike", "</div>\n<div/>")]), record_en
        )
        self.assertEqual(
            record_fr.search([("name", "ilike", "</div>\a<div/>")]), record_fr
        )
        self.assertEqual(
            record_fr.search([("name", "ilike", r"\%bonjour\\")]), record_fr
        )
        ...
        self.assertEqual(
            record_fr.search(
                [("name", "=", '<div class="my_class">%bonjour\\</div>\a<div/>')]
            ),
            record_fr,
        )
