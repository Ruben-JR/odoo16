...
class AccountMove(models.Model):
    ...
    l10n_it_amount_vat_signed = fields.Monetary(
        string="VAT", compute="_compute_amount_extended"
    )
    l10n_it_amount_pension_fund_signed = fields.Monetary(
        string="Pension Fund", compute="_compute_amount_extended"
    )
    l10n_it_amount_withholding_signed = fields.Monetary(
        string="Withholding", compute="_compute_amount_extended"
    )
    l10n_it_amount_before_withholding_signed = fields.Monetary(
        string="Total Before Withholding", compute="_compute_amount_extended"
    )
    ...
    def _compute_amount_extended(self):
        for move in self:
            ...
            move.l10n_it_amount_before_withholding_signed = (
                move.amount_untaxed_signed + totals["vat"] + totals["pension_fund"]
            )
    ...
    def _l10n_it_edi_filter_fatturapa_tax_details(self, line, tax_values):
        ...
        return (
            repartition_line.factor_percent >= 0
            and repartition_line_vat
            and repartition_line_vat.amount >= 0
        )
    ...
    def _prepare_fatturapa_export_values(self):
        ...
        withholding_lines = self.line_ids.filtered(
            lambda x: x.tax_line_id._l10n_it_filter_kind("withholding")
        )
        withholding_values = [
            WithholdingTaxData(x.tax_line_id, abs(x.balance)) for x in withholding_lines
        ]
        ...
        PensionFundTaxData = namedtuple(
            "TaxData",
            ["tax", "base_amount", "tax_amount", "vat_tax", "withholding_tax"],
        )
        pension_fund_lines = self.line_ids.filtered(
            lambda line: line.tax_line_id._l10n_it_filter_kind("pension_fund")
        )
        pension_fund_mapping = {}
        for line in self.line_ids:
            pension_fund_tax = line.tax_ids._l10n_it_filter_kind("pension_fund")
            if pension_fund_tax:
                pension_fund_mapping[pension_fund_tax.id] = (
                    line.tax_ids._l10n_it_filter_kind("vat"),
                    line.tax_ids._l10n_it_filter_kind("withholding"),
                )
        ...
        if enasarco_taxes:
            enasarco_values = {}
            enasarco_details = self._prepare_edi_tax_details(
                filter_to_apply=lambda line, tax_values: self.env["account.tax"]
                .browse([tax_values["id"]])
