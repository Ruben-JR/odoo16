...
class TestImage(HttpCase):
    ...
    def test_03_web_content_filename(self):
        ...
        self.assertEqual(
            res.headers["Content-Disposition"], "attachment; filename=testFilename.gif"
        )
        ...
        res.raise_for_status()
        self.assertEqual(
            res.headers["Content-Disposition"], "attachment; filename=custom.gif"
        )
        ...
        res.raise_for_status()
        self.assertEqual(
            res.headers["Content-Disposition"], "attachment; filename=custom.png"
        )
    ...
    def test_04_web_content_filename_secure(self):
        ...
        self.assertEqual(
            ...
            r"""inline; filename="foo-l'eb _ a\"!r\".gif"; filename*=UTF-8''"""
            + expected_ufilename,
        )
        res.raise_for_status()
        ...
        self.assertEqual(
            res.headers["Content-Disposition"],
            "inline; filename=custom_invalid_name_is-ok.gif",
        )
        res.raise_for_status()
        ...
        self.assertEqual(
            res.headers["Content-Disposition"],
            "inline; filename=.gif; filename*=UTF-8''%E4%BD%A0%E5%A5%BD.gif",
        )
        res.raise_for_status()
        ...
        self.assertEqual(
            res.headers["Content-Disposition"],
            "inline; filename=.gif; filename*=UTF-8''%E9%9D%A2%E5%9B%BE.gif",
        )
        res.raise_for_status()
        ...
        self.assertEqual(
            res.headers["Content-Disposition"],
            "inline; filename=hindi_.gif; filename*=UTF-8''hindi_%E0%A4%A8%E0%A4%AE%E0%A4%B8%E0%A5%8D%E0%A4%A4%E0%A5%87.gif",
        )
        res.raise_for_status()
        res = self.url_open(f"/web/image/{att.id}/arabic_مرحبا.gif")
        self.assertEqual(
            res.headers["Content-Disposition"],
            "inline; filename=arabic_.gif; filename*=UTF-8''arabic_%D9%85%D8%B1%D8%AD%D8%A8%D8%A7.gif",
        )
        res.raise_for_status()
        ...
        self.assertEqual(
            res.headers["Content-Disposition"],
            "inline; filename=4wzb_!!63148-0-t1.jpg_360x1Q75.jpg_.webp",
        )
        res.raise_for_status()
