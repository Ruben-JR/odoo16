...
class TestLeadConvert(crm_common.TestCrmCommon):
    ...
    def test_lead_lost(self):
        ...
        self.assertEqual(
            lead.lost_reason_id, self.lost_reason
        )  # TDE FIXME: should be called lost_reason_id non didjou
        ...
        self.assertEqual(
            len(update_message.tracking_value_ids), 2, "Tracking: active, lost reason"
        )
        self.assertTracking(
            update_message,
            [
    ...
    def test_lead_lost_crm_rights(self):
        ...
        lost_wizard = (
            self.env["crm.lead.lost"]
            .with_context({"active_ids": lead.ids})
            .create({"lost_reason_id": lost_reason.id})
        )
