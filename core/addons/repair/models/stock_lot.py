...
class StockLot(models.Model):
    ...
    repair_order_ids = fields.Many2many(
        "repair.order", string="Repair Orders", compute="_compute_repair_order_ids"
    )
    repair_order_count = fields.Integer(
        "Repair order count", compute="_compute_repair_order_ids"
    )

    @api.depends("name")
    def _compute_repair_order_ids(self):
        repair_orders = defaultdict(lambda: self.env["repair.order"])
        for repair_line in self.env["repair.line"].search(
    ...
    def action_view_ro(self):
        ...
        if len(self.repair_order_ids) == 1:
            action.update({"view_mode": "form", "res_id": self.repair_order_ids[0].id})
        else:
            action.update(
                {
