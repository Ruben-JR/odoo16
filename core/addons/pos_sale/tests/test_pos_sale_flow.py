...
class TestPoSSale(TestPointOfSaleHttpCommon):
    def test_settle_order_with_kit(self):
        ...
        self.assertEqual(
            sale_order.picking_ids[1].move_ids.quantity_done, 300
    ...
    def test_settle_order_with_incompatible_partner(self):
        ...
        self.start_tour(
            "/pos/ui?config_id=%d" % self.main_pos_config.id,
            "PosSettleOrderIncompatiblePartner",
            login="accountman",
        )
    ...
    def test_settle_order_with_different_product(self):
        ...
        self.assertEqual(sale_order.order_line[1].qty_delivered, 0)
        orderline_product_a = sale_order.order_line.filtered(
            lambda l: l.product_id.id == product_a.id
        )
        orderline_product_b = sale_order.order_line.filtered(
