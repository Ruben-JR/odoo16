...
class TestSaleDropshippingFlows(TestMrpSubcontractingCommon):
        ...
        compo01, compo02, compo03, kit = self.env["product.product"].create(
            [
                ...
                for name, seller in zip(
                    ["Compo01", "Compo02", "Compo03", "Kit"], partners
                )
            ]
        )
        ...
        sale_order = self.env["sale.order"].create(
            {
                ...
                "order_line": [
                    (
                        0,
                        0,
                        {"name": kit.name, "product_id": kit.id, "product_uom_qty": 1},
                    ),
                ],
            }
        )
    ...
    def test_partial_return_kit_and_delivered_qty(self):
        ...
        self.env["purchase.order"].search([], order="id desc", limit=1).button_confirm()
        self.assertEqual(
            sale_order.order_line.qty_delivered, 0.0, "Delivered components: 0/4"
        )
        ...
        wizard.process()
        self.assertEqual(
            sale_order.order_line.qty_delivered, 0.0, "Delivered components: 2/4"
        )
        ...
        wizard.process()
        self.assertEqual(
            sale_order.order_line.qty_delivered, 0.0, "Delivered components: 1/4"
        )
        ...
        self.assertEqual(
            sale_order.order_line.qty_delivered, 0.0, "Delivered components: 3/4"
        )
        ...
        picking04.button_validate()
        self.assertEqual(
            sale_order.order_line.qty_delivered, 1, "Delivered components: 4/4"
        )
