...
class MailChannel(models.Model):
    ...
    livechat_active = fields.Boolean(
        "Is livechat ongoing?",
    ...
    chatbot_message_ids = fields.One2many(
        "chatbot.message", "mail_channel_id", string="Chatbot Messages"
    )
    ...
    def _channel_message_notifications(self, message, message_format=False):
        ...
        """
        notifications = super()._channel_message_notifications(
        ...
        for channel in self:
            ...
            if channel.channel_type == "livechat":
                notifications.append(
                    [channel.uuid, "mail.channel/new_message", notifications[0][2]]
                )
        if not message.author_id:
            unpinned_members = self.channel_member_ids.filtered(
                lambda member: not member.is_pinned
            )
            if unpinned_members:
                unpinned_members.write({"is_pinned": True})
                notifications = (
                    self._channel_channel_notifications(unpinned_members.partner_id.ids)
                    + notifications
                )
    ...
    def channel_info(self):
        ...
        for channel in self:
            channel_infos_dict[channel.id]["channel"][
                "anonymous_name"
            ] = channel.anonymous_name
            ...
            if channel.livechat_operator_id:
                display_name = (
                    channel.livechat_operator_id.user_livechat_username
                    or channel.livechat_operator_id.display_name
                )
                channel_infos_dict[channel.id]["operator_pid"] = (
    ...
    def _send_history_message(self, pid, page_history):
        ...
        if page_history:
            html_links = [
                ...
                % (html_escape(page), html_escape(page))
    ...
    def _chatbot_restart(self, chatbot_script):
        self.write({"chatbot_current_step_id": False})

        self.chatbot_message_ids.unlink()

        return self._chatbot_post_message(
            chatbot_script,
            '<div class="o_mail_notification">%s</div>'
