...
class HrEmployeeBase(models.AbstractModel):
    ...
    leaves_count = fields.Float(
        "Number of Time Off", compute="_compute_remaining_leaves"
    )
    allocation_count = fields.Float(
        "Total number of days allocated.", compute="_compute_allocation_count"
    )
    allocations_count = fields.Integer(
        "Total number of allocations", compute="_compute_allocation_count"
    )
    ...
    def _get_remaining_leaves(self):
        ...
        return dict(
            (row["employee_id"], row["days"]) for row in self._cr.dictfetchall()
        )
    ...
    def _compute_allocation_count(self):
        ...
        data = self.env["hr.leave.allocation"]._read_group(
            ...
            ["number_of_days:sum", "employee_id"],
            ["employee_id"],
        )
        rg_results = dict(
            (
                d["employee_id"][0],
                {
                    "employee_id_count": d["employee_id_count"],
        ...
        for employee in self:
            ...
            employee.allocation_count = (
                float_round(result["number_of_days"], precision_digits=2)
                if result
                else 0.0
            )
    ...
    def _compute_allocation_remaining_display(self):
        ...
        leaves_taken = allocations.holiday_status_id._get_employees_days_per_allocation(
            self.ids
        )
        for employee in self:
            ...
            for leave_type in leaves_taken[employee.id]:
                ...
                for allocation in leaves_taken[employee.id][leave_type]:
                    if allocation:
                        virtual_remaining_leaves = leaves_taken[employee.id][
                            leave_type
                        ][allocation]["virtual_remaining_leaves"]
    ...
    def _compute_presence_state(self):
        ...
        employees = self.filtered(
            lambda employee: employee.hr_presence_state != "present"
    ...
    def _compute_leave_status(self):
        ...
        for employee in self:
            ...
            employee.is_absent = leave_data.get(employee.id) and leave_data.get(
                employee.id, {}
            ).get("current_leave_state") in ["validate"]
        ...
        if "parent_id" in values:
            ...
            if manager:
                to_change = self.filtered(
                    lambda e: e.leave_manager_id == e.parent_id.user_id
...
class HrEmployee(models.Model):
    ...
    def get_stress_days(self, start_date, end_date):
        ...
        for stress_day in stress_days:
            ...
            for d in range(num_days + 1):
                all_days[
                    str(stress_day.start_date + relativedelta(days=d))
                ] = stress_day.color
    ...
    def _get_stress_days(self, start_date, end_date):
        ...
        is_leave_user = self == self.env.user.employee_id and self.user_has_groups(
            "hr_holidays.group_hr_holidays_user"
        )

        if not is_leave_user and stress_days.department_ids:
            stress_days = stress_days.filtered(
                lambda sd: not sd.department_ids
                or self.department_id
                and not set(self.department_id.ids).isdisjoint(
                    sd.department_ids.get_children_department_ids().ids
                )
            )
