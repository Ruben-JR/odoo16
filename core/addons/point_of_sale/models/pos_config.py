...
class PosConfig(models.Model):
    ...
    iface_customer_facing_display_via_proxy = fields.Boolean(
        string="Customer Facing Display",
        help="Show checkout to customers with a remotely-connected screen.",
    )
    iface_customer_facing_display_local = fields.Boolean(
        string="Local Customer Facing Display", help="Show checkout to customers."
    )
    ...
    number_of_opened_session = fields.Integer(
        string="Number of Opened Session", compute="_compute_current_session"
    )
    ...
    def _compute_company_has_template(self):
        for config in self:
            config.company_has_template = (
                ...
                or config.company_id.chart_template_id
            )
        ...
        for pos_config in self:
            if pos_config.journal_id:
                pos_config.currency_id = (
                    ...
                    or pos_config.journal_id.company_id.currency_id.id
                )
            else:
    ...
    def _compute_current_session(self):
        ...
        for pos_config in self:
            ...
            session = pos_config.session_ids.filtered(
                lambda s: not s.state == "closed" and not s.rescue
            )
    ...
    @api.depends(
        "iface_customer_facing_display_via_proxy", "iface_customer_facing_display_local"
    )
    def _compute_customer_facing_display(self):
        for config in self:
            config.iface_customer_facing_display = (
                config.iface_customer_facing_display_via_proxy
                or config.iface_customer_facing_display_local
            )
    ...
    def _check_company_journal(self):
        for config in self:
            if (
                config.journal_id
                and config.journal_id.company_id.id != config.company_id.id
            ):
                raise ValidationError(
                    _(
    ...
    def _check_company_invoice_journal(self):
        for config in self:
            if (
                ...
                and config.invoice_journal_id.company_id.id != config.company_id.id
            ):
                raise ValidationError(
                    _(
    ...
    def _check_currencies(self):
        ...
        if (
            self.invoice_journal_id.currency_id
            and self.invoice_journal_id.currency_id != self.currency_id
        ):
            raise ValidationError(
                _(
    ...
    def _check_start_categ(self):
        for config in self:
            allowed_categ_ids = config.iface_available_categ_ids or self.env[
                "pos.category"
            ].search([])
            if (
                config.iface_start_categ_id
                and config.iface_start_categ_id not in allowed_categ_ids
            ):
                raise ValidationError(
                    _("Start category should belong in the available categories.")
                )
    ...
    def name_get(self):
        ...
        for config in self:
            ...
            if (not last_session) or (last_session.state == "closed"):
                result.append(
                    (config.id, _("%(pos_name)s (not used)", pos_name=config.name))
                )
            else:
                result.append(
                    (config.id, "%s (%s)" % (config.name, last_session.user_id.name))
                )
        return result
    ...
    def _reset_default_on_vals(self, vals):
        if (
            ...
            and any(self.mapped("iface_tipproduct"))
    ...
    def execute(self):
        return {"type": "ir.actions.client", "tag": "reload", "params": {"wait": True}}

    def _force_http(self):
        enforce_https = (
            self.env["ir.config_parameter"]
            .sudo()
            .get_param("point_of_sale.enforce_https")
        )
    ...
    def setup_invoice_journal(self, company):
        for pos_config in self:
            invoice_journal_id = pos_config.invoice_journal_id or self.env[
                "account.journal"
            ].search([("type", "=", "sale"), ("company_id", "=", company.id)], limit=1)
    ...
    def get_limited_products_loading(self, fields):
        ...
        products = self.env["product.product"].search_read(
            [("id", "in", product_ids)], fields=fields
        )
        return products
