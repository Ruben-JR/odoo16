...
class ResConfigSettings(models.TransientModel):
    ...
    is_default_pricelist_displayed = fields.Boolean(
    ...
    pos_iface_customer_facing_display_local = fields.Boolean(
        related="pos_config_id.iface_customer_facing_display_local", readonly=False
    )
    pos_iface_customer_facing_display_via_proxy = fields.Boolean(
    ...
    pos_iface_start_categ_id = fields.Many2one(
        "pos.category",
    ...
    pos_limited_partners_amount = fields.Integer(
        related="pos_config_id.limited_partners_amount", readonly=False
    )
    pos_limited_partners_loading = fields.Boolean(
        related="pos_config_id.limited_partners_loading", readonly=False
    )
    pos_limited_products_amount = fields.Integer(
        related="pos_config_id.limited_products_amount", readonly=False
    )
    pos_limited_products_loading = fields.Boolean(
    ...
    pos_partner_load_background = fields.Boolean(
    ...
    pos_product_load_background = fields.Boolean(
        related="pos_config_id.product_load_background", readonly=False
    )
    pos_proxy_ip = fields.Char(
        string="IP Address", compute="_compute_pos_proxy_ip", readonly=False, store=True
    )
    ...
    pos_start_category = fields.Boolean(
        related="pos_config_id.start_category", readonly=False
    )
    ...
    def _compute_pos_iface_start_categ_id(self):
        for res_config in self:
            if not res_config.pos_start_category:
                ...
                res_config.pos_iface_start_categ_id = (
                    res_config.pos_config_id.iface_start_categ_id
                )
            ...
            pricelists_in_current_currency = self.env["product.pricelist"].search(
                [
                    (
                        "company_id",
                        "in",
                        (False, res_config.pos_config_id.company_id.id),
                    ),
            ...
            if not res_config.pos_use_pricelist:
                ...
                if any(
                    ...
                    res_config.pos_available_pricelist_ids = (
                        pricelists_in_current_currency
                    )
                    res_config.pos_pricelist_id = pricelists_in_current_currency[:1]
                else:
                    res_config.pos_available_pricelist_ids = (
    ...
    def _compute_pos_iface_customer_facing_display_via_proxy(self):
        for res_config in self:
            if not res_config.pos_is_posbox:
                ...
                res_config.pos_iface_customer_facing_display_via_proxy = (
                    res_config.pos_config_id.iface_customer_facing_display_via_proxy
                )
