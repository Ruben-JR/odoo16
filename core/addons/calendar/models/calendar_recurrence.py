...
class RecurrenceRule(models.Model):
    ...
    def _compute_name(self):
        for recurrence in self:
            ...
            every = _(
                "Every %(count)s %(period)s", count=recurrence.interval, period=period
            )
    ...
    def _compute_dtstart(self):
        ...
        start_mapping = {group["recurrence_id"][0]: group["start"] for group in groups}
        for recurrence in self:
    ...
    def _range_calculation(self, event, duration):
        ...
        future_events = set(
            (x, y)
            for x, y in ranges
            if x.date() >= event.start.date() and y.date() >= event.start.date()
        )
    ...
    def _get_occurrences(self, dtstart):
        ...
        #                                                  *****              *****
        return (
            timezone.localize(occurrence, is_dst=False)
            .astimezone(pytz.utc)
            .replace(tzinfo=None)
