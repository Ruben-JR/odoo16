...
class WebsiteTrack(models.Model):
    ...
    product_id = fields.Many2one(
        "product.product", ondelete="cascade", readonly=True, index="btree_not_null"
    )
    ...
    def _compute_product_statistics(self):
        results = self.env["website.track"]._read_group(
            [
                ...
                ("product_id", "!=", False),
                "|",
                ("product_id.company_id", "in", self.env.companies.ids),
        ...
        for result in results:
            visitor_info = mapped_data.get(
                result["visitor_id"][0], {"product_count": 0, "product_ids": set()}
            )
