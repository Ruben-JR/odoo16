...
class TestPartner(MailCommon):
    ...
    def test_res_partner_log_portal_group(self):
        ...
        group_portal, group_user = self.env.ref("base.group_portal"), self.env.ref(
            "base.group_user"
        )
    ...
    def test_res_partner_get_mention_suggestions_priority(self):
        ...
        self.env["res.partner"].create(
            [{"name": f"{name}-{i}-not-user"} for i in range(0, 2)]
        )
        for i in range(0, 2):
            mail_new_test_user(
        ...
        self.assertEqual(
            list(
                map(
                    lambda p: isinstance(p["user"], dict)
                    and p["user"]["isInternalUser"],
                    partners_format,
                )
            ),
        ...
        self.assertEqual(
            list(map(lambda p: isinstance(p["user"], dict), partners_format)),
    ...
    def test_res_partner_merge_wizards(self):
        ...
        p1_msg1 = p1.message_post(
            body="<p>Log on P1</p>", subtype_id=self.env.ref("mail.mt_comment").id
        )
