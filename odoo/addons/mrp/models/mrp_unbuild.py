...
class MrpUnbuild(models.Model):
    ...
    product_qty = fields.Float(
        "Quantity", default=1.0, required=True, states={"done": [("readonly", True)]}
    )
...
""",
    ...
    def _onchange_product_id(self):
        if self.product_id:
            ...
            self.product_uom_id = (
                self.mo_id.product_id == self.product_id
                and self.mo_id.product_uom_id.id
    ...
    def action_unbuild(self):
        ...
        if any(
            ...
            raise UserError(
                _(
                    "Some of your byproducts are tracked, you have to specify a manufacturing order in order to retrieve the correct byproducts."
                )
            )
            ...
            if move.has_tracking != "none":
                ...
                if move in produce_moves and self.lot_id:
                    moves_lines = moves_lines.filtered(
                        lambda ml: self.lot_id in ml.produce_line_ids.lot_id
                    )  # FIXME sle: double check with arm
        ...
        if self.mo_id:
            ...
            self.mo_id.message_post(
                body=unbuild_msg, subtype_id=self.env.ref("mail.mt_note").id
            )
    ...
    def action_validate(self):
        ...
        if float_compare(available_qty, unbuild_qty, precision_digits=precision) >= 0:
            ...
            return {
                ...
                + _(": Insufficient Quantity To Unbuild"),
                "view_mode": "form",
