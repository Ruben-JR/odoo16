...
class WebsiteSale(http.Controller):
            ...
            def filter_template(template, attribute_values_list):
                ...
                for attribute_values in attribute_values_list:
                    ...
                    if len(ptavs) == len(template.attribute_line_ids):
                        ...
                        if any(
                            len(ptavs & combination) == len(ptavs)
    ...
    def _shop_get_query_url_kwargs(
        self, category, search, min_price, max_price, attrib=None, order=None, **post
    ):
        return {
            "category": category,
            "search": search,
    ...
    def old_product(self, product, category="", search="", **kwargs):
        ...
        return request.redirect(
            _build_url_w_params("/shop/%s" % slug(product), request.params), code=301
        )

    @http.route(["/shop/product/extra-images"], type="json", auth="user", website=True)
    def add_product_images(
        self, images, product_product_id, product_template_id, combination_ids=None
    ):
        """
        ...
        if product_template.has_configurable_attributes and product_product:
            product_product.write({"product_variant_image_ids": image_create_data})
        else:
            product_template.write({"product_template_image_ids": image_create_data})
    ...
    def remove_product_image(self, image_res_model, image_res_id):
        ...
        if image_res_model == "product.product":
            request.env["product.product"].browse(image_res_id).write(
                {"image_1920": False}
            )
        elif image_res_model == "product.template":
            request.env["product.template"].browse(image_res_id).write(
                {"image_1920": False}
            )
        else:
    ...
    @http.route(
        ["/shop/product/resequence-image"], type="json", auth="user", website=True
    )
    def resequence_product_image(self, image_res_model, image_res_id, move):
        ...
        if (
            image_res_model == "product.image"
            and image_to_resequence.video_url
            and product_images[new_image_idx]._name != "product.image"
        ):
            raise ValidationError(
                _(
                    "Can not resequence embedded image/video with a non compatible image."
                )
            )
        ...
        source_field = (
            hasattr(image_to_resequence, "video_url")
            and image_to_resequence.video_url
            and "video_url"
            or "image_1920"
        )
        target_field = (
            hasattr(other_image, "video_url")
            and other_image.video_url
            and "video_url"
            or "image_1920"
        )
    ...
    def checkout_check_address(self, order):
        ...
        if not all(
            ...
            return request.redirect(
                "/shop/address?partner_id=%d" % order.partner_shipping_id.id
            )
    ...
    def checkout_form_validate(self, mode, all_form_values, data):
        ...
        if data.get("partner_id"):
            ...
            if (
                partner.exists()
                and partner.name
                and not partner.sudo().can_edit_vat()
                and "name" in data
                and (data["name"] or False) != (partner.name or False)
            ):
                error["name"] = "error"
    ...
    def process_express_checkout(self, billing_address, **kwargs):
        ...
        if order_sudo.partner_id == public_partner:
            ...
            order_sudo.message_partner_ids = request.env["res.partner"].browse(
                billing_partner_id
            )
        elif any(
            billing_address[k] != order_sudo.partner_invoice_id[k]
            for k in billing_address
        ):
    ...
    # ------------------------------------------------------
    @http.route(
        ["/shop/extra_info"], type="http", auth="public", website=True, sitemap=False
    )
    ...
    def shop_payment_get_status(self, sale_order_id, **post):
        ...
        return {
            ...
            "message": request.env["ir.ui.view"]._render_template(
                "website_sale.payment_confirmation_status", {"order": order}
            ),
        }
    ...
    @http.route(
        ["/shop/print"], type="http", auth="public", website=True, sitemap=False
    )
    ...
    def products_recently_viewed_delete(self, product_id, **kwargs):
        ...
        if visitor_sudo:
            request.env["website.track"].sudo().search(
                [("visitor_id", "=", visitor_sudo.id), ("product_id", "=", product_id)]
            ).unlink()
        return {}
...
class CustomerPortal(portal.CustomerPortal):
    ...
    @http.route(
        "/my/orders/reorder_modal_content", type="json", auth="public", website=True
    )
    def _get_saleorder_reorder_content_modal(self, order_id, access_token):
        ...
        for line in sale_order.order_line:
            ...
            res = {
                ...
                "add_to_cart_allowed": line.with_user(request.env.user)
                .sudo()
                ._is_reorder_allowed(),
