...
class WebsiteSaleDelivery(WebsiteSale):
    ...
    def shop_payment(self, **post):
        ...
        if order:
            order._check_carrier_quotation(
                force_carrier_id=carrier_id, keep_carrier=keep_carrier
            )
            if carrier_id:
    ...
    def process_express_checkout(
        ...
        if order_sudo.partner_shipping_id.name.endswith(order_sudo.name):
            ...
            shipping_address[k] != order_sudo.partner_shipping_id[k]
            for k in shipping_address
        ):
            ...
            [
                {
                    ...
                    "minorAmount": payment_utils.to_minor_currency_units(
                        WebsiteSaleDelivery._get_rate(
                            carrier, order_sudo, is_express_checkout_flow=True
                        )["price"],
    ...
    def order_lines_2_google_api(self, order_lines):
        ...
        order_lines_not_delivery = order_lines.filtered(
            lambda line: not line.is_delivery
        )
        return super(WebsiteSaleDelivery, self).order_lines_2_google_api(
            order_lines_not_delivery
        )
    ...
    def _get_express_shop_payment_values(self, order, **kwargs):
        values = super(WebsiteSaleDelivery, self)._get_express_shop_payment_values(
            order, **kwargs
        )
