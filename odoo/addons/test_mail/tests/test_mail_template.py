...
class TestMailTemplateLanguages(TestMailTemplateCommon):
    ...
    def test_template_translation_lang(self):
        ...
        mail_id = test_template.send_mail(
            test_record.id, email_layout_xmlid="mail.test_layout"
        )
        mail = self.env["mail.mail"].sudo().browse(mail_id)
        self.assertEqual(
            mail.body_html,
            "<body><p>SpanishBody for %s</p> Spanish Layout para Spanish Model Description</body>"
