...
class Orderpoint(models.Model):
    ...
    show_supplier = fields.Boolean(
        "Show supplier column", compute="_compute_show_suppplier"
    )
    ...
    vendor_id = fields.Many2one(
        related="supplier_id.partner_id", string="Vendor", store=True
    )
    purchase_visibility_days = fields.Float(
        default=0.0, help="Visibility Days applied on the purchase routes."
    )

    @api.depends(
        "product_id.purchase_order_line_ids", "product_id.purchase_order_line_ids.state"
    )
    ...
    def _set_default_route_id(self):
        route_id = (
            self.env["stock.rule"].search([("action", "=", "buy")], limit=1).route_id
        )
        orderpoint_wh_supplier = self.filtered(lambda o: o.product_id.seller_ids)
        if (
            route_id
            and orderpoint_wh_supplier
            and (not self.product_id.route_ids or route_id in self.product_id.route_ids)
        ):
