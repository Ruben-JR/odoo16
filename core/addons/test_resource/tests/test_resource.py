...
class TestCalendar(TestResourceCommon):
    ...
    def test_plan_hours(self):
        ...
        time = self.calendar_jean.plan_hours(
            2,
            datetime_tz(2018, 4, 10, 0, 0, 0, tzinfo=self.jean.tz),
            compute_leaves=False,
        )
        ...
        time = self.calendar_jean.plan_hours(
            5,
            datetime_tz(2018, 4, 10, 15, 0, 0, tzinfo=self.jean.tz),
            compute_leaves=True,
        )
        self.assertEqual(time, datetime_tz(2018, 4, 12, 12, 0, 0, tzinfo=self.jean.tz))
        ...
        time = self.calendar_jean.plan_hours(
            0,
            datetime_tz(2018, 4, 11, 0, 0, 0, tzinfo=self.jean.tz),
            compute_leaves=True,
        )
        ...
        self.assertEqual(
            time, datetime_tz(2018, 4, 10, 8, 0, 0, 720000, tzinfo=self.jean.tz)
        )
    ...
    def test_plan_days(self):
        ...
        time = self.calendar_jean.plan_days(
            4,
            datetime_tz(2018, 4, 10, 16, 0, 0, tzinfo=self.jean.tz),
            compute_leaves=True,
        )
    ...
    def test_closest_time(self):
        ...
        self.assertEqual(
            calendar_dt,
            start,
            "It should not take into account resouce specific attendances",
        )

        dt = datetime_tz(2020, 4, 3, 5, 0, 0, tzinfo=self.john.tz)
        start = datetime_tz(2020, 4, 3, 5, 0, 0, tzinfo=self.john.tz)
        calendar_dt = self.calendar_john._get_closest_work_time(
            dt, resource=self.john.resource_id
        )
        self.assertEqual(
            calendar_dt, start, "It should have taken john's specific attendances"
        )

        dt = datetime_tz(
            2020, 4, 4, 1, 0, 0, tzinfo="UTC"
        )  # The next day in UTC, but still the 3rd in john's timezone (America/Los_Angeles)
        start = datetime_tz(2020, 4, 3, 16, 0, 0, tzinfo=self.john.tz)
        calendar_dt = self.calendar_john._get_closest_work_time(
            dt, resource=self.john.resource_id
        )
        self.assertEqual(
            calendar_dt, start, "It should have found the attendance on the 3rd April"
        )
...
class TestTimezones(TestResourceCommon):
    ...
    def test_plan_hours(self):
        ...
        dt = self.calendar_jean.plan_hours(
            10, datetime_tz(2018, 4, 10, 8, 0, 0, tzinfo=self.tz4)
        )
    ...
    def test_plan_days(self):
        ...
        dt = self.calendar_jean.plan_days(
            2, datetime_tz(2018, 4, 10, 8, 0, 0, tzinfo=self.tz4)
        )
