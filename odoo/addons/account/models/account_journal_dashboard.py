...
class account_journal(models.Model):
        ...
        for index in range(0, len(query_results)):
            if query_results[index].get("aggr_date") != None:
                ...
                week_index = next(
                    ...
                    if weeks[i][0] <= aggr_date < weeks[i][1]
                )
    ...
    def _get_bar_graph_select_query(self):
        ...
        return (
            ...
            SELECT
                """
            + sign
            + """ + SUM(move.amount_residual_signed) AS total,
    ...
    def get_journal_dashboard_datas(self):
        ...
        if self.type in ("bank", "cash"):
            ...
            ) = self._get_journal_bank_account_balance(
                domain=[("parent_state", "=", "posted")]
            )
            (
                outstanding_pay_account_balance,
                nb_lines_outstanding_pay_account_balance,
            ) = self._get_journal_outstanding_payments_account_balance(
                domain=[("parent_state", "=", "posted")]
            )
        ...
        return {
            ...
            "to_check_balance": formatLang(
                self.env, to_check_balance, currency_obj=currency
            ),
            "number_to_reconcile": number_to_reconcile,
            "account_balance": formatLang(
                self.env, currency.round(bank_account_balance), currency_obj=currency
            ),
            "has_at_least_one_statement": has_at_least_one_statement,
            "nb_lines_bank_account_balance": nb_lines_bank_account_balance,
            "outstanding_pay_account_balance": formatLang(
                self.env,
            ...
            "last_balance": formatLang(
                self.env, currency.round(last_balance) + 0.0, currency_obj=currency
            ),
            "number_draft": number_draft,
            "number_waiting": number_waiting,
            "number_late": number_late,
            "sum_draft": formatLang(
                self.env, currency.round(sum_draft) + 0.0, currency_obj=currency
            ),
            "sum_waiting": formatLang(
                self.env, currency.round(sum_waiting) + 0.0, currency_obj=currency
            ),
            "sum_late": formatLang(
                self.env, currency.round(sum_late) + 0.0, currency_obj=currency
            ),
