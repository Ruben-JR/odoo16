...
DAY_SELECT_SELECTION_NO_LAST = tuple(
    zip(DAY_SELECT_VALUES, (str(i) for i in range(1, 29)))
)
...
class AccrualPlanLevel(models.Model):
    ...
    accrual_plan_id = fields.Many2one(
        "hr.leave.accrual.plan", "Accrual Plan", required=True
    )
    ...
    added_value_type = fields.Selection(
        [("days", "Days"), ("hours", "Hours")], default="days", required=True
    )
    ...
    second_month_day_display = fields.Selection(
        _get_selection_days,
        compute="_compute_days_display",
        inverse="_inverse_second_month_day_display",
    )
    ...
    yearly_day_display = fields.Selection(
        _get_selection_days,
        compute="_compute_days_display",
        inverse="_inverse_yearly_day_display",
    )
    ...
    action_with_unused_accruals = fields.Selection(
        [("postponed", "Transferred to the next year"), ("lost", "Lost")],
    ...
    @api.depends(
        "first_day", "second_day", "first_month_day", "second_month_day", "yearly_day"
    )
    def _compute_days_display(self):
        ...
        for level in self:
            ...
            level.first_month_day_display = days_select[
                min(level.first_month_day - 1, 28)
            ][0]
            level.second_month_day_display = days_select[
                min(level.second_month_day - 1, 28)
            ][0]
    ...
    def _inverse_first_month_day_display(self):
        for level in self:
            if level.first_month_day_display == "last":
                ...
                level.first_month_day = (
                    DAY_SELECT_VALUES.index(level.first_month_day_display) + 1
                )
    ...
    def _inverse_second_month_day_display(self):
        for level in self:
            if level.second_month_day_display == "last":
                ...
                level.second_month_day = (
                    DAY_SELECT_VALUES.index(level.second_month_day_display) + 1
                )
    ...
    def _get_next_date(self, last_call):
        ...
        if self.frequency == "daily":
            ...
            first_date = last_call + relativedelta(
                month=first_month, day=self.first_month_day
            )
            second_date = last_call + relativedelta(
                month=second_month, day=self.second_month_day
            )
            if last_call < first_date:
                return first_date
            elif last_call < second_date:
                ...
                return last_call + relativedelta(
                    years=1, month=first_month, day=self.first_month_day
                )
            ...
            if last_call < date:
                ...
                return last_call + relativedelta(
                    years=1, month=month, day=self.yearly_day
                )
        else:
            return False
    ...
    def _get_previous_date(self, last_call):
        ...
        if self.frequency == "daily":
            ...
            if last_call >= second_date:
                ...
                return last_call + relativedelta(
                    years=-1, month=second_month, day=self.second_month_day
                )
        elif self.frequency == "yearly":
            month = MONTHS.index(self.yearly_month) + 1
