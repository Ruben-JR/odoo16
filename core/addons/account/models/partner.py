...
class AccountFiscalPosition(models.Model):
    ...
    tax_ids = fields.One2many(
        "account.fiscal.position.tax", "position_id", string="Tax Mapping", copy=True
    )
    note = fields.Html(
        "Notes",
        translate=True,
    ...
    def _compute_foreign_vat_header_mode(self):
        for record in self:
            ...
            if self.env["account.tax"].search(
                [("country_id", "=", record.country_id.id)], limit=1
            ):
                record.foreign_vat_header_mode = None
            elif self.env["account.tax.template"].search(
                [("chart_template_id.country_id", "=", record.country_id.id)], limit=1
            ):
    ...
    def _check_zip(self):
        for position in self:
            if (
                position.zip_from
                and position.zip_to
                and position.zip_from > position.zip_to
            ):
            ...
            if record.foreign_vat:
                if record.country_id == record.company_id.account_fiscal_country_id:
                    if record.foreign_vat == record.company_id.vat:
                        raise ValidationError(
                            _(
                                "You cannot create a fiscal position within your fiscal country with the same VAT number as the main one set on your company."
                            )
                        )
                    ...
                    if not record.state_ids:
                        if record.company_id.account_fiscal_country_id.state_ids:
                            ...
                            raise ValidationError(
                                _(
                                    "You cannot create a fiscal position with a foreign VAT within your fiscal country."
                                )
                            )
                ...
                if similar_fpos_count:
                    raise ValidationError(
                        _(
                            "A fiscal position with a foreign VAT already exists in this region."
                        )
                    )
    ...
    def _get_fiscal_position(self, partner, delivery=None):
        ...
        fp = self._get_fpos_by_region(
            delivery.country_id.id, delivery.state_id.id, delivery.zip, vat_required
        )

        # Then if VAT required found no match, try positions that do not require it
        if not fp and vat_required:
            fp = self._get_fpos_by_region(
                delivery.country_id.id, delivery.state_id.id, delivery.zip, False
            )
...
class AccountFiscalPositionTax(models.Model):
    ...
    tax_src_id = fields.Many2one(
        "account.tax", string="Tax on Product", required=True, check_company=True
    )
    tax_dest_id = fields.Many2one(
        "account.tax", string="Tax to Apply", check_company=True
    )
...
class ResPartner(models.Model):
    ...
    def mark_as_reconciled(self):
        ...
        return self.sudo().write(
            {"last_time_entries_checked": time.strftime(DEFAULT_SERVER_DATETIME_FORMAT)}
        )
    ...
    invoice_warn = fields.Selection(
        WARNING_MESSAGE, "Invoice", help=WARNING_HELP, default="no-message"
    )
    ...
    def _compute_bank_count(self):
        ...
        mapped_data = dict(
            [(bank["partner_id"][0], bank["partner_id_count"]) for bank in bank_data]
        )
        for partner in self:
    ...
    def _inverse_use_partner_credit_limit(self):
        for partner in self:
            if not partner.use_partner_credit_limit:
                partner.credit_limit = self.env["ir.property"]._get(
