...
class Lead(models.Model):
    ...
    recurring_revenue = fields.Monetary(
        "Recurring Revenues", currency_field="company_currency", tracking=True
    )
    ...
    partner_is_blacklisted = fields.Boolean(
        "Partner is blacklisted", related="partner_id.is_blacklisted", readonly=True
    )
    ...
    function = fields.Char(
        "Job Position", compute="_compute_function", readonly=False, store=True
    )
    ...
    mobile = fields.Char(
        "Mobile", compute="_compute_mobile", readonly=False, store=True
    )
    ...
    # Address fields
    street = fields.Char(
        "Street", compute="_compute_partner_address_values", readonly=False, store=True
    )
    street2 = fields.Char(
        "Street2", compute="_compute_partner_address_values", readonly=False, store=True
    )
    zip = fields.Char(
        "Zip",
        change_default=True,
    ...
    city = fields.Char(
        "City", compute="_compute_partner_address_values", readonly=False, store=True
    )
    ...
    is_automated_probability = fields.Boolean(
        "Is automated probability?", compute="_compute_is_automated_probability"
    )
    # Won/Lost
    ...
    calendar_event_count = fields.Integer(
        "# Meetings", compute="_compute_calendar_event_count"
    )
    ...
    def _compute_team_id(self):
        ...
        for lead in self:
            ...
            if not lead.user_id:
                continue
            user = lead.user_id
            if lead.team_id and user in (
                lead.team_id.member_ids | lead.team_id.user_id
            ):
                continue
            team_domain = (
                [("use_leads", "=", True)]
        ...
        for lead in self:
            ...
            if proposal:
                ...
                # unless customer has a company
                    ...
                    and (not lead.partner_id or lead.partner_id.company_id != proposal)
                ):
                    proposal = False
    ...
    def _compute_lang_id(self):
        ...
        if lang_codes:
            lang_id_by_code = dict(
                (code, self.env["res.lang"]._lang_get_id(code)) for code in lang_codes
            )
        else:
            lang_id_by_code = {}
    ...
    def _compute_phone_state(self):
        for lead in self:
            ...
            if lead.phone:
                ...
                try:
                    if phone_validation.phone_parse(
                        lead.phone, country_code
                    ):  # otherwise library not installed
    ...
    def _compute_calendar_event_count(self):
        if self.ids:
            ...
            mapped_data = {
                m["opportunity_id"][0]: m["opportunity_id_count"] for m in meeting_data
            }
        else:
            mapped_data = dict()
        for lead in self:
    ...
    def _compute_potential_lead_duplicates(self):
        ...
        for lead in self:
            ...
            email_search = iap_tools.mail_prepare_for_domain_search(
                lead.email_from, min_email_length=MIN_EMAIL_LENGTH
            )
            ...
            if lead.partner_name and len(lead.partner_name) >= MIN_NAME_LENGTH:
                duplicate_lead_ids |= return_if_relevant(
                    "crm.lead",
                    common_lead_domain + [("partner_name", "ilike", lead.partner_name)],
                )
            if lead.contact_name and len(lead.contact_name) >= MIN_NAME_LENGTH:
                duplicate_lead_ids |= return_if_relevant(
                    "crm.lead",
                    common_lead_domain + [("contact_name", "ilike", lead.contact_name)],
                )
                ...
                duplicate_lead_ids |= lead.with_context(active_test=False).search(
                    ...
                    + [
                        (
                            "partner_id",
                            "child_of",
                            lead.partner_id.commercial_partner_id.id,
                        )
                    ]
                )
            if lead.phone and len(lead.phone) >= MIN_PHONE_LENGTH:
                duplicate_lead_ids |= return_if_relevant(
                    "crm.lead",
                    common_lead_domain + [("phone_mobile_search", "ilike", lead.phone)],
                )
    ...
    def _onchange_phone_validation(self):
        if self.phone:
            self.phone = (
                self.phone_get_sanitized_number(
                    number_fname="phone", force_format="INTERNATIONAL"
                )
                or self.phone
            )
    ...
    def _onchange_mobile_validation(self):
        if self.mobile:
            self.mobile = (
                self.phone_get_sanitized_number(
                    number_fname="mobile", force_format="INTERNATIONAL"
                )
                or self.mobile
            )
    ...
    def _get_partner_phone_update(self):
        ...
        if self.partner_id and self.phone != self.partner_id.phone:
            lead_phone_formatted = (
                self.phone_get_sanitized_number(number_fname="phone")
                or self.phone
                or False
            )
            partner_phone_formatted = (
                self.partner_id.phone_get_sanitized_number(number_fname="phone")
    ...
    def search(self, args, offset=0, limit=None, order=None, count=False):
        ...
        # the earliest deadline per lead is kept.
        activity_asc = any(
            "my_activity_date_deadline asc" in item for item in order_items
        )
        ...
        my_lead_mapping = dict(
            (item["res_id"], item["date_deadline"]) for item in my_lead_activities
        )
        my_lead_ids = list(my_lead_mapping.keys())
        my_lead_domain = expression.AND([[("id", "in", my_lead_ids)], args])
        my_lead_order = ", ".join(
            item for item in order_items if "my_activity_date_deadline" not in item
        )

        # Search leads linked to those activities and order them. See docstring
        # of this method for more details.
        search_res = super(Lead, self).search(
            my_lead_domain, offset=0, limit=None, order=my_lead_order, count=count
        )
        my_lead_ids_ordered = sorted(
            search_res.ids,
            key=lambda lead_id: my_lead_mapping[lead_id],
            reverse=not activity_asc,
        )
        # keep only requested window (offset + limit, or offset+)
        my_lead_ids_keep = (
            my_lead_ids_ordered[offset : (offset + limit)]
            if limit
            else my_lead_ids_ordered[offset:]
        )
        # keep list of already skipped lead ids to exclude them from future search
        my_lead_ids_skip = (
            my_lead_ids_ordered[: (offset + limit)] if limit else my_lead_ids_ordered
        )
        ...
        for lead in self:
            if "stage_id" in vals:
                if vals["stage_id"] in won_stage_ids:
                    ...
                    lead.stage_id.id in won_stage_ids and lead.active
                ):  # a lead can be lost at won_stage
                    leads_leave_won += lead
            if "active" in vals:
                if not vals["active"] and lead.active:  # archive lead
                    if (
                        lead.stage_id.id in won_stage_ids
                        and lead not in leads_leave_won
                    ):
                        leads_leave_won += lead
                    leads_reach_lost += lead
    ...
    def copy(self, default=None):
        ...
        default = default or {}
        default["date_open"] = (
            self.env.cr.now() if self.type == "opportunity" else False
        )
    ...
    def _get_rainbowman_message(self):
        ...
        """
        self.env.cr.execute(
            query, {"user_id": self.user_id.id, "team_id": self.team_id.id}
        )
    ...
    def _get_opportunity_meeting_view_parameters(self):
        ...
        relevant_meeting_dts = (
            unfinished_meeting_dts if unfinished_meeting_dts else meeting_dts
        )
    ...
    def get_empty_list_help(self, help):
        ...
        return (
            '<p class="o_view_nocontent_smiling_face">%s</p><p class="oe_view_nocontent_alias">%s</p>'
            % (help_title, sub_title)
        )
    ...
    def _merge_data(self, fnames=None):
        ...
        for field_name in fnames:
            ...
            if fcallable and callable(fcallable):
                ...
                data[field_name] = _get_first_not_null(
                    field_name, self
    ...
    def _merge_get_fields_specific(self):
        return {
            ...
            "type": lambda fname, leads: "opportunity"
            if any(lead.type == "opportunity" for lead in leads)
            else "lead",
            "priority": lambda fname, leads: max(leads.mapped("priority"))
            if leads
            else False,
            "tag_ids": lambda fname, leads: leads.mapped("tag_ids"),
            "lost_reason_id": lambda fname, leads: False
            if leads and leads[0].probability
            else next(
                (lead.lost_reason_id for lead in leads if lead.lost_reason_id), False
            ),
        }
    ...
    def _get_lead_duplicates(self, partner=None, email=None, include_lost=False):
        ...
        for normalized_email in [
            tools.email_normalize(email) for email in tools.email_split(email)
        ]:
    ...
    def _find_matching_partner(self, email_only=False):
        ...
        if not partner and not email_only:
            ...
            for customer_potential_name in [
                self[field_name]
                for field_name in ["partner_name", "contact_name", "name"]
                if self[field_name]
            ]:
                partner = self.env["res.partner"].search(
                    [("name", "ilike", "%" + customer_potential_name + "%")], limit=1
                )
                if partner:
                    break
    ...
    def _create_customer(self):
        ...
        if not contact_name:
            contact_name = (
                Partner._parse_partner_name(self.email_from)[0]
                if self.email_from
                else False
            )
    ...
    def _notify_get_recipients_groups(self, msg_vals=None):
        ...
        self.ensure_one()
        if self.type == "lead":
            convert_action = self._notify_get_action_link(
                "controller", controller="/lead/convert", **local_msg_vals
            )
            salesman_actions = [
                {"url": convert_action, "title": _("Convert to opportunity")}
            ]
        else:
            won_action = self._notify_get_action_link(
                "controller", controller="/lead/case_mark_won", **local_msg_vals
            )
            lost_action = self._notify_get_action_link(
                "controller", controller="/lead/case_mark_lost", **local_msg_vals
            )
            salesman_actions = [
        ...
        if self.team_id:
            custom_params = dict(
                local_msg_vals, res_id=self.team_id.id, model=self.team_id._name
            )
        ...
        new_group = (
            ...
            and salesman_group_id in pdata["groups"],
    ...
    def _message_get_default_recipients(self):
        return {
            r.id: {"partner_ids": [], "email_to": r.email_normalized, "email_cc": False}
            for r in self
        }
    ...
    def _message_get_suggested_recipients(self):
        ...
        try:
            for lead in self:
                ...
                lang_code = (
                    lead.lang_code
                    if lead.lang_code and self.env["res.lang"]._lang_get(lead.lang_code)
                    else None
                )
                if lead.partner_id:
    ...
    def _message_partner_info_from_emails(self, emails, link_mail=False):
        result = super(Lead, self)._message_partner_info_from_emails(
        ...
        for partner_info in result:
            if not partner_info.get("partner_id") and (
                ...
                if (
                    email
                    and self.email_from
                    and email.lower() == self.email_from.lower()
                ):
                    partner_info["full_name"] = tools.formataddr(
                        (self.contact_name or self.partner_name, email)
                    )
                    break
        return result
    ...
    def _pls_get_naive_bayes_probabilities(self, batch_mode=False):
        ...
        result = dict(
            (
                team_id,
                dict(
                    (field, dict(won_total=0, lost_total=0)) for field in leads_fields
                ),
            )
            for team_id in frequency_team_ids
        )
        result[-1] = dict(
            (field, dict(won_total=0, lost_total=0)) for field in leads_fields
        )
        ...
        for lead_id, lead_values in leads_values_dict.items():
            ...
            lead_probabilities[lead_id] = round(
                100 * s_lead_won / (s_lead_won + s_lead_lost), 2
            )
        return lead_probabilities
    ...
    def _update_automated_probabilities(self):
        ...
        for probability, probability_lead_ids in probability_leads.items():
            for lead_ids_current in tools.split_every(
                PLS_UPDATE_BATCH_STEP, probability_lead_ids
            ):
                transactions_count += 1
                try:
                    self.env.cr.execute(
                        update_sql, (probability, probability, tuple(lead_ids_current))
                    )
    ...
    def _pls_prepare_update_frequency_table(self, rebuild=False, target_state=False):
        ...
        if (
            rebuild
        ):  # rebuild will treat every closed lead in DB, increment will treat current ongoing leads
            pls_leads = self
        else:
            # Only treat leads created after the PLS start Date
            pls_leads = self.filtered(
                lambda lead: fields.Date.to_date(pls_start_date)
        ...
        if rebuild:  # rebuild is ok
            ...
            team_ids = self.env["crm.team"].with_context(active_test=False).search(
                ...
                0
            ]  # If team_id is unset, consider it as team 0
        else:  # increment
        ...
        leads_pls_fields = (
            set()
        )  # ensure to keep each field unique (can have multiple tag_id leads_values_dict)
        for lead_id, values in leads_values_dict.items():
            team_id = values.get(
                "team_id", 0
            )  # If team_id is unset, consider it as team 0
            ...
            for field, value in values["values"]:
                if (
                    field != "probability"
                ):  # was added to lead values in batch mode to know won/lost state, but is not a pls fields.
                ...
                if (
                    field == "tag_id"
                ):  # handle tag_id separatelly (as in One Shot rebuild mode)
        ...
        if (
            not rebuild
        ):  # there is no existing frequency in rebuild mode as they were all deleted.
                    ...
                    values_to_create.append(
                        {
                            ...
                            "team_id": team_id
                            if team_id
                            else None,  # team_id = 0 means no team_id
                        }
                    )
    ...
    def _pls_get_won_lost_total_count(self, team_results):
        ...
        first_stage_id = self.env["crm.stage"].search(
            [("team_id", "=", False)], order="sequence, id", limit=1
        )
    ...
    def _pls_prepare_frequencies(
        ...
        for values in lead_values:
            if (
                target_state
            ):  # ignore probability values if target state (as probability is the old value)
    ...
    def _pls_get_lead_pls_values(self, domain=[]):
        ...
        if domain:
            ...
            from_clause, where_clause, where_params = (
                self.env["crm.lead"]
            ...
            query = sql.SQL(query % (str_fields, from_clause, where_clause)).format(
                *args
            )
            ...
            for lead in lead_results:
                ...
                for field in pls_fields + [
                    "probability"
                ]:  # add probability as used in _pls_prepare_frequencies (needed in rebuild mode)
                    value = lead[field]
                    if (
                        field == "team_id"
                    ):  # ignore team_id as stored separately in leads_values_dict[lead_id][team_id]
                        continue
                    if (
                        value or field == "probability"
                    ):  # 0 is a correct value for probability
                        lead_values.append((field, value))
                    elif field in (
                        "email_state",
                        "phone_state",
                    ):  # As ORM reads 'None' as 'False', do the same here
