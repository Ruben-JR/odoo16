...
class TestLeadConvert(TestCrmCommon):
    ...
    def test_potential_duplicates_with_phone(self):
        ...
        expected = (
            base_lead + dup1_2 + dup1_3 + dup1_4
        )  # dup1_1 is shorter than lead -> not a dupe
        self.assertEqual(
            base_lead.duplicate_lead_ids,
            expected,
            "CRM: missing %s, extra %s"
            % (
                (expected - base_lead.duplicate_lead_ids).mapped("name"),
                (base_lead.duplicate_lead_ids - expected).mapped("name"),
            ),
        )
        expected = (
            base_lead + dup1_1 + dup1_2 + dup1_3
        )  # dup1_4 has mobile of customer, but no link with dup1_1
        self.assertEqual(
            dup1_1.duplicate_lead_ids,
            expected,
            "CRM: missing %s, extra %s"
            % (
                (expected - dup1_1.duplicate_lead_ids).mapped("name"),
                (dup1_1.duplicate_lead_ids - expected).mapped("name"),
            ),
        )
