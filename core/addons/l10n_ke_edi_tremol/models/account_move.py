...
class AccountMove(models.Model):
        ...
        for move in self:
            ...
            for line in self.invoice_line_ids.filtered(
                ...
                if not line.tax_ids or len(line.tax_ids) > 1:
                    move_errors.append(
                        _(
                            "On line %s, you must select one and only one tax.",
                            line.name,
                        )
                    )
                else:
                    if line.tax_ids.amount == 0 and not (
                        line.product_id
                        and line.product_id.l10n_ke_hsn_code
                        and line.product_id.l10n_ke_hsn_name
                    ):
                        move_errors.append(
                            _(
                                "On line %s, a product with a HS Code and HS Name must be selected, since the tax is 0%% or exempt.",
                                line.name,
                            )
                        )
            ...
            for tax in move.invoice_line_ids.tax_ids:
                if tax.amount not in (16, 8, 0):
                    move_errors.append(
                        _(
                            "Tax '%s' is used, but only taxes of 16%%, 8%%, 0%% or Exempt can be sent. Please reconfigure or change the tax.",
                            tax.name,
                        )
                    )
    ...
    def _l10n_ke_cu_open_invoice_message(self):
        ...
        invoice_elements = [
            ...
            self._l10n_ke_fmt(
                self.commercial_partner_id.vat, 14
            ),  # 14 Symbols for the client PIN number
            self._l10n_ke_fmt(
        ...
        if self.move_type == "out_refund":
            invoice_elements.append(
                self._l10n_ke_fmt(self.reversed_entry_id.l10n_ke_cu_invoice_number, 19)
            ),  # 19 symbols for related invoice number
        invoice_elements.append(
            re.sub("[^A-Za-z0-9 ]+", "", self.name)[-15:].ljust(15).encode("cp1251")
    ...
    def _l10n_ke_cu_lines_messages(self):
        ...
        for line in self.invoice_line_ids.filtered(
            ...
            price = (
                round(line.price_total / line.quantity * 100 / (100 - line.discount), 2)
                * currency_rate
            )
            ...
            if percentage in vat_class:
                ...
                report_line_ids = (
                    line.tax_ids.invoice_repartition_line_ids.tag_ids._get_related_tax_report_expressions().report_line_id.ids
                )
                try:
                    exempt_report_line = self.env.ref(
                        "l10n_ke.tax_report_line_exempt_sales"
                    )
                except ValueError:
                    raise UserError(
                        _(
            ...
            hscode = (
                re.sub("[^0-9.]+", "", line.product_id.l10n_ke_hsn_code)[:10]
                .ljust(10)
                .encode("cp1251")
            ...
            hsname = (
                self._l10n_ke_fmt(line.product_id.l10n_ke_hsn_name, 20)
                if letter not in ("A", "B")
                else b"".ljust(20)
            )
            line_data = b";".join(
                [
                    ...
                    str(price)[:13].encode(
                        "cp1251"
                    ),  # 1 to 13 symbols for article's price
