    ...
    def test_create_project(self):
        ...
        with self.sudo("manager-a"):
            ...
            self.assertEqual(
                ...
                "A newly created project should be in the current user company",
            )
        ...
        with self.sudo("manager-b"):
            with self.allow_companies([self.company_a.id, self.company_b.id]):
                ...
                self.assertEqual(
                    ...
                    self.project_company_a.analytic_account_id.company_id,
            ...
            with self.allow_companies([self.company_a.id, self.company_b.id]):
                    ...
                    self.env["project.task"].with_context(
                        {
                            ...
                            "default_parent_id": self.task_1.id,
                            "default_project_id": self.project_company_b.id,
                        }
                    )
                ) as task_form:
                ...
                with self.debug_mode():
                    with Form(self.task_2) as task_form:
                        task_form.name = (
                            "Test Task 2 becomes child of Task 1 (other company)"
                        )
                ...
                self.assertEqual(
                    task.company_id,
                    task.project_id.company_id,
                    "The company of the orphan subtask should be the one from its project.",
                )
