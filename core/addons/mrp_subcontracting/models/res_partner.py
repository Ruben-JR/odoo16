...
class ResPartner(models.Model):
    ...
    def _compute_bom_ids(self):
        results = self.env["mrp.bom"].read_group(
            [
                (
            ...
            ["ids:array_agg(id)", "subcontractor_ids"],
            ["subcontractor_ids"],
        )
        for partner in self:
            bom_ids = []
            for res in results:
                if (
                    partner.id == res["subcontractor_ids"][0]
                    or res["subcontractor_ids"][0] in partner.child_ids.ids
                ):
                    bom_ids += res["ids"]
    ...
    def _compute_production_ids(self):
        ...
        for partner in self:
            ...
            for res in results:
                if (
                    partner.id == res["subcontractor_id"][0]
                    or res["subcontractor_id"][0] in partner.child_ids.ids
                ):
                    production_ids += res["ids"]
            partner.production_ids = production_ids

    def _compute_picking_ids(self):
        results = self.env["stock.picking"].read_group(
            [
                (
        ...
        for partner in self:
            ...
            for res in results:
                if (
                    partner.id == res["partner_id"][0]
                    or res["partner_id"][0] in partner.child_ids.ids
                ):
                    picking_ids += res["ids"]
    ...
    def _compute_is_subcontractor(self):
        ...
        for partner in self:
            partner.is_subcontractor = partner.user_has_groups(
                "base.group_portal"
