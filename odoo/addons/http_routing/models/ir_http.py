...
def url_for(url_from, lang_code=None, no_rewrite=False):
    ...
    routing = getattr(
        request, "website_routing", None
    )  # not modular, but not overridable
...
def is_multilang_url(local_url, lang_url_codes=None):
    ...
    if not lang_url_codes:
        lang_url_codes = [
            url_code for _, url_code, *_ in request.env["res.lang"].get_available()
        ]
...
class IrHttp(models.AbstractModel):
    ...
    def get_frontend_session_info(self):
        ...
        translation_hash = request.env["ir.http"].get_web_translations_hash(
            modules, lang
        )
    ...
    def _match(cls, path):
        ...
        try:
            ...
            nearest_url_lang = cls.get_nearest_lang(
                request.env["res.lang"]._lang_get_code(url_lang_str)
            )
            cookie_lang = cls.get_nearest_lang(
                request.httprequest.cookies.get("frontend_lang")
            )
            ...
            request.lang = request.env["res.lang"]._lang_get(
                ...
                or cookie_lang
                or context_lang
                or default_lang._get_cached("code")
            )
    ...
    def _frontend_pre_dispatch(cls):
        ...
        if request.httprequest.cookies.get("frontend_lang") != request.lang._get_cached(
            "code"
        ):
            request.future_response.set_cookie(
                "frontend_lang", request.lang._get_cached("code")
            )
    ...
    def _handle_error(cls, exception):
        ...
        try:
            ...
            code, html = 418, request.env["ir.ui.view"]._render_template(
