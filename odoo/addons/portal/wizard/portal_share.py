...
class PortalShare(models.TransientModel):
    ...
    def default_get(self, fields):
        ...
        if result["res_model"] and result["res_id"]:
            ...
            result["share_link"] = record.get_base_url() + record._get_share_url(
                redirect=True
            )
        return result
        ...
        for rec in self:
            ...
            if rec.res_model:
                ...
                if isinstance(res_model, self.pool["portal.mixin"]) and rec.res_id:
                    ...
                    rec.share_link = record.get_base_url() + record._get_share_url(
                        redirect=True
                    )
    ...
    def _send_public_link(self, note, partners=None):
        ...
        for partner in partners:
            ...
            self.resource_ref.message_post_with_view(
                template,
                values={
                    "partner": partner,
                    "note": self.note,
                ...
                subject=_(
                    "You are invited to access %s", self.resource_ref.display_name
                ),
                subtype_id=note.id,
