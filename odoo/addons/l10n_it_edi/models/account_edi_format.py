...
class AccountEdiFormat(models.Model):
    ...
    def _l10n_it_edi_check_ordinary_invoice_configuration(self, invoice):
        ...
        if not is_self_invoice and not seller.l10n_it_codice_fiscale:
            errors.append(
                _("%s must have a codice fiscale number", seller.display_name)
            )
        ...
        if not seller.zip:
            ...
            errors.append(
                _("%s must have a post code of length 5.", seller.display_name)
            )
        if not seller.city:
        ...
        if (
            not is_self_invoice
            and seller.l10n_it_has_tax_representative
            and not seller.l10n_it_tax_representative_partner_id.vat
        ):
            errors.append(
                _(
                    "Tax representative partner %s of %s must have a tax number.",
        ...
        if (
            not buyer.vat
            and not buyer.l10n_it_codice_fiscale
            and buyer.country_id.code == "IT"
        ):
            errors.append(
                _(
                    "The buyer, %s, or his company must have a VAT number and/or a tax code (Codice Fiscale).",
                    buyer.display_name,
                )
            )
        ...
        if is_self_invoice and not buyer.partner_id.l10n_it_pa_index:
            errors.append(
                _(
                    "Vendor bills sent as self-invoices to the SdI require a valid PA Index (Codice Destinatario) on the company's contact."
                )
            )

        for tax_line in invoice.line_ids.filtered(lambda line: line.tax_line_id):
            if (
                not tax_line.tax_line_id.l10n_it_kind_exoneration
                and tax_line.tax_line_id.amount == 0
            ):
                errors.append(
                    _(
    ...
    def _l10n_it_edi_check_taxes_configuration(self, invoice):
        ...
        for invoice_line in invoice.invoice_line_ids.filtered(
            ...
            if len(invoice_line.tax_ids) != 1:
                errors.append(
                    _(
                        "In line %s, you must select one and only one tax.",
                        invoice_line.name,
                    )
                )
        return errors
    ...
    def _l10n_it_edi_check_simplified_invoice_configuration(self, invoice):
        return (
            []
            if self._l10n_it_edi_is_simplified(invoice)
            else self._l10n_it_edi_check_buyer_invoice_configuration(invoice)
        )
    ...
    def _l10n_it_edi_check_buyer_invoice_configuration(self, invoice):
        ...
        if not buyer.zip:
            ...
            errors.append(
                _("%s must have a post code of length 5.", buyer.display_name)
            )
        if not buyer.city:
    ...
    def _l10n_it_get_invoice_features_for_document_type_selection(self, invoice):
        ...
        return {
            ...
            "partner_in_eu": self._l10n_it_edi_partner_in_eu(
            ...
            and self._l10n_it_goods_in_italy(invoice),
        }
    ...
    def _l10n_it_get_document_type(self, invoice):
        ...
        invoice_features = (
            self._l10n_it_get_invoice_features_for_document_type_selection(invoice)
        )
        for (
            code,
            document_type_features,
        ) in self._l10n_it_document_type_mapping().items():
            comparisons = []
    ...
    def _l10n_it_is_simplified_document_type(self, document_type):
        return (
            self._l10n_it_document_type_mapping()
            .get(document_type, {})
            .get("simplified", False)
        )
    ...
    def _receive_fattura_pa(self, proxy_user):
        ...
        for id_transaction, fattura in res.items():
            if self._save_incoming_attachment_fattura_pa(
                proxy_user, id_transaction, fattura["filename"], fattura["key"]
            ):
    ...
    def _save_incoming_attachment_fattura_pa(
        self, proxy_user, id_transaction, filename, key
    ):
        ...
        if self.env["ir.attachment"].search(
            [("name", "=", filename), ("res_model", "=", "account.move")], limit=1
        ):
    ...
    def _check_filename_is_fattura_pa(self, filename):
        return re.search(
            "[A-Z]{2}[A-Za-z0-9]{2,28}_[A-Za-z0-9]{0,5}.((?i:xml.p7m|xml))", filename
        )

    def _is_fattura_pa(self, filename, tree=None):
        return self.code == "fattura_pa" and self._check_filename_is_fattura_pa(
            filename
        )
        ...
        if self._is_fattura_pa(filename, tree):
            if len(tree.xpath("//FatturaElettronicaBody")) > 1:
                invoice.message_post(
                    body="The attachment contains multiple invoices, this invoice was not updated from it.",
    ...
    def _create_invoice_from_binary(self, filename, content, extension):
        ...
        if extension.lower() == ".xml.p7m" and self._is_fattura_pa(filename):
            ...
            if decoded_content is not None:
                return self._import_fattura_pa(
                    decoded_content, self.env["account.move"]
                )
    ...
    def _update_invoice_from_binary(self, filename, content, extension, invoice):
        ...
        return super()._update_invoice_from_binary(
            filename, content, extension, invoice
        )
    ...
    def _convert_date_from_xml(self, xsdate_str):
        ...
        xsdate_pattern = (
            r"^-?(?P<date>-?\d{4}-\d{2}-\d{2})(?P<tz>[zZ]|[+-]\d{2}:\d{2})?$"
        )
        try:
            match = re.match(xsdate_pattern, xsdate_str)
            converted_date = datetime.strptime(
                match.group("date"), DEFAULT_FACTUR_ITALIAN_DATE_FORMAT
            ).date()
        except Exception:
    ...
    def _l10n_it_edi_search_tax_for_import(
        self, company, percentage, extra_domain=None
    ):
    ...
    def _import_fattura_pa(self, tree, invoice):
        ...
        for body_tree in tree.xpath("//FatturaElettronicaBody"):
            ...
            invoice_ctx = invoice.with_company(company).with_context(
            ...
            with invoice_ctx._get_edi_creation() as invoice_form:
                ...
                extra_info, message_to_log = self._l10n_it_edi_get_extra_info(
                    company, document_type, body_tree
                )
                ...
                for element in elements:
                    invoice_form.narration = "%s%s<br/>" % (
                        invoice_form.narration or "",
                        element.text,
                    )
                ...
                if invoice_form.move_type not in ("out_invoice", "in_refund"):
                    ...
                    if elements:
                        if (
                            ...
                            and invoice_form.partner_id.commercial_partner_id
                        ):
                            bank = self.env["res.partner.bank"].search(
                                [
                                    ...
                                    (
                                        "company_id",
                                        "in",
                                        [invoice_form.company_id.id, False],
                                    ),
                                ],
                                order="company_id",
                                limit=1,
                ...
                for element in elements or []:
                    ...
                    if invoice_line_form:
                        message_to_log += self._import_fattura_pa_line(
                            element, invoice_line_form, extra_info
                        )
                ...
                if discount_elements:
                    ...
                    for discount_element in discount_elements:
                        ...
                        if discount_amount:
                            discounted_amount -= discount_sign * float(
                        ...
                        if discount_percentage:
                            discounted_amount *= (
                                1
                                - discount_sign
                                * float(discount_percentage[0].text)
                                / 100
                            )
                    ...
                    with invoice_form.invoice_line_ids.new() as invoice_line_global_discount:
                        ...
                        invoice_line_global_discount.name = (
                            "SCONTO" if general_discount < 0 else "MAGGIORAZIONE"
                        )
    ...
    def _import_fattura_pa_line(self, element, invoice_line_form, extra_info=None):
        ...
        if percentage is not None:
            ...
            conditions = (
                ...
                and [("l10n_it_kind_exoneration", "=", l10n_it_kind_exoneration)]
                or [("l10n_it_has_exoneration", "=", False)]
            )
            tax = self._l10n_it_edi_search_tax_for_import(
                company, percentage, conditions
            )
            if tax:
        ...
        if discount_elements:
            ...
            if discount_percentage and len(discount_elements) == 1:
                ...
                if discount_type and discount_type[0].text == "MG":
                    discount_sign = -1
                invoice_line_form.discount = discount_sign * float(
                    discount_percentage[0].text
                )
            # Discounts in cascade summarized in 1 percentage
            else:
                total = float(element.xpath(".//PrezzoTotale")[0].text)
                discount = 100 - (100 * total) / (
                    invoice_line_form.quantity * invoice_line_form.price_unit
                )
    ...
    def _get_move_applicability(self, move):
        ...
        is_it_purchase_document = (
            self._l10n_it_edi_is_self_invoice(move) and move.is_purchase_document()
        )
        if move.country_code == "IT" and (
            move.is_sale_document() or is_it_purchase_document
        ):
            return {
                "post": self._post_fattura_pa,
                "post_batching": lambda move: (
                    move.move_type,
    ...
    def _l10n_it_edi_export_invoice_as_xml(self, invoice):
        ...
        if not self._l10n_it_is_simplified_document_type(
            template_values["document_type"]
        ):
            content = self.env["ir.qweb"]._render(
                "l10n_it_edi.account_invoice_it_FatturaPA_export", template_values
            )
        else:
            content = self.env["ir.qweb"]._render(
                "l10n_it_edi.account_invoice_it_simplified_FatturaPA_export",
    ...
    def _l10n_it_post_invoices_step_1(self, invoices):
        ...
        for invoice in invoices:
            xml = "<?xml version='1.0' encoding='UTF-8'?>" + str(
                self._l10n_it_edi_export_invoice_as_xml(invoice)
            )
        ...
        if proxy_user._get_demo_state() == "demo":
            responses = {
                i["data"]["filename"]: {"id_transaction": "demo"}
                for i in to_send.values()
            }
        else:
            try:
                ...
                return {
                    invoice: {"error": e.message, "blocking_level": "error"}
                    for invoice in invoices
                }
        ...
        for filename, response in responses.items():
            ...
            if "id_transaction" in response:
                ...
                to_return[invoice].update(
                    {
                        "error": _(
                            "The invoice was sent to FatturaPA, but we are still awaiting a response. Click the link above to check for an update."
                        ),
                        "blocking_level": "info",
                    }
                )
    ...
    def _l10n_it_post_invoices_step_2(self, invoices):
        ...
        if proxy_user._get_demo_state() == "demo":
            ...
            return {
                invoice: {"attachment": invoice.l10n_it_edi_attachment_id}
                for invoice in invoices
            }
        else:
            try:
                responses = proxy_user._make_request(
                    proxy_user._get_server_url()
                    + "/api/l10n_it_edi/1/in/TrasmissioneFatture",
        ...
        for id_transaction, response in responses.items():
            ...
            if state == "awaiting_outcome":
                ...
                to_return[invoice] = {
                    "error": _(
                        "You are not allowed to check the status of this invoice."
                    ),
                    "blocking_level": "error",
                }
                ...
                if invoice._is_commercial_partner_pa():
                    to_return[invoice] = {
                        "error": _(
                            "The invoice has been succesfully transmitted. The addressee has 15 days to accept or reject it."
                        )
                    }
                else:
                    to_return[invoice] = {
            ...
            # about the notification itself, i.e. the error message in case of rejection.
                ...
                if (
                    not attachment_file
                ):  # It means there is no status update, so we can skip it
                    document = invoice.edi_document_ids.filtered(
                        lambda d: d.edi_format_id.code == "fattura_pa"
                    )
                    to_return[invoice] = {
                        "error": document.error,
                        "blocking_level": document.blocking_level,
                    }
                    continue
                ...
                if state == "notificaScarto":
                    ...
                    if outcome == "EC01":
                        ...
                        to_return[invoice] = {
                            "error": _("The invoice was refused by the addressee."),
                            "blocking_level": "error",
                        }
                    proxy_acks.append(id_transaction)
    ...
    def _get_proxy_identification(self, company):
        ...
        return self.env["res.partner"]._l10n_it_normalize_codice_fiscale(
    ...
    def _l10n_it_edi_upload(self, files, proxy_user):
        ...
        * error_level:   Info, warning, error.
        ...
        result = proxy_user._make_request(
            proxy_user._get_server_url() + "/api/l10n_it_edi/1/out/SdiRiceviFile",
            params={"files": files},
        )
        ...
        for filename in result.keys():
            if "error" in result[filename]:
                result[filename] = ERRORS.get(
                    ...
                    {"error": result[filename]["error"], "blocking_level": "error"},
                )
