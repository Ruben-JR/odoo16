...
class AccountBankStatementLine(models.Model):
    ...
    def _compute_running_balance(self):
        ...
        self.flush_model(
            ["internal_index", "date", "journal_id", "statement_id", "amount", "state"]
        )
    ...
    def create(self, vals_list):
        ...
        for i, st_line in enumerate(st_lines):
            ...
            self.env.remove_to_compute(
                st_line.move_id._fields["narration"], st_line.move_id
            )
    ...
    def read_group(
        ...
        for el in groupby:
            ...
            ) and "running_balance" in fields:
                show_running_balance = True
                break
        if show_running_balance:
            for group_line in result:
                group_line["running_balance"] = (
                    self.search(group_line.get("__domain"), limit=1).running_balance
                    or 0.0
                )
        return result
        ...
        if self.partner_name:
            ...
            for domain in domains:
                partner = self.env["res.partner"].search(
                    list(domain) + [("parent_id", "=", False)], limit=1
                )
                if partner:
                    return partner
