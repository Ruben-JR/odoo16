...
class WebsiteVisitorTests(MockVisitor, HttpCaseWithUserDemo):
    ...
    def test_merge_partner_with_visitor_both(self):
        ...
        # |    |      5013266 |    5013266 |
        self.assertTrue(
            visitor_demo.partner_id.id
            == int(visitor_demo.access_token)
            == self.partner_demo.id
        )
        self.assertTrue(
        ...
        self.env["website.track"].create(
            [
                {"visitor_id": visitor_demo.id, "url": "/demo"},
        ...
        self.env["base.partner.merge.automatic.wizard"]._merge(
            (self.partner_admin + self.partner_demo).ids, self.partner_admin
        )
        # Should be
        # | id | access_token | partner_id |
        # | -- | ------------ | ---------- |
        ...
        self.assertFalse(
            Visitor.search_count([("partner_id", "=", self.partner_demo.id)]),
