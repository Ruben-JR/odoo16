...
class WebsiteProfile(http.Controller):
    ...
    def _check_user_profile_access(self, user_id):
        ...
        if (
            user_sudo.karma == 0
            or not user_sudo.website_published
            or (
                user_sudo.id != request.session.uid
                and request.env.user.karma < request.website.karma_profile_min
            )
        ):
            return False
        return user_sudo
    ...
    @http.route(
        ["/profile/user/<int:user_id>"], type="http", auth="public", website=True
    )
    ...
    def view_user_profile_edition(self, **kwargs):
        ...
        if user_id and request.env.user.id != user_id and request.env.user._is_admin():
            ...
            values = self._prepare_user_values(
                searches=kwargs, user=user, is_public_user=False
            )
        else:
    ...
    @http.route(
        "/profile/user/save", type="http", auth="user", methods=["POST"], website=True
    )
    def save_edited_profile(self, **kwargs):
        ...
        whitelisted_values = {
            key: values[key] for key in user.SELF_WRITEABLE_FIELDS if key in values
        }
        user.write(whitelisted_values)
        if kwargs.get("url_param"):
            return request.redirect(
                "/profile/user/%d?%s" % (user.id, kwargs["url_param"])
            )
        else:
    ...
    @http.route(
        "/profile/ranks_badges", type="http", auth="public", website=True, sitemap=True
    )
        ...
        if user_count:
            ...
            if (
                ...
                current_user = User.sudo().search(
                    expression.AND([[("id", "=", my_user.id)], dom])
                )
                if current_user:
    ...
    def _get_position_map(self, position_domain, users, group_by):
        if group_by:
            ...
            position_map = dict(
                (user_data["user_id"], dict(user_data))
                for user_data in position_results
            )
        return position_map
    ...
    @http.route(
        "/profile/send_validation_email", type="json", auth="user", website=True
    )
    ...
    @http.route(
        "/profile/validate_email/close", type="json", auth="public", website=True
    )
