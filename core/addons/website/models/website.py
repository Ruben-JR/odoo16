...
class Website(models.Model):
    ...
    def _website_api_rpc(self, route, params):
        ...
        website_api_endpoint = IrConfigParameter.get_param(
            "website.website_api_endpoint", DEFAULT_ENDPOINT
        )
    ...
    def configurator_apply(self, **kwargs):
        ...
        if logo_attachment_id:
            ...
            not logo_attachment_id
            and company.logo
            and company.logo != company._get_logo()
        ):
        ...
        for page_code in requested_pages:
            configure_page(
                page_code, snippet_lists.get(page_code, []), pages_views, cta_data
            )
    ...
    def get_unique_key(self, string, template_module=False):
        ...
        while (
            ...
            .with_context(active_test=False)
            .sudo()
            .search([("key", "=", key_copy)] + domain_static)
        ):
            inc += 1
    ...
    def search_url_dependencies(self, res_model, res_ids):
        ...
        for model, _table, column, _translate in html_fields_attributes:
            ...
            if dependency_records:
                model_name = self.env["ir.model"]._display_name_for([model])[0][
                    "display_name"
                ]
                ...
                dependencies[model_name] += [
                    {
                        ...
                        "link": "website_url" in rec
                        and rec.website_url
                        or f"/web#id={rec.id}&view_type=form&model={model}",
    ...
    def viewref(self, view_id, raise_if_not_found=True):
        ...
        if isinstance(view_id, str):
            if "website_id" in self._context:
                domain = [("key", "=", view_id)] + self.env["website"].website_domain(
    ...
    def _enumerate_pages(self, query_string=None, force=False):
        ...
        for rule in router.iter_rules():
            if (
                ...
                and rule.endpoint.routing["sitemap"] is not True
            ):
    ...
    def _get_canonical_url_localized(self, lang, canonical_params):
        ...
        canonical_query_string = (
            f"?{urls.url_encode(canonical_params)}" if canonical_params else ""
        )
    ...
    def _get_canonical_url(self, canonical_params):
        ...
        return self._get_canonical_url_localized(
            lang=lang, canonical_params=canonical_params
        )
    ...
    def _disable_unused_snippets_assets(self):
        ...
        for (
            ...
            is_snippet_used = self._is_snippet_used(
                snippet_module,
                snippet_id,
                asset_version,
                asset_type,
    ...
    def _trigram_enumerate_words(self, search_details, search, limit):
        ...
        for search_detail in search_details:
            ...
            ids = {
                ...
                if row[1] and row[1] >= similarity_threshold
            }
