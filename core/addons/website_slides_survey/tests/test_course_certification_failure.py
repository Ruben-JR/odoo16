...
class TestCourseCertificationFailureFlow(TestSurveyCommon):
    def test_course_certification_failure_flow(self):
        ...
        self.assertNotIn(
            self.user_portal.partner_id,
            self.channel.partner_ids,
            "Portal user should have been kicked out of the course because they failed their last attempt",
        )
        ...
        self.assertIn(
            self.user_portal.partner_id,
            self.channel.partner_ids,
            "Portal user should be a member of the course once again",
        )
        new_slide_partner = self.slide_certification._action_set_viewed(
            self.user_portal.partner_id
        )
        self.slide_certification.with_user(
            self.user_portal
        ...
        self.assertEqual(
            1,
            len(
                new_slide_partner.user_input_ids.filtered(
        ...
        self.fill_in_answer(
            ...
            certification.question_ids,
            good_answers=True,
        )
        self.assertTrue(
            new_slide_partner.survey_scoring_success,
