...
class EventRegistration(models.Model):
    ...
    event_begin_date = fields.Datetime(
        string="Event Start Date", related="event_id.date_begin", readonly=True
    )
    event_end_date = fields.Datetime(
        string="Event End Date", related="event_id.date_end", readonly=True
    )
            ...
            if not registration.name and registration.partner_id:
                registration.name = (
                    registration._synchronize_partner_values(
                        registration.partner_id, fnames=["name"]
                    ).get("name")
                    or False
                )
    ...
    def _compute_email(self):
        for registration in self:
            if not registration.email and registration.partner_id:
                registration.email = (
                    registration._synchronize_partner_values(
                        registration.partner_id, fnames=["email"]
                    ).get("email")
                    or False
                )
    ...
    def _compute_phone(self):
        for registration in self:
            if not registration.phone and registration.partner_id:
                registration.phone = (
                    registration._synchronize_partner_values(
                        registration.partner_id, fnames=["phone"]
                    ).get("phone")
                    or False
                )
    ...
    def _compute_mobile(self):
        for registration in self:
            if not registration.mobile and registration.partner_id:
                registration.mobile = (
                    registration._synchronize_partner_values(
                        registration.partner_id, fnames=["mobile"]
                    ).get("mobile")
                    or False
                )
    ...
    def _synchronize_partner_values(self, partner, fnames=None):
        ...
        if partner:
            ...
            if contact_id:
                ...
                return dict(
                    (fname, contact[fname]) for fname in fnames if contact[fname]
                )
        return {}
    ...
    def name_get(self):
        ...
        for registration in self:
            if registration.partner_id.name:
                if (
                    registration.name
                    and registration.name != registration.partner_id.name
                ):
    ...
    def _message_get_suggested_recipients(self):
        ...
        if public_groups:
            public_users = (
                public_groups.sudo().with_context(active_test=False).mapped("users")
            )
        try:
            for attendee in self:
                is_public = (
                    attendee.sudo().with_context(active_test=False).partner_id.user_ids
                    in public_users
                    if public_users
                    else False
                )
                if attendee.partner_id and not is_public:
                    attendee._message_add_suggested_recipient(
                        recipients, partner=attendee.partner_id, reason=_("Customer")
                    )
                elif attendee.email:
                    attendee._message_add_suggested_recipient(
                        recipients, email=attendee.email, reason=_("Customer Email")
                    )
    ...
    def _message_get_default_recipients(self):
        ...
        return {
            r.id: {"partner_ids": [], "email_to": r.email, "email_cc": False}
            for r in self
        }
    ...
    def _message_post_after_hook(self, message, msg_vals):
        ...
        return super(EventRegistration, self)._message_post_after_hook(
            message, msg_vals
        )
    ...
    def _get_registration_summary(self):
        ...
        return {
            ...
            "company_name": self.event_id.company_id
            and self.event_id.company_id.name
            or False,
        }
