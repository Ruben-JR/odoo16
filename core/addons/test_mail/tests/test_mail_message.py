...
class TestMessageValues(TestMailCommon):
    ...
    def test_empty_message(self):
        ...
        record._message_update_content(
            message, "<p><br /></p>", attachment_ids=message.attachment_ids.ids
        )
    ...
    def test_mail_message_values_fromto_long_name(self):
        ...
        self.assertEqual(
            msg.reply_to,
            formataddr((test_record.name, reply_to_email)),
        ...
        self.assertEqual(
            msg.reply_to,
            formataddr((self.env.user.company_id.name, reply_to_email)),
    ...
    def test_mail_message_values_fromto_document_alias(self):
        ...
        self.assertIn(
            "-openerp-%d-mail.test" % self.alias_record.id, msg.message_id.split("@")[0]
        )
        reply_to_name = "%s %s" % (
    ...
    def test_mail_message_values_fromto_document_no_alias(self):
        ...
        self.assertIn(
            "-openerp-%d-mail.test.simple" % test_record.id,
            msg.message_id.split("@")[0],
        )
...
class TestMessageAccess(TestMailCommon):
    ...
    def setUpClass(cls):
        ...
        cls.user_employee_1 = mail_new_test_user(
            cls.env, login="tao", groups="base.group_user", name="Tao Lee"
        )
        cls.user_public = mail_new_test_user(
            cls.env, login="bert", groups="base.group_public", name="Bert Tartignole"
        )
        cls.user_portal = mail_new_test_user(
            cls.env, login="chell", groups="base.group_portal", name="Chell Gladys"
        )
    ...
    def test_mail_message_access_search(self):
        ...
        msg3 = self.env["mail.message"].create(
            {
                ...
                "subtype_id": False,
                "model": "mail.channel",
                "res_id": self.group_restricted_channel.id,
            }
        )
        ...
        messages = (
            self.env["mail.message"]
            .with_user(self.user_employee)
            .search([("subject", "like", "_ZTest"), ("body", "ilike", "A")])
        )
    ...
    def test_mail_message_access_read_crash_portal(self):
        with self.assertRaises(AccessError):
            self.message.with_user(self.user_portal).read(
    ...
    def test_mail_message_access_create_doc(self):
        ...
        Message.create(
            {"model": "mail.channel", "res_id": self.public_channel.id, "body": "Test"}
        )
