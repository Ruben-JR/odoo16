...
class MondialRelay(http.Controller):
    @http.route(
        ["/website_sale_delivery_mondialrelay/update_shipping"],
        type="json",
        auth="public",
        website=True,
    )
    def mondial_relay_update_shipping(self, **data):
        ...
        if order.carrier_id.country_ids:
            country_is_allowed = data["Pays"][
                :2
            ].upper() in order.carrier_id.country_ids.mapped(lambda c: c.code.upper())
        ...
        return {
            "address": request.env["ir.qweb"]._render(
                "website_sale.address_on_payment",
                {
                    "order": order,
...
class PaymentPortalMondialRelay(PaymentPortal):
    ...
    def shop_payment_transaction(self, *args, **kwargs):
        ...
        if (
            ...
            not order.partner_shipping_id.is_mondialrelay
            and order.carrier_id.is_mondialrelay
        ):
            raise ValidationError(
