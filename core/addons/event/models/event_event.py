...
class EventEvent(models.Model):
    ...
    kanban_state = fields.Selection(
        [("normal", "In Progress"), ("done", "Done"), ("blocked", "Blocked")],
        default="normal",
        copy=False,
    )
    ...
    def _compute_event_registrations_sold_out(self):
        ...
        for event in self:
            event.event_registrations_sold_out = (
                ...
                and all(ticket.is_sold_out for ticket in event.event_ticket_ids)
            )
    ...
    def _compute_event_mail_ids(self):
        ...
        for event in self:
            ...
            mails_to_remove = event.event_mail_ids.filtered(
                lambda mail: not (mail._origin.mail_done)
            ...
            if event.event_type_id.event_type_mail_ids:
                mails_to_keep_vals = {
                    mail._prepare_event_mail_values()
    ...
    def _get_mail_message_access(self, res_ids, operation, model_name=None):
        if (
            ...
            return "read"
        return super(EventEvent, self)._get_mail_message_access(
            res_ids, operation, model_name
        )
    ...
    def _get_ics_file(self):
        ...
        for event in self:
            ...
            cal_event.add("dtstart").value = event.date_begin.astimezone(
                pytz.timezone(event.date_tz)
            )
            cal_event.add("dtend").value = event.date_end.astimezone(
                pytz.timezone(event.date_tz)
            )
