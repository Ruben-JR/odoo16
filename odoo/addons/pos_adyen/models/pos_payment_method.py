...
class PosPaymentMethod(models.Model):
    ...
    adyen_latest_diagnosis = fields.Char(
        copy=False, groups="base.group_erp_manager"
    )  # used to determine if the terminal is still connected.
        ...
        for payment_method in self:
            ...
            if existing_payment_method:
                if existing_payment_method.company_id == payment_method.company_id:
                    ...
                    raise ValidationError(
                        _(
                            "Terminal %s is already used in company %s on payment method %s."
                        )
                        % (
                            payment_method.adyen_terminal_identifier,
    ...
    def proxy_adyen_request(self, data, operation=False):
        ...
        if (
            data["SaleToPOIRequest"]["MessageHeader"]["MessageCategory"] == "Payment"
