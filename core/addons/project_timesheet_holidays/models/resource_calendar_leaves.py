...
class ResourceCalendarLeaves(models.Model):
    ...
    def _work_time_per_day(self):
        ...
        leaves_read_group = self.env["resource.calendar.leaves"]._read_group(
            ...
            [
                "calendar_id",
                "ids:array_agg(id)",
                "resource_ids:array_agg(resource_id)",
                "min_date_from:min(date_from)",
                "max_date_to:max(date_to)",
            ],
            ["calendar_id"],
        )
        ...
        cal_attendance_intervals_dict = {
            res["calendar_id"][0]: {
                ...
                "resources": self.env["resource.resource"].browse(
                    res["resource_ids"]
                    if res["resource_ids"] and res["resource_ids"][0]
                    else []
                ),
    ...
    def _timesheet_create_lines(self):
        ...
        mapped_employee = {
            employee["resource_calendar_id"][0]: self.env["hr.employee"].browse(
                employee["ids"]
            )
        ...
        holidays_read_group = self.env["hr.leave"]._read_group(
            ...
            [
                "date_from_list:array_agg(date_from)",
                "date_to_list:array_agg(date_to)",
                "employee_id",
            ],
            ["employee_id"],
        )
        holidays_by_employee = {
            line["employee_id"][0]: [
                ...
                for date_from, date_to in zip(
                    line["date_from_list"], line["date_to_list"]
                )
            ]
        ...
        vals_list = []
        for leave in self:
            for employee in mapped_employee.get(
    ...
    def create(self, vals_list):
        ...
        results_with_leave_timesheet = results.filtered(
            lambda r: not r.resource_id.id
            and r.calendar_id.company_id.internal_project_id
            and r.calendar_id.company_id.leave_timesheet_task_id
        )
    ...
    def write(self, vals):
        ...
        if date_from or date_to or "calendar_id" in vals:
            global_time_off_updated = self.filtered(
                lambda r: (date_from is not None and r.date_from != date_from)
                or (date_to is not None and r.date_to != date_to)
                or (calendar_id is not None and r.calendar_id.id != calendar_id)
            )
