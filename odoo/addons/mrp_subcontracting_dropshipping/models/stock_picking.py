...
class StockPicking(models.Model):
    ...
    def _compute_is_dropship(self):
        dropship_subcontract_pickings = self.filtered(
            lambda p: p.location_dest_id.is_subcontracting_location
    ...
    def _action_done(self):
        ...
        for move in self.move_ids:
            if not (
                move.is_subcontract and move._is_dropshipped() and move.state == "done"
            ):
                continue
            ...
            if (
                float_compare(
                    diff, 0, precision_rounding=move.company_id.currency_id.rounding
                )
                <= 0
            ):
                continue
