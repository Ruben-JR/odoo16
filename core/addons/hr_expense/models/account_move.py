...
class AccountMove(models.Model):
    ...
    def is_purchase_document(self, include_receipts=False):
        return bool(
            include_receipts and self.sudo().expense_sheet_id
        ) or super().is_purchase_document(include_receipts)
    ...
    def _check_journal_move_type(self):
        return super(
            AccountMove, self.filtered(lambda x: not x.expense_sheet_id)
    ...
    def _compute_payment_state(self):
        company_paid = self.filtered(
            lambda m: m.expense_sheet_id.payment_mode == "company_account"
        )
            ...
            if move.expense_sheet_id:
                ...
                move.needed_terms = {
                    frozendict(
                        ...
                        "name": "",
                        "account_id": move.expense_sheet_id.expense_line_ids[
                            0
                        ]._get_expense_account_destination(),
                    }
                }
    ...
    def unlink(self):
        if self.expense_sheet_id:
            self.expense_sheet_id.state = "approve"
            self.expense_sheet_id.account_move_id = (
                False  # cannot change to delete='set null' in stable
            )
        return super().unlink()
