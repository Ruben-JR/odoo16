    ...
    def write(self, vals):
        ...
        for wiz in self:
            wiz.company_id.write(
                {
                    "fiscalyear_last_day": vals.get("fiscalyear_last_day")
                    or wiz.company_id.fiscalyear_last_day,
                    "fiscalyear_last_month": vals.get("fiscalyear_last_month")
    ...
    def _compute_linked_journal_id(self):
        for record in self:
            record.linked_journal_id = (
                ...
                or record.default_linked_journal_id()
            )

    def default_linked_journal_id(self):
        for journal_id in self.env["account.journal"].search(
        ...
        for record in self:
            ...
            if not selected_journal:
                new_journal_code = self.env[
                    "account.journal"
                ].get_next_bank_cash_default_code("bank", self.env.company)
