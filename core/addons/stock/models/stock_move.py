...
class StockMove(models.Model):
    ...
    group_id = fields.Many2one(
        "procurement.group", "Procurement Group", default=_default_group_id, index=True
    )
    ...
    move_line_nosuggest_ids = fields.One2many(
        "stock.move.line",
        "move_id",
        domain=["|", ("reserved_qty", "=", 0.0), ("qty_done", "!=", 0.0)],
    )
    ...
    restrict_partner_id = fields.Many2one("res.partner", "Owner ", check_company=True)
    ...
    show_details_visible = fields.Boolean(
        "Details Visible", compute="_compute_show_details_visible"
    )
    show_reserved_availability = fields.Boolean(
        "From Supplier", compute="_compute_show_reserved_availability"
    )
    product_type = fields.Selection(related="product_id.detailed_type", readonly=True)
    additional = fields.Boolean(
        "Whether the move was added after the picking's confirmation", default=False
    )
    is_locked = fields.Boolean(compute="_compute_is_locked", readonly=True)
    is_initial_demand_editable = fields.Boolean(
        "Is initial demand editable", compute="_compute_is_initial_demand_editable"
    )
    is_quantity_done_editable = fields.Boolean(
        "Is quantity done editable", compute="_compute_is_quantity_done_editable"
    )
    reference = fields.Char(
        compute="_compute_reference", string="Reference", store=True
    )
    move_lines_count = fields.Integer(compute="_compute_move_lines_count")
    package_level_id = fields.Many2one(
        "stock.package_level", "Package Level", check_company=True, copy=False
    )
    ...
    def _compute_display_assign_serial(self):
        for move in self:
            move.display_assign_serial = (
                ...
                and move.state in ("partially_available", "assigned", "confirmed")
    ...
    def _compute_display_clear_serial(self):
        ...
        for move in self:
            move.display_clear_serial = (
                move.display_assign_serial and move._get_move_lines()
            )
    ...
    def _compute_show_details_visible(self):
        ...
        multi_locations_enabled = self.user_has_groups(
        ...
        for move in self:
            if not move.product_id:
                ...
                move.show_details_visible = (
                    (
                        (consignment_enabled and move.picking_code != "incoming")
                        or show_details_visible
                        or move.has_tracking != "none"
                    )
                    and move._show_details_in_draft()
                    and move.show_operations is False
                )
    ...
    def _quantity_done_compute(self):
        ...
        if not any(self._ids):
            ...
            data = self.env["stock.move.line"]._read_group(
                ...
                ["move_id", "product_uom_id", "qty_done"],
            ...
            for move in self:
                ...
                move.quantity_done = sum(
                    self.env["uom.uom"]
                    .browse(line_uom_id)
                    ._compute_quantity(qty, uom, round=False)
                    for line_uom_id, qty in rec.get(
    ...
    def _quantity_done_set(self):
        def _process_decrease(move, quantity):
            ...
            res_mls = move._get_move_lines().sorted(
                lambda ml: float_is_zero(
                    ml.reserved_uom_qty, precision_rounding=ml.product_uom_id.rounding
                ),
                reverse=True,
            )
            ...
            for ml in res_mls:
                ...
                if move.picking_id.immediate_transfer:
                    ...
                    qty_unreserved = min(
                        qty_to_unreserve, ml.reserved_qty - ml.qty_done
                    )
        ...
        def _process_increase(move, quantity):
            ...
            if len(moves) > 1:
                return
            if move.reserved_availability < move.quantity_done and move.state not in [
                "done",
                "cancel",
            ]:
...
Please change the quantity done or the rounding precision of your unit of measure.""",
    ...
    def _multi_line_quantity_done_set(self, quantity_done):
        ...
        for move_line in move_lines:
            ml_quantity_done += move_line.product_uom_id._compute_quantity(
                move_line.qty_done, self.product_uom, round=False
            )
        if (
            float_compare(
    ...
    def _compute_reserved_availability(self):
        ...
        if not any(self._ids):
            ...
            # compute
            result = {
                data["move_id"][0]: data["reserved_qty"]
                for data in self.env["stock.move.line"]._read_group(
                    [("move_id", "in", self.ids)],
                    ["move_id", "reserved_qty"],
                    ["move_id"],
                )
            }
            for move in self:
    ...
    def _compute_lot_ids(self):
        ...
        for domain in [domain_nosuggest, domain_suggest]:
            lots_by_move_id = self.env["stock.move.line"]._read_group(
                domain,
                ["move_id", "lot_ids:array_agg(lot_id)"],
                ["move_id"],
            )
            lots_by_move_id_list.append(
                {
                    by_move["move_id"][0]: by_move["lot_ids"]
                    for by_move in lots_by_move_id
                }
            )
        for move in self:
            move.lot_ids = lots_by_move_id_list[
                0 if move.picking_type_id.show_reserved else 1
            ].get(move._origin.id, [])
    ...
    def _check_uom(self):
        moves_error = self.filtered(
            lambda move: move.product_id.uom_id.category_id
        ...
        if moves_error:
            ...
            for move in moves_error:
                user_warning += _(
                    ...
                    move.product_id.uom_id.category_id.name,
    ...
    def _propagate_product_packaging(self, product_package_id):
        ...
        already_propagated_ids = self.env.context.get(
            "product_packaging_propagation_ids", set()
        ) | set(self.ids)
        self = self.with_context(
            product_packaging_propagation_ids=already_propagated_ids
        )
        for move in self:
            # propagate on destination move
            for move_dest in move.move_dest_ids:
                if (
                    move_dest.id not in already_propagated_ids
                    and move_dest.state not in ["cancel", "done"]
                    and move_dest.product_packaging_id.id != product_package_id
                    and move_dest.move_orig_ids == move
                ):  # checks that you are the only parent move of your destination
                    move_dest.product_packaging_id = product_package_id
            # propagate on origin move
            for move_orig in move.move_orig_ids:
                if (
                    move_orig.id not in already_propagated_ids
                    and move_orig.state not in ["cancel", "done"]
                    and move_orig.product_packaging_id.id != product_package_id
                    and move_orig.move_dest_ids == move
                ):  # checks that you are the only child move of your origin
        ...
        return {
            ...
            "context": dict(
                ...
                and (
                    ...
                    or self.state == "done"
                    or self.origin_returned_move_id.id
                ),  # able to create lots, whatever the value of ` use_create_lots`.
                show_lots_text=self.has_tracking != "none"
                and self.picking_type_id.use_create_lots
                and not self.picking_type_id.use_existing_lots
                and self.state != "done"
                and not self.origin_returned_move_id.id,
                ...
                and not self.picking_id.immediate_transfer
    ...
    def action_assign_serial_show_details(self):
        ...
        if not self.next_serial:
            raise UserError(
                _("You need to set a Serial Number before generating more.")
            )
    ...
    def _generate_serial_numbers(self, next_serial_count=False):
        ...
        lot_names = self.env["stock.lot"].generate_lot_names(
            self.next_serial, next_serial_count or self.next_serial_count
        )
    ...
    def _get_relevant_state_among_moves(self):
        ...
        if moves_todo[:1].picking_id and moves_todo[:1].picking_id.move_type == "one":
            ...
            if most_important_move.state == "confirmed":
                return (
                    "confirmed" if most_important_move.product_uom_qty else "assigned"
                )
    ...
    def _onchange_suggest_packaging(self):
        ...
        if self.product_id and self.product_qty and self.product_uom:
            self.product_packaging_id = (
                self.product_id.packaging_ids._find_suitable_product_packaging(
    ...
    def _onchange_lot_ids(self):
        quantity_done = sum(
            ml.product_uom_id._compute_quantity(ml.qty_done, self.product_uom)
            for ml in self.move_line_ids.filtered(
                lambda ml: not ml.lot_id and ml.lot_name
            )
        )
        quantity_done += self.product_id.uom_id._compute_quantity(
    ...
    def _generate_serial_move_line_commands(self, lot_names, origin_move_line=None):
        ...
        if self.picking_type_id.show_reserved:
            ...
            move_lines = self.move_line_nosuggest_ids.filtered(
                lambda ml: not ml.lot_id and not ml.lot_name
            )
        ...
        for lot_name in lot_names:
            ...
            # ... or create a new move line with the serial name.
                ...
                move_line_cmd = dict(
                    move_line_vals, lot_name=lot_name, location_dest_id=loc.id
                )
    ...
    def _action_confirm(self, merge=True, merge_into=False):
        ...
        new_push_moves = self.filtered(
            lambda m: not m.picking_id.immediate_transfer
        )._push_apply()
        self._check_company()
        moves = self
        if merge:
        ...
        for move in neg_r_moves:
            ...
            for m in move.move_orig_ids | move.move_dest_ids:
                ...
                if to_loc == move.location_id:
                    ...
                    dest_move_ids += m.ids
            move.move_orig_ids, move.move_dest_ids = [(6, 0, orig_move_ids)], [
                (6, 0, dest_move_ids)
            ]
    ...
    def _prepare_procurement_values(self):
        ...
        if (
            ...
            date = self.product_id._get_date_with_security_lead_days(
                self.date, self.location_id
            )
        return {
    ...
    def _update_reserved_quantity(
        ...
        if (
            self.product_packaging_id
            and self.product_id.product_tmpl_id.categ_id.packaging_reserve_method
            == "full"
        ):
            available_quantity = self.product_packaging_id._check_qty(
                available_quantity, self.product_id.uom_id, "DOWN"
            )
        ...
        for reserved_quant, quantity in quants:
            to_update = next(
                (
                    ...
                    if line._reservation_is_updatable(quantity, reserved_quant)
                ),
                False,
            )
            if to_update:
    ...
    def _get_available_move_lines_in(self):
        move_lines_in = self.move_orig_ids.filtered(lambda m: m.state == "done").mapped(
            "move_line_ids"
        )
    ...
    def _get_available_move_lines_out(
        ...
        for k, g in groupby(move_lines_out_reserved, key=_keys_out_groupby):
            grouped_move_lines_out[k] = sum(
                self.env["stock.move.line"].concat(*list(g)).mapped("reserved_qty")
            )
            ...
            if move._should_bypass_reservation():
                ...
                if (
                    ...
                    if to_update:
                        to_update[
                            0
                        ].reserved_uom_qty += move.product_id.uom_id._compute_quantity(
                            missing_reserved_quantity,
                            move.product_uom,
                            rounding_method="HALF-UP",
                        )
                ...
                if float_is_zero(
                    ...
                    for (
                        ...
                        need = move.product_qty - sum(
                            move.move_line_ids.mapped("reserved_qty")
                        )
    ...
    def _create_extra_move(self):
        ...
        if (
            float_compare(
                self.quantity_done, self.product_uom_qty, precision_rounding=rounding
            )
            > 0
        ):
            ...
            extra_move = self.copy(default=extra_move_vals).with_context(
                avoid_putaway_rules=True
            )

            merge_into_self = all(
                self[field] == extra_move[field]
                for field in self._prepare_merge_moves_distinct_fields()
            )
    ...
    def _action_done(self, cancel_backorder=False):
        ...
        for move in moves:
            if move.state == "cancel" or (
                move.quantity_done <= 0 and not move.is_inventory
            ):
                continue
        ...
        for move in moves_todo:
            ...
            if (
                float_compare(
                    move.quantity_done, move.product_uom_qty, precision_digits=rounding
                )
                < 0
            ):
        ...
        for result_package in moves_todo.mapped(
        ...
        new_push_moves = moves_todo.filtered(
            lambda m: m.picking_id.immediate_transfer
        )._push_apply()
        if new_push_moves:
    ...
    def _split(self, qty, restrict_partner_id=False):
        ...
        if self.state in ("done", "cancel"):
            raise UserError(
                _("You cannot split a stock move that has been set to 'Done'.")
            )
        ...
        new_product_qty = self.product_id.uom_id._compute_quantity(
            self.product_qty - qty, self.product_uom, round=False
        )
    ...
    def _recompute_state(self):
        ...
        for move in self:
            ...
            if move.state in ("cancel", "done", "draft"):
                ...
                orig.state not in ("done", "cancel") for orig in move.move_orig_ids
            ):
                moves_state_to_write["waiting"].add(move.id)
            else:
    ...
    def _is_consuming(self):
        ...
        return self.picking_type_id.code == "outgoing" or (
            ...
            for move in self.move_orig_ids:
                if move.state not in ("done", "cancel"):
                    for (
                        ...
                        responsible,
                        visited,
                    ) in move._get_upstream_documents_and_responsibles(visited):
        ...
        for ml in self.move_line_ids:
            ...
            if ml.product_uom_id != self.product_uom:
                taken_qty = self.product_uom._compute_quantity(
                    ml_qty, ml.product_uom_id, round=False
                )
    ...
    def _show_details_in_draft(self):
        ...
        return self.state != "draft" or (
            self.picking_id.immediate_transfer and self.state == "draft"
        )
    ...
    def _get_forecast_availability_outgoing(self, warehouse):
        ...
        def _reconcile_out_with_ins(
            result, out, ins, demand, product_rounding, only_matching_move_dest=True
        ):
            index_to_remove = []
            for index, in_ in enumerate(ins):
                ...
                if out.id in ids_in_self:
                    result[out] = (
                        result[out][0] + taken_from_in,
                        max(d for d in (in_["move_date"], result[out][1]) if d),
                    )
        ...
        in_domain, out_domain = self.env[
            "report.stock.report_product_product_replenishment"
        ]._move_confirmed_domain(None, product_ids.ids, wh_location_query)
        outs = self.env["stock.move"].search(
            out_domain, order="reservation_date, priority desc, date, id"
        )
        ...
        (outs - self).read(
            ["product_id", "product_uom", "product_qty", "state"], load=False
        )  # remove self because data is already fetch
        ins.read(["product_id", "product_qty", "date", "move_dest_ids"], load=False)

        currents = product_ids.with_context(
            warehouse=warehouse.id
        )._get_only_qty_available()
        ...
        for product in product_ids:
            ...
            for out in reserved_outs_per_product[product.id]:
                ...
                reserved = out.product_uom._compute_quantity(
                    out.reserved_availability, product.uom_id
                )
            ...
            for out in outs_per_product[product.id]:
                ...
                if not float_is_zero(
                    taken_from_stock, precision_rounding=product_rounding
                ):
            ...
            for demand, out in unreconciled_outs:
                remaining = _reconcile_out_with_ins(
                    result,
                    out,
                    ins_per_product[product.id],
                    demand,
                ...
                if (
                    not float_is_zero(
                        remaining, precision_rounding=out.product_id.uom_id.rounding
                    )
                    and out not in result
                ):
