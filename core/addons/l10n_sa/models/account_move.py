...
class AccountMove(models.Model):
    ...
    l10n_sa_confirmation_datetime = fields.Datetime(
        string="Confirmation Date", readonly=True, copy=False
    )
    ...
    def _post(self, soft=True):
        ...
        for record in self:
            if record.country_code == "SA" and record.move_type in (
                "out_invoice",
                "out_refund",
            ):
                if not record.l10n_sa_show_delivery_date:
                    raise UserError(_("Delivery Date cannot be empty"))
                self.write({"l10n_sa_confirmation_datetime": fields.Datetime.now()})
        return res
