...
class TestSaleMrpFlow(ValuationReconciliationTestCommon):
    ...
    def setUpClass(cls, chart_template_ref=None):
        ...
        cls.uom_kg = cls.env["uom.uom"].search(
            [("category_id", "=", cls.categ_kgm.id), ("uom_type", "=", "reference")],
            limit=1,
        )
        cls.uom_kg.write({"name": "Test-KG", "rounding": 0.000001})
    ...
    def _process_quantities(self, moves, quantities_to_process):
        ...
        moves_to_process = moves.filtered(
            lambda m: m.product_id in quantities_to_process.keys()
        )
    ...
    def test_00_sale_mrp_flow(self):
        ...
        self.assertEqual(
            ...
            1,
            "User should see the closest manufacture order in the smart button",
        )
    ...
    def test_01_sale_mrp_delivery_kit(self):
        ...
        self.assertEqual(
            so.invoice_status,
            "no",
            'Sale MRP: so invoice_status should be "nothing to invoice" after invoicing',
        )
        ...
        self.assertEqual(
            so.invoice_status,
            "no",
            'Sale MRP: so invoice_status should be "no" after partial delivery of a kit',
        )
    ...
    def test_03_sale_mrp_simple_kit_qty_delivered(self):
        ...
        wiz = (
            Form(self.env[wiz_act["res_model"]].with_context(wiz_act["context"]))
            .save()
            .process()
        )
    ...
    def test_06_kit_qty_delivered_mixed_uom(self):
        ...
        for move in move_ids:
            corr_bom_line = bom_kit_uom_1.bom_line_ids.filtered(
                lambda b: b.product_id.id == move.product_id.id
            )
            computed_qty = move.product_uom._compute_quantity(
                move.product_uom_qty, corr_bom_line.product_uom_id
            )
            self.assertEqual(
                computed_qty, order_line.product_uom_qty * corr_bom_line.product_qty
            )
    ...
    def test_15_anglo_saxon_variant_price_unit(self):
        ...
        # Create the quants
        self.env["stock.quant"]._update_available_quantity(
            self.variant_KIT, self.stock_location, 1
        )
        self.env["stock.quant"]._update_available_quantity(
    ...
    def test_kit_margin_and_return_picking(self):
        ...
        price = line.product_id.with_company(line.company_id)._compute_average_price(
            0, line.product_uom_qty, line.move_ids
        )
