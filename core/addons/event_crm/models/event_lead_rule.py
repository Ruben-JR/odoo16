...
class EventLeadRule(models.Model):
    ...
    def _run_on_registrations(self, registrations):
        ...
        for rule in self:
            if rule.lead_creation_basis == "attendee":
                ...
                for toupdate_leads, group_key, group_registrations in rule_group_info[
                    rule
                ]:
                    if toupdate_leads:
    ...
    def _filter_registrations(self, registrations):
        ...
        company_ok = (
            lambda registration: registration.company_id == self.company_id
            if self.company_id
            else True
        )
        event_or_event_type_ok = (
            lambda registration: registration.event_id == self.event_id
