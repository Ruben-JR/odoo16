...
class TestSyncOdoo2Microsoft(TransactionCase):
    ...
    def assertMicrosoftEventInserted(self, values):
        MicrosoftSync._microsoft_insert.assert_called_once_with(
            self.microsoft_service, values
        )
    ...
    def test_stop_synchronization(self):
        ...
        self.assertTrue(
            self.env.user.microsoft_synchronization_stopped,
            "The microsoft synchronization flag should be switched on",
        )
        self.assertFalse(
            self.env.user._sync_microsoft_calendar(self.microsoft_service),
            "The microsoft synchronization should be stopped",
        )
    ...
    def test_restart_synchronization(self):
        ...
        self.assertMicrosoftEventPatched(
            event.microsoft_id,
            {
                "id": event.microsoft_id,
                "start": {
                    "dateTime": "%s-01-15T08:00:00+00:00" % year,
                    "timeZone": "Europe/London",
                },
                "end": {
                    "dateTime": "%s-01-15T18:00:00+00:00" % year,
                    "timeZone": "Europe/London",
                },
                "subject": "Event",
