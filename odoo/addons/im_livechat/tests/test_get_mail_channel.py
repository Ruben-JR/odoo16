...
class TestGetMailChannel(TransactionCase):
    ...
    def test_get_mail_channel(self):
        ...
        for i in range(5):
            ...
            channel_operators = [
                channel_info["operator_pid"] for channel_info in mail_channels
            ]
            channel_operator_ids = [
                channel_operator[0] for channel_operator in channel_operators
            ]
            self.assertTrue(
                all(
    ...
    def test_channel_get_livechat_visitor_info(self):
        ...
        channel_info = self.livechat_channel.with_user(
            public_user
        ...
        self.assertEqual(
            channel_info["channel"]["anonymous_country"],
            {"code": "BE", "id": belgium.id, "name": "Belgium"},
        )
        ...
        self.assertEqual(
            sorted(
                map(
                    lambda m: m["persona"]["partner"],
                    channel_info["channel"]["channelMembers"][0][1],
                ),
                key=lambda m: m["id"],
            ),
            sorted(
        ...
        self.assertEqual(
            channel_info["channel"]["channelMembers"],
            [
                (
                    "insert",
                    [
                        {
                            ...
                            "id": self.env["mail.channel.member"]
                            .search(
                                [
        ...
        self.assertEqual(
            channel_info["operator_pid"], (operator.partner_id.id, "Michel Operator")
        )
        self.assertFalse(channel_info["channel"]["anonymous_name"])
        self.assertEqual(channel_info["channel"]["anonymous_country"], [("clear",)])
        self.assertEqual(
    ...
    def _open_livechat_mail_channel(self):
        ...
        for i in range(5):
            mail_channel = self.livechat_channel._open_livechat_mail_channel(
                "Anonymous"
            )
    ...
    def test_channel_not_pinned_for_operator_before_first_message(self):
        ...
        channel_info = self.livechat_channel.with_user(
            public_user
        )._open_livechat_mail_channel(anonymous_name="whatever")
        operator_channel_member = self.env["mail.channel.member"].search(
            [
                ("channel_id", "=", channel_info["id"]),
        ...
        self.assertEqual(
            len(operator_channel_member), 1, "operator should be member of channel"
        )
        self.assertFalse(
            operator_channel_member.is_pinned,
            "channel should not be pinned for operator initially",
        )
        ...
        self.assertIn(
            channel_info["id"],
            operator_channel_member.partner_id._get_channels_as_member().ids,
            "channel should be fetched by operator on new page",
        )
    ...
    def test_operator_livechat_username(self):
        ...
        channel_info = (
            self.livechat_channel.with_user(public_user)
            .sudo()
            ._open_livechat_mail_channel(anonymous_name="whatever")
        )
        channel = self.env["mail.channel"].browse(channel_info["id"])
        channel.with_user(operator).message_post(
        ...
        self.assertEqual(
            message_formats[0]["author"]["user_livechat_username"],
            operator.livechat_username,
        )
        self.assertFalse(
            message_formats[0].get("email_from"),
            "should not send email_from to livechat user",
        )
    ...
    def test_read_channel_unpined_for_operator_after_one_day(self):
        ...
        channel_info = self.livechat_channel.with_user(
            public_user
        )._open_livechat_mail_channel(anonymous_name="visitor")
        member_of_operator = self.env["mail.channel.member"].search(
            [
                ("channel_id", "=", channel_info["id"]),
                ("partner_id", "in", self.operators.partner_id.ids),
            ]
        ...
        member_of_operator.channel_id.with_user(
            self.operators.filtered(
