...
class TestFifoPrice(ValuationReconciliationTestCommon):
    def test_00_test_fifo(self):
        ...
        self.assertEqual(
            product_cable_management_box.value_svl, 500.0, "Wrong stock value"
        )
        ...
        self.assertEqual(
            product_cable_management_box.standard_price,
            50.0,
            "Standard price as fifo price of second reception incorrect!",
        )
        self.assertEqual(
            product_cable_management_box.value_svl,
            2900.0,
            "Stock valuation should be 2900",
        )
        ...
        self.assertEqual(
            product_cable_management_box.value_svl,
            1600.0,
            "Stock valuation should be 1600",
        )
        ...
        self.assertEqual(
            product_cable_management_box.value_svl,
            1560.0,
            "Stock valuation should be 1560",
        )
        self.assertEqual(
            product_cable_management_box.qty_available,
            19.5,
            "Should still have 19.5 in stock",
        )
        ...
        NewUSD = self.env["res.currency"].create(
            {
                ...
                "symbol": "$²",
                "rate_ids": [
                    (0, 0, {"rate": 1.2834, "name": time.strftime("%Y-%m-%d")})
                ],
            }
        )
        ...
        self.assertEqual(
            product_fifo_negative.qty_available, -100, "Stock qty should be -100"
        )
        ...
        self.assertEqual(
            product_fifo_negative.qty_available, -500, "Stock qty should be -500"
        )
        ...
        self.assertEqual(
            original_out_move.product_id.value_svl,
            12000.0,
            "Value of the move should be 12000",
        )
        self.assertEqual(
            original_out_move.product_id.qty_available,
            150.0,
            "Qty available should be 150",
        )
