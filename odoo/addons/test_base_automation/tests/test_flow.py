...
if 'partner_id' in env.context['old_values'][record.id]:
    record.write({'state': 'draft'})""",
                ...
                },
                {
                    ...
                    "model_id": self.env.ref(
                        "test_base_automation.model_base_automation_line_test"
                    ).id,
                    "state": "code",
                    "code": "records.write({'user_id': %s})" % (self.user_demo.id),
                    "trigger": "on_create",
...
record['name'] = record.name + 'X'""",
    ...
    def test_00_check_to_state_open_pre(self):
        ...
        self.assertEqual(
            lead.user_id,
            self.user_root,
            "Responsible should not change on creation of Lead with state 'open'.",
        )
    ...
    def test_01_check_to_state_draft_post(self):
        ...
        self.assertEqual(
            lead.user_id,
            self.user_demo,
            "Responsible should be change on creation of Lead with state 'draft'.",
        )
    ...
    def test_02_check_from_draft_to_done_with_steps(self):
        ...
        self.assertEqual(
            lead.user_id,
            self.user_root,
            "Responsible should not change on creation of Lead with state from 'draft' to 'open'.",
        )
        ...
        self.assertEqual(
            lead.user_id,
            self.user_root,
            "Responsible should not chang on creation of Lead with state from 'pending' to 'done'.",
        )
    ...
    def test_03_check_from_draft_to_done_without_steps(self):
        ...
        self.assertEqual(
            lead.user_id,
            self.user_demo,
            "Responsible should be change on write of Lead with state from 'open' to 'done'.",
        )
    ...
    def test_10_recomputed_field(self):
        ...
        self.assertEqual(
            lead.user_id,
            self.user_demo,
            "Responsible should be change on write of Lead when Customer becomes True.",
        )
    ...
    def test_11b_recomputed_field(self):
        mail_automation = self.env["base.automation"].search(
            [("name", "=", "Base Automation: test send an email")]
        )
        send_mail_count = 0
        ...
        patchers = [
            patch(
                "odoo.addons.base_automation.models.base_automation.BaseAutomation._get_actions",
                _patched_get_actions,
            ),
            patch(
    ...
    def test_20_indirect_line(self):
        ...
        self.assertEqual(
            lead.user_id,
            self.user_demo,
            "Responsible should change on creation of Lead test line.",
        )
        self.assertEqual(len(lead.line_ids), 1, "New test line is not created")
        self.assertEqual(
            lead.line_ids.user_id,
            self.user_demo,
            "Responsible should be change on creation of Lead test line.",
        )
