...
def _l10n_it_edi_add_accounts(env, company):
    ...
    account_templates = {
        account_code: env.ref(f"l10n_it_edi_withholding.{account_code}")
        for account_code in ["1611", "2603"]
    }
    try:
        with env.cr.savepoint():
            for account_code, account_template in account_templates.items():
                template_vals = [
                    (
                        account_template,
                        company.chart_template_id._get_account_vals(
                            company, account_template, account_code, {}
                        ),
                    )
                ]
                generated_accounts_ref[
                    account_template
                ] = company.chart_template_id._create_records_with_xmlid(
                    "account.account", template_vals, company
                )
            _logger.info(
                "Created withholding accounts for company: %s(%s).",
                company.name,
                company.id,
            )
        ...
        generated_accounts_ref = {}
        _logger.error(
            "Cash basis transition accounts already exist for company: %s(%s).",
            company.name,
            company.id,
        )
...
def _l10n_it_edi_withholding_add_taxes(env, company):
    ...
    try:
        with env.cr.savepoint():
            ...
            _logger.info(
                "Created withholding taxes for company: %s(%s).",
                company.name,
                company.id,
            )
            ...
            for tax in all_taxes.filtered(
                lambda x: (
                    x.sequence <= 20
                    and not x.l10n_it_pension_fund_type
                    and not x.l10n_it_withholding_type
                )
            ):
                if (
                    ...
                    tax.sequence += offset
            _logger.info(
                "Increased sequence number of old taxes by %s for company: %s(%s).",
                offset,
                company.name,
                company.id,
            )
        ...
        generated_taxes_ref = {}
        _logger.error(
            "Withholding and Pension Fund taxes already exist for company: %s(%s).",
            company.name,
            company.id,
        )
...
def _l10n_it_edi_setup_accounts_on_taxes(
    env, company, generated_taxes_ref, generated_accounts_ref
):
    ...
    for tax, value in generated_taxes_ref["account_dict"]["account.tax"].items():
        ...
        if transition_account:
            tax.cash_basis_transition_account_id = generated_accounts_ref.get(
                transition_account
            )

    # The tax repartition lines accounts has already been generated from the template by l10n_it
    referenced_accounts = {}
    repartitions_dict = generated_taxes_ref["account_dict"][
    ...
    for dummy, value_dict in repartitions_dict.items():
        ...
        referenced_accounts[template_xml_id] = env.ref(
            f"{xml_id[0]}.{company.id}_{xml_id[1]}"
        )
    ...
    for repartition_line, value in repartitions_dict.items():
        ...
        if template:
            repartition_line.account_id = referenced_accounts[
                template.get_external_id()[template.id]
            ]
    _logger.info(
        "Cash Basis Transition Accounts and Repartition accounts on taxes for company: %s(%s).",
        company.name,
        company.id,
    )
...
def _l10n_it_edi_withholding_post_init(cr, registry):
    ...
    if chart_template:
        for company in env["res.company"].search(
            ...
            _logger.info(
                "Company %s already has the Italian localization installed, updating...",
                company.name,
            )
            generated_taxes_ref = _l10n_it_edi_withholding_add_taxes(env, company)
            generated_accounts_ref = (
                {}
                if not generated_taxes_ref
                else _l10n_it_edi_add_accounts(env, company)
            )
            if generated_accounts_ref:
                _l10n_it_edi_setup_accounts_on_taxes(
                    env, company, generated_taxes_ref, generated_accounts_ref
                )
