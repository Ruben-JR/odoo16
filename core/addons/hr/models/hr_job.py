...
class Job(models.Model):
    ...
    def _compute_employees(self):
        ...
        result = dict(
            (data["job_id"][0], data["job_id_count"]) for data in employee_data
        )
        for job in self:
    ...
    def create(self, vals_list):
        ...
        return super(Job, self.with_context(mail_create_nosubscribe=True)).create(
            vals_list
        )
