...
class TestMoveCancelPropagation(PurchaseTestCommon):
    ...
    def setUpClass(cls):
        ...
        cls.group = cls.env["procurement.group"].create(
            {"partner_id": cls.customer.id, "name": "New Group"}
        )
    ...
    def test_02_cancel_confirm_purchase_order_one_steps(self):
        ...
        picking_in = purchase_order.picking_ids.filtered(
            lambda r: r.picking_type_id == self.warehouse.in_type_id
        )
        # Cancel Purchase order.
        purchase_order.button_cancel()
    ...
    def test_03_cancel_draft_purchase_order_two_steps(self):
        ...
        """
        self.warehouse.write(
            {"delivery_steps": "pick_ship", "reception_steps": "two_steps"}
        )
        self.picking_out.write(
            {"location_id": self.warehouse.out_type_id.default_location_src_id.id}
        )
        ...
        internal = picking_ids.filtered(
            lambda r: r.picking_type_id == self.warehouse.int_type_id
            and r.group_id.id == self.group.id
        )
        pick = picking_ids.filtered(
            lambda r: r.picking_type_id == self.warehouse.pick_type_id
            and r.group_id.id == self.group.id
        )
    ...
    def test_05_cancel_draft_purchase_order_three_steps(self):
        ...
        """
        self.warehouse.write(
            {"delivery_steps": "pick_pack_ship", "reception_steps": "three_steps"}
        )
        self.picking_out.write(
            {"location_id": self.warehouse.out_type_id.default_location_src_id.id}
        )
        ...
        pack = picking_ids.filtered(
            lambda r: r.picking_type_id == self.warehouse.pack_type_id
        )
