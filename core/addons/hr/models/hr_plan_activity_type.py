...
class HrPlanActivityType(models.Model):
    ...
    summary = fields.Char(
        "Summary", compute="_compute_default_summary", store=True, readonly=False
    )
    ...
    def _compute_default_summary(self):
        for plan_type in self:
            if (
                not plan_type.summary
                and plan_type.activity_type_id
                and plan_type.activity_type_id.summary
            ):
    ...
    def get_responsible_id(self, employee):
        ...
        if self.responsible == "coach":
            ...
            if not responsible:
                warning = _(
                    "No specific user given on activity %s.", self.activity_type_id.name
                )
        return {
