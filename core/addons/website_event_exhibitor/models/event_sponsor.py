...
class Sponsor(models.Model):
    ...
    partner_id = fields.Many2one(
        "res.partner", "Partner", required=True, auto_join=True
    )
    ...
    mobile = fields.Char(
        "Sponsor Mobile", compute="_compute_mobile", readonly=False, store=True
    )
    # image
        ...
        for sponsor in self:
            if not sponsor.event_id.is_ongoing:
                ...
                opening_from_tz = event_tz.localize(
                    datetime.combine(now_tz.date(), float_to_time(sponsor.hour_from))
                )
                opening_to_tz = event_tz.localize(
                    datetime.combine(now_tz.date(), float_to_time(sponsor.hour_to))
                )
        ...
        for values in values_list:
            if values.get("is_exhibitor") and not values.get("room_name"):
                exhibitor_name = (
                    values["name"]
                    if values.get("name")
                    else self.env["res.partner"].browse(values["partner_id"]).name
                )
    ...
    def write(self, values):
        ...
        if (
            ...
            and not values.get("chat_room_id")
            and not values.get("room_name")
        ):
        ...
        for sponsor in self:
            if sponsor.partner_id:
                sponsor._message_add_suggested_recipient(
                    recipients, partner=sponsor.partner_id, reason=_("Sponsor")
                )
        return recipients
