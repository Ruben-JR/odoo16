...
class PaymentTransaction(models.Model):
    ...
    def _get_specific_rendering_values(self, processing_values):
        ...
        rendering_values = {
            ...
            "currency_code": const.CURRENCY_MAPPING[
                self.provider_id.asiapay_currency_id.name
            ],
            "mps_mode": "SCP",
    ...
    def _get_tx_from_notification_data(self, provider_code, notification_data):
        ...
        if not reference:
            raise ValidationError(
                "AsiaPay: "
                + _("Received data with missing reference %(ref)s.", ref=reference)
            )

        tx = self.search(
            [("reference", "=", reference), ("provider_code", "=", "asiapay")]
        )
        if not tx:
            raise ValidationError(
                "AsiaPay: "
    ...
    def _process_notification_data(self, notification_data):
        ...
        if not success_code:
            raise ValidationError(
                "AsiaPay: " + _("Received data with missing success code.")
            )
        ...
        if success_code in const.SUCCESS_CODE_MAPPING["done"]:
            ...
            self._set_error(
                _(
                    ...
                    "response code %s). Please try again.",
                    success_code,
