...
class AccountMove(models.Model):
    ...
    def _compute_edi_error_message(self):
        for move in self:
            if move.edi_error_count == 0:
                ...
                error_levels = set(
                    [doc.blocking_level for doc in move.edi_document_ids]
                )
                if "error" in error_levels:
                    move.edi_error_message = str(move.edi_error_count) + _(
                        " Electronic invoicing error(s)"
                    )
                    move.edi_blocking_level = "error"
                elif "warning" in error_levels:
                    move.edi_error_message = str(move.edi_error_count) + _(
                        " Electronic invoicing warning(s)"
                    )
                    move.edi_blocking_level = "warning"
                else:
                    move.edi_error_message = str(move.edi_error_count) + _(
                        " Electronic invoicing info(s)"
                    )
    ...
    def _update_payments_edi_documents(self):
        ...
        for payment in self:
            edi_formats = (
                payment._get_reconciled_invoices().journal_id.edi_format_ids
                + payment.edi_document_ids.edi_format_id
            )
            edi_formats = self.env["account.edi.format"].browse(
                edi_formats.ids
            )  # Avoid duplicates
            for edi_format in edi_formats:
                existing_edi_document = payment.edi_document_ids.filtered(
...
class AccountMoveLine(models.Model):
    ...
    def reconcile(self):
        ...
        invoices_per_payment_before = {
            pay: pay._get_reconciled_invoices() for pay in payments
        }
        res = super().reconcile()
        invoices_per_payment_after = {
            pay: pay._get_reconciled_invoices() for pay in payments
        }
