...
class ResConfigSettings(models.TransientModel):
    ...
    show_effect = fields.Boolean(
        string="Show Effect", config_parameter="base_setup.show_effect"
    )
    ...
    profiling_enabled_until = fields.Datetime(
        "Profiling enabled until", config_parameter="base.profiling_enabled_until"
    )
    ...
    def _compute_company_informations(self):
        ...
        informations += (
            "%s\n" % self.company_id.state_id.display_name
            if self.company_id.state_id
            else ""
        )
        informations += (
            "%s" % self.company_id.country_id.display_name
        ...
        vat_display = "\n" + vat_display + ": "
        informations += (
            "%s %s" % (vat_display, self.company_id.vat) if self.company_id.vat else ""
        )
