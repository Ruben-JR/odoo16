...
class TestChannelAccessRights(MailCommon):
    ...
    def setUpClass(cls):
        ...
        cls.user_employee_1 = mail_new_test_user(
            cls.env, login="user_employee_1", groups="base.group_user", name="Tao Lee"
        )
        ...
        cls.group_restricted_channel = cls.env["mail.channel"].browse(
            cls.env["mail.channel"].channel_create(
                name="Channel for Groups", group_id=cls.env.ref("base.group_user").id
            )["id"]
        )
        # Public Channel
        cls.public_channel = cls.env["mail.channel"].browse(
            cls.env["mail.channel"].channel_create(
                name="Public Channel", group_id=None
            )["id"]
        )
        # Group
        cls.private_group = cls.env["mail.channel"].browse(
            cls.env["mail.channel"].create_group(
                partners_to=cls.user_employee.partner_id.ids, name="Group"
            )["id"]
        )
        # Chat
        cls.chat_user_employee = cls.env["mail.channel"].browse(
            cls.env["mail.channel"].channel_get(cls.user_employee.partner_id.ids)["id"]
        )
        cls.chat_user_employee_1 = cls.env["mail.channel"].browse(
            cls.env["mail.channel"].channel_get(cls.user_employee_1.partner_id.ids)[
                "id"
            ]
        )
        cls.chat_user_portal = cls.env["mail.channel"].browse(
            cls.env["mail.channel"].channel_get(cls.user_portal.partner_id.ids)["id"]
        )
        cls.chat_user_public = cls.env["mail.channel"].browse(
            cls.env["mail.channel"].channel_get(cls.user_public.partner_id.ids)["id"]
        )
    ...
    def test_access_public(self):
        ...
        with self.assertRaises(AccessError):
            self.env["mail.channel"].browse(self.public_channel.id).write(
                {"name": "modified"}
            )
        with self.assertRaises(AccessError):
            self.env["mail.channel"].browse(self.group_restricted_channel.id).write(
                {"name": "modified"}
            )
        with self.assertRaises(AccessError):
            self.env["mail.channel"].browse(self.private_group.id).write(
                {"name": "modified"}
            )
        with self.assertRaises(AccessError):
            self.env["mail.channel"].browse(self.chat_user_public.id).write(
                {"name": "modified"}
            )
    ...
    def test_access_employee(self):
        ...
        with self.assertRaises(AccessError):
            self.env["mail.channel"].browse(self.chat_user_employee_1.id).write(
                {"name": "modified"}
            )
        ...
        self.env["mail.channel"].browse(self.chat_user_employee.id).write(
            {"name": "modified again"}
        )
    ...
    def test_access_portal(self):
        ...
        with self.assertRaises(AccessError):
            self.env["mail.channel"].browse(self.chat_user_portal.id).write(
                {"name": "modified"}
            )
...
class TestChannelInternals(MailCommon):
    ...
    def setUpClass(cls):
        ...
        cls.test_channel = cls.env["mail.channel"].browse(
            cls.env["mail.channel"]
            .with_context(cls._test_context)
            .channel_create(name="Channel", group_id=None)["id"]
        )
        cls.test_partner = (
            cls.env["res.partner"]
            .with_context(cls._test_context)
    ...
    def test_channel_chat_message_post_should_update_last_interest_dt(self):
        channel_info = (
            ...
            .channel_get((self.partner_employee | self.user_admin.partner_id).ids)
        )
        chat = (
            self.env["mail.channel"]
            .with_user(self.user_admin)
            .browse(channel_info["id"])
        )
    ...
    def test_channel_recipients_channel(self):
        ...
        channel = self.env["mail.channel"].browse(self.test_channel.ids)
        channel.add_members(
            (self.partner_employee | self.partner_admin | self.test_partner).ids
        )
    ...
    def test_channel_user_synchronize(self):
        ...
        group_restricted_channel = self.env["mail.channel"].browse(
            self.env["mail.channel"].channel_create(
                name="Sic Mundus", group_id=self.env.ref("base.group_user").id
            )["id"]
        )

        self.test_channel.add_members(
            (self.partner_employee | self.partner_employee_nomail).ids
        )
        group_restricted_channel.add_members(
            (self.partner_employee | self.partner_employee_nomail).ids
        )
    ...
    def test_channel_info_get(self):
        ...
        initial_channel_info = self.env["mail.channel"].channel_get(
            partners_to=self.test_partner.ids
        )
        # shape of channelMembers is [('insert', data...)], [0][1] accesses the data
        self.assertEqual(
            set(
                m["persona"]["partner"]["id"]
                for m in initial_channel_info["channel"]["channelMembers"][0][1]
            ),
        ...
        same_channel_info = self.env["mail.channel"].channel_get(
            partners_to=self.test_partner.ids
        )
        ...
        together_channel_info = self.env["mail.channel"].channel_get(
            partners_to=(self.partner_employee_nomail + self.test_partner).ids
        )
        ...
        solo_channel_info = self.env["mail.channel"].channel_get(
            partners_to=self.partner_employee_nomail.ids
        )
    ...
    def test_channel_info_get_should_update_last_interest_dt(self):
        ...
        self.assertEqual(
            channel_info["last_interest_dt"],
            retrieve_time.strftime(DEFAULT_SERVER_DATETIME_FORMAT),
        )
    ...
    def test_channel_message_post_should_not_allow_adding_wrong_parent(self):
        ...
        self.assertFalse(
            message_format2["parent_id"], "should not allow parent from wrong thread"
        )
        message_format3 = channels[1].message_post(
            body="Body3", parent_id=message.id + 100
        )
        self.assertFalse(
            message_format3["parent_id"], "should not allow non-existing parent"
        )
    ...
    def test_channel_private_unfollow(self):
        ...
        public_channel = self.env["mail.channel"].browse(
            self.env["mail.channel"].channel_create(
                name="Channel for Everyone", group_id=None
            )["id"]
        )
        private_group = self.env["mail.channel"].browse(
            self.env["mail.channel"].create_group(
                partners_to=self.user_employee.partner_id.ids, name="Group"
            )["id"]
        )
        chat_user_current = self.env["mail.channel"].browse(
            self.env["mail.channel"].channel_get(self.env.user.partner_id.ids)["id"]
        )
    ...
    def test_channel_unfollow_should_not_post_message_if_the_partner_has_been_removed(
        self,
    ):
        """
    ...
    def test_channel_should_generate_correct_default_avatar(self):
        test_channel = self.env["mail.channel"].browse(
            self.env["mail.channel"].channel_create(
    ...
    def test_mail_message_starred_group(self):
        ...
        self.assertEqual(
            self.user_employee.with_user(self.user_employee)._init_messaging()[
                "starred_counter"
            ],
            1,
        )
    ...
    def test_multi_company_chat(self):
        ...
        with self.with_user("employee"):
            ...
            self.assertTrue(
                initial_channel_info, "should be able to chat with multi company user"
            )
    ...
    def test_create_chat_channel_should_only_pin_the_channel_for_the_current_user(self):
        ...
        member_of_current_user = self.env["mail.channel.member"].search(
            [
                ("channel_id", "=", chat["id"]),
                ("partner_id", "=", self.env.user.partner_id.id),
            ]
        )
        member_of_correspondent = self.env["mail.channel.member"].search(
            [("channel_id", "=", chat["id"]), ("partner_id", "=", self.test_partner.id)]
        )
