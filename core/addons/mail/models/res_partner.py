...
class Partner(models.Model):
    ...
    def _message_get_default_recipients(self):
        return {
            r.id: {"partner_ids": [r.id], "email_to": False, "email_cc": False}
            for r in self
        }
    ...
    def search_for_channel_invite(self, search_term, channel_id=None, limit=30):
        ...
        return {
            ...
            "partners": list(
                self.env["res.partner"].browse(query).mail_partner_format().values()
            ),
        }
    ...
    def get_mention_suggestions(self, search, limit=8, channel_id=None):
        ...
        search_dom = expression.AND(
            [[("active", "=", True), ("type", "!=", "private")], search_dom]
        )
        if channel_id:
            search_dom = expression.AND(
                [[("channel_ids", "in", channel_id)], search_dom]
            )
        domain_is_user = expression.AND(
            [[("user_ids", "!=", False), ("user_ids.active", "=", True)], search_dom]
        )
        priority_conditions = [
            expression.AND(
        ...
        for domain in priority_conditions:
            ...
            # really slow.
            query = self._search(
                expression.AND([[("id", "not in", partners.ids)], domain]),
