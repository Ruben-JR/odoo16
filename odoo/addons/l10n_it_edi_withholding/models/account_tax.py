...
class AccountTax(models.Model):
    ...
    def _l10n_it_get_tax_kind(self):
        return (
            (self.l10n_it_withholding_type and "withholding")
    ...
    def _validate_withholding(self):
        for tax in self:
            ...
            if (
                tax.l10n_it_withholding_type or tax.l10n_it_withholding_reason
            ) and tax.l10n_it_pension_fund_type:
                raise ValidationError(
                    _(
                        "Tax '%s' cannot be both a Withholding tax and a Pension fund tax. Please create two separate ones.",
                        tax.name,
                    )
                )
