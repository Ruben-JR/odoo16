...
class TestPurchaseMrpFlow(TransactionCase):
    ...
    def test_01_purchase_mrp_kit_qty_change(self):
        ...
        self.assertEqual(
            self.po.picking_ids.move_ids_without_package[0].product_uom_qty,
            2,
            "The quantity of components must be created according to the BOM",
        )
        self.assertEqual(
            self.po.picking_ids.move_ids_without_package[1].product_uom_qty,
            1,
            "The quantity of components must be created according to the BOM",
        )
        self.assertEqual(
            self.po.picking_ids.move_ids_without_package[2].product_uom_qty,
            3,
            "The quantity of components must be created according to the BOM",
        )
    ...
    def test_procurement_with_preferred_route(self):
        ...
        move_stock, move_check = self.env["stock.move"].search(
            [("product_id", "=", product.id)]
        )

        self.assertRecordValues(
            move_check | move_stock,
            [
                {
                    "location_id": self.warehouse.wh_input_stock_loc_id.id,
                    "location_dest_id": self.warehouse.wh_qc_stock_loc_id.id,
                    "state": "waiting",
                    "move_dest_ids": move_stock.ids,
                },
                {
                    "location_id": self.warehouse.wh_qc_stock_loc_id.id,
    ...
    def test_procurement_with_preferred_route_2(self):
        ...
        self.assertEqual(
            ...
            buy_route,
            "The route buy should be set on the orderpoint",
        )
