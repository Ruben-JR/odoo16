...
class StockLandedCost(models.Model):
    ...
    account_move_id = fields.Many2one(
        "account.move", "Journal Entry", copy=False, readonly=True
    )
    ...
    company_id = fields.Many2one(
        "res.company", string="Company", related="account_journal_id.company_id"
    )
    stock_valuation_layer_ids = fields.One2many(
        "stock.valuation.layer", "stock_landed_cost_id"
    )
...
class StockLandedCostLine(models.Model):
    ...
    cost_id = fields.Many2one(
        "stock.landed.cost", "Landed Cost", required=True, ondelete="cascade"
    )
...
class AdjustmentLines(models.Model):
    ...
    cost_id = fields.Many2one(
        "stock.landed.cost", "Landed Cost", ondelete="cascade", required=True
    )
    ...
    def _create_account_move_line(
        ...
        if qty_out > 0:
            ...
            if self.env.company.anglo_saxon_accounting:
                expense_account_id = (
                    self.product_id.product_tmpl_id.get_product_accounts()["expense"].id
                )
                debit_line = dict(
                    base_line,
                    name=(self.name + ": " + str(qty_out) + _(" already out")),
