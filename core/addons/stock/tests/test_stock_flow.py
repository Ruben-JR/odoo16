...
class TestStockFlow(TestStockCommon):
    ...
    def test_00_picking_create_and_transfer_quantity(self):
        ...
        self.assertEqual(
            moves.product_uom_qty, 4.0, "Wrong move quantity for product A."
        )
        ...
        self.assertEqual(
            moves.product_uom_qty, 5.0, "Wrong move quantity for product B."
        )
        ...
        self.assertEqual(
            movesC.product_uom_qty, 6.0, "Wrong quantity of moves product C."
        )
    ...
    def test_10_pickings_transfer_with_different_uom(self):
        ...
        PackSdozAround = self.StockPackObj.search(
            [
                ("product_id", "=", self.SDozARound.id),
                ("picking_id", "=", picking_in_A.id),
            ],
        ...
        self.assertEqual(
            PackSdozAround.reserved_qty,
            11,
            "Wrong quantity in pack operation (%s found instead of 11)"
            % (PackSdozAround.reserved_qty),
        )
        ...
        self.assertEqual(
            sum(total_qty),
            11,
            "Expecting 11 SDozenRound , got %.4f SDozenRound on location stock!"
            % (sum(total_qty)),
        )
        self.assertEqual(
            self.SDozARound.qty_available,
            11,
            "Wrong quantity available (%s found instead of 11)"
            % (self.SDozARound.qty_available),
        )
        # Check quants and available quantity for product gB
        quants = self.StockQuantObj.search(
            [("product_id", "=", self.gB.id), ("location_id", "=", self.stock_location)]
        )
        total_qty = [quant.quantity for quant in quants]
        ...
        self.assertEqual(
            PackSdozAround.reserved_uom_qty,
            1584,
            "Wrong quantity in pack operation (%s found instead of 1584)"
            % (PackSdozAround.reserved_uom_qty),
        )
        self.assertEqual(
            PackSdozAround.product_uom_id.id,
            self.uom_unit.id,
            "Wrong uom in pack operation for product SDozARound.",
        )
        ...
        self.StockPackObj.search(
            [("product_id", "=", self.kgB.id), ("picking_id", "=", picking_in_B.id)]
        ).write({"reserved_uom_qty": 0.020, "product_uom_id": self.uom_kg.id})
        self.StockPackObj.search(
            [("product_id", "=", self.gB.id), ("picking_id", "=", picking_in_B.id)]
        ).write({"reserved_uom_qty": 526, "product_uom_id": self.uom_gm.id})
        self.StockPackObj.search(
            [("product_id", "=", self.DozA.id), ("picking_id", "=", picking_in_B.id)]
        ).write({"reserved_uom_qty": 4, "product_uom_id": self.uom_dozen.id})
        self.StockPackObj.create(
            {
                "product_id": self.DozA.id,
                "reserved_uom_qty": 48,
        ...
        moves_SDozARound = self.MoveObj.search(
            [
                ("product_id", "=", self.SDozARound.id),
                ("picking_id", "=", picking_in_B.id),
            ],
        ...
        self.assertEqual(
            moves_SDozARound.product_uom_qty,
            1584,
            "Wrong move quantity (%s found instead of 1584)"
            % (moves_SDozARound.product_uom_qty),
        )
        self.assertEqual(
            moves_SDozARound.product_uom.id,
            self.uom_unit.id,
            "Wrong uom in move for product SDozARound.",
        )
        ...
        self.assertEqual(
            sum(total_qty),
            22,
            "Expecting 22 SDozenRound , got %.4f SDozenRound on location stock!"
            % (sum(total_qty)),
        )
        self.assertEqual(
            self.SDozARound.qty_available,
            22,
            "Wrong quantity available (%s found instead of 22)"
            % (self.SDozARound.qty_available),
        )
        # Check quants and available quantity for product gB.
        quants = self.StockQuantObj.search(
        ...
        SDozARound_qty = self.MoveObj.search(
            [
                ("product_id", "=", self.SDozARound.id),
                ("picking_id", "=", picking_out.id),
            ],
        ...
        self.assertEqual(
            SDozARound_qty,
            3,
            "Wrong move quantity availability (%s found instead of 3)"
            % (SDozARound_qty),
        )
        ...
        self.assertEqual(
            sum(total_qty),
            19,
            "Expecting 19 SDozRound , got %.4f SDozRound on location stock!"
            % (sum(total_qty)),
        )
        self.assertEqual(
            self.SDozARound.qty_available,
        ...
        self.assertEqual(
            move.product_uom.id, self.uom_tone.id, "Wrong unit of measure in done move."
        )
        self.assertEqual(
    ...
    def test_50_create_in_out_with_product_pack_lines(self):
        ...
        packop2 = (
            picking_in.move_line_ids[0]
            .with_context(bypass_reservation_update=True)
            .copy({"reserved_uom_qty": 0})
        )
        packop2.qty_done = 6
        packop2.result_package_id = pack2
        picking_in._action_done()
        quants = self.env["stock.quant"]._gather(
            self.productE, self.env["stock.location"].browse(self.stock_location)
        )
        self.assertEqual(
    ...
    def test_picking_form_immediate_transfer(self):
        picking_form = Form(
            self.env["stock.picking"].with_context(default_immediate_transfer=True)
        )
        ...
        with picking_form.move_ids_without_package.new() as move:
            ...
            self.assertTrue(
                move._get_modifier("forecast_availability", "column_invisible")
            )
            self.assertTrue(
                move._get_modifier("reserved_availability", "column_invisible")
            )
        ...
        picking_form = Form(
            self.env["stock.picking"].with_context(default_immediate_transfer=False)
        )
    ...
    def test_auto_assign_backorder(self):
        ...
        backorder_wizard = Form(
            self.env[action_dict["res_model"]].with_context(action_dict["context"])
        ).save()
        backorder_wizard.process()
    ...
    def test_receive_tracked_product(self):
        ...
        move_form = Form(
            receipt.move_ids, view="stock.view_stock_move_nosuggest_operations"
        )
        ...
        quant = self.productA.stock_quant_ids.filtered(
            lambda q: q.location_id.id == self.stock_location
        )
    ...
    def test_2steps_and_automatic_reservation(self):
        ...
        self.assertEqual(
            out_move.move_line_ids.reserved_qty, 1.0, "The out move should be reserved"
        )
    ...
    def test_multi_picking_validation(self):
        ...
        self.env.user.write(
            {"groups_id": [(4, self.env.ref("stock.group_reception_report").id)]}
        )
