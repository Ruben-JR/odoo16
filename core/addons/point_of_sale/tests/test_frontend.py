...
class TestPointOfSaleHttpCommon(AccountTestInvoicingHttpCommon):
    ...
    def setUpClass(cls, chart_template_ref=None):
        ...
        all_pos_product = env["product.product"].search(
        ...
        chair_fabrics_line = env["product.template.attribute.line"].create(
            {
                ...
                "attribute_id": chair_fabrics_attribute.id,
                "value_ids": [
                    (6, 0, [chair_fabrics_leather.id, chair_fabrics_other.id])
                ],
            }
        )
        ...
        env["ir.property"]._set_default(
            "property_product_pricelist", "res.partner", public_pricelist, main_company
        )
...
class TestUi(TestPointOfSaleHttpCommon):
    ...
    def test_04_product_configurator(self):
        self.main_pos_config.open_ui()
        self.start_tour(
            "/pos/ui?config_id=%d" % self.main_pos_config,
            "ProductConfiguratorTour",
            login="accountman",
        )
    ...
    def test_fixed_tax_negative_qty(self):
        ...
        self.assertEqual(
            lines[1].account_id,
            zero_amount_product.categ_id.property_account_income_categ_id,
        )
