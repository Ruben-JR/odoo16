        ...
        **kw
        ...
        def get_timesheets():
            ...
            timesheets = Timesheet_sudo.search(
                domain, order=orderby, limit=_items_per_page, offset=pager["offset"]
            )
            if field:
                if groupby == "date":
                    ...
                    grouped_timesheets = [
                        (Timesheet_sudo.browse(group["ids"]), group["unit_amount"])
            ...
            grouped_timesheets = (
                [(timesheets, sum(Timesheet_sudo.search(domain).mapped("unit_amount")))]
                if timesheets
                else []
            )
            return timesheets, grouped_timesheets
