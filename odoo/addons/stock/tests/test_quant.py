...
class StockQuant(TransactionCase):
    ...
    def test_increase_available_quantity_3(self):
        ...
        quant = self.env["stock.quant"].search(
            [("location_id", "=", self.stock_location.id)], limit=1
        )
        if not quant:
            self.skipTest("Cannot test concurrent transactions without demo data.")
        product = quant.product_id
        available_quantity = self.env["stock.quant"]._get_available_quantity(
            product, self.stock_location, allow_negative=True
        )
        ...
        with closing(self.registry.cursor()) as cr:
            ...
            self.env["stock.quant"]._update_available_quantity(
                product, self.stock_location, 1.0
            )
        ...
        self.assertEqual(
            len(self.gather_relevant(product, self.stock_location, strict=True)), 2
        )
    ...
    def test_increase_available_quantity_5(self):
        ...
        self.env["stock.quant"]._update_available_quantity(
            self.product, stock_sub_location, 1.0
        )
        ...
        self.assertEqual(
            self.env["stock.quant"]._get_available_quantity(
                self.product, stock_sub_location
            ),
            1.0,
        )

        self.env["stock.quant"]._update_available_quantity(
            product2, stock_sub_location, 1.0
        )
        self.env["stock.quant"]._update_available_quantity(
    ...
    def test_decrease_available_quantity_2(self):
        ...
        self.assertEqual(
            len(self.gather_relevant(self.product, self.stock_location)), 1
        )
    ...
    def test_decrease_available_quantity_4(self):
        ...
        self.assertEqual(
            len(self.gather_relevant(self.product, self.stock_location)), 0
        )
    ...
    def test_increase_reserved_quantity_3(self):
        ...
        self.assertEqual(
            len(self.gather_relevant(self.product, self.stock_location)), 4
        )
        self.env["stock.quant"]._update_reserved_quantity(
            self.product, self.stock_location, 10.0
        )
    ...
    def test_decrease_reserved_quantity_1(self):
        ...
        self.env["stock.quant"]._update_reserved_quantity(
            self.product, self.stock_location, -10.0, strict=True
        )
        self.assertEqual(
            self.env["stock.quant"]._get_available_quantity(
                self.product, self.stock_location
    ...
    def test_increase_decrease_reserved_quantity_1(self):
        ...
        with self.assertRaises(UserError):
            self.env["stock.quant"]._update_reserved_quantity(
                self.product, self.stock_location, -1.0, strict=True
            )
        self.assertEqual(
            self.env["stock.quant"]._get_available_quantity(
                self.product, self.stock_location
    ...
    def test_action_done_1(self):
        ...
        self.env["stock.quant"]._update_reserved_quantity(
            self.product, self.stock_location, -2.0, strict=True
        )
        self.assertEqual(
            self.env["stock.quant"]._get_available_quantity(
                self.product, self.stock_location
        ...
        self.env["stock.quant"]._update_available_quantity(
            self.product, pack_location, 2.0
        )
        self.assertEqual(
            self.env["stock.quant"]._get_available_quantity(
                self.product, pack_location
            ),
            2.0,
        )
    ...
    def test_mix_tracked_untracked_1(self):
        ...
        with self.assertRaises(UserError):
            self.env["stock.quant"]._update_reserved_quantity(
                self.product_serial, self.stock_location, -1.0, strict=True
            )
    ...
    def test_access_rights_1(self):
        ...
        quant.with_user(self.stock_user).with_context(inventory_mode=True).write(
            {"quantity": 3.0}
        )
        with self.assertRaises(AccessError):
    ...
    def test_closest_removal_strategy_tracked(self):
        ...
        self.env["stock.quant"]._update_available_quantity(
            self.product_serial, self.stock_subloc2, 1.0, lot_id=lot1, in_date=in_date
        )
        # Add a product from lot2 in stock_location/subloc3
        self.env["stock.quant"]._update_available_quantity(
            self.product_serial, self.stock_subloc3, 1.0, lot_id=lot2, in_date=in_date
        )
    ...
    def test_unpack_and_quants_merging(self):
        ...
        self.env["stock.quant"]._update_available_quantity(
            self.product, stock_location, 1.0
        )
        ...
        quant = self.env["stock.quant"].search(
            [("product_id", "=", self.product.id), ("on_hand", "=", True)]
        )
        self.assertEqual(len(quant), 1)
