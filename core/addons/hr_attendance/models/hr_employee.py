...
class HrEmployee(models.Model):
    ...
    def _compute_total_overtime(self):
        for employee in self:
            if employee.company_id.hr_attendance_overtime:
                employee.total_overtime = float_round(
                    sum(employee.overtime_ids.mapped("duration")), 2
                )
            else:
    ...
    def _compute_presence_state(self):
        ...
        for employee in employees:
            if (
                ...
                and employee.hr_presence_state == "to_define"
    ...
    def _attendance_action(self, next_action):
        ...
        if employee.user_id:
            modified_attendance = (
                employee.with_user(employee.user_id).sudo()._attendance_action_change()
            )
        else:
    ...
    def read_group(
        ...
        if (
            "pin" in groupby
            or "pin" in self.env.context.get("group_by", "")
            or self.env.context.get("no_group_by")
        ):
            raise exceptions.UserError(_("Such grouping is not allowed."))
        return super(HrEmployee, self).read_group(
            domain,
            fields,
            groupby,
            offset=offset,
    ...
    def _compute_presence_icon(self):
        ...
        self.filtered(
            lambda employee: not employee.show_hr_icon_display
