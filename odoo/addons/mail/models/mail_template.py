    ...
    def _search_template_category(self, operator, value):
        if operator in ["in", "not in"] and isinstance(value, list):
            value_templates = (
                ...
                .search([])
                .filtered(lambda t: t.template_category in value)
            )
        ...
        if operator in ["=", "!="] and isinstance(value, str):
            value_templates = (
                self.env["mail.template"]
                .search([])
                .filtered(lambda t: t.template_category == value)
            )
    ...
    def send_mail(
        ...
        if email_layout_xmlid and values["body_html"]:
            ...
            template_ctx = {
                ...
                .sudo()
                .new(dict(body=values["body_html"], record_name=record.display_name)),
