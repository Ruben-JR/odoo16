...
class PosSession(models.Model):
    ...
    def _get_pos_ui_restaurant_floor(self, params):
        ...
        table_params["search_params"]["domain"] = AND(
            [table_params["search_params"]["domain"], [("floor_id", "in", floor_ids)]]
        )
        tables = self.env["restaurant.table"].search(
            table_params["search_params"]["domain"], order="floor_id"
        )
        tables_by_floor_id = {}
        for floor_id, table_group in groupby(tables, key=lambda table: table.floor_id):
            ...
            tables_by_floor_id[floor_id.id] = floor_tables.read(
                table_params["search_params"]["fields"]
            )
