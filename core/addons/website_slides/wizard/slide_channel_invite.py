...
class SlideChannelInvite(models.TransientModel):
    ...
    def action_invite(self):
        ...
        try:
            ...
            raise AccessError(
                _(
                    "You are not allowed to add members to this course. Please contact the course responsible or an administrator."
                )
            )
    ...
    def _prepare_mail_values(self, slide_channel_partner):
        ...
        subject = self._render_field("subject", slide_channel_partner.ids)[
            slide_channel_partner.id
        ]
        body = self._render_field("body", slide_channel_partner.ids, post_process=True)[
            slide_channel_partner.id
        ]
        # post the message
        mail_values = {
        ...
        if email_layout_xmlid:
            ...
            if body:
                ...
                _logger.warning(
                    "QWeb template %s not found when sending slide channel mails. Sending without layout.",
                    email_layout_xmlid,
                )
