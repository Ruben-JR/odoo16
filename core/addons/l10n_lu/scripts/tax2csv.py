...
class LuxTaxGenerator:
    def __init__(self, filename):
        ...
        self.sheet_fiscal_pos_map = self.workbook.sheet_by_name(
    ...
    def tax_codes_to_csv(self):
        ...
        def add_new_tax_code(tax_code_id, new_name, new_parent_code):
            ...
            if parent_code and parent_code != new_parent_code:
                raise RuntimeError(
                    'tax code "%s" already exist with '
                    "parent %s while trying to add it with "
                    "parent %s" % (tax_code_id, parent_code, new_parent_code)
                )
            else:
        ...
        for row in taxes_iterator:
            ...
            if row["child_depend"] and row["amount"] != 1:
                raise RuntimeError(
                    "amount must be one if child_depend " "for %s" % row["id"]
                )
            # base parent
            ...
            if base_code != "NA":
                if row["child_depend"]:
                    raise RuntimeError(
                        "base code specified " "with child_depend for %s" % row["id"]
                    )
            if not row["child_depend"]:
                # ... in lux, we have the same code for invoice and refund
                if base_code != "NA":
                    assert row["base_code_id:id"], (
                        "missing base_code_id for %s" % row["id"]
                    )
                assert row["ref_base_code_id:id"] == row["base_code_id:id"]
                add_new_tax_code(
                    row["base_code_id:id"], "Base - " + row["name"], base_code
                )
            # tax parent
            ...
            if tax_code == "NA":
                if row["amount"] and not row["child_depend"]:
                    raise RuntimeError(
                        "TAX_CODE not specified " "for non-zero tax %s" % row["id"]
                    )
                if row["tax_code_id:id"]:
                    raise RuntimeError(
                        "tax_code_id specified " "for tax %s" % row["id"]
                    )
            else:
                if row["child_depend"]:
                    raise RuntimeError(
                        "TAX_CODE specified " "with child_depend for %s" % row["id"]
                    )
                if not row["amount"]:
                    raise RuntimeError(
                        "TAX_CODE specified " "for zero tax %s" % row["id"]
                    )
                if not row["tax_code_id:id"]:
                    raise RuntimeError(
                        "tax_code_id not specified " "for tax %s" % row["id"]
                    )
            if not row["child_depend"] and row["amount"]:
                ...
                add_new_tax_code(
                    row["tax_code_id:id"], "Taxe - " + row["name"], tax_code
                )
    ...
    def taxes_to_csv(self):
        ...
        for row in sorted(taxes_iterator, key=lambda r: r["description"]):
            ...
            if row["parent_id:id"]:
                ...
                cur_seq = seq
            writer.writerow(
                [pycompat.to_text(v) for v in list(row.values())[3:]] + [cur_seq]
            )

    def fiscal_pos_map_to_csv(self):
        writer = pycompat.csv_writer(
            open("account.fiscal." "position.tax.template-%s.csv" % self.suffix, "wb")
        )
