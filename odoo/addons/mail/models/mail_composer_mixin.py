...
class MailComposerMixin(models.AbstractModel):
    ...
    def _render_field(self, field, *args, **kwargs):
        ...
        if field not in self._fields:
            raise ValueError(
                _("The field %s does not exist on the model %s", field, self._name)
            )

        composer_value = self[field]

        if not self.template_id or self.is_mail_template_editor:
        ...
        if field == "body":
            ...
            if not self.can_edit_body or composer_value in (
                ...
                return super(MailComposerMixin, self.sudo())._render_field(
                    field, *args, **kwargs
                )
