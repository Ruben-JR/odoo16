...
class TestCompanyCheck(common.TransactionCase):
    def setUp(self):
        ...
        self.company_c = self.env["res.company"].create({"name": "Company C"})
    ...
    def test_company_check_0(self):
        ...
        m1 = self.env["test_new_api.model_child"].create(
            {"company_id": self.company_a.id}
        )
    ...
    def test_company_check_no_access(self):
        ...
        with common.Form(
            self.env["test_new_api.model_private_address_onchange"].with_user(user)
        ) as form:
