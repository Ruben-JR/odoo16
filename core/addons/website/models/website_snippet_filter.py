...
class WebsiteSnippetFilter(models.Model):
    ...
    field_names = fields.Char(
        help="A list of comma-separated field names", required=True
    )
        ...
        for record in self:
            for field_name in record.field_names.split(","):
                if not field_name.strip():
                    raise ValidationError(
                        _("Empty field name in %r") % (record.field_names)
                    )
    ...
    def _prepare_values(self, limit=None, search_domain=None):
        ...
        max_limit = (
            max(self.limit, 16)
            if self.env.context.get("_bugfix_force_minimum_max_limit_to_16")
            else self.limit
        )
