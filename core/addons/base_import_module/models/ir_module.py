    ...
    def _import_module(self, module, path, force=False):
        ...
        manifest_path = next(
            (
                ...
                if os.path.exists(opj(path, name))
            ),
            None,
        )
        if manifest_path:
        ...
        if unmet_dependencies:
            if unmet_dependencies == set(["web_studio"]) and _is_studio_custom(path):
                err = _("Studio customizations require Studio")
            else:
                err = _("Unmet module dependencies: \n\n - %s") % "\n - ".join(
                    known_mods.filtered(
                        lambda mod: mod.name in unmet_dependencies
                    ).mapped("shortdesc")
                )
            raise UserError(err)
            ...
            for filename in terp.get(kind, []):
                ...
                with _file_open(self.env, pathname) as fp:
                    if ext == ".csv":
                        ...
                        convert_xml_import(
                            self.env.cr, module, fp, idref, mode, noupdate
                        )
        ...
        if os.path.isdir(path_static):
            for root, dirs, files in os.walk(path_static):
                for static_file in files:
                    ...
                    url_path = "/{}{}".format(
                        module, full_path.split(path)[1].replace(os.path.sep, "/")
                    )
    ...
    def module_uninstall(self):
        ...
        if modules_to_delete:
            ...
            assets = self.env["ir.asset"].search(
                [("id", "in", assets_data.mapped("res_id"))]
            )
            assets.unlink()
            _logger.info(
