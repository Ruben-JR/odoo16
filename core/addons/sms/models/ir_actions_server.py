...
class ServerActions(models.Model):
    ...
    def _compute_sms_template_id(self):
        to_reset = self.filtered(
            lambda act: act.state != "sms"
            or (act.model_id != act.sms_template_id.model_id)
        )
        if to_reset:
    ...
    def _check_model_coherency(self):
        ...
        for action in self:
            if action.state == "sms" and (
                action.model_id.transient or not action.model_id.is_mail_thread
            ):
                raise ValidationError(
                    _(
                        "Sending SMS can only be done on a mail.thread or a transient model"
                    )
                )
