...
class TestWorkEntryLeave(TestWorkEntryHolidaysBase):
    ...
    def test_resource_leave_different_calendars(self):
        ...
        self.richard_emp.resource_calendar_id = self.env["resource.calendar"].create(
            {"name": "Other calendar"}
        )
        ...
        self.assertEqual(
            len(resource_leave),
            2,
            "it should have created one resource leave per calendar",
        )
        self.assertEqual(
            resource_leave.mapped("work_entry_type_id"),
            self.leave_type.work_entry_type_id,
            "they should have the corresponding work_entry type",
        )

    def test_create_mark_conflicting_work_entries(self):
        work_entry = self.create_work_entry(
            datetime(2019, 10, 10, 9, 0), datetime(2019, 10, 10, 12, 0)
        )
        self.assertNotEqual(
            work_entry.state, "conflict", "It should not be conflicting"
        )
        leave = self.create_leave(
        ...
        self.assertEqual(
            work_entry.leave_id, leave, "It should be linked to conflicting leave"
        )
    ...
    def test_write_mark_conflicting_work_entries(self):
        ...
        work_entry = self.create_work_entry(
            datetime(2019, 10, 9, 9, 0), datetime(2019, 10, 10, 9, 0)
        )  # the day before
        self.assertNotEqual(
            work_entry.state, "conflict", "It should not be conflicting"
        )
    ...
    def test_validate_leave_with_overlap(self):
        ...
        work_entry_1 = self.create_work_entry(
            datetime(2019, 10, 8, 9, 0), datetime(2019, 10, 11, 9, 0)
        )  # overlaps
        work_entry_2 = self.create_work_entry(
            datetime(2019, 10, 11, 9, 0), datetime(2019, 10, 11, 10, 0)
        )  # included
        adjacent_work_entry = self.create_work_entry(
            datetime(2019, 10, 12, 18, 0), datetime(2019, 10, 13, 18, 0)
        )  # after and don't overlap
        leave.action_validate()
        self.assertNotEqual(
            adjacent_work_entry.state, "conflict", "It should not conflict"
        )
        ...
        self.assertEqual(
            ...
            "conflict",
            "The leave work entry should conflict",
        )
    ...
    def test_refuse_leave(self):
        ...
        adjacent_work_entry = self.create_work_entry(
            datetime(2019, 10, 7, 9, 0), datetime(2019, 10, 10, 9, 0)
        )
        self.assertTrue(
            all(work_entries.mapped(lambda w: w.state == "conflict")),
            "Attendance work entries should all conflict with the leave",
        )
        self.assertNotEqual(
            adjacent_work_entry.state,
            "conflict",
            "Non overlapping work entry should not conflict",
        )
        leave.action_refuse()
        self.assertTrue(
            all(work_entries.mapped(lambda w: w.state != "conflict")),
            "Attendance work entries should no longer conflict",
        )
        self.assertNotEqual(
            adjacent_work_entry.state,
            "conflict",
    ...
    def test_refuse_approved_leave(self):
        ...
        leave_work_entry = self.richard_emp.contract_ids._generate_work_entries(
            start, end
        )
        ...
        self.assertEqual(
            len(work_entries),
            2,
            "Attendance work entries should have been re-created (morning and afternoon)",
        )
        self.assertTrue(
            all(work_entries.mapped(lambda w: w.state != "conflict")),
    ...
    def test_archived_work_entry_conflict(self):
        ...
        self.assertEqual(
            work_entry.state,
            "conflict",
            "Attendance work entries should conflict with the leave",
        )
        work_entry.toggle_active()
        self.assertEqual(
            work_entry.state,
            "cancelled",
            "Attendance work entries should be cancelled and not conflict",
        )
    ...
    def test_work_entry_cancel_leave(self):
        ...
        with freeze_time(datetime(2022, 3, 21)):
            ...
            self.richard_emp.contract_ids._generate_work_entries(
                datetime(2022, 3, 21, 6), datetime(2022, 3, 25, 20)
            )
            work_entries = self.env["hr.work.entry"].search(
                [("employee_id", "=", self.richard_emp.id)]
            )
