...
class ContractHistory(models.Model):
    ...
    structure_type_id = fields.Many2one(
        "hr.payroll.structure.type", string="Salary Structure Type", readonly=True
    )
    hr_responsible_id = fields.Many2one(
        "res.users", string="HR Responsible", readonly=True
    )
    ...
    resource_calendar_id = fields.Many2one(
        "resource.calendar", string="Working Schedule", readonly=True
    )
    ...
    contract_type_id = fields.Many2one(
        "hr.contract.type", "Contract Type", readonly=True
    )
    ...
    under_contract_state = fields.Selection(
        [("done", "Under Contract"), ("blocked", "Not Under Contract")],
        string="Contractual Status",
        compute="_compute_under_contract_state",
    )
    ...
    def _compute_under_contract_state(self):
        for history in self:
            history.under_contract_state = (
                "done" if history.is_under_contract else "blocked"
            )
