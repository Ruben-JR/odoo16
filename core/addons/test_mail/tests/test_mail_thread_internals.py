...
class TestAPI(TestMailCommon, TestRecipients):
    ...
    def test_message_update_content(self):
        ...
        ticket_record._message_update_content(
            message, "<p>New Body</p>", attachment_ids=new_attachments.ids
        )
        ...
        # void attachments
        ticket_record._message_update_content(
            message, "<p>Another Body, void attachments</p>", attachment_ids=[]
        )
    ...
    def test_message_update_content_check(self):
        ...
        with self.assertRaises(exceptions.UserError):
            ticket_record._message_update_content(message, "<p>New Body</p>")
...
class TestChatterTweaks(TestMailCommon, TestRecipients):
    ...
    def test_post_subscribe_recipients(self):
        original = self.test_record.message_follower_ids
        self.test_record.with_user(self.user_employee).with_context(
            {"mail_create_nosubscribe": True, "mail_post_autofollow": True}
        ).message_post(
            body="Test Body",
            message_type="comment",
            subtype_xmlid="mail.mt_comment",
...
class TestDiscuss(TestMailCommon, TestRecipients):
    ...
    def test_set_star(self):
        msg = self.test_record.with_user(self.user_admin).message_post(
            body="My Body", subject="1"
        )
    ...
    def test_unlink_notification_message(self):
        ...
        channel_message = (
            self.env["mail.message"]
            .sudo()
            .search([("model", "=", "mail.channel"), ("res_id", "in", channel.ids)])
        )
        self.assertEqual(
            len(channel_message), 1, "Test message should have been posted"
        )

        channel.sudo().unlink()
        remaining_message = channel_message.exists()
        self.assertEqual(
            len(remaining_message), 0, "Test message should have been deleted"
        )
