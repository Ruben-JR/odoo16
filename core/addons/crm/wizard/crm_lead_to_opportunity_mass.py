...
class Lead2OpportunityMassConvert(models.TransientModel):
    ...
    def _compute_team_id(self):
        ...
        for convert in self:
            ...
            user = (
                convert.user_id
                or convert.user_ids
                and convert.user_ids[0]
                or self.env.user
            )
            if (
                convert.team_id
                and user in convert.team_id.member_ids | convert.team_id.user_id
            ):
                continue
            team = self.env["crm.team"]._get_default_team_id(
                user_id=user.id, domain=None
            )
    ...
    def _convert_and_allocate(self, leads, user_ids, team_id=False):
        ...
        return super(Lead2OpportunityMassConvert, self)._convert_and_allocate(
            leads, salesmen_ids, team_id=team_id
        )
    ...
    def action_mass_convert(self):
        ...
        if self.name == "convert" and self.deduplicate:
            ...
            )  # only update active_ids when there are set
    ...
    def _convert_handle_partner(self, lead, action, partner_id):
        if self.action == "each_exist_or_create":
            ...
            action = "create"
        return super(Lead2OpportunityMassConvert, self)._convert_handle_partner(
            lead, action, partner_id
        )
