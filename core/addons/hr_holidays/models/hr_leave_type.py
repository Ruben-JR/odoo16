...
class HolidaysType(models.Model):
    ...
    virtual_leaves_taken = fields.Float(
        compute="_compute_leaves",
        string="Virtual Time Off Already Taken",
        help="Sum of validated and non validated time off requests.",
    )
    closest_allocation_to_expire = fields.Many2one(
        "hr.leave.allocation", "Allocation", compute="_compute_leaves"
    )
    ...
    employee_requests = fields.Selection(
        [("yes", "Extra Days Requests Allowed"), ("no", "Not Allowed")],
        default="no",
        required=True,
    ...
    allocation_validation_type = fields.Selection(
        [("officer", "Approved by Time Off Officer"), ("no", "No validation needed")],
        default="no",
        string="Approval",
        compute="_compute_allocation_validation_type",
        store=True,
        readonly=False,
    ...
    def _search_valid(self, operator, value):
        ...
        """
        date_to = self._context.get(
            "default_date_from"
        ) or fields.Date.today().strftime("%Y-1-1")
        date_from = self._context.get(
            "default_date_to"
        ) or fields.Date.today().strftime("%Y-12-31")
        employee_id = (
            self._context.get("default_employee_id", self._context.get("employee_id"))
        ...
        return [
            (
                "id",
                new_operator,
                [x["holiday_status_id"] for x in self._cr.dictfetchall()],
            )
        ]
                    ...
                    [
                        (
                            ...
                            fields.datetime.combine(
                                allocation.date_to or datetime.date.max, time.max
                            ),
                            allocation,
                        )
                        for allocation in allocations.filtered(
                            lambda allocation: allocation.employee_id.id == employee_id
                            and allocation.holiday_status_id == holiday_status_id
                        )
                    ]
                )

                allocation_employees[employee_id][
                    holiday_status_id
                    ...
                    leave_intervals = Intervals(
                        [
                            ...
                            for leave in leaves.filtered(
                                lambda leave: leave.employee_id.id == employee_id
                                and leave.holiday_status_id == holiday_status_id
                            )
                        ]
                    )
        ...
        if leaves_employees:
            for employee_id, leaves_interval_by_status in leaves_employees.items():
                for holiday_status_id in leaves_interval_by_status:
                    days_consumed = allocations_days_consumed[employee_id][
                        holiday_status_id
                    ]
                    if allocation_employees[employee_id][holiday_status_id]:
                        allocations = (
                            allocation_employees[employee_id][holiday_status_id]
                            & leaves_interval_by_status[holiday_status_id]
                        )
                        ...
                        leave_intervals = leaves_interval_by_status[
                            holiday_status_id
                        ]._items
                        sorted_allocations_with_remaining_leaves = self.env[
                            "hr.leave.allocation"
                        ]
                        for leave_interval in leave_intervals:
                            ...
                            for leave in leaves:
                                ...
                                if holiday_status_id.requires_allocation != "no":
                                    for (
                                    ...
                                    ) in sorted_available_allocations:
                                        if (
                                            available_allocation.date_to
                                            and available_allocation.date_to
                                            < leave.date_from.date()
                                        ) or (
                                            available_allocation.date_from
                                            > leave.date_to.date()
                                        ):
                                            continue
                                        virtual_remaining_leaves = (
                                            available_allocation.number_of_days
                                            if leave_unit == "days"
                                            else available_allocation.number_of_hours_display
                                        ...
                                        max_leaves = min(
                                            virtual_remaining_leaves, leave_duration
                                        )
                                        days_consumed[available_allocation][
                                            "virtual_leaves_taken"
                                        ] += max_leaves
                                        ...
                                        if (
                                            ...
                                            > 0
                                            and available_allocation.date_to
                                            and available_allocation.date_to > date
                                        ):
                                            sorted_allocations_with_remaining_leaves |= (
                                                available_allocation
                                            )
                                    if leave_duration > 0:
                        ...
                        allocations_days_consumed[employee_id][holiday_status_id][
                            ...
                            sorted_allocations_with_remaining_leaves[0]
                            if sorted_allocations_with_remaining_leaves
                            else False
                        )
            ...
            for holiday_status_id, intervals in allocation_intervals_by_status.items():
                ...
                future_allocation_intervals = intervals & Intervals(
                    [
                        (
                            ...
                            fields.datetime.combine(date, time.max)
                ...
                for future_allocation_interval in future_allocation_intervals._items:
                    if future_allocation_interval[0].date() > search_date:
                        continue
                    employee_quantity_available = future_allocation_interval[
                        2
                    ].employee_id._get_work_days_data_batch(
                    ...
                    for allocation in future_allocation_interval[2]:
                        ...
                        if future_allocation_interval[1] != fields.datetime.combine(
                            date, time.max
                        ) + timedelta(days=5 * 365):
                        ...
                        if allocation.type_request_unit in ["day", "half_day"]:
                            ...
                            remaining_days_allocation = (
                                ...
                                - days_consumed["virtual_leaves_taken"]
                            )
                        else:
                        ...
                        if quantity_available <= remaining_days_allocation:
                            search_date = future_allocation_interval[
                                1
                            ].date() + timedelta(days=1)
                        days_consumed["virtual_remaining_leaves"] += min(
                            quantity_available, remaining_days_allocation
                        )
                        days_consumed["max_leaves"] = (
                            allocation.number_of_days
                            if allocation.type_request_unit in ["day", "half_day"]
                            else allocation.number_of_hours_display
                        )
                        days_consumed["remaining_leaves"] = (
                            days_consumed["max_leaves"] - days_consumed["leaves_taken"]
                        )
                        if remaining_days_allocation >= quantity_available:
                            break
                        # Check valid allocations with still availabe leaves on it
                        if (
                            days_consumed["virtual_remaining_leaves"] > 0
                            and allocation.date_to
                            and allocation.date_to > date
                        ):
                            allocations_with_remaining_leaves |= allocation
                allocations_sorted = sorted(
                    allocations_with_remaining_leaves, key=lambda a: a.date_to
                )
    ...
    def get_employees_days(self, employee_ids, date=None):
        ...
        allocations_days_consumed = self._get_employees_days_per_allocation(
            employee_ids, date
        )
        ...
        for employee_id in allocations_days_consumed:
            for holiday_status_id in allocations_days_consumed[employee_id]:
                for allocation in allocations_days_consumed[employee_id][
                    holiday_status_id
                ]:
                    if allocation:
                        if allocation.date_to and (
                        ...
                        for leave_key in leave_keys:
                            result[employee_id][
                                holiday_status_id
                                if isinstance(holiday_status_id, int)
                                else holiday_status_id.id
                            ][leave_key] += allocations_days_consumed[employee_id][
                                holiday_status_id
                            ][
                                allocation
                            ][
                        ...
                        ]["closest_allocation_to_expire"] = allocations_days_consumed[
                            employee_id
                        ][
                            holiday_status_id
                        ][
                        ...
                        for leave_key in leave_keys:
                            if allocations_days_consumed[employee_id][
                                holiday_status_id
                            ][False].get(leave_key):
                                result[employee_id][
                                    holiday_status_id
    ...
    def get_days_all_request(self):
        leave_types = sorted(
            self.search([]).filtered(
                lambda x: ((x.virtual_remaining_leaves > 0 or x.max_leaves))
            ),
    ...
    def _get_days_request(self):
        self.ensure_one()
        result = self._get_employees_days_per_allocation(
            self.closest_allocation_to_expire.employee_id.ids
        )
        ...
        if self.closest_allocation_to_expire:
            ...
            employee_allocations = result[
                self.closest_allocation_to_expire.employee_id.id
            ][self].items()
        ...
        return (
            self.name,
            {
                "remaining_leaves": ("%.2f" % self.remaining_leaves)
                .rstrip("0")
                .rstrip("."),
                "virtual_remaining_leaves": ("%.2f" % self.virtual_remaining_leaves)
                .rstrip("0")
                .rstrip("."),
                "max_leaves": ("%.2f" % self.max_leaves).rstrip("0").rstrip("."),
                "leaves_taken": ("%.2f" % self.leaves_taken).rstrip("0").rstrip("."),
                "virtual_leaves_taken": ("%.2f" % self.virtual_leaves_taken)
                .rstrip("0")
                .rstrip("."),
                "leaves_requested": (
                    "%.2f"
                ...
                "closest_allocation_remaining": ("%.2f" % closest_allocation_remaining)
                .rstrip("0")
                .rstrip("."),
                "closest_allocation_expire": format_date(
                    self.env, self.closest_allocation_to_expire.date_to
                )
                if self.closest_allocation_to_expire.date_to
                else False,
                "request_unit": self.request_unit,
                "icon": self.sudo().icon_id.url,
            },
            self.requires_allocation,
            self.id,
        )
    ...
    def _compute_leaves(self):
        ...
        if employee_id:
            data_days = (
                self.get_employees_days(employee_id)[employee_id[0]]
                if isinstance(employee_id, list)
                else self.get_employees_days([employee_id])[employee_id]
            )
        ...
        for holiday_status in self:
            ...
            holiday_status.closest_allocation_to_expire = result.get(
                "closest_allocation_to_expire", 0
            )
    ...
    def _compute_accrual_count(self):
        accrual_allocations = self.env["hr.leave.accrual.plan"]._read_group(
            [("time_off_type_id", "in", self.ids)],
            ["time_off_type_id"],
            ["time_off_type_id"],
        )
        mapped_data = dict(
            (data["time_off_type_id"][0], data["time_off_type_id_count"])
            for data in accrual_allocations
        )
        for leave_type in self:
    ...
    def requested_name_get(self):
        return self._context.get("holiday_status_name_get", True) and self._context.get(
            "employee_id"
        )
    ...
    def name_get(self):
        ...
        for record in self:
            name = record.name
            if record.requires_allocation == "yes" and not self._context.get(
                "from_manager_leave_form"
            ):
                name = "%(name)s (%(count)s)" % {
                    "name": name,
    ...
    def _search(
        ...
        leave_ids = super(HolidaysType, self)._search(
            args,
            offset=offset,
            limit=(None if post_sort else limit),
            order=order,
            count=count,
