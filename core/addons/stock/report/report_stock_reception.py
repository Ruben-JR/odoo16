    ...
    def _get_report_values(self, docids, data=None):
        ...
        for move in move_ids:
            ...
            if move.move_dest_ids:
                qty_already_assigned = min(
                    product_to_assigned_qty[move.product_id], move.product_qty
                )
            ...
            if move.product_qty != qty_already_assigned:
                if move.state == "draft":
                    product_to_qty_draft[move.product_id] += (
                        move.product_qty - qty_already_assigned
                    )
                else:
                    ...
                    if move.picking_id.immediate_transfer:
                        ...
                        quantity_to_assign = move.product_uom._compute_quantity(
                            move.quantity_done,
            ...
            for out in outs:
                ...
                if "done" not in doc_states and out.state == "partially_available":
                    qty_to_reserve -= out.product_uom._compute_quantity(
                        out.reserved_availability, product_uom
                    )
                moves_in_ids = []
                qty_done = 0
                ...
                if float_compare(
                    qty_to_reserve, qty_done, precision_rounding=product_uom.rounding
                ) > 0 and not float_is_zero(
                    qty_expected, precision_rounding=product_uom.rounding
                ):
        ...
        for product_id, qty_and_ins in product_to_total_assigned.items():
            ...
            for out_move in out_moves:
                if float_is_zero(
                    total_assigned,
                    precision_rounding=out_move.product_id.uom_id.rounding,
                ):
        ...
        for out, qty_to_link, ins in zip(outs, qtys, in_ids):
            ...
            if out.id in out_to_new_out:
                ...
                if potential_ins[0].state != "done" and out.reserved_availability:
                    ...
                    and out.reserved_availability > qty_to_link
                ):
                    ...
                    for move_line_id in new_out.move_line_ids:
                        if assigned_amount + move_line_id.reserved_qty > qty_to_link:
                            new_move_line = move_line_id.copy(
                                {"reserved_uom_qty": 0, "qty_done": 0}
                            )
                            new_move_line.reserved_uom_qty = (
                                move_line_id.reserved_uom_qty
                            )
                            move_line_id.reserved_uom_qty = (
            ...
            for in_move in reversed(potential_ins):
                quantity_remaining = in_move.product_qty - sum(
                ...
                if (
                    ...
                    or float_compare(
                        0,
                        quantity_remaining,
                        precision_rounding=in_move.product_id.uom_id.rounding,
                    )
                    >= 0
                ):
                ...
                if float_is_zero(
                    qty_to_link, precision_rounding=out.product_id.uom_id.rounding
                ):
            ...
            if new_move_vals:
                ...
                if new_out.reserved_availability > new_out.product_qty:
                    ...
                    reserved_amount_to_remain = (
                        new_out.reserved_availability - new_out.product_qty
                    )
                    for move_line_id in new_out.move_line_ids:
            ...
            formatted.update(
                {
                    ...
                    "partner_name": source.partner_id.name
