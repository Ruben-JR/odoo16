...
class AccountJournalGroup(models.Model):
    ...
    _sql_constraints = [
        (
            ...
            "A journal group name must be unique per company.",
        ),
    ]
...
class AccountJournal(models.Model):
    ...
    def _compute_available_payment_method_ids(self):
        ...
        unique_pay_methods = [
            k for k, v in method_information.items() if v["mode"] == "unique"
        ]
        ...
        for payment_type in ("inbound", "outbound"):
            for code, vals in method_information.items():
                ...
                for journal in self.filtered_domain(method_domain):
                    protected_pay_method_ids = pay_methods_by_company.get(
                        journal.company_id._origin.id, set()
                    ...
                    pay_method_ids_commands_x_journal[journal].append(
        ...
        ) in pay_method_ids_commands_x_journal.items():
    ...
    def _constrains_account_control_ids(self):
        self.env["account.move.line"].flush_model(
            ["account_id", "journal_id", "display_type"]
        )
        ...
        if self._cr.fetchone():
            raise ValidationError(
                _(
                    "Some journal items already exist in this journal but with other accounts than the allowed ones."
                )
            )
        ...
        if method_ids:
            ...
            raise ValidationError(
                _(
                    ...
                    "(%s)",
                    ", ".join([method.display_name for method in methods]),
                )
            )
    ...
    def copy(self, default=None):
        ...
        default.update(code=copy_code, name=_("%s (copy)") % (self.name or ""))
    ...
    def _fill_missing_values(self, vals):
        ...
        if journal_type in ("bank", "cash"):
            ...
            # === Fill missing code ===
            if "code" not in vals:
                vals["code"] = self.get_next_bank_cash_default_code(
            ...
            if not has_liquidity_accounts:
                ...
                vals["default_account_id"] = (
                    self.env["account.account"].create(default_account_vals).id
                )
    ...
    def _create_document_from_attachment(self, attachment_ids=None):
        ...
        with invoices._disable_discount_precision():
            for attachment in attachments:
                decoders = self.env[
                    "account.move"
                ]._get_create_document_from_attachment_decoders()
                invoice = False
    ...
    def _get_journal_outstanding_payments_account_balance(self, domain=None, date=None):
        ...
        accounts = self._get_journal_inbound_outstanding_payment_accounts().union(
        ...
        for res in self._cr.dictfetchall():
            ...
            if (
                ...
                total_balance += company_currency._convert(
                    balance, balance_currency, self.company_id, conversion_date
                )
            else:
    ...
    def _get_last_bank_statement(self, domain=None):
        ...
        """
        self.ensure_one()
        last_statement_domain = (domain or []) + [
            ("journal_id", "=", self.id),
            ("statement_id", "!=", False),
        ]
    ...
    def _is_payment_method_available(self, payment_method_code):
        ...
        return self.filtered_domain(
            self.env["account.payment.method"]._get_payment_method_domain(
                payment_method_code
            )
        )
