...
class Message(models.Model):
    ...
    l10n_in_audit_log_preview = fields.Html(
        string="Description", compute="_compute_l10n_in_audit_log_preview"
    )
    l10n_in_audit_log_account_move_id = fields.Many2one(
        "account.move",
        ...
        for message in self:
            ...
            for value in tracking_value_ids:
                audit_log_preview += Markup(
                    "<li>%s <i class='o_TrackingValue_separator fa fa-long-arrow-rightmx-1 text-600' title='%s' role='img' aria-label='%s'></i>%s (%s)</li>"
                ) % (
                    _("Changed"),
                    _("Changed"),
                    value._get_old_display_value()[0],
                    value._get_new_display_value()[0],
    ...
    def _compute_l10n_in_audit_log_document_name(self):
        messages_of_account_move = self.filtered(
            lambda m: m.model == "account.move" and m.res_id
        )
        ...
        for message in messages_of_account_move:
            message.l10n_in_audit_log_account_move_id = moves_by_id.get(
