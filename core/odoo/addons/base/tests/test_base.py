...
class TestSafeEval(BaseCase):
    ...
    def test_01_safe_eval(self):
        ...
        self.assertEqual(
            actual, expected, "Simple python expressions are not working with safe_eval"
        )
...
class TestBase(TransactionCase):
    def _check_find_or_create(
        ...
        if should_create and check_partner:
            self.assertTrue(
                partner.id > check_partner.id,
                "find_or_create failed - should have found existing",
            )
        elif check_partner:
            self.assertEqual(
                partner,
    ...
    def test_10_res_partner_find_or_create(self):
        ...
        partner_2 = res_partner.browse(
            res_partner.name_create("sarah.john@connor.com")[0]
        )
        found_2 = self._check_find_or_create(
            "john@connor.com",
            "john@connor.com",
            "john@connor.com",
            check_partner=partner_2,
            should_create=True,
        )
    ...
    def test_15_res_partner_name_search(self):
        ...
        for name, active in DATA:
            partner_id, dummy = res_partner.with_context(
    ...
    def test_20_res_partner_address_sync(self):
        ...
        p1 = res_partner.browse(
            res_partner.name_create(
        ...
        p1.write({"phone": p1phone, "parent_id": ghoststep.id})
        self.assertEqual(p1.street, ghoststep.street, "Address fields must be synced")
        self.assertEqual(
            p1.phone, p1phone, "Phone should be preserved after address sync"
        )
        self.assertEqual(
            p1.type, "contact", "Type should be preserved after address sync"
        )
        self.assertEqual(
            p1.email,
        ...
        p1.write({"street": p1street, "type": "invoice"})
        self.assertEqual(
            p1.street,
            p1street,
        ...
        self.assertNotEqual(
            ghoststep.street, p1street, "Parent address must never be touched"
        )

        # turn on sync again
        p1.write({"type": "contact"})
        self.assertEqual(
            p1.street, ghoststep.street, "Address fields must be synced again"
        )
        self.assertEqual(
            p1.phone, p1phone, "Phone should be preserved after address sync"
        )
        ...
        self.assertEqual(
            p1.street, ghoststreet, "Address fields must be synced automatically"
        )
        ...
        self.assertEqual(
            ghoststep.street, ghoststreet, "Touching contact should never alter parent"
        )
    ...
    def test_30_res_partner_first_contact_sync(self):
        ...
        self.assertEqual(
            ironshield.street, p1.street, "Address fields should be copied to company"
        )
    ...
    def test_commercial_partner_nullcompany(self):
        ...
        self.assertEqual(
            p1.commercial_partner_id, p0, "partner's parent is their commercial partner"
        )
        p12 = P.create({"name": "12", "email": "12", "parent_id": p1.id})
        self.assertEqual(
            p12.commercial_partner_id, p0, "partner's GP is their commercial partner"
        )
    ...
    def test_50_res_partner_commercial_sync(self):
        res_partner = self.env["res.partner"]
        p0 = res_partner.create(
            {"name": "Sigurd Sunknife", "email": "ssunknife@gmail.com"}
        )
        sunhelm = res_partner.create(
            {
                "name": "Sunhelm",
                "is_company": True,
                ...
                "child_ids": [
                    Command.link(p0.id),
                    Command.create(
                        {"name": "Alrik Greenthorn", "email": "agr@sunhelm.com"}
                    ),
                ],
            }
        )
        ...
        p11 = res_partner.create(
            {"name": "Gini Graywool", "email": "ggr@sunhelm.com", "parent_id": p1.id}
        )
        ...
        sunhelm.write(
            {
                "child_ids": [
                    Command.create(
                        {"name": "Ulrik Greenthorn", "email": "ugr@sunhelm.com"}
                    )
                ]
            }
        )
        ...
        for p in (p0, p1, p11, p2, p3):
            self.assertEqual(
                p.commercial_partner_id,
                sunhelm,
                "Incorrect commercial entity resolution",
            )
            self.assertEqual(
                p.vat, sunhelm.vat, "Commercial fields must be automatically synced"
            )
        ...
        for p in (p0, p1, p11, p2, p3):
            self.assertEqual(
                p.vat,
                sunhelmvat,
                "Commercial fields must be automatically and recursively synced",
            )
        ...
        p1.write(
            {"parent_id": sunhelm.id, "is_company": True, "name": "Sunhelm Subsidiary"}
        )
        self.assertEqual(
            p1.vat,
            p1vat,
        ...
        self.assertEqual(
            p0.vat, sunhelmvat2, "Commercial fields must be automatically synced"
        )
    ...
    def test_60_read_group(self):
        ...
        groups_data = res_users.with_context(active_test=False).read_group(
            [], fields=["login"], groupby=["login"], orderby="login DESC"
        )
        self.assertGreater(
            len(groups_data), 6, "Incorrect number of results when grouping on a field"
        )

        # group on local char field with limit
        groups_data = res_users.read_group(
            domain,
            fields=["login"],
            groupby=["login"],
            orderby="login DESC",
        ...
        self.assertEqual(
            len(groups_data),
            3,
            "Incorrect number of results when grouping on a field with limit",
        )
        self.assertEqual(
            [g["login"] for g in groups_data],
            ["bob", "alice2", "alice"],
            "Result mismatch",
        )
        ...
        groups_data = res_users.read_group(
            domain, fields=["name", "color", "function"], groupby=["function", "login"]
        )
        self.assertEqual(
            len(groups_data), 3, "Incorrect number of results when grouping on a field"
        )
        self.assertEqual(
            ["5$ Wrench", "Eavesdropper", "Friend"],
            [g["function"] for g in groups_data],
            "incorrect read_group order",
        )
        for group_data in groups_data:
            self.assertIn(
                "color",
                group_data,
                "Aggregated data for the column 'color' is not present in read_group return values",
            )
            self.assertEqual(
                group_data["color"],
                3,
                "Incorrect sum for aggregated data for the column 'color'",
            )

        # group on inherited char field, reverse order
        groups_data = res_users.read_group(
            domain, fields=["name", "color"], groupby="name", orderby="name DESC"
        )
        self.assertEqual(
        ...
        groups_data = res_users.read_group(
            domain, fields=["name", "color"], groupby="name", orderby="color DESC, name"
        )
        self.assertEqual(
            len(groups_data), 4, "Incorrect number of results when grouping on a field"
        )
        self.assertEqual(
            [g["name"] for g in groups_data],
            ["Eve", "Nab", "Bob", "Alice"],
        ...
        self.assertEqual(
            [g["name_count"] for g in groups_data],
            [1, 2, 1, 2],
        ...
        groups_data = res_users.read_group(
            domain, fields=["function", "color", "date"], groupby=["date"]
        )
        self.assertEqual(
            len(groups_data), 4, "Incorrect number of results when grouping on a field"
        )
        self.assertEqual(
            [g["date"] for g in groups_data],
            ["January 2014", "September 2014", "January 2015", "March 2015"],
            "Incorrect ordering of the list",
        )
        self.assertEqual(
            [g["date_count"] for g in groups_data],
            [1, 1, 1, 3],
            "Incorrect number of results",
        )
        ...
        groups_data = res_users.read_group(
            domain, fields=["function", "color", "date"], groupby=["date:year"]
        )
        self.assertEqual(
            len(groups_data), 2, "Incorrect number of results when grouping on a field"
        )
        self.assertEqual(
            [g["date:year"] for g in groups_data],
            ["2014", "2015"],
            "Incorrect ordering of the list",
        )
        self.assertEqual(
        ...
        self.assertEqual(
            [g["date"] for g in groups_data],
            ["March 2015", "January 2015", "September 2014", "January 2014"],
            "Incorrect ordering of the list",
        )
        self.assertEqual(
            [g["date_count"] for g in groups_data],
        ...
        groups_data = res_users.read_group(
            domain, fields=["function", "color", "title"], groupby=["title"]
        )
        self.assertEqual(
            len(groups_data), 2, "Incorrect number of results when grouping on a field"
        )
        ...
        self.assertEqual(
            [g["title"] for g in groups_data],
            [(title_lady.id, "Lady..."), (title_sir.id, "Sir...")],
            "Incorrect ordering of the list",
        )
        self.assertEqual(
            [g["title_count"] for g in groups_data],
            [4, 2],
            "Incorrect number of results",
        )
        self.assertEqual(
        ...
        # m2o is returned as a (id, label) pair
        self.assertEqual(
            [(title_sir.id, "Sir..."), (title_lady.id, "Lady...")],
            [g["title"] for g in groups_data],
            "Incorrect ordering of the list",
        )
        self.assertEqual(
        ...
        self.assertEqual(
            [g["title"] for g in groups_data],
            [(title_lady.id, "Lady..."), (title_sir.id, "Sir...")],
            "Incorrect ordering of the result",
        )
        self.assertEqual(
            [g["title_count"] for g in groups_data],
            [4, 2],
    ...
    def test_61_private_read_group(self):
        ...
        # group on local char field without domain and without active_test (-> empty WHERE clause)
        groups_data = res_users.with_context(active_test=False)._read_group(
            [], fields=["login"], groupby=["login"], orderby="login DESC"
        )
        self.assertGreater(
            len(groups_data), 6, "Incorrect number of results when grouping on a field"
        ...
        # group on local char field with limit
        groups_data = res_users._read_group(
            domain,
            fields=["login"],
            groupby=["login"],
            orderby="login DESC",
        ...
        self.assertEqual(
            [g["function"] for g in groups_data],
            ["5$ Wrench", "Eavesdropper", "Friend"],
            "incorrect _read_group order",
        )
        for group_data in groups_data:
            self.assertIn(
                "color",
                group_data,
                "Aggregated data for the column 'color' is not present in _read_group return values",
            )
            self.assertEqual(
                group_data["color"],
                3,
...
class TestParentStore(TransactionCase):
    ...
    def test_duplicate_parent(self):
        ...
        self.assertFalse(
            new_struct & old_struct, "After duplication, nodes should not be mixed"
        )
    ...
    def test_duplicate_children_03(self):
        ...
        new_cat0 = self.cat0.copy({"child_ids": []})
        new_cat0.write(
            {"child_ids": [Command.link(new_cat1.id), Command.link(new_cat2.id)]}
        )
        new_struct = new_cat0.search([("parent_id", "child_of", new_cat0.id)])
        self.assertEqual(
            len(new_struct),
...
class TestGroups(TransactionCase):
    def test_res_groups_fullname_search(self):
        ...
        self.assertItemsEqual(
            groups.ids,
            [g.id for g in all_groups if "Technical" in g.full_name],
        ...
        self.assertItemsEqual(
            groups.ids,
            [g.id for g in all_groups if "Sales /" in g.full_name],
        ...
        groups = all_groups.search(
            [
                (
                    "full_name",
                    "in",
                    ["Administration / Access Rights", "Contact Creation"],
                )
            ]
        )
        self.assertTrue(
            groups,
            "did not match search for 'Administration / Access Rights' and 'Contact Creation'",
        )
    ...
    def test_remove_groups(self):
        ...
        p = self.env["res.users"].create(
            {"login": "p", "name": "P", "groups_id": [Command.set([portal.id])]}
        )
        ...
        c = self.env["res.groups"].create(
            {
                ...
                "users": [Command.set([p.id, u2.id, default.id])],
            }
        )
        d = self.env["res.groups"].create(
            {
                "name": "D",
                "implied_ids": [Command.set(a.ids)],
                "users": [Command.set([u2.id, default.id])],
            }
        )
