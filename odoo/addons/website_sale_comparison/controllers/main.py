...
class WebsiteSaleProductComparison(WebsiteSale):
    @http.route(
        "/shop/compare", type="http", auth="public", website=True, sitemap=False
    )
    ...
    def get_product_data(self, product_ids, cookies=None):
        ...
        if cookies is not None:
            ret["cookies"] = json.dumps(
                ...
                .search([("id", "in", list(set(product_ids + cookies)))])
                .ids
            )

        products = products.with_context(
            pricelist=pricelist.id, display_default_code=False
        )
