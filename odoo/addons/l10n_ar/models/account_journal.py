...
class AccountJournal(models.Model):
    ...
    def _check_afip_configurations(self):
        ...
        invoices = self.env["account.move"].search(
            [("journal_id", "in", journals.ids), ("posted_before", "=", True)], limit=1
        )
        if invoices:
            raise ValidationError(
                _(
                    "You can not change the journal's configuration if it already has validated invoices"
                )
                + " ("
    ...
    def _check_afip_pos_number(self):
        ...
        if to_review.filtered(lambda x: x.l10n_ar_afip_pos_number > 99999):
            raise ValidationError(
                _("Please define a valid AFIP POS number (5 digits max)")
            )
