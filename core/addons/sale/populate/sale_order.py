...
class SaleOrder(models.Model):
    ...
    def _populate_factories(self):
        ...
        def x_ids_by_company(recordset, with_false=True):
            x_by_company = dict(
                groupby(recordset, key=lambda x_record: x_record.company_id.id)
            )
            if with_false:
            ...
            return {
                com: (
                    self.env[recordset._name].concat(*x_records) | x_inter_company
                ).ids
                for com, x_records in x_by_company.items()
                if com
            }
        ...
        user_ids_by_company = x_ids_by_company(
