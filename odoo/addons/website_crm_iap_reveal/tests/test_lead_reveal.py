...
class TestLeadMine(TestCrmCommon, MockIAPReveal):
    ...
    def test_reveal_error_credit(self):
        ...
        self.assertEqual(
            set(self.test_views.mapped("reveal_state")), set(["to_process"])
        )
    ...
    def test_reveal_error_no_result(self):
        ...
        self.assertEqual(
            set(self.test_views.mapped("reveal_state")), set(["not_found"])
        )
    ...
    def test_reveal(self):
        ...
        for counter, base_name in enumerate(["Heinrich", "Rivil", "LidGen"]):
            ...
            lead = self._new_leads.filtered(
                lambda lead: lead.name == "%s GmbH - %s" % (base_name, rule.suffix)
            )
