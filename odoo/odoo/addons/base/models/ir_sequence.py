...
def _alter_sequence(cr, seq_name, number_increment=None, number_next=None):
    ...
    if number_increment == 0:
        raise UserError(_("Step must not be zero."))
    cr.execute(
        "SELECT relname FROM pg_class WHERE relkind=%s AND relname=%s", ("S", seq_name)
    )
    if not cr.fetchone():
...
def _update_nogap(self, number_increment):
    ...
    self._cr.execute(
        "SELECT number_next FROM %s WHERE id=%%s FOR UPDATE NOWAIT" % self._table,
        [self.id],
    )
    self._cr.execute(
        "UPDATE %s SET number_next=number_next+%%s WHERE id=%%s " % self._table,
...
class IrSequence(models.Model):
            ...
            if seq.implementation == "standard":
                if new_implementation in ("standard", None):
                    ...
                    if values.get("number_next"):
                        _alter_sequence(
                            self._cr, "ir_sequence_%03d" % seq.id, number_next=n
                        )
                    if seq.number_increment != i:
                        _alter_sequence(
                            self._cr, "ir_sequence_%03d" % seq.id, number_increment=i
                        )
                    ...
                    for sub_seq in seq.date_range_ids:
                        _drop_sequences(
                            self._cr, ["ir_sequence_%03d_%03d" % (seq.id, sub_seq.id)]
                        )
            else:
    ...
    def next_by_code(self, sequence_code, sequence_date=None):
        ...
        seq_ids = self.search(
            [("code", "=", sequence_code), ("company_id", "in", [company_id, False])],
            order="company_id",
        )
        if not seq_ids:
            _logger.debug(
...
class IrSequenceDateRange(models.Model):
    ...
    def _next(self):
        if self.sequence_id.implementation == "standard":
            number_next = _select_nextval(
                self._cr, "ir_sequence_%03d_%03d" % (self.sequence_id.id, self.id)
            )
        else:
    ...
    def unlink(self):
        _drop_sequences(
            self._cr, ["ir_sequence_%03d_%03d" % (x.sequence_id.id, x.id) for x in self]
        )
