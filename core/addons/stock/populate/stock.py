...
COMPANY_NB_WITH_STOCK = (
    3  # Need to be smaller than 5 (_populate_sizes['small'] of company)
)
    ...
    def _populate_factories(self):
        ...
        return [
            ...
            (
                "reception_steps",
                populate.iterate(
                    ["one_step", "two_steps", "three_steps"], [0.6, 0.2, 0.2]
                ),
            ),
            (
                "delivery_steps",
                populate.iterate(
                    ["ship_only", "pick_ship", "pick_pack_ship"], [0.6, 0.2, 0.2]
                ),
            ),
        ]
...
class Location(models.Model):
    ...
    def _populate(self, size):
        ...
        for company_id in company_ids:
            loc_ids_by_company[company_id] = loc_ids_by_company[company_id][
                ::-1
            ]  # Inverse the order to use pop()
            ...
            for warehouse in warehouses:
                ...
                nb_loc_to_take = min(
                    nb_loc_by_warehouse, len(loc_ids_by_company[company_id])
                )
                ...
                nb_by_level = (
                    int(math.log(nb_loc_to_take, depth)) + 1
                    if depth > 1
                    else nb_loc_to_take
                )  # number of loc to put by level

                _logger.info(
                    "Create locations (%d) tree for a warehouse (%s) - depth : %d, width : %d"
                    % (nb_loc_to_take, warehouse.code, depth, nb_by_level)
                )
                ...
                def link_next_locations(parent, level):
                    if level < depth:
                        ...
                        child_locations.location_id = (
                            parent  # Quite slow, because the ORM flush each time
                        )
...
class StockPutawayRule(models.Model):
    ...
    def _populate_factories(self):
        ...
        location_ids = (
            ...
            .browse(self.env.registry.populated_models["stock.location"])
            .filtered(lambda loc: loc.usage == "internal")
        )
...
class StockWarehouseOrderpoint(models.Model):
    ...
    def _populate_factories(self):
        ...
        location_by_warehouse = {
            ...
            .search([("id", "child_of", warehouse.lot_stock_id.id)])
            .ids
            for warehouse in warehouses
        }
        ...
        supplierinfos = self.env["product.supplierinfo"].browse(
        ...
        for suplierinfo in supplierinfos:
            products = (
                suplierinfo.product_id
                or suplierinfo.product_tmpl_id.product_variant_ids
            )
        ...
        valid_product = {
            company_id: product_ids | valid_product[False]
            for company_id, product_ids in valid_product.items()
            if company_id
        }
        invalid_product = {
            company_id: list(all_product_ids - product_ids)
            for company_id, product_ids in valid_product.items()
            if company_id
        }
        valid_product = {
        ...
        def get_location_product(iterator, field_name, model_name):
            ...
            for warehouse in warehouses:
                ...
                generator_valid_product_loc_dict[warehouse.id] = cartesian_product(
                    ...
                    + random.sample(
                        location_by_warehouse[warehouse.id],
                        len(location_by_warehouse[warehouse.id]),
                    ),
                    random.sample(
                        valid_product[warehouse.company_id.id],
                        len(valid_product[warehouse.company_id.id]),
                    ),
                )
                generator_invalid_product_loc_dict[warehouse.id] = cartesian_product(
                    ...
                    random.sample(
                        invalid_product[warehouse.company_id.id],
                        len(invalid_product[warehouse.company_id.id]),
                    ),
                )
            ...
            for values in iterator:
                ...
                if random.random() < 0.95:
                    loc_id, product_id = next(
                        generator_valid_product_loc_dict[values["warehouse_id"]]
                    )
                else:
                    loc_id, product_id = next(
                        generator_invalid_product_loc_dict[values["warehouse_id"]]
                    )
        ...
        return [
            ...
            (
                "qty_multiple",
                populate.iterate([0.0, 1.0, 2.0, 10.0], [0.4, 0.2, 0.2, 0.2]),
            ),
        ]
...
class PickingType(models.Model):
    ...
    def _populate_factories(self):
            ...
            for values in iterator:
                ...
                if values["code"] == "internal":
                    ...
                    values["default_location_dest_id"] = random.choice(
                        locations_company - inter_location
                    ).id
        ...
        def get_show_reserved(values, counter, random):
            return (
                values["show_operations"] and values["code"] != "incoming"
        ...
        return [
            ...
            (
                "code",
                populate.iterate(["incoming", "outgoing", "internal"], [0.3, 0.3, 0.4]),
            ),
            ("name", populate.compute(get_name)),
...
class Picking(models.Model):
    ...
    def _populate_factories(self):
        ...
        picking_types_ids = (
            ...
            .browse(self.env.registry.populated_models["stock.picking.type"])
            .ids
        )
        ...
        partners_by_company = {
            com: self.env["res.partner"].concat(*partners) | partners_inter_company
            for com, partners in partners_by_company.items()
            if com
        }
        ...
        def get_partner_id(values, counter, random):
            ...
            return (
                ...
                and random.choice(partners_by_company[company.id]).id
                or False
            )
        ...
        def _compute_locations(iterator, field_name, model_name):
            ...
            locations_by_company = dict(
                groupby(locations_companies, key=lambda loc: loc.company_id.id)
            )
            locations_by_company = {
            ...
            for values in iterator:
                ...
                if not source_loc or random.random() > 0.8:
                    if picking_type.code == "incoming":
                        ...
                        source_loc = random.choice(
                            locations_internal & locations_company
                        )
                ...
                if not dest_loc or random.random() > 0.8:
                    if picking_type.code == "incoming":
                        ...
                        dest_loc = random.choice(
                            (locations_internal & locations_company) - source_loc
                        )
...
class StockMove(models.Model):
        ...
        def validate_pickings(pickings, sample_ratio):
            ...
            for picking in picking_to_validate:
                ...
                for move in picking.move_ids:
                    ...
                    missing_to_do = move.product_uom._compute_quantity(
                        missing_to_do, move.product_uom, rounding_method="HALF-UP"
                    )
                            ...
                            sml_values.append(
                                dict(
                                    **move._prepare_move_line_vals(),
                                    ...
                                    package_id=package_for_picking
            ...
            _logger.info(
                "Create packages (%d) for pickings to validate" % len(package_values)
            )
            ...
            picking_to_validate.with_context(
    ...
    def _populate_factories(self):
        product_ids = (
            self.env["product.product"]
            .browse(self.env.registry.populated_models["product.product"])
            .filtered(lambda p: p.type in ("product", "consu"))
            .ids
        )
