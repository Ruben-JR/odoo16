...
class TestProjectrecurrence(TransactionCase):
    ...
    def set_task_create_date(self, task_id, create_date):
        self.env.cr.execute(
            "UPDATE project_task SET create_date=%s WHERE id=%s", (create_date, task_id)
        )
    ...
    def test_recurrence_simple(self):
        with freeze_time("2020-02-01"):
            ...
            self.assertEqual(
                task.recurrence_id.repeat_interval, 2, "recurrence should be updated"
            )
            self.assertEqual(
                task.recurrence_id.repeat_number, 11, "recurrence should be updated"
            )
    ...
    def test_recurrence_cron_repeat_after_subtasks(self):
        ...
        with freeze_time("2020-01-01"):
            ...
            self.assertEqual(
                subtask.recurrence_id.next_recurrence_date, date(2020, 1, 15)
            )
            (
                project_tasks,
                project_task_count,
                project_subtask_count,
            ...
            for task in project_tasks:
                self.assertEqual(
                    ...
                    "All tasks should have a display project id set",
                )
        ...
        with freeze_time("2020-02-15"):
            ...
            self.env["project.task.recurrence"]._cron_create_recurring_tasks()
            _, project_task_count, project_subtask_count = get_task_and_subtask_counts(
                domain
            )
            self.assertEqual(project_task_count, 4)
            self.assertEqual(project_subtask_count, 3)
    ...
    def test_recurrence_cron_repeat_after_subsubtasks(self):
        ...
        with freeze_time("2020-01-01"):
            ...
            with Form(
                recurrent_subtask.with_context(tracking_disable=True)
            ) as task_form:
            ...
            recurrent_subsubtask = recurrent_subtask.child_ids.filtered(
                lambda t: t.name == "Grandchild task 1 (recurrent)"
            )
            non_recurrent_subsubtask = recurrent_subtask.child_ids.filtered(
                lambda t: t.name == "Grandchild task 2"
            )
            with Form(
                recurrent_subsubtask.with_context(tracking_disable=True)
            ) as subtask_form:
            ...
            grand_child_task_3, grand_child_task_4, grand_child_task_5 = self.env[
                "project.task"
            ].create(
                [
            ...
            with Form(
                non_recurrent_subsubtask.with_context(tracking_disable=True)
            ) as subtask_form:
            ...
            self.assertEqual(
                recurrent_subtask.recurrence_id.next_recurrence_date, date(2020, 1, 15)
            )
            project_task_count, project_subtask_count = get_task_and_subtask_counts(
                domain
            )
        ...
        with freeze_time("2020-01-15"):
            ...
            self.assertEqual(
                project_task_count, 12 + 1 + 4
            )  # 12 + the recurring task 5 + the 2 childs (3, 4) + 1 grandchild (5) + 1 grandgrandchild (6)
            ...
            self.assertEqual(
                bottom_genealogy_name.count("Grandchild task 1 (recurrent)"), 6
            )
        ...
        self.assertEqual(bottom_genealogy_name.count("Child task 1"), 1)
        self.assertEqual(
            bottom_genealogy_name.count("Grandchild task 1 (recurrent)"), 10
        )
        self.assertEqual(bottom_genealogy_name.count("Grandchild task 5"), 1)

        for f in self.env["project.task.recurrence"]._get_recurring_fields():
            self.assertTrue(
                all_tasks[0][f] == all_tasks[1][f] == all_tasks[2][f],
                "Field %s should have been copied" % f,
            )
