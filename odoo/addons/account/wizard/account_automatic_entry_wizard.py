...
class AutomaticEntryWizard(models.TransientModel):
    ...
    destination_account_id = fields.Many2one(
        string="To", comodel_name="account.account", help="Account to transfer to."
    )
    ...
    def _check_date(self):
        for wizard in self:
            if wizard.move_line_ids.move_id._get_violated_lock_dates(
                wizard.date, False
            ):
                raise ValidationError(
                    _("The date selected is protected by a lock date")
                )
            ...
            if wizard.action == "change_period":
                for move in wizard.move_line_ids.move_id:
                    if move._get_violated_lock_dates(move.date, False):
                        raise ValidationError(
                            _(
                                "The date of some related entries is protected by a lock date"
                            )
                        )
    ...
    def default_get(self, fields):
        ...
        if any(move.state != "posted" for move in move_line_ids.mapped("move_id")):
            raise UserError(
                _("You can only change the period/account for posted journal items.")
            )
        ...
        if any(
            line.company_id != move_line_ids[0].company_id for line in move_line_ids
        ):
            raise UserError(
                _(
    ...
    def _get_move_dict_vals_change_account(self):
        ...
        for line in self.move_line_ids.filtered(
            ...
            counterpart_balances[(line.partner_id, counterpart_currency)][
                "amount_currency"
            ...
            grouped_source_lines[
                (line.partner_id, line.currency_id, line.account_id)
            ] += line
                ...
                line_vals.append(
                    {
                        ...
                        "analytic_distribution": counterpart_vals[
                            "analytic_distribution"
                        ],
                    }
                )
    ...
    def _get_move_dict_vals_change_period(self):
        ...
        move_data = {
            "new_date": {
                ...
                "ref": self._format_strings(
                    _("{label}: Adjusting Entry of {new_date}"),
                    self.move_line_ids[0].move_id,
                ),
                "date": fields.Date.to_string(self.date),
                "journal_id": self.journal_id.id,
            }
        ...
        for date, grouped_lines in groupby(
            self.move_line_ids, lambda m: m.move_id.date
        ):
            ...
            move_data[date] = {
                ...
                "line_ids": [],
                "ref": self._format_strings(
                    _("{label}: Adjusting Entry of {date}"),
                    grouped_lines[0].move_id,
                    amount,
                ),
    ...
    def _do_action_change_period(self, move_vals):
        ...
        for move in self.move_line_ids.move_id:
            ...
            move.message_post(
                body=self._format_strings(
                    _(
                        "Adjusting Entries have been created for this invoice:<ul><li>%(link1)s cancelling "
            ...
            accrual_move_messages[accrual_move] += [
                self._format_strings(
                    _(
                        "Adjusting Entry for {link}: {percent:.2f}% of {amount} recognized on {new_date}"
                    ),
                    move,
                    amount,
                )
    ...
    def _format_new_transfer_move_log(self, acc_transfer_per_move):
        format = _(
            "<li>{amount} ({debit_credit}) from {link}, <strong>%(account_source_name)s</strong></li>"
        )
        rslt = _(
            "This entry transfers the following amounts to <strong>%(destination)s</strong> <ul>",
        ...
        for move, balances_per_account in acc_transfer_per_move.items():
            for account, balance in balances_per_account.items():
                ...
                ):  # Otherwise, logging it here is confusing for the user
                    rslt += self._format_strings(format, move, balance) % {
    ...
    def _format_transfer_source_log(self, balances_per_account, transfer_move):
        transfer_format = _(
            "<li>{amount} ({debit_credit}) from <strong>%s</strong> were transferred to <strong>{account_target_name}</strong> by {link}</li>"
        )
        content = ""
        for account, balance in balances_per_account.items():
