...
class PaymentWizard(models.TransientModel):
    ...
    def _get_manual_payment_provider(self, env=None):
        ...
        return env["payment.provider"].search(
            [("module_id", "=", module_id), ("company_id", "=", env.company.id)],
            limit=1,
        )
    ...
    def _get_default_payment_provider_onboarding_value(self, key):
        ...
        self._payment_provider_onboarding_cache[
            "payment_method"
        ] = self.env.company.payment_onboarding_payment_method
        ...
        if "payment_paypal" in installed_modules:
            ...
            self._payment_provider_onboarding_cache["paypal_email_account"] = (
                provider["paypal_email_account"] or self.env.user.email or ""
            )
            self._payment_provider_onboarding_cache["paypal_pdt_token"] = provider[
                "paypal_pdt_token"
            ]
        ...
        self._payment_provider_onboarding_cache["manual_post_msg"] = manual_payment[
            "pending_msg"
        ]
        self._payment_provider_onboarding_cache["journal_name"] = (
            journal.name if journal.name != "Bank" else ""
        )
