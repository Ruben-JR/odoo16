...
class LoyaltyRule(models.Model):
    ...
    def default_get(self, fields_list):
        ...
        if "program_type" in self.env.context:
            ...
            if (
                ...
                and isinstance(
                    program_default_values[program_type]["rule_ids"][1][2], dict
                )
            ):
                result.update(
                    {
                        k: v
                        for k, v in program_default_values[program_type]["rule_ids"][1][
                            2
                        ].items()
                        if k in fields_list
                    }
    ...
    def _get_valid_product_domain(self):
        ...
        if self.product_tag_id:
            domain = expression.OR(
                [domain, [("all_product_tag_ids", "in", self.product_tag_id.id)]]
            )
