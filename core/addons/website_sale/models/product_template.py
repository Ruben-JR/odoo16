...
class ProductTemplate(models.Model):
    ...
    def _get_sales_prices(self, pricelist):
        ...
        for template in self:
            ...
            product_taxes = template.sudo().taxes_id.filtered(
                lambda t: t.company_id == t.env.company
            )
            ...
            if base_price and base_price != price_reduce:
                if not template.compare_list_price:
                    ...
                    base_price = taxes.compute_all(
                        base_price, pricelist.currency_id, 1, template, partner_sudo
                    )[tax_display]
                template_price_vals["base_price"] = base_price
            template_price_vals["price_reduce"] = self.env[
                "account.tax"
            ]._fix_tax_included_price_company(
            ...
            template_price_vals["price_reduce"] = taxes.compute_all(
                template_price_vals["price_reduce"],
    ...
    def _get_combination_info(
        ...
        if self.env.context.get("website_id"):
            ...
            prevent_zero_price_sale = (
                not price and current_website.prevent_zero_price_sale
            )
            combination_info.update(
                base_unit_name=product.base_unit_name,
                base_unit_price=product.base_unit_count
                and list_price / product.base_unit_count,
                price=price,
    ...
    def _price_with_tax_computed(
        ...
        show_tax_excluded = self.user_has_groups(
        ...
        return taxes.compute_all(price, pricelist.currency_id, 1, product, partner)[
            tax_display
        ]
    ...
    def _get_website_ribbon(self):
        ...
        return (
            self.product_tag_ids.ribbon_id[:1]
            or self.product_variant_ids.additional_product_tag_ids.ribbon_id[:1]
        )
