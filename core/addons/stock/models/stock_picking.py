...
class PickingType(models.Model):
    ...
    return_picking_type_id = fields.Many2one(
        "stock.picking.type", "Operation Type for Returns", check_company=True
    )
    ...
    show_operations = fields.Boolean(
        ...
        default=_default_show_operations,
    ...
    def _name_search(
        ...
        if len(parts) == 2:
            domain = [
                ("warehouse_id.name", operator, parts[0]),
    ...
    def _check_default_location(self):
        for record in self:
            if (
                ...
                and record.default_location_dest_id.scrap_location
            ):
...
class Picking(models.Model):
    ...
    name = fields.Char(
        "Reference", default="/", copy=False, index="trigram", readonly=True
    )
    origin = fields.Char(
        "Source Document",
        index="trigram",
    ...
    move_ids = fields.One2many(
        "stock.move", "picking_id", string="Stock Moves", copy=True
    )
    move_ids_without_package = fields.One2many(
        "stock.move",
        "picking_id",
        string="Stock moves not in package",
    ...
    product_id = fields.Many2one(
        "product.product", "Product", related="move_ids.product_id", readonly=True
    )
        ...
        for picking in self:
            ...
            if any(
                ...
                and not float_is_zero(
                    m.reserved_availability, precision_rounding=m.product_uom.rounding
                )
    ...
    def _compute_delay_alert_date(self):
        delay_alert_date_data = self.env["stock.move"]._read_group(
            [("id", "in", self.move_ids.ids), ("delay_alert_date", "!=", False)],
            ["delay_alert_date:max"],
            "picking_id",
        )
        delay_alert_date_data = {
            data["picking_id"][0]: data["delay_alert_date"]
    ...
    def _compute_products_availability(self):
        ...
        for picking in pickings:
            ...
            if any(
                ...
                forecast_date = max(
                    picking.move_ids.filtered("forecast_expected_date").mapped(
                ...
                if forecast_date:
                    ...
                    picking.products_availability_state = (
                        ...
                        and picking.scheduled_date < forecast_date
                        else "expected"
                    )
    ...
    def _compute_show_operations(self):
        for picking in self:
            ...
            if picking.picking_type_id.show_operations:
                if (
                    picking.state == "draft" and picking.immediate_transfer
    ...
    def _compute_json_popover(self):
        picking_no_alert = self.filtered(
            lambda p: p.state in ("done", "cancel") or not p.delay_alert_date
        )
            ...
            picking.json_popover = json.dumps(
                {
                    ...
                    "delay_alert_date": format_datetime(
                        self.env, picking.delay_alert_date, dt_format=False
                    ),
                    "late_elements": [
                        {
                            "id": late_move.id,
    ...
    @api.depends(
        "move_type", "immediate_transfer", "move_ids.state", "move_ids.picking_id"
    )
    ...
    def _compute_has_packages(self):
        ...
        cnt_by_picking = self.env["stock.move.line"]._read_group(
            domain, ["picking_id"], ["picking_id"]
        )
        cnt_by_picking = {
            d["picking_id"][0]: d["picking_id_count"] for d in cnt_by_picking
        }
        for picking in self:
    ...
    def onchange_partner_id(self):
        for picking in self:
            picking_id = (
                isinstance(picking.id, int)
                and picking.id
                or getattr(picking, "_origin", False)
                and picking._origin.id
            )
            if picking_id:
    ...
    def _onchange_picking_type(self):
        ...
        if self.partner_id and self.partner_id.picking_warn:
            if (
                self.partner_id.picking_warn == "no-message"
                and self.partner_id.parent_id
            ):
                partner = self.partner_id.parent_id
            elif (
                self.partner_id.picking_warn not in ("no-message", "block")
    ...
    def _onchange_locations(self):
        ...
        if any(
            line.reserved_qty or line.qty_done for line in self.move_ids.move_line_ids
        ):
            return {
                "warning": {
    ...
    def write(self, vals):
        if vals.get("picking_type_id") and any(
            picking.state != "draft" for picking in self
        ):
            raise UserError(
                _(
    ...
    def should_print_delivery_address(self):
        ...
        return (
            self.move_ids
            and self.move_ids[0].partner_id
    ...
    def _send_confirmation_email(self):
        for stock_pick in self.filtered(
            ...
            delivery_template_id = (
                stock_pick.company_id.stock_mail_confirmation_template_id.id
            )
            stock_pick.with_context(force_send=True).message_post_with_template(
                delivery_template_id, email_layout_xmlid="mail.mail_notification_light"
            )
    ...
    @api.depends(
        ...
        "move_ids.move_line_ids.package_level_id",
    )
            ...
            for move in self.move_ids:
                if not move.package_level_id:
                    if (
                        move.state == "assigned"
                        and move.picking_id
                        and not move.picking_id.immediate_transfer
                        or move.state == "done"
                    ):
    ...
    def _check_move_lines_map_quant_package(self, package):
        return package._check_move_lines_map_quant(
            self.move_line_ids.filtered(lambda ml: ml.package_id == package),
            "reserved_qty",
        )
    ...
    def _sanity_check(self, separate_pickings=True):
        ...
        pickings_without_quantities = self.filtered(
            lambda p: p.id in no_quantities_done_ids
            and p.id in no_reserved_quantities_ids
        )
    ...
    def button_validate(self):
        ...
        if (
            self.user_has_groups("stock.group_reception_report")
                ...
                wh_location_ids = self.env["stock.location"]._search(
                    [
                        (
                            "id",
                            "child_of",
                            self.picking_type_id.warehouse_id.view_location_id.ids,
                        ),
                        ("usage", "!=", "supplier"),
                    ]
                )
    ...
    def _autoconfirm_picking(self):
        ...
        for picking in self:
            ...
            if not picking.move_ids and not picking.package_level_ids:
                continue
            if picking.immediate_transfer or any(
                move.additional for move in picking.move_ids
            ):
    ...
    def _put_in_pack(self, move_line_ids, create_package_level=True):
        ...
        for pick in self:
            ...
            if not package.package_type_id:
                package_type = (
                    move_lines_to_pack.move_id.product_packaging_id.package_type_id
                )
            ...
            if len(move_lines_to_pack) == 1:
                ...
                move_lines_to_pack.location_dest_id = (
                ...
                package_level = self.env["stock.package_level"].create(
                    {
                        ...
                        "picking_id": pick.id,
                        "location_id": False,
                        "location_dest_id": move_lines_to_pack.mapped(
                            "location_dest_id"
                        ).id,
                        "move_line_ids": [(6, 0, move_lines_to_pack.ids)],
    ...
    def button_scrap(self):
        ...
        for move in self.move_ids:
            if move.state not in ("draft", "cancel") and move.product_id.type in (
                "product",
                "consu",
            ):
