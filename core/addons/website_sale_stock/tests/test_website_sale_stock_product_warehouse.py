...
class TestWebsiteSaleStockProductWarehouse(TestSaleProductAttributeValueCommon):
    ...
    def test_01_get_combination_info(self):
        ...
        for wh, qty_a, qty_b in [
            (self.warehouse_1, 10, 0),
            (self.warehouse_2, 15, 10),
            (False, 10, 0),
        ]:
            # set warehouse_id
            ...
            combination_info = product.product_tmpl_id.with_context(
            ...
            self.assertEqual(
                ...
                qty_a,
                "%s units of Product A should be available in warehouse %s"
                % (qty_a, wh),
            )
            ...
            self.assertEqual(
                combination_info["free_qty"],
                qty_b,
                "%s units of Product B should be available in warehouse %s"
                % (qty_b, wh),
            )
    ...
    def test_02_update_cart_with_multi_warehouses(self):
        ...
        with MockRequest(self.env, website=self.website, sale_order_id=so.id):
            ...
            values = so._cart_update(
                product_id=self.product_A.id, line_id=so.order_line.id, set_qty=20
            )
