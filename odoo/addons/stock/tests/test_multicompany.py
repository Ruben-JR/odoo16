...
class TestMultiCompany(TransactionCase):
    ...
    def test_move_1(self):
        ...
        Company B.
        """
        product = self.env["product.product"].create({"name": "p1", "type": "product"})
    ...
    def test_intercom_lot_push(self):
        ...
        self.assertFalse(
            move_push in move_to_transit.move_dest_ids,
        ...
        self.assertEqual(
            self.env["stock.quant"]._get_available_quantity(
                product_lot, intercom_location, lot_1
            ),
            1.0,
        )
        ...
        self.assertEqual(
            self.env["stock.quant"]._get_available_quantity(
                product_lot, self.stock_location_b, lot_2
            ),
            1.0,
        )
    ...
    def test_intercom_lot_pull(self):
        ...
        self.assertEqual(
            self.env["stock.quant"]._get_available_quantity(
                product_lot, intercom_location, lot_b
            ),
            1.0,
        )
        with self.assertRaises(UserError):
