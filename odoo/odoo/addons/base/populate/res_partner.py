...
class Partner(models.Model):
    ...
    def _populate_factories(self):
        ...
        address_factories_groups = [
            ...
            [  # US, 3 records
                (
                    "street",
                    populate.iterate(["Main street", "3th street {counter}", False]),
                ),
                (
                    "street2",
                    populate.iterate(
                        [False, "", "Behind the tree {counter}"], [90, 5, 5]
                    ),
                ),
                (
                    "city",
                    populate.randomize(["Sans Fransisco", "Los Angeles", "", False]),
                ),
            ...
            [  # other corner cases, 4 records
                ...
                (
                    "country_id",
                    populate.randomize(
                        [False] + self.env["res.country"].search([]).ids
                    ),
                ),
            ],
        ]
        ...
        def get_name(values=None, counter=0, **kwargs):
            ...
            return "%s_%s_%s" % (
                "company" if is_company else "partner",
                int(complete),
                counter,
            )
        ...
        return [
            ...
            (
                ...
                populate.randomize(
                    [
                        tz
                        for tz in self.env["res.partner"]
                        ._fields["tz"]
                        .get_values(self.env)
                    ]
                ),
            ...
            (
                "ref",
                populate.randomize(
                    [False, "", "{counter}", "p-{counter}"], [10, 10, 30, 50]
                ),
            ),
            (
                "industry_id",
            ...
            (
                "parent_id",
                populate.constant(False),
            ),  # will be setted in _populate override
        ]
    ...
    def _populate_set_companies(self, records):
        ...
        companies = records.filtered(
            lambda p: p.is_company and r_company.getrandbits(1)
        )  # 50% change to have partners
        partners = records.filtered(
            lambda p: not p.is_company and r_partner.getrandbits(1)
        )  # 50% change to have a company
        ...
        for count, (company, partners) in enumerate(
            companies_partners.items(), start=1
        ):
            if count % 100 == 0:
