...
class AnalyticMixin(models.AbstractModel):
    ...
    analytic_distribution_search = fields.Json(
        store=False, search="_search_analytic_distribution"
    )
    ...
    def _search_analytic_distribution(self, operator, value):
        if operator not in ["=", "!=", "ilike", "not ilike"] or not isinstance(
            value, (str, bool)
        ):
            raise UserError(_("Operation not supported"))
        operator_name_search = "=" if operator in ("=", "!=") else "ilike"
        account_ids = list(
            self.env["account.analytic.account"]._name_search(
                name=value, operator=operator_name_search
            )
        )
    ...
    def _search(
        ...
        for arg in args:
            if (
                ...
                and arg[0] == "analytic_distribution"
        ...
        if self.env.context.get("validate_analytic", False):
            ...
            for analytic_account_id, percentage in (
                ...
                root_plan = (
                    ...
                    .browse(int(analytic_account_id))
                    .root_plan_id
                )
