...
class Project(models.Model):
    ...
    timesheet_ids = fields.One2many(
        "account.analytic.line", "project_id", "Associated Timesheets"
    )
    ...
    is_internal_project = fields.Boolean(
        compute="_compute_is_internal_project", search="_search_is_internal_project"
    )
    remaining_hours = fields.Float(
        compute="_compute_remaining_hours",
        string="Remaining Invoiced Time",
        compute_sudo=True,
    )
    ...
    def _compute_allow_timesheets(self):
        without_account = self.filtered(
            lambda t: not t.analytic_account_id and t._origin
        )
        ...
        for project in self:
            if project.allow_timesheets and not project.analytic_account_id:
                raise ValidationError(
                    _("You cannot use timesheets without an analytic account.")
                )
    ...
    def _compute_timesheet_count(self):
        ...
        timesheet_project_map = {
            project_info["project_id"][0]: project_info["project_id_count"]
            for project_info in timesheet_read_group
        }
        for project in self:
    ...
    def create(self, vals_list):
        ...
        for vals in vals_list:
            allow_timesheets = vals.get(
                "allow_timesheets", defaults.get("allow_timesheets")
            )
            analytic_account_id = vals.get(
                "analytic_account_id", defaults.get("analytic_account_id")
            )
    ...
    def name_get(self):
        ...
        for project in self:
            if project.is_internal_project:
                name_mapping[
                    project.id
                ] = f"{name_mapping[project.id]} - {project.company_id.name}"
        return list(name_mapping.items())

    @api.model
    def _init_data_analytic_account(self):
        self.search(
            [("analytic_account_id", "=", False), ("allow_timesheets", "=", True)]
...
class Task(models.Model):
    ...
    analytic_account_active = fields.Boolean(
        ...
        compute="_compute_analytic_account_active",
        compute_sudo=True,
    )
    allow_timesheets = fields.Boolean(
        "Allow timesheets",
    ...
    effective_hours = fields.Float(
        "Hours Spent", compute="_compute_effective_hours", compute_sudo=True, store=True
    )
    ...
    encode_uom_in_days = fields.Boolean(
        compute="_compute_encode_uom_in_days", default=lambda self: self._uom_in_days()
    )
    ...
    def _compute_effective_hours(self):
        if not any(self._ids):
            for task in self:
                task.effective_hours = round(
                    sum(task.timesheet_ids.mapped("unit_amount")), 2
                )
            return
        timesheet_read_group = self.env["account.analytic.line"].read_group(
            [("task_id", "in", self.ids)], ["unit_amount", "task_id"], ["task_id"]
        )
        timesheets_per_task = {
    ...
    def _search_remaining_hours_percentage(self, operator, value):
        if operator not in OPERATOR_MAPPING:
            raise NotImplementedError(
                _("This operator %s is not supported in this search method.", operator)
            )
        query = f"""
            SELECT id
              FROM {self._table}
    ...
    def write(self, values):
        ...
        if (
            ...
            raise UserError(
                _(
                    "This task must be part of a project because there are some timesheets linked to it."
                )
            )
        ...
        if self.env.context.get("hr_timesheet_display_remaining_hours"):
            ...
            for task in self:
                if (
                    ...
                    name_mapping[task.id] = (
                        name_mapping.get(task.id, "") + "\u00A0" + hours_left
                    )
            return list(name_mapping.items())
