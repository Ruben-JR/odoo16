...
class AccountChartTemplate(models.Model):
    ...
    def _load_template(
        ...
        if self == self.env.ref("l10n_sa.sa_chart_template_standard"):
            ifrs_journal_id = self.env["account.journal"].search(
                [("company_id", "=", company.id), ("code", "=", "IFRS")], limit=1
            )
            if ifrs_journal_id:
                ifrs_account_ids = [
                    self.env.ref("l10n_sa.sa_account_100101").id,
            ...
            zakat_journal_id = self.env["account.journal"].search(
                [("company_id", "=", company.id), ("code", "=", "ZAKAT")], limit=1
            )
            if zakat_journal_id:
                zakat_account_ids = [
                    self.env.ref("l10n_sa.sa_account_201019").id,
                    self.env.ref("l10n_sa.sa_account_400072").id,
                ]
                zakat_accounts = self.env["account.account"].browse(
