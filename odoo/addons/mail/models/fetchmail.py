...
IMAP4._create_socket = lambda self, timeout=MAIL_TIMEOUT: socket.create_connection(
    (self.host or None, self.port), timeout
)
...
class FetchmailServer(models.Model):
    ...
    user = fields.Char(
        string="Username", readonly=True, states={"draft": [("readonly", False)]}
    )
...
odoo_mailgate: "|/path/to/odoo-mailgate.py --host=localhost -u %(uid)d -p PASSWORD -d %(dbname)s"
    ...
    def _update_cron(self):
        ...
        try:
            ...
            cron.toggle(
                model=self._name,
                domain=[("state", "=", "done"), ("server_type", "!=", "local")],
            )
        except ValueError:
            pass
