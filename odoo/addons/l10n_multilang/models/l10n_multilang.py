...
class AccountChartTemplate(models.Model):
    ...
    def _load(self, company):
        ...
        for chart_template in self.filtered("spoken_languages"):
            ...
            if module and module.state == "installed":
                ...
                if langs:
                    chart_template._process_single_company_coa_translations(
                        company.id, langs
                    )
        return res
    ...
    def process_translations(self, langs, in_field, in_ids, out_ids):
        ...
        """
        for lang in langs:
            for in_id, out_id in zip(
                in_ids.with_context(lang=lang), out_ids.with_context(lang=lang)
            ):
        ...
        for chart_template_id in self:
            ...
            if langs:
                ...
                for company in company_ids:
                    chart_template_id._process_single_company_coa_translations(
                        company.id, langs
                    )
        return True
    ...
    def _process_fiscal_pos_translations(self, company_id, langs, field):
        in_ids, out_ids = self._get_template_from_model(
            company_id, "account.fiscal.position"
        )
        return self.process_translations(langs, field, in_ids, out_ids)
    ...
    def _get_template_from_model(self, company_id, model):
        ...
        for module, out_data in grouped_out_data.items():
            ...
            expected_in_xml_id_names = {
                xml_id.name.partition(str(company_id) + "_")[-1]: xml_id
                for xml_id in out_data
            }
            ...
            for name, xml_id in expected_in_xml_id_names.items():
                ...
                if name not in in_xml_ids:
                    continue
                in_records += self.env[model + ".template"].browse(
...
class BaseLanguageInstall(models.TransientModel):
    ...
    def lang_install(self):
        ...
        for coa in self.env["account.chart.template"].search(
            ...
            if coa_langs_codes:
                ...
                for company in self.env["res.company"].search(
                    ...
                    # write account.account translations in the real COA
                    coa.sudo()._process_accounts_translations(
                        company.id, coa_langs_codes, "name"
                    )
                    # write account.group translations
                    coa.sudo()._process_account_group_translations(
                        company.id, coa_langs_codes, "name"
                    )
                    # copy account.tax name translations
                    coa.sudo()._process_taxes_translations(
                        company.id, coa_langs_codes, "name"
                    )
                    # copy account.tax description translations
                    coa.sudo()._process_taxes_translations(
                        company.id, coa_langs_codes, "description"
                    )
                    # copy account.fiscal.position translations
                    coa.sudo()._process_fiscal_pos_translations(
                        company.id, coa_langs_codes, "name"
                    )
