...
class TestWiseOperator(TransactionCase):
    def test_wise_operator(self):
        ...
        self.assertEqual(
            len(records.ids), 3, "The number of quants created is not correct"
        )
        ...
        self.assertEqual(
            set(pack_ids2.mapped("location_id.id")),
            set([self.shelf1.id, self.shelf2.id]),
        )

        # put the move lines from delivery_order_wise2 into delivery_order_wise1
        for pack_id2 in pack_ids2:
            new_pack_id1 = pack_id2.copy(
                default={"picking_id": delivery_order_wise1.id, "move_id": move1.id}
            )
            new_pack_id1.qty_done = pack_id2.reserved_qty

        new_move_lines = delivery_order_wise1.move_line_ids.filtered(
            lambda p: p.qty_done
        )
        self.assertEqual(sum(new_move_lines.mapped("reserved_qty")), 0)
        self.assertEqual(sum(new_move_lines.mapped("qty_done")), 5)
        self.assertEqual(
            set(new_move_lines.mapped("location_id.id")),
            set([self.shelf1.id, self.shelf2.id]),
        )

        # put the move line from delivery_order_wise1 into delivery_order_wise2
        new_pack_id2 = pack_ids1.copy(
            default={"picking_id": delivery_order_wise2.id, "move_id": move2.id}
        )
        new_pack_id2.qty_done = pack_ids1.reserved_qty

        new_move_lines = delivery_order_wise2.move_line_ids.filtered(
            lambda p: p.qty_done
        )
        ...
        self.assertEqual(
            len(records.ids), 0, "This should not have created a negative quant"
        )

        # Check the other delivery order has changed its state back to ready
        self.assertEqual(
            delivery_order_wise2.state,
            "assigned",
            "Delivery order 2 should be back in ready state",
        )
        ...
        self.assertTrue(
            all(
                [
                    x.location_id.id == self.ref("stock.stock_location_customers")
                    and x.quantity > 0.0
                    or x.location_id.id != self.ref("stock.stock_location_customers")
                    and x.quantity == 0.0
                    for x in records
                ]
            ),
