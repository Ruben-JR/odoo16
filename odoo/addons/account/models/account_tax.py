...
class AccountTax(models.Model):
    ...
    real_amount = fields.Float(
        string="Real amount to apply", compute="_compute_real_amount", store=True
    )
    ...
    def _check_children_scope(self):
        for tax in self:
            ...
            if any(
                ...
                or child.tax_scope != tax.tax_scope
    ...
    def _search(
        ...
        return super(AccountTax, self)._search(
    ...
    @api.depends(
        "invoice_repartition_line_ids", "amount", "invoice_repartition_line_ids.factor"
    )
    ...
    def _compute_amount(
        ...
        if self.amount_type == "fixed":
            ...
            if base_amount:
                return (
                    math.copysign(quantity, base_amount)
                    * self.amount
        ...
        if self.amount_type == "division" and not price_include:
            return (
                base_amount / (1 - self.amount / 100) - base_amount
                if (1 - self.amount / 100)
                else 0.0
            )
    ...
    def json_friendly_compute_all(
        ...
        is_refund = (
            is_refund
            or (tax_type == "sale" and price_unit < 0)
            or (tax_type == "purchase" and price_unit > 0)
        )

        rslt = self.with_context(caba_no_transition_account=True).compute_all(
            price_unit,
    ...
    def compute_all(
        ...
        total_excluded = currency.round(
            recompute_base(
                base, incl_fixed_amount, incl_percent_amount, incl_division_amount
            )
        )
    ...
    def _convert_to_tax_line_dict(
        ...
        return {
            ...
            or self.env["account.tax.repartition.line"],
    ...
    def _get_generation_dict_from_base_line(self, line_vals, tax_vals):
        ...
        tax_account = (
            tax_repartition_line._get_aml_target_tax_account() or line_vals["account"]
        )
        return {
            ...
            "analytic_distribution": line_vals["analytic_distribution"]
            if tax_vals["analytic"]
            else {},
        }
    ...
    def _aggregate_taxes(
        ...
        def add_tax_values(
            record, results, grouping_key, serialized_grouping_key, tax_values
        ):
        ...
        grouping_key_generator = (
            grouping_key_generator or default_grouping_key_generator
        )
        ...
        for base_line, to_update_vals, tax_values_list in to_process:
            ...
            global_tax_details["base_amount_currency"] += to_update_vals[
                "price_subtotal"
            ]

            currency = base_line["currency"] or self.env.company.currency_id
            base_amount = currency.round(
                to_update_vals["price_subtotal"] / base_line["rate"]
            )
            global_tax_details["base_amount"] += base_amount

            for tax_values in tax_values_list:
                if filter_tax_values_to_apply and not filter_tax_values_to_apply(
                ...
                if (
                    ...
                    not in global_tax_details["tax_details_per_record"][record]
                ):
                    record_global_tax_details = global_tax_details[
                        "tax_details_per_record"
                    ][record]
                    record_global_tax_details["base_amount_currency"] = to_update_vals[
                        "price_subtotal"
                    ]
    ...
    def _compute_taxes(
        ...
        for base_line in base_lines:
            ...
            currency = base_line["currency"] or self.env.company.currency_id
            res["totals"][currency]["amount_untaxed"] += to_update_vals[
                "price_subtotal"
            ]
        ...
        for line_vals in tax_lines or []:
            grouping_key = frozendict(
                self._get_generation_dict_from_tax_line(line_vals)
            )
        ...
        for tax_detail in global_tax_details["tax_details"].values():
            ...
            if tax_lines is not None:
                matched_tax_lines = [
                    x
                    for x in tax_lines
                    if (x["group_tax"] or x["tax_repartition_line"].tax_id).tax_group_id
...
class AccountTaxRepartitionLine(models.Model):
    ...
    def _on_change_account_id(self):
        if not self.account_id or self.repartition_type == "base":
            ...
            self.use_in_tax_closing = self.account_id.internal_group not in (
                "income",
                "expense",
            )
    ...
    def validate_tax_template_link(self):
        for record in self:
            if record.invoice_tax_id and record.refund_tax_id:
                raise ValidationError(
                    _(
                        "Tax distribution lines should apply to either invoices or refunds, not both at the same time. invoice_tax_id and refund_tax_id should not be set together."
                    )
                )
    ...
    def validate_tags_country(self):
        for record in self:
            if (
                record.tag_ids.country_id
                and record.tax_id.country_id != record.tag_ids.country_id
            ):
                raise ValidationError(
                    _(
                        "A tax should only use tags from its country. You should use another tax and a fiscal position if you wish to uses the tags from foreign tax reports."
                    )
                )
    ...
    def _get_aml_target_tax_account(self):
        ...
        """
        self.ensure_one()
        if self.tax_id.tax_exigibility == "on_payment" and not self._context.get(
