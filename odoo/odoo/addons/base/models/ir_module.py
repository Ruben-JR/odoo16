...
class ModuleCategory(models.Model):
    ...
    def _compute_module_nr(self):
        ...
        for cat in self.filtered("id"):
            cr.execute(
                "SELECT id FROM ir_module_category WHERE parent_id=%s", (cat.id,)
            )
            cat.module_nr = sum(
                [result.get(c, 0) for (c,) in cr.fetchall()], result.get(cat.id, 0)
            )
...
class Module(models.Model):
    ...
    def _get_desc(self):
        for module in self:
            ...
            )  # avoid to log warning for fake community module
            if module_path:
                path = modules.check_resource_path(
    ...
    dependencies_id = fields.One2many(
        "ir.module.module.dependency", "module_id", string="Dependencies", readonly=True
    )
    exclusion_ids = fields.One2many(
        "ir.module.module.exclusion", "module_id", string="Exclusions", readonly=True
    )
    ...
    state = fields.Selection(
        STATES, string="Status", default="uninstallable", readonly=True, index=True
    )
    ...
    def _state_update(self, newstate, states_to_update, level=100):
        ...
        for module in self:
            ...
            update_demo = update_mods._state_update(
                newstate, states_to_update, level=level - 1
            )
            module_demo = (
                module.demo or update_demo or any(mod.demo for mod in ready_mods)
            )
    ...
    def downstream_dependencies(
        ...
        self._cr.execute(
            query,
            (tuple(self.ids), tuple(exclude_states), tuple(known_deps.ids or self.ids)),
        )
    ...
    def button_uninstall(self):
        un_installable_modules = set(odoo.conf.server_wide_modules) & set(
            self.mapped("name")
        )
    ...
    def install_from_urls(self, urls):
        ...
        if not os.access(ad_dir, os.W_OK):
            msg = (
                _("Automatic install of downloaded Apps is currently disabled.")
                + "\n\n"
                + _(
        ...
        try:
            ...
            for module_name, url in urls.items():
                ...
                try:
                    _logger.info(
                        "Downloading module `%s` from OpenERP Apps", module_name
                    )
                    ...
                    raise UserError(
                        _(
                            "The `%s` module appears to be unavailable at the moment, please try again later.",
                            module_name,
                        )
                    )
                else:
            ...
            for module_name, url in urls.items():
                ...
                module_path = modules.get_module_path(
                    module_name, downloaded=True, display_warning=False
                )
                bck = backup(module_path, False)
                _logger.info(
                    "Copy downloaded module `%s` to `%s`", module_name, module_path
                )
                ...
                for d in os.listdir(base_path):
                    if d != "base" and os.path.isdir(os.path.join(base_path, d)):
                        destdir = os.path.join(
                            tmp, OPENERP, "addons", d
                        )  # XXX 'odoo' subdirectory ?
            ...
            installed = self.search(
                [("id", "in", downloaded.ids), ("state", "=", "installed")]
            )

            to_install = self.search(
                [("name", "in", list(urls)), ("state", "=", "uninstalled")]
            )
