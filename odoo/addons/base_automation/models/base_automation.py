...
class BaseAutomation(models.Model):
    ...
    def _check_trigger_state(self):
        if any(
            action.trigger == "on_change" and action.state != "code" for action in self
        ):
        ...
        if any(
            action.trigger == "on_unlink"
            and action.state in ["mail_post", "followers", "next_activity"]
            for action in self
        ):
            raise exceptions.ValidationError(
                _(
                    "Email, followers or activities action types cannot be used when deleting records."
                )
            )
    ...
    def _compute_trg_date_id(self):
        invalid = self.filtered(
            lambda act: act.trigger != "on_time"
            or (act.model_id and act.trg_date_id.model_id != act.model_id)
        )
        if invalid:
    ...
    def _compute_trg_date_calendar_id(self):
        invalid = self.filtered(
            lambda act: act.trigger != "on_time"
            or not act.trg_date_id
            or act.trg_date_range_type != "day"
        )
        if invalid:
    ...
    def _compute_filter_pre_domain(self):
        to_reset = self.filtered(
            lambda act: act.trigger not in ("on_write", "on_create_or_write")
        )
        if to_reset:
    ...
    def _compute_on_change_field_ids(self):
        ...
        for action in (self - to_reset).filtered("on_change_field_ids"):
            action.on_change_field_ids = action.on_change_field_ids.filtered(
                lambda field: field.model_id == action.model_id
            )
    ...
    def _compute_trigger_field_ids(self):
        ...
        for action in (self - to_reset).filtered("trigger_field_ids"):
            action.trigger_field_ids = action.trigger_field_ids.filtered(
                lambda field: field.model_id == action.model_id
            )
        ...
        if self.trigger == "on_change" and self.state != "code":
            ...
            return {
                "warning": {
                    "title": _("Warning"),
                    "message": _(
                        'The "%(trigger_value)s" %(trigger_label)s can only be used with the "%(state_value)s" action type'
                    )
                    % {
    ...
    def _check_trigger_fields(self, record):
        ...
        def differ(name):
            ...
            return name in old_vals and field.convert_to_cache(
                record[name], record, validate=False
    ...
    def _register_hook(self):
        ...
        for action_rule in self.with_context({}).search([]):
            ...
            if Model is None:
                _logger.warning(
                    "Action rule with ID %d depends on model %s"
    ...
    def _check(self, automatic=False, use_new_cursor=False):
        ...
        for action in self.with_context(active_test=True).search(
            ...
            if (
                action.trg_date_id.name == "date_action_last"
                and "create_date" in records._fields
            ):
                get_record_dt = (
                    lambda record: record[action.trg_date_id.name] or record.create_date
                )
            else:
