...
class Menu(models.Model):
    ...
    def name_get(self):
        if not self._context.get("display_website") and not self.env.user.has_group(
    ...
    def create(self, vals_list):
        ...
        for vals in vals_list:
            ...
            if "website_id" in vals:
                ...
                default_menu = self.env.ref(
                    "website.main_menu", raise_if_not_found=False
                )
        ...
        for menu in data["data"]:
            ...
            if not menu["url"] or "#" in menu["url"]:
                ...
                if (
                    request
                    and menu["url"]
                    and menu["url"].startswith("#")
                    and len(menu["url"]) > 1
                ):
                    # Working on case 2.: prefix anchor with referer URL
                    referer_url = werkzeug.urls.url_parse(
