...
class TestTokenAccess(TransactionCase):
    ...
    def test_normal_user_should_be_able_to_reset_his_own_token(self):
        ...
        user.with_user(user).google_calendar_account_id._set_auth_tokens(
            "my_new_token", "my_new_rtoken", 3600
        )

        self.assertEqual(
            user.google_calendar_account_id.calendar_rtoken, "my_new_rtoken"
        )
        self.assertEqual(user.google_calendar_account_id.calendar_token, "my_new_token")
        self.assertNotEqual(
            user.google_calendar_account_id.calendar_token_validity, old_validity
        )
    ...
    def test_normal_user_should_not_be_able_to_reset_other_user_tokens(self):
        ...
        with self.assertRaises(AccessError):
            user2.with_user(user1).google_calendar_account_id._set_auth_tokens(
                False, False, 0
            )
