...
class TestSoLineMilestones(TestSaleCommon):
    ...
    def test_reached_milestones_delivered_quantity(self):
        ...
        self.assertEqual(
            self.sol1.qty_delivered, 0.0, "Delivered quantity should start at 0"
        )
        self.assertEqual(
            self.sol2.qty_delivered, 0.0, "Delivered quantity should start at 0"
        )

        self.milestone1.is_reached = True
        self.assertEqual(
            self.sol1.qty_delivered,
            10.0,
    ...
    def test_update_reached_milestone_quantity(self):
        self.milestone1.is_reached = True
        self.assertEqual(
            self.sol1.qty_delivered, 10.0, "Delivered quantity should start at 10"
        )
    ...
    def test_create_milestone_on_project_set_on_sales_order(self):
        ...
        project = self.env["project.project"].create(
            {"name": "Test Project For Milestones", "partner_id": self.partner_a.id}
        )
