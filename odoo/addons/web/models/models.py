...
class IrActionsActWindowView(models.Model):
    ...
    view_mode = fields.Selection(
        selection_add=[("qweb", "QWeb")], ondelete={"qweb": "cascade"}
    )
...
class Base(models.AbstractModel):
    ...
    def _web_read_group(
        ...
        if expand and len(groupby) == 1:
            for group in groups:
                group["__data"] = self.web_search_read(
    ...
    def _read_progress_bar(self, domain, group_by, progress_bar):
        ...
        records_values = self.search_read(
            domain or [], [progress_bar["field"], group_by_name]
        )
        ...
        for record_values in records_values:
            ...
            if group_by_value and field_type in ["date", "datetime"]:
                ...
                group_by_value = date_utils.start_of(
                    fields.Datetime.to_datetime(group_by_value), group_by_modifier
                )
                group_by_value = pytz.timezone("UTC").localize(group_by_value)
                tz_info = None
                if (
                    field_type == "datetime"
                    and self._context.get("tz") in pytz.all_timezones
                ):
    ...
    def search_panel_select_multi_range(self, field_name, **kwargs):
        ...
        Comodel = self.env.get(field.comodel_name).with_context(
