...
class Alias(models.Model):
    ...
    def create(self, vals_list):
        ...
        """
        alias_names = [
            vals["alias_name"] for vals in vals_list if vals.get("alias_name")
        ]
        if alias_names:
            sanitized_names = self._clean_and_check_unique(alias_names)
            for vals in vals_list:
                if vals.get("alias_name"):
                    vals["alias_name"] = sanitized_names[
                        alias_names.index(vals["alias_name"])
                    ]
    ...
    def name_get(self):
        ...
        for record in self:
            if record.alias_name and record.alias_domain:
                res.append(
                    (record["id"], "%s@%s" % (record.alias_name, record.alias_domain))
                )
    ...
    def _clean_and_check_unique(self, names):
        ...
        for sanitized_name in sanitized_names:
            if sanitized_name in [catchall_alias, bounce_alias]:
                ...
                raise UserError(
                    _(
                        "The e-mail alias %(matching_alias_name)s is already used as %(alias_duplicate)s alias. Please choose another alias.",
        ...
        if (
            ...
            raise UserError(
                _(
                    "The e-mail alias %(matching_alias_name)s is already used by the %(document_name)s %(model_name)s. Choose another alias or change it on the other document.",
                    matching_alias_name=matching_alias_name,
        ...
        raise UserError(
            _(
                "The e-mail alias %(matching_alias_name)s is already linked with %(alias_model_name)s. Choose another alias or change it on the linked model.",
                matching_alias_name=matching_alias_name,
