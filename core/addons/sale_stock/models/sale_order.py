...
class SaleOrder(models.Model):
    ...
    show_json_popover = fields.Boolean(
        "Has late picking", compute="_compute_json_popover"
    )
    ...
    def write(self, values):
        ...
        if values.get("partner_shipping_id"):
            ...
            for record in self:
                ...
                addresses = (
                    record.partner_shipping_id.display_name,
        ...
        if "commitment_date" in values:
            ...
            for order in self:
                order.order_line.move_ids.date_deadline = (
                    deadline_datetime or order.expected_date
                )
