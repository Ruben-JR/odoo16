...
class AccountReport(models.Model):
    ...
    chart_template_id = fields.Many2one(
        string="Chart of Accounts", comodel_name="account.chart.template"
    )
    ...
    default_opening_date_filter = fields.Selection(
        ...
        compute=lambda x: x._compute_report_option_filter(
            "default_opening_date_filter", "last_month"
        ),
        readonly=False,
        store=True,
        depends=["root_report_id"],
    ...
    filter_account_type = fields.Boolean(
        ...
        compute=lambda x: x._compute_report_option_filter("filter_account_type"),
        readonly=False,
        store=True,
        depends=["root_report_id"],
    )
    ...
    filter_fiscal_position = fields.Boolean(
        ...
        compute=lambda x: x._compute_report_option_filter("filter_fiscal_position"),
        readonly=False,
        store=True,
        depends=["root_report_id"],
    )
    ...
    def write(self, vals):
        ...
        if "country_id" in vals:
            ...
            for expression in tax_tags_expressions:
                ...
                tag_reports = (
                    tax_tags._get_related_tax_report_expressions().report_line_id.report_id
                )
...
class AccountReportLine(models.Model):
    ...
    parent_id = fields.Many2one(
        string="Parent Line", comodel_name="account.report.line", ondelete="set null"
    )
...
class AccountReportExpression(models.Model):
            ...
            if tag_name:
                ...
                if len(existing_tags) < 2:
                    ...
                    tag_vals = self._get_tags_create_vals(
                        tag_name, country.id, existing_tag=existing_tags
                    )
                    self.env["account.account.tag"].create(tag_vals)
            ...
            for former_formula in former_formulas_list:
                ...
                if not new_tax_tags:
                    ...
                    if former_tax_tags and all(
                        ...
                        positive_tags, negative_tags = former_tax_tags.sorted(
                            lambda x: x.tax_negate
                        )
                        positive_tags.name, negative_tags.name = (
                            f"+{vals['formula']}",
                            f"-{vals['formula']}",
                        )
                    else:
            ...
            other_expression_using_tag = (
                ...
                .search(
                    [
                        ...
                        (
                            "report_line_id.report_id.country_id.id",
                            "=",
                            tag.country_id.id,
                        ),
                        ("id", "not in", self.ids),
    ...
    def _get_matching_tags(self, sign=None):
        ...
        for tag_expression in tag_expressions:
            ...
            or_domains.append(
                self.env["account.account.tag"]._get_tax_tags_domain(
                    tag_expression.formula, country.id, sign
                )
            )
        ...
        return (
            ...
            .search(osv.expression.OR(or_domains))
        )
    ...
    def action_view_carryover_lines(self, options):
        date_from, date_to, dummy = self.report_line_id.report_id._get_date_bounds_info(
        ...
        return {
            ...
            "views": [
                (
                    self.env.ref(
                        "account_reports.account_report_external_value_tree"
                    ).id,
                    "list",
                )
            ],
...
class AccountReportExternalValue(models.Model):
    ...
    def _check_fiscal_position(self):
        for record in self:
            if (
                record.foreign_vat_fiscal_position_id
                and record.foreign_vat_fiscal_position_id.country_id
                != record.report_country_id
            ):
                raise ValidationError(
                    _(
                        "The country set on the foreign VAT fiscal position must match the one set on the report."
                    )
                )
