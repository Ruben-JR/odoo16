...
class TestMrpProductionBackorder(TestMrpCommon):
    ...
    def test_no_tracking_2(self):
        ...
        )  # the reservation of the first MO should've been moved here
    ...
    def test_no_tracking_pbm_1(self):
        ...
        (
            ...
            qty_base_1=4, qty_final=4, picking_type_id=self.warehouse.manu_type_id
        )
    ...
    def test_tracking_backorder_series_lot_1(self):
        ...
        for i in range(nb_product_todo):
            ...
            if i + 1 != nb_product_todo:  # If last MO, don't make a backorder
                ...
                backorder.save().action_backorder()
            active_production = (
                active_production.procurement_group_id.mrp_production_ids[-1]
            )
    ...
    def test_backorder_name(self):
        ...
        default_picking_type_id = self.env[
            "mrp.production"
        ]._get_default_picking_type_id(self.env.company.id)
    ...
    def test_split_merge(self):
        ...
        self.assertEqual(
            mo1.move_raw_ids.filtered(lambda m: m.product_id == p1).product_qty, 12
        )
        self.assertEqual(
            mo2.move_raw_ids.filtered(lambda m: m.product_id == p1).product_qty, 12
        )
        self.assertEqual(
            mo3.move_raw_ids.filtered(lambda m: m.product_id == p1).product_qty, 16
        )
        self.assertEqual(
            mo1.move_raw_ids.filtered(lambda m: m.product_id == p2).product_qty, 3
        )
        self.assertEqual(
            mo2.move_raw_ids.filtered(lambda m: m.product_id == p2).product_qty, 3
        )
        self.assertEqual(
            mo3.move_raw_ids.filtered(lambda m: m.product_id == p2).product_qty, 4
        )
