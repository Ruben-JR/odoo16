...
class TestAccountPayment(AccountTestInvoicingCommon):
    ...
    def test_payment_move_sync_create_write(self):
        ...
        self.assertRecordValues(
            payment.line_ids.sorted("balance"),
            [
                expected_counterpart_line,
                expected_liquidity_line,
            ],
    ...
    def test_internal_transfer(self):
        ...
        payment.action_post()
        paired_payment = self.env["account.payment"].search(
            [("payment_type", "=", "outbound")]
        )
        expected_payment_values = {
            "amount": 50.0,
        ...
        self.assertRecordValues(
            paired_payment.line_ids.sorted("balance"),
            [
                expected_liquidity_line,
                expected_counterpart_line,
            ],
    ...
    def test_payment_name(self):
        ...
        with Form(
            AccountPayment.with_context(default_move_journal_types=("bank", "cash"))
        ) as payment_form:
    ...
    def test_internal_transfer_right_accounts(self):
        ...
        self.assertRecordValues(
            payment.paired_internal_transfer_payment_id.line_ids,
            [
        ...
        bank.inbound_payment_method_line_ids.payment_account_id = (
            company.account_journal_payment_debit_account_id.copy({"code": 1001})
        )
        bank.outbound_payment_method_line_ids.payment_account_id = (
            company.account_journal_payment_credit_account_id.copy({"code": 1002})
        )

        bank_2.inbound_payment_method_line_ids.payment_account_id = (
            company.account_journal_payment_debit_account_id.copy({"code": 2001})
        )
        bank_2.outbound_payment_method_line_ids.payment_account_id = (
            company.account_journal_payment_credit_account_id.copy({"code": 2002})
        )
