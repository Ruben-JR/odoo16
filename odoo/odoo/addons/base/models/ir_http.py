...
class IrHttp(models.AbstractModel):
    ...
    def routing_map(cls, key=None):
        ...
        if key not in cls._routing_map:
            ...
            routing_map = werkzeug.routing.Map(
                strict_slashes=False, converters=cls._get_converters()
            )
            for url, endpoint in cls._generate_routing_rules(
                mods, converters=cls._get_converters()
            ):
                routing = submap(endpoint.routing, ROUTING_KEYS)
                if (
                    routing["methods"] is not None
                    and "OPTIONS" not in routing["methods"]
                ):
    ...
    def get_web_translations_hash(self, modules, lang):
        ...
        return hashlib.sha1(
            json.dumps(translation_cache, sort_keys=True).encode()
        ).hexdigest()
