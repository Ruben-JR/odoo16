...
class Generate2307Wizard(models.TransientModel):
        ...
        for move in moves:
            ...
            values = {
                "invoice_date": format_date(
                    self.env, move.invoice_date, date_format="MM/dd/yyyy"
                ),
            ...
            for invoice_line in move.invoice_line_ids.filtered(
                ...
                for tax in invoice_line.tax_ids.filtered(lambda x: x.l10n_ph_atc):
                    values["product_name"] = re.sub(
                        r"[\(\)]", "", invoice_line.product_id.name
                    )
                    values["atc"] = tax.l10n_ph_atc
                    values["price_subtotal"] = invoice_line.price_subtotal
                    values["amount"] = tax.amount
                    values["tax_amount"] = tax._compute_amount(
                        invoice_line.price_subtotal, invoice_line.price_unit
                    )
    ...
    def action_generate(self):
        ...
        return {
            ...
            "target": "self",
            "url": "/web/content?model=l10n_ph_2307.wizard&download=true&field=generate_xls_file&filename=Form_2307.xls&id={}".format(
                self.id
            ),
        }
