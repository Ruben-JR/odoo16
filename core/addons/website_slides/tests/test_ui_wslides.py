...
class TestUICommon(HttpCaseWithUserDemo, HttpCaseWithUserPortal):
    def setUp(self):
        ...
        pdf_path = get_module_resource(
            "website_slides", "static", "src", "img", "presentation.pdf"
        )
        pdf_content = base64.b64encode(open(pdf_path, "rb").read())
        img_path = get_module_resource(
            "website_slides", "static", "src", "img", "slide_demo_gardening_1.jpg"
        )
...
class TestUi(TestUICommon):
    def test_course_member_employee(self):
        ...
        user_demo.write(
            {"karma": 1, "groups_id": [(6, 0, self.env.ref("base.group_user").ids)]}
        )
