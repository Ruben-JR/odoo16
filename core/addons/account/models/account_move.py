...
class AccountMove(models.Model):
    ...
    def _compute_hide_post_button(self):
        for record in self:
            record.hide_post_button = (
                ...
                and record.date > fields.Date.today()
            )
    ...
    def _compute_journal_id(self):
        for record in self:
            ...
            if (
                ...
                or record.company_id != record.journal_id.company_id
            ):
                self.env.add_to_compute(self._fields["company_id"], record)
            if (
                not record.currency_id
                or record.journal_id.currency_id
                and record.currency_id != record.journal_id.currency_id
            ):
    ...
    def _compute_payment_state(self):
        ...
        for invoice in self:
            ...
            currency = (
                currencies if len(currencies) == 1 else invoice.company_id.currency_id
            )
            ...
            if invoice.state == "posted":
                ...
                if new_pmt_state == "paid":
                    reverse_move_types = []
                    for x in reconciliation_vals:
                        for rec_move_type, rec_reversed_entry_id in zip(
                            x["counterpart_move_types"] or [],
                            x["counterpart_reversed_entry_ids"] or [],
                        ):
                    ...
                    if (
                        (
                            invoice.move_type in ("in_invoice", "in_receipt")
                            and reverse_move_types == ["in_refund"]
                        )
                        or (
                            invoice.move_type in ("out_invoice", "out_receipt")
                            and reverse_move_types == ["out_refund"]
                        )
                        or (
                            invoice.move_type in ("entry", "out_refund", "in_refund")
        ...
        for move in self:
            if move.is_invoice(include_receipts=True):
                ...
                if move.id:
                    ...
                    for base_line in base_lines:
                        ...
                        for grouping_dict, values in base_line.epd_needed.items():
                            epd_values = epd_aggregated_values.setdefault(
                                grouping_dict, {"price_subtotal": 0.0}
                            )
                    ...
                    for grouping_dict, values in epd_aggregated_values.items():
                        ...
                        if grouping_dict.get("tax_ids"):
                            taxes = self.env["account.tax"].browse(
                                grouping_dict["tax_ids"][0][2]
                            )

                        kwargs["base_lines"].append(
                            self.env["account.tax"]._convert_to_tax_base_line_dict(
                                ...
                                account=self.env["account.account"].browse(
                                    grouping_dict["account_id"]
                                ),
                                analytic_distribution=values.get(
                ...
                rounding_line = move.line_ids.filtered(
                    lambda l: l.display_type == "rounding"
                )
                if rounding_line:
                    amount_total_rounded = (
                        move.tax_totals["amount_total"] - rounding_line.balance
                    )
                    move.tax_totals["formatted_amount_total_rounded"] = (
                        formatLang(
                            self.env,
                            amount_total_rounded,
    ...
    def _compute_show_payment_term_details(self):
        ...
        for invoice in self:
            if (
                ...
                invoice.show_discount_details = any(
                    line.discount_date for line in payment_term_lines
                )
    ...
    def _compute_tax_lock_date_message(self):
        for move in self:
            ...
            if lock_dates:
                accounting_date = move._get_accounting_date(
                    invoice_date, affects_tax_report
                )
                ...
                for lock_date, lock_type in lock_dates[:-1]:
                    tax_lock_date_message += _(
                        " The %(lock_type)s lock date is set on %(lock_date)s.",
                        lock_type=lock_type,
                        lock_date=format_date(move.env, lock_date),
                    )
    ...
    def _compute_display_inactive_currency_warning(self):
        for move in self.with_context(active_test=False):
            move.display_inactive_currency_warning = (
                move.currency_id and not move.currency_id.active
            )
    ...
    def _compute_show_reset_to_draft_button(self):
        for move in self:
            move.show_reset_to_draft_button = (
                not move.restrict_mode_hash_table and move.state in ("posted", "cancel")
            )
    ...
    def _compute_partner_credit_warning(self):
        for move in self:
            ...
            if show_warning:
                ...
                updated_credit = (
                    move.partner_id.commercial_partner_id.credit + amount_total_currency
                )
    ...
    def _build_credit_warning_message(self, record, updated_credit):
        ...
        msg += ": %s" % formatLang(
            self.env, updated_credit, currency_obj=record.company_id.currency_id
        )
        return msg
    ...
    def _fetch_duplicate_supplier_reference(self, only_posted=False):
        ...
        if not moves.ids:
            ...
            casted_values = ", ".join(
                [
                    f"%({field_name})s::{moves._fields[field_name].column_type[0]}"
                    for field_name in place_holders
                ]
            )
            move_table_and_alias = (
                f'(VALUES ({casted_values})) AS move({", ".join(place_holders)})'
            )
        ...
        return {
            ...
            .browse(res["move_id"]): self.env["account.move"]
            .browse(res["duplicate_ids"])
    ...
    def _inverse_currency_id(self):
        for invoice in self:
            if (
                ...
                and invoice.journal_id.currency_id != invoice.currency_id
            ):
    ...
    @api.constrains(
        "ref", "move_type", "partner_id", "journal_id", "invoice_date", "state"
    )
    def _check_duplicate_supplier_reference(self):
        """Assert the move which is about to be posted isn't a duplicated move from another posted entry"""
        move_to_duplicate_moves = self.filtered(
            lambda m: m.state == "posted"
        )._fetch_duplicate_supplier_reference(only_posted=True)
        if any(duplicate_move for duplicate_move in move_to_duplicate_moves.values()):
            duplicate_move_ids = list(
                set(
                    move_id
                    for move_ids in (
                        move.ids + duplicate.ids
                        for move, duplicate in move_to_duplicate_moves.items()
                        if duplicate
                    )
    ...
    def _validate_taxes_country(self):
        ...
        for record in self:
            ...
            impacted_countries = (
                amls.tax_ids.country_id
                | amls.tax_line_id.country_id
                | amls.tax_tag_ids.country_id
            )
    ...
    def _sync_unbalanced_lines(self, container):
        ...
        for invoice in (x for x in container["records"] if x.state != "posted"):
            ...
            existing_balancing_line = invoice.line_ids.filtered(
    ...
    def _sync_dynamic_line(
        ...
        def dirty():
            *path, dirty_fname = needed_dirty_fname.split(".")
            dirty_recs = (
                container["records"].mapped(".".join(path)).filtered(dirty_fname)
            )
        ...
        for move, data in zip(self, data_list):
            if move.move_type in ("out_invoice", "in_invoice"):
                data["line_ids"] = [
                    ...
                    and line_vals.get("display_type")
                    not in ("payment_term", "tax", "rounding")
                ]
    ...
    def copy(self, default=None):
        ...
        if (
            fields.Date.to_date(default.get("date")) or self.date
        ) <= self.company_id._get_user_fiscal_lock_date():
        ...
        message_origin = (
            ""
            if not copied_am.auto_post_origin_id
            else "<br/>"
            + _(
                "This recurring entry originated from %s",
                copied_am.auto_post_origin_id._get_html_link(),
            )
        )
    ...
    def write(self, vals):
        ...
        for move in self:
            if (
                ...
                and set(vals).intersection(move._get_integrity_hash_fields())
            ):
                raise UserError(
                    _(
                        "You cannot edit the following fields due to restrict mode being activated on the journal: %s."
                    )
                    % ", ".join(move._get_integrity_hash_fields())
                )
            if (
                move.restrict_mode_hash_table
                and move.inalterable_hash
                and "inalterable_hash" in vals
            ) or (move.secure_sequence_number and "secure_sequence_number" in vals):
                raise UserError(
                    _(
                        "You cannot overwrite the values ensuring the inalterability of the accounting."
                    )
                )
            if (
                move.posted_before
            ...
            if (
                ...
                raise UserError(
                    _(
                        "You cannot edit the journal of an account move if it already has a sequence number assigned."
                    )
                )
    ...
    def _get_quick_edit_suggestions(self):
        ...
        price_untaxed = taxes.with_context(force_price_include=True).compute_all(
            self.quick_edit_total_amount - self.tax_totals["amount_total"]
        )["total_excluded"]
        return {
            "account_id": account_id,
            "tax_ids": taxes.ids,
    ...
    def _check_total_amount(self, amount_total):
        ...
        if not float_is_zero(
            ...
            if "Untaxed Amount" in totals["groups_by_subtotal"]:
                totals["groups_by_subtotal"]["Untaxed Amount"][0][
                    "tax_group_amount"
    ...
    def _get_new_hash(self, secure_seq_number):
        ...
        return self._compute_hash(prev_move.inalterable_hash if prev_move else "")
    ...
    def _get_invoice_counterpart_amls_for_early_payment_discount_per_payment_term_line(
        ...
        def grouping_key_generator(base_line, tax_values):
            return self.env["account.tax"]._get_generation_dict_from_base_line(
                base_line, tax_values
            )
        ...
        for base_line in base_lines:
            base_line["taxes"] = base_line["taxes"].filtered(
        ...
        for aml in self.line_ids.filtered(lambda x: x.display_type == "payment_term"):
            ...
            if early_pay_discount_computation == "included" and product_lines.tax_ids:
                ...
                if aml.discount_percentage not in tax_computation_per_percentage:
                    ...
                    for base_line in base_lines:
                        ...
                        amount_currency = self.currency_id.round(
                            self.direction_sign * to_update_vals["price_subtotal"]
                            - invoice_line.amount_currency
                        )
                        balance = self.company_currency_id.round(
                            amount_currency / base_line["rate"]
                        )
                    ...
                    for tax_detail in tax_details_with_epd["tax_details"].values():
                        ...
                        tax_amount_currency = self.currency_id.round(
                            self.direction_sign * tax_detail["tax_amount_currency"]
                            - tax_amount_without_epd["amount_currency"]
                        )
                        tax_amount = self.company_currency_id.round(
                            self.direction_sign * tax_detail["tax_amount"]
                            - tax_amount_without_epd["balance"]
                        )

                        if self.currency_id.is_zero(
                            tax_amount_currency
                        ) and self.company_currency_id.is_zero(tax_amount):
                            continue
                ...
                for tax_detail in tax_computation_per_percentage[
                    aml.discount_percentage
                ].values():
                    ...
                    grouping_dict = {
                        ...
                        "group_tax_id": tax_detail["tax_id"]
                        if tax_detail["tax_id"] != tax.id
                        else None,
                    }

                    res["tax_lines"][aml][frozendict(grouping_dict)] = {
                        "name": _("Early Payment Discount (%s)", tax.name),
                        "amount_currency": aml.currency_id.round(
                            tax_detail["amount_currency"] * percentage_paid
                        ),
                        "balance": aml.company_currency_id.round(
                            tax_detail["balance"] * percentage_paid
                        ),
                    }

                for grouping_dict, base_detail in base_per_percentage[
                    aml.discount_percentage
                ].items():
                ...
                last_tax_line = (
                    list(res["tax_lines"][aml].values())
                    or list(res["base_lines"][aml].values())
                )[-1]
    ...
    def _post(self, soft=True):
        ...
        for invoice in to_post:
            ...
            if (
                ...
                and (
                    invoice.journal_id.sale_activity_user_id or invoice.invoice_user_id
                ).id
                ...
                invoice.activity_schedule(
                    date_deadline=min(
                        (
                            date
                            for date in invoice.line_ids.mapped("date_maturity")
                            if date
                        ),
                    ...
                    user_id=invoice.journal_id.sale_activity_user_id.id
                    or invoice.invoice_user_id.id,
                )
    ...
    def _link_invoice_origin_to_purchase_orders(self, timeout=10):
        for move in self.filtered(lambda m: m.move_type in self.get_purchase_types()):
            ...
            move._find_and_set_purchase_orders(
                references, move.partner_id.id, move.amount_total, timeout
            )
        return self
    ...
    def _get_accounting_date(self, invoice_date, has_tax):
        ...
        highest_name = self.highest_name or self._get_last_sequence(
    ...
    def _generate_qr_code(self, silent_errors=False):
        ...
        if qr_code_method:
            ...
            if not self.partner_bank_id._eligible_for_qr_code(
                ...
                raise UserError(
                    _("The chosen QR-code type is not eligible for this invoice.")
                )
        else:
            ...
            for candidate_method, _candidate_name in self.env[
                ...
                if self.partner_bank_id._eligible_for_qr_code(
                    candidate_method,
                    self.partner_id,
                    self.currency_id,
                    raises_error=False,
    ...
    def _disable_discount_precision(self):
        ...
        with patch(
            "odoo.addons.base.models.decimal_precision.DecimalPrecision.precision_get",
            new=precision_get,
        ):
            yield
        ...
        try:
            ...
            for env, val in previous_values.items():
                if val != EMPTY:
                    ...
                    env.context = frozendict(
                        {k: v for k, v in env.context.items() if k != key}
                    )
    ...
    def message_new(self, msg_dict, custom_values=None):
        ...
        def is_internal_partner(partner):
            ...
            return partner == company.partner_id or (
                partner.user_ids
        ...
        if custom_values.get("company_id"):
            extra_domain = [
                "|",
                ("company_id", "=", custom_values["company_id"]),
                ("company_id", "=", False),
            ]
        ...
        followers = [
            partner
            for partner in self._mail_find_partner_from_emails(
                cc_mail_addresses, extra_domain
            )
            if partner
        ]
        ...
        senders = partners = [
            partner
            for partner in self._mail_find_partner_from_emails(
                from_mail_addresses, extra_domain
            )
            if partner
        ]
        ...
        if partners:
            ...
            if is_internal_partner(partners[0]):
                ...
                partners = [
                    partner
                    for partner in self._mail_find_partner_from_emails(
                        body_mail_addresses, extra_domain
                    )
                    if not is_internal_partner(partner)
                    and partner.company_id.id in (False, company.id)
                ]
    ...
    def _message_post_after_hook(self, new_message, message_values):
        ...
        if (
            len(self) != 1
            or not attachments
            or self.env.context.get("no_new_invoice")
        ...
        decoders = self.env[
            "account.move"
        ]._get_update_invoice_from_attachment_decoders(self)
        with self._disable_discount_precision():
            for decoder in sorted(decoders, key=lambda d: d[0]):
                ...
                for attachment in attachments.sorted(
                    lambda x: x != self.message_main_attachment_id
                ):
    ...
    def _track_subtype(self, init_values):
        ...
        if not self.is_invoice(include_receipts=True):
            if self.payment_id and "state" in init_values:
                self.payment_id._message_track(
                    ["state"], {self.payment_id.id: init_values}
                )
    ...
    def _conditional_add_to_compute(self, fname, condition):
        ...
        to_reset = self.filtered(
            lambda move: condition(move) and not self.env.is_protected(field, move)
        )
