...
class HrContract(models.Model):
        ...
        return self.env["hr.leave"].search(
            [
                ...
                (
                    "date_from",
                    "<=",
                    max([end or date.max for end in self.mapped("date_end")]),
                ),
    ...
    def _get_interval_leave_work_entry_type(self, interval, leaves, bypassing_codes):
        ...
        if (
            ...
            and interval[2].work_entry_type_id.code in bypassing_codes
        ):
    ...
    def write(self, vals):
        ...
        for contract in self:
            ...
            for leave in leaves:
                overlapping_contracts = leave._get_overlapping_contracts(
                    contract_states=[("state", "!=", "cancel")]
                )
                ...
                for overlapping_contract in overlapping_contracts:
                    ...
                    # transaction
                    if (
                        overlapping_contract.state == "draft"
                        and overlapping_contract not in self
                    ):
                        continue
                    new_date_from = max(
                        leave.date_from,
                        datetime.combine(
                            overlapping_contract.date_start, datetime.min.time()
                        ),
                    )
                    new_date_to = min(
                        leave.date_to,
                        datetime.combine(
                            overlapping_contract.date_end or date.max,
                            datetime.max.time(),
                        ),
                    )
