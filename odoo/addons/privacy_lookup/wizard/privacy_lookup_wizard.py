    ...
    def _get_query(self):
        ...
        for model_name in self.env:
            ...
            additional_query = """
                ...
                WHERE
                """.format(
                table_name=table_name, active="active" if has_active else True
            )
            additional_values = [res_model_id]
                ...
                if field_name in model and model._fields[field_name].store:
                    ...
                    if (
                        ...
                        additional_query += """
                            ...
                            search_type="="
                            if is_normalized
                            else "ilike",  # Manage Foo Bar <foo@bar.com>
                            rec_name=rec_name,
                        )
                        additional_values += [
                        ...
                        additional_query += """
                            ...
                            """.format(
                            field_name=field_name,
                            search_type="=" if is_normalized else "ilike",
                        )  # Manage Foo Bar <foo@bar.com>
                        additional_values += [
                            email_normalized if is_normalized else email
                        ]
                    if is_normalized:
        ...
        for wizard in self:
            ...
            wizard.records_description = "\n".join(
                "{model_name} ({count}): {ids_str}".format(
                    ...
                    if not self.env.user.user_has_groups("base.group_no_one")
                    else "%s - %s" % (model.name, model.model),
                    count=len(ids),
    ...
    def _compute_resource_ref(self):
        for line in self:
            if line.res_model and line.res_model in self.env and not line.is_unlinked:
                ...
                try:
                    self.env[line.res_model].browse(line.res_id).check_access_rule(
                        "read"
                    )
    ...
    def _compute_res_name(self):
        for line in self:
            ...
            line.res_name = (
                name[0][1]
                if name
                else ("%s/%s") % (line.res_model_id.name, line.res_id)
            )
