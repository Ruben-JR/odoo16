...
class PatchedHTTPAdapter(requests.adapters.HTTPAdapter):
    ...
    def get_connection(self, url, proxies=None):
        ...
        def patched_load_cert_chain(
            l10n_es_odoo_certificate, keyfile=None, password=None
        ):
            (
                cert_file,
                key_file,
                _certificate,
            ) = l10n_es_odoo_certificate.sudo()._decode_certificate()
...
class AccountEdiFormat(models.Model):
    ...
    def _l10n_es_edi_get_invoices_tax_details_info(
        ...
        def full_filter_invl_to_apply(invoice_line):
            if "ignore" in invoice_line.tax_ids.flatten_taxes_hierarchy().mapped(
                "l10n_es_type"
            ):
                return False
            ...
            if recargo_tax and taxes:
                ...
                if not recargo_tax_details.get(recargo_main_tax):
                    recargo_tax_details[recargo_main_tax] = [
                        x
                        for x in tax_details["tax_details"].values()
                        if x["group_tax_details"][0]["tax_repartition_line"].tax_id
                        == recargo_tax[0]
                    ][0]
        ...
        for tax_values in tax_details["tax_details"].values():
            if invoice.is_sale_document():
                ...
                if tax_subject_info_list:
                    ...
                    tax_details_info["Sujeta"]["NoExenta"]["DesgloseIVA"][
                        "DetalleIVA"
                    ] += tax_subject_info_list
        ...
        if (
            ...
            tax_details_info["NoSujeta"]["ImportePorArticulos7_14_Otros"] = round(
        ...
        if (
            ...
            tax_details_info["NoSujeta"]["ImporteTAIReglasLocalizacion"] = round(
    ...
    def _l10n_es_edi_get_partner_info(self, partner):
        ...
        if (not partner.country_id or partner.country_id.code == "ES") and partner.vat:
            # ES partner with VAT.
            partner_info["NIF"] = (
                partner.vat[2:] if partner.vat.startswith("ES") else partner.vat
            )
    ...
    def _l10n_es_edi_get_invoices_info(self, invoices):
        ...
        for invoice in invoices:
            ...
            if invoice.is_sale_document():
                info["IDFactura"]["IDEmisorFactura"] = {
                ...
                if (
                    not com_partner.country_id
                    or com_partner.country_id.code in eu_country_codes
                ):
                ...
                if (
                    not country_code
                    or country_code == "ES"
                    or country_code not in eu_country_codes
                ):
                    invoice_node["ClaveRegimenEspecialOTrascendencia"] = "01"
                else:
                    invoice_node[
            ...
            if invoice.is_sale_document():
                ...
                if com_partner.country_id.code in ("ES", False) and not (
                    com_partner.vat or ""
                ).startswith("ESN"):
