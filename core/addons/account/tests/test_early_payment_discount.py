...
class TestAccountEarlyPaymentDiscount(AccountTestInvoicingCommon):
    ...
    def test_register_discounted_payment_multi_line_multi_discount_tax_mixed_too_late(
        self,
    ):
        self.env.company.early_pay_discount_computation = "mixed"
        inv_mixed_lines_multi_discount = self.env["account.move"].create(
        ...
        payments = (
            self.env["account.payment.register"]
            .with_context(active_model="account.move", active_ids=active_ids)
            .create({"payment_date": "2019-01-31", "group_payment": True})
            ._create_payments()
        )
        self.assertTrue(payments.is_reconciled)
        self.assertRecordValues(
    ...
    def test_mixed_epd_with_draft_invoice(self):
        ...
        with Form(
            ...
            self.assert_tax_totals(
                invoice._values["tax_totals"],
                {
                    "amount_untaxed": 1000,
                    "amount_total": 1090,
                    "groups_by_subtotal": {
    ...
    def test_mixed_epd_with_tax_no_duplication(self):
        ...
        self.assertEqual(
            len(inv.line_ids), 6
        )  # 1 prod, 1 tax, 2 epd, 1 epd tax discount, 1 payment terms
