...
class ResCompany(models.Model):
    ...
    def write(self, vals):
        ...
        if (
            "hr_attendance_overtime" in vals
            and not vals["hr_attendance_overtime"]
            and overtime_enabled_companies
        ):
        ...
        if not is_disabling_overtime and (
            start_date
            or "overtime_company_threshold" in vals
            or "overtime_employee_threshold" in vals
        ):
            for company in self:
                # If we modify the thresholds only
                if (
                    start_date == company.overtime_start_date
                    and (
