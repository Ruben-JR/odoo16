...
class PosOrder(models.Model):
    ...
    def _get_valid_session(self, order):
        ...
        if rescue_session:
            _logger.warning(
                "reusing recovery session %s for saving order %s",
                rescue_session.name,
                order["name"],
            )
        ...
        if new_session.config_id.cash_control and new_session.rescue:
            last_session = self.env["pos.session"].search(
                [
                    ("config_id", "=", new_session.config_id.id),
                    ("id", "!=", new_session.id),
                ],
                limit=1,
            )
            new_session.cash_register_balance_start = (
    ...
    def _prepare_invoice_line(self, order_line):
        ...
        name = (
            order_line.product_id.default_code + " " + display_name
    ...
    def _get_pos_anglo_saxon_price_unit(self, product, partner_id, quantity):
        moves = (
            self.filtered(lambda o: o.partner_id.id == partner_id)
            .mapped("picking_ids.move_ids")
            ._filter_anglo_saxon_moves(product)
            .sorted(lambda x: x.date)
        )
        price_unit = product.with_company(self.company_id)._compute_average_price(
            0, quantity, moves
        )
        return price_unit
    ...
    def _compute_batch_amount_all(self):
        ...
        """
        amounts = {
            order_id: {"paid": 0, "return": 0, "taxed": 0, "taxes": 0}
            for order_id in self.ids
        }
        for order in self.env["pos.payment"].read_group(
        ...
        for order in self.env["pos.payment"].read_group(
            ["&", ("pos_order_id", "in", self.ids), ("amount", "<", 0)],
            ["pos_order_id", "amount"],
            ["pos_order_id"],
        ):
            amounts[order["pos_order_id"][0]]["return"] = order["amount"]
        for order in self.env["pos.order.line"].read_group(
            [("order_id", "in", self.ids)],
            ["order_id", "price_subtotal", "price_subtotal_incl"],
            ["order_id"],
        ):
            amounts[order["order_id"][0]]["taxed"] = order["price_subtotal_incl"]
            amounts[order["order_id"][0]]["taxes"] = (
                order["price_subtotal_incl"] - order["price_subtotal"]
            )
    ...
    def _unlink_except_draft_or_cancel(self):
        for pos_order in self.filtered(
            lambda pos_order: pos_order.state not in ["draft", "cancel"]
        ):
            raise UserError(
    ...
    def _get_partner_bank_id(self):
        ...
        has_pay_later = any(
            not pm.journal_id for pm in self.payment_ids.mapped("payment_method_id")
        )
        if has_pay_later:
        ...
        if self.config_id.cash_rounding:
                ...
                if rounding_line:
                    if rounding_line_difference:
                        rounding_line.with_context(check_move_validity=False).write(
                            {
                                "debit": rounding_applied < 0.0
                                and -rounding_applied
                                or 0.0,
                                "credit": rounding_applied > 0.0
                                and rounding_applied
                                or 0.0,
                    ...
                    self.env["account.move.line"].with_context(
                    ...
                    ).create(
                        {
                            ...
                            "move_id": new_move.id,
                            "currency_id": new_move.currency_id
                            if new_move.currency_id != new_move.company_id.currency_id
                            else False,
            ...
            if rounding_line_difference:
                ...
                existing_terms_line.with_context(skip_invoice_sync=True).write(
                    {
                        "debit": existing_terms_line_new_val > 0.0
                        and existing_terms_line_new_val
                        or 0.0,
                        "credit": existing_terms_line_new_val < 0.0
                        and -existing_terms_line_new_val
                        or 0.0,
                    }
                )
    ...
    def action_pos_order_paid(self):
        ...
        if (
            not self.config_id.cash_rounding
    ...
    def _prepare_aml_values_list_per_nature(self):
            ...
            aml_vals_list_per_nature["tax"].append(
                {
                    ...
                    "group_tax_id": None
                    if tax_rep.tax_id.id == tax_line_vals["tax_id"]
                    else tax_line_vals["tax_id"],
        ...
        if self.company_id.anglo_saxon_accounting and self.picking_ids.ids:
            ...
            for stock_move in stock_moves:
                expense_account = stock_move.product_id._get_product_accounts()[
                    "expense"
                ]
                stock_output_account = (
                    stock_move.product_id.categ_id.property_stock_account_output_categ_id
                )
    ...
    def _create_misc_reversal_move(self, payment_moves):
        ...
        reversal_entry = (
            self.env["account.move"]
            .with_context(default_journal_id=self.config_id.journal_id.id)
            .create(
                {
                    "journal_id": self.config_id.journal_id.id,
                    "date": fields.Date.context_today(self),
                    "ref": _(
                        "Reversal of POS closing entry %s for order %s from session %s",
                        self.session_move_id.name,
                        self.name,
        ...
        pos_account_receivable = (
            self.company_id.account_default_pos_receivable_account_id
        )
        reversal_entry_receivable = reversal_entry.line_ids.filtered(
            lambda l: l.account_id == pos_account_receivable
        )
    ...
    def _generate_pos_order_invoice(self):
        ...
        for order in self:
            ...
            ):  # If the session isn't closed this isn't needed.
    ...
    def action_receipt_to_customer(self, name, client, ticket):
        ...
        mail = (
            self.env["mail.mail"]
            .sudo()
            .create(self._prepare_mail_values(name, client, ticket))
        )
        mail.send()
    ...
    def _export_for_ui(self, order):
        ...
        return {
            ...
            "statement_ids": [
                [0, 0, payment] for payment in order.payment_ids.export_for_ui()
            ],
...
class PosOrderLine(models.Model):
    ...
    price_subtotal = fields.Float(
        string="Subtotal w/o Tax", digits=0, readonly=True, required=True
    )
    price_subtotal_incl = fields.Float(
        string="Subtotal", digits=0, readonly=True, required=True
    )
    ...
    def create(self, vals_list):
        for vals in vals_list:
            if vals.get("order_id") and not vals.get("name"):
                # set name based on the sequence specified on the config
                config = (
                    self.env["pos.order"].browse(vals["order_id"]).session_id.config_id
                )
    ...
    def _export_for_ui(self, orderline):
        return {
            ...
            "id": orderline.id,
            "pack_lot_ids": [
                [0, 0, lot] for lot in orderline.pack_lot_ids.export_for_ui()
            ],
    ...
    def _compute_margin(self):
        for line in self:
            ...
            line.margin_percent = (
                not float_is_zero(
                    line.price_subtotal, precision_rounding=line.currency_id.rounding
                )
