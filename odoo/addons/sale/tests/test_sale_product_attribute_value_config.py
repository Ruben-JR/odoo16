...
class TestSaleProductAttributeValueConfig(TestSaleProductAttributeValueCommon):
    ...
    def test_02_get_combination_info(self):
        ...
        res = self.computer._get_combination_info(
            self.env["product.template.attribute.value"], computer_variant.id
        )
        self.assertEqual(res["product_template_id"], self.computer.id)
        self.assertEqual(res["product_id"], computer_variant.id)
        self.assertEqual(res["display_name"], "Super Computer (256 GB, 8 GB, 1 To)")
        ...
        (
            ...
            currency_ratio,
            discount_ratio,
        ) = self._setup_pricelist()

        res = self.computer._get_combination_info(
            combination, add_qty=2, pricelist=pricelist
        )
        self.assertEqual(res["product_template_id"], self.computer.id)
        self.assertEqual(res["product_id"], computer_variant.id)
        self.assertEqual(res["display_name"], "Super Computer (256 GB, 8 GB, 1 To)")
        ...
        combination += self._get_product_template_attribute_value(
            self.keyboard_excluded
        )
        res = self.computer._get_combination_info(
            combination, add_qty=2, pricelist=pricelist
        )
    ...
    def test_03_get_combination_info_discount_policy(self):
        ...
        res = self.computer._get_combination_info(
            combination, add_qty=1, pricelist=pricelist
        )
    ...
    def _add_keyboard_attribute(self):
        ...
        self.computer_keyboard_attribute_lines = self.env[
            "product.template.attribute.line"
        ].create(
            {
                "product_tmpl_id": self.computer.id,
                "attribute_id": self.keyboard_attribute.id,
                "value_ids": [
                    (6, 0, [self.keyboard_included.id, self.keyboard_excluded.id])
                ],
            }
        )
        self.computer_keyboard_attribute_lines.product_template_value_ids[
            0
        ].price_extra = 5
        self.computer_keyboard_attribute_lines.product_template_value_ids[
            1
