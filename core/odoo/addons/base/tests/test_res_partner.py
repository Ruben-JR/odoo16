...
class TestPartner(TransactionCase):
    def test_name_search(self):
        ...
        self.assertEqual(
            set(i[0] for i in ns_res), set((test_partner | test_user.partner_id).ids)
        )

        ns_res = self.env["res.partner"].name_search(
            "Vlad", args=[("user_ids.email", "ilike", "vlad")]
        )
        ...
        ns_res = (
            ...
            .sudo()
            .name_search("Vlad", args=[("user_ids.email", "ilike", "vlad")])
        )
        self.assertEqual(set(i[0] for i in ns_res), set(test_user.partner_id.ids))
    ...
    def test_name_get(self):
        ...
        res_jetha = test_partner_jetha.with_context(
            show_address=1, address_inline=1
        ).name_get()
        self.assertEqual(
            res_jetha[0][1],
            "Jethala, Powder gali, Gokuldham Society",
            "name should contain comma separated name and address",
        )
        res_bhide = test_partner_bhide.with_context(
            show_address=1, address_inline=1
        ).name_get()
        self.assertEqual(
            res_bhide[0][1],
    ...
    def test_commercial_field_sync(self):
        ...
        Partner = self.env["res.partner"]
        company_1 = Partner.create(
            {"name": "company 1", "is_company": True, "vat": "BE0123456789"}
        )
        company_2 = Partner.create(
            {"name": "company 2", "is_company": True, "vat": "BE9876543210"}
        )
        ...
        self.assertEqual(
            partner.vat, company_1.vat, "VAT should be inherited from the company 1"
        )

        # create a delivery address for the partner
        delivery = Partner.create(
            {"name": "somewhere", "type": "delivery", "parent_id": partner.id}
        )
        self.assertEqual(
            delivery.commercial_partner_id.id,
            company_1.id,
            "Commercial partner should be recomputed",
        )
        self.assertEqual(
            delivery.vat, company_1.vat, "VAT should be inherited from the company 1"
        )
        ...
        self.assertEqual(
            partner.vat, company_2.vat, "VAT should be inherited from the company 2"
        )
        self.assertEqual(
            delivery.commercial_partner_id.id,
            company_2.id,
        ...
        self.assertEqual(
            delivery.vat,
            company_2.vat,
            "VAT should be inherited from the company 2 to delivery",
        )
    ...
    def test_lang_computation_form_view(self):
        ...
        self.assertEqual(
            partner_form.lang, "fr_FR", "New partner's lang should take user input"
        )
        with partner_form.child_ids.new() as child:
        ...
        self.assertEqual(partner.lang, "fr_FR")
        self.assertEqual(
            partner.child_ids.filtered(lambda p: p.name == "First Child").lang, "de_DE"
        )
        self.assertEqual(
    ...
    def test_display_address_missing_key(self):
        ...
        country = self.env["res.country"].create(
            {"name": "TestCountry", "address_format": "%(city)s %(zip)s"}
        )
