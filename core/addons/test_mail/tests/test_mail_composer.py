...
class TestMailComposer(TestMailCommon, TestRecipients):
    ...
    def setUpClass(cls):
        ...
        ).users -= cls.user_rendering_restricted
        ...
        cls.test_record_report = cls.env["ir.actions.report"]._render_qweb_pdf(
            cls.test_report, cls.test_record.ids
        )
...
class TestComposerForm(TestMailComposer):
    ...
    def test_mail_composer_comment_wtpl(self):
        ...
        )  # FIXME: currently not taking template value
        self.assertFalse(composer_form.auto_delete_message)
        self.assertEqual(composer_form.author_id, self.env.user.partner_id)
        self.assertEqual(
            composer_form.body, "<p>TemplateBody %s</p>" % self.test_record.name
        )
        self.assertEqual(composer_form.composition_mode, "comment")
        ...
        self.assertEqual(composer_form.res_id, self.test_record.id)
        self.assertEqual(
            composer_form.subject, "TemplateSubject %s" % self.test_record.name
        )
        self.assertEqual(composer_form.subtype_id, self.env.ref("mail.mt_comment"))
    ...
    def test_mail_composer_mass(self):
        ...
        self.assertFalse(
            composer_form.record_name,
            "MailComposer: mass mode should have void record name",
        )
        self.assertFalse(composer_form.reply_to)
        self.assertFalse(composer_form.reply_to_force_new)
        self.assertEqual(
            composer_form.res_id,
            self.test_records[0].id,
...
class TestComposerInternals(TestMailComposer):
            ...
            with self.subTest(composition_mode=composition_mode, batch=batch):
                ...
                if composition_mode == "comment":
                    ...
                    self.assertEqual(
                        len(generated),
                        1,
                        "MailComposer: should have 1 additional attachment for report",
                    )
                    self.assertEqual(
                        generated.name, f"TestReport for {self.test_record.name}.html"
                    )
    ...
    def test_mail_composer_configuration(self):
        ...
        for composition_mode, batch in (("comment", False), ("mass_mail", True)):
            with self.subTest(composition_mode=composition_mode, batch=batch):
                ...
                if composition_mode == "comment":
                    self.assertFalse(
                        composer.auto_delete, "TODO: should be updated with template"
                    )
                    self.assertFalse(composer.auto_delete_message)
                    self.assertTrue(
                        composer.email_add_signature,
                        "TODO: should be False as template negates this config",
                    )
                    self.assertEqual(composer.email_layout_xmlid, "mail.test_layout")
                    self.assertEqual(composer.message_type, "comment")
                    self.assertEqual(
        ...
        for composition_mode, batch in (("comment", False), ("mass_mail", True)):
            with self.subTest(composition_mode=composition_mode, batch=batch):
                ...
                if composition_mode == "comment":
                    self.assertFalse(
                        composer.body,
                        "Void template body resets while other fields not, maybe to fix",
                    )
                    self.assertEqual(composer.mail_server_id, self.mail_server_global)
                    self.assertEqual(composer.record_name, "Manual update")
                    self.assertEqual(
                ...
                if composition_mode == "comment":
                    ...
                    self.assertEqual(
                        composer.mail_server_id,
                        self.mail_server_global,
                        "TODO: Values are kept (should be reset ?)",
                    )
                    # self.assertEqual(composer.record_name, self.test_record.name)
                    self.assertEqual(
                        composer.record_name, "Manual update", "TODO: Reset not called"
                    )
        ...
        for composition_mode, batch in (("comment", False), ("mass_mail", True)):
            with self.subTest(composition_mode=composition_mode, batch=batch):
                ...
                if composition_mode == "comment":
                    ...
                    )  # should not change in comment mode
                else:
                    ...
                    self.assertFalse(
                        composer.reply_to_force_new
                    )  # should probably become True, not supported currently
    ...
    def test_mail_composer_parent(self):
        ...
        parent = self.test_record.message_post(
            body="Test", partner_ids=(self.partner_1 + self.partner_2).ids
        )
    ...
    def test_mail_composer_rights_attachments(self):
        ...
        template_1_attachment_name = list(template_1_attachments.mapped("name")) + [
            "TestReport for TestRecord (thanks TDE).html"
        ]
    ...
    def test_mail_composer_rights_portal(self):
        ...
        with patch.object(MailTestTicket, "check_access_rights", return_value=True):
            self.env["mail.compose.message"].with_user(portal_user).with_context(
                ...
                {"subject": "Subject", "body": "<p>Body text</p>", "partner_ids": []}
            )._action_send_mail()

            self.assertEqual(self.test_record.message_ids[0].body, "<p>Body text</p>")
            self.assertEqual(
                self.test_record.message_ids[0].author_id, portal_user.partner_id
            )
            ...
            self.env["mail.compose.message"].with_user(portal_user).with_context(
                ...
                {"subject": "Subject", "body": "<p>Body text 2</p>"}
            )._action_send_mail()
    ...
    def test_mail_composer_save_template(self):
        ...
        self.assertEqual(
            ...
            "%s: %s"
            % (
                self.env["ir.model"]._get(self.test_record._name).name,
                "Template Subject",
            ),
        )
        self.assertEqual(
...
class TestComposerResultsComment(TestMailComposer):
    ...
    def test_mail_composer_notifications_delete(self):
        ...
        self.assertEqual(
            len(self._new_mails.exists()), 0, "To fix: does not respect auto_delete"
        )
    ...
    def test_mail_composer_wtpl_complete(self):
        ...
        for email_layout_xmlid, use_lang in product(
            ...
            with self.subTest(email_layout_xmlid=email_layout_xmlid, use_lang=use_lang):
                ...
                self.assertEqual(
                    test_record.message_partner_ids, self.partner_employee_2
                )
                ...
                with self.mock_mail_gateway(
                    ...
                    self.assertEqual(
                        self._new_mails.mapped("state"), ["sent"] * len(self._new_mails)
                    )
                    self.assertEqual(
                        len(self._mails),
                        5,
                ...
                self.assertMailMail(
                    test_record.customer_id + new_partners,
                    "sent",
                    mail_message=message,
                    author=author,  # author is different in batch and monorecord mode (raw or rendered email_from)
                    email_values={
                ...
                self.assertEqual(
                    set(message.attachment_ids.mapped("res_id")), set(test_record.ids)
                )
                self.assertTrue(
...
class TestComposerResultsMass(TestMailComposer):
    ...
    def test_mail_composer_delete(self):
        ...
        self.assertFalse(
            ...
            "TODO: Template is forced over composer value, which is not correct",
        )
        self.assertEqual(
            len(self._new_msgs), 2, "Should have created 1 mail.mail per record"
        )
            ...
            with self.subTest(email_layout_xmlid=email_layout_xmlid, use_lang=use_lang):
                ...
                for record, exp_lang in zip(self.test_records, langs):
                    ...
                    email = self._find_sent_email(
                        ...
                        [record.customer_id.email_formatted],
                    )
                    self.assertTrue(bool(email), "Email not found, check recipients")
                    ...
                    self.assertEqual(
                        email["body"], f"<p>TemplateBody {record.name}</p>"
                    )
    ...
    def test_mail_composer_wtpl_recipients(self):
        ...
        composer_form = Form(
            self.env["mail.compose.message"].with_context(
                default_model="mail.test.ticket", default_template_id=self.template.id
            )
        )
        composer = composer_form.save()
        self.assertEqual(composer.composition_mode, "comment")
        with self.mock_mail_gateway(mail_unlink_sent=False), self.assertRaises(
            ValueError
        ):
            composer._action_send_mail()
    ...
    def test_mail_composer_wtpl_reply_to_force_new(self):
        ...
        composer_form.reply_to = (
            "{{ '\"' + object.name + '\" <%s>' % 'dynamic.reply.to@test.com' }}"
        )
        composer = composer_form.save()
...
class TestComposerResultsMassStatus(TestMailComposer):
    ...
    def test_mailing_duplicates_document_based(self):
        ...
        self.assertEqual(
            len(self._mails),
            4,
            "Should have sent 4 emails, and skipped an excluded and 2 duplicate emails.",
        )
        ...
        self.assertEqual(
            len(self._mails),
            6,
            "Should have sent 6 emails, and skipped an excluded email",
        )
