...
class SaleOrder(models.Model):
    ...
    visible_project = fields.Boolean(
        "Display project", compute="_compute_visible_project", readonly=True
    )
    ...
    def _compute_milestone_count(self):
        ...
        line_data = {
            res["sale_line_id"][0]: res["sale_line_id_count"] for res in read_group
        }
        for order in self:
            order.milestone_count = sum(
                line_data.get(line.id, 0) for line in order.order_line
            )
    ...
    def action_view_task(self):
        ...
        if (
            len(task_projects) == 1 and len(self.tasks_ids) > 1
        ):  # redirect to task of the project (with kanban stage, ...)
            action = (
                self.with_context(active_id=task_projects.id)
    ...
    def action_view_project_ids(self):
        ...
        if len(self.project_ids) == 1:
            action.update(
                {"views": [(view_form_id, "form")], "res_id": self.project_ids.id}
            )
        else:
    ...
    def action_view_milestone(self):
        ...
        default_sale_line = (
            default_project.sale_line_id or self.order_line and self.order_line[0]
        )
        return {
            "type": "ir.actions.act_window",
            "name": _("Milestones"),
            ...
            "views": [
                (
                    self.env.ref("sale_project.sale_project_milestone_view_tree").id,
                    "tree",
                )
            ],
            "view_mode": "tree",
