    ...
    if node.tag in ("field", "value"):
        ...
        if t == "file":
            ...
            if not module.get_module_resource(self.module, path):
                raise IOError(
                    "No such file or directory: '%s' in %s" % (path, self.module)
                )
...
class xml_import(object):
    ...
    def _test_xml_id(self, xml_id):
        if "." in xml_id:
            ...
            assert (
                "." not in id
            ), """The ID reference "%s" must contain
...
form: module.record_id""" % (
    ...
    def _tag_delete(self, rec):
        ...
        if d_search:
            ...
            try:
                ...
                _logger.warning(
                    "Skipping deletion for failed search `%r`", d_search, exc_info=True
                )
        ...
        if d_id:
            try:
                ...
                _logger.warning(
                    "Skipping deletion for missing XML ID `%r`", d_id, exc_info=True
                )
    ...
    def _tag_report(self, rec):
        ...
        warnings.warn(
            f"The <report> tag is deprecated, use a <record> tag for {xml_id!r}.",
            DeprecationWarning,
        )
        ...
        report = self.env["ir.actions.report"]._load_records(
    ...
    def _tag_act_window(self, rec):
        ...
        self._test_xml_id(xml_id)
        warnings.warn(
            f"The <act_window> tag is deprecated, use a <record> for {xml_id!r}.",
            DeprecationWarning,
        )
        view_id = False
        if rec.get("view_id"):
                ...
                if f_name in model._fields:
                    ...
                    if field_type == "many2one":
                        ...
                        if field.get("type") == "xml":
                            _logger.warning(
                                'HTML field %r is declared as `type="xml"`', f_name
                            )
            res[f_name] = f_val
        if extra_vals:
    ...
    def _tag_template(self, el):
        ...
        if groups:
            ...
            record.append(
                Field(
                    name="groups_id", eval="[Command.set([" + ", ".join(grp_lst) + "])]"
                )
            )
...
def convert_csv_import(
    ...
    datas = [line for line in reader if any(line)]
...
def convert_xml_import(
    ...
    obj = xml_import(
        cr, module, idref, mode, noupdate=noupdate, xml_filename=xml_filename
    )
