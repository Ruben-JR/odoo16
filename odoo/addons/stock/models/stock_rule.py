...
class StockRule(models.Model):
    ...
    def _get_message_dict(self):
        ...
        if self.action in ("push", "pull", "pull_push"):
            ...
            message_dict = {
                "pull": _(
                    "When products are needed in <b>%s</b>, <br/> <b>%s</b> are created from <b>%s</b> to fulfill the need.",
                    destination,
                    operation,
                    source,
                )
                + suffix,
                "push": _(
                    "When products arrive in <b>%s</b>, <br/> <b>%s</b> are created to send them in <b>%s</b>.",
                    source,
                    operation,
                    destination,
                ),
    ...
    def _run_pull(self, procurements):
        ...
        procurements = sorted(
            procurements,
            key=lambda proc: float_compare(
                proc[0].product_qty,
                0.0,
                precision_rounding=proc[0].product_uom.rounding,
            )
            > 0,
        )
        for procurement, rule in procurements:
            ...
            if rule.procure_method == "mts_else_mto":
                ...
                if (
                    ...
                    procure_method = "make_to_order"
                    for move in procurement.values.get(
    ...
    def _get_stock_move_values(
        ...
        partner = self.partner_address_id or (
            values.get("group_id", False) and values["group_id"].partner_id
        )
        if partner:
        ...
        move_values = {
            ...
            and values["product_packaging_id"].id,
        }
    ...
    def _get_lead_days(self, product, **values):
        ...
        """
        delay = sum(
            self.filtered(lambda r: r.action in ["pull", "pull_push"]).mapped("delay")
        )
        ...
        if global_visibility_days:
            delay_description.append(
                (
                    _("Global Visibility Days"),
                    _("+ %d day(s)") % int(global_visibility_days),
                )
            )
...
class ProcurementGroup(models.Model):
    ...
    def run(self, procurements, raise_user_error=True):
        ...
        for procurement in procurements:
            ...
            if procurement.product_id.type not in ("consu", "product") or float_is_zero(
    ...
    def _search_rule(self, route_ids, packaging_id, product_id, warehouse_id, domain):
        ...
        if route_ids:
            res = Rule.search(
                expression.AND([[("route_id", "in", route_ids.ids)], domain]),
        ...
        if not res and packaging_id:
            ...
            if packaging_routes:
                res = Rule.search(
                    expression.AND(
                        [[("route_id", "in", packaging_routes.ids)], domain]
                    ),
                    order="route_sequence, sequence",
                    limit=1,
                )
        if not res:
            ...
            if product_routes:
                res = Rule.search(
                    expression.AND([[("route_id", "in", product_routes.ids)], domain]),
                    order="route_sequence, sequence",
                    limit=1,
                )
        if not res and warehouse_id:
            ...
            if warehouse_routes:
                res = Rule.search(
                    expression.AND(
                        [[("route_id", "in", warehouse_routes.ids)], domain]
                    ),
                    order="route_sequence, sequence",
                    limit=1,
                )
