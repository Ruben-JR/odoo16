...
class account_journal(models.Model):
    ...
    def _fill_bank_cash_dashboard_data(self, dashboard_data):
        ...
        self.env["account.bank.statement"].browse(
            i for i in last_statements.values() if i
        ).mapped(
            "balance_end_real"
        )  # prefetch
        ...
        for journal in bank_cash_journals:
            last_statement = self.env["account.bank.statement"].browse(
    ...
    def _fill_sale_purchase_dashboard_data(self, dashboard_data):
        ...
        query, params = sale_purchase_journals._get_open_bills_to_pay_query().select(
            *field_list
        )
        self.env.cr.execute(query, params)
        query_results_to_pay = group_by_journal(self.env.cr.dictfetchall())

        query, params = sale_purchase_journals._get_draft_bills_query().select(
            *field_list
        )
        self.env.cr.execute(query, params)
        query_results_drafts = group_by_journal(self.env.cr.dictfetchall())

        query, params = sale_purchase_journals._get_late_bills_query().select(
            *field_list
        )
        self.env.cr.execute(query, params)
        ...
        curr_cache = {}
        sale_purchase_journals._fill_dashboard_data_count(
            dashboard_data, "account.move", "entries_count", []
        )
        for journal in sale_purchase_journals:
            currency = journal.currency_id or journal.company_id.currency_id
            (number_waiting, sum_waiting) = self._count_results_and_sum_amounts(
                query_results_to_pay[journal.id], currency, curr_cache=curr_cache
            )
            (number_draft, sum_draft) = self._count_results_and_sum_amounts(
                query_results_drafts[journal.id], currency, curr_cache=curr_cache
            )
            (number_late, sum_late) = self._count_results_and_sum_amounts(
                late_query_results[journal.id], currency, curr_cache=curr_cache
            )
    ...
    def _count_results_and_sum_amounts(
        self, results_dict, target_currency, curr_cache=None
    ):
    ...
    def _get_journal_dashboard_outstanding_payments(self):
        ...
        for journal in self:
            ...
            result[journal.id] = self._count_results_and_sum_amounts(
                query_result[journal.id], currency, curr_cache
            )
        return result
