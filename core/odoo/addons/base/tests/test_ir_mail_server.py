...
class TestIrMailServer(TransactionCase, MockSmtplibCase):
    ...
    def test_mail_server_priorities(self):
        ...
        mail_server, mail_from = self.env["ir.mail_server"]._find_mail_server(
            email_from='"Name name@strange.name" <specific_user@test.com>'
        )
        self.assertEqual(
            mail_server, self.server_user, "Must extract email from full name"
        )
        self.assertEqual(
            mail_from,
            '"Name name@strange.name" <specific_user@test.com>',
        ...
        self.assertEqual(
            mail_from, "specific_user@test.com", "Should not change the mail from"
        )

        mail_server, mail_from = self.env["ir.mail_server"]._find_mail_server(
            email_from="unknown_email@test.com"
        )
        ...
        self.assertEqual(
            mail_from, "unknown_email@TEST.COM", "Domain is case insensitive"
        )
        ...
        self.assertEqual(
            mail_server, self.server_notification, "Should take the notification email"
        )
    ...
    def test_mail_server_send_email(self):
        ...
        self.env["ir.config_parameter"].sudo().set_param(
            "mail.catchall.domain", "ééééééé.com"
        )
        with self.mock_smtplib_connection():
    ...
    @mute_logger("odoo.models.unlink")
    @patch.dict(
        config.options, {"from_filter": "test.com", "smtp_server": "example.com"}
    )
    ...
    def test_mail_server_get_email_addresses(self):
        ...
        self.env["ir.config_parameter"].set_param(
            "mail.default.from", "notifications@example.com"
        )
