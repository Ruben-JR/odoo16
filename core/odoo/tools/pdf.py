...
def add_banner(pdf_stream, text=None, logo=False, thickness=2 * cm):
    ...
    odoo_logo = Image.open(
        file_open("base/static/img/main_partner-image.png", mode="rb")
    )
...
PdfFileReader.__init__ = (
    lambda self, stream, strict=True, warndest=None, overwriteWarnings=True: old_init(
        self, stream=stream, strict=strict, warndest=None, overwriteWarnings=False
    )
)
...
class OdooPdfFileWriter(PdfFileWriter):
    ...
    def embed_odoo_attachment(self, attachment, subtype=None):
        ...
        self.addAttachment(
            attachment.name, attachment.raw, subtype=subtype or attachment.mimetype
        )
    ...
    def _create_attachment_object(self, attachment):
        ...
        if attachment.get("description"):
            filespec_object.update(
                {NameObject("/Desc"): createStringObject(attachment["description"])}
            )
