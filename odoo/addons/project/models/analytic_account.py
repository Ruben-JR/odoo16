...
class AccountAnalyticAccount(models.Model):
    ...
    def _compute_project_count(self):
        ...
        mapping = {
            m["analytic_account_id"][0]: m["analytic_account_id_count"]
    ...
    def _check_company_id(self):
        for record in self:
            if record.company_id and not all(
                record.company_id == c for c in record.project_ids.mapped("company_id")
            ):
                raise UserError(
                    _(
                        "You cannot change the company of an analytic account if it is related to a project."
                    )
                )
