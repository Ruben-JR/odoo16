...
class WebsiteSaleSlides(WebsiteSlides):
    ...
    def _prepare_additional_channel_values(self, values, **kwargs):
        values = super(WebsiteSaleSlides, self)._prepare_additional_channel_values(
            values, **kwargs
        )
        ...
        if channel.enroll == "payment":
            ...
            if product:
                ...
                values[
                    "product_info"
                ] = channel.product_id.product_tmpl_id._get_combination_info(
                    product_id=channel.product_id.id, pricelist=pricelist
                )
