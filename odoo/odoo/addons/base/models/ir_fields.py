...
class IrFieldsConverter(models.AbstractModel):
    ...
    def _str_to_boolean(self, model, field, value):
        ...
        return True, [
            self._format_import_error(
                ValueError,
                _("Unknown value '%s' for boolean field '%%(field)s'"),
                value,
    ...
    _str_to_reference = (
        _str_to_char
    ) = _str_to_text = _str_to_binary = _str_to_html = _str_id
    ...
    def _str_to_many2many(self, model, field, value):
        ...
        if field.name in self._context.get("import_set_empty_fields", []) and any(
            [id is None for id in ids]
        ):
            ids = [id for id in ids if id]
        elif field.name in self._context.get("import_skip_records", []) and any(
            [id is None for id in ids]
        ):
    ...
    def _str_to_one2many(self, model, field, records):
        name_create_enabled_fields = (
            self._context.get("name_create_enabled_fields") or {}
        )
        prefix = field.name + "/"
