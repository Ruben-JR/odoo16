...
class TestIrCron(TransactionCase, CronMixinCase):
    ...
    def setUp(self):
        ...
        self.test_partner = self.env["res.partner"].create({"name": "TestCronRecord"})
    ...
    def test_cron_direct_trigger(self):
        ...
        self.assertEqual(
            fields.Datetime.to_string(self.cron.lastcall), "2020-10-22 08:00:00"
        )
    ...
    def test_cron_keep_future_triggers(self):
        ...
        ready_jobs = self.registry["ir.cron"]._get_all_ready_jobs(self.cr)
        self.assertIn(
            self.cron.id, [job["id"] for job in ready_jobs], "cron should be ready"
        )
