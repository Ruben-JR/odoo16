...
class TestAccountMoveInalterableHash(AccountTestInvoicingCommon):
    ...
    def test_account_move_hash_integrity_report(self):
        ...
        moves = self.init_invoice(
            "out_invoice", self.partner_a, "2023-01-01", amounts=[1000, 2000]
        ) | self.init_invoice(
            "out_invoice", self.partner_b, "2023-01-02", amounts=[1000, 2000]
        )
        moves.action_post()
        ...
        self.assertEqual(
            integrity_check["msg_cover"], "This journal is not in strict mode."
        )
        ...
        new_moves = (
            self.init_invoice(
                "out_invoice", self.partner_a, "2023-01-03", amounts=[1000, 2000]
            )
            | self.init_invoice(
                "out_invoice", self.partner_b, "2023-01-04", amounts=[1000, 2000]
            )
            | self.init_invoice(
                "out_invoice", self.partner_a, "2023-01-05", amounts=[1000, 2000]
            )
            | self.init_invoice(
                "out_invoice", self.partner_b, "2023-01-06", amounts=[1000, 2000]
            )
            | self.init_invoice(
                "out_invoice", self.partner_a, "2023-01-07", amounts=[1000, 2000]
            )
        )
        ...
        integrity_check = moves.company_id._check_hash_integrity()["results"][0]
        self.assertRegex(
            integrity_check["msg_cover"], f"Entries are hashed from {moves[2].name}.*"
        )
        self.assertEqual(
            integrity_check["first_move_date"],
            format_date(self.env, fields.Date.to_string(moves[2].date)),
        )
        self.assertEqual(
            integrity_check["last_move_date"],
        ...
        Model.write(
            moves[-1].line_ids[0], {"partner_id": self.partner_a.id}
        )  # Revert the previous change
    ...
    def test_account_move_hash_versioning_1(self):
        ...
        """
        self.init_invoice(
            "out_invoice", self.partner_a, "2023-01-01", amounts=[1000, 2000], post=True
        )  # Not hashed
        self.company_data["default_journal_sale"].restrict_mode_hash_table = True
        moves = (
            self.init_invoice(
                "out_invoice", self.partner_a, "2023-01-02", amounts=[1000, 2000]
            )
            | self.init_invoice(
                "out_invoice", self.partner_b, "2023-01-03", amounts=[1000, 2000]
            )
            | self.init_invoice(
                "out_invoice", self.partner_b, "2023-01-04", amounts=[1000, 2000]
            )
        ...
        integrity_check = moves.company_id._check_hash_integrity()["results"][0]
        self.assertRegex(
            integrity_check["msg_cover"], f"Entries are hashed from {moves[0].name}.*"
        )
        self.assertEqual(
            integrity_check["first_move_date"],
    ...
    def test_account_move_hash_versioning_v1_to_v2(self):
        ...
        Model.write(
            moves[4], {"date": fields.Date.from_string("2023-01-02")}
        )  # Revert the previous change
        moves_v1_bis = (
            self.init_invoice(
                "out_invoice", self.partner_a, "2023-01-10", amounts=[1000, 2000]
            )
            | self.init_invoice(
                "out_invoice", self.partner_b, "2023-01-11", amounts=[1000, 2000]
            )
            | self.init_invoice(
                "out_invoice", self.partner_b, "2023-01-12", amounts=[1000, 2000]
            )
        )
    ...
    def test_account_move_hash_versioning_3(self):
        ...
        moves_v3 = (
            self.init_invoice(
                "out_invoice", self.partner_a, "2023-01-01", amounts=[30 * 0.17, 2000]
            )
            | self.init_invoice(
                "out_invoice", self.partner_b, "2023-01-02", amounts=[1000, 2000]
            )
        ...
        self.assertRegex(
            integrity_check_v3["msg_cover"],
            f"Entries are hashed from {moves_v3[0].name}.*",
        )
