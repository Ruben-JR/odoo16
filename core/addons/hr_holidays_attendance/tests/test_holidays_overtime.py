...
class TestHolidaysOvertime(TransactionCase):
    ...
    def test_deduct_button_visibility(self):
        with self.with_user("user"):
            ...
            self.new_attendance(
                check_in=datetime(2021, 1, 2, 8), check_out=datetime(2021, 1, 2, 18)
            )
            self.assertEqual(
                self.user.total_overtime, 10, "Should have 10 hours of overtime"
            )
    ...
    def test_leave_adjust_overtime(self):
        ...
        self.assertTrue(
            leave.overtime_id.adjustment, "An adjustment overtime should be created"
        )
        ...
        leave.unlink()
        self.assertFalse(
            overtime.exists(), "Overtime should be deleted along with the leave"
        )
        self.assertEqual(self.employee.total_overtime, 8)
