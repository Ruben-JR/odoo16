...
class AccountAnalyticAccount(models.Model):
    ...
    def _check_company_consistency(self):
        ...
        if self._cr.fetchone():
            raise UserError(
                _(
                    "You can't set a different company on your analytic account since there are some analytic items linked to it."
                )
            )
    ...
    def _compute_root_plan(self):
        for account in self:
            account.root_plan_id = (
                int(account.plan_id.parent_path[:-1].split("/")[0])
                if account.plan_id.parent_path
                else None
            )
