...
class Product(models.Model):
    ...
    orderpoint_ids = fields.One2many(
        "stock.warehouse.orderpoint", "product_id", "Minimum Stock Rules"
    )
    ...
    nbr_reordering_rules = fields.Integer(
        "Reordering Rules", compute="_compute_nbr_reordering_rules", compute_sudo=False
    )
    ...
    putaway_rule_ids = fields.One2many(
        "stock.putaway.rule", "product_id", "Putaway Rules"
    )
    storage_category_capacity_ids = fields.One2many(
        "stock.storage.category.capacity", "product_id", "Storage Category Capacity"
    )
            ...
            for item in values:
                if isinstance(item, int):
                    ...
                    domain = expression.OR(
                        [[(self.env[model]._rec_name, "ilike", item)], domain]
                    )
            if domain:
    ...
    def _get_domain_locations_new(self, location_ids):
        ...
        for location in locations:
            ...
            dest_loc_domain = (
                dest_loc_domain and ["|"] + dest_loc_domain or dest_loc_domain
            )
            dest_loc_domain.append(
                ("location_dest_id.parent_path", "=like", location.parent_path + "%")
            )
    ...
    def view_header_get(self, view_id, view_type):
        ...
        if (
            ...
            and self._context.get("active_model") == "stock.location"
        ):
            return _(
                "Products: %(location)s",
    ...
    def action_update_quantity_on_hand(self):
        return self.product_tmpl_id.with_context(
            default_product_id=self.id, create=True
    ...
    def write(self, values):
        if "active" in values:
            self.filtered(lambda p: p.active != values["active"]).with_context(
                active_test=False
            ).orderpoint_ids.write({"active": values["active"]})
    ...
    def _filter_to_unlink(self):
        ...
        lines = self.env["stock.lot"]._read_group(
            domain, ["product_id"], ["product_id"]
        )
        linked_product_ids = [group["product_id"][0] for group in lines]
        return super(
            Product, self - self.browse(linked_product_ids)
        )._filter_to_unlink()
...
class ProductTemplate(models.Model):
    ...
    def write(self, vals):
        ...
        if "uom_id" in vals:
            ...
            done_moves = self.env["stock.move"].search(
                [
                    (
                        "product_id",
                        "in",
                        updated.with_context(active_test=False)
            ...
            if done_moves:
                raise UserError(
                    _(
                        "You cannot change the unit of measure as there are already stock moves for this product. If you want to change the unit of measure, you should rather archive this product and create a new one."
                    )
                )
        if (
            "type" in vals
            and vals["type"] != "product"
        ...
        if any("type" in vals and vals["type"] != prod_tmpl.type for prod_tmpl in self):
            ...
            if existing_done_move_lines:
                raise UserError(
                    _("You can not change the type of a product that was already used.")
                )
            ...
            if existing_reserved_move_lines:
                raise UserError(
                    _(
                        "You can not change the type of a product that is currently reserved on a stock move. If you need to change the type, you should first unreserve the stock move."
                    )
                )
        if (
            "type" in vals
    ...
    def action_open_routes_diagram(self):
        ...
        if (
            not self.user_has_groups("stock.group_stock_multi_warehouses")
            and len(products) == 1
        ):
    ...
    def action_product_tmpl_forecast_report(self):
        self.ensure_one()
        if self.env.ref(
            "stock.stock_replenishment_product_template_action",
...
class ProductCategory(models.Model):
    ...
    putaway_rule_ids = fields.One2many(
        "stock.putaway.rule", "category_id", "Putaway Rules"
    )
