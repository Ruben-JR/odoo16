...
class EventRegistration(models.Model):
    ...
    sale_order_line_id = fields.Many2one(
        "sale.order.line", string="Sales Order Line", ondelete="cascade", copy=False
    )
    ...
    def write(self, vals):
        ...
        if vals.get("event_ticket_id"):
            self.filtered(
                lambda registration: registration.event_ticket_id
                and registration.event_ticket_id.id != vals["event_ticket_id"]
            )._sale_order_ticket_type_change_notify(
        ...
        res.update(
            {
                ...
                "payment_status_value": dict(
                    self._fields["payment_status"]._description_selection(self.env)
                )[self.payment_status],
