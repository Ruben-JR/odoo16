...
class TestDiscussFullPerformance(TransactionCase):
    ...
    def test_init_messaging(self):
        ...
        self.channel_channel_public_1 = self.env["mail.channel"].browse(
        ...
        self.channel_channel_public_2 = self.env["mail.channel"].browse(
            self.env["mail.channel"].channel_create(
        ...
        self.channel_channel_group_1 = self.env["mail.channel"].browse(
            self.env["mail.channel"].channel_create(
        ...
        self.channel_channel_group_2 = self.env["mail.channel"].browse(
            self.env["mail.channel"].channel_create(
        ...
        # create chats
        self.channel_chat_1 = self.env["mail.channel"].browse(
            self.env["mail.channel"].channel_get(
                (self.users[0] + self.users[14]).partner_id.ids
            )["id"]
        )
        self.channel_chat_2 = self.env["mail.channel"].browse(
            self.env["mail.channel"].channel_get(
                (self.users[0] + self.users[15]).partner_id.ids
            )["id"]
        )
        self.channel_chat_3 = self.env["mail.channel"].browse(
            self.env["mail.channel"].channel_get(
                (self.users[0] + self.users[2]).partner_id.ids
            )["id"]
        )
        self.channel_chat_4 = self.env["mail.channel"].browse(
            self.env["mail.channel"].channel_get(
                (self.users[0] + self.users[3]).partner_id.ids
            )["id"]
        )
        # create groups
        self.channel_group_1 = self.env["mail.channel"].browse(
        ...
        self.channel_livechat_1 = self.env["mail.channel"].browse(
        ...
        self.channel_livechat_2 = self.env["mail.channel"].browse(
        ...
        self.channel_livechat_2.with_user(
            self.env.ref("base.public_user")
        ).sudo().message_post(body="test")
        # add needaction
        self.users[0].notification_type = "inbox"
        message = self.channel_channel_public_1.message_post(
            body="test",
            ...
            "channels": [
                {
                                    ...
                                    [
                                        {
                                            ...
                                            "id": self.channel_general.channel_member_ids.filtered(
                    ...
                    "is_minimized": False,
                    "is_pinned": True,
                    "last_interest_dt": self.channel_general.channel_member_ids.filtered(
                    ...
                    "last_message_id": next(
                        res["message_id"]
                        for res in self.channel_general._channel_last_message_ids()
                    ),
                ...
                },
                {
                                    ...
                                    [
                                        {
                                            ...
                                            "id": self.channel_channel_public_1.channel_member_ids.filtered(
                                                lambda m: m.partner_id
                                                == self.users[0].partner_id
                                            ).id,
                                            "persona": {
                    ...
                    "is_minimized": False,
                    "is_pinned": True,
                    "last_interest_dt": self.channel_channel_public_1.channel_member_ids.filtered(
                        lambda m: m.partner_id == self.users[0].partner_id
                    ).last_interest_dt.strftime(
                        DEFAULT_SERVER_DATETIME_FORMAT
                    ),
                    "last_message_id": next(
                        res["message_id"]
                        for res in self.channel_channel_public_1._channel_last_message_ids()
                    ),
                ...
                },
                {
                                    ...
                                    [
                                        {
                                            ...
                                            "id": self.channel_channel_public_2.channel_member_ids.filtered(
                                                lambda m: m.partner_id
                                                == self.users[0].partner_id
                                            ).id,
                                            "persona": {
                    ...
                    "is_minimized": False,
                    "is_pinned": True,
                    "last_interest_dt": self.channel_channel_public_2.channel_member_ids.filtered(
                        lambda m: m.partner_id == self.users[0].partner_id
                    ).last_interest_dt.strftime(
                        DEFAULT_SERVER_DATETIME_FORMAT
                    ),
                    "last_message_id": next(
                        res["message_id"]
                        for res in self.channel_channel_public_2._channel_last_message_ids()
                    ),
                    "message_needaction_counter": 0,
                ...
                },
                {
                                    ...
                                    [
                                        {
                                            ...
                                            "id": self.channel_channel_group_1.channel_member_ids.filtered(
                                                lambda m: m.partner_id
                                                == self.users[0].partner_id
                                            ).id,
                                            "persona": {
                    ...
                    "is_minimized": False,
                    "is_pinned": True,
                    "last_interest_dt": self.channel_channel_group_1.channel_member_ids.filtered(
                        lambda m: m.partner_id == self.users[0].partner_id
                    ).last_interest_dt.strftime(
                        DEFAULT_SERVER_DATETIME_FORMAT
                    ),
                    "last_message_id": next(
                        res["message_id"]
                        for res in self.channel_channel_group_1._channel_last_message_ids()
                    ),
                    "message_needaction_counter": 0,
                    "name": "group restricted channel 1",
                    "rtcSessions": [("insert", [])],
                ...
                },
                {
                                    ...
                                    [
                                        {
                                            ...
                                            "id": self.channel_channel_group_2.channel_member_ids.filtered(
                                                lambda m: m.partner_id
                                                == self.users[0].partner_id
                                            ).id,
                                            "persona": {
                    ...
                    "is_minimized": False,
                    "is_pinned": True,
                    "last_interest_dt": self.channel_channel_group_2.channel_member_ids.filtered(
                        lambda m: m.partner_id == self.users[0].partner_id
                    ).last_interest_dt.strftime(
                        DEFAULT_SERVER_DATETIME_FORMAT
                    ),
                    "last_message_id": next(
                        res["message_id"]
                        for res in self.channel_channel_group_2._channel_last_message_ids()
                    ),
                    "message_needaction_counter": 0,
                    "name": "group restricted channel 2",
                    "rtcSessions": [("insert", [])],
                ...
                },
                {
                                    ...
                                    [
                                        {
                                            ...
                                            "id": self.channel_group_1.channel_member_ids.filtered(
                                                lambda m: m.partner_id
                                                == self.users[0].partner_id
                                            ).id,
                                            "persona": {
                    ...
                    "is_minimized": False,
                    "is_pinned": True,
                    "last_interest_dt": self.channel_group_1.channel_member_ids.filtered(
                        lambda m: m.partner_id == self.users[0].partner_id
                    ).last_interest_dt.strftime(
                        DEFAULT_SERVER_DATETIME_FORMAT
                    ),
                ...
                },
                {
                                    ...
                                    [
                                        {
                                            ...
                                            "id": self.channel_chat_1.channel_member_ids.filtered(
                                                lambda m: m.partner_id
                                                == self.users[0].partner_id
                                            ).id,
                                            "persona": {
                    ...
                    "is_minimized": False,
                    "is_pinned": True,
                    "last_interest_dt": self.channel_chat_1.channel_member_ids.filtered(
                        lambda m: m.partner_id == self.users[0].partner_id
                    ).last_interest_dt.strftime(DEFAULT_SERVER_DATETIME_FORMAT),
                    "last_message_id": False,
                    "message_needaction_counter": 0,
                ...
                },
                {
                                    ...
                                    [
                                        {
                                            ...
                                            "id": self.channel_chat_2.channel_member_ids.filtered(
                                                lambda m: m.partner_id
                                                == self.users[0].partner_id
                                            ).id,
                                            "persona": {
                    ...
                    "is_minimized": False,
                    "is_pinned": True,
                    "last_interest_dt": self.channel_chat_2.channel_member_ids.filtered(
                        lambda m: m.partner_id == self.users[0].partner_id
                    ).last_interest_dt.strftime(DEFAULT_SERVER_DATETIME_FORMAT),
                    "last_message_id": False,
                    "message_needaction_counter": 0,
                ...
                },
                {
                                    ...
                                    [
                                        {
                                            ...
                                            "id": self.channel_chat_3.channel_member_ids.filtered(
                                                lambda m: m.partner_id
                                                == self.users[0].partner_id
                                            ).id,
                                            "persona": {
                    ...
                    "is_minimized": False,
                    "is_pinned": True,
                    "last_interest_dt": self.channel_chat_3.channel_member_ids.filtered(
                        lambda m: m.partner_id == self.users[0].partner_id
                    ).last_interest_dt.strftime(DEFAULT_SERVER_DATETIME_FORMAT),
                    "last_message_id": False,
                    "message_needaction_counter": 0,
                    ...
                    "seen_partners_info": [
                        ...
                        },
                        {
                            "fetched_message_id": False,
                            "id": self.channel_chat_3.channel_member_ids.filtered(
                                lambda m: m.partner_id == self.users[2].partner_id
                            ).id,
                ...
                },
                {
                                    ...
                                    [
                                        {
                                            ...
                                            "id": self.channel_chat_4.channel_member_ids.filtered(
                                                lambda m: m.partner_id
                                                == self.users[0].partner_id
                                            ).id,
                                            "persona": {
                    ...
                    "is_minimized": False,
                    "is_pinned": True,
                    "last_interest_dt": self.channel_chat_4.channel_member_ids.filtered(
                        lambda m: m.partner_id == self.users[0].partner_id
                    ).last_interest_dt.strftime(DEFAULT_SERVER_DATETIME_FORMAT),
                    "last_message_id": False,
                    "message_needaction_counter": 0,
                    ...
                    "seen_partners_info": [
                        ...
                        },
                        {
                            "fetched_message_id": False,
                            "id": self.channel_chat_4.channel_member_ids.filtered(
                                lambda m: m.partner_id == self.users[3].partner_id
                            ).id,
                ...
                },
                {
                                    ...
                                    [
                                        {
                                            ...
                                            "id": self.channel_livechat_1.channel_member_ids.filtered(
                                                lambda m: m.partner_id
                                                == self.users[0].partner_id
                                            ).id,
                                            "persona": {
                    ...
                    "is_minimized": False,
                    "is_pinned": True,
                    "last_interest_dt": self.channel_livechat_1.channel_member_ids.filtered(
                        lambda m: m.partner_id == self.users[0].partner_id
                    ).last_interest_dt.strftime(
                        DEFAULT_SERVER_DATETIME_FORMAT
                    ),
                    "last_message_id": next(
                        res["message_id"]
                        for res in self.channel_livechat_1._channel_last_message_ids()
                    ),
                    "message_needaction_counter": 0,
                ...
                },
                {
                                    ...
                                    [
                                        {
                                            ...
                                            "id": self.channel_livechat_2.channel_member_ids.filtered(
                                                lambda m: m.partner_id
                                                == self.users[0].partner_id
                                            ).id,
                                            "persona": {
                    ...
                    "is_minimized": False,
                    "is_pinned": True,
                    "last_interest_dt": self.channel_livechat_2.channel_member_ids.filtered(
                        lambda m: m.partner_id == self.users[0].partner_id
                    ).last_interest_dt.strftime(
                        DEFAULT_SERVER_DATETIME_FORMAT
                    ),
                    "last_message_id": next(
                        res["message_id"]
                        for res in self.channel_livechat_2._channel_last_message_ids()
                    ),
                    "message_needaction_counter": 0,
