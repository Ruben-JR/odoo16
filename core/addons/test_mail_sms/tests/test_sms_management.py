...
class TestSMSWizards(TestSMSActionsCommon):
    ...
    def test_sms_resend(self):
        ...
        with self.with_user("employee"):
            wizard = (
                self.env["sms.resend"]
                .with_context(default_mail_message_id=self.msg.id)
                .create({})
            )
            wizard.write(
                {
                    "recipient_ids": [
                        (1, r.id, {"resend": True}) for r in wizard.recipient_ids
                    ]
                }
            )
            ...
            wizard.write(
                {
                    "recipient_ids": [
                        (
                            1,
                            r.id,
                            {"resend": True, "sms_number": self.random_numbers[idx]},
                        )
                        for idx, r in enumerate(wizard.recipient_ids.sorted())
                    ]
                }
            )
            with self.mockSMSGateway():
    ...
    def test_sms_resend_internals(self):
        ...
        with self.with_user("employee"):
            ...
            self.assertEqual(
                set(wizard.mapped("recipient_ids.partner_name")),
                set((self.partner_1 | self.partner_2).mapped("display_name")),
            )
            wizard.write(
                {
                    "recipient_ids": [
            ...
            wizard.write(
                {
                    "recipient_ids": [
                        (
                            ...
                            {
                                "resend": True
                                if r.partner_id == self.partner_1
                                else False
                            },
                        )
                        for r in wizard.recipient_ids
                    ]
                }
            )
            with self.mockSMSGateway():
