...
class UoMCategory(models.Model):
    ...
    reference_uom_id = fields.Many2one(
        "uom.uom", "Reference UoM", store=False
    )  # Dummy field to keep track of reference uom change
    ...
    def _onchange_uom_ids(self):
        if len(self.uom_ids) == 1:
            ...
            if reference_count == 0 and self._origin.id:
                raise UserError(
                    _(
                        "UoM category %s must have at least one reference unit of measure.",
                        self.name,
                    )
                )
            if self.reference_uom_id:
                new_reference = self.uom_ids.filtered(
                    lambda o: o.uom_type == "reference"
                    and o._origin.id != self.reference_uom_id.id
                )
            else:
                new_reference = self.uom_ids.filtered(
                    lambda o: o.uom_type == "reference"
                    and o._origin.uom_type != "reference"
                )
            if new_reference:
                other_uoms = (
                    self.uom_ids.filtered(lambda u: u._origin.id) - new_reference
                )
                for uom in other_uoms:
                    uom.factor = uom._origin.factor / (
                        new_reference._origin.factor or 1
                    )
                    if uom.factor > 1:
...
class UoM(models.Model):
    ...
    ratio = fields.Float(
        "Combined Ratio", compute="_compute_ratio", inverse="_set_ratio", store=False
    )
    ...
    def _check_category_reference_uniqueness(self):
        for category in self.category_id:
            ...
            if reference_count > 1:
                ...
                raise ValidationError(
                    _("UoM category %s should have a reference unit of measure.")
                    % category.name
                )
    ...
    def write(self, values):
        ...
        if (
            "uom_type" not in values or values["uom_type"] != "reference"
        ) and not self.env.context.get("allow_to_change_reference"):
