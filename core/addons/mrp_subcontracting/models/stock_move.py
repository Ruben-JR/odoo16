...
class StockMove(models.Model):
    ...
    def _compute_show_details_visible(self):
        ...
        for move in self:
            ...
            if self.env.user.has_group("base.group_portal"):
                move.show_details_visible = any(
                    not p._has_been_recorded()
                    for p in move._get_subcontract_production()
                )
                continue
            productions = move._get_subcontract_production()
            if (
        ...
        if "date" in values:
            for move in self:
                if move.state in ("done", "cancel") or not move.is_subcontract:
                    continue
                move.move_orig_ids.production_id.filtered(
    ...
    def action_show_details(self):
        ...
        if self.state != "done" and (
            self._subcontrating_should_be_record()
            or self._subcontrating_can_be_record()
        ):
            return self._action_record_components()
        action = super(StockMove, self).action_show_details()
        if self.is_subcontract and all(
            p._has_been_recorded() for p in self._get_subcontract_production()
        ):
            action["views"] = [
            ...
            if self.picking_type_id.show_reserved:
                action["views"] = [
                    (
                        self.env.ref(
                            "mrp_subcontracting.mrp_subcontracting_view_stock_move_operations"
                        ).id,
                        "form",
                    )
                ]
                ...
                action["views"] = [
                    (
                        self.env.ref(
                            "mrp_subcontracting.mrp_subcontracting_view_stock_move_nosuggest_operations"
                        ).id,
                        "form",
                    )
                ]
    ...
    def action_show_subcontract_details(self):
        ...
        if self.env.user.has_group("base.group_portal"):
            form_view = self.env.ref(
                "mrp_subcontracting.mrp_subcontracting_portal_move_form_view"
            )
    ...
    def _set_quantities_to_reservation(self):
        move_untouchable = self.filtered(
            lambda m: m.is_subcontract
            and m._get_subcontract_production()._has_tracked_component()
        )
        return super(
            StockMove, self - move_untouchable
        )._set_quantities_to_reservation()

    def _action_cancel(self):
        for move in self:
            if move.is_subcontract:
                active_production = move.move_orig_ids.production_id.filtered(
                    lambda p: p.state not in ("done", "cancel")
                )
                moves = self.env.context.get("moves_todo")
                if (
                    not moves
                    or active_production not in moves.move_orig_ids.production_id
                ):
    ...
    def _action_confirm(self, merge=True, merge_into=False):
        ...
        for move in self:
            ...
            if (
                float_compare(
                    move.product_qty, 0, precision_rounding=move.product_uom.rounding
                )
                <= 0
            ):
    ...
    def _has_tracked_subcontract_components(self):
        return any(
            m.has_tracking != "none"
            for m in self._get_subcontract_production().move_raw_ids
        )
    ...
    def _should_bypass_set_qty_producing(self):
        if (
            self.production_id | self.raw_material_production_id
    ...
    def _update_subcontract_order_qty(self, new_quantity):
        for move in self:
            ...
            productions = move.move_orig_ids.production_id.filtered(
                lambda p: p.state not in ("done", "cancel")
            )[::-1]
