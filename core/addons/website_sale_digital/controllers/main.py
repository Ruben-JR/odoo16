...
class WebsiteSaleDigital(CustomerPortal):
    ...
    def portal_order_page(self, order_id=None, **post):
        response = super(WebsiteSaleDigital, self).portal_order_page(
            order_id=order_id, **post
        )
    ...
    def download_attachment(self, attachment_id):
        ...
        try:
            ...
            if res_id not in purchased_product_ids:  # No related purchase was found.
                return request.redirect(
                    self.orders_page
                )  # Prevent the user from downloading.
        ...
        if attachment["type"] == "url":
            ...
            extension = (
                extension
                if extension
                else mimetypes.guess_extension(attachment["mimetype"] or "")
            )
            filename = attachment["name"]
            filename = (
                filename if os.path.splitext(filename)[1] else filename + extension
            )
