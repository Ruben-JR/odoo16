...
class ProductProduct(models.Model):
    ...
    def _get_cart_qty(self, website=None):
        if not self.allow_out_of_stock_order:
            ...
            cart = (
                ...
                and website.sale_get_order()
                or None
            )
            if cart:
                return sum(
                    cart._get_common_product_lines(product=self).mapped(
                        "product_uom_qty"
                    )
                )
        return 0
    ...
    def _is_sold_out(self):
        ...
        ).product_tmpl_id._get_combination_info(product_id=self.id)
        return (
            combination_info["product_type"] == "product"
            and combination_info["free_qty"] <= 0
        )

    def _website_show_quick_add(self):
        return (
            self.allow_out_of_stock_order or not self._is_sold_out()
    ...
    def _send_availability_email(self):
        for product in self.search([("stock_notification_partner_ids", "!=", False)]):
            ...
            for partner in product.stock_notification_partner_ids:
                ...
                msg = (
                    self_ctxt.env["mail.message"]
                    .sudo()
                    .new(dict(body=body_html, record_name=product_ctxt.name))
                )
