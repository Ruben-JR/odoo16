...
class ProjectCustomerPortal(CustomerPortal):
    def _prepare_project_sharing_session_info(self, project, task=None):
        ...
        session_info["user_companies"]["allowed_companies"][company.id].update(
            ...
            timesheet_uom_factor=project_time_mode_uom._compute_quantity(
                1.0, timesheet_encode_uom, round=False
            ),
        )
    ...
    def _task_get_page_view_values(self, task, access_token, **kwargs):
        ...
        subtask_domain = expression.AND(
            [domain, [("task_id", "in", task.child_ids.ids)]]
        )
        timesheets = request.env["account.analytic.line"].sudo().search(task_domain)
        subtasks_timesheets = (
            request.env["account.analytic.line"].sudo().search(subtask_domain)
        )
        timesheets_by_subtask = defaultdict(
            lambda: request.env["account.analytic.line"].sudo()
        )
