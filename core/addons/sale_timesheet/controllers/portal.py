...
class SaleTimesheetCustomerPortal(TimesheetCustomerPortal):
    ...
    def _get_search_domain(self, search_in, search):
        ...
        if search_in in ("so", "all"):
            search_domain = expression.OR(
                [search_domain, [("so_line.order_id.name", "ilike", search)]]
            )
        if search_in in ("invoice", "all"):
            invoices = (
                request.env["account.move"].sudo().search([("name", "ilike", search)])
            )
            domain = request.env["account.analytic.line"]._timesheet_get_sale_domain(
    ...
    def _get_groupby_mapping(self):
        ...
        groupby_mapping.update(
            sol="so_line", so="order_id", invoice="timesheet_invoice_id"
        )
    ...
    def _task_get_page_view_values(self, task, access_token, **kwargs):
        ...
        if invoice_ids and request.env["account.move"].check_access_rights(
    ...
    def portal_my_timesheets(
        ...
        **kw,
        ...
        return super().portal_my_timesheets(
            page, sortby, filterby, search, search_in, groupby, **kw
        )
