...
class AccountAccount(models.Model):
    ...
    def _check_account_type_unique_current_year_earning(self):
        ...
        for res in result:
            if res.get("company_id_count", 0) >= 2:
                ...
                raise ValidationError(
                    _(
                        'You cannot have more than one account with "Current Year Earnings" as type. (accounts: %s)',
                        [a.code for a in account_unaffected_earnings],
                    )
                )

    name = fields.Char(
        string="Account Name", required=True, index="trigram", tracking=True
    )
    ...
    root_id = fields.Many2one(
        "account.root", compute="_compute_account_root", store=True
    )
    ...
    is_off_balance = fields.Boolean(
        compute="_compute_is_off_balance", default=False, store=True, readonly=True
    )
    ...
    def _constrains_allowed_journal_ids(self):
        ...
        if ids:
            raise ValidationError(
                _(
                    "Some journal items already exist with this account but in other journals than the allowed ones."
                )
            )
    ...
    def _check_company_consistency(self):
        ...
        if self._cr.fetchone():
            raise UserError(
                _(
                    "You can't change the company of your account since there are some journal items linked to it."
                )
            )
    ...
    def _check_account_type_sales_purchase_journal(self):
        ...
        if self._cr.fetchone():
            raise ValidationError(
                _(
                    "The account is already in use in a 'sale' or 'purchase' journal. This means that the account's type couldn't be 'receivable' or 'payable'."
                )
            )
    ...
    def _compute_account_type(self):
        ...
        for account in all_accounts:
            accounts_with_codes[account["company_id"][0]][account["code"]] = account[
                "account_type"
            ]
        for account in accounts_to_process:
            ...
            account.account_type = (
                accounts_with_codes[account.company_id.id][codes_list[closest_index]]
                if closest_index != -1
                else "asset_current"
            )
    ...
    def _set_opening_debit_credit(self, amount, field):
        ...
        if opening_move.state == "draft":
            ...
            if account_op_lines:
                ...
                if amount:
                    # modify the line
                    opening_move_line.with_context(check_move_validity=False)[
                        field
                    ] = amount
                else:
    ...
    def _get_most_frequent_account_for_partner(
        ...
        most_frequent_account = self._get_most_frequent_accounts_for_partner(
            company_id, partner_id, move_type, filter_never_user_accounts=True, limit=1
        )
    ...
    def load(self, fields, data):
        ...
        if "import_file" in self.env.context and "opening_balance" in fields:
            ...
            for company in companies:
                if company.account_opening_move_id.filtered(
    ...
    def _unlink_except_account_set_on_customer(self):
        ...
        if partner_prop_acc:
            ...
            raise UserError(
                _(
                    "You cannot remove/deactivate the account %s which is set on a customer or vendor.",
                    account_name,
                )
            )
    ...
    def _unlink_except_linked_to_fiscal_position(self):
        if self.env["account.fiscal.position.account"].search(
            [
                "|",
            ...
            raise UserError(
                _(
                    'You cannot remove/deactivate the accounts "%s" which are set on the account mapping of a fiscal position.',
    ...
    def _unlink_except_linked_to_tax_repartition_line(self):
        if self.env["account.tax.repartition.line"].search(
            [("account_id", "in", self.ids)], limit=1
        ):
            raise UserError(
                _(
                    'You cannot remove/deactivate the accounts "%s" which are set on a tax repartition line.',
                    ", ".join(f"{a.code} - {a.name}" for a in self),
                )
            )
