...
class ResConfigSettings(models.TransientModel):
    ...
    hr_employee_self_edit = fields.Boolean(
        string="Employee Editing", config_parameter="hr.hr_employee_self_edit"
    )

    @api.constrains(
        "module_hr_presence", "hr_presence_control_email", "hr_presence_control_ip"
    )
    def _check_advanced_presence(self):
        ...
        for settings in self:
            if settings.module_hr_presence and not (
                settings.hr_presence_control_email or settings.hr_presence_control_ip
            ):
                raise ValidationError(
                    _(
                        "You should select at least one Advanced Presence Control option."
                    )
                )
