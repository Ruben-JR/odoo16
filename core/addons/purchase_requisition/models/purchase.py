...
class PurchaseOrder(models.Model):
    ...
    requisition_id = fields.Many2one(
        "purchase.requisition", string="Purchase Agreement", copy=False
    )
    is_quantity_copy = fields.Selection(
        related="requisition_id.is_quantity_copy", readonly=False
    )
    ...
    def get_tender_best_lines(self):
        ...
        for line in order_lines:
            if (
                ...
                continue
            if (
                not product_to_best_price_line[line.product_id]
                or product_to_best_price_line[line.product_id][0].price_subtotal
                > line.price_subtotal
            ):
                product_to_best_price_line[line.product_id] = line
            elif (
                product_to_best_price_line[line.product_id][0].price_subtotal
                == line.price_subtotal
            ):
                product_to_best_price_line[line.product_id] |= line
            if (
                ...
                product_to_best_date_line[line.product_id][0].date_planned
                == line.date_planned
            ):
                product_to_best_date_line[line.product_id] |= line
            if (
                not product_to_best_price_unit[line.product_id]
                or product_to_best_price_unit[line.product_id][0].price_unit
                > line.price_unit
            ):
                product_to_best_price_unit[line.product_id] = line
            elif (
                product_to_best_price_unit[line.product_id][0].price_unit
                == line.price_unit
            ):
                product_to_best_price_unit[line.product_id] |= line
        best_price_ids = set()
...
class PurchaseOrderLine(models.Model):
        ...
        for pol in self:
            ...
            for line in pol.order_id.requisition_id.line_ids:
                if line.product_id == pol.product_id:
                    ...
                    partner = (
                        pol.order_id.partner_id or pol.order_id.requisition_id.vendor_id
                    )
    ...
    def action_clear_quantities(self):
        zeroed_lines = self.filtered(
            lambda l: l.state not in ["cancel", "purchase", "done"]
        )
