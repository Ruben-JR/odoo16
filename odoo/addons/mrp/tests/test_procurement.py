...
class TestProcurement(TestMrpCommon):
    ...
    def test_procurement_3(self):
        ...
        warehouse.reception_route_id.rule_ids.filtered(
            lambda p: p.location_src_id == warehouse.wh_input_stock_loc_id
            and p.location_dest_id == warehouse.wh_qc_stock_loc_id
        ).write({"procure_method": "make_to_stock"})
    ...
    def test_link_date_mo_moves(self):
        ...
        new_sheduled_date = fields.Datetime.to_datetime(
            mo.date_planned_start
        ) + timedelta(days=5)
        mo.date_planned_start = new_sheduled_date

        self.assertAlmostEqual(
            mo.move_raw_ids.date, mo.date_planned_start, delta=timedelta(seconds=1)
        )
        self.assertAlmostEqual(
            mo.move_finished_ids.date,
    ...
    def test_procurement_with_empty_bom(self):
        ...
        move_values = production._get_move_raw_values(
            comp1, 40.0, self.env.ref("uom.product_uom_unit")
        )
    ...
    def test_auto_assign(self):
        ...
        mo.is_locked = False
        self.assertEqual(
            mo.move_raw_ids.reserved_availability,
            0,
        ...
        self.assertEqual(
            pick_output.move_ids_without_package.reserved_availability,
            10,
        ...
        self.assertEqual(
            mo_assign_at_confirm.move_raw_ids.reserved_availability,
            5,
            "Components should have been auto-reserved",
        )
        ...
        self.env["mrp.bom"].create(
            {
                ...
                "type": "normal",
                "bom_line_ids": [
                    (0, 0, {"product_id": product_4.id, "product_qty": 1})
                ],
            }
        )
