...
class AccountInvoiceSend(models.TransientModel):
    ...
    invoice_without_email = fields.Text(
    ...
    invoice_ids = fields.Many2many(
        "account.move", "account_move_account_invoice_send_rel", string="Invoices"
    )
    composer_id = fields.Many2one(
        "mail.compose.message", string="Composer", required=True, ondelete="cascade"
    )
    template_id = fields.Many2one(
        "mail.template", "Use template", domain="[('model', '=', 'account.move')]"
    )
    ...
    move_types = fields.Char(
        string="Move types", compute="_compute_move_types", readonly=True
    )
    ...
    def _compute_composition_mode(self):
        for wizard in self:
            wizard.composer_id.composition_mode = (
                "comment" if len(wizard.invoice_ids) == 1 else "mass_mail"
            )
            ...
            if wizard.is_email and len(wizard.invoice_ids) > 1:
                ...
                if invoices:
                    wizard.invoice_without_email = "%s\n%s" % (
                        _(
                            "The following invoice(s) will not be sent by email, because the customers don't have email address."
                        ),
