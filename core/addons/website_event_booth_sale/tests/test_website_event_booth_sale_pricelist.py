...
class TestWebsiteBoothPriceList(TestEventBoothSaleCommon, TestWebsiteEventSaleCommon):
    ...
    def test_pricelist_different_currency(self):
        ...
        with MockRequest(
            self.env, sale_order_id=self.so.id, website=self.current_website
        ):
            self.WebsiteSaleController.pricelist(promo=None)
            self.so._cart_update(
                line_id=so_line.id, product_id=self.event_booth_product.id, set_qty=1
            )
        ...
        self.assertEqual(
            so_line.price_reduce,
            360,
            'Incorrect amount based on the pricelist "With Discount" and its currency.',
        )
