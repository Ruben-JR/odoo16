...
class SmsSms(models.Model):
    ...
    def action_set_error(self, failure_type):
        ...
        if notifications:
            notifications.write(
                {"notification_status": "exception", "failure_type": failure_type}
            )
            if not self._context.get("sms_skip_msg_notification", False):
                notifications.mail_message_id._notify_message_notification_update()

    def action_set_outgoing(self):
        self.write({"state": "outgoing", "failure_type": False})
        notifications = (
            self.env["mail.notification"]
            .sudo()
            .search(
    ...
    def _split_batch(self):
        batch_size = int(
            ...
            .sudo()
            .get_param("sms.session.batch.size", 500)
        )
    ...
    def _send(self, unlink_failed=False, unlink_sent=True, raise_exception=False):
        ...
        try:
            ...
            self._postprocess_iap_sent_sms(
                iap_results, unlink_failed=unlink_failed, unlink_sent=unlink_sent
            )
    ...
    def _postprocess_iap_sent_sms(
        ...
        if unlink_failed:
            todelete_sms_ids += [
                item["res_id"] for item in iap_results if item["state"] != "success"
            ]
        if unlink_sent:
            todelete_sms_ids += [
                item["res_id"] for item in iap_results if item["state"] == "success"
