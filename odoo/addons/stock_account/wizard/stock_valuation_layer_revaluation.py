...
class StockValuationLayerRevaluation(models.TransientModel):
    ...
    currency_id = fields.Many2one(
        "res.currency", "Currency", related="company_id.currency_id", required=True
    )

    product_id = fields.Many2one(
        "product.product", "Related product", required=True, check_company=True
    )
    ...
    current_quantity_svl = fields.Float(
        "Current Quantity", related="product_id.quantity_svl"
    )

    added_value = fields.Monetary("Added value", required=True)
    new_value = fields.Monetary("New value", compute="_compute_new_value")
    new_value_by_qty = fields.Monetary(
        "New value by quantity", compute="_compute_new_value"
    )
    reason = fields.Char("Reason", help="Reason of the revaluation")

    account_journal_id = fields.Many2one(
        "account.journal", "Journal", check_company=True
    )
