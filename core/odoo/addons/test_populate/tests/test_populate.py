...
class TestPopulate(common.TransactionCase):
    ...
    def test_dependency(self):
        ...
        self.assertEqual(
            ordered_models_names, ["test.populate.category", "test.populate"]
        )
    ...
    def test_populate(self):
        ...
        self.assertEqual(
            records.mapped("some_ref")[5:20],
            [1, 0, 2, 4, 4, 3, 4, 1, 2, 2, 2, 4, 4, 1, 2],
        )
        self.assertEqual(
            records.mapped("sequence")[:20],
            [6, 10, 1, 1, 1, 3, 8, 9, 1, 5, 9, 5, 7, 3, 5, 3, 6, 4, 9, 2],
        )  # Test randint
    ...
    def test_populate_inherit(self):
        ...
        records = self.check_test_populate_values(
            populated, model
        )  # should be same values as base class
    ...
    def check_test_populate_values(self, populated, model):
        ...
        self.assertIn(
            "filling", records.mapped("name")[6]
        )  # filling when cartesian and iterate are done
        # iterate then pseudo random
        self.assertEqual(records.mapped("some_ref")[:5], [0, 1, 2, 3, 4])  # iterate

        # some custom multi field generator (as cartesian product in this example)
        self.assertEqual(
            records.mapped("dependant_field_1")[:6],
            ["d1_1", "d1_1", "d1_1", "d1_2", "d1_2", "d1_2"],
        )
        self.assertEqual(
            records.mapped("dependant_field_2")[:6],
            ["d2_1", "d2_2", "d2_3_0", "d2_1", "d2_2", "d2_3_1"],
        )
        used_category_ids = set(records.mapped("category_id").ids[:20])
        self.assertEqual(
            len(used_category_ids), 6
        )  # event if id may change, with given seed, the 6 category are used
        generated_category_ids = set(populated["test.populate.category"])
        self.assertFalse(
            used_category_ids - generated_category_ids
        )  # all category are the generated one
        self.assertFalse(
            hasattr(self.env.registry, "populated_models"),
            "populated_models flag has been removed from registry",
        )
...
class TestPopulateValidation(common.TransactionCase):
    ...
    def test_populate_factories(self):
        for model in self.env.values():
            ...
            self.assertFalse(
                missing, "Fields %s not found in model %s" % (missing, model._name)
            )
    ...
    def test_populate_missing_factories(self):
        ...
        for model in self.env.values():
            ...
            if not factories:
                ...
                if all(
                    module.startswith("test_") for module in ir_model.modules.split(",")
                ):
                    continue
                no_factory_models.append(model._name)
            else:
                factories_fields = next(
                    populate.chain_factories(factories, model._name)
                ).keys()
                ...
                electable_fields = set(
                    [key for key, field in model._fields.items() if is_electable(field)]
                )
                no_factory_fields = set(electable_fields - factories_fields)
                if no_factory_fields:
                    _logger.info(
                        "Model %s has some undefined field: %s",
                        model._name,
                        no_factory_fields,
                    )
