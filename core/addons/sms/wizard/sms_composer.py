...
class SendSMS(models.TransientModel):
    ...
    recipient_single_description = fields.Text(
        "Recipients (Partners)", compute="_compute_recipient_single", compute_sudo=False
    )
    ...
    def _compute_recipient_single(self):
        for composer in self:
            ...
            if (
                not records
                or not issubclass(type(records), self.pool["mail.thread"])
                or not composer.comment_single_recipient
            ):
            ...
            records.ensure_one()
            res = records._sms_get_recipients_info(
                force_field=composer.number_field_name, partner_fallback=False
            )
            composer.recipient_single_description = (
                res[records.id]["partner"].name
                or records._sms_get_default_partners().display_name
            )
    ...
    def _compute_sanitized_numbers(self):
        for composer in self:
            if composer.numbers:
                ...
                sanitized_numbers = [
                    info["sanitized"]
                    for info in sanitize_res.values()
                    if info["sanitized"]
                ]
                invalid_numbers = [
        ...
        for record in self:
            if (
                ...
                record.body = record.template_id._render_field(
                    "body", [record.res_id], compute_lang=True
                )[record.res_id]
    ...
    def _action_send_sms(self):
        ...
        if self.composition_mode == "numbers":
            ...
            if records is None or not issubclass(
                type(records), self.pool["mail.thread"]
            ):
                return self._action_send_sms_numbers()
    ...
    def _action_send_sms_mass(self, records=None):
        ...
        if (
            sms_all
            and self.mass_keep_log
            and records
            and issubclass(type(records), self.pool["mail.thread"])
        ):
    ...
    def _prepare_body_values(self, records):
        if self.template_id and self.body == self.template_id.body:
            all_bodies = self.template_id._render_field(
                "body", records.ids, compute_lang=True
            )
        else:
            all_bodies = self.env["mail.render.mixin"]._render_template(
                self.body, records._name, records.ids
            )
        return all_bodies
        ...
        if composition_mode == "comment":
            if not body and template_id and res_id:
                ...
                result["body"] = template._render_template(
                    template.body, res_model, [res_id]
                )[res_id]
            elif template_id:
                template = self.env["sms.template"].browse(template_id)
