...
class WebsiteSlidesSurvey(WebsiteProfile):
    def _prepare_user_profile_values(self, user, **kwargs):
        ...
        values = super(WebsiteSlidesSurvey, self)._prepare_user_profile_values(
            user, **kwargs
        )
        ...
        domain = expression.AND(
            [
                ...
                else [("partner_id", "=", values["user"].partner_id.id)],
            ]
        )
        ...
        if "certification_search" in kwargs:
            ...
            domain = expression.AND(
                [domain, [("survey_id.title", "ilike", kwargs["certification_search"])]]
            )
