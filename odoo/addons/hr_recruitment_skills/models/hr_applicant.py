...
class HrApplicant(models.Model):
    ...
    applicant_skill_ids = fields.One2many(
        "hr.applicant.skill", "applicant_id", string="Skills"
    )
    ...
    def _compute_is_interviewer(self):
        ...
        for applicant in self:
            applicant.is_interviewer = not is_recruiter and self.env.user in (
    ...
    def create_employee_from_applicant(self):
        ...
        action["context"]["default_employee_skill_ids"] = [
            (
                0,
                0,
                {
                ...
                [
                    {
                        ...
                        "skill_id": skill.id,
                        "skill_level_id": applicant.applicant_skill_ids.filtered(
                            lambda s: s.skill_id == skill
                        ).skill_level_id.id,
