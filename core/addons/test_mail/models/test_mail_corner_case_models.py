...
class MailTestLang(models.Model):
    ...
    def _notify_get_recipients_groups(self, msg_vals=None):
        groups = super(MailTestLang, self)._notify_get_recipients_groups(
            msg_vals=msg_vals
        )
            ...
            group_options["actions"] = [
                {
                    "url": self._notify_get_action_link(
                        "controller", controller="/test_mail/do_stuff", **local_msg_vals
                    ),
...
class MailTestTrackAll(models.Model):
    ...
    selection_field = fields.Selection(
        string="Selection", selection=[["first", "FIRST"]], tracking=True
    )
