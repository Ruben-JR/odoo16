...
class StockRule(models.Model):
    ...
    def _run_manufacture(self, procurements):
        ...
        for company_id, productions_values in productions_values_by_company.items():
            ...
            productions.filtered(
                lambda p: (not p.orderpoint_id and p.move_raw_ids)
                or (
                    p.move_dest_ids.procure_method != "make_to_order"
                    and not p.move_raw_ids
                    and not p.workorder_ids
                )
            ).action_confirm()
        ...
        for procurement, rule in procurements:
            ...
            if rule.picking_type_id == warehouse_id.sam_type_id:
                if (
                    ...
                    procurement.values["group_id"] = (
                        procurement.values["group_id"]
                        .stock_move_ids.filtered(
                            lambda m: m.state not in ["done", "cancel"]
                        )
                        .move_orig_ids.group_id[:1]
                    )
                    continue
                ...
                if manu_type_id:
                    ...
                    name = self.env["ir.sequence"].next_by_code("mrp.production") or _(
                        "New"
                    )
                ...
                if group:
                    ...
                    procurement.values["group_id"] = self.env[
                        "procurement.group"
                    ].create({"name": name})
    ...
    def _prepare_mo_vals(
        ...
        mo_values = {
            ...
            "date_planned_finished": fields.Datetime.from_string(
                values["date_planned"]
            ),
        ...
        if (
            location_dest_id.warehouse_id.manufacture_steps == "pbm_sam"
            and values.get("move_dest_ids")
            and values.get("group_id")
            and values["move_dest_ids"][0].origin != values["group_id"].name
        ):
    ...
    def _get_lead_days(self, product, **values):
        ...
        if not bypass_delay_description:
            delay_description.append(
                (_("Manufacture Security Lead Time"), _("+ %d day(s)", security_delay))
            )
        days_to_order = values.get(
            "days_to_order", product.product_tmpl_id.days_to_prepare_mo
        )
        if not bypass_delay_description:
            delay_description.append(
...
class ProcurementGroup(models.Model):
    ...
    def run(self, procurements, raise_user_error=True):
        ...
        return super(ProcurementGroup, self).run(
            procurements_without_kit, raise_user_error=raise_user_error
        )
