    ...
    def name_get(self):
        ...
        for leave_type in self:
            if (
                ...
                and leave_type.requires_allocation == "no"
            ):
                if not employee_id:
                    employee_id = (
                        self.env["hr.employee"]
                        .browse(self._context.get("employee_id"))
                        .sudo()
                    )
        ...
        for employee_id, allocations in res.items():
            for allocation_id in allocations:
                if allocation_id in deductible_time_off_type_ids:
                    res[employee_id][allocation_id]["virtual_remaining_leaves"] = (
                        self.env["hr.employee"]
                        .sudo()
                        .browse(employee_id)
                        .total_overtime
    ...
    def _compute_hr_attendance_overtime(self):
        ...
        for leave_type in self:
            if leave_type.company_id:
                leave_type.hr_attendance_overtime = (
                    leave_type.company_id.hr_attendance_overtime
                )
            else:
                leave_type.hr_attendance_overtime = (
                    self.env.company.hr_attendance_overtime
                )
