...
class TestOutOfOffice(TestHrHolidaysCommon):
    ...
    def test_leave_ooo(self):
        ...
        self.assertEqual(
            len(members_data), 2, "Channel info should get info for the 2 members"
        )
        partner_info = next(
            member
            for member in members_data
            if member["persona"]["partner"]["email"] == partner.email
        )
        partner2_info = next(
            member
            for member in members_data
            if member["persona"]["partner"]["email"] == partner2.email
        )
        self.assertFalse(
            partner2_info["persona"]["partner"]["out_of_office_date_end"],
            "current user should not be out of office",
        )
        self.assertEqual(
            partner_info["persona"]["partner"]["out_of_office_date_end"],
...
class TestOutOfOfficePerformance(TestHrHolidaysCommon, TransactionCaseWithUserDemo):
    ...
    def test_search_absent_employee(self):
        present_employees = self.env["hr.employee"].search(
            [("is_absent", "!=", "True")]
        )
