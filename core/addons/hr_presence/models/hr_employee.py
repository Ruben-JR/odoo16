...
class Employee(models.AbstractModel):
    ...
    def _compute_presence_state(self):
        ...
        employees = self.filtered(
            lambda e: e.hr_presence_state != "present" and not e.is_absent
        )
        company = self.env.company
        employee_to_check_working = employees.filtered(
            lambda e: not e.is_absent
            and (e.email_sent or e.ip_connected or e.manually_set_present)
        )
        working_now_list = employee_to_check_working._get_employee_working_now()
        for employee in employees:
            if (
                not employee.is_absent
                and company.hr_presence_last_compute_date
                and employee.id in working_now_list
                and company.hr_presence_last_compute_date.day == Datetime.now().day
                and (
                    employee.email_sent
    ...
    def _check_presence(self):
        company = self.env.company
        if (
            not company.hr_presence_last_compute_date
            or company.hr_presence_last_compute_date.day != Datetime.now().day
        ):
            self.env["hr.employee"].search([("company_id", "=", company.id)]).write(
                {
                    "email_sent": False,
                    "ip_connected": False,
        ...
        if literal_eval(
            ...
            .sudo()
            .get_param("hr_presence.hr_presence_control_ip", "False")
        ):
        ...
        if literal_eval(
            self.env["ir.config_parameter"]
            .sudo()
            .get_param("hr_presence.hr_presence_control_email", "False")
        ):
    ...
    def _action_open_presence_view(self):
        ...
        return {
            ...
            "views": [
                [self.env.ref("hr_presence.hr_employee_view_kanban").id, "kanban"],
                [False, "tree"],
                [False, "form"],
            ],
            ...
            "search_view_id": [
                self.env.ref("hr_presence.hr_employee_view_presence_search").id,
                "search",
            ],
            "context": {
