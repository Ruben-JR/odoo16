...
class TestMailSchedule(EventCase, MockEmail):
    ...
    def test_event_mail_schedule(self):
        ...
        self.assertEqual(
            ...
            test_event.create_date + relativedelta(hours=1),
        )
        ...
        self.assertEqual(
            ...
            event_date_begin + relativedelta(days=-1),
        )
        ...
        self.assertEqual(
            event_next_scheduler.scheduled_date, event_date_end + relativedelta(hours=1)
        )
        ...
        self.assertTrue(
            all(reg.state == "open" for reg in reg1 + reg2),
            "Registrations: should be auto-confirmed",
        )
        self.assertTrue(
            all(reg.create_date == now for reg in reg1 + reg2),
        ...
        self.assertEqual(
            ...
            2,
            "event: should have 2 scheduled communication (1 / registration)",
        )
        ...
        self.assertMailMailWEmails(
            ...
            content=None,
            fields_values={
                "subject": "Your registration at %s" % test_event.name,
        ...
        for mail_registration in after_sub_scheduler_2.mail_registration_ids:
            self.assertEqual(
                mail_registration.scheduled_date, now + relativedelta(hours=1)
            )
            self.assertFalse(
                mail_registration.mail_sent,
        ...
        self.assertFalse(
            after_sub_scheduler_2.mail_done,
        ...
        self.assertEqual(
            len(self._new_mails),
            2,
            "event: should have scheduled 2 mails (1 / registration)",
        )
        self.assertMailMailWEmails(
            [formataddr((reg1.name, reg1.email)), formataddr((reg2.name, reg2.email))],
            "outgoing",
            content=None,
            fields_values={
                "subject": "%s: tomorrow" % test_event.name,
                "email_from": self.user_eventmanager.company_id.email_formatted,
            },
        )
        ...
        self.assertTrue(
            after_sub_scheduler.mail_done,
            "event: scheduler on registration not updated next to draft registration",
        )
        self.assertTrue(
            after_sub_scheduler_2.mail_done,
            "event: scheduler on registration not updated next to draft registration",
        ...
        self.assertEqual(
            len(after_sub_scheduler.mail_registration_ids),
            3,
            "event: should have 3 scheduled communication (1 / registration)",
        )
        new_mail_reg = after_sub_scheduler.mail_registration_ids.filtered(
            lambda mail_reg: mail_reg.registration_id == reg3
        )
        ...
        self.assertEqual(
            new_mail_reg.scheduled_date, now_start + relativedelta(hours=1)
        )
        self.assertTrue(
            new_mail_reg.mail_sent,
            "event: registration mail should be sent at registration creation",
        ...
        self.assertMailMailWEmails(
            [
                formataddr((reg1.name, reg1.email)),
                formataddr((reg2.name, reg2.email)),
                formataddr((reg3.name, reg3.email)),
            ],
            "outgoing",
            content=None,
            fields_values={
                "subject": "%s: today" % test_event.name,
                "email_from": self.user_eventmanager.company_id.email_formatted,
            },
        )
    ...
    def test_unique_event_mail_ids(self):
        ...
        mail_done = test_event.event_mail_ids.filtered(
            lambda mail: mail.mail_done and mail.mail_registration_ids
        )
        ...
        self.assertTrue(
            mail_done in test_event.event_mail_ids,
