...
class RecurrenceRule(models.Model):
        ...
        for odoo_attendee_email in set(existing_attendees.mapped("email")):
            ...
            if email_normalize(odoo_attendee_email) not in emails:
                attendees = existing_attendees.exists().filtered(
                    lambda att: att.email == email_normalize(odoo_attendee_email)
                )
                self.calendar_event_ids.write(
                    {
                        "need_sync": False,
    ...
    def _create_from_google(self, gevents, vals_list):
        ...
        recurrence = super(
            RecurrenceRule, self.with_context(dont_notify=True)
    ...
    def _get_sync_domain(self):
        ...
        return [
            ("calendar_event_ids.user_id", "=", self.env.user.id),
            ("rrule", "!=", False),
        ]
    ...
    def _google_values(self):
        ...
        values["recurrence"] = (
            ["RRULE:%s" % rrule] if "RRULE:" not in rrule else [rrule]
        )
