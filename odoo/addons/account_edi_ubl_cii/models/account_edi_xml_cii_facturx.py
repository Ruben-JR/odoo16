...
class AccountEdiXmlCII(models.AbstractModel):
        ...
        constraints.update(
            {
                ...
                and vals["record"]["commercial_partner_id"]["zip"][:2] in ["35", "38"]
                else None,
            }
        )
        return constraints
    ...
    def _export_invoice_vals(self, invoice):
        ...
        buyer_siret = False
        if (
            "siret" in invoice.commercial_partner_id._fields
            and invoice.commercial_partner_id.siret
        ):
        ...
        for tax_detail_vals in template_values["tax_details"]["tax_details"].values():
            ...
            if (
                tax_detail_vals.get("tax_category_code") == "K"
                and not template_values["scheduled_delivery_time"]
            ):
    ...
    def _import_fill_invoice_form(self, journal, tree, invoice, qty_factor):
        ...
        partner_type = (
            invoice.journal_id.type == "purchase"
            and "SellerTradeParty"
            or "BuyerTradeParty"
        )
        invoice.partner_id = self.env["account.edi.format"]._retrieve_partner(
            ...
            vat=_find_value(
                f"//ram:{partner_type}/ram:SpecifiedTaxRegistration/ram:ID"
            ),
        )
        ...
        payment_reference_node = tree.find(
        ...
        invoice_date_due_node = tree.find(
    ...
    def _import_fill_invoice_line_form(
        self, journal, tree, invoice_form, invoice_line, qty_factor
    ):
        logs = []
        ...
        invoice_line.product_id = self.env["account.edi.format"]._retrieve_product(
            default_code=_find_value(
                ".//ram:SpecifiedTradeProduct/ram:SellerAssignedID", tree
            ),
            name=_find_value(".//ram:SpecifiedTradeProduct/ram:Name", tree),
