...
class SlidesPortalChatter(PortalChatter):
    def _portal_post_has_content(
        self, res_model, res_id, message, attachment_ids=None, **kw
    ):
        ...
        if res_model == "slide.channel" and kw.get("rating_value"):
            return True
        return super()._portal_post_has_content(
            res_model, res_id, message, attachment_ids=attachment_ids, **kw
        )
    ...
    def portal_chatter_post(self, res_model, res_id, message, **kw):
        result = super(SlidesPortalChatter, self).portal_chatter_post(
        ...
        if result and res_model == "slide.channel":
            ...
            if (
                rating_value
                and slide_channel
                and request.env.user.partner_id.id == int(kw.get("pid"))
            ):
            ...
            result.update(
                {
                    ...
                    "force_submit_url": result.get("default_message_id")
    ...
    def mail_update_message(
        ...
        **post
        ...
        if not _check_special_access(
            res_model, res_id, token=post.get("token"), _hash=post.get("hash"), pid=pid
        ):
            raise Forbidden()
