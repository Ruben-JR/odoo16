...
class ReplenishmentReport(models.AbstractModel):
    ...
    def _compute_draft_quantity_count(
        ...
        incoming_moves = self.env["stock.move"]._read_group(
            in_domain, ["product_qty:sum"], "product_id"
        )
        outgoing_moves = self.env["stock.move"]._read_group(
    ...
    def _serialize_docs(
        ...
        res = copy.copy(docs)
        if product_template_ids:
            res["product_templates"] = docs["product_templates"].read(
            ...
            res["product_variants"] = docs["product_variants"].read(
                fields=["id", "display_name"]
            )
        ...
        for index, line in enumerate(docs["lines"]):
            res["lines"].append(
                {
                    ...
                    "move_out": line["move_out"].read(
                        self._fields_for_serialized_moves()
                    )[0]
                    if line["move_out"]
                    else False,
                    "move_in": line["move_in"].read(
                        self._fields_for_serialized_moves()
                    )[0]
                    if line["move_in"]
                    else False,
            ...
            if line["move_out"] and line["move_out"]["picking_id"]:
                res["lines"][-1]["move_out"].update(
                    {
                        "picking_id": line["move_out"]["picking_id"].read(
                            fields=["id", "priority"]
                        )[0],
                    }
                )
    ...
    def _get_report_data(self, product_template_ids=False, product_variant_ids=False):
        ...
        res.update(
            self._compute_draft_quantity_count(
                product_template_ids, product_variant_ids, wh_location_ids
            )
        )
    ...
    def _get_report_lines(
        ...
        def _reconcile_out_with_ins(
            lines, out, ins, demand, product_rounding, only_matching_move_dest=True
        ):
            index_to_remove = []
        ...
        for out in outs:
            ...
            for move in moves_orig:
                ...
                if (
                    float_compare(
                        out_qty_reserved, out.product_qty, precision_rounding=rounding
                    )
                    >= 0
                ):
                    break
        ...
        reserved_outs = self.env["stock.move"].search(
            [("id", "in", reserved_outs.ids)], order="priority desc, date, id"
        )
        ...
        for product in (ins | outs).product_id:
            ...
            for out in outs_per_product[product.id]:
                ...
                if not float_is_zero(
                    reserved_availability, precision_rounding=product_rounding
                ):
