...
class MailResendMessage(models.TransientModel):
    ...
    partner_ids = fields.One2many(
        "mail.resend.partner", "resend_wizard_id", string="Recipients"
    )
    notification_ids = fields.Many2many(
        "mail.notification", string="Notifications", readonly=True
    )
    ...
    def resend_mail_action(self):
        ...
        for wizard in self:
            ...
            to_cancel = wizard.partner_ids.filtered(lambda p: not p.resend).mapped(
                "partner_id"
            )
            ...
            notif_to_cancel = wizard.notification_ids.filtered(
                lambda notif: notif.notification_type == "email"
                and notif.res_partner_id in to_cancel
                and notif.notification_status in ("exception", "bounce")
            )
            ...
            if to_send:
                ...
                recipients_data = self.env["mail.followers"]._get_recipient_data(
                    None, "comment", False, pids=to_send.ids
                )[0]
        ...
        for wizard in self:
            for notif in wizard.notification_ids:
                notif.filtered(
                    lambda notif: notif.notification_type == "email"
                    and notif.notification_status in ("exception", "bounce")
                ).sudo().write({"notification_status": "canceled"})
