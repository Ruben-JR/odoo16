...
def model_create_single(method):
    ...
    _create_logger.warning(
        "The model %s is not overriding the create method in batch", method.__module__
    )
...
class Environment(Mapping):
    ...
    def flush_all(self):
        ...
        self._recompute_all()
        for model_name in OrderedSet(
            field.model_name for field in self.cache.get_dirty_fields()
        ):
...
class Cache(object):
    ...
    def __repr__(self):
        ...
        for field, field_cache in sorted(
            self._data.items(), key=lambda item: str(item[0])
        ):
    ...
    def set(self, record, field, value, dirty=False, check_dirty=True):
        ...
        if dirty:
            ...
            _logger.error(
                "cache.set() removing flag dirty on %s.%s",
                record,
                field.name,
                stack_info=True,
            )
    ...
    def update_raw(self, records, field, values, dirty=False, check_dirty=True):
        ...
        if dirty:
            ...
            if dirty_ids and not dirty_ids.isdisjoint(records._ids):
                _logger.error(
                    "cache.update() removing flag dirty on %s.%s",
                    records,
                    field.name,
                    stack_info=True,
                )
    ...
    def get_missing_ids(self, records, field):
        ...
        if field.translate:
            ...
            for record_id in records._ids:
                ...
                if cache_value is False or not (
                    cache_value is None or lang in cache_value
                ):
                    yield record_id
        else:
            for record_id in records._ids:
        ...
        def process(model, field, field_cache):
            ...
            for id_, value in env.cr.fetchall():
                ...
                if value == cached or (not value and not cached):
                    continue
                invalids.append(
                    (model.browse(id_), field, {"cached": cached, "fetched": value})
                )
