...
class PostgreSQLHandler(logging.Handler):
    ...
    def emit(self, record):
        ...
        if not dbname:
            return
        with contextlib.suppress(Exception), tools.mute_logger(
            "odoo.sql_db"
        ), sql_db.db_connect(dbname, allow_uri=True).cursor() as cr:
...
def init_logger():
    ...
    warnings.filterwarnings(
        "ignore", r"^invalid escape sequence \'?\\.", category=DeprecationWarning
    )
    ...
    warnings.filterwarnings(
        "ignore", category=BytesWarning, module="reportlab.platypus.paraparser"
    )
