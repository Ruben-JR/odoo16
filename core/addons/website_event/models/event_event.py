...
class Event(models.Model):
    ...
    is_participating = fields.Boolean(
        "Is Participating", compute="_compute_is_participating"
    )
    # website
    ...
    menu_register_cta = fields.Boolean(
        ...
        compute="_compute_menu_register_cta",
        readonly=False,
        store=True,
    )
    ...
    def _compute_menu_register_cta(self):
        ...
        for event in self:
            if (
                ...
                or not event.menu_register_cta
            ):
        ...
        for fname in self._get_menu_update_fields():
            if fname in force_update:
                ...
                menus_update_by_field[fname] |= menus_state_by_field[fname][
                    "activated"
                ].filtered(lambda event: not event[fname])
                menus_update_by_field[fname] |= menus_state_by_field[fname][
                    "deactivated"
                ].filtered(lambda event: event[fname])
    ...
    def _update_website_menus(self, menus_update_by_field=None):
        ...
        for event in self:
            ...
            if event.menu_id and (
                not menus_update_by_field
                or event in menus_update_by_field.get("location_menu")
            ):
                event._update_website_menu_entry(
                    "location_menu", "location_menu_ids", "location"
                )
            if event.menu_id and (
                not menus_update_by_field
        ...
        if self[fname_bool] and not self[fname_o2m]:
            ...
            for name, url, xml_id, menu_sequence, menu_type in menu_data:
                new_menu = self._create_menu(
                    menu_sequence, name, url, xml_id, menu_type
                )
    ...
    def _create_menu(self, sequence, name, url, xml_id, menu_type):
        ...
        if not url:
            ...
            url = (
                "/event/" + slug(self) + "/page" + page_result["url"]
            )  # url contains starting "/"
    ...
    def _get_event_resource_urls(self):
        url_date_start = self.date_begin.astimezone(timezone(self.date_tz)).strftime(
            "%Y%m%dT%H%M%S"
        )
        url_date_stop = self.date_end.astimezone(timezone(self.date_tz)).strftime(
            "%Y%m%dT%H%M%S"
        )
        params = {
            "action": "TEMPLATE",
    ...
    def _search_build_dates(self):
        ...
        def get_month_filter_domain(filter_name, months_delta):
            ...
            filter_string = (
                ...
                if months_delta == 0
                    ...
                    self.env,
                    value=today + relativedelta(months=months_delta),
                    date_format="LLLL",
            ...
            return [
                ...
                [
                    (
                        "date_end",
                        ">=",
                        sd(first_day_of_the_month + relativedelta(months=months_delta)),
                    ),
                    (
                        "date_begin",
                        "<",
        ...
        return [
            ...
            [
                "today",
                _("Today"),
                [("date_end", ">", sd(today)), ("date_begin", "<", sdn(today))],
                0,
            ],
    ...
    def _search_get_detail(self, website, order, options):
        ...
        if country:
            if country == "online":
                ...
                domain.append(
                    ["|", ("country_id", "=", int(country)), ("country_id", "=", False)]
                )
