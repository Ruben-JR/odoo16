...
class configmanager(object):
    def __init__(self, fname=None):
        ...
        self.parser = parser = optparse.OptionParser(
            version=version, option_class=MyOption
        )
        ...
        group.add_option(
            "--pidfile", dest="pidfile", help="file where the server pid will be stored"
        )
        group.add_option(
            "--addons-path",
            dest="addons_path",
        ...
        group.add_option(
            ...
            action="callback",
            callback=self._check_upgrade_path,
            nargs=1,
            type="string",
        )
        group.add_option(
        ...
        group.add_option(
            ...
            "--data-dir",
            dest="data_dir",
            my_default=_get_default_datadir(),
        ...
        group.add_option(
            "--no-xmlrpc", dest="http_enable", action="store_false", help=hidden
        )
        ...
        group.add_option(
            "--test-enable",
            action="callback",
            callback=self._test_enable_callback,
            dest="test_enable",
        ...
        group.add_option(
            ...
            help='setup a handler at LEVEL for a given PREFIX. An empty PREFIX indicates the root logger. This option can be repeated. Example: "odoo.orm:DEBUG" or "werkzeug:CRITICAL" (default: ":INFO")',
        )
        group.add_option(
            "--log-web",
            action="append_const",
        ...
        group.add_option(
            ...
            action="append_const",
            dest="log_handler",
            const="odoo.sql_db:DEBUG",
            help="shortcut for --log-handler=odoo.sql_db:DEBUG",
        )
        group.add_option(
            "--log-db", dest="log_db", help="Logging database", my_default=False
        )
        group.add_option(
            "--log-db-level",
            dest="log_db_level",
        ...
        group.add_option(
            "--pg_path", dest="pg_path", help="specify the pg executable path"
        )
        group.add_option(
            "--db_host",
            dest="db_host",
    ...
    def _parse_config(self, args=None):
        ...
        if os.name == "nt":
            rcfilepath = os.path.join(
                os.path.abspath(os.path.dirname(sys.argv[0])), "odoo.conf"
            )
        else:
            ...
            die(
                os.path.isfile(rcfilepath) and os.path.isfile(old_rcfilepath),
        ...
        for arg in keys:
            ...
            # ... or keep, but cast, the config file value.
                ...
                isinstance(self.options[arg], str)
                and self.casts[arg].type in optparse.Option.TYPE_CHECKER
            ):
                self.options[arg] = optparse.Option.TYPE_CHECKER[self.casts[arg].type](
                    self.casts[arg], arg, self.options[arg]
                )
        ...
        self.options["root_path"] = self._normalize(
        ...
        if not self.options["addons_path"] or self.options["addons_path"] == "None":
            ...
            self.options["addons_path"] = ",".join(
                self._normalize(x) for x in self.options["addons_path"].split(",")
            )

        self.options["upgrade_path"] = (
            ",".join(
                self._normalize(x) for x in self.options["upgrade_path"].split(",")
            )
        ...
        self.options["demo"] = (
            dict(self.options["init"]) if not self.options["without_demo"] else {}
        )
        self.options["update"] = (
            opt.update and dict.fromkeys(opt.update.split(","), 1) or {}
        )
        self.options["translate_modules"] = (
            opt.translate_modules
            and [m.strip() for m in opt.translate_modules.split(",")]
            or ["all"]
        )
        self.options["translate_modules"].sort()

        dev_split = [s.strip() for s in opt.dev_mode.split(",")] if opt.dev_mode else []
        self.options["dev_mode"] = dev_split + (
            ["reload", "qweb", "xml"] if "all" in dev_split else []
        )
        ...
        conf.server_wide_modules = [
            m.strip()
            for m in self.options["server_wide_modules"].split(",")
            if m.strip()
        ]
        return opt
    ...
    def _warn_deprecated_options(self):
        if self.options["osv_memory_age_limit"]:
            ...
            self.options["transient_age_limit"] = self.options.pop(
                "osv_memory_age_limit"
            )
        ...
        for path in value.split(","):
            ...
            if not os.path.isdir(res):
                raise optparse.OptionValueError(
                    "option %s: no such directory: %r" % (opt, res)
                )
            if not self._is_addons_path(res):
                raise optparse.OptionValueError(
                    "option %s: the path %r is not a valid addons directory"
                    % (opt, path)
                )
            ad_paths.append(res)
    ...
    def _check_upgrade_path(self, option, opt, value, parser):
        ...
        for path in value.split(","):
            ...
            if not os.path.isdir(res):
                raise optparse.OptionValueError(
                    "option %s: no such directory: %r" % (opt, path)
                )
            if not self._is_upgrades_path(res):
                raise optparse.OptionValueError(
                    "option %s: the path %r is not a valid upgrade directory"
                    % (opt, path)
                )
        ...
        for opt in sorted(self.options):
            ...
            if opt in ("log_level",):
                p.set(
                    "options",
                    opt,
                    loglevelnames.get(self.options[opt], self.options[opt]),
                )
    ...
    def __setitem__(self, key, value):
        ...
        if (
            key in self.options
            and isinstance(self.options[key], str)
            and key in self.casts
            and self.casts[key].type in optparse.Option.TYPE_CHECKER
        ):
            self.options[key] = optparse.Option.TYPE_CHECKER[self.casts[key].type](
                self.casts[key], key, self.options[key]
            )
