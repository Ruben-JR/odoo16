...
class MailThread(models.AbstractModel):
    ...
    message_main_attachment_id = fields.Many2one(
        string="Main Attachment", comodel_name="ir.attachment", copy=False
    )
    ...
    def _search_message_partner_ids(self, operator, operand):
        ...
        followers = (
            ...
            .sudo()
            .search([("res_model", "=", self._name), ("partner_id", operator, operand)])
        )
    ...
    def _search_message_is_follower(self, operator, operand):
        ...
        if (operator == "=" and operand) or (operator == "!=" and not operand):
            ...
            # using read() below is much faster than followers.mapped('res_id')
            return [
                ("id", "not in", [res["res_id"] for res in followers.read(["res_id"])])
            ]
    ...
    def _search_has_message(self, operator, value):
        ...
        return [
            (
                "id",
                operator_new,
                ("SELECT res_id FROM mail_message WHERE model=%s", [self._name]),
            )
        ]
    ...
    def _compute_message_attachment_count(self):
        ...
        attachment_count_dict = dict(
            (d["res_id"], d["res_id_count"]) for d in read_group_var
        )
        for record in self:
    ...
    def create(self, vals_list):
        ...
        for thread, values in zip(threads, vals_list):
            ...
            thread._message_auto_subscribe(
                create_values, followers_existing_policy="update"
            )
    ...
    def get_empty_list_help(self, help):
        ...
        if (
            catchall_domain and model and res_id
        ...
        if alias:
            ...
            if "oe_view_nocontent_alias" not in help:
                return (
                    "%(static_help)s<p class='oe_view_nocontent_alias'>%(dyn_help)s</p>"
                    % {
                        "static_help": help,
                        "dyn_help": _(
    ...
    def _compute_field_value(self, field):
        if not self._context.get("tracking_disable") and not self._context.get(
            "mail_notrack"
        ):
            self._track_prepare(
    ...
    def _check_can_update_message_content(self, messages):
        ...
        if any(message.subtype_id.id != note_id for message in messages):
            raise exceptions.UserError(
                _(
                    "Only logged notes can have their content updated on model '%s'",
                    self._name,
                )
            )
    ...
    def _mail_get_partner_fields(self):
        ...
        return [fname for fname in ("partner_id", "partner_ids") if fname in self]
    ...
    def _mail_get_partners(self):
        ...
        return dict(
            (
                record.id,
                self.env["res.partner"].union(
                    *[record[fname] for fname in partner_fields]
                ),
            )
            for record in self
        )
    ...
    def _message_track_post_template(self, changes):
        ...
        for _field_name, (template, post_kwargs) in templates.items():
            ...
            if isinstance(template, str):
                ...
                self._fallback_lang().message_post_with_template(
                    template.id, **post_kwargs
                )
        return True
    ...
    def _routing_create_bounce_email(
        ...
        if bounce_from:
            bounce_mail_values["email_from"] = tools.formataddr(
                ("MAILER-DAEMON", bounce_from)
            )
        elif (
            self.env["ir.config_parameter"].sudo().get_param("mail.catchall.alias")
            not in message["To"]
        ):
            bounce_mail_values["email_from"] = tools.decode_message_header(
                message, "To"
            )
        else:
            bounce_mail_values["email_from"] = tools.formataddr(
                ("MAILER-DAEMON", self.env.user.email_normalized)
            )
    ...
    def _routing_handle_bounce(self, email_message, message_dict):
        ...
        bounced_msg_id, bounced_message = (
        ...
        if bounced_email:
            ...
            if (
                ...
                and issubclass(type(bounced_record), self.pool["mail.thread"])
            ):
    ...
    def _routing_check_route(self, message, message_dict, route, raise_exception=True):
        ...
        if not author_id:
            if record_set:
                authors = self._mail_find_partner_from_emails(
                    [email_from], records=record_set
                )
            elif alias and alias.alias_parent_model_id and alias.alias_parent_thread_id:
                records = self.env[alias.alias_parent_model_id.model].browse(
                    alias.alias_parent_thread_id
                )
                authors = self._mail_find_partner_from_emails(
                    [email_from], records=records
                )
            else:
                authors = self._mail_find_partner_from_emails(
                    [email_from], records=None
                )
            if authors:
        ...
        if alias:
            ...
            if error_message:
                ...
                self._routing_create_bounce_email(
                    email_from, body, message, references=message_id
                )
                return False
        ...
        if valid_email:
            ...
            for model in [
                ...
                self.env[model.model].sudo().search(
                    [("message_bounce", ">", 0), ("email_normalized", "=", valid_email)]
                )._message_reset_bounce(valid_email)
    ...
    def _detect_loop_sender(self, message, message_dict, routes):
        ...
        for model in models:
            ...
            if mail_incoming_messages_count >= LOOP_THRESHOLD:
                _logger.info(
                    "Email address %s created too many <%s>.", email_from, model
                )
                ...
                self._routing_create_bounce_email(
                    email_from, body, message, references=references
                )
                return True
    ...
    def _detect_loop_headers(self, msg_dict):
        ...
        if "-loop-detection-bounce-email@" in msg_dict.get(
            "references", ""
    ...
    def _message_route_process(self, message, message_dict, routes):
        ...
        for model, thread_id, custom_values, user_id, alias in routes or ():
            ...
            if not (
                thread_id
                and hasattr(Model, "message_update")
            ...
            post_params = dict(
                subtype_id=subtype_id, partner_ids=partner_ids, **message_dict
            )
            ...
            if (
                ...
                partner_from_found = message_dict.get("author_id") and message_dict[
                    "author_id"
                ] != self.env["ir.model.data"]._xmlid_to_res_id("base.partner_root")
            ...
            for part in message.walk():
                ...
                if part.get_content_type() == "text/plain" and (
                    ...
                    body = tools.append_content_to_html(
                        body,
                        tools.ustr(content, encoding, errors="replace"),
                        preserve=True,
                    )
                # 3) text/html -> raw
    ...
    def _message_parse_extract_bounce(self, email_message, message_dict):
        ...
        if dsn_part and len(dsn_part.get_payload()) > 1:
            ...
            if final_recipient_data and ";" in final_recipient_data:
                bounced_email = tools.email_normalize(
        ...
        if email_part:
            ...
            bounced_msg_id = tools.mail_header_msgid_re.findall(
                tools.decode_message_header(email_payload, "Message-Id")
            )
            if bounced_msg_id:
                bounced_message = (
                    self.env["mail.message"]
                    .sudo()
                    .search([("message_id", "in", bounced_msg_id)])
                )
        ...
        if email and not partner:
            ...
            if partner_info.get("partner_id"):
                partner = (
                    self.env["res.partner"]
                    .sudo()
                    .browse([partner_info["partner_id"]])[0]
                )
        if email and email in [
            val[1] for val in result[self.ids[0]]
        ]:  # already existing email -> skip
            return result
        if (
            partner and partner in self.message_partner_ids
        ):  # recipient already in the followers -> skip
            return result
        if partner and partner.id in [
            val[0] for val in result[self.ids[0]]
        ]:  # already existing partner ID -> skip
            return result
        if partner and partner.email:  # complete profile: id, name <email>
            result[self.ids[0]].append(
                (partner.id, partner.email_formatted, lang, reason)
            )
        elif partner:  # incomplete profile: id, name
            result[self.ids[0]].append(
                (partner.id, "%s" % (partner.name), lang, reason)
            )
    ...
    def _mail_find_user_for_gateway(self, email, alias=None):
        ...
        if catchall_domain:
            left_part = (
                ...
                if normalized_email.split("@")[1] == catchall_domain.lower()
                else False
            )
            if left_part:
                if (
                    self.env["mail.alias"]
                    .sudo()
                    .search_count([("alias_name", "=", left_part)])
                ):
                    return self.env["res.users"]
    ...
    def _mail_find_partner_from_emails(
        ...
        normalized_emails = [
            tools.email_normalize(contact)
            for contact in emails
            if tools.email_normalize(contact)
        ]
        if catchall_domain:
            domain_left_parts = [
                email.split("@")[0]
                for email in normalized_emails
                if email and email.split("@")[1] == catchall_domain.lower()
            ]
            if domain_left_parts:
                found_alias_names = (
                    self.env["mail.alias"]
                    .sudo()
                    .search([("alias_name", "in", domain_left_parts)])
                    .mapped("alias_name")
                )
                normalized_emails = [
                    email
                    for email in normalized_emails
                    if email.split("@")[0] not in found_alias_names
                ]

        done_partners = [
            follower
            for follower in followers
            if follower.email_normalized in normalized_emails
        ]
        remaining = [
            email
            for email in normalized_emails
            if email not in [partner.email_normalized for partner in done_partners]
        ]
        ...
        for contact in emails:
            ...
            partner = next(
                (
                    partner
                    for partner in done_partners
                    if partner.email_normalized == normalized_email
                ),
                self.env["res.partner"],
            )
    ...
    def _message_partner_info_from_emails(self, emails, link_mail=False):
        ...
        for idx, contact in enumerate(emails):
            ...
            partner_info = {
                "full_name": partner.email_formatted if partner else contact,
        ...
        if attachments:  # generate
            ...
            for attachment in attachments:
                ...
                if body and (cid and cid in cids_in_body or name in names_in_body):
                    attachement_values["access_token"] = self.env[
                        "ir.attachment"
            ...
            new_attachments = (
                self.env["ir.attachment"].sudo().create(attachement_values_list)
            )
            cid_mapping = {}
            name_mapping = {}
            for counter, new_attachment in enumerate(new_attachments):
                ...
                if "access_token" in attachement_values_list[counter]:
                    if cid:
                        cid_mapping[cid] = (
                            new_attachment.id,
                            attachement_values_list[counter]["access_token"],
                        )
            ...
            if (cid_mapping or name_mapping) and body:
                ...
                for node in root.iter("img"):
                    ...
                    if not attachment_data and node.get("data-filename"):
                        attachment_data = name_mapping.get(
                            node.get("data-filename"), False
                        )
                    if attachment_data:
                        node.set(
                            "src", "/web/image/%s?access_token=%s" % attachment_data
                        )
                        postprocessed = True
                if postprocessed:
                    return_values["body"] = lxml.html.tostring(
    ...
    def message_post(
        ...
        **kwargs,
        ...
        if "channel_ids" in kwargs:
            raise ValueError(
                _(
                    "Posting a message with channels as listeners is not supported since Odoo 14.3+. Please update code accordingly."
                )
            )
        if "model" in msg_kwargs or "res_id" in msg_kwargs:
            raise ValueError(
                _(
                    "message_post does not support model and res_id parameters anymore. Please call message_post on record."
                )
            )
        if "subtype" in kwargs:
            raise ValueError(
                _(
                    "message_post does not support subtype parameter anymore. Please give a valid subtype_id or subtype_xmlid value instead."
                )
            )
        ...
        self = (
            self._fallback_lang()
        )  # add lang to context immediately since it will be useful in various flows latter.
        ...
        if self.env.user._is_public() and "guest" in self.env.context:
            ...
            author_id, email_from = self._message_compute_author(
                author_id, email_from, raise_on_email=True
            )
        ...
        if (
            msg_values["author_id"]
            and msg_values["message_type"] != "notification"
            and not self._context.get("mail_create_nosubscribe")
        ):
            if (
                self.env["res.partner"].browse(msg_values["author_id"]).active
    ...
    def _message_set_main_attachment_id(self, attachment_ids):
        ...
        if (
            ...
            prioritary_attachments = (
                all_attachments.filtered(lambda x: x.mimetype.endswith("pdf"))
    ...
    def _message_compose_with_view(self, views_or_xmlid, message_log=False, **kwargs):
        ...
        for record in self:
            ...
            rendered_template = self.env["ir.qweb"]._render(
                view_ref, values, minimal_qcontext=True, raise_if_not_found=False
            )
            if not rendered_template:
                continue
            if message_log:
                messages_as_sudo += record._message_log(
                    body=rendered_template, **kwargs
                )
            else:
                kwargs["body"] = rendered_template
                # a bit complicated to handle, to be improved soon in master
                _mails_as_sudo, _messages_as_sudo = record.message_post_with_template(
                    False, **kwargs
                )
    ...
    def message_notify(
        ...
        **kwargs,
        ...
        if "subtype_id" not in msg_values:
            msg_values["subtype_id"] = self.env["ir.model.data"]._xmlid_to_res_id(
                "mail.mt_note"
            )
        if "reply_to" not in msg_values:
            msg_values["reply_to"] = self._notify_get_reply_to(default=email_from)[
    ...
    def _message_log_with_view(self, views_or_xmlid, **kwargs):
        ...
        return self._message_compose_with_view(
            views_or_xmlid, message_log=True, **kwargs
        )
    ...
    def _message_create(self, values_list):
        ...
        for values in values_list:
            ...
            create_values["partner_ids"] = [
                Command.link(pid) for pid in create_values.get("partner_ids", [])
            ]
    ...
    def _notify_by_email_prepare_rendering_context(
        ...
        signature = ""
        email_add_signature = (
            msg_vals.get("email_add_signature")
            if msg_vals and "email_add_signature" in msg_vals
            else message.email_add_signature
        )
        if email_add_signature:
            ...
            author_user = (
                self.env.user
                if self.env.user.partner_id == author
                else author.user_ids[0]
        ...
        if force_email_company:
            ...
            company = (
                ...
                if self and "company_id" in self and self.company_id
        ...
        if force_email_lang:
            ...
            if template and template.lang:
                lang = template._render_lang([self.env.context["default_res_id"]])[
        ...
        if (
            ...
            for tracking_value in (
                ...
                if (
                    ...
                    or self.user_has_groups(groups)
                ):
                    tracking.append(
                        (
                            tracking_value.field_desc,
    ...
    def _notify_get_recipients_groups(self, msg_vals=None):
        ...
        """
        is_thread_notification = self._notify_get_recipients_thread_info(
            msg_vals=msg_vals
        )["is_thread_notification"]
        return [
            [
                "user",
    ...
    def _notify_get_recipients_thread_info(self, msg_vals=None):
        ...
        and its sub-methods."""
        res_model = (
            msg_vals["model"] if msg_vals and "model" in msg_vals else self._name
        )
        res_id = (
            msg_vals["res_id"]
            if msg_vals and "res_id" in msg_vals
            else self.ids[0]
            if self.ids
            else False
        )
        return {
            "is_thread_notification": res_model
            and (res_model != "mail.thread")
            and res_id
        }
    ...
    def _message_subscribe(self, partner_ids=None, subtype_ids=None, customer_ids=None):
        ...
        if not subtype_ids:
            ...
            self.env["mail.followers"]._insert_followers(
                ...
                self.ids,
                partner_ids,
                subtypes=dict((pid, subtype_ids) for pid in partner_ids),
                customer_ids=customer_ids,
                check_existing=True,
    ...
    def _message_auto_subscribe_followers(self, updated_values, default_subtype_ids):
        ...
        if (
            ...
            and (
                getattr(field, "track_visibility", False)
                or getattr(field, "tracking", False)
            )
        ):
    ...
    def _message_update_content(
        ...
        if attachment_ids:
            msg_values.update(
                self._message_post_process_attachments(
                    [],
                    attachment_ids,
                    {
                        "body": body,
    ...
    def _get_mail_thread_data(self, request_list):
        ...
        if "attachments" in request_list:
            res[
                "attachments"
            ] = self._get_mail_thread_data_attachments()._attachment_format()
            res["mainAttachment"] = (
            ...
            res["followers"] = [
                {
                    ...
                    "partner": follower.partner_id.mail_partner_format()[
                        follower.partner_id
                    ],
                }
