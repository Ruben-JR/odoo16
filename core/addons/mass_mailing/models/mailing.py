...
class MassMailing(models.Model):
    ...
    def default_get(self, fields_list):
        ...
        if (
            ...
            and ("schedule_type" in fields_list and "schedule_date" in fields_list)
            and fields.Datetime.from_string(default_calendar_date)
            > fields.Datetime.now()
        ):
            vals.update(
        ...
        if (
            "contact_list_ids" in fields_list
    ...
    body_html = fields.Html(
        string="Body converted to be sent by mail", render_engine="qweb", sanitize=False
    )
    ...
    ab_testing_completed = fields.Boolean(
        related="campaign_id.ab_testing_completed", store=True
    )
    ...
    def _compute_clicks_ratio(self):
        ...
        mapped_data = dict(
            [(m["id"], 100 * m["nb_clicks"] / m["nb_mails"]) for m in mass_mailing_data]
        )
        for mass_mailing in self:
    ...
    def _compute_next_departure(self):
        cron_next_call = (
            self.env.ref("mass_mailing.ir_cron_mass_mailing_queue").sudo().nextcall
        )
    ...
    def _compute_mailing_filter_count(self):
        ...
        mapped_data = {
            data["mailing_model_id"][0]: data["mailing_model_id_count"]
        ...
        if mass_mailing_copy:
            ...
            if self.mailing_type == "mail":
                action["views"] = [
                    (
                        self.env.ref(
                            "mass_mailing.mailing_mailing_view_form_full_width"
                        ).id,
                        "form",
                    ),
                ]
    ...
    def action_retry_failed(self):
        failed_mails = (
            self.env["mail.mail"]
            .sudo()
            .search([("mailing_id", "in", self.ids), ("state", "=", "exception")])
        )
    ...
    def _action_view_documents_filtered(self, view_filter):
        ...
        if view_filter == "reply":
            found_traces = self.mailing_trace_ids.filtered(
                lambda trace: trace.trace_status == view_filter
            )
            ...
            found_traces = self.mailing_trace_ids.filtered(
                lambda trace: trace.trace_status in ("sent", "open", "reply")
            )
            ...
            found_traces = self.mailing_trace_ids.filtered(
                lambda trace: trace.sent_datetime
            )
        else:
    ...
    def action_fetch_favorites(self, extra_domain=None):
        ...
        values_list.sort(
            key=lambda values: values["user_id"][0] != self.env.user.id
            if values["user_id"]
            else -1
        )
        ...
        if len(list_ids) > 0:
            ...
            message = (
                ...
                if value
                    ...
                    "The recipient <strong>subscribed to %s</strong> mailing list(s)"
                )
            )
            for record in records:
                ...
                if len(record_lists) > 0:
                    record.sudo().message_post(
                        body=message
                        % ", ".join(
                            str(list.name) for list in record_lists.mapped("list_id")
                        )
                    )
    ...
    def _get_ab_testing_description_values(self):
        ...
        other_ab_testing_mailings = self._get_ab_testing_siblings_mailings().filtered(
            lambda m: m.id != self.id
        )
        other_ab_testing_pc = sum(
            [mailing.ab_testing_pc for mailing in other_ab_testing_mailings]
        )
        return {
            "mailing": self,
    ...
    def _get_seen_list(self):
        ...
        if "partner_id" in target._fields and target._fields["partner_id"].store:
            ...
            "partner_email" in target._fields and target._fields["partner_email"].store
        ):
            ...
            raise UserError(
                _("Unsupported mass mailing model %s", self.mailing_model_id.name)
            )
                ...
                [
                    ...
                    [
                        (
                            "mass_mailing_id",
                            "=",
                            self._get_ab_testing_siblings_mailings().ids,
                        )
                    ],
                ]
            )
    ...
    def _action_send_statistics(self):
        ...
        for mailing in self:
            ...
            if mail_user.has_group("mass_mailing.group_mass_mailing_user"):
                rendering_data["mailing_report_token"] = self._get_unsubscribe_token(
                    mail_user.id
                )
    ...
    def _prepare_statistics_email_values(self):
        ...
        return {
            ...
            "top_button_url": url_join(
                web_base_url, f"/web#id={self.id}&model=mailing.mailing&view_type=form"
            ),
            "kpi_data": [
                kpi,
                {
    ...
    def _get_default_mailing_domain(self):
        ...
        if hasattr(self.env[self.mailing_model_name], "_mailing_get_default_domain"):
            mailing_domain = self.env[
                self.mailing_model_name
            ]._mailing_get_default_domain(self)

        if (
            self.mailing_type == "mail"
            and "is_blacklisted" in self.env[self.mailing_model_name]._fields
        ):
            mailing_domain = expression.AND(
                [[("is_blacklisted", "=", False)], mailing_domain]
            )
