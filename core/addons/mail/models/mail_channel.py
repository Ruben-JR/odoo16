...
class Channel(models.Model):
    ...
    channel_member_ids = fields.One2many(
        "mail.channel.member", "channel_id", string="Members", groups="base.group_user"
    )
    rtc_session_ids = fields.One2many(
        "mail.channel.rtc.session", "channel_id", groups="base.group_system"
    )
    is_member = fields.Boolean(
        "Is Member", compute="_compute_is_member", search="_search_is_member"
    )
    ...
    def _compute_is_member(self):
        ...
        members = (
            self.env["mail.channel.member"]
            .sudo()
            .search(expression.AND([[("channel_id", "in", self.ids)], user_domain]))
        )
    ...
    def _compute_member_count(self):
        read_group_res = self.env["mail.channel.member"]._read_group(
            domain=[("channel_id", "in", self.ids)],
            fields=["channel_id"],
            groupby=["channel_id"],
        )
        member_count_by_channel_id = {
            item["channel_id"][0]: item["channel_id_count"] for item in read_group_res
        }
        for channel in self:
    ...
    def _compute_group_public_id(self):
        ...
        channels.filtered(
            lambda channel: not channel.group_public_id
    ...
    def create(self, vals_list):
        for vals in vals_list:
            ...
            membership_pids = [
                cmd[2]["partner_id"] for cmd in membership_ids_cmd if cmd[0] == 0
            ]
    ...
    def write(self, vals):
        if "channel_type" in vals:
            ...
            if failing_channels:
                raise UserError(
                    _("Cannot change the channel type of: %(channel_names)s"),
    ...
    def init(self):
        self._cr.execute(
            ...
            ("mail_channel_member_seen_message_id_idx",),
        )
        if not self._cr.fetchone():
            self._cr.execute(
                "CREATE INDEX mail_channel_member_seen_message_id_idx ON mail_channel_member (channel_id,partner_id,seen_message_id)"
            )
    ...
    def _subscribe_users_automatically_get_members(self):
        ...
        return dict(
            (
                channel.id,
                (channel.group_ids.users.partner_id - channel.channel_partner_ids).ids,
            )
            for channel in self
        )
    ...
    def add_members(
        ...
        for channel in self:
            ...
            if channel.group_public_id:
                invalid_partners = partners.filtered(
                    lambda partner: channel.group_public_id
                    not in partner.user_ids.groups_id
                )
                if invalid_partners:
                    raise UserError(
                        _(
                            ...
                            partner_names=", ".join(
                                partner.name for partner in invalid_partners
                            ),
                        )
                    )
                if guests:
            ...
            existing_members = self.env["mail.channel.member"].search(
                expression.AND(
                    [
                        [("channel_id", "=", channel.id)],
                        expression.OR(
                ...
                if user:
                    notifications.append(
                        (
                            ...
                            "mail.channel/joined",
                            {
                                "channel": member.channel_id.with_user(user)
            ...
            for member in new_members.filtered(lambda member: member.guest_id):
                member.channel_id.message_post(
                    body=_('<div class="o_mail_notification">joined the channel</div>'),
                    message_type="notification",
                    subtype_xmlid="mail.mt_comment",
                )
                    ...
                    "mail.channel/insert",
                    {
                        "channelMembers": [
                            (
                                "insert",
                                list(
                                    new_members._mail_channel_member_format().values()
                                ),
                            )
                        ],
                        "id": channel.id,
            ...
            if existing_members:
                ...
                notifications.append(
                    (
                        current_partner or current_guest,
                        "mail.channel/insert",
                        {
                            "channelMembers": [
                                (
                                    "insert",
                                    list(
                                        existing_members._mail_channel_member_format().values()
                                    ),
                                )
                            ],
                            "id": channel.id,
        ...
        if invite_to_rtc_call:
            for channel in self:
                ...
                if current_channel_member and current_channel_member.rtc_session_ids:
                    current_channel_member._rtc_invite_members(
        ...
        for channel in self.sudo():
            ...
            if channel.group_public_id:
                if (
                    ...
                    or channel.group_public_id not in partner.user_ids.groups_id
                ):
                    return False
    ...
    def _rtc_cancel_invitations(self, member_ids=None):
        ...
        if member_ids:
            channel_member_domain = expression.AND(
                [channel_member_domain, [("id", "in", member_ids)]]
            )
        ...
        if members:
            channel_data["invitedMembers"] = [
                (
                    "insert-and-unlink",
                    list(
                        members._mail_channel_member_format(
                            fields={
                                "id": True,
                                "channel": {},
                                "persona": {
                                    "partner": {"id", "name", "im_status"},
                                    "guest": {"id", "name", "im_status"},
                                },
                            }
                        ).values()
                    ),
    ...
    def _notify_get_recipients(self, message, msg_vals, **kwargs):
        ...
        message_type = (
            msg_vals.get("message_type", "email") if msg_vals else msg_sudo.message_type
        )
        ...
        email_from = tools.email_normalize(
            msg_vals.get("email_from") or msg_sudo.email_from
        )
    ...
    def _notify_thread(self, message, msg_vals=False, **kwargs):
        ...
        rdata = super(Channel, self)._notify_thread(
            message, msg_vals=msg_vals, **kwargs
        )

        message_format_values = message.message_format()[0]
        bus_notifications = self._channel_message_notifications(
    ...
    def message_post(self, *, message_type="notification", **kwargs):
        self.filtered(
            lambda channel: channel.is_chat or channel.channel_type == "group"
    ...
    def _check_can_update_message_content(self, message):
        ...
        if not message.message_type == "comment":
            raise UserError(
                _(
                    "Only messages type comment can have their content updated on model 'mail.channel'"
                )
            )
    ...
    def _message_add_reaction_after_hook(self, message, content):
        ...
        reactions = (
            ...
            .sudo()
            .search([("message_id", "=", message.id), ("content", "=", content)])
        )
        self.env["bus.bus"]._sendone(
            self,
            "mail.message/insert",
        ...
        return super()._message_add_reaction_after_hook(
    ...
    def _message_remove_reaction_after_hook(self, message, content):
        ...
        return super()._message_remove_reaction_after_hook(
            message=message, content=content
        )
        ...
        for partner in self.env["res.partner"].browse(partner_ids):
            ...
            if user_id:
                ...
                for channel_info in user_channels.channel_info():
                    notifications.append(
                        (partner, "mail.channel/legacy_insert", channel_info)
                    )
    ...
    def channel_info(self):
        ...
        if not self:
            return []
        channel_infos = []
        rtc_sessions_by_channel = (
            self.sudo().rtc_session_ids._mail_rtc_session_format_by_channel()
        )
        channel_last_message_ids = dict(
            (r["id"], r["message_id"]) for r in self._channel_last_message_ids()
        )
        current_partner = self.env["res.partner"]
        current_guest = self.env["mail.guest"]
        guest = self.env["mail.guest"]._get_guest_from_context()
        ...
        all_needed_members = self.env["mail.channel.member"].search(
            expression.AND(
                [[("channel_id", "in", self.ids)], all_needed_members_domain]
            ),
            order="id",
        )
        ...
        invited_members_by_channel = defaultdict(
            lambda: self.env["mail.channel.member"]
        )
        member_of_current_user_by_channel = defaultdict(
            lambda: self.env["mail.channel.member"]
        )
        ...
        for channel in self:
            ...
            if current_partner or current_guest:
                ...
                member = member_of_current_user_by_channel.get(
                    channel, self.env["mail.channel.member"]
                ).with_prefetch(
                    [m.id for m in member_of_current_user_by_channel.values()]
                )
                if member:
                    channel_data["channelMembers"] = [
                        ("insert", list(member._mail_channel_member_format().values()))
                    ]
                    ...
                    info["last_interest_dt"] = member.last_interest_dt.strftime(
                        DEFAULT_SERVER_DATETIME_FORMAT
                    )
                    if member.rtc_inviting_session_id:
                        info["rtc_inviting_session"] = {
                            "id": member.rtc_inviting_session_id.id
                        }
            # add members info
            if channel.channel_type != "channel":
                ...
                channel_data["channelMembers"] = [
                    (
                        "insert",
                        list(
                            members_by_channel[channel]
                            ._mail_channel_member_format()
                            .values()
                        ),
                    )
                ]
                info["seen_partners_info"] = sorted(
                    [
                        ...
                        for cp in members_by_channel[channel]
                        if cp.partner_id
                    ],
            ...
            info.update(
                {
                    "invitedMembers": [
                        (
                            "insert",
                            list(
                                invited_members_by_channel[channel]
                                ._mail_channel_member_format(
                                    fields={
                                        "id": True,
                                        "channel": {},
                    ...
                    "rtcSessions": [
                        ("insert", rtc_sessions_by_channel.get(channel, []))
                    ],
                }
            )
    ...
    def _channel_fetch_message(self, last_id=False, limit=20):
        ...
        return (
            self.env["mail.message"]
            ._message_fetch(domain=domain, limit=limit)
            .message_format()
        )
    ...
    def channel_get(self, partners_to, pin=True):
        ...
        if result:
            ...
            channel = self.create(
                {
                    "channel_partner_ids": [
                        Command.link(partner_id) for partner_id in partners_to
                    ],
                    "channel_member_ids": [
                        Command.create(
                            {
    ...
    def channel_fetch_preview(self):
        ...
        last_messages = (
            self.env["mail.message"].browse(channels_preview).message_format()
        )
    ...
    def load_more_members(self, known_member_ids):
        ...
        return {
            "channelMembers": [
                ("insert", list(unknown_members._mail_channel_member_format().values()))
            ],
            "memberCount": count,
        }
    ...
    def execute_command_help(self, **kwargs):
        ...
        if self.channel_type == "channel":
            ...
            all_channel_members = self.env["mail.channel.member"].with_context(
                active_test=False
            )
            channel_members = all_channel_members.search(
                [("partner_id", "!=", partner.id), ("channel_id", "=", self.id)]
            )
            msg = _(
                "You are in a private conversation with <b>@%s</b>.",
                _(" @").join(
                    html_escape(member.partner_id.name or member.guest_id.name)
    ...
    def execute_command_who(self, **kwargs):
        ...
        members = [
            m.partner_id._get_html_link(title=f"@{m.partner_id.name}")
            if m.partner_id
            else f"<strong>@{html_escape(m.guest_id.name)}</strong>"
        ...
        if len(members) == 0:
            ...
            msg = _(
                "Users in this channel: %(members)s %(dots)s and you.",
