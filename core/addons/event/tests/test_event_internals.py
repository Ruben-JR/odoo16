...
class TestEventData(TestEventInternalsCommon):
    ...
    def test_event_date_computation(self):
        ...
        with freeze_time(self.reference_now):
            ...
            self.assertEqual(registration.get_date_range_str(), "today")

            event.date_begin = datetime(2020, 2, 1, 15, 0, 0)
            self.assertEqual(registration.get_date_range_str(), "tomorrow")

            event.date_begin = datetime(2020, 2, 2, 6, 0, 0)
            self.assertEqual(registration.get_date_range_str(), "in 2 days")

            event.date_begin = datetime(2020, 2, 20, 17, 0, 0)
            self.assertEqual(registration.get_date_range_str(), "next month")

            event.date_begin = datetime(2020, 3, 1, 10, 0, 0)
            self.assertEqual(registration.get_date_range_str(), "on Mar 1, 2020")
    ...
    def test_event_ongoing(self):
        ...
        finished_or_upcoming_event_ids = self.env["event.event"]._search(
            [("is_ongoing", "=", False)]
        )
