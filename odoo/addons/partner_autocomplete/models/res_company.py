...
class ResCompany(models.Model):
    ...
    def _enrich(self):
        ...
        company_data = self.env["res.partner"].enrich_company(
            company_domain, False, self.vat, timeout=COMPANY_AC_TIMEOUT
        )
        ...
        if company_data.get("child_ids"):
            company_data["child_ids"] = [
                dict(
                    child_data,
                    **self._enrich_extract_m2o_id(
                        child_data, ["state_id", "country_id"]
                    )
                )
        ...
        if additional_data:
            ...
            template_values["flavor_text"] = _(
                "Company auto-completed by Odoo Partner Autocomplete Service"
            )
    ...
    def _get_company_domain(self):
        ...
        company_domain = (
            tools.url_domain_extract(self.website) if self.website else False
        )
