...
class ResPartner(models.Model):
    ...
    l10n_id_pkp = fields.Boolean(
        string="ID PKP", compute="_compute_l10n_id_pkp", store=True, readonly=False
    )
...
class ResConfigSettings(models.TransientModel):
    ...
    l10n_id_tax_address = fields.Char(
        "Tax Address",
        related="company_id.partner_id.l10n_id_tax_address",
        readonly=False,
    )
    l10n_id_tax_name = fields.Char(
        "Tax Name", related="company_id.partner_id.l10n_id_tax_address", readonly=False
    )
