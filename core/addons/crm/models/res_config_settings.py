...
class ResConfigSettings(models.TransientModel):
    ...
    predictive_lead_scoring_start_date = fields.Date(
        ...
        inverse="_inverse_pls_start_date_str",
    )
    ...
    predictive_lead_scoring_fields = fields.Many2many(
        ...
        inverse="_inverse_pls_fields_str",
    )
    ...
    def _compute_crm_auto_assignment_data(self):
        ...
        for setting in self:
            if setting.crm_use_auto_assignment and assign_cron:
                ...
                setting.crm_auto_assignment_interval_type = (
                    setting.crm_auto_assignment_run_datetime
                ) = False
    ...
    def _compute_predictive_lead_scoring_field_labels(self):
        for setting in self:
            if setting.predictive_lead_scoring_fields:
                ...
                setting.predictive_lead_scoring_field_labels = _(
                    "%s and %s", ", ".join(field_names[:-1]), field_names[-1]
                )
            else:
    ...
    def set_values(self):
        ...
        if assign_cron:
            ...
            cron_vals = {
                ...
                if self.crm_auto_assignment_run_datetime
                else assign_cron.nextcall,
            }
            cron_vals = {
                field_name: value
    ...
    def action_crm_assign_leads(self):
        ...
        return (
            ...
            .action_assign_leads(work_days=2, log=False)
        )
