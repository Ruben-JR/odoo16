...
class SaleOrder(models.Model):
    ...
    applied_coupon_ids = fields.Many2many(
        "loyalty.card", string="Manually Applied Coupons", copy=False
    )
    code_enabled_rule_ids = fields.Many2many(
        "loyalty.rule", string="Manually Triggered Rules", copy=False
    )
    coupon_point_ids = fields.One2many(
        "sale.order.coupon.points", "order_id", copy=False
    )
    ...
    def action_confirm(self):
        ...
        for coupon, change in (
            self.filtered(lambda s: s.state != "sale")._get_point_changes().items()
        ):
    ...
    def _action_cancel(self):
        ...
        for coupon, changes in (
            previously_confirmed.filtered(lambda s: s.state not in ("sale", "done"))
            ._get_point_changes()
            .items()
        ):
        ...
        self.coupon_point_ids.coupon_id.sudo().filtered(
            lambda c: not c.program_id.is_nominative
    ...
    def _get_reward_values_product(self, reward, coupon, product=None, **kwargs):
        ...
        taxes = self.fiscal_position_id.map_tax(
            product.taxes_id.filtered(lambda t: t.company_id == self.company_id)
        )
    ...
    def _discountable_order(self, reward):
        ...
        lines = (
            ...
            else (self.order_line - self._get_no_effect_on_threshold_lines())
        )
        for line in lines:
    ...
    def _discountable_cheapest(self, reward):
        ...
        discountable = cheapest_line.price_unit * (
            1 - (cheapest_line.discount or 0) / 100
        )
    ...
    def _discountable_specific(self, reward):
        ...
        for lines in discount_lines.values():
            ...
            if line_reward.discount_mode == "percent":
                ...
                discounted_amounts = {
                    line.tax_id: abs(line.price_total) for line in lines
                }
    ...
    def _get_reward_values_discount(self, reward, coupon, **kwargs):
        ...
        if reward.discount_mode == "per_point":
            max_discount = min(
                ...
                reward.currency_id._convert(
                    reward.discount * self._get_real_points_for_coupon(coupon),
                    self.currency_id,
                    self.company_id,
                    fields.Date.today(),
                ),
        ...
        mapped_taxes = {
            tax: self.fiscal_position_id.map_tax(tax) for tax in discountable_per_tax
        }
        reward_dict = {
            tax: {
                "name": _(
                    ...
                    tax_str=len(discountable_per_tax)
                    and any(t.name for t in mapped_taxes[tax])
                    and _(
                        " - On product with the following taxes: %(taxes)s",
                        taxes=", ".join(mapped_taxes[tax].mapped("name")),
                    )
                    or "",
                ),
                "product_id": reward.discount_line_product_id.id,
                ...
                "sequence": sequence,
                "tax_id": [(Command.CLEAR, 0, 0)]
                + [(Command.LINK, tax.id, False) for tax in mapped_taxes[tax]],
            }
    ...
    def _get_trigger_domain(self):
        ...
        return [
            ...
            ("program_id.date_to", ">=", fields.Date.context_today(self)),
        ]
    ...
    def _get_reward_coupons(self):
        ...
        """
        self.ensure_one()
        return self.coupon_point_ids.coupon_id.filtered(
    ...
    def _compute_invoice_status(self):
        ...
        for order in self:
            if order.invoice_status != "to invoice":
                continue
            if not any(
                not line.is_reward_line and line.invoice_status == "to invoice"
                for line in order.order_line
            ):
    ...
    def _get_real_points_for_coupon(self, coupon, post_confirm=False):
        ...
        if coupon.program_id.applies_on != "future" and self.state not in (
            "sale",
            "done",
        ):
        ...
        points -= sum(
            self.order_line.filtered(lambda l: l.coupon_id == coupon).mapped(
                "points_cost"
            )
        )
    ...
    def _apply_program_reward(self, reward, coupon, **kwargs):
        ...
        if reward.is_global_discount:
            ...
            if (
                ...
                and global_discount_reward.discount >= reward.discount
            ):
        ...
        for coupon in all_coupons:
            ...
            for reward in coupon.program_id.reward_ids:
                ...
                if (
                    ...
                    and total_is_zero
                    and (not has_payment_reward or reward.program_id.is_payment_program)
                ):
                    continue
    ...
    def _update_programs_and_rewards(self):
        ...
        self.applied_coupon_ids = self.applied_coupon_ids.filtered(
            lambda c: (
    ...
    def __try_apply_program(self, program, coupon, status):
        ...
        if coupon:
            ...
            if not coupon:
                ...
                self._add_points_for_coupon(
                    {coupon: x for coupon, x in zip(coupons, all_points)}
                )
    ...
    def _try_apply_code(self, code):
        ...
        if not program:
            ...
            if (
                ...
                coupon.expiration_date and coupon.expiration_date < fields.Date.today()
            ):
        ...
        if not program or not program.active:
            ...
            program.limit_usage and program.total_order_count >= program.max_usage
        ) or (program.date_to and program.date_to < fields.Date.context_today(self)):
