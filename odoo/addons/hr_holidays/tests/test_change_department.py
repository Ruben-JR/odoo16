...
class TestChangeDepartment(TestHrHolidaysCommon):
    def test_employee_change_department_request_change_department(self):
            ...
            return self.HolidaysEmployeeGroup.create(
                {
                    ...
                    "date_from": (
                        datetime.today() + relativedelta(days=start)
                    ).strftime("%Y-%m-%d %H:%M"),
        ...
        self.assertEqual(
            ...
            self.rd_dept,
            "hr_holidays: approved futur leave request should change department if employee change department",
        )
        ...
        self.assertEqual(
            ...
            self.hr_dept,
            "hr_holidays: refused futur leave request should change department if employee change department",
        )
