...
class TestCowViewSaving(TestViewSavingCommon):
    ...
    def test_cow_cross_inherit(self):
        ...
        generic_view = View.with_context(website_id=None)._get_view_id(
            "website.main_view"
        )
        specific_view = View.with_context(website_id=1)._get_view_id(
            "website.main_view"
        )
        generic_view_arch = (
            View.browse(generic_view)
            .with_context(load_all_views=True)
            .get_combined_arch()
        )
        specific_view_arch = (
            View.browse(specific_view)
            .with_context(load_all_views=True, website_id=1)
            .get_combined_arch()
        )
        self.assertEqual(
    ...
    def test_multi_website_view_obj_active(self):
        ...
        * A generic active child view, that is inactive on website 1
        ...
        self.assertEqual(
            inherit_view.active, True, "_view_obj should return the generic one"
        )
        inherit_view = View.with_context(website_id=1)._view_obj(self.inherit_view.key)
        self.assertEqual(
            inherit_view.active, False, "_view_obj should return the specific one"
        )
    ...
    def test_no_cow_on_translate(self):
        ...
        view.update_field_translations_sha(
            "arch_db", {french.code: {sha: "contenu de base"}}
        )
...
class Crawler(HttpCase):
    ...
    def test_multi_website_views_retrieving(self):
        ...
        with self.assertRaises(ValueError):
            ...
            view = View.with_context(website_id=website_1.id)._view_obj(
                "_theme_kea_sale.products"
            )
        view = View.with_context(website_id=website_2.id)._view_obj(
            "_theme_kea_sale.products"
        )
        self.assertEqual(
            len(view),
