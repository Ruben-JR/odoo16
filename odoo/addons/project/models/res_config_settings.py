...
class ResConfigSettings(models.TransientModel):
    ...
    group_project_milestone = fields.Boolean(
        "Milestones",
        implied_group="project.group_project_milestone",
    ...
    def set_values(self):
        ...
        project_task_dep_change_subtype_id = self.env.ref(
            "project.mt_project_task_dependency_change"
        )
        if task_dep_change_subtype_id.hidden != (
            not self["group_project_task_dependencies"]
        ):
            task_dep_change_subtype_id.hidden = not self[
                "group_project_task_dependencies"
            ]
            project_task_dep_change_subtype_id.hidden = not self[
                "group_project_task_dependencies"
            ]
