...
class AccountEdiFormat(models.Model):
    ...
    def _get_move_applicability(self, move):
        ...
        if self._is_ubl_cii_available(move.company_id) and move.move_type in (
            "out_invoice",
            "out_refund",
        ):
    ...
    def _create_invoice_from_xml_tree(self, filename, tree, journal=None):
        ...
        if not journal:
            ...
            if context_move_type in self.env["account.move"].get_sale_types():
                ...
                raise UserError(
                    _(
                        "The journal in which to upload should either be a sale or a purchase journal."
                    )
                )
        ...
        if (
            not self._is_ubl_cii_available(journal.company_id)
            and self.code != "facturx_1_0_05"
        ):
            return super()._create_invoice_from_xml_tree(
                filename, tree, journal=journal
            )
    ...
    def _update_invoice_from_xml_tree(self, filename, tree, invoice):
        ...
        if (
            not self._is_ubl_cii_available(invoice.company_id)
            and self.code != "facturx_1_0_05"
        ):
        ...
        if invoice_xml_builder is not None:
            invoice = invoice_xml_builder._import_invoice(
                invoice.journal_id, filename, tree, invoice
            )
            if invoice:
                return invoice
