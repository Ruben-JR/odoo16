...
class TestLeadConvert(crm_common.TestLeadConvertCommon):
    ...
    def test_duplicates_computation(self):
        ...
        lead_lost = dup_leads.filtered(
            lambda lead: lead.name == "Duplicate lead: same email_from, lost"
        )
        _opp_proba100 = dup_leads.filtered(
            lambda lead: lead.name
            == "Duplicate opp: same email_from, proba 100 but not won"
        )
        opp_won = dup_leads.filtered(
            lambda lead: lead.name == "Duplicate opp: same email_from, won"
        )
        ...
        self.assertEqual(
            result, test_lead + dup_leads - (lead_lost + opp_won + opp_lost)
        )

        # include_lost = remove archived opp only
        result = test_lead._get_lead_duplicates(
            partner=test_lead.partner_id, email=test_lead.email_from, include_lost=True
        )
    ...
    def test_lead_convert_batch_internals(self):
        ...
        for opp in self.lead_1 | lead_w_partner | lead_w_contact:
            ...
            if opp == self.lead_1:
                ...
                self.assertEqual(
                    opp.stage_id, self.stage_team1_1
