...
class OnboardingProgress(models.Model):
    ...
    progress_step_ids = fields.One2many(
        "onboarding.progress.step", "progress_id", "Progress Steps Trackers"
    )
    _sql_constraints = [
        (
            "onboarding_company_uniq",
            "unique (onboarding_id,company_id)",
    ...
    def _compute_onboarding_state(self):
        progress_steps_data = self.env["onboarding.progress.step"].read_group(
            [
                ("progress_id", "in", self.ids),
                ("step_state", "in", ["just_done", "done"]),
            ],
            ["progress_id"],
            ["progress_id"],
        )
        result = dict(
            (data["progress_id"][0], data["progress_id_count"])
            for data in progress_steps_data
        )
        for progress in self:
            progress.onboarding_state = (
                "not_done"
                if result.get(progress.id, 0) != len(progress.onboarding_id.step_ids)
                else "done"
            )
