...
class TestCreatePicking(common.TestProductCommon):
    ...
    def test_00_create_picking(self):
        ...
        self.assertEqual(
            self.po.state, "purchase", 'Purchase: PO state should be "Purchase'
        )
        self.assertEqual(
            self.po.incoming_picking_count, 1, "Purchase: one picking should be created"
        )
        self.assertEqual(
            len(self.po.order_line.move_ids), 1, "One move should be created"
        )
        # Change purchase order line product quantity
        self.po.order_line.write({"product_qty": 7.0})
        self.assertEqual(
            len(self.po.order_line.move_ids), 1, "The two moves should be merged in one"
        )
        ...
        self.assertEqual(
            self.po.order_line.mapped("qty_received"),
            [7.0],
        ...
        self.assertEqual(
            self.po.incoming_picking_count, 2, "New picking should be created"
        )
        moves = self.po.order_line.mapped("move_ids").filtered(
    ...
    def test_01_check_double_validation(self):
        ...
        self.po.button_confirm()
        self.assertEqual(
            self.po.state, "to approve", 'Purchase: PO state should be "to approve".'
        )
    ...
    def test_04_mto_multiple_po(self):
        ...
        self.assertEqual(
            sum(customer_picking.move_ids.mapped("reserved_availability")),
            100.0,
    ...
    def test_update_qty_purchased(self):
        ...
        self.assertEqual(
            ...
            "purchase",
            "Purchase order should be in purchase state.",
        )
