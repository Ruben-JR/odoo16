...
class AccountMove(models.Model):
    ...
    l10n_it_edi_attachment_id = fields.Many2one(
        "ir.attachment", copy=False, string="FatturaPA Attachment", ondelete="restrict"
    )
    ...
    def _compute_l10n_it_einvoice(self):
        ...
        for invoice in self:
            einvoice = invoice.edi_document_ids.filtered(
                lambda d: d.edi_format_id == fattura_pa
            ).sudo()
    ...
    def invoice_generate_xml(self):
        self.ensure_one()
        report_name = self.env[
            "account.edi.format"
        ]._l10n_it_edi_generate_electronic_invoice_filename(self)

        data = "<?xml version='1.0' encoding='UTF-8'?>" + str(
            self._l10n_it_edi_export_invoice_as_xml()
        )
        ...
        self.message_post(
            body=(
                _("E-Invoice is generated on %s by %s")
                % (fields.Datetime.now(), self.env.user.display_name)
            )
        )
    ...
    def _l10n_it_edi_prepare_fatturapa_line_details(
        ...
        lines = self.invoice_line_ids.filtered(
            lambda l: not l.display_type in ("line_note", "line_section")
        )
        for num, line in enumerate(lines):
            sign = -1 if line.move_id.is_inbound() else 1
            price_subtotal = (
                (line.balance * sign) if convert_to_euros else line.price_subtotal
            )
            ...
            if line.quantity and line.discount != 100.0:
                price_unit = price_subtotal / (
                    (1 - (line.discount or 0.0) / 100.0) * abs(line.quantity)
                )
            else:
            ...
            if not is_downpayment and line.price_subtotal < 0:
                ...
                if downpayment_moves:
                    downpayment_moves_description = ", ".join(
                        [m.name for m in downpayment_moves]
                    )
    ...
    def _l10n_it_edi_prepare_fatturapa_tax_details(
        self, tax_details, reverse_charge_refund=False
    ):
        ...
        for _tax_name, tax_dict in tax_details["tax_details"].items():
            ...
            if tax.price_include and tax.amount_type == "percent":
                if expected_base_amount and float_compare(
                    base_amount, expected_base_amount, 2
                ):
    ...
    def _l10n_it_edi_filter_fatturapa_tax_details(self, line, tax_values):
        ...
        return (
            repartition_line.factor_percent >= 0 and repartition_line.tax_id.amount >= 0
        )
    ...
    def _prepare_fatturapa_export_values(self):
        ...
        def format_alphanumeric(text_to_convert):
            return (
                text_to_convert.encode("latin-1", "replace").decode("latin-1")
                if text_to_convert
                else False
            )
        ...
        is_self_invoice = self.env["account.edi.format"]._l10n_it_edi_is_self_invoice(
            self
        )
        document_type = self.env["account.edi.format"]._l10n_it_get_document_type(self)
        if self.env["account.edi.format"]._l10n_it_is_simplified_document_type(
            document_type
        ):
        ...
        tax_details = self._prepare_edi_tax_details(
            filter_to_apply=self._l10n_it_edi_filter_fatturapa_tax_details
        )
        ...
        if is_self_invoice:
            document_total += sum(
                [
                    abs(v["tax_amount_currency"])
                    for k, v in tax_details["tax_details"].items()
                ]
            )
            if reverse_charge_refund:
        ...
        conversion_line = (
            self.invoice_line_ids.sorted(lambda l: abs(l.balance), reverse=True)[0]
        ...
        conversion_rate = (
            ...
            if convert_to_euros and conversion_line
            else None
        )

        invoice_lines = self._l10n_it_edi_prepare_fatturapa_line_details(
            reverse_charge_refund, is_downpayment, convert_to_euros
        )
        tax_lines = self._l10n_it_edi_prepare_fatturapa_tax_details(
            tax_details, reverse_charge_refund
        )

        # Reduce downpayment views to a single recordset
        downpayment_moves = [
            l.get("downpayment_moves", self.env["account.move"]) for l in invoice_lines
        ]
...
class AccountTax(models.Model):
    ...
    l10n_it_has_exoneration = fields.Boolean(
        string="Has exoneration of tax (Italy)", help="Tax has a tax exoneration."
    )
    ...
    def _check_exoneration_with_no_tax(self):
        for tax in self:
            if tax.l10n_it_has_exoneration:
                if (
                    not tax.l10n_it_kind_exoneration
                    or not tax.l10n_it_law_reference
                    or tax.amount != 0
                ):
                    raise ValidationError(
                        _(
                            "If the tax has exoneration, you must enter a kind of exoneration, a law reference and the amount of the tax must be 0.0."
                        )
                    )
                if (
                    tax.l10n_it_kind_exoneration == "N6"
                    and tax.l10n_it_vat_due_date == "S"
                ):
                    raise UserError(
                        _(
