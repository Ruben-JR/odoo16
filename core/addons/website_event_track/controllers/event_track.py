...
class EventTrackController(http.Controller):
    ...
    def _event_tracks_get_values(self, event, tag=None, **searches):
        ...
        if searches.get("search"):
            search_domain = expression.AND(
                [search_domain, [("name", "ilike", searches["search"])]]
            )
        ...
        if search_tags:
            ...
            search_domain = expression.AND([search_domain, *search_domain_items])

        # fetch data to display with TZ set for both event and tracks
        now_tz = utc.localize(
            fields.Datetime.now().replace(microsecond=0), is_dst=False
    ...
    def _prepare_calendar_values(self, event):
        ...
        track_time_slots = set().union(
            *(
                time_slot.keys()
                for time_slot in [
                    time_slots for time_slots in time_slots_by_tracks.values()
                ]
            )
        )
        ...
        tracks_by_rounded_times = dict(
            (time_slot, dict((location, {}) for location in locations))
    ...
    @http.route(
        "/event/track/toggle_reminder", type="json", auth="public", website=True
    )
    ...
    @http.route(
        ["""/event/track_tag/search_read"""], type="json", auth="public", website=True
    )
