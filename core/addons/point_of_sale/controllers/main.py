    ...
    def show_ticket_validation_screen(self, access_token="", **kwargs):
        ...
        if kwargs and request.httprequest.method == "POST":
            ...
            if not user_is_connected:
                error, error_message = self.details_form_validate(
                    kwargs, partner_creation=True
                )
            else:
                ...
                for field in self.MANDATORY_BILLING_FIELDS:
                    if not partner[field]:
                        ...
                        error_message.append(
                            _(
                                "The %s must be filled in your details.",
                                request.env["ir.model.fields"]
                                ._get("res.partner", field)
                                .field_description,
                            )
                        )
            # Check that the "optional" additional fields are filled.
            error, error_message = self.extra_details_form_validate(
                partner_values, additional_partner_fields, error, error_message
            )
            error, error_message = self.extra_details_form_validate(
                invoice_values, additional_invoice_fields, error, error_message
            )
            if not error:
    ...
    def _get_invoice(
        ...
        if request.env.user._is_public():
            partner_values.update(
                {key: kwargs[key] for key in self.MANDATORY_BILLING_FIELDS}
            )
            partner_values.update(
                {
                    key: kwargs[key]
            ...
            partner = (
                request.env["res.partner"].sudo().create(partner_values)
            )  # In this case, partner_values contains the whole partner info form.
        # If the user is connected, then we can update if needed its fields with the additional localized fields if any, then proceed.
            ...
            partner.write(
                partner_values
            )  # In this case, partner_values only contains the additional fields that can be updated.
