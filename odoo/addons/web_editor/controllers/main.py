...
class Web_Editor(http.Controller):
    ...
    def export_icon_to_png(
        ...
        size=100,
        alpha=255,
        font="/web/static/src/libs/fontawesome/fonts/fontawesome-webfont.ttf",
        width=None,
        height=None,
    ):
    ...
    def update_checklist(
        self, res_model, res_id, filename, checklistId, checked, **kwargs
    ):
        ...
        value = etree.tostring(htmlelem[0][0], encoding="utf-8", method="html")[
            5:-6
        ].decode("utf-8")
    ...
    def update_stars(self, res_model, res_id, filename, starsId, rating):
        ...
        for star in stars:
            ...
            if star_index < rating and (
                not "fa-star" in classname or "fa-star-o" in classname
            ):
    ...
    def _load_resources(
        ...
        for v in views:
            for asset_call_node in etree.fromstring(v["arch"]).xpath(
                ...
                attr = asset_call_node.get(
                    resources_type_info["t_call_assets_attribute"]
                )
                ...
                for file_info in request.env["ir.qweb"]._get_asset_content(asset_name)[
                    ...
                    if (
                        ...
                        or file_type == "scss"
                        and not only_user_custom_files
                    ):
    ...
    @http.route(
        ["/web_editor/media_library_search"], type="json", auth="user", website=True
    )
    def media_library_search(self, **params):
        ICP = request.env["ir.config_parameter"].sudo()
        endpoint = ICP.get_param(
            "web_editor.media_library_endpoint", DEFAULT_LIBRARY_ENDPOINT
        )
    ...
    @http.route(
        "/web_editor/save_library_media", type="json", auth="user", methods=["POST"]
    )
    def save_library_media(self, media):
        ...
        response = requests.post(
            "%s/media-library/1/download_urls" % library_endpoint, data=params
        )
        ...
        for id, url in response.json().items():
            ...
            context = {
                "binary_field_real_user": request.env["res.users"]
                .sudo()
                .browse([SUPERUSER_ID])
            }
            attachment = (
            ...
            if media[id]["is_dynamic_svg"]:
                ...
                attachment["url"] = "/web_editor/shape/illustration/%s?%s" % (
                    slug(attachment),
                    colorParams,
                )
