...
class PaymentTransaction(models.Model):
    ...
    callback_res_id = fields.Integer(
        string="Callback Record ID", groups="base.group_system"
    )
    ...
    def _compute_refunds_count(self):
        ...
        data = {
            x["source_transaction_id"][0]: x["source_transaction_id_count"]
            for x in rg_data
        }
        for record in self:
    ...
    def _check_state_authorized_supported(self):
        ...
        illegal_authorize_state_txs = self.filtered(
            ...
            and not tx.provider_id.support_manual_capture
        )
    ...
    def _generate_callback_hash(
        self, callback_model_id, callback_res_id, callback_method
    ):
        ...
        if callback_model_id and callback_res_id and callback_method:
            ...
            callback_hash = hmac_tool(
                self.env(su=True), "generate_callback_hash", token
            )
        ...
        return self.create(
            {
                ...
                "reference": self._compute_reference(
                    self.provider_code, prefix=f"R-{self.reference}"
                ),
    ...
    def _execute_callback(self):
        ...
        for tx in self.filtered(lambda t: not t.sudo().callback_is_done):
            ...
            valid_callback_hash = self._generate_callback_hash(
                model_sudo.id, res_id, method
            )
            if not consteq(ustr(valid_callback_hash), callback_hash):
                _logger.warning(
                    "invalid callback signature for transaction with reference %s",
                    tx.reference,
                )
            ...
            tx_sudo.callback_is_done = (
                success or success is None
            )  # Missing returns are successful.
    ...
    def _cron_finalize_post_processing(self):
        ...
        if not txs_to_post_process:
            ...
            client_handling_limit_date = datetime.now() - relativedelta.relativedelta(
                minutes=10
            )
