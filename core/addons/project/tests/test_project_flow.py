...
class TestProjectFlow(TestProjectCommon, MailCommon):
    ...
    def test_task_process_without_stage(self):
        ...
        task = self.format_and_process(
            EMAIL_TPL,
            to="project+pigs@mydomain.com, valid.lelitre@agrolait.com",
        ...
        self.assertEqual(
            len(task),
            1,
            "project: message_process: a new project.task should have been created",
        )
        ...
        self.assertEqual(
            task.message_ids.subtype_id,
            self.env.ref("project.mt_task_new"),
            "project: message_process: first message of new task should have Task Created subtype",
        )
        self.assertEqual(
            task.message_ids.author_id,
            self.user_projectuser.partner_id,
            "project: message_process: second message should be the one from Agrolait (partner failed)",
        )
        self.assertEqual(
            task.message_ids.subject,
            "Frogs",
            "project: message_process: second message should be the one from Agrolait (subject failed)",
        )
        # Test: task content
        self.assertEqual(
            task.name, "Frogs", "project_task: name should be the email subject"
        )
        self.assertEqual(
            task.project_id, self.project_pigs, "project_task: incorrect project"
        )
        self.assertEqual(
            task.stage_id.sequence,
            False,
            "project_task: shouldn't have a stage, i.e. sequence=False",
        )
    ...
    def test_task_process_with_stages(self):
        ...
        # Test: task content
        self.assertEqual(
            task.name, "Cats", "project_task: name should be the email subject"
        )
        self.assertEqual(
            task.project_id, self.project_goats, "project_task: incorrect project"
        )
        self.assertEqual(
            task.stage_id.sequence,
            1,
            "project_task: should have a stage with sequence=1",
        )
    ...
    def test_task_from_email_alias(self):
        ...
        # Test: task content
        self.assertEqual(
            task.name, "Super Frog", "project_task: name should be the email subject"
        )
        self.assertEqual(
            task.project_id, self.project_goats, "project_task: incorrect project"
        )
    ...
    def test_subtask_process(self):
        ...
        child_task_1 = Task.with_context(
            default_project_id=self.project_goats.id, default_parent_id=parent_task.id
        ).create(
            {
        ...
        child_task_1.write({"display_project_id": self.project_pigs.id})
        ...
        child_task_2.write({"display_project_id": self.project_goats.id})
    ...
    def test_rating(self):
        ...
        self.assertEqual(
            first_task.rating_count, 0, "Task should have no rating associated with it"
        )
