...
def _initialize_db(
    id, db_name, demo, lang, user_password, login="admin", country_code=None, phone=None
):
    try:
...
def exp_duplicate_database(db_original_name, db_name):
    ...
    with closing(db.cursor()) as cr:
        ...
        cr.execute(
            sql.SQL("CREATE DATABASE {} ENCODING 'unicode' TEMPLATE {}").format(
                sql.Identifier(db_name), sql.Identifier(db_original_name)
            )
        )
...
def exp_rename(old_name, new_name):
    ...
    with closing(db.cursor()) as cr:
        ...
        try:
            ...
            raise Exception(
                "Couldn't rename database %s to %s: %s" % (old_name, new_name, e)
            )
...
def exp_list_countries():
    ...
    root = ET.parse(
        os.path.join(
            odoo.tools.config["root_path"], "addons/base/data/res_country_data.xml"
        )
    ).getroot()
