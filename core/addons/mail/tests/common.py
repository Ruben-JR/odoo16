...
class MockEmail(common.BaseCase, MockSmtplibCase):
    ...
    def mock_mail_gateway(self, mail_unlink_sent=False):
        ...
        with self.mock_smtplib_connection(), patch.object(
            IrMailServer,
            "build_email",
            autospec=True,
            wraps=IrMailServer,
            side_effect=_ir_mail_server_build_email,
        ) as build_email_mocked, patch.object(
            IrMailServer,
            "send_email",
            autospec=True,
            wraps=IrMailServer,
            side_effect=send_email_origin,
        ) as send_email_mocked, patch.object(
            MailMail,
            "create",
            autospec=True,
            wraps=MailMail,
    ...
    def _init_mail_gateway(cls):
        ...
        cls.env["ir.config_parameter"].set_param(
            "mail.catchall.domain", cls.alias_domain
        )
        cls.env["ir.config_parameter"].set_param(
            "mail.catchall.alias", cls.alias_catchall
        )
        cls.env["ir.config_parameter"].set_param("mail.default.from", cls.default_from)
        cls.mailer_daemon_email = formataddr(
            ("MAILER-DAEMON", "%s@%s" % (cls.alias_bounce, cls.alias_domain))
        )

    @classmethod
    def _init_outgoing_gateway(cls):
        cls.env["ir.mail_server"].search([]).unlink()
        cls.mail_server_domain, cls.mail_server_global = cls.env[
            "ir.mail_server"
        ].create(
            [
                {
    ...
    def gateway_reply_wrecord(self, template, record, use_in_reply_to=True):
        ...
        return self.gateway_mail_reply_wrecord(
            template, record, use_in_reply_to=use_in_reply_to
        )
    ...
    def _find_mail_mail_wid(self, mail_id, status=None, mail_message=None, author=None):
        ...
        filtered = self._filter_mail(
            status=status, mail_message=mail_message, author=author
        )
    ...
    def _find_mail_mail_wpartners(
        self, recipients, status, mail_message=None, author=None
    ):
        ...
        for mail in filtered:
            ...
            raise AssertionError(
                "mail.mail not found for message %s / status %s / recipients %s / author %s"
                % (mail_message, status, recipients.ids, author)
            )
        return mail
    ...
    def _find_mail_mail_wemail(self, email_to, status, mail_message=None, author=None):
        ...
        for mail in filtered:
            ...
            raise AssertionError(
                "mail.mail not found for email_to %s / status %s in %s"
                % (email_to, status, repr([m.email_to for m in self._new_mails]))
            )
        return mail

    def _find_mail_mail_wrecord(
        self, record, status=None, mail_message=None, author=None
    ):
        ...
        for mail in filtered:
            ...
            raise AssertionError(
                "mail.mail not found for record %s in %s"
                % (record, repr([m.email_to for m in self._new_mails]))
            )
        return mail
    ...
    def _find_sent_email(self, email_from, emails_to, subject=None):
        ...
        if len(sent_emails) > 1 and subject:
            ...
            sent_email = next(
                (mail for mail in sent_emails if mail["subject"] == subject), False
            )
        else:
    ...
    def assertMailMail(
        ...
        found_mail = self._find_mail_mail_wpartners(
            recipients, status, mail_message=mail_message, author=author
        )
    ...
    def assertMailMailWEmails(
        ...
        for email_to in emails:
            found_mail = self._find_mail_mail_wemail(
                email_to, status, mail_message=mail_message, author=author
            )
            self.assertTrue(bool(found_mail))
            self._assertMailMail(
                found_mail,
    ...
    def assertMailMailWRecord(
        ...
        found_mail = self._find_mail_mail_wrecord(
            record, mail_message=mail_message, author=author
        )
        self.assertTrue(bool(found_mail))
        self._assertMailMail(
            found_mail,
    ...
    def assertMailMailWId(
        self, mail_id, status, author=None, content=None, fields_values=None
    ):
    ...
    def assertNoMail(self, recipients, mail_message=None, author=None):
        ...
        try:
            self._find_mail_mail_wpartners(
                recipients, False, mail_message=mail_message, author=author
            )
        except AssertionError:
            pass
        else:
            raise AssertionError(
                "mail.mail exists for message %s / recipients %s but should not exist"
                % (mail_message, recipients.ids)
            )
        finally:
    ...
    def assertSentEmail(self, author, recipients, **values):
        ...
        unknown = set(values.keys()) - set(
            direct_check + content_check + list_check + other_check
        )
        if unknown:
...
class MailCase(MockEmail):
    ...
    def mock_bus(self):
        ...
        with patch.object(
            ImBus, "create", autospec=True, wraps=ImBus, side_effect=_bus_bus_create
    ...
    def mock_mail_app(self):
        ...
        ) as _mail_message_create_mock, patch.object(
            MailNotification,
            "create",
            autospec=True,
        ...
        return [
            {
                ...
                "type": "user"
                if partner.user_ids and not partner.partner_share
                else partner.user_ids and "portal" or "customer",
    ...
    def assertPostNotifications(self, recipients_info, mail_unlink_sent=False):
        ...
        try:
            with self.mock_mail_gateway(
                mail_unlink_sent=mail_unlink_sent
            ), self.mock_bus(), self.mock_mail_app():
                yield
        finally:
    ...
    def assertMailNotifications(self, messages, recipients_info):
        ...
        for message_info in recipients_info:
            mbody, mtype = message_info.get("content", ""), message_info.get(
            ...
            if messages:
                message = messages.filtered(
                    lambda message: mbody in message.body
                    and message.message_type == mtype
                    and message.subtype_id == msubtype
                )
            else:
                message = (
            ...
            for recipient in message_info["notif"]:
                partner, ntype, ngroup, nstatus = (
                    recipient["partner"],
                    recipient["type"],
                    recipient.get("group"),
                    recipient.get("status", "sent"),
                )
                nis_read, ncheck_send = recipient.get(
                    "is_read", False if recipient["type"] == "inbox" else True
                ), recipient.get("check_send", True)
                if not ngroup:
                    ngroup = "user"
                ...
                partner_notif = notifications.filtered(
                    ...
                    and n.res_partner_id == partner
                    and n.notification_type == ntype
                    and n.notification_status == nstatus
                    and n.is_read == nis_read
                )
                self.assertTrue(
                    partner_notif,
                    "Mail: not found notification for %s (type: %s, state: %s, message: %s)"
                    % (partner, ntype, nstatus, message.id),
                )
                self.assertEqual(
                    partner_notif.author_id,
                    partner_notif.mail_message_id.author_id,
                    "Mail: Message and notification should have the same author",
                )
            ...
            if not any(p for recipients in email_groups.values() for p in recipients):
                self.assertNoMail(
                    partners, mail_message=message, author=message.author_id
                )
    ...
    def assertTracking(self, message, data):
        ...
        for field_name, value_type, old_value, new_value in data:
            tracking = tracking_values.filtered(
                lambda track: track.field.name == field_name
            )
            ...
            if value_type == "char":
                ...
                self.assertEqual(
                    tracking.old_value_char, old_value and old_value.display_name or ""
                )
                self.assertEqual(
                    tracking.new_value_char, new_value and new_value.display_name or ""
                )
...
class MailCommon(common.TransactionCase, MailCase):
    ...
    @classmethod
    def _create_records_for_batch(
        cls, model, count, additional_values=None, prefix=None
    ):
    ...
    def _activate_multi_lang(
        ...
        * template
        ...
        cls.addClassCleanup(
            code_translations.python_translations[("test_mail", "es_ES")].pop,
            "NotificationButtonTitle",
        )
        ...
        cls.addClassCleanup(
            code_translations.python_translations[("mail", "es_ES")].pop, "View %s"
        )
