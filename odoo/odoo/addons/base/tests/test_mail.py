...
class TestSanitizer(BaseCase):
    ...
    def test_html(self):
        ...
        for tag in [
            ...
            self.assertIn(
                tag, sanitized_html, "html_sanitize stripped too much of original html"
            )
   ...
   font-size: 30px; max-width: 100%; after: "Not sure
    ...
    def test_style_class(self):
        html = html_sanitize(
            test_mail_examples.REMOVE_CLASS,
    ...
    def test_edi_source(self):
        ...
        self.assertNotIn(
            "</body></html>", html, "html_sanitize did not remove extra closing tags"
        )
    ...
    def test_quote_thunderbird(self):
        ...
        for ext in test_mail_examples.QUOTE_THUNDERBIRD_1_OUT:
            self.assertIn(
                '<span data-o-mail-quote="1">%s</span>' % misc.html_escape(ext), html
            )
...
class TestHtmlTools(BaseCase):
    ...
    def test_prepend_html_content(self):
        ...
        self.assertEqual(
            result, "<html><body><span>content</span><div>test</div></body></html>"
        )
...
class TestEmailTools(BaseCase):
    ...
    def test_email_domain_normalize(self):
        self.assertEqual(
            ...
            "test.com",
            "Should have normalized the domain",
        )
        self.assertEqual(
            email_domain_normalize("email@test.com"),
            False,
            "The domain is not valid, should return False",
        )
        self.assertEqual(
            email_domain_normalize(False),
            False,
            "The domain is not valid, should return False",
        )
...
class _FakeSMTP:
    ...
    def sendmail(
        self, smtp_from, smtp_to_list, message_str, mail_options=(), rcpt_options=()
    ):
