...
def update_taxes_from_templates(cr, chart_template_xmlid):
    def _create_tax_from_template(company, template, old_tax=None):
        ...
        def _avoid_name_conflict():
            conflict_tax = env["account.tax"].search(
                [
                    ("name", "=", template.name),
        ...
        chart_template = env["account.chart.template"].with_context(
        ...
        chart_template.create_record_with_xmlid(
            company, template, "account.tax", template_vals
        )
    ...
    def _update_fiscal_positions_from_templates(
        company, chart_template_id, new_taxes_template
    ):
        ...
        fp_template_ref = _get_template_to_real_xmlid_mapping(
            company, "account.fiscal.position"
        )

        tax_template_vals = []
        for position_template in positions:
            fp = env["account.fiscal.position"].browse(
                fp_template_ref.get(position_template.id)
            )
            if not fp:
                continue
            for position_tax in position_template.tax_ids:
                ...
                dest_id = (
                    ...
                    and tax_template_ref[position_tax.tax_dest_id.id]
                    or False
                )
                ...
                if not position_tax_template_exist and (
                    position_tax.tax_src_id in new_taxes_template
                    or position_tax.tax_dest_id in new_taxes_template
                ):
    ...
    companies = env["res.company"].search(
        [("chart_template_id", "child_of", chart_template_id)]
    )
    outdated_taxes = []
    new_taxes_template = []
    for company in companies:
        ...
        templates = (
            ...
            .search([("chart_template_id", "=", chart_template_id)])
        )
