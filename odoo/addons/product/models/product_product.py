...
class ProductProduct(models.Model):
    ...
    combination_indices = fields.Char(
        compute="_compute_combination_indices", store=True, index=True
    )
    ...
    def _compute_concurrency_field(self):
        ...
        for record in self:
            record[self.CONCURRENCY_CHECK_FIELD] = max(
                filter(
                    None,
                    (
                        record.product_tmpl_id.write_date
                        or record.product_tmpl_id.create_date,
                        record.write_date
    ...
    def _check_barcode_uniqueness(self):
        ...
        if len(matched_products) > len(
            all_barcode
        ):  # It means that you find more than `self` -> there are duplicates
        ...
        for product in self:
            ...
            if self.env["ir.model.access"].check("product.supplierinfo", "read", False):
                for supplier_info in product.seller_ids:
                    if supplier_info.partner_id.id == product._context.get(
                        "partner_id"
                    ):
                        product.code = (
    ...
    def create(self, vals_list):
        ...
        products = super(
            ProductProduct, self.with_context(create_product_product=True)
        ).create(vals_list)
    ...
    def _search(
        ...
        if self._context.get("search_default_categ_id"):
            args.append(
                (("categ_id", "child_of", self._context["search_default_categ_id"]))
            )
    ...
    def toggle_active(self):
        ...
        tmpl_to_activate = self.filtered(
            lambda product: (
                not product.product_tmpl_id.active
