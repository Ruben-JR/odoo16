...
class PosSession(models.Model):
    ...
    def _pos_data_process(self, loaded_data):
        ...
        if self.config_id.module_pos_hr:
            loaded_data["employee_by_id"] = {
                employee["id"]: employee for employee in loaded_data["hr.employee"]
            }
    ...
    def _loader_params_hr_employee(self):
        if len(self.config_id.employee_ids) > 0:
            domain = [
                ...
                ("id", "in", self.config_id.employee_ids.ids),
            ]
        else:
    ...
    def _get_pos_ui_hr_employee(self, params):
        ...
        manager_ids = (
            ...
            .browse(user_ids)
            .filtered(
                lambda user: self.config_id.group_pos_manager_id in user.groups_id
            )
            .mapped("id")
        )

        employees_barcode_pin = (
            self.env["hr.employee"].browse(employee_ids).get_barcodes_and_pin_hashed()
        )
        bp_per_employee_id = {bp_e["id"]: bp_e for bp_e in employees_barcode_pin}
        for employee in employees:
            employee["role"] = (
                "manager"
                if employee["user_id"] and employee["user_id"] in manager_ids
                else "cashier"
            )
