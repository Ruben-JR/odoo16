...
class TestChannelInternals(MailCommon):
    ...
    def test_01_join_call(self):
        ...
        channel_member = channel.sudo().channel_member_ids.filtered(
            lambda channel_member: channel_member.partner_id
    ...
    def test_30_add_members_while_in_call_should_invite_new_members_to_call(self):
        ...
        with self.mock_bus():
            channel.add_members(
                partner_ids=test_user.partner_id.ids,
                guest_ids=test_guest.ids,
        ...
        found_bus_notifs = self.assertBusNotifications(
            [
                (
                    ...
                    "res.partner",
                    test_user.partner_id.id,
                ),  # channel joined (not asserted below)
                (
                    self.cr.dbname,
                    "mail.channel",
                    channel.id,
                ),  # message_post "invited" (not asserted below)
                (
                    self.cr.dbname,
                    "res.partner",
                    self.user_employee.partner_id.id,
                ...
                (
                    ...
                    "mail.channel",
                    channel.id,
                ),  # new members (not asserted below)
                (
                    self.cr.dbname,
                    "res.partner",
                    test_user.partner_id.id,
                ...
                (
                    ...
                    "mail.channel",
                    channel.id,
                ),  # new member (guest) (not asserted below)
                (
                    self.cr.dbname,
                    "mail.guest",
                    test_guest.id,
                ),  # channel joined for guest (not asserted below)
            ],
            message_items=[
                {
                    "type": "mail.thread/insert",
    ...
    def test_60_rtc_sync_sessions_should_gc_and_return_outdated_and_active_sessions(
        ...
        test_session = (
            self.env["mail.channel.rtc.session"]
            .sudo()
        ...
        with self.assertBus(
            ...
            [
                ...
                },
                {
                    ...
                    "payload": {
                        "id": channel.id,
                        "rtcSessions": [
                            ("insert-and-unlink", [{"id": test_session.id}])
                        ],
                    },
                },
            ],
            ...
            (
                ...
                check_rtc_session_ids=[join_call_values["sessionId"]] + unused_ids
            )
