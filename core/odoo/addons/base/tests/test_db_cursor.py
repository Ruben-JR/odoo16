...
class TestCursorHooksTransactionCaseCleanup(common.TransactionCase):
    ...
    def test_isolation_first(self):
        def mutate_second_test_ref():
            for name in ["precommit", "postcommit", "prerollback", "postrollback"]:
                del self.env.cr.precommit.data.get(
                    f"test_cursor_hooks_savepoint_case_cleanup_test_second_{name}", [""]
                )[0]
    ...
    def test_isolation_second(self):
        ...
        for callback_name, callbacks, reference in zip(
            callback_names, commit_callbacks, references
        ):
            callbacks.data.setdefault(
                f"test_cursor_hooks_savepoint_case_cleanup_test_second_{callback_name}",
                reference,
            )
        ...
        for callback_name, reference in zip(callback_names, references):
            self.assertTrue(
                bool(reference),
                f"{callback_name} failed to clean up between transaction tests",
            )
            self.assertTrue(
                reference[0] == "not_empty",
                f"{callback_name} failed to clean up between transaction tests",
            )
