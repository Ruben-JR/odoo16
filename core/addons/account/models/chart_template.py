...
def migrate_set_tags_and_taxes_updatable(cr, registry, module):
    ...
    if xml_record_ids:
        cr.execute(
            "update ir_model_data set noupdate = 'f' where id in %s",
            (tuple(xml_record_ids),),
        )
...
def preserve_existing_tags_on_taxes(cr, registry, module):
    ...
    if xml_records:
        cr.execute(
            "update ir_model_data set noupdate = 't' where id in %s",
            [tuple(xml_records.ids)],
        )
...
def update_taxes_from_templates(cr, chart_template_xmlid):
    ...
    def _create_taxes_from_template(company, template2tax_mapping):
        ...
        def _avoid_name_conflict(company, template):
            conflict_taxes = env["account.tax"].search(
                [
                    ...
                    ("tax_scope", "=", template.tax_scope),
                ]
            )
            if conflict_taxes:
                for index, conflict_taxes in enumerate(conflict_taxes):
                    conflict_taxes.name = (
                        f"[old{index if index > 0 else ''}] {conflict_taxes.name}"
                    )
        ...
        new_template2tax_company = templates_to_create._generate_tax(company)[
            "tax_template_to_tax"
        ]
        return [
            (template.id, tax.id) for template, tax in new_template2tax_company.items()
        ]
    ...
    def _update_taxes_from_template(template2tax_mapping):
        ...
        for template, existing_tax in template2tax_mapping:
            tax_rep_lines = (
                existing_tax.invoice_repartition_line_ids
                + existing_tax.refund_repartition_line_ids
            )
            template_rep_lines = (
    ...
    def _get_template_to_real_xmlid_mapping(model, templates):
        ...
        template_xmlids = [
            xmlid.split(".", 1)[1] for xmlid in templates.get_external_id().values()
        ]
    ...
    def _is_tax_and_template_same(template, tax):
        ...
        template_rep_lines = (
            template.invoice_repartition_line_ids + template.refund_repartition_line_ids
        )
        return (
            ...
            and (
                tax.amount_type
                == "group"  # if the amount_type is group we don't do checks on rep. lines
                or (
                    len(tax_rep_lines) == len(template_rep_lines)
                    and all(
                        rep_line_tax.factor_percent == rep_line_template.factor_percent
                        for rep_line_tax, rep_line_template in zip(
                            tax_rep_lines, template_rep_lines
                        )
                    )
                )
            )
    ...
    def _cleanup_tags(tags):
        ...
        for tag in tags:
            tax_using_tag = (
                ...
                .sudo()
                .search([("tag_ids", "in", tag.id)], limit=1)
            )
            aml_using_tag = (
    ...
    def _update_fiscal_positions_from_templates(
        chart_template, new_tax_template_by_company, all_tax_templates
    ):
        fp_templates = env["account.fiscal.position.template"].search(
            [("chart_template_id", "=", chart_template.id)]
        )
        template2tax = _get_template_to_real_xmlid_mapping(
            "account.tax", all_tax_templates
        )
        template2fp = _get_template_to_real_xmlid_mapping(
            "account.fiscal.position", fp_templates
        )
        ...
        for company_id in new_tax_template_by_company:
            ...
            for position_template in fp_templates:
                fp = (
                    env["account.fiscal.position"].browse(
                        template2fp_company.get(position_template.id)
                    )
                    if template2fp_company
                    else None
                )
                if not fp:
                    continue
                for position_tax in position_template.tax_ids:
                    src_id = template2tax.get(company_id).get(
                        position_tax.tax_src_id.id
                    )
                    dest_id = (
                        position_tax.tax_dest_id
                        and template2tax.get(company_id).get(
                            position_tax.tax_dest_id.id
                        )
                        or False
                    )
                    position_tax_template_exist = fp.tax_ids.filtered(
                        lambda tax_fp: tax_fp.tax_src_id.id == src_id
                        and tax_fp.tax_dest_id.id == dest_id
                    )
                    if not position_tax_template_exist and (
                        position_tax.tax_src_id
                        in new_tax_template_by_company[company_id]
                        or position_tax.tax_dest_id
                        in new_tax_template_by_company[company_id]
                    ):
                        fp_tax_template_vals.append(
                            (
                                position_tax,
                                {
                                    "tax_src_id": src_id,
            ...
            chart_template._create_records_with_xmlid(
    ...
    def _validate_taxes_country(chart_template, template2tax):
        ...
        for company_id in template2tax:
            ...
            for template_id in template2tax[company_id]:
                ...
                if (
                    not chart_template.country_id
                    or tax.country_id != chart_template.country_id
                ) and tax.country_id != company.account_fiscal_country_id:
                    raise ValidationError(
                        _(
                            ...
                            chart_template.country_id.name,
                        )
                    )
    ...
    templates = (
        ...
        .search([("chart_template_id", "=", chart_template.id)])
    )
    ...
    new_tax_template_by_company = defaultdict(
        env["account.tax.template"].browse
    )  # only contains completely new taxes (not previous taxe had the xmlid)
    ...
    for company in companies:
        ...
        for template in templates:
            tax = (
                env["account.tax"].browse(template2oldtax_company.get(template.id))
                if template2oldtax_company
                else None
            )
        ...
        new_template2tax += _create_taxes_from_template(
            company, templates_to_tax_create
        )
        _update_taxes_from_template(templates_to_tax_update)
    _update_fiscal_positions_from_templates(
        chart_template, new_tax_template_by_company, templates
    )
    if outdated_taxes:
...
class AccountGroupTemplate(models.Model):
    ...
    chart_template_id = fields.Many2one(
        "account.chart.template", string="Chart Template", required=True
    )
...
class AccountChartTemplate(models.Model):
    ...
    parent_id = fields.Many2one(
        "account.chart.template", string="Parent Chart Template"
    )
    ...
    use_storno_accounting = fields.Boolean(
        string="Use Storno accounting", default=False
    )
    ...
    bank_account_code_prefix = fields.Char(
        string="Prefix of the bank accounts", required=True
    )
    cash_account_code_prefix = fields.Char(
        string="Prefix of the main cash accounts", required=True
    )
    transfer_account_code_prefix = fields.Char(
        string="Prefix of the main transfer accounts", required=True
    )
    ...
    account_journal_suspense_account_id = fields.Many2one(
        "account.account.template", string="Journal Suspense Account"
    )
    account_journal_payment_debit_account_id = fields.Many2one(
        "account.account.template", string="Journal Outstanding Receipts Account"
    )
    account_journal_payment_credit_account_id = fields.Many2one(
        "account.account.template", string="Journal Outstanding Payments Account"
    )

    default_cash_difference_income_account_id = fields.Many2one(
        "account.account.template", string="Cash Difference Income Account"
    )
    default_cash_difference_expense_account_id = fields.Many2one(
        "account.account.template", string="Cash Difference Expense Account"
    )
    default_pos_receivable_account_id = fields.Many2one(
        "account.account.template", string="PoS receivable account"
    )
    ...
    property_account_receivable_id = fields.Many2one(
        "account.account.template", string="Receivable Account"
    )
    property_account_payable_id = fields.Many2one(
        "account.account.template", string="Payable Account"
    )
    property_account_expense_categ_id = fields.Many2one(
        "account.account.template", string="Category of Expense Account"
    )
    property_account_income_categ_id = fields.Many2one(
        "account.account.template", string="Category of Income Account"
    )
    property_account_expense_id = fields.Many2one(
        "account.account.template", string="Expense Account on Product Template"
    )
    property_account_income_id = fields.Many2one(
        "account.account.template", string="Income Account on Product Template"
    )
    property_stock_account_input_categ_id = fields.Many2one(
        "account.account.template", string="Input Account for Stock Valuation"
    )
    property_stock_account_output_categ_id = fields.Many2one(
        "account.account.template", string="Output Account for Stock Valuation"
    )
    property_stock_valuation_account_id = fields.Many2one(
        "account.account.template", string="Account Template for Stock Valuation"
    )
    property_tax_payable_account_id = fields.Many2one(
        "account.account.template", string="Tax current account (payable)"
    )
    property_tax_receivable_account_id = fields.Many2one(
        "account.account.template", string="Tax current account (receivable)"
    )
    property_advance_tax_payment_account_id = fields.Many2one(
        "account.account.template", string="Advance tax payment account"
    )
        ...
        return self.env["account.account"].create(
            {
                ...
                "code": self.env["account.account"]._search_new_account_code(
                    company, code_digits, company.bank_account_code_prefix or ""
                ),
                "account_type": "asset_current",
                "company_id": company.id,
            }
    ...
    def _load(self, company):
        ...
        if not company.account_journal_early_pay_discount_loss_account_id:
            company.account_journal_early_pay_discount_loss_account_id = (
                self._create_cash_discount_loss_account(company, self.code_digits)
            )
        if not company.account_journal_early_pay_discount_gain_account_id:
            company.account_journal_early_pay_discount_gain_account_id = (
                self._create_cash_discount_gain_account(company, self.code_digits)
            )
    ...
    def existing_accounting(self, company_id):
        ...
        for model in model_to_check:
            if (
                self.env[model]
                .sudo()
                .search([("company_id", "=", company_id.id)], order="id DESC", limit=1)
            ):
                return True
        if (
            ...
            .sudo()
            .search(
                [("company_id", "=", company_id.id), ("state", "!=", "draft")],
                order="id DESC",
                limit=1,
            )
        ):
    ...
    def generate_journals(self, acc_template_ref, company, journals_dict=None):
        ...
        for vals_journal in self._prepare_all_journals(
    ...
    def _install_template(
        self, company, code_digits=None, obj_wizard=None, acc_ref=None, taxes_ref=None
    ):
        ...
        if self.parent_id:
            tmp1, tmp2 = self.parent_id._install_template(
                company, code_digits=code_digits, acc_ref=acc_ref, taxes_ref=taxes_ref
            )
            acc_ref.update(tmp1)
            taxes_ref.update(tmp2)
        # Ensure, even if individually, that everything is translated according to the company's language.
        tmp1, tmp2 = self.with_context(lang=company.partner_id.lang)._load_template(
            company, code_digits=code_digits, account_ref=acc_ref, taxes_ref=taxes_ref
        )
        acc_ref.update(tmp1)
        taxes_ref.update(tmp2)
    ...
    def _load_template(
        ...
        generated_tax_res = self.with_context(
            active_test=False
        ).tax_template_ids._generate_tax(company)
        taxes_ref.update(generated_tax_res["tax_template_to_tax"])

        # Generating Accounts from templates.
        account_template_ref = self.generate_account(
        ...
        for tax, value in generated_tax_res["account_dict"]["account.tax"].items():
            if value["cash_basis_transition_account_id"]:
                tax.cash_basis_transition_account_id = account_ref.get(
                    value["cash_basis_transition_account_id"]
                )

        for repartition_line, value in generated_tax_res["account_dict"][
    ...
    def generate_account(
        ...
        acc_template = account_tmpl_obj.search(
            [("nocreate", "!=", True), ("chart_template_id", "=", self.id)], order="id"
        )
        template_vals = []
    ...
    def _prepare_reconcile_model_vals(
        self, company, account_reconcile_model, acc_template_ref, tax_template_ref
    ):
        """This method generates a dictionary of all the values for the account.reconcile.model that will be created."""
        self.ensure_one()
        account_reconcile_model_lines = self.env[
            "account.reconcile.model.line.template"
        ].search([("model_id", "=", account_reconcile_model.id)])
        return {
            ...
            "match_journal_ids": [
                (6, None, account_reconcile_model.match_journal_ids.ids)
            ],
            ...
            "match_partner_ids": [
                (6, None, account_reconcile_model.match_partner_ids.ids)
            ],
            "match_partner_category_ids": [
                (6, None, account_reconcile_model.match_partner_category_ids.ids)
            ],
            "line_ids": [
                (
                    0,
    ...
    def generate_account_reconcile_model(
        ...
        for account_reconcile_model in account_reconcile_models:
            vals = self._prepare_reconcile_model_vals(
                company, account_reconcile_model, acc_template_ref, tax_template_ref
            )
            self.create_record_with_xmlid(
                company, account_reconcile_model, "account.reconcile.model", vals
            )
    ...
    def generate_fiscal_position(self, tax_template_ref, acc_template_ref, company):
        ...
        fps = self._create_records_with_xmlid(
            "account.fiscal.position", template_vals, company
        )
                ...
                tax_template_vals.append(
                    (
                        tax,
                        {
                            ...
                            and tax_template_ref[tax.tax_dest_id].id
                            or False,
                            "position_id": fp.id,
                        },
                    )
        ...
        self._create_records_with_xmlid(
            "account.fiscal.position.account", account_template_vals, company
        )
...
class AccountTaxTemplate(models.Model):
    ...
    def _try_instantiating_foreign_taxes(self, country, company):
        ...
        def get_existing_tax_account(foreign_tax_rep_line, force_tax=None):
            ...
            search_domain = [
                ...
                "&",
                (
                    "invoice_tax_id.type_tax_use",
                    "=",
                    tax_rep_line.invoice_tax_id.type_tax_use,
                ),
                (
                    "invoice_tax_id.country_id",
                    "=",
                    company.account_fiscal_country_id.id,
                ),
                "&",
                (
                    "refund_tax_id.type_tax_use",
                    "=",
                    tax_rep_line.refund_tax_id.type_tax_use,
                ),
        ...
        templates_to_instantiate = (
            ...
            .with_context(active_test=False)
            .search([("chart_template_id.country_id", "=", country.id)])
        )
        default_company_taxes = (
            company.account_sale_tax_id + company.account_purchase_tax_id
        )
        rep_lines_accounts = templates_to_instantiate._generate_tax(company)[
            "account_dict"
        ]
        ...
        for tax_rep_line, account_dict in tax_rep_lines_accounts_dict.items():
            ...
            if not rep_account:
                existing_account = get_existing_tax_account(
                    tax_rep_line, force_tax=default_company_taxes
                )
                ...
                if existing_account:
                    rep_account = create_foreign_tax_account(
                        existing_account, _("Foreign tax account (%s)", country.code)
                    )
        ...
        for tax, account_dict in caba_transition_dict.items():
            transition_account_template = account_dict[
            ...
            if transition_account_template:
                ...
                if not transition_account:
                    ...
                    if tax_accounts:
                        transition_account = create_foreign_tax_account(
                            tax_accounts[0], _("Cash basis transition account")
                        )
        ...
        for property_field in group_property_fields:
            ...
            if default_acc:
                groups_company.write(
                    {
                        property_field: create_foreign_tax_account(
                            default_acc, _("Foreign account (%s)", country.code)
                        )
                    }
                )
    ...
    def _get_tax_vals(self, company, tax_template_to_tax):
        ...
        if self.invoice_repartition_line_ids:
            ...
            ] = self.invoice_repartition_line_ids.get_repartition_line_create_vals(
                company
            )
        if self.refund_repartition_line_ids:
            val[
                "refund_repartition_line_ids"
            ] = self.refund_repartition_line_ids.get_repartition_line_create_vals(
                company
            )
    ...
    def _get_tax_vals_complete(self, company):
        ...
        if self.cash_basis_transition_account_id.code:
            ...
            if cash_basis_account_id:
                vals.update(
                    {"cash_basis_transition_account_id": cash_basis_account_id.id}
                )

        vals.update(
            {
                "invoice_repartition_line_ids": self.invoice_repartition_line_ids._get_repartition_line_create_vals_complete(
                    company
                ),
                "refund_repartition_line_ids": self.refund_repartition_line_ids._get_repartition_line_create_vals_complete(
                    company
                ),
            }
        )
    ...
    def _generate_tax(self, company):
        ...
        ChartTemplate = self.env["account.chart.template"].with_context(
            default_company_id=company.id
        )
        ...
        while templates_todo:
            ...
            for template in templates:
                if all(
                    child in tax_template_to_tax for child in template.children_tax_ids
                ):
                    ...
                    if self.chart_template_id.country_id:
                        ...
                        raise UserError(
                            _(
                                "Please first define a fiscal country for company %s.",
                                company.name,
                            )
                        )
            ...
            taxes = ChartTemplate._create_records_with_xmlid(
            ...
            for tax, (template, vals) in zip(taxes, tax_template_vals):
                ...
                for i in range(0, len(all_template_rep_lines)):
                    ...
                    if template_account:
                        todo_dict["account.tax.repartition.line"][
                            all_tax_rep_lines[i]
                        ] = {
        ...
        return {"tax_template_to_tax": tax_template_to_tax, "account_dict": todo_dict}
...
class AccountTaxRepartitionLineTemplate(models.Model):
    ...
    plus_report_expression_ids = fields.Many2many(
    ...
    minus_report_expression_ids = fields.Many2many(
        ...
        for vals in vals_list:
            if vals.get("use_in_tax_closing") is None:
                ...
                if vals.get("account_id"):
                    account_type = (
                        self.env["account.account.template"]
                        .browse(vals.get("account_id"))
                        .account_type
                    )
                    if account_type:
                        vals["use_in_tax_closing"] = not (
                            account_type.startswith("income")
                            or account_type.startswith("expense")
                        )
    ...
    def validate_tax_template_link(self):
        for record in self:
            if record.invoice_tax_id and record.refund_tax_id:
                raise ValidationError(
                    _(
                        "Tax distribution line templates should apply to either invoices or refunds, not both at the same time. invoice_tax_id and refund_tax_id should not be set together."
                    )
                )
    ...
    def _validate_report_expressions(self):
        for record in self:
            all_engines = set(
                (
                    record.plus_report_expression_ids
                    + record.minus_report_expression_ids
                ).mapped("engine")
            )
    ...
    def _get_repartition_line_create_vals_complete(self, company):
        ...
        for idx, template_line in zip(
            range(1, len(rslt)), self
        ):  # ignore first ORM command ( (5, 0, 0) )
            account_id = False
            if template_line.account_id:
                ...
                if not account_id:
                    _logger.warning(
                        "The account with code '%s' was not found but is supposed to be linked to a tax",
    ...
    def _get_tags_to_add(self):
        ...
        domains = []
        for sign, report_expressions in (
            ("+", self.plus_report_expression_ids),
            ("-", self.minus_report_expression_ids),
        ):
            for report_expression in report_expressions:
                ...
                domains.append(
                    self.env["account.account.tag"]._get_tax_tags_domain(
                        report_expression.formula, country.id, sign=sign
                    )
                )
        ...
        if domains:
            tags_to_add |= (
                ...
                .with_context(active_test=False)
                .search(osv.expression.OR(domains))
            )
...
class AccountFiscalPositionTemplate(models.Model):
    ...
    tax_ids = fields.One2many(
        "account.fiscal.position.tax.template", "position_id", string="Tax Mapping"
    )
    note = fields.Text(string="Notes")
    auto_apply = fields.Boolean(
        string="Detect Automatically", help="Apply automatically this fiscal position."
    )
    vat_required = fields.Boolean(
        string="VAT required", help="Apply only if partner has a VAT number."
    )
    country_id = fields.Many2one(
        "res.country", string="Country", help="Apply only if delivery country matches."
    )
...
class AccountFiscalPositionAccountTemplate(models.Model):
    ...
    account_src_id = fields.Many2one(
        "account.account.template", string="Account Source", required=True
    )
...
class AccountReconcileModelLineTemplate(models.Model):
    ...
    tax_ids = fields.Many2many(
        "account.tax.template", string="Taxes", ondelete="restrict"
    )
