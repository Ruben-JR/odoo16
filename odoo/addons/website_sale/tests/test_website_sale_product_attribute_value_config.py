...
class TestWebsiteSaleProductAttributeValueConfig(
    AccountTestInvoicingCommon, TestSaleProductAttributeValueCommon
):
    @classmethod
    ...
    def test_get_combination_info(self):
        ...
        self.assertEqual(
            combination_info["list_price"], 2222 * discount_rate * currency_ratio
        )
        ...
        # CASE: B2C setting
        group_tax_included = self.env.ref(
            "account.group_show_line_subtotals_tax_included"
        ).with_context(active_test=False)
        group_tax_excluded = self.env.ref(
        ...
        self.assertEqual(
            combination_info["price_extra"], round(222 * currency_ratio * tax_ratio, 2)
        )
        self.assertEqual(combination_info["has_discounted_price"], False)
        ...
        self.assertEqual(
            combination_info["list_price"],
            pricelist.currency_id.round(2222 * currency_ratio * tax_ratio),
            0,
        )
        self.assertEqual(
            combination_info["price_extra"],
            pricelist.currency_id.round(222 * currency_ratio * tax_ratio),
            0,
        )
    ...
    def test_get_combination_info_with_fpos(self):
        ...
        self.assertEqual(
            combination_info["price_extra"],
            173.91,
            "173.91$ + 0% tax (mapped from fp 15% -> 0%)",
        )


@tagged("post_install", "-at_install", "product_pricelist")
class TestWebsiteSaleProductPricelist(
    AccountTestInvoicingCommon, TestSaleProductAttributeValueCommon
):
    def test_cart_update_with_fpos(self):
        ...
        tax6 = self.env["account.tax"].create(
            {
                "name": "Test tax 6",
                "amount": 6,
                "price_include": True,
        ...
        with MockRequest(self.env, website=current_website, sale_order_id=so.id):
            so._cart_update(
                product_id=test_product.product_variant_id.id, line_id=sol.id, set_qty=2
            )
        self.assertEqual(
