...
class WebsiteSaleProductConfiguratorController(ProductConfiguratorController):
    ...
    def show_advanced_configurator_website(self, product_id, variant_values, **kw):
        ...
        if (
            not force_dialog
            and not has_optional_products
            and (product.product_variant_count <= 1 or variant_values)
        ):
        ...
        return self.show_advanced_configurator(
            product_id, variant_values, request.website.pricelist_id, **kw
        )

    @http.route(
        ["/sale_product_configurator/optional_product_items_website"],
        type="json",
        auth="public",
        methods=["POST"],
        website=True,
    ...
    def optional_product_items_website(self, product_id, **kw):
        ...
        return self.optional_product_items(
            product_id, request.website.pricelist_id, **kw
        )


class WebsiteSale(main.WebsiteSale):
    def _prepare_product_values(self, product, category, search, **kwargs):
        values = super(WebsiteSale, self)._prepare_product_values(
            product, category, search, **kwargs
        )

        values["optional_product_ids"] = [
