...
class WebsiteSaleBackend(WebsiteBackend):
    ...
    def fetch_dashboard_data(self, website_id, date_from, date_to):
        ...
        results = super(WebsiteSaleBackend, self).fetch_dashboard_data(
        ...
        sales_values = dict(
            ...
            summary=dict(
                ...
                order_to_invoice_count=0,
                order_carts_abandoned_count=0,
                payment_to_capture_count=0,
                total_sold=0,
                order_per_day_ratio=0,
                order_sold_ratio=0,
                order_convertion_pctg=0,
            ),
        )

        results["dashboards"]["sales"] = sales_values

        results["groups"]["sale_salesman"] = request.env["res.users"].has_group(
        ...
        for product_line in report_product_lines:
            product_tmpl_id = request.env["product.template"].browse(
                product_line["product_tmpl_id"][0]
            )
        ...
        so_group_data = request.env["sale.order"].read_group(
            sale_order_domain, fields=["state"], groupby="state"
        )
            ...
            payment_to_capture_count=request.env["payment.transaction"].search_count(
                [
                    ...
                    (
                        ...
                        .search(sale_order_domain + [("state", "!=", "cancel")])
                        .ids,
                    ),
                ]
            ),
            total_sold=sum(
                price_line["price_subtotal"] for price_line in report_price_lines
            ),
        )
        ...
        sales_values["summary"]["order_sold_ratio"] = (
            round(
                float(sales_values["summary"]["total_sold"])
                / sales_values["summary"]["order_count"],
                2,
            )
            if sales_values["summary"]["order_count"]
            else 0
        )
        sales_values["summary"]["order_convertion_pctg"] = (
            100.0
            * sales_values["summary"]["order_count"]
            / sales_values["summary"]["order_carts_count"]
            if sales_values["summary"]["order_carts_count"]
            else 0
        )
        ...
        sales_values["graph"] += [
            {
                "values": self._compute_sale_graph(
                    date_date_from, date_date_to, sale_report_domain
                ),
                "key": "Untaxed Total",
            },
            {
                "values": self._compute_sale_graph(
                    date_date_from - timedelta(days=date_diff_days),
                    date_date_from,
                    sale_report_domain,
                    previous=True,
                ),
    ...
    def fetch_utm_data(self, date_from, date_to):
        ...
        return {
            "campaign_id": self.compute_utm_graph_data(
                "campaign_id", orders_data_groupby_campaign_id
            ),
            "medium_id": self.compute_utm_graph_data(
                "medium_id", orders_data_groupby_medium_id
            ),
            "source_id": self.compute_utm_graph_data(
                "source_id", orders_data_groupby_source_id
            ),
        }

    def compute_utm_graph_data(self, utm_type, utm_graph_data):
        return [
            {"utm_type": data[utm_type][1], "amount_total": data["amount_total"]}
    ...
    def _compute_sale_graph(self, date_from, date_to, sales_domain, previous=False):
        ...
        date_list = [
            (date_from + timedelta(days=x)) for x in range(0, days_between + 1)
        ]

        daily_sales = request.env["sale.report"].read_group(
            domain=sales_domain, fields=["date", "price_subtotal"], groupby="date:day"
        )
        ...
        sales_graph = [
            {
                "0": fields.Date.to_string(d)
                if not previous
                else fields.Date.to_string(d + timedelta(days=days_between)),
                # Respect read_group format in models.py
                "1": daily_sales_dict.get(
                    babel.dates.format_date(
                        d, format="dd MMM yyyy", locale=get_lang(request.env).code
                    ),
                    0,
                ),
            }
