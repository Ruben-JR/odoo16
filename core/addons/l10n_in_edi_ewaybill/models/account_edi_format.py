...
class AccountEdiFormat(models.Model):
    ...
    def _l10n_in_edi_ewaybill_base_irn_or_direct(self, move):
        ...
        if move.move_type == "out_refund":
            return "direct"
        einvoice_in_edi_format = move.journal_id.edi_format_ids.filtered(
            lambda f: f.code == "in_einvoice_1_03"
        )
        return (
            einvoice_in_edi_format
            and einvoice_in_edi_format._get_move_applicability(move)
            and "irn"
            or "direct"
        )
    ...
    def _check_move_configuration(self, move):
        ...
        if not move.l10n_in_mode:
            ...
            error_message.append(
                _("- Transporter is required when E-waybill is managed by transporter")
            )
        elif (
            move.l10n_in_mode == "0"
            and move.l10n_in_transporter_id
            and not move.l10n_in_transporter_id.vat
        ):
            ...
            if (
                not move.l10n_in_transportation_doc_no
                and move.l10n_in_transportation_doc_date
            ):
                error_message.append(
                    _(
                        "- Transport document number and date is required when Transportation Mode is Rail,Air or Ship"
                    )
                )
        if error_message:
        ...
        if not re.match("^.{1,16}$", is_purchase and move.ref or move.name):
            error_message.append(
                _(
                    "%s number should be set and not more than 16 characters",
        ...
        for line in move.invoice_line_ids.filtered(
            lambda line: not (
                line.display_type in ("line_section", "line_note", "rounding")
    ...
    def _l10n_in_edi_ewaybill_cancel_invoice(self, invoices):
        ...
        cancel_json = {
            "ewbNo": ewaybill_response_json.get("ewayBillNo")
            or ewaybill_response_json.get("EwbNo"),
        ...
        if response.get("error"):
            ...
            if "238" in error_codes:
                ...
                authenticate_response = self._l10n_in_edi_ewaybill_authenticate(
                    invoices.company_id
                )
                if not authenticate_response.get("error"):
                    error = []
                    response = self._l10n_in_edi_ewaybill_cancel(
                        invoices.company_id, cancel_json
                    )
                    if response.get("error"):
                        error = response["error"]
                        error_codes = [e.get("code") for e in error]
            if "312" in error_codes:
                ...
                error_message = "<br/>".join(
                    [
                        ...
                        % (
                            ...
                            html_escape(
                                e.get("message")
                                or self._l10n_in_edi_ewaybill_get_error_message(
                                    e.get("code")
                                )
                            ),
                        )
                ...
                invoices.message_post(
                    ...
                    % (
                        _(
                            "Somehow this E-waybill has been canceled in the government portal before. You can verify by checking the details into the government (https://ewaybillgst.gov.in/Others/EBPrintnew.asp)"
                        ),
                        _("Error"),
                        error_message,
                    ),
    ...
    def _l10n_in_edi_ewaybill_irn_post_invoice_edi(self, invoices):
        ...
        response = self._l10n_in_edi_irn_ewaybill_generate(
            invoices.company_id, generate_json
        )
        if response.get("error"):
            error = response["error"]
            error_codes = [e.get("code") for e in error]
            ...
            if "4002" in error_codes or "4026" in error_codes:
                ...
                response = self._l10n_in_edi_irn_ewaybill_get(
                    invoices.company_id, generate_json.get("Irn")
                )
                if not response.get("error"):
                    error = []
                    odoobot = self.env.ref("base.partner_root")
                    invoices.message_post(
                        author_id=odoobot.id,
                        body=_(
                            "Somehow this E-waybill has been generated in the government portal before. You can verify by checking the invoice details into the government (https://ewaybillgst.gov.in/Others/EBPrintnew.asp)"
                        ),
                    )
    ...
    def _l10n_in_edi_ewaybill_post_invoice_edi(self, invoices):
        ...
        response = self._l10n_in_edi_ewaybill_generate(
            invoices.company_id, generate_json
        )
        if response.get("error"):
            error = response["error"]
            ...
            if "604" in error_codes:
                ...
                response = self._l10n_in_edi_ewaybill_get_by_consigner(
                    invoices.company_id,
                    generate_json.get("docType"),
                    generate_json.get("docNo"),
                )
                if not response.get("error"):
                    error = []
                    odoobot = self.env.ref("base.partner_root")
    ...
    def _get_l10n_in_edi_saler_buyer_party(self, move):
        ...
        if move.is_purchase_document(include_receipts=True):
            res = {
                ...
                "ship_to_details": move._l10n_in_get_warehouse_address()
    ...
    def _l10n_in_edi_ewaybill_generate_json(self, invoices):
        ...
        json_payload = {
            "supplyType": invoices.is_purchase_document(include_receipts=True)
            and "I"
            or "O",
        ...
        is_overseas = invoices.l10n_in_gst_treatment in (
            "overseas",
            ...
            json_payload.update(
                {
                    ...
                    "transDocDate": invoices.l10n_in_transportation_doc_date
                    and invoices.l10n_in_transportation_doc_date.strftime("%d/%m/%Y")
                    or "",
                }
            )
    ...
    def _get_l10n_in_edi_ewaybill_line_details(self, line, line_tax_details, sign):
        ...
        line_details = {
            ...
            "qtyUnit": line.product_id.uom_id.l10n_in_code
            and line.product_id.uom_id.l10n_in_code.split("-")[0]
            or "OTH",
        ...
        if tax_details_by_code.get("igst_rate"):
            line_details.update(
                {
                    "igstRate": self._l10n_in_round_value(
                        tax_details_by_code["igst_rate"]
                    )
                }
            )
        else:
            line_details.update(
                {
                    "cgstRate": self._l10n_in_round_value(
                        tax_details_by_code.get("cgst_rate", 0.00)
                    ),
                    "sgstRate": self._l10n_in_round_value(
        ...
        if tax_details_by_code.get("cess_rate"):
            line_details.update(
                {
                    "cessRate": self._l10n_in_round_value(
                        tax_details_by_code.get("cess_rate")
                    )
                }
            )
        return line_details
    ...
    def _l10n_in_edi_irn_ewaybill_get(self, company, irn):
        ...
        return self._l10n_in_edi_connect_to_server(
            company, url_path="/iap/l10n_in_edi/1/get_ewaybill_by_irn", params=params
        )
    ...
    def _l10n_in_edi_ewaybill_check_authentication(self, company):
        ...
        if (
            sudo_company.l10n_in_edi_ewaybill_username
            and sudo_company._l10n_in_edi_ewaybill_token_is_valid()
        ):
            return True
        elif (
            sudo_company.l10n_in_edi_ewaybill_username
            and sudo_company.l10n_in_edi_ewaybill_password
        ):
    ...
    def _l10n_in_edi_ewaybill_connect_to_server(self, company, url_path, params):
        ...
        endpoint = (
            ...
            .sudo()
            .get_param("l10n_in_edi_ewaybill.endpoint", default_endpoint)
        )
    ...
    def _l10n_in_edi_ewaybill_authenticate(self, company):
        ...
        if response and response.get("status_cd") == "1":
            company.sudo().l10n_in_edi_ewaybill_auth_validity = (
    ...
    @api.model
    def _l10n_in_edi_ewaybill_get_by_consigner(
        self, company, document_type, document_number
    ):
        is_authenticated = self._l10n_in_edi_ewaybill_check_authentication(company)
        if not is_authenticated:
            return self._l10n_in_edi_ewaybill_no_config_response()
