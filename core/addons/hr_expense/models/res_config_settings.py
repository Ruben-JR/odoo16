...
class ResConfigSettings(models.TransientModel):
    ...
    use_mailgateway = fields.Boolean(
    ...
    expense_journal_id = fields.Many2one(
        "account.journal", related="company_id.expense_journal_id", readonly=False
    )
    ...
    def get_values(self):
        ...
        res.update(
            expense_alias_prefix=self.env.ref(
                "hr_expense.mail_alias_expense"
            ).alias_name,
        )
        return res
    ...
    def _compute_expense_alias_prefix(self):
        self.filtered(lambda w: not w.use_mailgateway).update(
