...
class WebsiteForum(WebsiteProfile):
    ...
    def _prepare_user_values(self, **kwargs):
        ...
        values["forum_welcome_message"] = request.httprequest.cookies.get(
    ...
    def forum_faq(self, forum, **post):
        values = self._prepare_user_values(
            forum=forum, searches=dict(), header={"is_guidelines": True}, **post
        )
    ...
    @http.route(
        "/forum/get_url_title", type="json", auth="user", methods=["POST"], website=True
    )
    ...
    @http.route(
        [
            """/forum/<model("forum.forum"):forum>/<model("forum.post", "[('forum_id','=',forum.id),('parent_id','=',False),('can_view', '=', True)]"):question>"""
        ],
        type="http",
        auth="public",
        website=True,
    ...
    def question(self, forum, question, **post):
        ...
        if (request.httprequest.referrer or "").startswith(
    ...
    def forum_post(self, forum, **post):
        ...
        values = self._prepare_user_values(
            forum=forum, searches={}, header={"ask_hide": True}, new_question=True
        )
    ...
    def post_create(self, forum, post_parent=None, **post):
        if post.get("content", "") == "<p><br></p>":
            return request.render(
                "http_routing.http_error",
                {
                    ...
                    "status_message": post_parent
                    and _("Reply should not be empty.")
        ...
        return request.redirect(
            "/forum/%s/%s"
            % (slug(forum), post_parent and slug(post_parent) or new_question.id)
        )
