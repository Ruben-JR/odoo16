...
class Warehouse(models.Model):
    ...
    wh_input_stock_loc_id = fields.Many2one(
        "stock.location", "Input Location", check_company=True
    )
    wh_qc_stock_loc_id = fields.Many2one(
        "stock.location", "Quality Control Location", check_company=True
    )
    wh_output_stock_loc_id = fields.Many2one(
        "stock.location", "Output Location", check_company=True
    )
    ...
    int_type_id = fields.Many2one(
        "stock.picking.type", "Internal Type", check_company=True
    )
    return_type_id = fields.Many2one(
        "stock.picking.type", "Return Type", check_company=True
    )
    crossdock_route_id = fields.Many2one(
        "stock.route", "Crossdock Route", ondelete="restrict"
    )
    reception_route_id = fields.Many2one(
        "stock.route", "Receipt Route", ondelete="restrict"
    )
    delivery_route_id = fields.Many2one(
        "stock.route", "Delivery Route", ondelete="restrict"
    )
            ...
            if "active" in vals:
                ...
                if move_ids:
                    raise UserError(
                        ...
                        % (
                            ", ".join(move_ids.mapped("picking_type_id.name")),
                            warehouse.name,
                        )
                    )
                else:
                ...
                if picking_type_using_locations:
                    raise UserError(
                        _(
                            "%s use default source or destination locations from warehouse %s that will be archived."
                        )
                        % (
                            ", ".join(picking_type_using_locations.mapped("name")),
                            warehouse.name,
                        )
                    )
    ...
    def _check_multiwarehouse_group(self):
        cnt_by_company = (
            ...
            .sudo()
            ._read_group(
                [("active", "=", True)], ["company_id"], groupby=["company_id"]
            )
        )
        if cnt_by_company:
    ...
    def _create_or_update_sequences_and_picking_types(self):
        ...
        if "out_type_id" in warehouse_data:
            PickingType.browse(warehouse_data["out_type_id"]).write(
                {"return_picking_type_id": warehouse_data.get("return_type_id", False)}
            )
        ...
        return {
            ...
            "crossdock_route_id": {
                ...
                "route_create_values": {
                    ...
                    and self.reception_steps != "one_step",
                    "company_id": self.company_id.id,
                    "sequence": 20,
                },
                "rules_values": {"active": True, "procure_method": "make_to_order"},
            },
        }
    ...
    def create_resupply_routes(self, supplier_warehouses):
        ...
        for supplier_wh in supplier_warehouses:
            ...
            pull_rules_list += self._get_supply_pull_rules_values(
                [
                    self.Routing(
                        transit_location, input_location, self.in_type_id, "pull"
                    )
                ],
                values={
    ...
    def _get_transit_locations(self):
        return (
            ...
            self.env.ref("stock.stock_location_inter_wh", raise_if_not_found=False)
    ...
    def _update_reception_delivery_resupply(self, reception_new, delivery_new):
        ...
        for warehouse in self:
            input_loc, output_loc = warehouse._get_input_output_locations(
                reception_new, delivery_new
            )
            if (
                reception_new
                and warehouse.reception_steps != reception_new
                and (
                    warehouse.reception_steps == "one_step"
                    or reception_new == "one_step"
                )
            ):
    ...
    def _get_picking_type_create_values(self, max_sequence):
        ...
        return {
            ...
            "int_type_id": {
                ...
                "active": self.reception_steps != "one_step"
                or self.delivery_steps != "ship_only"
                or self.user_has_groups("stock.group_stock_multi_locations"),
