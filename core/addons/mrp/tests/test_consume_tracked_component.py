...
from odoo.addons.mrp.tests.common_consume_tracked_component import (
    TestConsumeTrackedComponentCommon,
)
...
class TestConsumeTrackedComponent(TestConsumeTrackedComponentCommon):
    def test_option_disabled_and_qty_available(self):
        ...
        for mov in mo_all.move_raw_ids:
            if (
                mov.has_tracking == "none"
                or mov.raw_material_production_id.state == "done"
            ):
                self.assertEqual(
                    mov.product_qty,
                    mov.quantity_done,
                ...
                self.assertEqual(
                    0, mov.quantity_done, "Done quantity shall be equal to 0."
                )
    ...
    def test_option_enabled_and_qty_not_available(self):
        ...
        for mov in mo_all.move_raw_ids:
            self.assertEqual(
                0, mov.reserved_availability, "Reserved quantity shall be equal to 0."
            )
            ...
            for mov in mo.move_raw_ids:
                if mov.has_tracking == "none":
                    ...
                    self.assertEqual(
                        raw_tracked_qty,
                        mov.quantity_done,
                        "Done quantity shall be equal to " + str(raw_tracked_qty) + ".",
                    )
            mo.action_cancel()
