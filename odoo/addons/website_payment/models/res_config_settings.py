...
class ResConfigSettings(models.TransientModel):
    ...
    module_payment_paypal = fields.Boolean(string="Paypal - Express Checkout")
    ...
    def _compute_providers_state(self):
        ...
        stripe = self.env.ref(
            "payment.payment_provider_stripe", raise_if_not_found=False
        )
        for config in self:
            providers = config._get_activated_providers()
            first_provider = (
                stripe
                if stripe and stripe in providers
                else providers[0]
                if providers
                else providers
            )
