...
class WebsiteSlides(WebsiteProfile):
    ...
    def _get_slide_detail(self, slide):
        ...
        if slide.channel_id.channel_type == "documentation":
            ...
            related_domain = expression.AND(
                [base_domain, [("category_id", "=", slide.category_id.id)]]
            )
    ...
    def _slugify_tags(self, tag_ids, toggle_tag_id=None):
        ...
        return ",".join(
            slug(tag) for tag in request.env["slide.channel.tag"].browse(tag_ids)
        )
    ...
    def _channel_search_tags_slug(self, search_tags):
        ...
        try:
            tag_ids = list(
                filter(None, [unslug(tag)[1] for tag in (search_tags or "").split(",")])
            )
        except Exception:
            return ChannelTag
        # perform a search to filter on existing / valid tags implicitly
    ...
    def slides_channel_home(self, **post):
        ...
        if not request.env.user._is_public():
            ...
            channels_my = tools.lazy(
                lambda: channels_all.filtered(lambda channel: channel.is_member).sorted(
        ...
        channels_popular = tools.lazy(
            lambda: channels_all.sorted("total_votes", reverse=True)[:3]
        )
        channels_newest = tools.lazy(
            lambda: channels_all.sorted("create_date", reverse=True)[:3]
        )
    ...
    def slides_channel_all_values(
        ...
        _, details, fuzzy_search_term = request.website._search_with_fuzzy(
    ...
    def channel(
        ...
        **kw,
        ...
        pager = request.website.pager(
            ...
            url_args=pager_args,
            scope=page_count
            if page_count < self._pager_max_pages
            else self._pager_max_pages,
        )
    ...
    def slide_channel_tag_group_search_read(self, fields, domain):
        can_create = request.env["slide.channel.tag.group"].check_access_rights(
            "create", raise_exception=False
        )
        return {
    ...
    @http.route(
        ["/slides/channel/send_share_email"], type="json", auth="user", website=True
    )
    ...
    def slide_channel_subscribe(self, channel_id):
        return (
            ...
            .browse(channel_id)
            .message_subscribe(partner_ids=[request.env.user.partner_id.id])
        )
    ...
    def slide_shared_view(self, slide_id, **kwargs):
        ...
        if status == "authorized":
            return request.redirect(
                "%s?%s"
                % (
                    user_slide_authorization["slide"].website_url,
                    werkzeug.urls.url_encode(kwargs),
                )
            )
    ...
    def slide_get_pdf_content(self, slide):
        response = Response()
        response.data = (
            slide.binary_content and base64.b64decode(slide.binary_content) or b""
        )
    ...
    @http.route(
        "/slides/slide/get_html_content", type="json", auth="public", website=True
    )
    def get_html_content(self, slide_id):
        fetch_res = self._fetch_slide(slide_id)
        if fetch_res.get("error"):
            return fetch_res
        return {"html_content": fetch_res["slide"].html_content}
    ...
    def slide_set_completed_and_redirect(self, slide, next_slide_id=None):
        ...
        return request.redirect(
            "/slides/slide/%s" % (slug(next_slide) if next_slide else slug(slide))
        )
    ...
    @http.route(
        "/slides/slide/set_uncompleted", website=True, type="json", auth="public"
    )
    ...
    @http.route(
        "/slides/slide/toggle_is_preview", type="json", auth="user", website=True
    )
    ...
    @http.route(
        ["/slides/slide/send_share_email"], type="json", auth="user", website=True
    )
    ...
    @http.route(
        ["/slides/slide/quiz/save_to_session"], type="json", auth="public", website=True
    )
    ...
    def slide_category_search_read(self, fields, domain):
        ...
        can_create = request.env["slide.slide"].check_access_rights(
            "create", raise_exception=False
        )
        return {
    ...
    def prepare_preview(self, channel_id, slide_category, url=None):
        ...
        if slide_category == "video":
            ...
            if slide.video_source_type == "youtube":
                ...
                identical_video = existing_videos.filtered(
                    lambda existing_video: slide.google_drive_id
                    == existing_video.google_drive_id
                )
    ...
    def slide_tag_search_read(self, fields, domain):
        can_create = request.env["slide.tag"].check_access_rights(
            "create", raise_exception=False
        )
        return {
    ...
    def _prepare_user_profile_values(self, user, **post):
        ...
        if post.get("channel_id"):
            values.update(
                {"edit_button_url_param": "channel_id=" + str(post["channel_id"])}
            )
