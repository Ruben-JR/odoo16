...
class AccountFiscalPosition(models.Model):
    ...
    l10n_ar_afip_responsibility_type_ids = fields.Many2many(
        ...
        help="List of AFIP responsibilities where this fiscal position "
    ...
    def _get_fiscal_position(self, partner, delivery=None):
        ...
        domain = [
            ...
            (
                ...
                "=",
                partner.l10n_ar_afip_responsibility_type_id.id,
            ),
    ...
    def _onchange_afip_responsibility(self):
        if self.company_id.account_fiscal_country_id.code == "AR":
            if self.l10n_ar_afip_responsibility_type_ids and any(
                ["country_group_id", "country_id", "zip_from", "zip_to"]
            ):
                return {
                    "warning": {
                        "title": _("Warning"),
                        "message": _(
                            "If use AFIP Responsibility then the country / zip codes will be not take into account"
                        ),
                    }
                }
