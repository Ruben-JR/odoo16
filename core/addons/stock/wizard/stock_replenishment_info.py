...
class StockReplenishmentInfo(models.TransientModel):
    ...
    json_replenishment_history = fields.Char(
        compute="_compute_json_replenishment_history"
    )
    ...
    def _compute_json_lead_days(self):
        ...
        for replenishment_report in self:
            if (
                not replenishment_report.orderpoint_id.product_id
            ...
            replenishment_report.json_lead_days = dumps(
                {
                    "lead_days_date": format_date(
                        self.env, replenishment_report.orderpoint_id.lead_days_date
                    ),
