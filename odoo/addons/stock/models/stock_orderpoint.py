...
class StockWarehouseOrderpoint(models.Model):
    ...
    visibility_days = fields.Float(
        compute="_compute_visibility_days",
        inverse="_set_visibility_days",
        readonly=False,
    ...
    def _check_product_uom(self):
        ...
        if any(
            orderpoint.product_id.uom_id.category_id
            != orderpoint.product_uom.category_id
            for orderpoint in self
        ):
            raise ValidationError(
                _(
                    "You have to select a product unit of measure that is in the same category as the default unit of measure of the product"
                )
            )
    ...
    def action_stock_replenishment_info(self):
        ...
        action["name"] = _(
            "Replenishment Information for %s in %s",
        ...
        for orderpoint in self:
            ...
            if (
                ...
                qty_forecast_with_visibility = (
                    orderpoint.product_id.with_context(product_context).read(
                        ["virtual_available"]
                    )[0]["virtual_available"]
                ...
                remainder = (
                    ...
                    and qty_to_order % orderpoint.qty_multiple
                    or 0.0
                )
    ...
    def _set_default_route_id(self):
        ...
        for g in rules_groups:
            ...
            orderpoints = self.filtered(
                lambda o: o.location_id.id == g["location_dest_id"][0]
            )
    ...
    def _get_product_context(self, visibility_days=0):
        ...
        return {
            ...
            "to_date": datetime.combine(
                self.lead_days_date + relativedelta.relativedelta(days=visibility_days),
                time.max,
            ),
        }
    ...
    def _get_orderpoint_action(self):
        ...
        all_replenish_location_ids = self.env["stock.location"].search(
        ...
        for products in map(
            self.env["product.product"].browse, split_every(5000, all_product_ids)
        ):
            for loc in all_replenish_location_ids:
                quantities = products.with_context(location=loc.id).mapped(
                    "virtual_available"
                )
                for product, quantity in zip(products, quantities):
                    if (
                        float_compare(
                            quantity, 0, precision_rounding=product.uom_id.rounding
                        )
                        >= 0
                    ):
                        continue
        ...
        qty_by_product_loc, dummy = (
            self.env["product.product"]
            .browse(product_ids)
            ._get_quantity_in_progress(location_ids=location_ids)
        )
        ...
        orderpoint_by_product_location = {
            (record.get("product_id")[0], record.get("location_id")[0]): record.get(
                "qty_to_order"
            )
        ...
        for (product, location_id), product_qty in to_refill.items():
            ...
            if orderpoint_id:
                ...
                warehouse_id = (
                    self.env["stock.location"].browse(location_id).warehouse_id
                )
    ...
    def _prepare_procurement_values(self, date=False, group=False):
        ...
        date_planned = self.product_id._get_date_with_security_lead_days(
            date_planned, self.location_id
        )
        return {
    ...
    def _get_orderpoint_products(self):
        return self.env["product.product"].search(
            [("type", "=", "product"), ("stock_move_ids", "!=", False)]
        )
