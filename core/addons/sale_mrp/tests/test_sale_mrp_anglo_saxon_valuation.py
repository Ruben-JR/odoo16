...
class TestSaleMRPAngloSaxonValuation(ValuationReconciliationTestCommon):
        ...
        return self.env["product.product"].create(
            {
                ...
                else self.env.ref("product.product_category_all").id,
            }
        )
    ...
    def test_sale_mrp_kit_bom_cogs(self):
        ...
        self.assertAlmostEqual(
            ...
            1.53,
            "Should not include the value of consumable component",
        )
        cogs_aml = amls.filtered(
            lambda aml: aml.account_id == self.company_data["default_account_expense"]
        )
        self.assertAlmostEqual(
            cogs_aml.debit, 1.53, "Should not include the value of consumable component"
        )
