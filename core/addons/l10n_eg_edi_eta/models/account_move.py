...
class AccountMove(models.Model):
    ...
    def _compute_eta_long_id(self):
        for rec in self:
            response_data = rec.l10n_eg_eta_json_doc_id and json.loads(
                rec.l10n_eg_eta_json_doc_id.raw
            ).get("response")
            if response_data:
    ...
    def action_post_sign_invoices(self):
        ...
        invoices = self.filtered(
            lambda r: r.country_code == "EG"
            and r.state == "posted"
            and not r.l10n_eg_submission_number
            and r.edi_document_ids.filtered(lambda e: e.edi_format_id.code == "eg_eta")
        )
        if not invoices:
            return
    ...
    def _l10n_eg_edi_exchange_currency_rate(self):
        ...
        return (
            abs(
                self.invoice_line_ids[0].balance
                / self.invoice_line_ids[0].amount_currency
            )
