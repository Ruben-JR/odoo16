...
class ChooseDeliveryPackage(models.TransientModel):
    ...
    delivery_package_type_id = fields.Many2one(
        "stock.package.type", "Delivery Package Type", check_company=True
    )
    ...
    def _onchange_package_type_weight(self):
        if (
            self.delivery_package_type_id.max_weight
            and self.shipping_weight > self.delivery_package_type_id.max_weight
        ):
            warning_mess = {
    ...
    def action_put_in_pack(self):
        ...
        if (
            not self.picking_id.picking_type_id.show_reserved
