...
class TestStockLandedCostsCommon(ValuationReconciliationTestCommon):
    ...
    def setUpClass(cls, chart_template_ref=None):
        ...
        cls.categ_manual_periodic = cls.env.ref("product.product_category_all").copy(
            {"property_valuation": "manual_periodic", "property_cost_method": "fifo"}
        )
        ...
        cls.brokerage_quantity = cls.Product.create(
            {"name": "Brokerage Cost", "type": "service"}
        )
        cls.transportation_weight = cls.Product.create(
            {"name": "Transportation Cost", "type": "service"}
        )
        cls.packaging_volume = cls.Product.create(
            {"name": "Packaging Cost", "type": "service"}
        )
