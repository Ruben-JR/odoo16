...
class ProjectUpdate(models.Model):
    ...
    def _get_template_values(self, project):
        ...
        show_sold = (
            template_values["project"].allow_billable
            and len(services.get("data", [])) > 0
        )
        return {
            **template_values,
            "show_sold": show_sold,
            "show_profitability": bool(profitability),
            "show_activities": template_values["show_activities"]
            or show_sold
            or bool(profitability),
            "services": services,
            "profitability": profitability,
            "format_value": lambda value, is_hour: str(round(value, 2))
            if not is_hour
            else format_duration(value),
        }
        ...
        for sol in sols:
            ...
            if sol.product_uom.category_id == company_uom.category_id or is_unit:
                product_uom_qty = sol.product_uom._compute_quantity(
                    sol.product_uom_qty, company_uom, raise_if_failure=False
                )
                qty_delivered = sol.product_uom._compute_quantity(
                    sol.qty_delivered, company_uom, raise_if_failure=False
                )
    ...
    def _get_profitability_values(self, project):
        ...
        if not (
            self.user_has_groups("project.group_project_manager") and costs_revenues
        ):
            return {}
        ...
        return {
            ...
            "revenues": revenues,
            "revenues_formatted": format_amount(
                self.env, revenues, project.currency_id
            ),
            "margin": margin,
