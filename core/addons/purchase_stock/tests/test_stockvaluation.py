...
class TestStockValuation(TransactionCase):
    ...
    def test_different_uom(self):
        ...
        replenish_wizard = self.env["product.replenish"].create(
            {
                ...
                .search([("company_id", "=", self.env.user.id)], limit=1)
                .id,
            }
        )
        ...
        order_line = last_po_id.order_line.search(
            [("product_id", "=", self.product1.id)]
        )
        self.assertEqual(
...
class TestStockValuationWithCOA(AccountTestInvoicingCommon):
        ...
        cls.product1.categ_id.write(
            {
                ...
                "property_stock_valuation_account_id": cls.stock_valuation_account.id,
                "property_stock_journal": cls.stock_journal.id,
                "property_account_creditor_price_difference_categ": cls.product1.product_tmpl_id.get_product_accounts()[
                    "expense"
                ],
                "property_valuation": "real_time",
            }
    ...
    def test_change_currency_rate_average_1(self):
        ...
        price_unit_usd = po1.currency_id._convert(
            po1.order_line.price_unit,
            po1.company_id.currency_id,
            self.env.company,
            fields.Date.today(),
            round=False,
        )
        ...
        self.assertAlmostEqual(
            move1.stock_valuation_layer_ids.unit_cost, price_unit_usd_new_rate
        )

        self.assertAlmostEqual(
            self.product1.value_svl, price_unit_usd_new_rate * 10, delta=0.1
        )
    ...
    def test_fifo_anglosaxon_return(self):
        ...
        stock_return_picking_form = Form(
            self.env["stock.return.picking"].with_context(
                active_ids=receipt_po2.ids,
                active_id=receipt_po2.ids[0],
    ...
    def test_anglosaxon_valuation(self):
        ...
        self.assertEqual(
            ...
            1,
            "For now, only one line for the stock valuation account",
        )
        self.assertAlmostEqual(
            receipt_aml.debit, 10, "Should be equal to the PO line unit price (10)"
        )
        ...
        self.assertEqual(
            len(stock_valuation_aml),
            2,
            "A second line should have been generated for the price difference.",
        )
        self.assertAlmostEqual(
            price_diff_aml.debit, 5, "Price difference should be equal to 5 (15-10)"
        )
        self.assertAlmostEqual(
            sum(stock_valuation_aml.mapped("debit")),
            15,
        ...
        self.assertEqual(
            len(input_aml),
            3,
            "Only three lines should have been generated in stock input account: one when receiving the product, one when making the invoice.",
        )
        ...
        self.assertEqual(
            ...
            15,
            "Total debit value on stock input account should be equal to the invoice price of the product.",
        )
        self.assertEqual(
            sum(invoice_amls.mapped("credit")),
            0,
            "Invoice account move lines should not contains information on stock input at this point.",
        )
        self.assertEqual(
            sum(picking_aml.mapped("credit")),
            15,
            "Total credit value on stock input account should be equal to the invoice price of the product.",
        )
    ...
    def test_standard_valuation_multicurrency(self):
        ...
        self.cr.execute(
            ...
            (self.usd_currency.id, company.id),
        )
        self.env["res.currency.rate"].search([]).unlink()
        self.env["res.currency.rate"].create(
            {
        ...
        self.assertAlmostEqual(
            payable_aml.debit,
            50,
            "Total debit value should be equal to the original PO price of the product.",
        )
        self.assertAlmostEqual(
            picking_aml.credit,
            10,
            "credit value for stock should be equal to the standard price of the product.",
        )
        self.assertAlmostEqual(diff_aml.credit, 40, "credit value for price difference")

        # check EUR
        self.assertAlmostEqual(
            payable_aml.amount_currency,
            100,
            "Total debit value should be equal to the original PO price of the product.",
        )
        self.assertAlmostEqual(
            picking_aml.amount_currency,
            -20,
            "credit value for stock should be equal to the standard price of the product.",
        )
        self.assertAlmostEqual(
            diff_aml.amount_currency, -80, "credit value for price difference"
        )
    ...
    def test_valuation_multicurecny_with_tax(self):
        ...
        picking_aml = self.env["account.move.line"].search(
            [("account_id", "=", self.stock_valuation_account.id)]
        )
        ...
        self.assertAlmostEqual(
            picking_aml.amount_currency,
            95,
            "credit value for stock should be equal to the untaxed price of the product.",
        )
    ...
    def test_average_realtime_anglo_saxon_valuation_multicurrency_same_date(self):
        ...
        payable_line = move_lines.filtered(
        ...
        stock_line = move_lines.filtered(
            lambda l: l.account_id == self.stock_input_account and l.balance > 0
        )
    ...
    def test_realtime_anglo_saxon_valuation_multicurrency_different_dates(self):
        ...
        line_product_average = po.order_line.filtered(
            lambda l: l.product_id == self.product1
        )
        line_product_standard = po.order_line.filtered(
            lambda l: l.product_id == product_standard
        )
        ...
        stock_lines = product_lines.filtered(
            lambda l: l.account_id == self.stock_input_account
        )
    ...
    def test_average_realtime_with_delivery_anglo_saxon_valuation_multicurrency_different_dates(
        self,
    ):
        """
        The PO and invoice are in the same foreign currency.
        ...
        (
            picking.move_ids.filtered(
                lambda l: l.purchase_line_id == line_product_avg
        ...
        self.assertRecordValues(
            inv.line_ids.full_reconcile_id.reconciled_line_ids,
            [
                # pylint: disable=C0326
                {
                    "balance": -42.86,
    ...
    def test_average_realtime_with_delivery_anglo_saxon_valuation_multicurrency_same_dates(
        self,
    ):
        ...
        self.assertRecordValues(
            inv.line_ids.full_reconcile_id.reconciled_line_ids.sorted("id"),
            [
                # pylint: disable=C0326
                {
                    "balance": -60.0,
    ...
    def test_average_realtime_with_two_delivery_anglo_saxon_valuation_multicurrency_different_dates(
        self,
    ):
        """
        The PO and invoice are in the same foreign currency.
        ...
        with freeze_time(date_delivery1):
            ...
            (
                backorder_picking.move_ids.filtered(
                    lambda l: l.purchase_line_id == line_product_avg
                ).write({"quantity_done": 5.0})
            )
        ...
        self.assertRecordValues(
            inv1.line_ids.full_reconcile_id.reconciled_line_ids.sorted("id"),
            [
                # pylint: disable=C0326
    ...
    def test_anglosaxon_valuation_price_total_diff_discount(self):
        ...
        self.assertAlmostEqual(
            ...
            20,
            "Credit of 20 because of the difference between the PO and its invoice",
        )
    ...
    def test_anglosaxon_valuation_discount(self):
        ...
        self.assertAlmostEqual(
            sum(stock_valuation_aml.mapped("credit")),
            10,
            "Credit of 10 because of the 10% discount",
        )
