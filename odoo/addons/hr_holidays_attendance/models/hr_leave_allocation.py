...
class HolidaysAllocation(models.Model):
    ...
    def _compute_overtime_deductible(self):
        for allocation in self:
            allocation.overtime_deductible = (
                allocation.hr_attendance_overtime
                and allocation.holiday_status_id.overtime_deductible
            )
    ...
    def action_draft(self):
        ...
        if any(
            [
                a.employee_overtime < float_round(a.number_of_hours_display, 2)
                for a in overtime_allocations
            ]
        ):
            raise ValidationError(
                _(
