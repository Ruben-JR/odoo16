...
class WebsiteSaleCart(TransactionCase):
    ...
    def test_add_cart_deleted_product(self):
        ...
        with self.assertRaises(UserError):
            with MockRequest(
                product.with_user(self.public_user).env,
                website=self.website.with_user(self.public_user),
            ):
                self.WebsiteSaleController.cart_update_json(
                    product_id=product_id, add_qty=1
                )
    ...
    def test_update_cart_zero_qty(self):
        ...
        with MockRequest(product.with_user(portal_user).env, website=website):
            ...
            self.WebsiteSaleController.cart_update_json(
                product_id=product.id, line_id=sale_order.order_line.id, set_qty=0
            )
            ...
            self.WebsiteSaleController.cart_update_json(
                product_id=product.id, set_qty=0
            )
            self.assertEqual(sale_order.order_line, SaleOrderLine)

            self.WebsiteSaleController.cart_update_json(
                product_id=product.id, add_qty=0
            )
            self.assertEqual(sale_order.order_line, SaleOrderLine)
