...
class TestAccountMoveReconcile(AccountTestInvoicingCommon):
    ...
    def assertFullReconcile(self, full_reconcile, lines):
        ...
        if full_reconcile.exchange_move_id:
            lines += exchange_difference_move.line_ids.filtered(
    ...
    def _create_line_for_reconciliation(
        self, balance, amount_currency, currency, move_date
    ):
    ...
    def test_full_reconcile_bunch_lines(self):
        ...
        line_1 = self._create_line_for_reconciliation(
            1000.0, 1000.0, comp_curr, "2016-01-01"
        )
        line_2 = self._create_line_for_reconciliation(
            -300.0, -300.0, comp_curr, "2016-01-01"
        )
        line_3 = self._create_line_for_reconciliation(
            -400.0, -400.0, comp_curr, "2016-01-01"
        )
        line_4 = self._create_line_for_reconciliation(
            -500.0, -500.0, comp_curr, "2016-01-01"
        )
        line_5 = self._create_line_for_reconciliation(
    ...
    def test_reconcile_lines_corner_case_2_zero_amount_currency_same_foreign_currency(
        ...
        self.assertTrue(res.get("full_reconcile"))
        self.assertRecordValues(
            res["full_reconcile"].exchange_move_id,
            [{"date": fields.Date.from_string("2017-01-01")}],
        )
        self.assertRecordValues(
            res["full_reconcile"].exchange_move_id.line_ids,
            [
                {
                    "debit": 0.0,
                    "credit": 0.01,
            ...
            [
                {
                    ...
                    "credit_amount_currency": 0.0,
                    "debit_move_id": line_2.id,
                    "credit_move_id": res["full_reconcile"]
                    .exchange_move_id.line_ids[0]
                    .id,
                }
            ],
        )
    ...
    def test_reconcile_lines_corner_case_3_zero_balance_one_foreign_currency(self):
        ...
        self.assertRecordValues(
            res["partials"].exchange_move_id,
            [{"date": fields.Date.from_string("2017-01-01")}],
        )
        self.assertRecordValues(
            res["partials"].exchange_move_id.line_ids,
            [
                {
                    "debit": 0.01,
                    "credit": 0.0,
    ...
    def test_reconcile_special_mexican_workflow_1(self):
        ...
        pay1_liquidity_line = pay1.line_ids.filtered(
            lambda x: x.account_id.account_type != "asset_receivable"
        )
        pay1_rec_line = pay1.line_ids.filtered(
            lambda x: x.account_id.account_type == "asset_receivable"
        )
        pay1.action_post()
        pay1.write(
        ...
        pay2.action_post()
        pay2_rec_line = pay2.line_ids.filtered(
            lambda x: x.account_id.account_type == "asset_receivable"
        )
        ...
        self.assertRecordValues(
            res["partials"].exchange_move_id,
            [{"date": fields.Date.from_string("2019-06-24")}],
        )
        self.assertRecordValues(
            res["partials"].exchange_move_id.line_ids,
            [
        ...
        self.assertRecordValues(
            res["partials"].exchange_move_id,
            [{"date": fields.Date.from_string("2019-06-28")}],
        )
        self.assertRecordValues(
            res["partials"].exchange_move_id.line_ids,
            [
        ...
        self.assertRecordValues(
            res["partials"].exchange_move_id,
            [{"date": fields.Date.from_string("2019-09-24")}],
        )
        self.assertRecordValues(
            res["partials"].exchange_move_id.line_ids,
            [
    ...
    def test_reconcile_cash_basis_workflow_single_currency(self):
        ...
        self.assertRecordValues(
            res["tax_cash_basis_moves"][0].line_ids,
            [
        ...
        self.assertRecordValues(
            res["tax_cash_basis_moves"][1].line_ids,
            [
                # Base amount of tax_1 & tax_2:
                {
                    "debit": 8.33,
        ...
        self.assertRecordValues(
            res["tax_cash_basis_moves"].line_ids,
            [
                # Base amount of tax_1 & tax_2:
                {
                    "debit": 0.01,
            ...
            [
                {
                    ...
                    "credit": 0.01,
                    "tax_ids": [],
                    "tax_line_id": self.cash_basis_tax_tiny_amount.id,
                },
                {
                    "account_id": self.cash_basis_transfer_account.id,
                    "debit": 0.01,
    ...
    def test_reconcile_cash_basis_workflow_multi_currency(self):
        ...
        self.assertFullReconcileAccount(
            res["full_reconcile"], self.extra_receivable_account_1
        )
        self.assertEqual(len(res.get("tax_cash_basis_moves", [])), 2)
        self.assertRecordValues(
            res["tax_cash_basis_moves"][0].line_ids,
        ...
        caba_transition_lines_1 = res["tax_cash_basis_moves"].line_ids.filtered(
        ...
        self.assertFullReconcileAccount(
            res["full_reconcile"], self.extra_receivable_account_2
        )
        self.assertEqual(len(res.get("tax_cash_basis_moves", [])), 3)
        self.assertRecordValues(
            res["tax_cash_basis_moves"][0].line_ids,
        ...
        caba_transition_lines_2 = res["tax_cash_basis_moves"].line_ids.filtered(
            lambda x: x.account_id == self.cash_basis_transfer_account
        )
        ...
        caba_transition_lines_3 = res["tax_cash_basis_moves"].line_ids.filtered(
            lambda x: x.account_id == self.cash_basis_transfer_account
        )
    ...
    def test_reconcile_cash_basis_exchange_difference_transfer_account_check_entries_1(
        ...
        caba_rounding_correction = exchange_diff.line_ids.filtered(
            lambda line: line.account_id == self.cash_basis_transfer_account
        ).sorted(lambda line: (line.account_id, line.debit, line.credit))

        self.assertFalse(
            caba_rounding_correction,
            "No cash basis rounding correction should have been created, as the difference between amounts is only due to exchange difference.",
        )
    ...
    def test_reconcile_cash_basis_exchange_difference_transfer_account_check_entries_2(
        ...
        """
        self.env.company.tax_exigibility = True
        currency_id = self.setup_multi_currency_data(
            default_values={
                "name": "bitcoin",
                "symbol": "bc",
        ...
        self.assertFalse(
            exchange_move,
            "No exchange move difference should be created for the full reconcile object ,as there is no cash basis rounding.",
        )
    ...
    def test_reconcile_cash_basis_refund_multicurrency(self):
        self.env.company.tax_exigibility = True
        rates_data = self.setup_multi_currency_data(
            default_values={
                "name": "Playmock",
                "symbol": "🦌",
                "rounding": 0.01,
    ...
    def test_reconcile_cash_basis_tax_grid_refund(self):
        ...
        tax_cash_basis_moves = res["tax_cash_basis_moves"].sorted(
    ...
    def test_caba_double_tax(self):
        ...
        test_analytic_account = self.env["account.analytic.account"].create(
            {"name": "test_analytic_account", "plan_id": analytic_plan.id}
        )
        ...
        pmt_wizard = (
            self.env["account.payment.register"]
            .with_context(active_model="account.move", active_ids=rslt.ids)
            .create(
                {
        ...
        self.assertEqual(
            caba_move.line_ids.filtered(lambda x: x.tax_line_id).balance,
            66.66,
            "Tax amount should take into account both lines",
        )
    ...
    def test_caba_rounding_adjustment_monocurrency(self):
        ...
        self.assertRecordValues(
            invoice.tax_cash_basis_created_move_ids.filtered(
                lambda x: x.date == payment_date_1
            ).line_ids,
            [
        ...
        self.assertRecordValues(
            invoice.tax_cash_basis_created_move_ids.filtered(
                lambda x: x.date == payment_date_2
            ).line_ids,
            [
                # pylint: disable=bad-whitespace
                {
    ...
    def test_caba_rounding_adjustment_multicurrency(self):
        ...
        currency_id = self.setup_multi_currency_data(
            {"name": "Minovsky Dollar", "rounding": 0.01}
        )["currency"].id
