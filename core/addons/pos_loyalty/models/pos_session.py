...
class PosSession(models.Model):
    ...
    def _get_pos_ui_product_product(self, params):
        ...
        products = self.env["product.product"].search_read(
            [("id", "in", products.ids)], fields=params["search_params"]["fields"]
        )
    ...
    def _set_loyalty_cards(self, partners):
        ...
        loyalty_programs = self.config_id._get_program_ids().filtered(
            lambda p: p.program_type == "loyalty"
        )
        ...
        for group in self.env["loyalty.card"].read_group(
            ...
            fields=[f"{field_name}:array_agg" for field_name in loyalty_card_fields]
            + ["ids:array_agg(id)"],
            groupby=["partner_id"],
        ):
            loyalty_cards = {}
            for i in range(group["partner_id_count"]):
                loyalty_cards[group["ids"][i]] = {
                    field_name: group[field_name][i]
                    for field_name in loyalty_card_fields
                }
