...
class TestSaleRefund(TestSaleCommon):
    ...
    def test_refund_cancel(self):
        ...
        for line in self.sale_order.order_line:
            if line.product_id.invoice_policy == "delivery":
                ...
                self.assertEqual(
                    ...
                    "The quantity invoiced is +1 unit from the one of the sale line, as we modified invoice lines (%s)"
                    % (line.name,),
                )
                self.assertEqual(
                    line.qty_to_invoice,
                    -1,
                    "The quantity to invoice is negative as we invoice more than ordered",
                )
        ...
        self.assertEqual(
            invoice_refund.payment_state,
            "paid",
            "Last Customer creadit note should be in paid state",
        )
        self.assertEqual(
            self.sale_order.invoice_count,
            2,
        ...
        for line in self.sale_order.order_line:
            if line.product_id.invoice_policy == "delivery":
                ...
                self.assertEqual(
                    ...
                    "The quantity to invoice should be the ordered quantity",
                )
                self.assertEqual(
                    line.qty_invoiced,
                    0,
                    "The quantity invoiced is zero as the refund (paid) completely cancel the first invoice",
                )
