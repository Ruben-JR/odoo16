...
class TestHrContracts(TestContractCommon):
    ...
    def setUpClass(cls):
        ...
        cls.test_contract = dict(
            name="Test", wage=1, employee_id=cls.employee.id, state="open"
        )
    ...
    def test_contract_enddate(self):
        self.test_contract.update(
            dict(date_end=datetime.now() + relativedelta(days=100))
        )
    ...
    def test_contract_start_date(self):
        self.test_contract.update(
            dict(date_start=datetime.now(), state="draft", kanban_state="done")
        )
        self.contract = self.contracts.create(self.test_contract)
        self.apply_cron()
        self.assertEqual(self.contract.state, "open")
    ...
    def test_contract_auto_expire(self):
        self.test_contract.update(
            dict(date_start=date.today() - relativedelta(days=10), state="open")
        )
        self.contract = self.contracts.create(self.test_contract)
        self.apply_cron()
        self.assertEqual(self.contract.state, "open")
