...
class SaleOrder(models.Model):
    ...
    def write(self, vals):
        ...
        if vals.get("partner_id"):
            registrations_toupdate = self.env["event.registration"].search(
    ...
    def action_confirm(self):
        ...
        for so in self:
            ...
            so.order_line._update_registrations(
                confirm=so.amount_total == 0, cancel_to_draft=False
            )
            if len(self) == 1:
                return (
...
class SaleOrderLine(models.Model):
    ...
    def _compute_event_id(self):
        event_lines = self.filtered(
            lambda line: line.product_id and line.product_id.detailed_type == "event"
        )
