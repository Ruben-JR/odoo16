...
class Users(models.Model):
    ...
    notification_type = fields.Selection(
        [("email", "Handle by Emails"), ("inbox", "Handle in Odoo")],
        "Notification",
        required=True,
        default="email",
    ...
    def write(self, vals):
        ...
        user_portal_access_dict = (
            {user.id: user.has_group("base.group_portal") for user in self}
            if log_portal_access
            else {}
        )
    ...
    def _init_messaging(self):
        ...
        values = {
            ...
            "currentGuest": False,
            "current_partner": self.partner_id.mail_partner_format().get(
                self.partner_id
            ),
            "current_user_id": self.id,
            "current_user_settings": self.env["res.users.settings"]
            ._find_or_create_for_user(self)
            ...
            "shortcodes": self.env["mail.shortcode"]
            .sudo()
            .search_read([], ["source", "substitution"]),
            "starred_counter": self.env["mail.message"].search_count(
