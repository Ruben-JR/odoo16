...
class Certificate(models.Model):
    ...
    password = fields.Char(
        help="Passphrase for the PFX certificate", groups="base.group_system"
    )
    date_start = fields.Datetime(
        readonly=True, help="The date on which the certificate starts to be valid"
    )
    date_end = fields.Datetime(
        readonly=True, help="The date on which the certificate expires"
    )
    ...
    def create(self, vals_list):
        ...
        for certificate in certificates:
            ...
            if spain_dt > cert_date_end:
                raise ValidationError(
                    _("The certificate is expired since %s", certificate.date_end)
                )
        return certificates
