...
class HrEmployeePrivate(models.Model):
    ...
    user_id = fields.Many2one(
        "res.users", "User", related="resource_id.user_id", store=True, readonly=False
    )
    ...
    spouse_complete_name = fields.Char(
        string="Spouse Complete Name", groups="hr.group_hr_user", tracking=True
    )
    ...
    identification_id = fields.Char(
        string="Identification No", groups="hr.group_hr_user", tracking=True
    )
    ...
    visa_expire = fields.Date(
        "Visa Expire Date", groups="hr.group_hr_user", tracking=True
    )
    ...
    has_work_permit = fields.Binary(
        string="Work Permit", groups="hr.group_hr_user", tracking=True
    )
    ...
    emergency_contact = fields.Char(
        "Contact Name", groups="hr.group_hr_user", tracking=True
    )
    emergency_phone = fields.Char(
        "Contact Phone", groups="hr.group_hr_user", tracking=True
    )
    ...
    def read(self, fields, load="_classic_read"):
        ...
        if private_fields:
            raise AccessError(
                _(
                    'The fields "%s" you try to read is not available on the public employee profile.'
                )
                % (",".join(private_fields))
            )
    ...
    def _search(
        ...
        """
        if self.check_access_rights("read", raise_exception=False):
            return super(HrEmployeePrivate, self)._search(
                args,
                offset=offset,
                limit=limit,
                order=order,
    ...
    def write(self, vals):
        if "address_home_id" in vals:
            ...
            if account_id:
                self.env["res.partner.bank"].browse(account_id).partner_id = vals[
                    "address_home_id"
                ]
    ...
    def toggle_active(self):
        ...
        archived_addresses = unarchived_employees.mapped("address_home_id").filtered(
            lambda addr: not addr.active
        )
        ...
        if (
            len(self) == 1
            and not self.active
            and not self.env.context.get("no_wizard", False)
        ):
            return {
                "type": "ir.actions.act_window",
    ...
    def _compute_related_contacts(self):
        ...
        for employee in self:
            employee.related_contact_ids |= (
                employee.address_home_id | employee.user_partner_id
            )
