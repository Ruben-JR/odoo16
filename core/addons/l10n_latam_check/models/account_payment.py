...
class AccountPayment(models.Model):
    ...
    def _compute_check_number(self):
        ...
        latam_checks = self.filtered(
            ...
            or (
                x.payment_method_line_id.code == "check_printing"
                and x.l10n_latam_manual_checks
            )
        )
    ...
    def _inverse_check_number(self):
        ...
        avoid_inverse = self.filtered(
            lambda x: x.l10n_latam_manual_checks
            or x.payment_method_line_id.code == "new_third_party_checks"
        )
    ...
    def _clean_l10n_latam_check_issuer_vat(self):
        for rec in self.filtered("l10n_latam_check_issuer_vat"):
            ...
            if hasattr(stdnum_vat, "compact"):
                rec.l10n_latam_check_issuer_vat = stdnum_vat.compact(
    ...
    def _check_l10n_latam_check_issuer_vat(self):
        for rec in self.filtered(
            lambda x: x.l10n_latam_check_issuer_vat and x.company_id.country_id
        ):
            if not self.env["res.partner"]._run_vat_test(
                rec.l10n_latam_check_issuer_vat, rec.company_id.country_id
            ):
                error_message = self.env["res.partner"]._build_vat_error_message(
                    rec.company_id.country_id.code.lower(),
                    rec.l10n_latam_check_issuer_vat,
                    "Check Issuer VAT",
                )
    ...
    def _compute_l10n_latam_check_warning_msg(self):
        ...
        latam_draft_checks = self.filtered(
            ...
            and (
                x.l10n_latam_manual_checks
                or x.payment_method_line_id.code
                in [
        ...
        for rec in latam_draft_checks:
            ...
            # new third party check
                ...
                rec.check_number
                and rec.payment_method_line_id.code == "new_third_party_checks"
                and rec.l10n_latam_check_bank_id
                and rec.l10n_latam_check_issuer_vat
            ):
                same_checks = self.search(
                    [
                        ...
                        (
                            "l10n_latam_check_bank_id",
                            "=",
                            rec.l10n_latam_check_bank_id.id,
                        ),
                        (
                            "l10n_latam_check_issuer_vat",
                            "=",
            ...
            if not rec.currency_id.is_zero(rec.l10n_latam_check_id.amount - rec.amount):
                msgs.append(
                    _(
                        ...
                        rec.amount,
                        rec.l10n_latam_check_id.amount,
                    )
                )
            if rec.payment_method_line_id.code in [
                "in_third_party_checks",
                "out_third_party_checks",
            ]:
                if rec.l10n_latam_check_id.state != "posted":
                    msgs.append(
                        _(
                            'Selected check "%s" is not posted',
                            rec.l10n_latam_check_id.display_name,
                        )
                    )
                elif (
                    rec.payment_type == "outbound"
                    and rec.l10n_latam_check_id.l10n_latam_check_current_journal_id
                    != rec.journal_id
                ) or (
                    ...
                    msgs.append(
                        _(
                            ...
                            else rec.destination_journal_id.name,
                        )
                    )
                elif (
                    rec.payment_type == "inbound"
                    and not rec.is_internal_transfer
                    and rec.l10n_latam_check_id.l10n_latam_check_current_journal_id
                ):
                    msgs.append(
                        _(
                            "Check '%s' is on journal '%s', it can't be received it again",
                            rec.l10n_latam_check_id.display_name,
                            rec.journal_id.name,
                        )
                    )
    ...
    def _compute_l10n_latam_check_current_journal(self):
        new_checks = self.filtered(
            lambda x: x.payment_method_line_id.code == "new_third_party_checks"
        )
        payments = self.env["account.payment"].search(
            [("l10n_latam_check_id", "in", new_checks.ids), ("state", "=", "posted")],
            order="date desc, id desc",
        )
        ...
        for rec in new_checks:
            ...
            if not last_operation:
                ...
                continue
            if (
                last_operation.is_internal_transfer
                and last_operation.payment_type == "outbound"
            ):
                rec.l10n_latam_check_current_journal_id = (
                    last_operation.paired_internal_transfer_payment_id.journal_id
                )
    ...
    def _constrains_check_number(self):
        ...
        return super(
            AccountPayment, self - third_party_checks
        )._constrains_check_number()
    ...
    def _onchange_check_number(self):
        for rec in self.filtered(
            lambda x: x.journal_id.company_id.country_id.code == "AR"
            and x.l10n_latam_check_number
    ...
    def action_unmark_sent(self):
        ...
        if self.filtered("l10n_latam_manual_checks"):
            raise UserError(
                _("Unmark sent is not implemented for electronic or deferred checks")
            )
    ...
    def _prepare_move_line_default_vals(self, write_off_line_vals=None):
        ...
        check = (
            self
            if (
                self.payment_method_line_id.code == "new_third_party_checks"
                or self.l10n_latam_manual_checks
            )
    ...
    def name_get(self):
        ...
        for i, (res_name, rec) in enumerate(zip(res_names, self)):
            if (
                ...
                res_names[i] = (
                    res_name[0],
                    "%s %s" % (res_name[1], _("(Check %s)", rec.check_number)),
                )
        return res_names
    ...
    def button_open_check_operations(self):
        ...
        operations = (
            self.l10n_latam_check_operation_ids.filtered(lambda x: x.state == "posted")
            + self
        )
        action = {
    ...
    def _create_paired_internal_transfer_payment(self):
        ...
        third_party_checks = self.filtered(
            ...
            in ["in_third_party_checks", "out_third_party_checks"]
        )
