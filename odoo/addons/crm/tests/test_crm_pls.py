...
class TestCRMPLS(TransactionCase):
    ...
    def test_predictive_lead_scoring(self):
        ...
        self.assertEqual(
            lead_9_country_freq.lost_count, 0.0
        ...
        self.assertEqual(
            lead_4_stage_won_freq.lost_count, 0.1
        )  # unchanged - consider stages with <= sequence when lost
        ...
        self.assertEqual(
            lead_9_stage_won_freq.won_count, 2.1
        )  # + 1 - consider every stages when won
        ...
        self.assertEqual(
            lead_9_country_freq.lost_count, 0.1
        )  # unchanged (did not exists before)
        ...
        leads[3].stage_id = stage_ids[
            0
        ]  # probability should only change a bit as frequencies are almost the same (except 0.0 -> 0.1)
        ...
        lead_tag_1_2.tag_ids = [
            (3, tag_ids[0]),
            (4, tag_ids[1]),
        ]  # remove tag 1 / add tag 2
    ...
    def test_pls_no_share_stage(self):
        ...
        self.env["crm.stage"].search([("team_id", "=", False)]).write(
            {"team_id": team_id}
        )
