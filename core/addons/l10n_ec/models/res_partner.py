...
class ResPartner(models.Model):
                        ...
                        if partner.l10n_latam_identification_type_id.id == it_dni.id:
                            error_message = (
                                _(
                                    ...
                                    "it must be like this form 1234567897"
                                )
                                % partner.vat
                            )
                        if partner.l10n_latam_identification_type_id.id == it_ruc.id:
                            error_message = (
                                _(
                                    ...
                                    "it must be like this form 1234567897001"
                                )
                                % partner.vat
                            )
    ...
    def _l10n_ec_get_identification_type(self):
        ...
        def id_type_in(*args):
            return any(
                [
                    self.l10n_latam_identification_type_id == self.env.ref(arg)
                    for arg in args
                ]
            )
        ...
        if id_type_in("l10n_ec.ec_dni"):
            ...
            "l10n_latam_base.it_fid", "l10n_latam_base.it_vat"
        ) or self.l10n_latam_identification_type_id.country_id != self.env.ref(
            "base.ec"
        ):
