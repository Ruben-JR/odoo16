...
class One2manyCase(TransactionCase):
    def setUp(self):
        ...
        for name, editions in books_data:
            ...
            for edition in editions:
                self.Edition.create(
                    {"res_model_id": book_model_id, "name": edition, "res_id": book_id}
                )
    ...
    def test_rpcstyle_one_by_one_on_new(self):
        self.multi = self.env["test_new_api.multi"].new({"name": "What is up?"})
        for name in range(10):
            self.multi.lines = [Command.create({"name": str(name)})]
        self.operations()
    ...
    def test_many2one_integer(self):
        ...
        res_books_with_movie_edition = self.Book.search(
            [("editions", "in", movie_editions.ids)]
        )
        self.assertFalse(t(res_books_with_movie_edition))

        res_books_without_movie_edition = self.Book.search(
            [("editions", "not in", movie_editions.ids)]
        )
        self.assertItemsEqual(t(res_books_without_movie_edition), t(books))

        res_books_without_one_movie_edition = self.Book.search(
            [("editions", "not in", movie_editions[:1].ids)]
        )
        self.assertItemsEqual(t(res_books_without_one_movie_edition), t(books))

        res_books_with_one_movie_edition_name = self.Book.search(
            [("editions", "=", movie_editions[:1].name)]
        )
        self.assertFalse(t(res_books_with_one_movie_edition_name))

        res_books_without_one_movie_edition_name = self.Book.search(
            [("editions", "!=", movie_editions[:1].name)]
        )
        self.assertItemsEqual(t(res_books_without_one_movie_edition_name), t(books))

        res_movies_not_of_edition_name = self.Movie.search(
            [("editions", "!=", one_movie_edition.name)]
        )
        self.assertItemsEqual(
    ...
    def test_cache_invalidation(self):
        ...
        with self.assertQueryCount(0):
            self.assertEqual(
                attachment.name, record0.display_name, "field should be computed"
            )
        ...
        with self.assertQueryCount(0):
            self.assertEqual(
                attachment.name, record1.display_name, "field should be recomputed"
            )
        with self.assertQueryCount(1):
    ...
    def test_recomputation_ends(self):
        ...
        child = self.env["test_new_api.model_child_m2o"].create(
