...
class Digest(models.Model):
    ...
    def _compute_available_fields(self):
        for digest in self:
            ...
            for field_name, field in digest._fields.items():
                if (
                    field.type == "boolean"
                    and field_name.startswith(("kpi_", "x_kpi_", "x_studio_kpi_"))
                    and digest[field_name]
                ):
    ...
    def _action_send(self, update_periodicity=True):
        ...
        to_slowdown = (
            ...
            else self.env["digest.digest"]
        )

        for digest in self:
            for user in digest.user_ids:
                digest.with_context(
                    digest_slowdown=digest in to_slowdown, lang=user.lang
    ...
    def _compute_kpis(self, company, user):
        ...
        kpis = [
            dict(
                kpi_name=field_name,
                kpi_fullname=self.env["ir.model.fields"]
                ._get(self._name, field_name)
                .field_description,
                kpi_action=False,
                kpi_col1=dict(),
                kpi_col2=dict(),
        ...
        for col_index, (tf_name, tf) in enumerate(self._compute_timeframes(company)):
            digest = (
                self.with_context(start_datetime=tf[0][0], end_datetime=tf[0][1])
                .with_user(user)
                .with_company(company)
            )
            previous_digest = (
                self.with_context(start_datetime=tf[1][0], end_datetime=tf[1][1])
                .with_user(user)
                .with_company(company)
            )
    ...
    def _compute_preferences(self, company, user):
        ...
        if self._context.get("digest_slowdown"):
            ...
            preferences.append(
                Markup(
                    '<p>%s<br /><a href="%s" target="_blank" style="color:#875A7B; font-weight: bold;">%s</a></p>'
                )
                % (
        ...
        return [
            ...
            (
                ...
                (
                    ...
                    (
                        start_datetime + relativedelta(weeks=-2),
                        start_datetime + relativedelta(weeks=-1),
                    ),
                ),
            ),
            (
                ...
                (
                    ...
                    (
                        start_datetime + relativedelta(months=-2),
