...
class StockQuant(models.Model):
    ...
    currency_id = fields.Many2one(
        "res.currency", compute="_compute_value", groups="stock.group_stock_manager"
    )
    ...
    def _compute_value(self):
        ...
        for quant in self:
            ...
            if (
                ...
                or (quant.owner_id and quant.owner_id != quant.company_id.partner_id)
                or float_is_zero(
                    quant.quantity, precision_rounding=quant.product_id.uom_id.rounding
                )
            ):
                quant.value = 0
                continue
            ...
            if float_is_zero(
                quantity, precision_rounding=quant.product_id.uom_id.rounding
            ):
                quant.value = 0.0
                continue
            quant.value = (
                quant.quantity
                * quant.product_id.with_company(quant.company_id).value_svl
                / quantity
            )
