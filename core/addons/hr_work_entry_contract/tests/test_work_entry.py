...
class TestWorkEntry(TestWorkEntryBase):
    ...
    def test_work_entry(self):
        ...
        attendance_nb = len(
            self.resource_calendar_id._attendance_intervals_batch(
                self.start.replace(tzinfo=pytz.utc), self.end.replace(tzinfo=pytz.utc)
            )[False]
        )
    ...
    def test_approve_multiple_day_work_entry(self):
        ...
        self.assertTrue(
            all((b.state == "validated" for b in work_entries)),
            "Work entries should be approved",
        )
    ...
    def test_validate_conflict_work_entry(self):
        ...
        self.assertFalse(
            ...
            "It should not validate work_entries conflicting with others",
        )
    ...
    def test_outside_calendar(self):
        ...
        (
            ...
            work_entry_3,
            work_entry_4,
        ) = self.create_work_entries(
            [
                # Outside but not a leave
                (datetime(2018, 10, 10, 3, 0), datetime(2018, 10, 10, 4, 0)),
                # Outside and a leave
                (
                    datetime(2018, 10, 10, 1, 0),
                    datetime(2018, 10, 10, 2, 0),
                    self.work_entry_type_leave,
                ),
                # Overlapping and a leave
                (
                    datetime(2018, 10, 10, 7, 0),
        ...
        (
            work_entry_1 | work_entry_2 | work_entry_3 | work_entry_4
        )._mark_leaves_outside_schedule()
    ...
    def test_write_conflict(self):
        ...
        self.assertNotEqual(
            work_entry_1.state, "conflict", "It should no longer conflict"
        )
        self.assertNotEqual(
            work_entry_2.state, "conflict", "It should no longer conflict"
        )
    ...
    def test_no_overlap_sql_constraint(self):
        ...
        work_entry_1.state = "validated"
        work_entry_2 = self.create_work_entry(
            datetime(2018, 10, 10, 8, 0), datetime(2018, 10, 10, 10, 0)
        )
        self.assertEqual(work_entry_1.state, "conflict")
