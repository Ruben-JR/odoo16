...
class TestHolidaysFlow(TestHrHolidaysCommon):
    ...
    def test_01_leave_request_flow_limited(self):
        ...
        with freeze_time("2022-01-15"):
            ...
            def _check_holidays_status(holiday_status, ml, lt, rl, vrl):
                self.assertEqual(
                    holiday_status.max_leaves,
                    ml,
                    "hr_holidays: wrong type days computation",
                )
                self.assertEqual(
                    holiday_status.leaves_taken,
                    lt,
                    "hr_holidays: wrong type days computation",
                )
                self.assertEqual(
            ...
            hol2 = HolidaysEmployeeGroup.create(
                {
                    ...
                    "employee_id": self.employee_emp_id,
                    "holiday_status_id": self.holidays_status_limited.id,
                    "date_from": (datetime.today() + relativedelta(days=2)).strftime(
                        "%Y-%m-%d %H:%M"
                    ),
            ...
            self.assertEqual(
                hol2.state, "draft", "hr_holidays: resetting should lead to draft state"
            )
            ...
            hol3.action_draft()
            self.assertEqual(
                hol3.state, "draft", "hr_holidays: resetting should lead to draft state"
            )
            hol3.action_confirm()
            self.assertEqual(
                hol3.state,
                "confirm",
                "hr_holidays: confirming should lead to confirm state",
            )
    ...
    def test_10_leave_summary_reports(self):
        ...
        ctx = {"model": "hr.employee", "active_ids": [self.ref("hr.employee_admin")]}
        data_dict = {
