...
def content_disposition(filename):
    return "attachment; filename*=UTF-8''{}".format(url_quote(filename, safe=""))
    ...
    for ctrl in build_controllers():
        for method_name, method in inspect.getmembers(ctrl, inspect.ismethod):
            ...
            def is_method_a_route(cls):
                return (
                    getattr(getattr(cls, method_name, None), "original_routing", None)
                    is not None
                )
...
class Request:
    ...
    def _get_session_and_dbname(self):
        ...
        # response cookie.
        sid = self.httprequest.args.get("session_id") or self.httprequest.headers.get(
    ...
    def geoip(self):
        ...
        if "_geoip" not in self.session:
            ...
            self.session._geoip = (
                self.registry["ir.http"]._geoip_resolve()
                if self.db
                else self._geoip_resolve()
            )
    ...
    def redirect(self, location, code=303, local=True):
        ...
        if local:
            location = "/" + url_parse(location).replace(
                scheme="", netloc=""
            ).to_url().lstrip("/")
        if self.db:
    ...
    def _save_session(self):
        ...
        if not sess.is_explicit and (sess.is_dirty or cookie_sid != sess.sid):
            self.future_response.set_cookie(
                "session_id", sess.sid, max_age=SESSION_LIFETIME, httponly=True
            )
        ...
        with contextlib.closing(self.registry.cursor()) as cr:
            ...
            try:
                ...
                if (
                    ...
                    and self.dispatcher.routing_type != "json"
                ):
...
class Application:
    ...
    def nodb_routing_map(self):
        ...
        for url, endpoint in _generate_routing_rules(
            [""] + odoo.conf.server_wide_modules, nodb_only=True
        ):
    ...
    def __call__(self, environ, start_response):
        ...
        try:
            ...
            if (
                "werkzeug" in config["dev_mode"]
                and request.dispatcher.routing_type != "json"
            ):
                raise
