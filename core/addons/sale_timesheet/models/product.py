...
class ProductTemplate(models.Model):
    ...
    project_id = fields.Many2one(
        domain="[('company_id', '=', current_company_id), ('allow_billable', '=', True), ('pricing_type', '=', 'task_rate'), ('allow_timesheets', 'in', [service_policy == 'delivered_timesheet', True])]"
    )
    project_template_id = fields.Many2one(
        domain="[('company_id', '=', current_company_id), ('allow_billable', '=', True), ('allow_timesheets', 'in', [service_policy == 'delivered_timesheet', True])]"
    )
    ...
    def _unlink_except_master_data(self):
        ...
        if time_product.product_tmpl_id in self:
            raise ValidationError(
                _(
                    "The %s product is required by the Timesheets app and cannot be archived nor deleted."
                )
                % time_product.name
            )
