...
class IrUiView(models.Model):
    ...
    def save_embedded_field(self, el):
        ...
        try:
            ...
            raise ValidationError(
                _(
                    "Invalid field value for %s: %s",
                    Model._fields[field].string,
        ...
        if value is not None:
            ...
            if not self.env.context.get("lang") and self.get_default_lang_code():
                Model.browse(int(el.get("data-oe-id"))).with_context(
                    lang=self.get_default_lang_code()
                ).write({field: value})
            else:
    ...
    def save_oe_structure(self, el):
        ...
        xpath = etree.Element(
            "xpath",
            expr="//*[hasclass('oe_structure')][@id='{}']".format(el.get("id")),
            position="replace",
        )
        arch.append(xpath)
    ...
    def _are_archs_equal(self, arch1, arch2):
        ...
        if len(arch1) != len(arch2):
            return False
        return all(
            self._are_archs_equal(arch1, arch2) for arch1, arch2 in zip(arch1, arch2)
        )
    ...
    def _view_get_inherited_children(self, view):
        if self._context.get("no_primary_children", False):
            ...
            return view.inherit_children_ids.filtered(
                lambda extension: extension.mode != "primary"
                or extension.id in original_hierarchy
            )
    ...
    def _view_obj(self, view_id):
        if isinstance(view_id, str):
            return self.search([("key", "=", view_id)], limit=1) or self.env.ref(
                view_id
            )
    ...
    def get_related_views(self, key, bundles=False):
        ...
        return views.filtered(
            lambda v: not v.groups_id or len(user_groups.intersection(v.groups_id))
        )
    ...
    def save_snippet(self, name, arch, template_key, snippet_key, thumbnail_url):
        ...
        used_names = self.search(
            expression.AND([[("name", "=like", "%s%%" % name)], website_domain])
        ).mapped("name")
