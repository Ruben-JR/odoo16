...
class TestItEdiExport(TestItEdi):
    ...
    def setUpClass(cls):
        ...
        cls.tax_zero_percent_hundred_percent_repartition = cls.env[
            "account.tax"
        ].create(
            {
                "name": "all of nothing",
        ...
        cls.non_latin_and_latin_invoice = (
            cls.env["account.move"]
            .with_company(cls.company)
            .create(
                {
        ...
        cls.below_400_codice_simplified_invoice = (
            cls.env["account.move"]
            .with_company(cls.company)
            .create(
                {
        ...
        cls.total_400_VAT_simplified_invoice = (
            cls.env["account.move"]
            .with_company(cls.company)
            .create(
                {
        ...
        cls.more_400_simplified_invoice = (
            cls.env["account.move"]
            .with_company(cls.company)
            .create(
                {
        ...
        cls.non_domestic_simplified_invoice = (
            cls.env["account.move"]
            .with_company(cls.company)
            .create(
                {
                                ...
                                **cls.standard_line,
                                ...
                                "tax_ids": [
                                    (
                                        ...
                                        0,
                                        [
                                            cls.tax_zero_percent_hundred_percent_repartition.id
                                        ],
                                    )
                                ],
                            },
                                ...
                                **cls.standard_line,
                                ...
                                "tax_ids": [
                                    (
                                        ...
                                        0,
                                        [
                                            cls.tax_zero_percent_zero_percent_repartition.id
                                        ],
                                    )
                                ],
                            },
    ...
    def test_price_included_taxes(self):
        ...
        price_included_lines = lines.filtered(
            lambda line: line.tax_ids == self.price_included_tax
        )
        self.assertEqual(
            [line.price_subtotal for line in price_included_lines], [656.07, 656.07]
        )
        # So the taxable a base the edi expects (for this tax) is actually 1312.14
        price_included_tax_line = lines.filtered(
            lambda line: line.tax_line_id == self.price_included_tax
        )
        ...
        invoice_etree = etree.fromstring(
            self.edi_format._l10n_it_edi_export_invoice_as_xml(
                self.price_included_invoice
            )
        )
        # Remove the attachment and its details
        invoice_etree = self.with_applied_xpath(
            invoice_etree, "<xpath expr='.//Allegati' position='replace'/>"
        )
    ...
    def test_below_400_codice_simplified_invoice(self):
        invoice_etree = etree.fromstring(
            self.edi_format._l10n_it_edi_export_invoice_as_xml(
                self.below_400_codice_simplified_invoice
            )
        )
        expected_etree = self.with_applied_xpath(
    ...
    def test_send_pa_partner(self):
        ...
        self.assertEqual(
            res[self.pa_partner_invoice],
            {
                "attachment": self.pa_partner_invoice.l10n_it_edi_attachment_id,
                "success": True,
            },
        )
    ...
    def test_zero_percent_taxes(self):
        invoice_etree = etree.fromstring(
            self.edi_format._l10n_it_edi_export_invoice_as_xml(self.zero_tax_invoice)
        )
        expected_etree = self.with_applied_xpath(
            etree.fromstring(self.edi_basis_xml),
            """
