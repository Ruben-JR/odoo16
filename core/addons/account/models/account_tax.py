...
class AccountTax(models.Model):
    ...
    def _get_generation_dict_from_base_line(
        ...
        tax_account = (
            tax_repartition_line._get_aml_target_tax_account(
                force_caba_exigibility=force_caba_exigibility
            )
            or line_vals["account"]
        )
        return {
    ...
    def _aggregate_taxes(
        ...
        if self.env.company.tax_calculation_rounding_method == "round_globally":
            amount_per_tax_repartition_line_id = defaultdict(
                lambda: {
            ...
            for base_line, to_update_vals, tax_values_list in to_process:
                ...
                for tax_values in tax_values_list:
                    grouping_key = frozendict(
                        self._get_generation_dict_from_base_line(base_line, tax_values)
                    )
                    ...
                    tax_amount_with_delta = (
                        tax_values["tax_amount"] + total_amounts["delta_tax_amount"]
                    )
    ...
    def _compute_taxes(
        ...
        def grouping_key_generator(base_line, tax_values):
            return self._get_generation_dict_from_base_line(
                base_line, tax_values, force_caba_exigibility=include_caba_tags
            )
