...
class TestHttpMisc(TestHttpBase):
    ...
    def test_misc2_local_redirect(self):
        ...
        self.assertEqual(local_redirect("hello?a=b"), "/hello?a=b")
        self.assertEqual(
            local_redirect("www.example.com/hello?a=b"), "/www.example.com/hello?a=b"
        )
        self.assertEqual(
    ...
    def test_misc3_is_static_file(self):
        ...
        # Invalid URLs
        self.assertIsNone(
            root.get_static_file("/test_http/i-dont-exist"), "File doesn't exist"
        )
        self.assertIsNone(
            root.get_static_file("/test_http/__manifest__.py"), "File is not static"
        )
    ...
    def test_misc5_upload_file_retry(self):
        ...
        with patch.object(controllers, "should_fail", True), StringIO(
            "Hello world!"
        ) as file:
            res = self.url_open(
                "/test_http/upload_file", files={"ufile": file}, timeout=None
            )
            self.assertEqual(res.status_code, 200)
...
class TestHttpCors(TestHttpBase):
    ...
    def test_cors2_json(self):
        res_opt = self.opener.options(
            f"{self.base_url()}/test_http/cors_json", timeout=10, allow_redirects=False
        )
        ...
        res_post = self.url_open(
            "/test_http/cors_json", data=json.dumps({"params": {}}), headers=CT_JSON
        )
        self.assertEqual(res_post.status_code, 200)
        self.assertEqual(res_post.headers.get("Access-Control-Allow-Origin"), "*")
...
class TestHttpEnsureDb(TestHttpBase):
    ...
    def test_ensure_db0_db_selector(self):
        ...
        self.assertEqual(
            urlparse(res.headers.get("Location", "")).path, "/web/database/selector"
        )
    ...
    def test_ensure_db1_grant_db(self):
        ...
        self.assertEqual(
            urlparse(res.headers.get("Location", "")).path, "/test_http/ensure_db"
        )
        self.assertEqual(
