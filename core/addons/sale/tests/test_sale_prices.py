...
class TestSalePrices(SaleCommon):
    ...
    def test_update_prices(self):
        ...
        pricelist.item_ids = [
            fields.Command.create({"percent_price": 5.0, "compute_price": "percentage"})
        ]
    ...
    def test_sale_tax_mapping(self):
        ...
        fiscal_pos = self.env["account.fiscal.position"].create(
            {
                ...
                "tax_ids": [
                    Command.create({"tax_src_id": tax_a.id, "tax_dest_id": tax_b.id})
                ],
            }
        )
        ...
        self.assertEqual(
            self.empty_order.order_line.tax_id.id,
            tax_b.id,
    ...
    def test_fiscalposition_application(self):
        ...
        self.assertRecordValues(
            sale_order.order_line, [{"price_unit": 106, "price_subtotal": 100}]
        )
        ...
        sale_order = order_form.save()
        self.assertRecordValues(
            sale_order.order_line, [{"price_unit": 100, "price_subtotal": 100}]
        )
    ...
    def test_product_quantity_rounding(self):
        ...
        order.action_confirm()
        line = order.order_line
        quantity_precision = self.env["decimal.precision"].precision_get(
            "Product Unit of Measure"
        )
        expected_price_subtotal = line.price_unit * float_round(
            product_uom_qty, precision_digits=quantity_precision
        )
