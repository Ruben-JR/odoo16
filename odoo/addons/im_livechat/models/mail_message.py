...
class MailMessage(models.Model):
    ...
    def _message_format(self, fnames, format_reply=True, legacy=False):
        ...
        vals_list = super()._message_format(
            fnames=fnames, format_reply=format_reply, legacy=legacy
        )
        for vals in vals_list:
            message_sudo = self.browse(vals["id"]).sudo().with_prefetch(self.ids)
            mail_channel = (
                self.env["mail.channel"].browse(message_sudo.res_id)
                if message_sudo.model == "mail.channel"
            ...
            if mail_channel.channel_type == "livechat":
                ...
                if (
                    mail_channel.chatbot_current_step_id
                    and message_sudo.author_id
                    == mail_channel.chatbot_current_step_id.chatbot_script_id.operator_partner_id
                ):
                    chatbot_message_id = (
                        self.env["chatbot.message"]
                        .sudo()
                        .search([("mail_message_id", "=", message_sudo.id)], limit=1)
                    )
                    if chatbot_message_id.script_step_id:
                        vals[
                            "chatbot_script_step_id"
                        ] = chatbot_message_id.script_step_id.id
                        if (
                            chatbot_message_id.script_step_id.step_type
                            == "question_selection"
                        ):
                    ...
                    if chatbot_message_id.user_script_answer_id:
                        ...
                        ] = chatbot_message_id.user_script_answer_id.id
        return vals_list
