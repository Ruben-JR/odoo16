    ...
    * Official doc for EHF Billing 3.0 is the OpenPeppol BIS 3 doc +
    ...
    def _get_partner_party_tax_scheme_vals_list(self, partner, role):
        ...
        for vals in vals_list:
            ...
            if (
                partner.country_id.code == "AU"
                and partner.vat
                and not partner.vat.upper().startswith("AU")
            ):
                vals["company_id"] = "AU" + partner.vat

            if (
                partner.country_id.code == "LU"
                and "l10n_lu_peppol_identifier" in partner._fields
                and partner.l10n_lu_peppol_identifier
            ):
    ...
    def _get_partner_party_vals(self, partner, role):
        ...
        vals["endpoint_id_attrs"] = {
            "schemeID": COUNTRY_EAS.get(partner.country_id.code)
        }

        if (
            partner.country_code == "NO"
            and "l10n_no_bronnoysund_number" in partner._fields
        ):
            vals.update(
                {
        ...
        if (
            partner.country_id.code == "SG"
            and "l10n_sg_unique_entity_number" in partner._fields
        ):
            vals.update(
                {
    ...
    def _invoice_constraints_cen_en16931_ubl(self, invoice, vals):
        ...
        constraints = {
            ...
            if vals["vals"]["payment_means_vals_list"][0]["payment_means_code"]
            in (30, 58)
            else None,
            ...
            "cen_en16931_seller_EAS": self._check_required_fields(
                vals["vals"]["accounting_supplier_party_vals"]["party_vals"][
                    "endpoint_id_attrs"
                ],
                "schemeID",
                _(
                    "No Electronic Address Scheme (EAS) could be found for %s.",
                    vals["customer"].name,
                ),
            ),
            ...
            "cen_en16931_buyer_EAS": self._check_required_fields(
                vals["vals"]["accounting_customer_party_vals"]["party_vals"][
                    "endpoint_id_attrs"
                ],
                "schemeID",
                _(
            ...
            "cen_en16931_delivery_country_code": self._check_required_fields(
                vals["vals"]["delivery_vals_list"][0],
                "delivery_location_vals",
                _(
            ...
            and self._check_required_fields(
                vals["vals"]["invoice_period_vals_list"][0],
                ["start_date", "end_date"],
                _(
                    "For intracommunity supply, the actual delivery date or the invoicing period should be included."
                ),
    ...
    def _invoice_constraints_peppol_en16931_ubl(self, invoice, vals):
        ...
        constraints = {
            ...
            if self._check_required_fields(vals["vals"], "buyer_reference")
            and self._check_required_fields(vals["vals"], "order_reference")
            else None,
        }
            ...
            constraints.update(
                {
                    ...
                    if "l10n_nl_oin" not in vals["supplier"]._fields
                    or "l10n_nl_kvk" not in vals["supplier"]._fields
                    else "",
                ...
                constraints.update(
                    {
                        ...
                        if "l10n_nl_oin" not in vals["customer"]._fields
                        or "l10n_nl_kvk" not in vals["customer"]._fields
                        else "",
                    }
                )
            ...
            constraints.update(
                {
                    ...
                    if "l10n_no_bronnoysund_number" not in vals["supplier"]._fields
                    or not vals["supplier"].l10n_no_bronnoysund_number
                    else "",
                }
            )
            ...
            constraints.update(
                {
                    ...
                    if "l10n_no_bronnoysund_number" not in vals["customer"]._fields
                    or not vals["customer"].l10n_no_bronnoysund_number
                    else "",
                }
            )
