...
class Meeting(models.Model):
    ...
    def default_get(self, fields):
        ...
        if self.env.context.get("default_res_model") and not self.env.context.get(
        ...
        if (
            ...
            and self.env.context["active_model"] != "calendar.event"
        ):
    ...
    def _default_partners(self):
        ...
        if (
            self._context.get("active_model") == "res.partner"
            and active_id
    ...
    def create(self, vals_list):
        ...
        valid_activity_model_ids = (
            model_name
            and self.env[model_name]
            .sudo()
            .browse(model_ids)
            .filtered(lambda m: "activity_ids" in m)
            .ids
            or []
        )
        ...
        ).attendee_ids._send_mail_to_attendees(
            self.env.ref(
    ...
    def _compute_field_value(self, field):
        if field.compute_sudo:
            return super(
                Meeting, self.with_context(prefetch_fields=False)
    ...
    def _read(self, fields):
        ...
        for field_name in private_fields:
            ...
            replacement = field.convert_to_cache(
                _("Busy") if field_name == "name" else False, others_private_events
            )
        ...
        if update_recurrence:
            if break_recurrence:
                ...
                if recurrence_update_setting == "all_events":
                    ...
                    self.recurrence_id._write_events(
                        values, dtstart=future_update_start
                    )
        else:
            super().write(values)
    ...
    def name_get(self):
        ...
        hidden = self.filtered(
            lambda evt: evt.privacy == "private"
            and evt.user_id.id != self.env.uid
            and self.env.user.partner_id not in evt.partner_ids
        )
        ...
        if not self.env.su and private_fields:
            ...
            self.env["bus.bus"]._sendone(
                ...
                "mail.simple_notification",
                {
                    ...
                    "message": _(
                        "Grouping by %s is not allowed on private events.",
                        ", ".join(
                            [
    ...
    def _create_videocall_channel_id(self, name, partner_ids):
        videocall_channel_id = self.env["mail.channel"].create_group(
        ...
        if self.recurrency:
            ...
            recurrent_events_without_channel.videocall_channel_id = (
                videocall_channel_id["id"]
            )
    ...
    def _split_recurrence(self, time_values):
        ...
        if self.follow_recurrence and self.recurrency:
            ...
            previous_week_day_field = weekday_to_field(
                self.recurrence_id.base_event_id._get_start_date().weekday()
            )
    ...
    def _rewrite_recurrence(self, values, time_values, recurrence_values):
        ...
        [base_time_values] = self.recurrence_id.base_event_id.read(
            ["start", "stop", "allday"]
        )
        update_dict = {}
        ...
        if start_update or stop_update:
            ...
            if stop_update:
                if not start_update:
                    ...
                    start_date = base_time_values["start_date"] + (
        ...
        if time_values or recurrence_values:
            ...
            old_recurrence_values = {
                field: rec_vals.pop(field) for field in rec_fields if field in rec_vals
            }
            base_event.write({**values, **time_values})
            # Delete all events except the base event and the currently modified
            expandable_events = self.recurrence_id.calendar_event_ids - (
                self.recurrence_id.base_event_id + self
            )
    ...
    def _find_attendee_batch(self):
        ...
        self_attendees = self.attendee_ids.filtered(
        ...
        for event in remaining_events:
            ...
            if (
                event.partner_id in event_checked_partners
                and event.partner_id in event_partners
            ):
                filter_events |= event
                result[event.id] = event.attendee_ids.filtered(
                    lambda attendee: attendee.partner_id == event.partner_id
                )[:1]
        ...
        for event in remaining_events:
            ...
            attendee = event.attendee_ids.filtered(
                lambda a: a.partner_id in event_checked_partners
                and a.state != "needsAction"
            )
    ...
    def _get_ics_file(self):
        ...
        for meeting in self:
            ...
            if not is_html_empty(meeting.description):
                if (
                    "appointment_type_id" in meeting._fields
