...
class website_form_model(models.Model):
    ...
    def _get_form_writable_fields(self):
        ...
        included = {
            ...
            .sudo()
            .search(
                [("model_id", "=", self.id), ("website_form_blacklisted", "=", False)]
            )
        }
        return {
            k: v
            for k, v in self.get_authorized_fields(self.model).items()
            if k in included
        }
    ...
    def get_authorized_fields(self, model_name):
        ...
        for field in list(fields_get):
            ...
            if (
                ...
                or field in MAGIC_FIELDS
                or fields_get[field]["type"] in ["many2one_reference", "properties"]
            ):
