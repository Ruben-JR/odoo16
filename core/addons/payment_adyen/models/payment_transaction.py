...
class PaymentTransaction(models.Model):
    ...
    def _get_tx_from_notification_data(self, provider_code, notification_data):
        ...
        if not reference:
            raise ValidationError(
                "Adyen: " + _("Received data with missing merchant reference")
            )

        event_code = notification_data.get(
    ...
    def _adyen_create_refund_tx_from_notification_data(
        ...
        return source_tx._create_refund_transaction(
            amount_to_refund=converted_amount,
    ...
    def _process_notification_data(self, notification_data):
        ...
        if "pspReference" in notification_data and event_code in [
            "AUTHORISATION",
            "REFUND",
        ]:
        ...
        refusal_reason = notification_data.get(
            "refusalReason"
        ) or notification_data.get("reason")
        if not payment_state:
            raise ValidationError(
                "Adyen: " + _("Received data with missing payment state.")
            )
        ...
        if payment_state in RESULT_CODES_MAPPING["pending"]:
            ...
            if event_code in ["AUTHORISATION", "REFUND"]:
                ...
                self._log_message_on_linked_documents(
                    _(
                        "The void of the transaction with reference %s failed.",
                        self.reference,
                    )
                )
            else:  # 'CAPTURE'
                ...
                self._log_message_on_linked_documents(
                    _(
                        "The capture of the transaction with reference %s failed.",
                        self.reference,
                    )
                )
        ...
        token = self.env["payment.token"].create(
            {
                ...
                "adyen_shopper_reference": additional_data[
