...
class EventBoothRegistration(models.Model):
    ...
    contact_mobile = fields.Char(
        ...
        compute="_compute_contact_mobile",
        readonly=False,
        store=True,
    )
    ...
    def action_confirm(self):
        for registration in self:
            values = {
                field: registration[field].id
                if isinstance(registration[field], models.BaseModel)
    ...
    def _cancel_pending_registrations(self):
        body = "<p>%(message)s: <ul>%(booth_names)s</ul></p>" % {
            ...
            "booth_names": "".join(
                "<li>%s</li>" % booth.display_name for booth in self.event_booth_id
            ),
        }
