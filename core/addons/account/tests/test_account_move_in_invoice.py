...
class TestAccountMoveInInvoiceOnchanges(AccountTestInvoicingCommon):
    ...
    def _assert_payment_move_state(
        self, move_type, amount, counterpart_values_list, payment_state
    ):
        ...
        def create_reverse(move, amount):
            ...
            (move + reverse_move).line_ids.filtered(
                lambda line: line.account_type
                in ("asset_receivable", "liability_payable")
            ).reconcile()
    ...
    def test_invoice_sent_to_additional_partner(self):
        ...
        additional_partner_mail = self.env["mail.mail"].search(
            [("res_id", "=", invoice.id), ("recipient_ids", "=", additional_partner.id)]
        )
        self.assertTrue(additional_partner_mail)

        self.assertIn(
            "access_token=",
            additional_partner_mail.body_html,
