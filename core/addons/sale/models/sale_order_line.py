...
class SaleOrderLine(models.Model):
    ...
    salesman_id = fields.Many2one(
        related="order_id.user_id", string="Salesperson", store=True, precompute=True
    )
    ...
    price_subtotal = fields.Monetary(
        string="Subtotal", compute="_compute_amount", store=True, precompute=True
    )
    price_tax = fields.Float(
        string="Total Tax", compute="_compute_amount", store=True, precompute=True
    )
    ...
    product_type = fields.Selection(
        related="product_id.detailed_type", depends=["product_id"]
    )
    ...
    def _get_sale_order_line_multiline_description_variants(self):
        ...
        """
        if (
            not self.product_custom_attribute_value_ids
            and not self.product_no_variant_attribute_value_ids
        ):
            return ""
            ...
            if line.product_id and line.product_uom_qty and line.product_uom:
                line.product_packaging_id = (
                    line.product_id.packaging_ids.filtered(
                        "sales"
                    )._find_suitable_product_packaging(
                        line.product_uom_qty, line.product_uom
                    )
                    or line.product_packaging_id
                )
        ...
        for line in self:
            if not line.product_packaging_id:
                ...
                packaging_uom_qty = line.product_uom._compute_quantity(
                    line.product_uom_qty, packaging_uom
                )
        ...
        for item in data:
            ...
            if so_line.product_uom.category_id == uom.category_id:
                qty = uom._compute_quantity(
                    item["unit_amount"], so_line.product_uom, rounding_method="HALF-UP"
                )
            else:
    ...
    def _compute_product_updatable(self):
        for line in self:
            if line.state in ["done", "cancel"] or (
                line.state == "sale"
                and (line.qty_invoiced > 0 or line.qty_delivered > 0)
            ):
        ...
        for vals in vals_list:
            if vals.get("discount"):
                vals["discount"] = float_round(
                    vals["discount"], precision_digits=precision
                )
    ...
    def create(self, vals_list):
        ...
        for line in lines:
            if line.product_id and line.state == "sale":
                ...
                if (
                    line.product_id.expense_policy not in [False, "no"]
                    and not line.order_id.analytic_account_id
                ):
    ...
    def write(self, values):
        ...
        if (
            ...
            and "product_packaging_qty" in values
            and "product_packaging_id" not in values
        ):
