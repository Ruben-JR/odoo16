...
class StorageCategory(models.Model):
    ...
    capacity_ids = fields.One2many(
        "stock.storage.category.capacity", "storage_category_id", copy=True
    )
...
class StorageCategoryProductCapacity(models.Model):
    ...
    storage_category_id = fields.Many2one(
        "stock.storage.category", ondelete="cascade", required=True, index=True
    )
    ...
    package_type_id = fields.Many2one(
        "stock.package.type", "Package Type", ondelete="cascade", check_company=True
    )
    quantity = fields.Float("Quantity", required=True)
    product_uom_id = fields.Many2one(related="product_id.uom_id")
    company_id = fields.Many2one(
        "res.company", "Company", related="storage_category_id.company_id"
    )
