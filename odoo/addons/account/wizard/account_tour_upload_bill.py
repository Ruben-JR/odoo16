...
class AccountTourUploadBill(models.TransientModel):
    ...
    selection = fields.Selection(
        selection=lambda self: self._selection_values(), default="sample"
    )
        ...
        addr = [
            ...
            for x in [
                ...
                " ".join(
                    [
                        x
                        for x in [self.env.company.state_id.name, self.env.company.zip]
                        if x
                    ]
                ),
        ...
        if journal_alias.alias_name and journal_alias.alias_domain:
            values.append(
                (
                    "email",
                    _(
                        "Or send a bill to %s@%s",
                        journal_alias.alias_name,
                        journal_alias.alias_domain,
                    ),
                )
            )
        return values
    ...
    def apply(self):
        if self._context.get("active_model") == "account.journal" and self._context.get(
            "active_ids"
        ):
            purchase_journal = self.env["account.journal"].browse(
        ...
        if self.selection == "upload":
            return purchase_journal.with_context(
                default_journal_id=purchase_journal.id, default_move_type="in_invoice"
            ).create_document_from_attachment(attachment_ids=self.attachment_ids.ids)
            ...
            if tools.config["test_enable"] or tools.config["test_file"]:
                ...
                bodies = self.env["ir.actions.report"]._prepare_html(
            ...
            email_alias = "%s@%s" % (
                purchase_journal.alias_name,
                purchase_journal.alias_domain,
            )
            new_wizard = self.env["account.tour.upload.bill.email.confirm"].create(
...
class AccountTourUploadBillEmailConfirm(models.TransientModel):
    ...
    def apply(self):
        ...
        bill_ids = (
            ...
            .search([("journal_id", "=", purchase_journal.id)])
            .ids
        )
