...
class TestAngloSaxonAccounting(TestValuationReconciliationCommon):
    def test_cogs_should_use_price_from_the_right_company(self):
        ...
        product.with_company(
            company_a_data["company"]
        ).standard_price = company_a_standard_price
        company_b_standard_price = 10.0
        product.with_company(
            company_b_data["company"]
        ).standard_price = company_b_standard_price
        ...
        company_a_invoice = self._create_invoice_for_so(
            company_a_order, product, "2021-01-10"
        )
