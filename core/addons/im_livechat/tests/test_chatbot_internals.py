...
class ChatbotCase(chatbot_common.ChatbotCase):
    def test_chatbot_duplicate(self):
        ...
        step_pricing_contact_us_copy = chatbot_copy.script_step_ids.filtered(
            lambda step: "For any pricing question, feel free ton contact us at pricing@mycompany.com"
            in step.message
        )
        ...
        self.assertEqual(
            step_email_copy.triggering_answer_ids.name, "I want to buy the software"
        )
        self.assertNotEqual(
    ...
    def test_chatbot_steps(self):
        ...
        with self.assertRaises(
            ValidationError, msg="Should raise an error since it's not a valid email"
        ):
        ...
        self.assertEqual(
            mail_channel.chatbot_current_step_id, self.step_email_validated
        )
