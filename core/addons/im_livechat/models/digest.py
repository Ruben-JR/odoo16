...
class Digest(models.Model):
    ...
    def _compute_kpi_livechat_rating_value(self):
        ...
        for record in self:
            ...
            record.kpi_livechat_rating_value = (
                ratings["great"] * 100 / sum(ratings.values())
                if sum(ratings.values())
                else 0
            )
    ...
    def _compute_kpi_livechat_conversations_value(self):
        for record in self:
            start, end, company = record._get_kpi_compute_parameters()
            record.kpi_livechat_conversations_value = self.env[
                "mail.channel"
            ].search_count(
                [
    ...
    def _compute_kpi_livechat_response_value(self):
        for record in self:
            start, end, company = record._get_kpi_compute_parameters()
            response_time = (
                self.env["im_livechat.report.operator"]
                .sudo()
                ._read_group(
                    [
                        ("start_date", ">=", start),
                    ...
                    ["partner_id", "time_to_answer"],
                    ["partner_id"],
                )
            )
            record.kpi_livechat_response_value = (
                "%.2f" % sum([response["time_to_answer"] for response in response_time])
                or 0
            )
