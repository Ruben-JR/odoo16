...
class StockMove(models.Model):
    ...
    analytic_account_line_id = fields.Many2one("account.analytic.line", copy=False)
    ...
    def _prepare_common_svl_vals(self):
        ...
        return {
            ...
            and "%s - %s" % (self.reference, self.product_id.name)
    ...
    def _action_done(self, cancel_backorder=False):
        ...
        valued_moves = {
            value_type: moves.exists() for value_type, moves in valued_moves.items()
        }
        ...
        company = (
            valued_moves["in"].mapped("company_id")
            and valued_moves["in"].mapped("company_id")[0]
            or self.env.company
        )
    ...
    def _get_src_account(self, accounts_data):
        return (
            ...
            or accounts_data["stock_input"].id
        )

    def _get_dest_account(self, accounts_data):
        return (
            self.location_dest_id.valuation_in_account_id.id
            or accounts_data["stock_output"].id
        )
    ...
    def _prepare_analytic_lines(self):
        ...
        if self.state != "done":
            ...
            and not self._ignore_automatic_valuation()
        ):
            accounts_data = self.product_id.product_tmpl_id.get_product_accounts()
            account_valuation = accounts_data.get("stock_valuation", False)
            analytic_line_vals = (
                self.stock_valuation_layer_ids.account_move_id.line_ids.filtered(
                    lambda l: l.account_id == account_valuation
                )._prepare_analytic_lines()
            )
            amount = -sum(
                sum(vals["amount"] for vals in lists) for lists in analytic_line_vals
            )
            unit_amount = -sum(
                sum(vals["unit_amount"] for vals in lists)
                for lists in analytic_line_vals
            )
        ...
        if self.analytic_account_line_id:
            ...
            return self._generate_analytic_lines_data(unit_amount, amount)
    ...
    def _prepare_account_move_vals(
        ...
        return {
            ...
            "move_type": "entry",
            "is_storno": self.env.context.get("is_returned")
            and self.env.company.account_storno,
        }
    ...
    def _account_analytic_entry_move(self):
        ...
        analytic_lines = (
            self.env["account.analytic.line"].sudo().create(analytic_lines_vals)
        )
        for move_id, analytic_line in zip(moves_to_link, analytic_lines):
            self.env["stock.move"].browse(
                move_id
            ).analytic_account_line_id = analytic_line
    ...
    def _account_entry_move(self, qty, description, svl_id, cost):
        ...
        if self._is_in():
            if self._is_returned(valued_type="in"):
                am_vals.append(
                    self.with_company(company_to)
                    .with_context(is_returned=True)
        ...
        if self._is_out():
            ...
            if self._is_returned(valued_type="out"):
                am_vals.append(
                    self.with_company(company_from)
                    .with_context(is_returned=True)
                    ._prepare_account_move_vals(
                        acc_valuation,
                        acc_src,
                ...
                am_vals.append(
                    self.with_company(company_from)._prepare_account_move_vals(
                        acc_valuation,
                        acc_dest,
                        journal_id,
                        qty,
    ...
    def _is_returned(self, valued_type):
        ...
        if valued_type == "out":
            return (
                self.location_dest_id and self.location_dest_id.usage == "supplier"
