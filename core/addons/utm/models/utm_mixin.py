...
class UtmMixin(models.AbstractModel):
    ...
    def _get_unique_names(self, model_name, names):
        ...
        seach_domain = expression.OR(
            [[("name", "ilike", name)] for name in names_without_counter]
        )
        existing_names = {
            vals["name"]
            for vals in self.env[model_name].search_read(seach_domain, ["name"])
        }
        ...
        for name in names:
            ...
            result.append(
                f"{name_without_counter} [{counter}]" if counter > 1 else name
            )
