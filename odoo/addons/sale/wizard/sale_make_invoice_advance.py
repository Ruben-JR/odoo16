...
class SaleAdvancePaymentInv(models.TransientModel):
    ...
    def _create_invoices(self, sale_orders):
        ...
        if self.advance_payment_method == "delivered":
            ...
            if not any(
                line.display_type and line.is_downpayment for line in order.order_line
            ):
            ...
            invoice = (
                ...
                .sudo()
                .create(self._prepare_invoice_values(order, down_payment_so_line))
                .with_user(self.env.uid)
            )  # Unsudo the invoice after creation
    ...
    def _prepare_so_line_values(self, order):
        ...
        if not float_is_zero(
            amount_total, precision_rounding=self.currency_id.rounding
        ):
            for line in order.order_line:
                ...
                for account, distribution in distrib_dict.items():
                    analytic_distribution[
                        account
                    ] = distribution * line.price_total + analytic_distribution.get(
                        account, 0
                    )
