...
class EventTypeMail(models.Model):
    ...
    def _selection_template_model(self):
        return super(EventTypeMail, self)._selection_template_model() + [
            ("sms.template", "SMS")
        ]
...
class EventMailScheduler(models.Model):
    ...
    @api.model
    def _selection_template_model(self):
        return super(EventMailScheduler, self)._selection_template_model() + [
            ("sms.template", "SMS")
        ]
    ...
    def execute(self):
        for scheduler in self:
            ...
            if (
                ...
                and scheduler.notification_type == "sms"
            ):
                ...
                if scheduler.scheduled_date <= now and (
                    ...
                    scheduler.event_id.registration_ids.filtered(
                        ...
                        template=scheduler.template_ref, mass_keep_log=True
                    )
                    scheduler.update(
                        {
                            "mail_done": True,
    ...
    def set_template_ref_model(self):
        ...
        if self.notification_type == "sms":
            ...
            self.template_ref = (
                "{},{}".format("sms.template", record.id) if record else False
            )
...
class EventMailRegistration(models.Model):
    ...
    def execute(self):
        ...
        todo = self.filtered(
            ...
            and reg_mail.scheduler_id.notification_type == "sms"
        )
        for reg_mail in todo:
            reg_mail.registration_id._message_sms_schedule_mass(
                template=reg_mail.scheduler_id.template_ref, mass_keep_log=True
            )
