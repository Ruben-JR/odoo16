...
class AccountMove(models.Model):
    ...
    def _get_invoice_reference(self):
        ...
        vendor_refs = [
            ref
            for ref in set(
                self.line_ids.mapped("purchase_line_id.order_id.partner_ref")
            )
            if ref
        ]
        if self.ref:
    ...
    def _onchange_partner_id(self):
        ...
        if (
            ...
            and self.currency_id != self.partner_id.property_purchase_currency_id
            and self.partner_id.property_purchase_currency_id.id
        ):
    ...
    def find_matching_subset_invoice_lines(self, invoice_lines, goal_total, timeout):
        ...
        try:
            subsets = _find_matching_subset_invoice_lines(
                sorted(
                    invoice_lines,
            ...
            _logger.warning(
                "Timed out during search of a matching subset of invoice lines"
            )
            return []
    ...
    def _match_purchase_orders(self, po_references, partner_id, amount_total, timeout):
        ...
        if po_references and amount_total:
            matching_pos |= self.env["purchase.order"].search(
            ...
            if matching_pos:
                matching_pos_invoice_lines = [
                    ...
                    for line in matching_pos.order_line
                    if line.product_qty
                ]
                ...
                if (
                    ...
                    < sum(
                        line["amount_to_invoice"] for line in matching_pos_invoice_lines
                    )
                    < amount_total + TOLERANCE
                ):
                    ...
                    il_subset = self.find_matching_subset_invoice_lines(
                        matching_pos_invoice_lines, amount_total, timeout
                    )
                    if il_subset:
                        return "subset_total_match", self.env[
                            "purchase.order.line"
                        ].union(*[line["line"] for line in il_subset])
                    else:
    ...
    def _find_and_set_purchase_orders(
        ...
        method, matched_po_lines = self._match_purchase_orders(
            po_references, partner_id, amount_total, timeout
        )
