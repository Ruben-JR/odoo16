...
class TestSalePurchase(TestCommonSalePurchaseNoChart):
    ...
    def test_sale_create_purchase(self):
        ...
        self.assertEqual(
            len(purchase_order),
            1,
            "Only one PO should have been created, from the 2 Sales orders",
        )
        self.assertEqual(
            len(purchase_order.order_line), 2, "The purchase order should have 2 lines"
        )
        self.assertEqual(
        ...
        self.assertEqual(
            purchase_order.state, "draft", "The created PO should be in draft state"
        )
    ...
    def test_update_ordered_sale_quantity(self):
        ...
        self.assertEqual(
            len(purchase_order2.order_line), 1, "The 2nd PO has only one line"
        )
        self.assertEqual(
