...
class MockIAPReveal(MockIAPEnrich):
    ...
    def mock_IAP_mine(self, mine, name_list=None, default_data=None, sim_error=None):
        ...
        with patch.object(
            CRMLeadMiningRequest, "_iap_contact_mining", side_effect=_iap_contact_mining
        ), patch.object(
            Lead, "create", autospec=True, wraps=Lead, side_effect=_crm_lead_create
        ):
            yield

    def _get_iap_company_data(self, base_name, service=None, add_values=None):
        company_data = super(MockIAPReveal, self)._get_iap_company_data(
