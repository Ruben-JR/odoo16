...
class TestSaleService(TestCommonSaleTimesheet):
    ...
    def test_sale_service(self):
        ...
        task = project.task_ids.filtered(
            lambda t: t.name
            == "%s - %s" % (self.sale_order.name, self.product_delivery_timesheet2.name)
        )
        ...
        self.assertEqual(
            ...
            "Sale Service: Task Email should be the same as the SO customer Email",
        )
        ...
        self.assertEqual(
            self.sale_order.invoice_status,
            "invoiced",
            "Sale Service: SO should be invoiced",
        )
        self.assertEqual(
    ...
    def test_delivered_quantity(self):
        ...
        self.assertEqual(
            so_line_deliver_new_task_project.qty_delivered,
    ...
    def test_sale_create_project(self):
        ...
        Stage = self.env["project.task.type"].with_context(
    ...
    def test_sale_task_in_project_with_project(self):
        ...
        self.assertEqual(
            ...
            "Changing the project on the SO should set the analytic account accordingly.",
        )
        ...
        self.product_order_timesheet4.write({"project_template_id": False})
        ...
        self.assertEqual(
            so_line1.task_id.project_id,
            self.project_global,
            "The project on so_line1's task should be project_global as configured on its parent sale_order",
        )
        self.assertTrue(
            so_line2.task_id,
            "so_line2 should create a task as its product's service_tracking is set as 'task_in_project'",
        )
        self.assertEqual(
            so_line2.task_id.project_id,
            self.project_global,
            "The project on so_line2's task should be project_global as configured on its parent sale_order",
        )
        self.assertFalse(
            so_line3.task_id.name,
            "so_line3 should not create a task as its product's service_tracking is set as 'project_only'",
        )
        self.assertNotEqual(
            so_line3.project_id,
            self.project_template,
            "so_line3 should create a new project and not directly use the configured template",
        )
        self.assertIn(
            self.project_template.name,
            so_line3.project_id.name,
            "The created project for so_line3 should use the configured template",
        )
    ...
    def test_billable_task_and_subtask(self):
        ...
        self.assertEqual(
            task2.partner_id,
            so_line_deliver_new_task_project.order_partner_id,
            "A new task in a billable project should have the same SO line as its project",
        )
    ...
    def test_remaining_hours_prepaid_services(self):
        ...
        """
        # 1) Check the remaining hours in the SOL containing a prepaid service product
        prepaid_service_sol = self.so.order_line.filtered(
        ...
        self.assertEqual(
            ...
            prepaid_service_sol.product_uom_qty - prepaid_service_sol.qty_delivered,
    ...
    def test_several_uom_sol_to_planned_hours(self):
        ...
        for task in tasks:
            self.assertEqual(
                task.planned_hours, planned_hours_for_uom[task.sale_line_id.name]
            )
    ...
    def test_add_product_analytic_account(self):
        ...
        self.assertEqual(
            ...
            {str(self.project_global.analytic_account_id.id): 100},
    ...
    def test_different_uom_sol_to_hours_when_confrim_sale_order(self):
        ...
        self.sale_order.action_confirm()
        project_allocated_hours = self.sale_order.order_line[
            0
        ].project_id.allocated_hours
        self.assertEqual(
            30, project_allocated_hours, "current set the project allocated hours."
        )
