...
class TestReorderingRule(TransactionCase):
    ...
    def test_reordering_rule_1(self):
        ...
        self.assertEqual(
            next_picking[0]
            .move_ids.filtered(lambda m: m.product_id == self.product_01)
            .product_qty,
            10,
        )
        self.assertEqual(
    ...
    def test_reordering_rule_2(self):
        ...
        customer_picking.action_confirm()
        self.assertEqual(
            self.product_01.with_context(location=subloc_1.id).virtual_available, -10
        )
        self.assertEqual(
            self.product_01.with_context(location=subloc_2.id).virtual_available, -10
        )
        ...
        self.assertEqual(
            ...
            2,
            "Not enough purchase order lines created.",
        )
        ...
        purchase_order.button_confirm()
        self.assertEqual(
            self.product_01.with_context(location=subloc_1.id).virtual_available, 5
        )
        self.assertEqual(
            self.product_01.with_context(location=subloc_2.id).virtual_available, 0
        )

        # increment the qty of the second po line
        purchase_order.order_line.filtered(
            lambda pol: pol.orderpoint_id == order_point_2
        ).with_context(debug=True).product_qty = 15
        self.assertEqual(
            self.product_01.with_context(location=subloc_1.id).virtual_available, 5
        )
        self.assertEqual(
        ...
        self.assertEqual(
            ...
            10,
        )  # 5 on the main loc, 5 on subloc_1

        self.assertEqual(purchase_order.picking_ids.move_ids[-1].product_qty, 5)
        self.assertEqual(
            purchase_order.picking_ids.move_ids[-1].location_dest_id,
            warehouse_1.lot_stock_id,
        )
    ...
    def test_reordering_rule_3(self):
        ...
        # create reordering rules
        # Required for `warehouse_id` to be visible in the view
        self.env["res.users"].browse(2).groups_id += self.env.ref(
    ...
    def test_replenish_report_1(self):
        ...
        po_line_product_mto = po.order_line.filtered(
    ...
    def test_multi_locations_and_reordering_rule(self):
        ...
        wizard = Form(
            self.env[(action.get("res_model"))].with_context(action["context"])
        ).save()
        wizard.process()
    ...
    def test_add_line_to_existing_draft_po(self):
            ...
            self.assertRecordValues(
                ...
                [
                    ...
                    },
                    {
                        ...
                        "date_planned": expected_delivery_date + td(days=1),
                    },
                ],
            )
        ...
        self.env["ir.config_parameter"].set_param(
            "purchase_stock.delta_days_merge", "2"
        )
        ...
        with freeze_time(dt.today() + td(days=2)):
            ...
            self.assertRecordValues(
                ...
                [
                    ...
                    {
                        "product_id": product_02.id,
                        "date_planned": expected_delivery_date + td(days=2),
                    },
                ],
            )
        ...
        with freeze_time(dt.today() + td(days=3)):
            ...
            op_02.action_replenish()
            self.assertEqual(
                po04.order_line.product_id,
                self.product_01,
                "There should be only a line for product 01",
            )
