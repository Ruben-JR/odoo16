...
class TestSaleProjectProfitability(TestProjectProfitabilityCommon, TestSaleCommon):
    def test_project_profitability(self):
        ...
        self.assertNotEqual(
            self.delivery_service_order_line.untaxed_amount_to_invoice, 0.0
        )
        ...
        self.assertNotEqual(
            self.delivery_service_order_line.untaxed_amount_invoiced, 0.0
        )
        invoice_type = service_policy_to_invoice_type[
            self.delivery_service_order_line.product_id.service_policy
        ]
            ...
            self.project._get_profitability_items(False),
            {
                "revenues": {
                    "data": [
                        {
                            ...
                            "sequence": sequence_per_invoice_type[invoice_type],
                            "to_invoice": sum(
                                service_sols.mapped("untaxed_amount_to_invoice")
                            ),
                            "invoiced": sum(
                    ...
                    "total": {
                        ...
                        + material_order_line.untaxed_amount_to_invoice,
                        "invoiced": sum(service_sols.mapped("untaxed_amount_invoiced"))
                        + material_order_line.untaxed_amount_invoiced,
                    },
                },
