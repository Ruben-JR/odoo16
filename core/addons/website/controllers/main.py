        ...
        for key, value in kw.items():
            if value and key in path_args:
                ...
                if isinstance(value, list) or isinstance(value, set):
                    fragments.append(
                        werkzeug.urls.url_encode([(key, item) for item in value])
                    )
                else:
...
class Website(Home):
    ...
    @http.route(
        ["/robots.txt"], type="http", auth="public", website=True, sitemap=False
    )
    ...
    def sitemap_xml_index(self, **kwargs):
        ...
        if not content:
            ...
            while True:
                ...
                if urls.strip():
                    ...
                    last_sitemap = create_sitemap(
                        "/sitemap-%d-%d.xml" % (current_website.id, pages), content
                    )
                else:
                    break
            ...
            if not pages:
                ...
                pages_with_website = [
                    "%d-%d" % (current_website.id, p) for p in range(1, pages + 1)
                ]
    ...
    def website_configurator(self, step=1, **kwargs):
        ...
        action_url = (
            "/web#action=website.website_configurator&menu_id=%s"
            % request.env.ref("website.menu_website_configuration").id
        )
        if step > 1:
    ...
    def get_suggested_link(self, needle, limit=10):
        ...
        for url, name in last_modified_pages.mapped(lambda p: (p.url, p.name)):
            if (
                ...
                or needle.lower() in url.lower()
                and url not in matching_urls
            ):
        ...
        for name, url, mod in current_website.get_suggested_controllers():
            if needle.lower() in name.lower() or needle.lower() in url.lower():
                ...
                icon = (
                    mod
                    and "<img src='%s' width='24px' height='24px' class='mr-2 rounded' /> "
                    % (module_sudo and module_sudo.icon or mod)
                    or ""
                )
    ...
    def get_dynamic_filter(
        self, filter_id, template_key, limit=None, search_domain=None, with_sample=False
    ):
        dynamic_filter = (
        ...
        return (
            dynamic_filter
            and dynamic_filter._render(template_key, limit, search_domain, with_sample)
            or []
        )

    @http.route(
        "/website/snippet/options_filters", type="json", auth="user", website=True
    )
    def get_dynamic_snippet_filters(self, model_name=None, search_domain=None):
        ...
        dynamic_filter = (
            request.env["website.snippet.filter"]
            .sudo()
            .search_read(domain, ["id", "name", "limit", "model_name"], order="id asc")
        )
        return dynamic_filter

    @http.route(
        "/website/snippet/filter_templates", type="json", auth="public", website=True
    )
    ...
    @http.route(
        "/website/get_current_currency", type="json", auth="public", website=True
    )
    ...
    @http.route(
        "/website/snippet/autocomplete", type="json", auth="public", website=True
    )
    def autocomplete(
        self,
        search_type=None,
    ...
    @http.route(
        "/website/get_switchable_related_views", type="json", auth="user", website=True
    )
    ...
    def google_console_search(self, key, **kwargs):
        ...
        trusted = gsc[
            gsc.startswith("google")
            and len("google") : gsc.endswith(".html")
            and -len(".html")
            or None
        ]
    ...
    @http.route(
        "/website/google_maps_api_key", type="json", auth="public", website=True
    )
    ...
    def _get_customize_data(self, keys, is_view_data):
        ...
        domain = expression.AND(
            [[("key", "in", keys)], request.website.website_domain()]
        )
        return Model.search(domain).filter_duplicate()

    @http.route(
        ["/website/theme_customize_data_get"], type="json", auth="user", website=True
    )
    ...
    @http.route(
        ["/website/theme_customize_data"], type="json", auth="user", website=True
    )
