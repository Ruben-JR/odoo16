...
class TestHrAttendanceOvertime(TransactionCase):
    ...
    def test_overtime_company_settings(self):
        ...
        overtime = self.env["hr.attendance.overtime"].search_count(
            [("employee_id", "=", self.employee.id), ("date", "=", date(2021, 1, 4))]
        )
        self.assertFalse(overtime, "No overtime should be created")

        self.company.write(
            {"hr_attendance_overtime": True, "overtime_start_date": date(2021, 1, 1)}
        )
        overtime = self.env["hr.attendance.overtime"].search_count(
            [("employee_id", "=", self.employee.id), ("date", "=", date(2021, 1, 4))]
        )
        ...
        self.assertFalse(
            overtime, "No overtime should be created before the start date"
        )
    ...
    def test_overtime_unclosed(self):
        ...
        self.assertEqual(
            overtime.duration,
            0,
            "Overtime entry has been reset due to an unclosed attendance.",
        )
    ...
    def test_overtime_company_threshold(self):
        ...
        self.assertAlmostEqual(
            overtime.duration, 10 / 60, msg="Overtime should be equal to 10 minutes."
        )
    ...
    def test_overtime_both_threshold(self):
        ...
        self.assertTrue(
            overtime,
            "Overtime entry should exist since the employee threshold has been lowered.",
        )
        self.assertAlmostEqual(
            overtime.duration, -(5 / 60), msg="Overtime should be equal to -5 minutes."
        )
        ...
        self.assertFalse(
            overtime,
            "Overtime entry should be unlinked since both overtime cancel each other.",
        )
