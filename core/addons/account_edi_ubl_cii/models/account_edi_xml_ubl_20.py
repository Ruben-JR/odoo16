...
class AccountEdiXmlUBL20(models.AbstractModel):
        ...
        return [
            {
                ...
                "registration_address_vals": self._get_partner_address_vals(
                    commercial_partner
                ),
            }
        ]
    ...
    def _get_partner_party_vals(self, partner, role):
        return {
            ...
            "party_legal_entity_vals": self._get_partner_party_legal_entity_vals_list(
                partner
            ),
    ...
    def _get_financial_account_vals(self, partner_bank):
        ...
        if partner_bank.bank_id:
            vals[
                "financial_institution_branch_vals"
    ...
    def _get_invoice_line_price_vals(self, line):
        ...
        return {
            ...
            "product_price_dp": self.env["decimal.precision"].precision_get(
                "Product Price"
            ),
    ...
    def _export_invoice_vals(self, invoice):
        ...
        vals = {
            ...
            "vals": {
                ...
                "note_vals": [html2plaintext(invoice.narration)]
        ...
        constraints.update(
            {
                ...
                "ubl20_commercial_customer_name_required": self._check_required_fields(
    ...
    def _import_fill_invoice_form(self, journal, tree, invoice, qty_factor):
        ...
        self._import_retrieve_and_fill_partner(
            invoice, name=name, phone=phone, mail=mail, vat=vat
        )
        ...
        payment_terms_node = tree.find(
            "./{*}PaymentTerms/{*}Note"
        )  # e.g. 'Payment within 10 days, 2% discount'
        ...
        logs += self._import_fill_invoice_allowance_charge(
        ...
        for i, invl_el in enumerate(tree.findall("./{*}" + invoice_line_tag)):
            ...
            invl_logs = self._import_fill_invoice_line_form(
                journal, invl_el, invoice, invoice_line, qty_factor
            )
            logs += invl_logs
    ...
    def _import_fill_invoice_line_form(
        ...
        if description_node is not None:
            ...
            invoice_line.name = (
                name_node.text
            )  # Fallback on Name if Description is not found.
        ...
        self._import_fill_invoice_line_values(
            tree, xpath_dict, invoice_line, qty_factor
        )
        ...
        return self._import_fill_invoice_line_taxes(
            journal, tax_nodes, invoice_line, inv_line_vals, logs
        )
    ...
    def _import_retrieve_partner_map(self, company, move_type="purchase"):
        ...
        def with_vat(tree, extra_domain):
            vat_node = tree.find(
                f".//{{*}}Accounting{role}Party/{{*}}Party//{{*}}CompanyID"
            )
            vat = None if vat_node is None else vat_node.text
            return self.env["account.edi.format"]._retrieve_partner_with_vat(
                vat, extra_domain
            )

        def with_phone_mail(tree, extra_domain):
            phone_node = tree.find(
                f".//{{*}}Accounting{role}Party/{{*}}Party//{{*}}Telephone"
            )
            mail_node = tree.find(
                f".//{{*}}Accounting{role}Party/{{*}}Party//{{*}}ElectronicMail"
            )

            phone = None if phone_node is None else phone_node.text
            mail = None if mail_node is None else mail_node.text
            return self.env["account.edi.format"]._retrieve_partner_with_phone_mail(
                phone, mail, extra_domain
            )

        def with_name(tree, extra_domain):
            name_node = tree.find(
                f".//{{*}}Accounting{role}Party/{{*}}Party//{{*}}Name"
            )
    ...
    def _import_retrieve_product_map(self, company):
        def with_code_barcode(tree, extra_domain):
            ...
            return self.env["product.product"].search(
                extra_domain + expression.OR(domains), limit=1
            )
        ...
        def with_name(tree, extra_domain):
            ...
            return self.env["product.product"].search(
                extra_domain + [("name", "ilike", name_node.text)], limit=1
            )
