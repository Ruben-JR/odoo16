...
class TestImLivechatReport(TestImLivechatCommon):
    ...
    def setUpClass(cls):
        ...
        with patch.object(
            type(cls.env["im_livechat.channel"]),
            "_get_available_users",
            lambda _: cls.operators,
        ):
            channel_id = cls.livechat_channel._open_livechat_mail_channel("Anonymous")[
                "id"
            ]
        ...
        partner_message = cls._create_message(
            channel, cls.operator, "2023-03-17 05:05:54"
        )
        partner_message |= cls._create_message(
            channel, cls.operator, "2023-03-17 09:15:54"
        )
    ...
    def test_im_livechat_report_operator(self):
        result = self.env["im_livechat.report.operator"].read_group(
            [], ["time_to_answer:avg", "duration:avg"], []
        )
