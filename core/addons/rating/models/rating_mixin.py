...
class RatingMixin(models.AbstractModel):
    ...
    def _compute_rating_stats(self):
        ...
        domain = expression.AND(
            [self._rating_domain(), [("rating", ">=", rating_data.RATING_LIMIT_MIN)]]
        )
    ...
    def _search_rating_avg(self, operator, value):
        if operator not in rating_data.OPERATOR_MAPPING:
            raise NotImplementedError(
                "This operator %s is not supported in this search method." % operator
            )
        rating_read_group = (
        ...
        res_ids = [
            ...
            if rating_data.OPERATOR_MAPPING[operator](
                float_compare(res["rating_avg"], value, 2), 0
            )
        ]
    ...
    def _compute_rating_satisfaction(self):
        ...
        read_group_res = self.env["rating.rating"]._read_group(
            domain, ["res_id", "rating"], groupby=["res_id", "rating"], lazy=False
        )
    ...
    def write(self, values):
        ...
        with self.env.norecompute():
            ...
            for record in self:
                if (
                    record._rec_name in values
                ):  # set the res_name of ratings to be recomputed
                ...
                if record._rating_get_parent_field_name() in values:
                    record.rating_ids.sudo().write(
                        {
                            "parent_res_id": record[
                                record._rating_get_parent_field_name()
                            ].id
                        }
                    )
    ...
    def _rating_get_access_token(self, partner=None):
        ...
        ratings = self.rating_ids.sudo().filtered(
    ...
    def rating_send_request(
        ...
        if force_send:
            ...
            )  # default value is True, should be set to false if not?
        for record in self:
            record.message_post_with_template(
                template.id,
                composition_mode=composition_mode,
                email_layout_xmlid=email_layout_xmlid
                if email_layout_xmlid is not None
    ...
    def _rating_get_repartition(self, add_stats=False, domain=None):
        ...
        if domain:
            base_domain += domain
        rg_data = self.env["rating.rating"].read_group(
        ...
        for rating_rg in rg_data:
            ...
            values[rating_val_round] = (
                values.get(rating_val_round, 0) + rating_rg["rating_count"]
            )
        # add other stats
        if add_stats:
