...
class TestMrpAccount(TestMrpCommon):
    ...
    def setUpClass(cls):
        ...
        cls.categ_standard = cls.env["product.category"].create(
            {"name": "STANDARD", "property_cost_method": "standard"}
        )
        cls.categ_real = cls.env["product.category"].create(
            {"name": "REAL", "property_cost_method": "fifo"}
        )
        ...
        cls.env["stock.move"].search(
            [("product_id", "in", [cls.product_bolt.id, cls.product_screw.id])]
        )._do_unreserve()
