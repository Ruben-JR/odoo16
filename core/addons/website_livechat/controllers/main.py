...
class WebsiteLivechat(LivechatController):
    ...
    @http.route(
        '/livechat/channel/<model("im_livechat.channel"):channel>',
        type="http",
        auth="public",
        website=True,
        sitemap=True,
    ...
    def channel_rating(self, channel, **kw):
        ...
        ratings = (
            ...
            .sudo()
            .search(domain, order="create_date desc", limit=100)
        )
        ...
        for grade in repartition:
            percentage[grade] = (
                round(repartition[grade] * 100.0 / sum(repartition.values()), 1)
                if sum(repartition.values())
                else 0
            )

        # filter only on the team users that worked on the last 100 ratings and get their detailed stat
        ratings_per_partner = {
            partner_id: dict(great=0, okay=0, bad=0)
            for partner_id in ratings.mapped("rated_partner_id.id")
        }
        total_ratings_per_partner = dict.fromkeys(
            ratings.mapped("rated_partner_id.id"), 0
        )
        ...
        for partner_id, rating in ratings_per_partner.items():
            for k, v in ratings_per_partner[partner_id].items():
                ratings_per_partner[partner_id][k] = round(
                    100 * v / total_ratings_per_partner[partner_id], 1
                )
    ...
    def get_session(
        ...
        **kwargs
        ...
        if visitor_sudo:
            anonymous_name = visitor_sudo.with_context(
                lang=visitor_sudo.lang_id.code
            ).display_name
        return super(WebsiteLivechat, self).get_session(
            channel_id,
            anonymous_name,
