...
class TestAPI(SavepointCaseWithUserDemo):
    ...
    def test_55_environment_lang(self):
        ...
        self.assertEqual(
            partner.with_context(lang=None).env.lang,
            None,
            "None lang context should have None env.lang",
        )
        self.assertEqual(
            partner.with_context(lang="en_US").env.lang,
            "en_US",
            "en_US active lang context should have en_US env.lang",
        )
        self.assertEqual(
            partner.with_context(lang="fr_FR").env.lang,
            "fr_FR",
            "fr_FR active lang context should have fr_FR env.lang",
        )
        self.assertEqual(
            partner.with_context(lang="nl_NL").env.lang,
            None,
            "Inactive lang context lang should have None env.lang",
        )
        self.assertEqual(
            partner.with_context(lang="Dummy").env.lang,
            None,
            "Ilegal lang context should have None env.lang",
        )
    ...
    def test_80_contains(self):
        ...
        with self.assertRaisesRegex(
            ...
            42 in ps
        with self.assertRaisesRegex(
            TypeError, r"inconsistent models in: ir\.ui\.menu.* in res\.partner.*"
        ):
    ...
    def test_80_set_operations(self):
        ...
        with self.assertRaisesRegex(
            TypeError, r"unsupported operand types in: res\.partner.* \+ 'string'"
        ):
            ps + "string"
        with self.assertRaisesRegex(
            TypeError, r"inconsistent models in: res\.partner.* \+ ir\.ui\.menu.*"
        ):
            ps + ms
        with self.assertRaisesRegex(
            TypeError, r"inconsistent models in: res\.partner.* - ir\.ui\.menu.*"
