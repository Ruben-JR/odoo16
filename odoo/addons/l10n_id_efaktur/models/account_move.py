...
class AccountMove(models.Model):
    ...
    l10n_id_replace_invoice_id = fields.Many2one(
        "account.move",
    ...
    l10n_id_csv_created = fields.Boolean(
        "CSV Created", compute="_compute_csv_created", copy=False
    )
    ...
    def _onchange_l10n_id_tax_number(self):
        for record in self:
            if (
                record.l10n_id_tax_number
                and record.move_type not in self.get_purchase_types()
            ):
                raise UserError(
                    _(
    ...
    def _constraint_kode_ppn(self):
        ...
        for move in self.filtered(
            lambda m: m.l10n_id_need_kode_transaksi and m.l10n_id_kode_transaksi != "08"
        ):
            if any(
                ppn_tag.id in line.tax_tag_ids.ids
                for line in move.line_ids
                if line.display_type == "product"
            ) and any(
                ppn_tag.id not in line.tax_tag_ids.ids
                for line in move.line_ids
                if line.display_type == "product"
            ):
                raise UserError(
                    _(
                        "Cannot mix VAT subject and Non-VAT subject items in the same invoice with this kode transaksi."
                    )
                )
        for move in self.filtered(
            lambda m: m.l10n_id_need_kode_transaksi and m.l10n_id_kode_transaksi == "08"
    ...
    def _constrains_l10n_id_tax_number(self):
        for record in self.filtered("l10n_id_tax_number"):
            if record.l10n_id_tax_number != re.sub(
                r"\D", "", record.l10n_id_tax_number
            ):
            ...
            if len(record.l10n_id_tax_number) != 16:
                ...
                record.l10n_id_tax_number[:2]
                not in dict(self._fields["l10n_id_kode_transaksi"].selection).keys()
            ):
            ...
            if move.l10n_id_need_kode_transaksi:
                if not move.l10n_id_kode_transaksi:
                    raise ValidationError(
                        _("You need to put a Kode Transaksi for this partner.")
                    )
                if move.l10n_id_replace_invoice_id.l10n_id_tax_number:
                    ...
                    move.l10n_id_tax_number = "%s1%s" % (
                        move.l10n_id_kode_transaksi,
                        rep_efaktur_str[3:],
                    )
                else:
                    efaktur = self.env["l10n_id_efaktur.efaktur.range"].pop_number(
                        move.company_id.id
                    )
                    if not efaktur:
                        raise ValidationError(
                            _(
                                "There is no Efaktur number available.  Please configure the range you get from the government in the e-Faktur menu. "
                            )
                        )
                    move.l10n_id_tax_number = "%s0%013d" % (
                        str(move.l10n_id_kode_transaksi),
                        efaktur,
                    )
    ...
    def reset_efaktur(self):
        ...
        for move in self:
            if move.l10n_id_attachment_id:
                raise UserError(
                    _(
                        "You have already generated the tax report for this document: %s",
                        move.name,
                    )
                )
            self.env["l10n_id_efaktur.efaktur.range"].push_number(
                move.company_id.id, move.l10n_id_tax_number[3:]
            )
            move.message_post(
    ...
    def download_csv(self):
        action = {
            ...
            + str(self.l10n_id_attachment_id.id)
            + "&filename_field=name&field=datas&download=true&name="
            + self.l10n_id_attachment_id.name,
            "target": "self",
        }
        return action
    ...
    def download_efaktur(self):
        ...
        for record in self:
            ...
            if record.partner_id.l10n_id_pkp and not record.l10n_id_tax_number:
                raise ValidationError(
                    _(
                        "Connect %(move_number)s with E-faktur to download this report",
    ...
    def _generate_efaktur_invoice(self, delimiter):
        ...
        for move in self.filtered(lambda m: m.state == "posted"):
            ...
            if move.l10n_id_replace_invoice_id:
                number_ref = (
                    str(move.l10n_id_replace_invoice_id.name)
                    + " replaced by "
                    + str(move.name)
                    + " "
                    + nik
            ...
            street = ", ".join(
                [x for x in (shipping_partner.street, shipping_partner.street2) if x]
            )
            ...
            if move.partner_id.vat and len(move.partner_id.vat) >= 12:
                ...
                not move.partner_id.vat or len(move.partner_id.vat) < 12
            ) and move.partner_id.l10n_id_nik:
            ...
            eTax["TANGGAL_FAKTUR"] = "{0}/{1}/{2}".format(
                move.invoice_date.day, move.invoice_date.month, move.invoice_date.year
            )
            eTax["NPWP"] = invoice_npwp
            eTax["NAMA"] = (
                move.partner_id.name
                if eTax["NPWP"] == "000000000000000"
                else move.partner_id.l10n_id_tax_name or move.partner_id.name
            )
            eTax["ALAMAT_LENGKAP"] = (
                move.partner_id.contact_address.replace("\n", "")
                if eTax["NPWP"] == "000000000000000"
                else shipping_partner.l10n_id_tax_address or street
            )
            eTax["JUMLAH_DPP"] = int(
                float_round(move.amount_untaxed, 0)
            )  # currency rounded to the unit
            eTax["JUMLAH_PPN"] = int(float_round(move.amount_tax, 0))
            eTax["ID_KETERANGAN_TAMBAHAN"] = (
                "1" if move.l10n_id_kode_transaksi == "07" else ""
            )
            eTax["REFERENSI"] = number_ref
            eTax["KODE_DOKUMEN_PENDUKUNG"] = "0"

            lines = move.line_ids.filtered(
                lambda x: x.move_id._is_downpayment()
                and x.price_unit < 0
                and x.display_type == "product"
            )
            eTax["FG_UANG_MUKA"] = 0
            eTax["UANG_MUKA_DPP"] = int(
                abs(sum(lines.mapped(lambda l: float_round(l.price_subtotal, 0))))
            )
            eTax["UANG_MUKA_PPN"] = int(
                abs(
                    sum(
                        lines.mapped(
                            lambda l: float_round(l.price_total - l.price_subtotal, 0)
                        )
                    )
                )
            )
            ...
            for line in move.line_ids.filtered(lambda l: l.display_type == "product"):
                ...
                invoice_line_total_price = (
                    line.price_subtotal / discount if discount else 0
                )
                invoice_line_unit_price = (
                    invoice_line_total_price / line.quantity if line.quantity else 0
                )
                ...
                if line.price_subtotal < 0:
                    ...
                    line_dict.update(
                        {
                            "DISKON": int(
                                float_round(
                                    invoice_line_total_price - line.price_subtotal, 0
                                )
                            ),
                    ...
                    invoice_line_discount_m2m = (
                        invoice_line_total_price - line.price_subtotal
                    )
            ...
            for sale in sales:
                of_values_list = (
                    ["OF"] + [str(sale[f]) for f in OF_HEAD_LIST[1:-2]] + ["0", "0"]
                )
        ...
        attachment = self.env["ir.attachment"].create(
            {
                "datas": out,
                "name": "efaktur_%s.csv"
                % (fields.Datetime.to_string(fields.Datetime.now()).replace(" ", "_")),
                "type": "binary",
            }
        )
