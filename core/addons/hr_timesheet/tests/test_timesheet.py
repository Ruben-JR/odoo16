    ...
    def assert_get_view_timesheet_encode_uom(self, expected):
        companies = self.env["res.company"].create(
            [
                {
                    "name": "foo",
                    "timesheet_encode_uom_id": self.env.ref("uom.product_uom_hour").id,
                },
                {
                    "name": "bar",
                    "timesheet_encode_uom_id": self.env.ref("uom.product_uom_day").id,
                },
            ]
        )
...
class TestTimesheet(TestCommonTimesheet):
    ...
    def test_log_timesheet(self):
        ...
        timesheet_uom = (
            self.project_customer.analytic_account_id.company_id.project_time_mode_id
        )
        ...
        self.assertEqual(
            ...
            self.project_customer.analytic_account_id,
    ...
    def test_create_unlink_project(self):
        ...
        self.assertFalse(
            non_tracked_project.analytic_account_id,
    ...
    def test_transfert_project(self):
        ...
        self.assertEqual(
            timesheet_count1, 0, "There are still timesheets linked to Project1"
        )
        self.assertEqual(
    ...
    def test_create_timesheet_employee_not_in_company(self):
        ...
        analytic_plan = self.env["account.analytic.plan"].create(
            {"name": "Plan Test", "company_id": company_3.id}
        )
    ...
    def test_get_view_timesheet_encode_uom(self):
        ...
        self.assert_get_view_timesheet_encode_uom(
            [
                ...
                (
                    "hr_timesheet.project_invoice_form",
                ...
                (
                    "hr_timesheet.view_task_tree2_inherited",
                    '//field[@name="planned_hours"]',
                    [None, "Initially Planned Days"],
                ),
                (
                    "hr_timesheet.view_task_project_user_graph_inherited",
                    '//field[@name="hours_planned"]',
                    [None, "Planned Days"],
                ),
                (
                    "hr_timesheet.timesheets_analysis_report_pivot_employee",
                    '//field[@name="unit_amount"]',
                    [None, "Days Spent"],
                ),
            ]
    ...
    def test_uom_change_timesheet(self):
        ...
        self.assertEqual(
            project.total_timesheet_time, 8, "Total timesheet time should be 8 hours"
        )
        self.assertEqual(
            project.timesheet_encode_uom_id.name,
            "Days",
            "Timesheet encode uom should be 'Days'",
        )
