...
class Team(models.Model):
    ...
    alias_user_id = fields.Many2one(
        "res.users",
        related="alias_id.alias_user_id",
        readonly=False,
        inherited=True,
        domain=lambda self: [
            (
    ...
    def _compute_opportunities_data(self):
        opportunity_data = self.env["crm.lead"]._read_group(
            ...
            ["expected_revenue:sum", "team_id"],
            ["team_id"],
        )
        counts = {
            datum["team_id"][0]: datum["team_id_count"] for datum in opportunity_data
        }
        amounts = {
            datum["team_id"][0]: datum["expected_revenue"] for datum in opportunity_data
        }
        for team in self:
    ...
    def _compute_opportunities_overdue_data(self):
        ...
        amounts = {
            datum["team_id"][0]: (datum["expected_revenue"])
        ...
        if frequencies:
            ...
            for frequency in frequencies:
                ...
                match = existing_noteam.filtered(
                    lambda frequ_nt: frequ_nt.variable == frequency.variable
    ...
    def _cron_assign_leads(self, work_days=None):
        ...
        if not work_days and assign_cron and assign_cron.active:
            if assign_cron.interval_type == "months":
                ...
                work_days = min(
                    30, assign_cron.interval_number * 7
                )  # max at 30 (better lead repartition)
            elif assign_cron.interval_type == "days":
                work_days = min(
                    30, assign_cron.interval_number * 1
                )  # max at 30 (better lead repartition)
            elif assign_cron.interval_type == "hours":
                work_days = max(
                    0.2, assign_cron.interval_number / 24
                )  # min at 0.2 to avoid small numbers issues
            elif assign_cron.interval_type == "minutes":
                work_days = max(
                    0.2, assign_cron.interval_number / 1440
                )  # min at 0.2 to avoid small numbers issues
    ...
    def _action_assign_leads(self, work_days=1):
        ...
        _logger.info(
            "### START Lead Assignment (%d teams, %d sales persons, %.2f work_days)",
            len(self),
    ...
    def _allocate_leads(self, work_days=1):
        ...
        for team in self:
            ...
            for lead in leads:
                if lead not in duplicates_lead_cache:
                    duplicates_lead_cache[lead] = lead._get_lead_duplicates(
        ...
        while population:
            ...
            teams_data[team]["leads"] = (
                teams_data[team]["leads"]
                .filtered(lambda l: l.id not in leads_done_ids)
                .exists()
            )
    ...
    def _allocate_leads_deduplicate(self, leads, duplicates_cache=None):
        ...
        leads_done_ids, leads_merged_ids, leads_dup_ids = (
            set(),
            set(),
            set(),
        )  # classification
        ...
        for lead in leads:
            if lead.id not in leads_done_ids:
                ...
                if lead not in duplicates_cache:
                    duplicates_cache[lead] = lead._get_lead_duplicates(
                        email=lead.email_from
                    )
