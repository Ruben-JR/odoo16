...
class TestProjectSubtasks(TestProjectCommon):
    ...
    def setUpClass(cls):
        ...
        cls.env["res.config.settings"].create({"group_subtask_project": True}).execute()
    ...
    def test_task_display_project_with_default_form(self):
        ...
        self.assertEqual(
            task.project_id, self.project_pigs, "The project should be assigned."
        )
        self.assertEqual(
            task.display_project_id,
            task.project_id,
            "The display project of a first layer task should be assigned to project_id.",
        )
    ...
    def test_task_display_project_with_any_task_form(self):
        ...
        for form_view in form_views:
            ...
            self.assertEqual(
                task.display_project_id,
                task.project_id,
                "The display project of a first layer task should be assigned to project_id, form_view name : %s."
                % form_view.name,
            )
    ...
    def test_subtask_display_project(self):
        ...
        self.assertEqual(
            self.task_1.child_ids.project_id,
            self.project_pigs,
            "The project should be assigned from the default project.",
        )
        self.assertFalse(
            self.task_1.child_ids.display_project_id,
            "The display project of a sub task should be false to project_id.",
        )
        ...
        self.assertEqual(
            self.task_1.child_ids.project_id,
            self.project_goats,
            "Changing display project id on a subtask should change project id",
        )
        ...
        self.assertFalse(
            self.task_1.child_ids.display_project_id,
            "Display Project of the task should be well assigned, to False",
        )
        self.assertEqual(
            self.task_1.child_ids.project_id,
            self.project_pigs,
            "Resetting display project to False on a subtask should change project id to parent project id",
        )
        ...
        self.assertEqual(
            self.task_1.project_id, self.project_goats, "Parent project should change."
        )
        self.assertFalse(
            self.task_1.child_ids.display_project_id,
            "Display Project of the task should be False",
        )
        self.assertEqual(
            self.task_1.child_ids.project_id,
            self.project_goats,
            "Resetting display project to False on a subtask should follow project of its parent",
        )
        ...
        with self.debug_mode():
            # 6)
            with Form(
                self.task_1.child_ids.with_context({"tracking_disable": True})
            ) as subtask_form:
                subtask_form.parent_id = self.env["project.task"]
            orphan_subtask = subtask_form.save()

            self.assertEqual(
                orphan_subtask.display_project_id,
                self.project_goats,
                "Display Project of the task should be well assigned",
            )
            self.assertEqual(
                orphan_subtask.project_id,
                self.project_goats,
                "Changing display project id on a subtask should change project id",
            )
            ...
            with Form(
                self.task_1.with_context({"tracking_disable": True})
            ) as task_form:
                task_form.child_ids.add(test_subtask_1)
                task_form.child_ids[0].display_project_id = self.project_goats
            with Form(
            ...
            self.assertEqual(
                orphan_subtask.project_id,
                self.project_pigs,
                "Removing parent should not change project",
            )
            self.assertEqual(
                orphan_subtask.display_project_id,
                self.project_pigs,
                "Removing parent should make the display project set as project.",
            )
    ...
    def test_subtask_stage(self):
        ...
        self.assertEqual(
            self.task_1.child_ids.stage_id,
            stage_a,
        ...
        self.assertEqual(
            self.task_1.child_ids.stage_id.name,
            "New",
    ...
    def test_copy_project_with_subtasks(self):
        self.env["project.task"].with_context({"mail_create_nolog": True}).create(
            {
                ...
                "child_ids": [
                    ...
                    Command.create(
                        {"name": "child 2", "display_project_id": self.project_goats.id}
                    ),
                    Command.create(
                        {"name": "child 3", "display_project_id": self.project_pigs.id}
                    ),
                    Command.create(
                        {
                            "name": "child 4 with subtask",
                            "child_ids": [
                                Command.create({"name": "child 5"}),
                                Command.create(
                                    {
                                        "name": "child 6 with project",
        ...
        task_count_with_subtasks_including_archived_in_project_goats = (
            self.project_goats.with_context(active_test=False).task_count_with_subtasks
        )
        ...
        self.assertEqual(
            project_goats_duplicated.with_context(
                active_test=False
