...
class MassMailingContactListRel(models.Model):
    ...
    list_id = fields.Many2one(
        "mailing.list", string="Mailing List", ondelete="cascade", required=True
    )
    ...
    def write(self, vals):
        if "opt_out" in vals and "unsubscription_date" not in vals:
            vals["unsubscription_date"] = (
                fields.Datetime.now() if vals["opt_out"] else False
            )
        if vals.get("unsubscription_date"):
            vals["opt_out"] = True
...
class MassMailingContact(models.Model):
    ...
    def default_get(self, fields_list):
        ...
        if "subscription_list_ids" in fields_list and not res.get(
    ...
    def _search_opt_out(self, operator, value):
        ...
        if (
            "default_list_ids" in self._context
            and isinstance(self._context["default_list_ids"], (list, tuple))
            and len(self._context["default_list_ids"]) == 1
        ):
            [active_list_id] = self._context["default_list_ids"]
            contacts = self.env["mailing.contact.subscription"].search(
    ...
    def _compute_opt_out(self):
        if (
            ...
            [active_list_id] = self._context["default_list_ids"]
            for record in self:
                active_subscription_list = record.subscription_list_ids.filtered(
    ...
    def create(self, vals_list):
        ...
        for vals in vals_list:
            if vals.get("list_ids") and vals.get("subscription_list_ids"):
                raise UserError(
                    _(
                        "You should give either list_ids, either subscription_list_ids to create new contacts."
                    )
                )
        ...
        return super(
            MassMailingContact, self.with_context(default_list_ids=False)
        ).create(vals_list)
