...
class CalendarEvent(models.Model):
    ...
    def default_get(self, fields):
        ...
        if "applicant_id" not in defaults:
            res_model = defaults.get("res_model", False) or self.env.context.get(
                "default_res_model"
            )
            res_model_id = defaults.get("res_model_id", False) or self.env.context.get(
                "default_res_model_id"
            )
            if (res_model and res_model == "hr.applicant") or (
                res_model_id
                and self.env["ir.model"].sudo().browse(res_model_id).model
                == "hr.applicant"
            ):
