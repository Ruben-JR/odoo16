...
class TestPoSBasicConfig(TestPoSCommon):
    ...
    def test_orders_with_invoiced(self):
        ...
        def _before_closing_cb():
            ...
            self.assertEqual(
                "invoiced",
                invoiced_order.state,
                msg="state should be 'invoiced' for invoiced orders.",
            )
    ...
    def test_return_order_invoiced(self):
        def _before_closing_cb():
            order = self.pos_session.order_ids.filtered(
                lambda order: "666-666-666" in order.pos_reference
            )
    ...
    def test_rounding_method(self):
        ...
        self.assertEqual(
            orders[0]["data"]["amount_return"], 0, msg="The amount return should be 0"
        )
        self.assertEqual(
            orders[1]["data"]["amount_return"], 0, msg="The amount return should be 0"
        )
    ...
    def test_invoice_past_order(self):
        ...
        with freeze_time(fields.Datetime.now() + relativedelta(days=2)):
            ...
            self.assertTrue(
                order_to_invoice.account_move.invoice_date
