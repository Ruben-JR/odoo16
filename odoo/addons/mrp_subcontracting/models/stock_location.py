...
class StockLocation(models.Model):
    ...
    def _check_subcontracting_location(self):
        for location in self:
            ...
            if location.is_subcontracting_location and (
                location.usage != "internal" or location.warehouse_id
            ):
                raise ValidationError(
                    _(
                        "In order to manage stock accurately, subcontracting locations must be type Internal, linked to the appropriate company and not specific to a warehouse."
                    )
                )
    ...
    def create(self, vals_list):
        ...
        new_subcontracting_locations = res.filtered(
            lambda l: l.is_subcontracting_location
        )
    ...
    def _activate_subcontracting_location_rules(self):
        ...
        for location in self:
            if (
                location.is_subcontracting_location
                and location != location.company_id.subcontracting_location_id
            ):
                locations_per_company.setdefault(location.company_id, []).extend(
                    location
                )
        new_rules_vals = []
        ...
        for company, locations in locations_per_company.items():
            ...
            if reference_location_id:
                reference_rules_from = self.env["stock.rule"].search(
                    [("location_src_id", "=", reference_location_id.id)]
                )
                reference_rules_to = self.env["stock.rule"].search(
                    [("location_dest_id", "=", reference_location_id.id)]
                )
                for location in locations:
                    existing_rules = {
                        ...
                        for rule in self.env["stock.rule"]
                        .with_context(active_test=False)
                        .search([("location_src_id", "=", location.id)])
                    }
                    for rule in reference_rules_from:
                        if (
                            ...
                            new_rules_vals.append(
                                rule.copy_data(
                                    {
                                        "location_src_id": location.id,
                                        "name": rule.name.replace(
                                            reference_location_id.name, location.name
                                        ),
                                    }
                                )[0]
                            )
                    ...
                    existing_rules = {
                        ...
                        for rule in self.env["stock.rule"]
                        .with_context(active_test=False)
                        .search([("location_dest_id", "=", location.id)])
                    }
    ...
    def _archive_subcontracting_location_rules(self):
        ...
        reference_rules = self.env["stock.rule"].search(
            [
                "|",
                ("location_src_id", "in", reference_location_ids.ids),
                ("location_dest_id", "in", reference_location_ids.ids),
            ]
        )
        ...
        rules_to_archive = self.env["stock.rule"].search(
            [
                "&",
                ("route_id", "in", reference_routes.ids),
                "|",
                ("location_src_id", "in", self.ids),
