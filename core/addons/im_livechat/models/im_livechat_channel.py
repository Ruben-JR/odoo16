...
class ImLivechatChannel(models.Model):
    ...
    image_128 = fields.Image(
        "Image", max_width=128, max_height=128, default=_default_image
    )
    ...
    def _compute_chatbot_script_count(self):
        ...
        mapped_data = {
            rule["channel_id"][0]: rule["chatbot_script_id"] for rule in data
        }
        for channel in self:
    ...
    def _compute_nbr_channel(self):
        data = self.env["mail.channel"]._read_group(
            [("livechat_channel_id", "in", self._ids), ("has_message", "=", True)],
            ["__count"],
            ["livechat_channel_id"],
            lazy=False,
        )
    ...
    def _get_livechat_mail_channel_vals(
        self,
        anonymous_name,
        operator=None,
        chatbot_script=None,
        ...
        operator_partner_id = (
            ...
            if operator
            else chatbot_script.operator_partner_id.id
        )
        members_to_add = [
            Command.create({"partner_id": operator_partner_id, "is_pinned": False})
        ]
        visitor_user = False
        if user_id:
            ...
            if (
                visitor_user
                and visitor_user.active
                and operator
                and visitor_user != operator
    ...
    def _open_livechat_mail_channel(
        ...
        if chatbot_script:
            if (
                chatbot_script.id
                not in self.env["im_livechat.channel.rule"]
                .search([("channel_id", "in", self.ids)])
                .mapped("chatbot_script_id")
                .ids
            ):
        ...
        if persisted:
            ...
            operator_partner_id = (
                user_operator.partner_id
                if user_operator
                else chatbot_script.operator_partner_id
            )
            display_name = (
                operator_partner_id.user_livechat_username
                or operator_partner_id.display_name
            )
            return {
    ...
    def get_livechat_info(self, username=None):
        ...
        info = {}
        info["available"] = (
            self.chatbot_script_count or len(self._get_available_users()) > 0
        )
...
class ImLivechatChannelRule(models.Model):
    ...
    chatbot_only_if_no_operator = fields.Boolean(
        string="Enabled only if no operator",
        help="Enable the bot only if there is no operator available",
    )
    channel_id = fields.Many2one(
        "im_livechat.channel", "Channel", help="The channel of the rule"
    )
    ...
    def match_rule(self, channel_id, url, country_id=False):
        ...
        if (
            country_id
        ):  # don't include the country in the research if geoIP is not installed
            domain = [
