...
class AccruedExpenseRevenue(models.TransientModel):
    ...
    def _get_default_company(self):
        if not self._context.get("active_model"):
            return
        orders = self.env[self._context["active_model"]].browse(
    ...
    def _compute_move_vals(self):
        ...
        orders = (
            self.env[self._context["active_model"]]
        ...
        for order in orders:
            if len(orders) == 1 and self.amount and order.order_line:
                ...
                if is_purchase:
                    account = (
                        order_line.product_id.property_account_expense_id
                        or order_line.product_id.categ_id.property_account_expense_categ_id
                    )
                else:
                    account = (
                        order_line.product_id.property_account_income_id
                        or order_line.product_id.categ_id.property_account_income_categ_id
                    )
                values = _get_aml_vals(
                    order, self.amount, 0, account.id, label=_("Manual entry")
                )
                move_lines.append(Command.create(values))
            else:
                other_currency = self.company_id.currency_id != order.currency_id
                rate = (
                    order.currency_id._get_rates(self.company_id, self.date).get(
                ...
                for order_line in lines:
                    if is_purchase:
                        ...
                        amount = self.company_id.currency_id.round(
                            order_line.qty_to_invoice * order_line.price_unit / rate
                        )
                    ...
                    values = _get_aml_vals(
                        order, amount, amount_currency, account.id, label=label
                    )
                    move_lines.append(Command.create(values))
        ...
        move_vals = {
            "ref": _(
                "Accrued %s entry as of %s", move_type, format_date(self.env, self.date)
            ),
